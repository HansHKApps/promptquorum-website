...
const POST_UI: Record<string, Record<string, string>> = {
  ...
  backLink: {
    en: '← Back to Smart Home',
    de: '← Zurück zu Smart Home',
    fr: '← Retour à Smart Home',
    ja: '← Smart Home に戻る',
    zh: '← 返回 Smart Home',
    es: '← Volver a Smart Home',
    pt: '← Voltar para Smart Home',
    ar: '← العودة إلى Smart Home',
    ko: '← Smart Home으로 돌아가기',
  },
  breadcrumbHome: {
    en: 'Home',
    de: 'Startseite',
...
// (HUB_THEMES in src/lib/smart-home/page-helpers.tsx) so the article
// badge matches the hub category badge for that cluster.
const THEME_COLORS: Record<string, { dot: string; badge: string }> = {
  // The four canonical Smart Home cluster themes — keys MUST stay in sync with
  // CANONICAL_THEMES in the Smart Home prebuild validator and with the hub badges
  // (HUB_THEMES in src/lib/smart-home/page-helpers.tsx).
  'Smart Home Foundations':            { dot: 'bg-slate-400',   badge: 'bg-slate-50 text-slate-700 border border-slate-200' },
  ...
  '2026 Protocols & Regulation':       { dot: 'bg-rose-400',    badge: 'bg-rose-50 text-rose-700 border border-rose-200' },
...
function SectionBlock({ section, colors, id, lang, renderLinks }: { section: LLMSection; colors: { dot: string; badge: string }; id?: string; lang: Language; renderLinks: (text: string) => React.ReactNode }) {
  ...
  return (
    ...
    >
      ...
      {/* Item-heading list — each row becomes its own card instead of a wide table column */}
                ...
                <dl className="text-sm text-text-secondary space-y-1.5">
                  {restCols.map((col) => (
                    <div key={col} className="flex flex-col sm:flex-row sm:gap-2">
                      <dt className="font-semibold text-text-primary shrink-0 sm:w-40">{col}:</dt>
      ...
      {/* Table */}
      {!section.itemHeadings && section.rows && section.columns && (
        <div className="relative overflow-x-auto my-6 -mx-4 px-4 sm:mx-0 sm:px-0" ref={tableScrollRef}>
...
function SmartHomePostContent({ slug, lang, articleData, availableLangs }: Props) {
  ...
  return (
    <div className="min-h-screen bg-white pt-32 pb-20 px-4 sm:px-6" key={`${slug}-${lang}`}>
      <div className="max-w-3xl mx-auto">
        ...
        <ClusterBanner cluster="smart-home" />
        ...
        <LangLinksBar cluster="smart-home" slug={slug} availableLangs={availableLangs ?? Object.keys(articleData)} initialLang={lang} />
...
export function SmartHomePostClient(props: Props) {
  return <SmartHomePostContent {...props} />
}
