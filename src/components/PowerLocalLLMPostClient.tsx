...
// Power Local LLM article renderer.
...
const POST_UI: Record<string, Record<string, string>> = {
  ...
  backLink: {
    ...
    de: '← Zurück zu Lokale LLMs Pro',
    fr: '← Retour aux LLM locaux avancés',
    ja: '← ローカルLLM活用 に戻る',
    zh: '← 返回 本地LLM进阶',
...
// (HUB_THEMES in src/lib/power-local-llm/page-helpers.tsx) so the article
  ...
  return (
    ...
    >
      ...
      {/* "From the Maker" — a founder/maintainer-supplied quote, given a distinct
          ...
          editorial. Detected by section id so it applies to every FeatureAppPost that
          has this section, not just one article. */}
    ...
    <div className="min-h-screen bg-white pt-32 pb-20 px-4 sm:px-6" key={`${slug}-${lang}`}>
      <div className="max-w-3xl mx-auto">
        ...
        <p className="text-center mt-16 pt-8 border-t border-primary/20">
          <a href={powerLLMHubPath(lang as any)} className="text-primary hover:text-primary/80 text-sm">
...
export function PowerLocalLLMPostClient(props: Props) {
  return <PowerLocalLLMPostContent {...props} />
}
