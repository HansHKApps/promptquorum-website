...
const HUB_HERO_DESC: Record<string, string> = {
  en: 'Local LLMs are large language models that run entirely on your own computer — no internet connection, no API fees, and no data leaving your machine. Compared with the OpenAI API you trade a little quality for privacy and zero running cost. If you are just starting, install Ollama and run Llama 3.2 3B or Qwen3 4B on 8 GB of RAM in under 10 minutes; the tier list below is a ranking of the best models, GPUs, and tools by VRAM — 4GB, 12GB, 24GB, 32GB and 48GB — for every budget.',
  de: 'Lokale LLMs — international meist als Local LLMs bezeichnet — sind große Sprachmodelle, die vollständig auf Ihrem eigenen Computer laufen: ohne Internetverbindung, ohne API-Gebühren und ohne dass Daten Ihr Gerät verlassen. Als lokale KI-Modelle auf eigener Hardware ersetzen sie ChatGPT für viele Aufgaben. Um ein LLM lokal zu betreiben, installieren Sie Ollama und führen Llama 3.2 3B oder Qwen3 4B mit 8 GB RAM in unter 10 Minuten aus; die folgenden Leitfäden bewerten die besten Modelle, GPUs und Tools für jedes Budget.',
