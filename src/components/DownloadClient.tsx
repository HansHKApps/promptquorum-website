...
// TODO(Hans): replace these placeholders with the real hosted installer URLs
// (Vercel static file, GitHub Release asset, or external file host). Do NOT
// ship real download buttons until these point at the actual signed builds.
    ...
    <div className="flex flex-col rounded-lg border border-primary/15 bg-surface-elevated p-6 shadow-sm">
      ...
      <div className="mt-auto pt-5">
        {isPlaceholder ? (
          <div>
            <span
              className="inline-flex cursor-not-allowed items-center rounded-md bg-secondary px-4 py-2.5 text-sm font-semibold text-secondary-foreground opacity-70"
              aria-disabled="true"
            >
              {download}
            </span>
            <p className="mt-2 text-xs text-text-muted">
              {pending} — <span className="font-mono">{href}</span>
            </p>
          </div>
        ) : (
          <a
  ...
  return (
    <div className="min-h-screen bg-white pt-32 pb-20 px-4 sm:px-6" dir={dir}>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mt-12 rounded-lg border border-tertiary/25 bg-surface p-6">
