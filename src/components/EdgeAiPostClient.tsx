...
const THEME_COLORS: Record<string, { dot: string; badge: string }> = {
  ...
  'Edge Silicon':          { dot: 'bg-sky-400',       badge: 'bg-sky-50 text-sky-700 border border-sky-200' },
  'Robots & Machines':     { dot: 'bg-rose-400',      badge: 'bg-rose-50 text-rose-700 border border-rose-200' },
  'Cameras & Drones':      { dot: 'bg-indigo-400',    badge: 'bg-indigo-50 text-indigo-700 border border-indigo-200' },
}
...
function renderMarkdownTable(lines: string[], lang: Language, renderLinks: (text: string) => React.ReactNode): React.JSX.Element {
...
function SectionBlock({ section, colors, id, lang, renderLinks }: { section: LLMSection; colors: { dot: string; badge: string }; id?: string; lang: Language; renderLinks: (text: string) => React.ReactNode }) {
  ...
  return (
    <div className="mt-8" id={id}>
      ...
      {section.component === 'EdgeHardwareSelector' && <EdgeHardwareSelector />}
      {section.component === 'RobotInferenceBudgetCalculator' && <RobotInferenceBudgetCalculator />}
      {section.component === 'EdgeVideoBandwidthCalculator' && <EdgeVideoBandwidthCalculator />}
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-2 text-sm text-text-secondary flex-wrap">
            ...
            <span>/</span>
            <a href={lang === 'en' ? '/edge-ai' : `/${lang}/edge-ai`} className="hover:text-primary">{POST_UI.breadcrumbHub[lang] ?? 'Edge AI'}</a>
            <span>/</span>
        ...
        <LangLinksBar cluster="edge-ai" slug={slug} availableLangs={availableLangs ?? Object.keys(articleData)} initialLang={lang} />
        ...
        <p className="text-center mt-8">
          <a href={lang === 'en' ? '/edge-ai' : `/${lang}/edge-ai`} className="text-primary hover:text-primary/80 text-sm">
...
export function EdgeAiPostClient(props: Props) {
