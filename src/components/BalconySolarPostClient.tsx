...
// Balcony Solar article renderer.
// Forked from LocalLLMsPostClient for the path-based-locale cluster.
// Differences:
...
const POST_UI: Record<string, Record<string, string>> = {
  ...
  ctaText: {
    ...
    ko: 'PromptQuorum으로 로컬 LLM을 25개 이상의 클라우드 모델과 동시에 비교하십시오.',
  },
  ctaButton: {
  ...
  backLink: {
    en: '← Back to Balcony Solar',
    de: '← Zurück zu Balcony Solar',
    fr: '← Retour à Balcony Solar',
    ja: '← Balcony Solar に戻る',
    zh: '← 返回 Balcony Solar',
    es: '← Volver a Balcony Solar',
    pt: '← Voltar para Balcony Solar',
    ar: '← العودة إلى Balcony Solar',
    ko: '← Balcony Solar으로 돌아가기',
  },
  breadcrumbHome: {
    en: 'Home',
    de: 'Startseite',
...
// Keys must match each article's `theme` string. NOTE: these are the literal `theme`
// values used by authored articles in src/lib/balcony-solar/articles/ (e.g.
// 'Balcony Solar Foundations', 'Balcony Solar Money Pages'), which is a SEPARATE naming
// scheme from the hub's category badges (HUB_THEMES in page-helpers.tsx, driven by
// BALCONY_SOLAR_CATEGORIES slug membership in categories.ts) — the two do not need to
// match 1:1, but colors are aligned by tier for visual consistency. Any theme value not
// listed here safely falls back to FALLBACK_THEME_COLOR below instead of throwing.
const THEME_COLORS: Record<string, { dot: string; badge: string }> = {
  'Balcony Solar Foundations':  { dot: 'bg-slate-400',   badge: 'bg-slate-50 text-slate-700 border border-slate-200' },
  'Balcony Solar Money Pages':  { dot: 'bg-emerald-400', badge: 'bg-emerald-50 text-emerald-700 border border-emerald-200' },
  'Balcony Solar Legislation':  { dot: 'bg-violet-400',  badge: 'bg-violet-50 text-violet-700 border border-violet-200' },
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-2 text-sm text-text-secondary flex-wrap">
            ...
            <span>/</span>
            <a href={balconySolarHubPath(lang as any)} className="hover:text-primary">{POST_UI.breadcrumbHub[lang] ?? 'Balcony Solar'}</a>
            <span>/</span>
        ...
        <ClusterBanner cluster="balcony-solar" />
        ...
        {/* Marketing CTA suppressed cluster-wide until /balcony-solar launches publicly. */}

        <p className="text-center mt-16 pt-8 border-t border-primary/20">
          <a href={balconySolarHubPath(lang as any)} className="text-primary hover:text-primary/80 text-sm">
...
export function BalconySolarPostClient(props: Props) {
  return <BalconySolarPostContent {...props} />
}
