...
const stagesData: StagesData = {
  ...
  ko: [
    {
      title: '프롬프트',
      what: '8가지 이상의 검증된 프레임워크(CRAFT, CO-STAR, SPECS, RISEN, APE, TRACE, RTF, Google)를 사용하여 초안 아이디어 또는 구조화된 프롬프트를 입력하세요.',
      whyCool: [
        '완벽하지 않아도 됩니다 — 대략적인 아이디어도 잘 작동합니다',
        '프레임워크를 선택해 생각을 자동으로 구조화할 수 있습니다',
        '한 문장 또는 완전히 구조화된 프롬프트로 시작할 수 있습니다',
        '충분한 컨텍스트를 제공했는지 즉시 피드백을 받을 수 있습니다'
      ],
      userBenefit: [
        '⚡ 낮은 진입 장벽(프롬프트 엔지니어링 경험 불필요)',
        '📋 프레임워크 가이드가 모든 중요한 세부 사항을 놓치지 않도록 도와줍니다',
        '✓ 프롬프트 완성도를 즉시 확인할 수 있습니다',
        '💡 어떤 프레임워크가 내 사용 사례에 가장 적합한지 배울 수 있습니다'
      ]
    },
    {
      title: '최적화',
      what: 'AI가 프롬프트를 분석하고 고급 최적화 기법을 사용해 정밀한 버전으로 변환합니다. 컨텍스트를 확장하고 의도를 명확히 합니다.',
      whyCool: [
        '수동 재작성 없이 자동으로 개선됩니다',
        '무엇이 변경되었고 왜 변경되었는지 정확히 보여줍니다',
        '다양한 개선 옵션 제공: 간결하게, 상세하게, 단순화, 구체화 등',
        '품질 점수로 개선 비율을 확인할 수 있습니다(0-100%)'
      ],
      userBenefit: [
        '⏱️ 프롬프트 당 30-60분 절약(수동 재작성 대비)',
        '📚 프롬프트가 왜 더 잘 작동하는지 배울 수 있습니다(학습 모드)',
        '📈 품질 점수와 개선 이력을 추적할 수 있습니다',
        '🔄 처음부터 다시 시작하지 않고 원클릭으로 개선 가능합니다'
      ]
    },
    {
      title: '디스패치',
      what: '최적화된 프롬프트를 한 번의 클릭으로 ChatGPT, Claude, Gemini 및 25개 이상의 AI 모델에 동시에 전송하세요.',
      whyCool: [
        '원클릭으로 여러 AI에 전송(탭 전환 불필요)',
        '다양한 관점 확보 — 모델마다 강점이 다릅니다',
        '내 특정 작업에 가장 적합한 AI를 발견할 수 있습니다',
        '모든 모델의 결과를 나란히 비교할 수 있습니다'
      ],
      userBenefit: [
        '🚀 ChatGPT, Claude, Gemini 탭 간 전환이 필요 없습니다',
        '🔍 숨겨진 편향을 발견하세요(단일 모델 결과는 오해를 불러올 수 있습니다)',
        '⚡ 한 번에 3-5개의 응답을 받아 작업 속도를 높입니다',
        '🎯 데이터 기반 결정: 어떤 모델이 가장 좋은 성능을 내는지 확인하세요'
      ]
    },
    {
      title: '쿼럼',
      what: '모든 응답을 나란히 비교하세요. AI가 응답을 분석해 합의점, 모순, 신뢰도를 찾고 최적의 답변을 자동으로 선별합니다.',
      whyCool: [
        '환각 현상 감소(세 모델 모두 동의하면 사실일 가능성이 높습니다)',
        '가중 투표 시스템이 자동으로 최적의 답변을 찾습니다',
        '모순 감지 기능이 검토가 필요한 불일치를 표시합니다',
        '합의 추출: 각 응답의 최선을 종합합니다'
      ],
      userBenefit: [
        '🛡️ 결과에 대한 신뢰도가 5-10배 향상됩니다',
        '❌ 단일 모델이 놓칠 수 있는 잘못된 정보를 잡아냅니다',
        '⏱️ 모든 응답을 직접 읽는 시간을 절약합니다',
        '✨ 자동으로 종합된 "최적의 답변"을 받습니다'
      ]
    }
  ],
  zh: [
...
const descriptionText: Partial<Record<Language, Record<number, string>>> = {
  ...
  ko: {
    0: '어떤 아이디어든 시작하세요, 초안도 괜찮습니다',
    1: 'AI가 정밀한 프롬프트로 변환합니다',
    2: '여러 AI에 동시에 전송합니다',
    3: '결과를 비교하고 최적의 답변을 찾으세요'
  }
}
