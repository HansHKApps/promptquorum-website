...
// Builds the full, all-locale search index. Single source of truth for both
// src/app/api/search-index/route.ts (unscoped) and
// src/app/api/search-index/[lang]/route.ts (locale-scoped) — neither route
// file keeps its own copy of this logic, so a newly added content cluster
// only needs to be wired in here once for both routes to pick it up.
//
// scripts/validate-search-index-clusters.mjs greps THIS file (not route.ts)
// for `@/lib/<name>/articles-barrel` imports and `hub: '<name>'` literals to
// guard against a cluster going unwired — keep both present verbatim per
// cluster block below.
