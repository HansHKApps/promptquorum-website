...
export const article: Partial<Record<Language, LLMArticle>> = {
    en: {
      ...
      seoTitle: 'Official LM Studio & Ollama OpenAI API Docs (2026)',
      ...
      metaDescription: 'Official OpenAI-compatible API docs for LM Studio (localhost:1234) & Ollama (localhost:11434): endpoints, Python, Node.js, streaming, Aider/Cline setup.',
      ...
      sections: {
        ...
        quickFacts: {
          ...
          content: [
            ...
            '**Code change:** 2 lines — `base_url` and `api_key`. Most other code stays the same.',
        ...
        whatIsOpenAI: {
          ...
          content: [
            ...
            'Because the format is shared, you do not need to learn a new API or rewrite most of your code.',
            '---',
          ...
          image: '/images/local-llm-openai-compatible-api-one-line-change-hero-en.webp',
          imageCaption: 'Switching from OpenAI to Ollama requires changing 2 lines -- base_url and api_key -- most other code stays the same.',
        },
        ollamaEndpoint: {
          ...
          image: '/images/local-llm-openai-compatible-api-api-request-flow-hero-en.webp',
          imageCaption: 'Ollama intercepts the OpenAI-formatted request and runs inference locally -- the response returns in the same OpenAI-compatible format, no internet required.',
        },
        pythonOpenAI: {
        ...
        functionCalling: {
          ...
          content: [
            ...
            'Function calling support depends on the model. Llama 4 Scout, Qwen3 8B, Gemma 4 26B-A4B, and Mistral Small 3.1 all support tool calling reliably. Llama 3.1 8B also supports it (legacy). Smaller models (3B) may not reliably produce structured tool call JSON.',
          ...
          imageCaption: 'Function calling flow with Ollama: the local model returns tool_call JSON, and your app executes the function -- supported by Llama 4 Scout, Qwen3 8B, Gemma 4 26B-A4B, and Mistral.',
        },
        aiCodingTools: {
          id: 'ai-coding-tools',
        ...
        commonMistakes: {
          ...
          imageCaption: 'Ollama (port 11434), vLLM (port 8000), and LM Studio (port 1234) all expose OpenAI-compatible endpoints -- comparable client code, different ports and use cases.',
        },
        faqSection: {
          id: 'common-questions',
          ...
          faqs: [
            ...
            },
            {
              ...
              a: 'Yes, for models that support it (Llama 4 Scout, Qwen3 8B, Gemma 4 26B-A4B, Mistral Small 3.1). Legacy models (Llama 3.1 8B) also supported. Pass tools=[] in the API call as you would with OpenAI. Ollama parses tool calls and returns structured JSON. Not all models support this -- check model documentation.',
            },
            {
...
schema: {
      ...
      faqSchema: {
        ...
        'mainEntity': [
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Yes, for models that support it (Llama 3.1 8B, Qwen3 8B, Mistral). Pass tools=[] in the API call as you would with OpenAI. Ollama parses tool calls and returns structured JSON. Not all models support this -- check model documentation.'
            }
          },
          {
            '@type': 'Question',
      ...
      howToSchema: {
        ...
        'step': [
          ...
          },
          {
            ...
            'text': 'Use client.chat.completions.create(model="model-name", messages=[...]) -- the same call shape as the OpenAI API. The model name must match what is loaded in LM Studio or installed in Ollama.',
          },
        ],
      },
      itemListSchema: {
        ...
        'itemListElement': [
          ...
          { '@type': 'ListItem', 'position': 4, 'name': 'Chat Completions endpoint', 'description': 'POST /v1/chat/completions -- the same endpoint path across LM Studio, Ollama, and vLLM; comparable client code works with all three' },
        ],
      },
    ...
    de: {
      ...
      seoTitle: 'Offizielle LM Studio & Ollama OpenAI API-Doku (2026)',
      ...
      metaDescription: 'Offizielle OpenAI-kompatible API-Doku für LM Studio (localhost:1234) & Ollama (localhost:11434): Endpunkte, Python, Node.js, Streaming, Aider/Cline-Setup.',
      ...
      sections: {
        tldr: {
          ...
          items: [
            ...
            'Die OpenAI-kompatible API nutzt über Ollama, vLLM und LM Studio dasselbe Basisformat -- kein Code-Wechsel nötig zum Provideraustausch.',
        ...
        quickFacts: {
          ...
          content: [
            ...
            '**Code-Änderung:** 2 Zeilen — `base_url` und `api_key`. Der Rest des Codes bleibt größtenteils gleich.',
            ...
            '**Authentifizierung:** startet meist ohne Auth, standardmäßig an Localhost gebunden. Fügen Sie Reverse Proxy und Authentifizierung hinzu, wenn Sie über Localhost hinaus exponieren, da vLLM API-Key-Auth unterstützt und LM Studio öffentlich zugänglich gemacht werden kann.',
        ...
        whatIsOpenAI: {
          ...
          content: [
            ...
            '```\n{\n  "choices": [{\"message\": {\"content\": \"...\"}}],\n  "usage\": {\"prompt_tokens\": 10, \"completion_tokens\": 20}\n}\n```',
            'Da das Format gemeinsam genutzt wird, müssen Sie keine neue API lernen oder den Großteil Ihres Codes umschreiben.',
            '---',
          ...
          image: '/images/local-llm-openai-compatible-api-one-line-change-hero-de.webp',
          imageCaption: 'Wechsel von OpenAI zu Ollama erfordert 2 Zeilen ändern -- base_url und api_key -- der Rest des Codes bleibt größtenteils gleich.',
        },
        ollamaEndpoint: {
          id: 'ollama-api-endpoint',
          ...
          image: '/images/local-llm-openai-compatible-api-api-request-flow-hero-de.webp',
          imageCaption: 'Ollama fängt OpenAI-formatierten Request ab und führt Inferenz lokal aus -- Response wird im selben OpenAI-kompatiblen Format zurückgegeben, kein Internet erforderlich.',
        },
        pythonOpenAI: {
          id: 'python-openai-library',
        ...
        functionCalling: {
          ...
          content: [
            ...
            'Function Calling Support hängt vom Modell ab. Llama 4 Scout, Qwen3 8B, Gemma 4 26B-A4B, und Mistral Small 3.1 unterstützen Tool Calling zuverlässig. Llama 3.1 8B und Qwen3 8B unterstützen es auch (legacy). Kleinere Modelle (3B) erzeugen möglicherweise nicht zuverlässig strukturiertes Tool Call JSON.',
          ...
          codeLanguage: 'python',
          image: '/images/openai-compatible-function-calling-en.svg',
          imageCaption: 'Function Calling Flow mit Ollama: lokales Modell gibt tool_call JSON zurück, und Ihre App führt Funktion aus -- unterstützt von Llama 4 Scout, Qwen3 8B, Gemma 4 26B-A4B, und Mistral.',
        },
        aiCodingTools: {
          id: 'ai-coding-tools',
        ...
        commonMistakes: {
          ...
          image: '/images/openai-compatible-platform-comparison-en.svg',
          imageCaption: 'Ollama (Port 11434), vLLM (Port 8000), und LM Studio (Port 1234) stellen alle OpenAI-kompatible Endpoints bereit -- vergleichbarer Client Code, unterschiedliche Ports und Use Cases.',
        },
        faqSection: {
          id: 'common-questions',
          ...
          faqs: [
            ...
            },
            {
              ...
              a: 'Ja, für Modelle die es unterstützen (Llama 4 Scout, Qwen3 8B, Gemma 4 26B-A4B, Mistral Small 3.1). Legacy Modelle (Llama 3.1 8B) auch unterstützt. Übergeben Sie tools=[] im API Aufruf wie Sie es mit OpenAI täten. Ollama parst Tool Calls und gibt strukturiertes JSON zurück. Nicht alle Modelle unterstützen das -- schau Modell Dokumentation.',
            },
            {
      ...
      howToSchema: {
        ...
        'step': [
          ...
          },
          {
            ...
            'text': 'Nutzen Sie client.chat.completions.create(model="model-name", messages=[...]) -- dieselbe Aufrufstruktur wie beim OpenAI API Aufruf. Der Modellname muss dem in LM Studio geladenen bzw. in Ollama installierten Modell entsprechen.',
          },
        ],
      },
      itemListSchema: {
        ...
        'itemListElement': [
          ...
          { '@type': 'ListItem', 'position': 4, 'name': 'Chat Completions Endpoint', 'description': 'POST /v1/chat/completions -- derselbe Endpoint-Pfad bei LM Studio, Ollama und vLLM; vergleichbarer Client-Code funktioniert mit allen drei' },
        ],
      },
      gammaEmbedUrl: '/presentations/local-llm-openai-compatible-api-static.html',
    ...
    fr: {
      ...
      seoTitle: 'LM Studio & Ollama : API OpenAI officielle (2026)',
      ...
      metaDescription: "Documentation officielle de l'API compatible OpenAI pour LM Studio (localhost:1234) et Ollama (localhost:11434) : endpoints, Python, Node.js, Aider/Cline.",
      ...
      sections: {
        ...
        whatIsOpenAI: {
          ...
          content: [
            ...
            '```\n{\n  "choices": [{\"message\": {\"content\": \"...\"}}],\n  "usage\": {\"prompt_tokens\": 10, \"completion_tokens\": 20}\n}\n```',
            'Le format étant partagé, vous n\'avez pas besoin d\'apprendre une nouvelle API ni de réécrire l\'essentiel de votre code.',
          ],
          image: '/images/local-llm-openai-compatible-api-one-line-change-hero-fr.webp',
          imageCaption: 'Passer d\'OpenAI à Ollama nécessite de changer 2 lignes -- base_url et api_key -- le reste du code reste globalement le même.',
        },
        ollamaEndpoint: {
          id: 'ollama-api-endpoint',
          ...
          image: '/images/local-llm-openai-compatible-api-api-request-flow-hero-fr.webp',
          imageCaption: 'Ollama intercepte la requête au format OpenAI et exécute l\'inférence en local -- la réponse revient au même format compatible OpenAI, sans connexion internet.',
        },
        pythonOpenAI: {
          id: 'python-openai-library',
        ...
        functionCalling: {
          ...
          content: [
            ...
            'La prise en charge des appels de fonctions dépend du modèle. Llama 3.1 8B, Qwen3 8B et la plupart des modèles récents la supportent. Les modèles plus petits (3B) peuvent ne pas l\'utiliser de manière fiable.',
        ...
        regionalContext: {
          ...
          content: [
            ...
            '**Japon / APPI :** En vertu de la loi japonaise sur la protection des informations personnelles (APPI), l\'inférence sur site contourne les exigences de transfert de données cloud. Ollama + Qwen3 8B fonctionne sur des ordinateurs portables d\'entreprise standard (8 Go de RAM) avec une latence de 30 à 50 tok/sec.',
        ...
        commonMistakes: {
          ...
          items: [
            ...
            '**Supposer qu\'il n\'y a jamais d\'authentification.** Ollama démarre généralement sans authentification, lié à localhost par défaut. Ajoutez un reverse proxy et une authentification si vous exposez l\'API au-delà de localhost -- vLLM prend en charge l\'authentification par clé API et LM Studio peut être exposé publiquement.',
            '---',
            '⚠️ **Avertissement :** l\'API d\'Ollama n\'a AUCUNE authentification par défaut. Si vous l\'exposez sur votre réseau (`OLLAMA_HOST=0.0.0.0`), toute personne sur ce réseau peut envoyer des requêtes, charger des modèles et consommer des ressources GPU. Pour un usage multi-utilisateurs ou en production, placez un reverse proxy (nginx, Caddy) avec authentification devant Ollama -- n\'exposez jamais le port 11434 directement sur internet.',
          ],
          image: '/images/openai-compatible-platform-comparison-en.svg',
          imageCaption: 'Ollama (port 11434), vLLM (port 8000) et LM Studio (port 1234) exposent tous des endpoints compatibles OpenAI -- code client comparable, ports et cas d\'usage différents.',
        },
        faqSection: {
          id: 'common-questions',
          ...
          faqs: [
            {
              ...
              a: 'Non. Définissez `base_url="http://localhost:11434/v1"` et `api_key="ollama"`. Tout le reste reste globalement le même. Si vous avez du code utilisant la bibliothèque OpenAI, échangez ces deux lignes et il fonctionne avec votre modèle local.',
            },
            {
            ...
            },
            {
              ...
              a: 'Oui, pour les modèles qui le supportent (Llama 3.1 8B, Qwen3 8B, Mistral). Passez tools=[] dans l\'appel API comme vous le feriez avec OpenAI. Ollama analyse les appels d\'outils et retourne du JSON structuré.',
            },
            {
...
schema: {
      ...
      faqSchema: {
        ...
        '@type': 'FAQPage',
        'mainEntity': [
          { '@type': 'Question', 'name': 'Dois-je modifier mon code OpenAI pour utiliser Ollama ?', 'acceptedAnswer': { '@type': 'Answer', 'text': 'Non. Définissez base_url="http://localhost:11434/v1" et api_key="ollama". Tout le reste reste globalement le même.' } },
          ...
          { '@type': 'Question', 'name': 'Ollama supporte-t-il les appels de fonctions ?', 'acceptedAnswer': { '@type': 'Answer', 'text': 'Oui, pour Llama 3.1 8B, Qwen3 8B, Mistral. Passez tools=[] comme avec OpenAI.' } },
      ...
      howToSchema: {
        ...
        'step': [
          ...
          { '@type': 'HowToStep', 'name': 'Appeler chat.completions.create', 'text': 'Utilisez client.chat.completions.create(model="nom-modele", messages=[...]) -- la même structure d\'appel que l\'API OpenAI. Le nom du modèle doit correspondre à celui chargé dans LM Studio ou installé dans Ollama.' },
        ],
      },
      itemListSchema: {
        '@context': 'https://schema.org',
        ...
        'itemListElement': [
          ...
          { '@type': 'ListItem', 'position': 4, 'name': 'Endpoint Chat Completions', 'description': 'POST /v1/chat/completions -- même chemin d\'endpoint sur LM Studio, Ollama et vLLM ; le même code client fonctionne avec les trois' },
        ],
      },
      gammaEmbedUrl: '/presentations/local-llm-openai-compatible-api-static.html',
    ...
    ja: {
      ...
      seoTitle: 'LM Studio & Ollama公式OpenAI APIドキュメント(2026)',
      ...
      metaDescription: 'LM Studio（localhost:1234）とOllama（localhost:11434）の公式OpenAI互換APIドキュメント。エンドポイント、Python、Node.js、Aider/Clineの設定方法。',
      ...
      sections: {
        ...
        whatIsOpenAI: {
          ...
          content: [
            ...
            '```\n{\n  "choices": [{\"message\": {\"content\": \"...\"}}],\n  "usage\": {\"prompt_tokens\": 10, \"completion_tokens\": 20}\n}\n```',
            '形式が共有されているため、新しいAPIを学んだりコードの大部分を書き直したりする必要はありません。',
          ],
          image: '/images/local-llm-openai-compatible-api-one-line-change-hero-ja.webp',
        ...
        ollamaEndpoint: {
          ...
          rows: [
            { 'エンドポイント': 'Chat Completions', 'URL': 'POST http://localhost:11434/v1/chat/completions', '説明': 'OpenAIの`/chat/completions`に対応' },
            { 'エンドポイント': 'Text Completions', 'URL': 'POST http://localhost:11434/v1/completions', '説明': 'OpenAIの`/completions`に対応' },
          ...
          image: '/images/local-llm-openai-compatible-api-api-request-flow-hero-ja.webp',
        ...
        functionCalling: {
          ...
          codeLanguage: 'python',
          image: '/images/openai-compatible-function-calling-en.svg',
          imageCaption: 'OllamaでのFunction Callingフロー：ローカルモデルがtool_call JSONを返し、アプリが関数を実行 -- Llama 3.1 8B・Qwen3 8B・Mistralで対応。',
        },
        aiCodingTools: {
          id: 'ai-coding-tools',
        ...
        regionalContext: {
          ...
          content: [
            '**日本（METI・APPI）：** 経済産業省（METI）は国産AIソブリンティを推進しています。個人情報保護法（APPI）に基づき、オンプレミスモデル推論はクラウドデータ転送要件を回避します。OllamaとLM StudioはMETI AI Governance 2024フレームワークに準拠した企業展開に適しています。Ollama + Qwen3 8Bは標準的な企業ノートPC（8GB RAM）で30〜50 tok/secで動作し、日本語処理のリアルタイム要件を満たします。',
        ...
        commonMistakes: {
          ...
          items: [
            ...
            '**認証が常にないと思い込む。** Ollamaは通常、認証なしでlocalhostにバインドされた状態で起動します。localhostを超えて公開する場合はリバースプロキシと認証を追加してください。vLLMはAPIキー認証に対応しており、LM Studioは公開設定も可能です。',
            '---',
            '⚠️ **警告：** OllamaのAPIはデフォルトで認証機能がありません。ネットワークに公開すると（`OLLAMA_HOST=0.0.0.0`）、そのネットワーク上の誰でもリクエストを送信し、モデルを読み込み、GPUリソースを消費できてしまいます。マルチユーザーや本番環境では、Ollamaの前に認証付きのリバースプロキシ（nginx、Caddyなど）を配置してください -- ポート11434を直接インターネットに公開しないでください。',
          ],
          image: '/images/openai-compatible-platform-comparison-en.svg',
        ...
        faqSection: {
          ...
          faqs: [
            ...
            { q: 'OllamaはAPIを通じたFunction Callingに対応していますか？', a: 'はい、対応モデル（Llama 3.1 8B・Qwen3 8B・Mistral）で動作します。OpenAIと同様にtools=[]を渡すだけです。' },
...
schema: {
      ...
      faqSchema: {
        ...
        'mainEntity': [
          ...
          { '@type': 'Question', 'name': 'Function Callingに対応していますか？', 'acceptedAnswer': { '@type': 'Answer', 'text': 'はい、Llama 3.1 8B・Qwen3 8B・MistralでOpenAI APIと同様にtools=[]を渡すだけです。' } },
      ...
      howToSchema: {
        ...
        'step': [
          ...
          { '@type': 'HowToStep', 'name': 'chat.completions.createを呼び出す', 'text': 'client.chat.completions.create(model="モデル名", messages=[...])を使用 -- OpenAI APIと同じ呼び出し構造です。モデル名はLM Studioに表示されているものまたはollamaにインストールされているものを使用してください。' },
        ],
      },
      itemListSchema: {
        '@context': 'https://schema.org',
        ...
        'itemListElement': [
          ...
          { '@type': 'ListItem', 'position': 4, 'name': 'Chat Completionsエンドポイント', 'description': 'POST /v1/chat/completions -- LM Studio・Ollama・vLLMで同じエンドポイントパス；同じクライアントコードですべてに対応' },
        ],
      },
      gammaEmbedUrl: '/presentations/local-llm-openai-compatible-api-static.html',
    ...
    zh: {
      ...
      seoTitle: 'LM Studio & Ollama官方OpenAI API文档(2026)',
      ...
      metaDescription: 'LM Studio（localhost:1234）与Ollama（localhost:11434）的官方OpenAI兼容API文档：端点、Python、Node.js、Aider/Cline配置方法。',
      ...
      sections: {
        ...
        whatIsOpenAI: {
          ...
          content: [
            ...
            '```\nPOST /chat/completions\n{\n  "model": "gpt-4o",\n  "messages": [...],\n  "temperature": 0.7\n}\n```',
            'Ollama的API在`localhost:11434/v1/chat/completions`接受相同格式的请求，并以OpenAI格式返回响应：',
            '```\n{\n  "choices": [{\"message\": {\"content\": \"...\"}}],\n  "usage\": {\"prompt_tokens\": 10, \"completion_tokens\": 20}\n}\n```',
            '由于格式共享，您无需学习新API或重写大部分代码。',
          ],
          image: '/images/local-llm-openai-compatible-api-one-line-change-hero-zh.webp',
        ...
        ollamaEndpoint: {
          ...
          rows: [
            { '端点类型': 'Chat Completions', 'URL': 'POST http://localhost:11434/v1/chat/completions', '说明': '对应OpenAI的`/chat/completions`' },
            { '端点类型': 'Text Completions', 'URL': 'POST http://localhost:11434/v1/completions', '说明': '对应OpenAI的`/completions`' },
          ...
          columns: ['端点类型', 'URL', '说明'],
          image: '/images/local-llm-openai-compatible-api-api-request-flow-hero-zh.webp',
        ...
        functionCalling: {
          ...
          codeLanguage: 'python',
          image: '/images/openai-compatible-function-calling-en.svg',
          imageCaption: 'Ollama函数调用流程：本地模型返回tool_call JSON，应用执行函数——Llama 3.1 8B、Qwen3 8B和Mistral支持此功能。',
        },
        aiCodingTools: {
          id: 'ai-coding-tools',
        ...
        commonMistakes: {
          ...
          items: [
            ...
            '**认为完全不存在身份验证。** Ollama通常在无身份验证的情况下启动，默认绑定到localhost。如果要将其暴露到localhost之外，请添加反向代理和身份验证——vLLM支持API密钥认证，LM Studio也可以配置为公开访问。',
            '---',
            '⚠️ **警告：** Ollama的API默认没有任何身份验证。如果将其暴露到网络（`OLLAMA_HOST=0.0.0.0`），该网络上的任何人都可以发送请求、加载模型并消耗GPU资源。对于多用户或生产环境，请在Ollama前面部署带身份验证的反向代理（nginx、Caddy等）——切勿将11434端口直接暴露到互联网。',
          ],
          image: '/images/openai-compatible-platform-comparison-en.svg',
        ...
        faqSection: {
          ...
          faqs: [
            ...
            { q: 'Ollama是否通过API支持函数调用？', a: '是的，支持的模型（Llama 3.1 8B、Qwen3 8B、Mistral）可用。与OpenAI一样传递tools=[]，Ollama解析工具调用并返回结构化JSON。' },
...
schema: {
      ...
      faqSchema: {
        ...
        'mainEntity': [
          ...
          { '@type': 'Question', 'name': 'Ollama支持函数调用吗？', 'acceptedAnswer': { '@type': 'Answer', 'text': '是的，Llama 3.1 8B、Qwen3 8B、Mistral支持。与OpenAI一样传递tools=[]即可。' } },
      ...
      howToSchema: {
        ...
        'step': [
          ...
          { '@type': 'HowToStep', 'name': '调用chat.completions.create', 'text': '使用client.chat.completions.create(model="模型名", messages=[...])——调用结构与OpenAI API相同。模型名必须与LM Studio中加载的或Ollama中安装的一致。' },
        ],
      },
      itemListSchema: {
        '@context': 'https://schema.org',
    ...
    es: {
      ...
      seoTitle: 'Documentación oficial API OpenAI: LM Studio y Ollama (2026)',
      ...
      metaDescription: 'Documentación oficial de la API compatible con OpenAI para LM Studio (localhost:1234) y Ollama (localhost:11434): endpoints, Python, Node.js, Aider/Cline.',
      ...
      sections: {
        ...
        quickFacts: {
          ...
          content: [
            ...
            '**Cambio de código:** 2 líneas — `base_url` y `api_key`. El resto del código permanece prácticamente igual.',
            ...
            '**Autenticación:** normalmente arranca sin autenticación, vinculada a localhost por defecto. Añade un reverse proxy y autenticación si la expones más allá de localhost, ya que vLLM soporta autenticación por API key y LM Studio puede exponerse públicamente.',
        ...
        whatIsOpenAI: {
          ...
          content: [
            ...
            '```\n{\n  "choices": [{\"message\": {\"content\": \"...\"}}],\n  "usage\": {\"prompt_tokens\": 10, \"completion_tokens\": 20}\n}\n```',
            'Como el formato es compartido, no necesitas aprender una nueva API ni reescribir la mayor parte de tu código.',
            '---',
          ...
          image: '/images/local-llm-openai-compatible-api-one-line-change-hero-es.webp',
          imageCaption: 'Cambiar de OpenAI a Ollama requiere modificar 2 líneas -- base_url y api_key -- el resto del código permanece prácticamente igual.',
        },
        ollamaEndpoint: {
          id: 'ollama-api-endpoint',
          ...
          image: '/images/local-llm-openai-compatible-api-api-request-flow-hero-es.webp',
        ...
        functionCalling: {
          ...
          content: [
            ...
            'El soporte de function calling depende del modelo. Llama 4 Scout, Qwen3 8B, Gemma 4 26B-A4B y Mistral Small 3.1 soportan tool calling de forma fiable. Llama 3.1 8B también lo soporta (versiones anteriores). Los modelos más pequeños (3B) pueden no producir JSON de tool call estructurado de forma fiable.',
          ...
          imageCaption: 'Flujo de function calling con Ollama: el modelo local devuelve JSON tool_call y tu app ejecuta la función -- soportado por Llama 4 Scout, Qwen3 8B, Gemma 4 26B-A4B y Mistral.',
        },
        aiCodingTools: {
          id: 'ai-coding-tools',
        ...
        commonMistakes: {
          ...
          imageCaption: 'Ollama (puerto 11434), vLLM (puerto 8000) y LM Studio (puerto 1234) exponen todos endpoints compatibles con OpenAI -- código cliente comparable, diferentes puertos y casos de uso.',
        },
        faqSection: {
          id: 'common-questions',
      ...
      faqSchema: {
        ...
        'mainEntity': [
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Sí, para los modelos que lo soportan (Llama 4 Scout, Qwen3 8B, Gemma 4 26B-A4B, Mistral Small 3.1). Pasa tools=[] en la llamada de API como lo harías con OpenAI. Ollama analiza las llamadas de herramientas y devuelve JSON estructurado. No todos los modelos lo soportan -- consulta la documentación del modelo.'
            }
          },
          {
            '@type': 'Question',
      ...
      howToSchema: {
        ...
        'step': [
          ...
          },
          {
            ...
            'text': 'Usa client.chat.completions.create(model="nombre-del-modelo", messages=[...]) -- la misma estructura de llamada que la API de OpenAI. El nombre del modelo debe coincidir con el cargado en LM Studio o instalado en Ollama.',
          },
        ],
      },
      itemListSchema: {
        ...
        'itemListElement': [
          ...
          { '@type': 'ListItem', 'position': 4, 'name': 'Endpoint Chat Completions', 'description': 'POST /v1/chat/completions -- misma ruta de endpoint en LM Studio, Ollama y vLLM; el mismo código cliente funciona con los tres' },
        ],
      },
      gammaEmbedUrl: '/presentations/local-llm-openai-compatible-api-static.html',
    ...
    ar: {
      ...
      seoTitle: 'توثيق ⁨OpenAI⁩ الرسمي: ⁨LM Studio⁩ و⁨Ollama⁩ (⁨2026⁩)',
      ...
      metaDescription: 'التوثيق الرسمي لواجهة ⁨OpenAI⁩ المتوافقة في ⁨LM Studio⁩ (⁨localhost:1234⁩) و⁨Ollama⁩ (⁨localhost:11434⁩): نقاط النهاية، ⁨Python⁩، ⁨Node.js⁩، إعداد ⁨Aider/Cline⁩.',
      ...
      sections: {
        ...
        quickFacts: {
          ...
          content: [
            ...
            '**التغيير البرمجي:** سطران — `base_url` و `api_key`. تبقى بقية الشيفرة كما هي في الغالب.',
            ...
            '**المصادقة:** تبدأ عادةً دون مصادقة، ومقيَّدة بـ localhost افتراضياً. أضف reverse proxy ومصادقة إذا كشفتها إلى ما هو أبعد من localhost، إذ تدعم vLLM المصادقة بمفتاح API ويمكن كشف LM Studio علناً.',
        ...
        whatIsOpenAI: {
          ...
          content: [
            ...
            '```\n{\n  "choices": [{\"message\": {\"content\": \"...\"}}],\n  "usage\": {\"prompt_tokens\": 10, \"completion_tokens\": 20}\n}\n```',
            'بما أن التنسيق مشترك، فلا حاجة لتعلّم واجهة جديدة أو إعادة كتابة معظم شيفرتك.',
            '---',
          ...
          image: '/images/local-llm-openai-compatible-api-one-line-change-hero-ar.webp',
          imageCaption: 'يتطلب التبديل من OpenAI إلى Ollama تعديل سطرين -- base_url و api_key -- وتبقى بقية الشيفرة كما هي في الغالب.',
        },
        ollamaEndpoint: {
          id: 'ollama-api-endpoint',
          ...
          columns: ['Endpoint', 'URL', 'Descripción'],
          image: '/images/local-llm-openai-compatible-api-api-request-flow-hero-ar.webp',
        ...
        functionCalling: {
          ...
          codeLanguage: 'python',
          image: '/images/openai-compatible-function-calling-es.svg',
          imageCaption: 'تدفق استدعاء الدوال مع Ollama: يعيد النموذج المحلي JSON من نوع tool_call ويشغّل تطبيقك الدالة -- مدعوم من Llama 4 Scout و Qwen3 8B و Gemma 4 26B-A4B و Mistral.',
        },
        aiCodingTools: {
          id: 'ai-coding-tools',
        ...
        commonMistakes: {
          ...
          image: '/images/openai-compatible-platform-comparison-es.svg',
          imageCaption: 'يكشف Ollama (المنفذ 11434) و vLLM (المنفذ 8000) و LM Studio (المنفذ 1234) جميعها عن endpoints متوافقة مع OpenAI -- شيفرة عميل متشابهة، منافذ وحالات استخدام مختلفة.',
        },
        faqSection: {
          id: 'common-questions',
          ...
          faqs: [
            ...
            },
            {
              ...
              a: 'نعم، للنماذج التي تدعمه (Llama 4 Scout و Qwen3 8B و Gemma 4 26B-A4B و Mistral Small 3.1). كما تدعمه النماذج الأقدم (Llama 3.1 8B) أيضاً. مرّر tools=[] في استدعاء الواجهة كما تفعل مع OpenAI. يحلّل Ollama استدعاءات الأدوات ويعيد JSON منظَّماً. لا تدعمه كل النماذج -- اطّلع على توثيق النموذج.',
            },
            {
      ...
      faqSchema: {
        ...
        'mainEntity': [
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'نعم، للنماذج التي تدعمه (Llama 4 Scout و Qwen3 8B و Gemma 4 26B-A4B و Mistral Small 3.1). مرّر tools=[] في استدعاء الواجهة كما تفعل مع OpenAI. يحلّل Ollama استدعاءات الأدوات ويعيد JSON منظَّماً. لا تدعمه كل النماذج -- اطّلع على توثيق النموذج.'
            }
          },
          {
            '@type': 'Question',
      ...
      howToSchema: {
        ...
        'step': [
          ...
          },
          {
            ...
            'text': 'استخدم client.chat.completions.create(model="اسم-النموذج", messages=[...]) -- بنية الاستدعاء نفسها المستخدمة مع واجهة OpenAI. يجب أن يطابق اسم النموذج النموذج المحمَّل في LM Studio أو المثبَّت في Ollama.',
          },
        ],
      },
      itemListSchema: {
        ...
        'itemListElement': [
          ...
          { '@type': 'ListItem', 'position': 4, 'name': 'endpoint Chat Completions', 'description': 'POST /v1/chat/completions -- مسار endpoint نفسه في LM Studio و Ollama و vLLM؛ تعمل شيفرة العميل نفسها مع الثلاثة' },
        ],
      },
      gammaEmbedUrl: '/presentations/local-llm-openai-compatible-api-static.html',
    ...
    pt: {
      ...
      seoTitle: 'LM Studio e Ollama: API OpenAI oficial (2026)',
      ...
      metaDescription: 'Documentação oficial da API compatível com OpenAI para LM Studio (localhost:1234) e Ollama (localhost:11434): endpoints, Python, Node.js, Aider/Cline.',
      ...
      faqSchema: {
        ...
        'mainEntity': [
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Sim, para modelos que suportam isso (Llama 3.1 8B, Qwen3 8B, Mistral). Passe tools=[] na chamada da API como faria com a OpenAI. O Ollama analisa as chamadas de ferramentas e retorna JSON estruturado. Nem todos os modelos suportam isso -- verifique a documentação do modelo.'
            }
          },
          {
            '@type': 'Question',
      ...
      howToSchema: {
        ...
        'step': [
          ...
          },
          {
            ...
            'text': 'Use client.chat.completions.create(model="model-name", messages=[...]) -- a mesma estrutura de chamada da API OpenAI. O nome do modelo deve corresponder ao que está carregado no LM Studio ou instalado no Ollama.',
          },
        ],
      },
      itemListSchema: {
        ...
        'itemListElement': [
          ...
          { '@type': 'ListItem', 'position': 4, 'name': 'Endpoint Chat Completions', 'description': 'POST /v1/chat/completions -- mesmo caminho de endpoint entre LM Studio, Ollama e vLLM; o mesmo código de cliente funciona com os três' },
        ],
      },
      gammaEmbedUrl: '/presentations/local-llm-openai-compatible-api-static.html',
  ...
  ko: {
      ...
      seoTitle: 'LM Studio & Ollama 공식 OpenAI API 문서 (2026)',
      ...
      metaDescription: 'LM Studio(localhost:1234) 및 Ollama(localhost:11434)의 공식 OpenAI 호환 API 문서: 엔드포인트, Python, Node.js, Aider/Cline 설정.',
      ...
      sections: {
        ...
        quickFacts: {
          ...
          title: '⚡ 빠른 정보',
          content: [
            '**Ollama API:** `http://localhost:11434/v1` — OpenAI의 `/chat/completions`에 대응',
            ...
            '**인증:** 일반적으로 인증 없이 시작되며 기본적으로 localhost에 바인딩됩니다. localhost 범위를 넘어 노출할 경우 리버스 프록시와 인증을 추가하세요. vLLM은 API 키 인증을 지원하며 LM Studio는 공개적으로 노출할 수도 있습니다.',
        ...
        whatIsOpenAI: {
          ...
          content: [
            '**OpenAI 호환이란 API 엔드포인트가 OpenAI API와 동일한 형식으로 응답을 반환한다는 의미입니다.** 이를 통해 OpenAI용으로 만들어진 모든 라이브러리나 도구가 다른 URL을 가리키는 것만으로 로컬 모델과 함께 작동할 수 있습니다. 이 표준의 구현 방식에서 [Ollama vs LM Studio](/ko/local-llms/ollama-vs-lm-studio)가 어떻게 다른지 알아보세요.',
            '예시: OpenAI Python 라이브러리는 다음과 같이 요청을 보냅니다:',
            '```\nPOST /chat/completions\n{\n  "model": "gpt-4o",\n  "messages": [...],\n  "temperature": 0.7\n}\n```',
            'Ollama의 API는 `localhost:11434/v1/chat/completions`에서 동일한 형식의 요청을 수신하며 OpenAI 형식으로 응답을 반환합니다:',
            '```\n{\n  "choices": [{\"message\": {\"content\": \"...\"}}],\n  "usage\": {\"prompt_tokens\": 10, \"completion_tokens\": 20}\n}\n```',
            '형식을 공유하기 때문에 새로운 API를 배울 필요도, 코드 대부분을 다시 작성할 필요도 없습니다.',
            '---',
          ...
          image: '/images/local-llm-openai-compatible-api-one-line-change-hero-ko.webp',
        ...
        ollamaEndpoint: {
          ...
          columns: ['Endpoint', 'URL', 'Description'],
          image: '/images/local-llm-openai-compatible-api-api-request-flow-hero-ko.webp',
        ...
        javascript: {
          ...
          content: [
            '**브라우저 측 JavaScript에서 Ollama를 호출하려면 브라우저와 서버가 같은 머신에 있어야 하거나 CORS를 허용해야 합니다.** 보안상의 이유로 localhost에 대한 브라우저 요청은 JavaScript가 localhost에서 제공될 때만 작동합니다. CORS를 원활하게 처리하는 브라우저용 UI는 [최고의 로컬 LLM 프론트엔드](/ko/local-llms/best-local-llm-frontends)를 확인하세요.',
        ...
        functionCalling: {
          ...
          content: [
            '**네, OpenAI API를 통해 로컬 모델에서 함수 호출이 가능합니다.** 함수 스키마를 정의하면 모델이 함수에 전달할 인수로 응답할 수 있습니다. 이를 통해 [코딩을 위한 최고의 로컬 LLM](/ko/local-llms/best-local-llms-for-coding)이 도구 생태계와 통합될 수 있습니다.',
          ...
          codeLanguage: 'python',
          image: '/images/openai-compatible-function-calling-en.svg',
          imageCaption: 'Ollama를 사용한 함수 호출 흐름: 로컬 모델이 tool_call JSON을 반환하면 앱이 함수를 실행합니다. Llama 4 Scout, Qwen3 8B, Gemma 4 26B-A4B, Mistral에서 지원됩니다.',
        },
        aiCodingTools: {
          id: 'ai-coding-tools',
        ...
        commonMistakes: {
          ...
          items: [
            ...
            '**브라우저에서의 CORS 오류.** 브라우저 측 스크립트에서 Ollama를 호출할 때 CORS 오류가 발생하면 보안 이유로 브라우저가 요청을 차단한 것입니다. CORS를 우회하는 에디터 기반 솔루션은 [VS Code 및 Cursor로 로컬 LLM 사용하기](/ko/local-llms/local-llms-with-vscode-cursor)를 참조하세요.',
          ...
          image: '/images/openai-compatible-platform-comparison-en.svg',
          imageCaption: 'Ollama(포트 11434), vLLM(포트 8000), LM Studio(포트 1234) 모두 OpenAI 호환 엔드포인트를 제공합니다. 비슷한 클라이언트 코드, 다른 포트와 사용 사례입니다.',
        },
        faqSection: {
          id: 'common-questions',
          ...
          faqs: [
            ...
            },
            {
              ...
              a: '네, 지원하는 모델에서 가능합니다(Llama 4 Scout, Qwen3 8B, Gemma 4 26B-A4B, Mistral Small 3.1). 레거시 모델(Llama 3.1 8B)도 지원됩니다. OpenAI와 동일하게 API 호출에 tools=[]를 전달하면 됩니다. Ollama는 도구 호출을 파싱하고 구조화된 JSON을 반환합니다. 모든 모델이 이를 지원하는 것은 아니므로 모델 문서를 확인하세요.',
            },
            {
        ...
        relatedReading: {
          ...
          title: '관련 문서',
          items: [
            '[Ollama 설치 방법](/ko/local-llms/how-to-install-ollama) -- API를 노출하기 위한 Ollama 설정.',
            '[Ollama vs LM Studio](/ko/local-llms/ollama-vs-lm-studio) -- 둘 다 OpenAI 호환 API를 제공합니다.',
            '[코딩 워크플로를 위한 로컬 LLM](/ko/local-llms/local-llms-for-coding-workflows) -- Aider, Cline 등 AI 코딩 도구를 로컬 모델과 함께 사용하는 방법.',
            '[최고의 로컬 LLM 프론트엔드](/ko/local-llms/best-local-llm-frontends) -- 이 API를 내부적으로 사용하는 UI.',
            '[VS Code 및 Cursor로 로컬 LLM 사용하기](/ko/local-llms/local-llms-with-vscode-cursor) -- 이 API를 사용하는 코드 에디터.',
            '[Text-Generation-WebUI vs vLLM vs llama.cpp](/ko/local-llms/text-generation-webui-vs-vllm-vs-llamacpp) -- 이 API를 노출하는 다양한 백엔드.',
            '[로컬 LLM 개발자 스택](/ko/local-llms/local-llm-developer-stack) -- API 호환성 설정을 넘어선 완전한 3계층 스택(CLI → API → 프로덕션).',
            '[구조화된 출력 및 JSON 모드](/ko/prompt-engineering/structured-output-and-json-mode) -- API를 사용하여 출력 형식을 강제하는 방법.',
            '[프롬프트 체이닝](/ko/prompt-engineering/prompt-chaining) -- 로컬 모델로 다단계 API 파이프라인 구축.',
            '[제약 프롬프팅](/ko/prompt-engineering/constrained-prompting) -- API 수준의 형식 강제.',
      ...
      howToSchema: {
        ...
        'step': [
          ...
          },
          {
            ...
            'text': 'client.chat.completions.create(model="model-name", messages=[...])를 사용하세요. OpenAI API 호출과 동일한 구조입니다. 모델 이름은 LM Studio에 로드되었거나 Ollama에 설치된 모델과 일치해야 합니다.',
          },
        ],
      },
      itemListSchema: {
