...
schema: {
      ...
      itemListSchema: {
        ...
        'itemListElement': [
          ...
          { '@type': 'ListItem', 'position': 8, 'name': 'Qwen2.5 7B',                 'description': '128K tokens context. ~32K practical. 14 GB RAM at 128K Q4_K_M. ollama run qwen2.5:7b' },
          { '@type': 'ListItem', 'position': 9, 'name': 'Qwen2.5 72B',                'description': '128K tokens context. ~64K practical. 55 GB RAM at 128K Q4_K_M. ollama run qwen2.5:72b' },
      ...
      faqSchema: {
        ...
        'mainEntity': [
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'As of May 2026: Llama 4 Scout supports 10M tokens (practical: 256K-1M on sufficient hardware). DeepSeek V4-Flash and Qwen 3.6 support 1M and 256K tokens respectively (Qwen 3.6 extendable to 1M via YaRN). Earlier models: Qwen3, Llama 3.3, Qwen2.5, and Mistral Small support 128K tokens. Ollama defaults to 2048 tokens -- set num_ctx explicitly in a Modelfile to access long contexts.',
            },
          },
          {
            '@type': 'Question',
    ...
    es: {
      ...
      itemListSchema: {
        ...
        'itemListElement': [
          ...
          { '@type': 'ListItem', 'position': 8, 'name': 'Qwen2.5 7B',                 'description': '128K tokens de contexto. ~32K práctico. 14 GB de RAM con 128K Q4_K_M. ollama run qwen2.5:7b' },
          { '@type': 'ListItem', 'position': 9, 'name': 'Qwen2.5 72B',                'description': '128K tokens de contexto. ~64K práctico. 55 GB de RAM con 128K Q4_K_M. ollama run qwen2.5:72b' },
      ...
      sections: {
        ...
        which128k: {
          ...
          rows: [
            ...
            { 'Modelo': 'Qwen2.5 7B', 'Ventana de contexto': '128K', 'Límite práctico': '~32K fiable', 'Comando Ollama': 'ollama run qwen2.5:7b' },
            { 'Modelo': 'Qwen2.5 72B', 'Ventana de contexto': '128K', 'Límite práctico': '~64K fiable', 'Comando Ollama': 'ollama run qwen2.5:72b' },
        ...
        ramForLongContext: {
          ...
          rows: [
            ...
            { 'Modelo': 'Qwen2.5 14B Q4_K_M', 'Contexto 4K': '~9 GB', 'Contexto 32K': '~12 GB', 'Contexto 128K': '~18 GB' },
      ...
      gammaEmbedUrl: '/presentations/long-context-local-llms-static.html',
      gammaDescription: 'El deck de diapositivas cubre: comparación de modelos con ventana de contexto de 128K (Llama 3.3, Qwen2.5, Mistral Small 3.1), uso de RAM en longitudes de contexto 4K/32K/128K, el efecto "lost in the middle" y los límites prácticos fiables (~32K para modelos 7B), y cómo configurar num_ctx en Ollama. Descarga el PDF como tarjeta de referencia de LLM local de contexto largo.',
    },
    zh: {
      theme: '最佳模型',
      title: '长上下文本地LLM 2026：32K、128K 模型对比与RAM需求',
      seoTitle: '本地LLM长上下文2026：128K~1M Qwen vs Llama vs DeepSeek比',
      intro: '2026年大多数本地LLM模型理论上支持128K令牌上下文窗口，但实际可用的上下文----质量保持高水平的地方----通常为16K-32K令牌。Llama 3.3 8B、Qwen2.5和Mistral Small 3.1都支持128K上下文，但在32K以上质量会下降。本地处理全长文档需要理解RAM扩展和「中间迷失」效应。',
      ...
      sections: {
        ...
        which128k: {
          ...
          rows: [
            ...
            { '模型': 'Qwen2.5 7B', '上下文窗口': '128K', '实际上限': '约32K可靠', 'Ollama命令': 'ollama run qwen2.5:7b' },
            { '模型': 'Qwen2.5 72B', '上下文窗口': '128K', '实际上限': '约64K可靠', 'Ollama命令': 'ollama run qwen2.5:72b' },
        ...
        ramForLongContext: {
          ...
          rows: [
            ...
            { '模型': 'Qwen2.5 14B Q4_K_M', '4K上下文': '约9 GB', '32K上下文': '约12 GB', '128K上下文': '约18 GB' },
...
schema: {
        ...
        'description': '2026年大多数本地LLM理论上支持128K标记上下文窗口，但实际可用上下文通常为16K-32K标记。Llama 3.3 8B、Qwen2.5和Mistral Small 3.1均支持128K上下文。RAM扩展规律与Ollama配置指南。',
      ...
      itemListSchema: {
        ...
        'itemListElement': [
          ...
          { '@type': 'ListItem', 'position': 4, 'name': 'Qwen2.5 7B',            'description': '128K上下文。实际上限32K。ollama run qwen2.5:7b' },
          { '@type': 'ListItem', 'position': 5, 'name': 'Qwen2.5 72B',           'description': '128K上下文。实际上限64K。ollama run qwen2.5:72b' },
      ...
      gammaEmbedUrl: '/presentations/long-context-local-llms-static.html',
      gammaDescription: '以下幻灯片涵盖：128K上下文窗口模型对比（Llama 3.3、Qwen2.5、Mistral Small 3.1）、4K/32K/128K上下文长度下的RAM需求、"中间迷失"效应与实际可靠限制（7B模型约32K），以及如何在Ollama中设置num_ctx。将PDF下载为长上下文本地LLM参考卡。',
    },
    de: {
      theme: 'Best Models',
      ...
      intro: 'Die meisten lokalen LLM-Modelle im Jahr 2026 unterstützen auf dem Papier Kontextfenster von 128K Token, aber der praktisch nutzbare Kontext -- wo die Qualität hoch bleibt -- liegt typischerweise bei 16K-32K Token. Llama 3.3/3.2/3.3, Qwen2.5 und Mistral Small 3.1 unterstützen alle 128K Kontext. Die Verarbeitung längerer Dokumente oder Bücher erfordert Verständnis für RAM-Skalierung und Qualitätsverschlechterung bei großen Bereichen.',
    ...
    fr: {
      ...
      intro: 'La plupart des modèles LLM locaux en 2026 prennent en charge sur le papier des fenêtres de contexte de 128K tokens, mais le contexte utilisable pratique -- où la qualité reste élevée -- est généralement de 16K-32K tokens. Llama 3.3/3.2/3.3, Qwen2.5 et Mistral Small 3.1 prennent tous en charge le contexte 128K. Traiter des documents ou des livres longs localement nécessite de comprendre la mise à l\'échelle RAM et la dégradation de la qualité sur les longues plages.',
      ...
      sections: {
        ...
        which128k: { id: 'models-128k', title: 'Quels LLMs locaux supportent 128K tokens de contexte en 2026 ?', rows: [{ 'Modèle': 'Llama 3.3 8B', 'Fenêtre de contexte': '128K', 'Limite pratique': '~32K fiable', 'Commande Ollama': 'ollama run llama3.2' }, { 'Modèle': 'Llama 3.2 3B', 'Fenêtre de contexte': '128K', 'Limite pratique': '~16K fiable', 'Commande Ollama': 'ollama run llama3.2:3b' }, { 'Modèle': 'Llama 3.3 70B', 'Fenêtre de contexte': '128K', 'Limite pratique': '~64K fiable', 'Commande Ollama': 'ollama run llama3.3:70b' }, { 'Modèle': 'Qwen2.5 7B', 'Fenêtre de contexte': '128K', 'Limite pratique': '~32K fiable', 'Commande Ollama': 'ollama run qwen2.5:7b' }, { 'Modèle': 'Qwen2.5 72B', 'Fenêtre de contexte': '128K', 'Limite pratique': '~64K fiable', 'Commande Ollama': 'ollama run qwen2.5:72b' }, { 'Modèle': 'Mistral Small 3.1 24B', 'Fenêtre de contexte': '128K', 'Limite pratique': '~32K fiable', 'Commande Ollama': 'ollama run mistral-small3.1' }, { 'Modèle': 'Gemma 2 2B', 'Fenêtre de contexte': '8K', 'Limite pratique': '~6K fiable', 'Commande Ollama': 'ollama run gemma2:2b' }, { 'Modèle': 'Mistral Small v0.3', 'Fenêtre de contexte': '32K', 'Limite pratique': '~16K fiable', 'Commande Ollama': 'ollama run llama3.2' }], columns: ['Modèle', 'Fenêtre de contexte', 'Limite pratique', 'Commande Ollama'], image: '/images/long-context-models-comparison-en.svg', imageCaption: '6 modèles LLM locaux avec support de contexte 128K -- limite fiable pratique de 32K pour les 7B, 64K pour les 70B.' },
        ramForLongContext: { id: 'ram-long-context', title: 'Combien de RAM le traitement de long contexte nécessite-t-il ?', content: ['**L\'utilisation RAM évolue avec la taille du modèle et la longueur du contexte.** Le cache KV stocke les états d\'attention pour tous les tokens traités -- il croît linéairement avec la longueur du contexte.', 'Un modèle 7B en Q4_K_M avec contexte 4K utilise ~6 Go de RAM. Le même modèle avec contexte 32K utilise ~8-9 Go. Avec contexte 128K : ~12-16 Go.'], rows: [{ 'Modèle': 'Llama 3.3 8B Q4_K_M', 'Contexte 4K': '~6 Go', 'Contexte 32K': '~9 Go', 'Contexte 128K': '~14 Go' }, { 'Modèle': 'Qwen2.5 14B Q4_K_M', 'Contexte 4K': '~9 Go', 'Contexte 32K': '~12 Go', 'Contexte 128K': '~18 Go' }, { 'Modèle': 'Mistral Small 3.1 24B Q4_K_M', 'Contexte 4K': '~14 Go', 'Contexte 32K': '~17 Go', 'Contexte 128K': '~24 Go' }, { 'Modèle': 'Llama 3.3 70B Q4_K_M', 'Contexte 4K': '~40 Go', 'Contexte 32K': '~45 Go', 'Contexte 128K': '~55 Go' }], columns: ['Modèle', 'Contexte 4K', 'Contexte 32K', 'Contexte 128K'], image: '/images/long-context-ram-scaling-en.svg', imageCaption: 'La RAM du cache KV évolue avec la longueur du contexte -- un modèle 7B en Q4_K_M nécessite ~6 Go à 4K, mais ~14 Go à 128K.' },
      ...
      gammaEmbedUrl: '/presentations/long-context-local-llms-static.html',
      gammaDescription: 'La présentation couvre : comparaison des modèles à fenêtre 128K tokens (Llama 3.3, Qwen2.5, Mistral Small 3.1), RAM selon la longueur de contexte 4K/32K/128K, l\'effet « perdu au milieu » avec limites pratiques (~32K pour les 7B) et comment configurer num_ctx dans Ollama. Téléchargez le PDF comme carte de référence LLM local long contexte.',
    },
    ja: {
      theme: 'Best Models',
      title: '長コンテキスト対応ローカルLLM 2026：32K、128Kトークンをローカルで処理できるモデルはどれ？',
      seoTitle: 'ローカルLLM 128K〜1Mコンテキスト2026：Qwen vs Llama vs DeepSeek',
      intro: '2026年のほとんどのローカルLLMモデルは理論上128Kトークンのコンテキストウィンドウをサポートしていますが、実用的で使用可能なコンテキスト -- 品質が高い場所 -- は通常16K～32Kトークンです。Llama 3.3/3.2/3.3、Qwen2.5、Mistral Small 3.1はすべて128Kコンテキストをサポートしています。長いドキュメントや本をローカルで処理するには、RAM スケーリングと長い範囲での品質低下を理解する必要があります。',
      ...
      sections: {
        ...
        which128k: { id: 'models-128k', title: '2026年に128KトークンのコンテキストをサポートするローカルLLMは？', rows: [{ 'モデル': 'Llama 3.3 8B', 'コンテキストウィンドウ': '128K', '実用的な上限': '~32K 信頼性あり', 'Ollamaコマンド': 'ollama run llama3.2' }, { 'モデル': 'Llama 3.2 3B', 'コンテキストウィンドウ': '128K', '実用的な上限': '~16K 信頼性あり', 'Ollamaコマンド': 'ollama run llama3.2:3b' }, { 'モデル': 'Llama 3.3 70B', 'コンテキストウィンドウ': '128K', '実用的な上限': '~64K 信頼性あり', 'Ollamaコマンド': 'ollama run llama3.3:70b' }, { 'モデル': 'Qwen2.5 7B', 'コンテキストウィンドウ': '128K', '実用的な上限': '~32K 信頼性あり', 'Ollamaコマンド': 'ollama run qwen2.5:7b' }, { 'モデル': 'Qwen2.5 72B', 'コンテキストウィンドウ': '128K', '実用的な上限': '~64K 信頼性あり', 'Ollamaコマンド': 'ollama run qwen2.5:72b' }, { 'モデル': 'Mistral Small 3.1 24B', 'コンテキストウィンドウ': '128K', '実用的な上限': '~32K 信頼性あり', 'Ollamaコマンド': 'ollama run mistral-small3.1' }, { 'モデル': 'Gemma 2 2B', 'コンテキストウィンドウ': '8K', '実用的な上限': '~6K 信頼性あり', 'Ollamaコマンド': 'ollama run gemma2:2b' }, { 'モデル': 'Mistral Small v0.3', 'コンテキストウィンドウ': '32K', '実用的な上限': '~16K 信頼性あり', 'Ollamaコマンド': 'ollama run llama3.2' }], columns: ['モデル', 'コンテキストウィンドウ', '実用的な上限', 'Ollamaコマンド'], image: '/images/long-context-models-comparison-en.svg', imageCaption: '128Kコンテキストをサポートする6つのローカルLLM。7Bモデルで32K、70Bモデルで64Kが実用的な信頼性の上限。' },
        ramForLongContext: { id: 'ram-long-context', title: '長いコンテキストの処理にはどれくらいのRAMが必要か？', content: ['**RAMの使用量はモデルサイズとコンテキスト長の両方に比例して増加します。** KVキャッシュはすべての処理済みトークンのAttention状態を格納し、コンテキスト長に線形に増加します。', 'Q4_K_Mの7Bモデルで4Kコンテキストは~6 GB RAMを使用。同じモデルで32Kコンテキストは~8-9 GB。128Kコンテキストでは~12-16 GB。'], rows: [{ 'モデル': 'Llama 3.3 8B Q4_K_M', '4Kコンテキスト': '~6 GB', '32Kコンテキスト': '~9 GB', '128Kコンテキスト': '~14 GB' }, { 'モデル': 'Qwen2.5 14B Q4_K_M', '4Kコンテキスト': '~9 GB', '32Kコンテキスト': '~12 GB', '128Kコンテキスト': '~18 GB' }, { 'モデル': 'Mistral Small 3.1 24B Q4_K_M', '4Kコンテキスト': '~14 GB', '32Kコンテキスト': '~17 GB', '128Kコンテキスト': '~24 GB' }, { 'モデル': 'Llama 3.3 70B Q4_K_M', '4Kコンテキスト': '~40 GB', '32Kコンテキスト': '~45 GB', '128Kコンテキスト': '~55 GB' }], columns: ['モデル', '4Kコンテキスト', '32Kコンテキスト', '128Kコンテキスト'], image: '/images/long-context-ram-scaling-en.svg', imageCaption: 'KVキャッシュのRAMはコンテキスト長とともに増加。Q4_K_Mの7Bモデルは4Kで~6 GB、128Kで~14 GB必要。' },
      ...
      gammaEmbedUrl: '/presentations/long-context-local-llms-static.html',
      gammaDescription: '以下のスライドデッキは：128Kトークンコンテキストモデル比較（Llama 3.3、Qwen2.5、Mistral Small 3.1）、4K/32K/128Kコンテキスト長でのRAM使用量、「中間迷子」効果と実用的な制限（7Bモデルで約32K）、Ollamaでnum_ctxを設定する方法をカバーしています。長コンテキストローカルLLMリファレンスカードとしてPDFをダウンロードしてください。',
    },
  };
