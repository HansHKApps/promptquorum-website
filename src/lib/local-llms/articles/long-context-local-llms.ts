...
schema: {
    ...
    ar: {
      ...
      seoTitle: 'LLM محلية بسياق طويل 2026: 6 نماذج من 128K إلى 1M',
      ...
      metaDescription: '6 نماذج LLM محلية حسب السياق: Llama 4 Scout (10M)، DeepSeek V4-Flash (1M)، Qwen 3.6 (256K)، ونماذج 128K. متطلبات RAM لكل مستوى وضبط num_ctx.',
      publishDate: '2026-04-04',
    ...
    pt: {
      ...
      seoTitle: 'LLMs locais contexto longo 2026: 6 modelos, 128K a 1M',
      ...
      metaDescription: '6 LLMs locais por contexto: Llama 4 Scout (10M), DeepSeek V4-Flash (1M), Qwen 3.6 (256K) e modelos de 128K. RAM por nível (14-55 GB) e o ajuste num_ctx.',
      publishDate: '2026-04-04',
    ...
    zh: {
      ...
      seoTitle: '本地LLM长上下文2026：6款模型128K到1M令牌',
      ...
      metaDescription: '6款本地LLM按上下文大小对比：Llama 4 Scout（10M）、DeepSeek V4-Flash（1M）、Qwen 3.6（256K）及128K模型。各档RAM需求（14-55 GB），以及Ollama默认仅2048的num_ctx设置。',
      nextStep: {
...
schema: {
    ...
    de: {
      ...
      seoTitle: 'Lokale LLMs mit langem Kontext 2026: 6 Modelle, 128K-1M',
      ...
      metaDescription: '6 lokale LLMs nach Kontextgröße: Llama 4 Scout (10M), DeepSeek V4-Flash (1M), Qwen 3.6 (256K), 128K-Modelle. RAM pro Stufe (14-55 GB), num_ctx-Setup.',
      nextStep: {
    ...
    fr: {
      ...
      seoTitle: 'LLM locaux long contexte 2026 : 6 modèles, 128K à 1M',
      ...
      metaDescription: '6 LLM locaux par contexte : Llama 4 Scout (10M), DeepSeek V4-Flash (1M), Qwen 3.6 (256K) et modèles 128K. RAM par palier (14-55 Go) et le réglage num_ctx.',
      nextStep: {
    ...
    ja: {
      ...
      seoTitle: '長コンテキスト・ローカルLLM 2026：6モデル128K〜1M',
      ...
      metaDescription: 'コンテキストサイズ別ローカルLLM 6選：Llama 4 Scout（10M）、DeepSeek V4-Flash（1M）、Qwen 3.6（256K）と128Kモデル。各段階のRAM要件（14-55 GB）と、Ollamaが2048のままにするnum_ctx設定。',
      nextStep: {
  ...
  ko: {
      ...
      theme: 'Best Models',
      title: 'Ollama 컨텍스트 창 설정: Strix Halo, RTX, Mac에서 64K~1M 토큰 (2026)',
      seoTitle: '로컬 LLM 긴 컨텍스트 2026: Ollama에서 64K~1M 토큰',
      intro: '2026년 컨텍스트 창 혁명이 시작되었습니다. Llama 4 Scout는 최대 1,000만 토큰 컨텍스트를 지원하며(실용적 범위: 256K~1M), DeepSeek V4-Flash는 1M 토큰을 제공하고, Qwen 3.6은 기본적으로 256K 토큰을 지원합니다(YaRN을 통해 1M으로 확장 가능). 대부분의 7B~8B 모델은 실용적 컨텍스트가 16K~32K 수준에 머물지만, 새로운 MoE 모델은 소비자용 하드웨어에서 256K~1M 토큰까지 한계를 확장합니다. Ollama의 기본값은 2048 토큰입니다. 이 가이드에서는 각 모델이 지원하는 컨텍스트 크기, 각 단계별 RAM 요구 사항, 그리고 긴 컨텍스트 설정 방법을 안내합니다.',
      metaDescription: 'Strix Halo, RTX 4090, Apple Silicon에서 Ollama 컨텍스트 창 설정: 64K, 128K, 256K, 1M 토큰. num_ctx 설정, 플랫폼별 VRAM, Vulkan 제한 사항.',
      publishDate: '2026-04-04',
      leadAnswerBlock: 'Llama 4 Scout는 최대 1,000만 토큰 컨텍스트를 지원합니다(소비자용 하드웨어에서 실용적 범위: 256K~1M). DeepSeek V4-Flash는 1M 토큰을 제공합니다. Qwen 3.6은 기본적으로 256K 토큰을 지원하며 YaRN을 통해 1M까지 확장할 수 있습니다. 7B~8B 모델은 16K~32K 토큰에서 안정적으로 동작하지만, 새로운 MoE 모델과 70B 이상 모델은 256K~1M까지 실용적 한계를 확장합니다. Ollama의 기본값은 2048 토큰입니다. 긴 컨텍스트를 사용하려면 num_ctx를 명시적으로 설정하십시오.',
      nextStep: {
      ...
      schema: {
        '@context': 'https://schema.org',
        '@type': 'TechArticle',
        'headline': '긴 컨텍스트 로컬 LLM 2026년 5월: 1M 토큰 -- Llama 4, DeepSeek V4, Qwen 3.6',
        'description': 'Llama 4 Scout (1,000만 토큰 컨텍스트), DeepSeek V4-Flash (1M), Qwen 3.6 (기본 256K, YaRN으로 1M 확장 가능)이 2026년 긴 컨텍스트 로컬 LLM을 선도합니다. 7B~8B 모델은 16K~32K에서 실용적이며, MoE 및 70B 이상 모델은 256K~1M에 도달합니다.',
        'datePublished': '2026-04-04',
        'dateModified': '2026-05-06',
        'url': 'https://www.promptquorum.com/local-llms/long-context-local-llms',
        'proficiencyLevel': 'Intermediate',
      ...
      itemListSchema: {
        '@context': 'https://schema.org',
        '@type': 'ItemList',
        'name': '컨텍스트 창 크기별 로컬 LLM -- Ollama 2026년 5월',
        'numberOfItems': 12,
        'itemListElement': [
          { '@type': 'ListItem', 'position': 1, 'name': 'Llama 4 Scout 17B (MoE)',     'description': '1,000만 토큰 컨텍스트. 150GB 이상에서 256K~1M 실용 가능. 100만 토큰 작업에 최적. ollama run llama4:scout' },
          { '@type': 'ListItem', 'position': 2, 'name': 'DeepSeek V4-Flash 14B',      'description': '1M 토큰 컨텍스트. 256K~512K 실용 가능. 256K 컨텍스트 시 22GB RAM. ollama run deepseek-v4-flash' },
          { '@type': 'ListItem', 'position': 3, 'name': 'Qwen 3.6 27B',               'description': '기본 256K 토큰 (YaRN으로 1M 확장 가능). 256K 실용 가능. YaRN으로 1M 사용 시 65GB 이상. ollama run qwen3.6:27b' },
          { '@type': 'ListItem', 'position': 4, 'name': 'Qwen 3 8B',                  'description': '128K 토큰 컨텍스트. 32K~64K 실용 가능. 128K Q4_K_M 시 22GB RAM. ollama run qwen3:8b' },
          { '@type': 'ListItem', 'position': 5, 'name': 'Llama 3.3 8B',               'description': '128K 토큰 컨텍스트. 32K 실용 가능. 128K Q4_K_M 시 14GB RAM. ollama run llama3.1:8b' },
          { '@type': 'ListItem', 'position': 6, 'name': 'Llama 3.2 3B',               'description': '128K 토큰 컨텍스트. 16K 실용 가능. 2.5GB RAM. ollama run llama3.2:3b' },
          { '@type': 'ListItem', 'position': 7, 'name': 'Mistral Small 3.1 24B',      'description': '128K 토큰 컨텍스트. 32K 실용 가능. 128K Q4_K_M 시 24GB RAM. ollama run mistral-small3.1' },
          { '@type': 'ListItem', 'position': 8, 'name': 'Qwen3 7B',                   'description': '128K 토큰 컨텍스트. 32K 실용 가능. 128K Q4_K_M 시 14GB RAM. ollama run qwen2.5:7b' },
          { '@type': 'ListItem', 'position': 9, 'name': 'Qwen3 72B',                  'description': '128K 토큰 컨텍스트. 64K 실용 가능. 128K Q4_K_M 시 55GB RAM. ollama run qwen2.5:72b' },
          { '@type': 'ListItem', 'position': 10, 'name': 'Llama 3.3 70B',             'description': '128K 토큰 컨텍스트. 64K 실용 가능. 128K Q4_K_M 시 55GB RAM. ollama run llama3.3:70b' },
          { '@type': 'ListItem', 'position': 11, 'name': 'Gemma 2 2B',                'description': '8K 토큰 컨텍스트. 6K 실용 가능. 1.7GB RAM. ollama run gemma2:2b' },
          { '@type': 'ListItem', 'position': 12, 'name': 'Mistral Small v0.3',           'description': '32K 토큰 컨텍스트. 16K 실용 가능. 4.5GB RAM. ollama run mistral' },
        ],
      },
      faqSchema: {
        '@context': 'https://schema.org',
        ...
        'mainEntity': [
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': '2026년 5월 기준, 가능합니다. DeepSeek V4-Flash는 기본적으로 1M 토큰을 지원합니다. Qwen 3.6은 256K를 기본 지원하며 YaRN으로 1M까지 확장 가능합니다. Llama 4 Scout는 최대 1,000만 토큰을 지원합니다. 전체 1M 컨텍스트를 실행하려면 150GB 이상의 VRAM이 필요하며, 소비자용 하드웨어에서는 256K~512K까지 안정적으로 사용할 수 있습니다. 비교하자면 Gemini 3.1 Pro는 여전히 클라우드 API가 필요합니다.',
            },
          },
          {
            '@type': 'Question',
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          items: [
            'Llama 4 Scout (MoE)는 최대 1,000만 토큰을 지원합니다. DeepSeek V4-Flash와 Qwen 3.6은 각각 1M 및 256K 토큰을 지원합니다(YaRN으로 1M 확장 가능). 2026년 5월은 백만 토큰 처리가 가능한 오픈 모델 최초의 세대가 시작된 시점입니다.',
            '**모델 크기별 실용적 컨텍스트**: 7B~8B 모델은 16K~32K 토큰에서 품질을 유지합니다. 70B 이상 모델과 MoE 모델은 256K~1M까지 확장됩니다. Llama 4 Scout는 충분한 VRAM에서 100만 토큰 전체 컨텍스트를 처리할 수 있습니다.',
            'RAM은 컨텍스트 길이와 모델 크기 모두에 따라 증가합니다. Q4_K_M의 Qwen 3.6 27B는 128K에서 약 22GB, 1M 토큰에서 65GB 이상이 필요합니다. Llama 4 Scout는 전체 1,000만 토큰 컨텍스트에 150GB 이상이 필요합니다.',
        ...
        which128k: {
          ...
          rows: [
            { 'Model': 'Llama 3.3 8B', 'Context Window': '128K', 'Practical Limit': '~32K 안정', 'Ollama Command': 'ollama run llama3.2' },
            { 'Model': 'Llama 3.2 3B', 'Context Window': '128K', 'Practical Limit': '~16K 안정', 'Ollama Command': 'ollama run llama3.2:3b' },
            { 'Model': 'Llama 3.3 70B', 'Context Window': '128K', 'Practical Limit': '~64K 안정', 'Ollama Command': 'ollama run llama3.3:70b' },
            { 'Model': 'Qwen3 7B', 'Context Window': '128K', 'Practical Limit': '~32K 안정', 'Ollama Command': 'ollama run qwen2.5:7b' },
            { 'Model': 'Qwen3 72B', 'Context Window': '128K', 'Practical Limit': '~64K 안정', 'Ollama Command': 'ollama run qwen2.5:72b' },
            { 'Model': 'Mistral Small 3.1 24B', 'Context Window': '128K', 'Practical Limit': '~32K 안정', 'Ollama Command': 'ollama run mistral-small3.1' },
            { 'Model': 'Gemma 2 2B', 'Context Window': '8K', 'Practical Limit': '~6K 안정', 'Ollama Command': 'ollama run gemma2:2b' },
            { 'Model': 'Mistral Small v0.3', 'Context Window': '32K', 'Practical Limit': '~16K 안정', 'Ollama Command': 'ollama run llama3.2' },
          ],
          columns: ['Model', 'Context Window', 'Practical Limit', 'Ollama Command'],
          image: '/images/long-context-models-comparison-en.svg',
          imageCaption: '128K 컨텍스트를 지원하는 로컬 LLM 6개 -- 7B 모델의 실용적 안정 한계는 32K, 70B 모델은 64K입니다.',
        },
        ramForLongContext: {
          id: 'ram-long-context',
        ...
        faqSection: {
          ...
          faqs: [
            ...
            },
            {
              ...
              a: '아니요. 2026년 4월 기준, 로컬에서 실행 가능한 모델 중 1M 토큰 컨텍스트를 지원하는 것은 없습니다. Gemini 3.1 Pro의 1M 토큰 창은 Google의 TPU 인프라가 필요합니다. 로컬에서는 현재 소비자용 하드웨어가 지원하는 최대 128K가 한계입니다. 1M 이상의 토큰 컨텍스트가 필요한 작업에서는 클라우드 API가 유일한 실용적 옵션입니다.',
            },
            {
