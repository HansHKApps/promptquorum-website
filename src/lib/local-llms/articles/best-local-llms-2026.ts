...
export const article: Partial<Record<Language, LLMArticle>> = {
    en: {
      ...
      current_benchmarks_used: ['MMLU', 'HumanEval', 'SWE-bench Verified', 'AIME 2026', 'GPQA Diamond', 'EvalPlus'],
      ...
      sections: {
        ...
        commonMistakes: {
          ...
          items: [
            ...
            'Comparing models across different hardware tiers -- Qwen3.6 27B\'s 84% MMLU doesn\'t directly "compete" with Qwen2.5-Coder 7B\'s 88% HumanEval when they require fundamentally different RAM (~17 GB vs ~5 GB). Choose the model that fits your hardware constraint, then verify its performance on your task.',
            'Downloading a large model before verifying available RAM -- a ~17-20 GB download takes 30-60 minutes on typical home internet. Run `free -h` (Linux) or check Activity Monitor (macOS) before pulling large models. If insufficient RAM is available, Ollama will begin CPU offloading, degrading speed to 2-5 tok/sec.',
...
schema: {
    ...
    ar: {
      ...
      sections: {
        ...
        llama33: {
          ...
          rows: [
            { 'المواصفة': 'درجة MMLU', 'القيمة': '84%' },
            { 'المواصفة': 'SWE-bench Verified', 'القيمة': '77%' },
            { 'المواصفة': 'RAM المطلوب (Q4_K_M)', 'القيمة': '~17 GB' },
            { 'المواصفة': 'نافذة السياق', 'القيمة': '262K token (حتى 1M ممتد)' },
            { 'المواصفة': 'أمر Ollama', 'القيمة': 'ollama pull batiai/qwen3.6-27b:q4' },
          ],
        ...
        fullBenchmark: {
          ...
          rows: [
            { 'النموذج': 'Qwen3.6 27B', 'MMLU': '84%', 'HumanEval': '—', 'RAM': '~17 GB', 'الأفضل لـ': 'عام (SWE-bench 77%)' },
    ...
    pt: {
      ...
      sections: {
        ...
        fullBenchmark: {
          id: 'full-benchmark-comparison',
          title: 'Comparação completa de benchmarks: Top 5 LLMs locais julho 2026',
          rows: [
            { 'Modelo': 'Qwen3.6 27B', 'MMLU': '84%', 'HumanEval': '—', 'RAM': '~17 GB', 'Melhor para': 'Geral (SWE-bench 77%)' },
        ...
        whichModel: {
          ...
          items: [
            ...
            '**8-16 GB de RAM (maioria dos laptops)**: Phi-4-mini ou a variante MoE Qwen3.6-35B-A3B -- Qwen3.6 27B (~17 GB) não cabe mais confortavelmente nesse nível.',
            '**24 GB+ de VRAM/RAM (melhor qualidade geral)**: Qwen3.6 27B (`ollama pull batiai/qwen3.6-27b:q4`) -- melhor qualidade geral nesse nível, 201 idiomas incluindo pt-BR.',
            ...
            '**Português e outros idiomas não ingleses**: Qwen3.6 27B (201 idiomas nativos, incluindo pt-BR) -- veja [Qwen vs Llama vs Mistral](/pt/local-llms/qwen-vs-llama-vs-mistral).',
          ],
          image: '/images/best-local-llms-2026-ram-tier-picker-hero-pt.webp',
          imageCaption: 'Seletor de nível de RAM para LLMs locais: Gemma 4 E2B cabe em ~2 GB, Phi-4-mini em ~2,5 GB, Qwen2.5-Coder 7B em ~5 GB, Gemma 4 26B-A4B em ~15 GB, e Qwen3.6 27B precisa de ~17 GB (24 GB+ recomendado).',
        },
        regionalContext: {
          id: 'regional-context',
        ...
        commonMistakes: {
          ...
          items: [
            ...
            '**Comparar modelos entre diferentes níveis de hardware** -- os 84% de MMLU do Qwen3.6 27B não "competem" diretamente com os 88% de HumanEval do Qwen2.5-Coder 7B quando exigem RAM fundamentalmente diferente (~17 GB vs ~5 GB). Escolha o modelo que cabe na sua restrição de hardware e depois verifique seu desempenho na sua tarefa.',
            '**Baixar um modelo grande sem verificar a RAM disponível** -- um download de ~17-20 GB leva de 30 a 60 minutos em uma internet residencial típica. Execute `free -h` (Linux) ou verifique o Monitor de Atividade (macOS) antes de baixar modelos grandes. Se não houver RAM suficiente, o Ollama começará a fazer offload para a CPU, reduzindo a velocidade para 2-5 tok/s.',
        ...
        faqSection: {
          ...
          faqs: [
            { q: 'Qual é o melhor LLM local em 2026?', a: 'Qwen3.6 27B é o melhor no geral em julho de 2026 -- 84% no MMLU, 77% no SWE-bench Verified, ~17 GB de RAM, 201 idiomas incluindo pt-BR e contexto 262K. Para casos específicos: Gemma 4 26B-A4B para raciocínio e matemática (89% AIME 2026, ~15 GB de RAM), Qwen2.5-Coder 7B para código (~5 GB de RAM), Phi-4-mini para configurações apenas com CPU (~2,5 GB de RAM), e Gemma 4 E2B para a menor pegada de RAM (~2 GB de RAM).' },
            { q: 'Qual LLM local tem melhor suporte em português?', a: 'Qwen3.6 27B tem o melhor suporte nativo para português brasileiro, com treinamento em 201 idiomas incluindo pt-BR. Para máquinas pequenas, Gemma 4 E2B também funciona bem em português.' },
            { q: 'Qual LLM local é melhor para conformidade com a LGPD?', a: 'Qualquer modelo desta lista rodando localmente satisfaz os requisitos da LGPD (Lei nº 13.709/2018) -- nenhum dado sai da infraestrutura da organização. Gemma 4 (licença Gemma) e Qwen3.6 27B (licença Qwen) são escolhas seguras para uso corporativo, ambas permitindo uso comercial.' },
            { q: 'Posso usar LLMs locais sem GPU?', a: 'Sim. Phi-4-mini (~2,5 GB) roda em CPU a 30-50 tok/s; Gemma 4 E2B (~2 GB) também roda bem apenas com CPU. Qwen3.6 27B e Gemma 4 26B-A4B funcionam melhor com uma GPU de 24 GB de VRAM.' },
            { q: 'Quanta RAM preciso para o Qwen3.6 27B?', a: 'O Qwen3.6 27B requer aproximadamente 17 GB de RAM com quantização Q4_K_M. Uma GPU com 24 GB de VRAM (RTX 4090, RTX 3090) ou um Mac com 24 GB+ de memória unificada roda com folga; fica apertado em placas de 16 GB. Ainda não existe uma tag oficial na biblioteca do Ollama -- use um GGUF da comunidade com `ollama pull batiai/qwen3.6-27b:q4`.' },
        ...
        notSureLocalIsRight: {
          ...
          content: [
            'Antes de escolher entre Qwen3.6 27B, Gemma 4 ou Qwen2.5-Coder, confirme que a inferência local realmente atende às suas necessidades. **[Compare LLMs locais com APIs em nuvem para entender a troca completa](https://www.promptquorum.com/pt/local-llms/local-llm-limitations)** -- você pode descobrir que uma API em nuvem é mais barata, mais rápida ou mais prática para o seu caso de uso específico, especialmente se precisar de acesso a informações em tempo real ou desempenho de raciocínio de ponta.',
    ...
    ja: {
      ...
      sections: {
        ...
        faqSection: {
          ...
          title: 'よくある質問',
          faqs: [
            { q: '2026年の最高のローカルLLMは何ですか？', a: 'Qwen3.6 27Bが2026年7月時点の総合最高のローカルLLMです -- MMLU 84%、SWE-bench Verified 77%、Q4_K_Mで約17GB RAM、201言語対応、262Kコンテキスト。用途別では、推論と数学にはGemma 4 26B-A4B（AIME 2026 89%、約15GB RAM）、コーディングにはQwen2.5-Coder 7B（約5GB RAM）、CPU専用にはPhi-4-mini（約2.5GB RAM）、最小RAMフットプリントにはGemma 4 E2B（約2GB RAM）が最適です。' },
            { q: 'Qwen3.6 27Bにはどれくらいのメモリが必要ですか？', a: 'Qwen3.6 27BはQ4_K_M量子化で約17GBのRAMが必要です。24GBのVRAMを搭載したGPU（RTX 4090、RTX 3090）や24GB以上のユニファイドメモリを持つMacであれば余裕があり、16GBのカードでは厳しくなります。Ollama公式ライブラリタグはまだないため、`ollama pull batiai/qwen3.6-27b:q4`でコミュニティ提供のGGUFを利用します。16GB未満の場合はMoEバリアントのQwen3.6-35B-A3Bまたはより小型のモデルを使用してください。' },
            { q: 'Gemma 4 26B-A4BはQwen3.6 27Bより優れていますか？', a: '推論と数学のタスクではそうです -- Gemma 4 26B-A4BはAIME 2026で89%を達成し、Mixture-of-Experts設計により4Bモデルに近い速度で生成します。汎用タスク（文章作成、分析、多言語、コーディング隣接作業）ではQwen3.6 27Bの方が幅広く対応でき、262Kと大きなコンテキストウィンドウを持ちます。Gemma 4 26B-A4Bは約15GB RAM、Qwen3.6 27Bは約17GB RAMを必要とし、両モデルとも活性パラメータ数に関わらず全パラメータをメモリに読み込みます。' },
            { q: '8GB RAM向けの最高のローカルLLMは何ですか？', a: 'Phi-4-miniが8GB RAMマシン向けの推奨モデルです -- Q4_K_Mで約2.5〜3.5GBに収まり、他のアプリケーション用に十分な余裕を残しながら、2倍のサイズのモデルに匹敵する命令追従品質を維持します。Qwen3.6 27B（約17GB）もGemma 4 26B-A4B（約15GB）も8GBマシンには収まらず、16〜24GB以上の帯域が必要です。' },
            ...
            { q: 'これらのモデルは商用利用に無料で使えますか？', a: 'はい、5つのモデルはすべてオープンウェイトで商用利用が許可されています：Qwen3.6 27BとQwen2.5-CoderはQwenライセンス（商用利用可）、Gemma 4（26B-A4BとE2Bの両方）はGoogleのGemmaライセンス（許容される用途の制限付きで商用利用可）、Phi-4-miniはMITライセンスです。導入前に必ずご自身の管轄区域におけるライセンス条項を確認してください。' },
            { q: 'Q4_K_M量子化とは何ですか？', a: 'Q4_K_Mはllama.cppとOllamaが提供する4ビット量子化方式です。モデルの重みを16ビットから4ビット精度に圧縮し、Qwen3.6 27Bを約55.6GB（フルBF16精度）から約17GBまで、最小限の品質低下で削減します。「Q4」は重みごとの4ビット精度を意味し、「K_M」は重要な重みパターンを保持する特定のバリアント（K-quants方式）です。初心者にはQ4_K_Mが推奨デフォルトです：速度、RAM使用量、出力品質のバランスが取れています。Ollamaは自動的にQ4_K_Mを適用するため、手動で設定する必要はありません。' },
        ...
        commonMistakes: {
          ...
          items: [
            ...
            '商用利用のライセンス制限を確認し忘れる。',
            '異なるハードウェア階層間でモデルを比較する -- Qwen3.6 27B（MMLU 84%）はQwen2.5-Coder 7B（HumanEval 88%）と根本的に異なるRAM要件（約17GB対約5GB）のため直接「競合」しません。ハードウェア制約に合うモデルを選び、その上でタスクでの性能を確認してください。',
    ...
    zh: {
      ...
      sections: {
        ...
        faqSection: {
          ...
          title: '常见问题',
          faqs: [
            { q: '2026年最佳的本地大语言模型是什么？', a: 'Qwen3.6 27B是2026年7月综合性能最佳的本地大语言模型——MMLU 84%，SWE-bench Verified 77%，Q4_K_M量化下约需17GB RAM，支持201种语言，262K上下文。针对具体场景：推理和数学任务首选Gemma 4 26B-A4B（AIME 2026得分89%，约需15GB RAM），代码任务首选Qwen2.5-Coder 7B（约需5GB RAM），仅CPU环境首选Phi-4-mini（约需2.5GB RAM），最小内存占用首选Gemma 4 E2B（约需2GB RAM）。' },
            { q: '运行Qwen3.6 27B需要多少内存？', a: 'Qwen3.6 27B在Q4_K_M量化下大约需要17GB内存。配备24GB显存的GPU（RTX 4090、RTX 3090）或拥有24GB以上统一内存的Mac可以轻松运行；16GB显卡则较为紧张。目前Ollama官方库还没有正式标签——可通过`ollama pull batiai/qwen3.6-27b:q4`使用社区提供的GGUF版本。如果硬件内存低于16GB，请改用MoE变体Qwen3.6-35B-A3B或更小的模型。' },
            { q: 'Gemma 4 26B-A4B比Qwen3.6 27B更好吗？', a: '在推理和数学任务上是的——Gemma 4 26B-A4B在AIME 2026上得分89%，得益于Mixture-of-Experts设计，生成速度接近4B模型。对于通用任务（写作、分析、多语言、代码相关工作），Qwen3.6 27B能力更全面，且拥有更大的262K上下文窗口。Gemma 4 26B-A4B约需15GB RAM，Qwen3.6 27B约需17GB RAM——两者都会将全部参数加载到内存中，无论激活参数数量多少。' },
            { q: '8GB内存下最佳的本地大语言模型是什么？', a: 'Phi-4-mini是8GB内存机器的推荐选择——在Q4_K_M量化下仅占用约2.5-3.5GB，为其他应用留出充足空间，同时保持与两倍体积模型相当的指令遵循质量。Qwen3.6 27B（约17GB）和Gemma 4 26B-A4B（约15GB）都无法在8GB机器上运行；这些需要16-24GB以上的档位。' },
            ...
            { q: '这些模型可以免费商用吗？', a: '可以，五个模型均为开源权重，允许商业使用：Qwen3.6 27B和Qwen2.5-Coder采用Qwen许可证（允许商用），Gemma 4（26B-A4B和E2B均适用）采用Google的Gemma许可证（在可接受使用限制下允许商用），Phi-4-mini采用MIT许可证。部署前请务必核实您所在司法管辖区的具体许可条款。' },
            { q: 'Q4_K_M量化是什么意思？', a: 'Q4_K_M是llama.cpp和Ollama提供的4位量化方案。它将模型权重从16位压缩到4位精度，将Qwen3.6 27B从约55.6GB（完整BF16精度）压缩到约17GB，同时质量损失极小。"Q4"表示每个权重4位精度；"K_M"是一种保留重要权重模式的特定变体（K-quants方法）。对于初学者，Q4_K_M是推荐的默认设置：它在速度、内存使用和输出质量之间取得平衡。Ollama会自动应用Q4_K_M，无需手动设置。' },
            ...
            { q: '这些模型与当前的前沿云端模型相比如何？', a: 'Qwen3.6 27B和Gemma 4 26B-A4B在纯文本基准测试上接近以往的前沿云端模型，但当前的前沿云端模型在复杂推理、视觉任务和真实世界指令遵循方面仍然领先。对于纯文本工作（分析、编码、写作），本地模型具有竞争力，并提供隐私保护和零延迟。需要最大能力或多模态任务时选择前沿云端模型；注重隐私、成本和速度时选择本地模型。' },
          ],
        },
        commonMistakes: {
          title: '常见错误',
          items: [
            ...
            '未在特定用例上部署前测试模型输出。',
            '忘记检查商业使用的许可证限制。',
            '在不同硬件档位之间比较模型----Qwen3.6 27B的MMLU 84%不能直接与Qwen2.5-Coder 7B的HumanEval 88%"竞争"，因为它们所需的内存（约17GB对约5GB）完全不同。请先选择符合您硬件条件的模型，再验证其在具体任务上的表现。',
            '下载大型模型前未确认可用内存----约17-20GB的下载在普通家庭网络下需要30-60分钟。下载大型模型前请运行`free -h`（Linux）或查看活动监视器（macOS）。如果可用内存不足，Ollama将开始CPU卸载，速度会降至每秒2-5个标记。',
  ...
  sections: {
    ...
    whichModel: {
      ...
      items: [
        ...
        '**비영어권 언어**: Qwen3.6 27B(201개 언어 지원) -- [Qwen vs Llama vs Mistral](/ko/local-llms/qwen-vs-llama-vs-mistral) 참조.',
      ],
    ...
    notSureLocalIsRight: {
      ...
      content: [
        'Qwen3.6 27B, Gemma 4 26B-A4B 중에서 선택하기 전에 로컬 추론이 실제로 귀하의 요구에 부합하는지 확인하십시오. **[로컬 LLM과 클라우드 API를 비교하여 전체 트레이드오프를 이해하십시오](https://www.promptquorum.com/ko/local-llms/local-llm-limitations)** -- 특히 실시간 정보 액세스나 프론티어 수준의 추론 성능이 필요한 경우 클라우드 API가 더 저렴하고 빠르며 실용적일 수 있습니다.',
