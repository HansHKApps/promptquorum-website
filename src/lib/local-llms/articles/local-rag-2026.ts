...
schema: {
    ...
    ar: {
      ...
      intro: 'يُتيح التوليد المعزَّز بالاسترجاع (RAG) لنموذج LLM المحلي الإجابة عن أسئلة حول مستنداتك الخاصة. ترفع ملفات PDF وملفات نصية، ويحوّلها النظام إلى embeddings، ويخزّنها في قاعدة بيانات متجهية، ويسترجع الأجزاء ذات الصلة عند الإجابة عن الأسئلة. اعتبارًا من أبريل 2026، RAG المحلي جاهز للإنتاج ويلغي تكاليف API.',
...
schema: {
  ...
  ko: {
    ...
    heroImage: '/images/local-rag-2026-overview-hero-ko.webp',
      ...
      intro: 'Retrieval-Augmented Generation (RAG)을 사용하면 로컬 LLM이 사용자 본인의 문서에 대한 질문에 답변할 수 있습니다. PDF와 텍스트 파일을 업로드하면 시스템이 이를 임베딩으로 변환하고 벡터 데이터베이스에 저장한 후, 질문에 답변할 때 관련 청크를 검색합니다. 2026년 4월 현재, 로컬 RAG는 프로덕션 환경에서 사용 가능한 수준이며 API 비용을 완전히 제거합니다.',
