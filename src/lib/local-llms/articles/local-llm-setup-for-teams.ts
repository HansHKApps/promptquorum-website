...
export const article: Partial<Record<Language, LLMArticle>> = {
    en: {
      ...
      heroImage: '/images/local-llm-setup-for-teams-overview-hero-en.webp',
      seoTitle: 'Local LLM Server for Teams: Access Control & Cost Tracking',
      intro: '**Deploy a shared local LLM server for 5-20 team members using vLLM + nginx load balancer.** Team-scale inference typically costs $50/month (electricity) vs. $1,000+/month (cloud APIs). This guide covers multi-user access, role-based permissions, usage metering, and cost attribution.',
      ...
      leadAnswerBlock: '**Deploy a shared local LLM server for 5-20 team members using vLLM + nginx load balancer. Team-scale inference typically costs $50/month (electricity) vs. $1,000+/month on cloud APIs.**',
...
schema: {
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'A shared local LLM server for a 5-20 person team, built on vLLM plus an nginx load balancer, costs roughly $50/month in electricity versus $1,000+/month for equivalent cloud API usage, with setup taking a day for a single server or up to a week for a redundant cluster.' },
            { type: 'plain-terms', text: 'Instead of every team member paying for their own AI subscription, a company can run one shared AI server that everyone connects to -- similar to a company file server. It costs more upfront (buying the hardware) but becomes much cheaper per month than paying a cloud AI provider for the same usage, especially once you have more than a handful of regular users.' },
          ],
          items: [
    ...
    de: {
      ...
      heroImage: '/images/local-llm-setup-for-teams-overview-hero-de.webp',
      seoTitle: 'Lokales LLM-Setup für Teams 2026: Self-Hosted KI',
      intro: '**Stellen Sie einen gemeinsamen lokalen LLM-Server für 5-20 Team-Mitglieder mit vLLM + nginx Load Balancer bereit.** Team-Inferenz kostet typischerweise 50 €/Monat (Strom) vs. 1.000+€/Monat (Cloud-APIs). Dieser Leitfaden behandelt Multi-User-Zugriff, rollenbasierte Berechtigungen, Nutzungsmessung und Kostenattribution.',
      ...
      publishDate: '2026-04-05',
      dateModified: '2026-08-28',
      leadAnswerBlock: '**Stellen Sie einen gemeinsamen lokalen LLM-Server für 5-20 Team-Mitglieder mit vLLM + nginx Load Balancer bereit. Team-Inferenz kostet typischerweise 50 €/Monat (Strom) vs. 1.000+€/Monat (Cloud-APIs).**',
...
schema: {
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'Ein gemeinsamer lokaler LLM-Server für ein 5-20-köpfiges Team, aufgebaut auf vLLM und einem nginx-Load-Balancer, kostet etwa 50 €/Monat an Strom gegenüber 1.000+ €/Monat für vergleichbare Cloud-API-Nutzung -- die Einrichtung dauert einen Tag für einen einzelnen Server oder bis zu einer Woche für einen redundanten Cluster.' },
            { type: 'plain-terms', text: 'Statt dass jedes Teammitglied sein eigenes KI-Abo bezahlt, kann ein Unternehmen einen gemeinsamen KI-Server betreiben, mit dem sich alle verbinden -- ähnlich einem Firmen-Dateiserver. Die Anfangsinvestition (Hardware) ist höher, aber die monatlichen Kosten sind deutlich niedriger als bei einem Cloud-KI-Anbieter für dieselbe Nutzung, besonders sobald mehr als eine Handvoll regelmäßiger Nutzer vorhanden ist.' },
          ],
          items: [
    ...
    fr: {
      ...
      heroImage: '/images/local-llm-setup-for-teams-overview-hero-fr.webp',
      seoTitle: 'Serveur LLM local pour équipes: Contrôle d\'accès et attribution des coûts',
      intro: '**Déployez un serveur LLM local partagé pour 5-20 membres d\'équipe en utilisant vLLM + nginx load balancer.** L\'inférence à l\'échelle d\'une équipe coûte généralement 50 €/mois (électricité) contre 1 000+€/mois (API cloud). Ce guide couvre l\'accès multi-utilisateurs, les permissions basées sur les rôles, la mesure d\'utilisation et l\'attribution des coûts.',
      ...
      publishDate: '2026-04-05',
      dateModified: '2026-08-28',
      leadAnswerBlock: '**Déployez un serveur LLM local partagé pour 5-20 membres d\'équipe en utilisant vLLM + nginx load balancer. L\'inférence à l\'échelle d\'une équipe coûte généralement 50 €/mois (électricité) contre 1 000+€/mois (API cloud).**',
...
schema: {
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: "Un serveur LLM local partagé pour une équipe de 5 à 20 personnes, construit sur vLLM et un équilibreur de charge nginx, coûte environ 50 €/mois en électricité contre 1 000+ €/mois pour un usage équivalent d'API cloud, avec une installation prenant une journée pour un serveur unique ou jusqu'à une semaine pour un cluster redondant." },
            { type: 'plain-terms', text: "Plutôt que chaque membre de l'équipe paie son propre abonnement IA, une entreprise peut exploiter un seul serveur IA partagé auquel tout le monde se connecte -- comme un serveur de fichiers d'entreprise. Cela coûte plus cher au départ (achat du matériel) mais devient bien moins cher par mois qu'un fournisseur d'IA cloud pour le même usage, surtout au-delà d'une poignée d'utilisateurs réguliers." },
          ],
          items: [
    ...
    ja: {
      ...
      heroImage: '/images/local-llm-setup-for-teams-overview-hero-ja.webp',
      seoTitle: 'チームローカルLLMサーバー: アクセス制御とコスト追跡',
      intro: '**vLLM + nginx ロードバランサーを使用して、5～20人のチームメンバー向けに共有ローカルLLMサーバーをデプロイします。** チーム規模の推論コストは通常月額50ドル（電力）対クラウドAPI月額1,000ドル以上です。このガイドはマルチユーザーアクセス、ロールベース権限、使用率測定、コスト配分をカバーしています。',
      metaDescription: 'vLLM、nginx、アクセス制御、利用状況追跡を使用したチームローカルLLMサーバー。マルチユーザーセットアップ、コスト比較、ロールベース権限。',
      publishDate: '2026-04-05',
      dateModified: '2026-08-28',
      leadAnswerBlock: '**vLLM + nginx ロードバランサーを使用して、5～20人のチームメンバー向けに共有ローカルLLMサーバーをデプロイします。チーム規模の推論コストは通常月額50ドル（電力）対クラウドAPI月額1,000ドル以上です。**',
...
schema: {
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: '5～20人のチーム向けの共有ローカルLLMサーバーは、vLLMとnginxロードバランサーで構築すると、電力代が月額約50ドルで済み、同等のクラウドAPI利用の月額1,000ドル以上と比べてはるかに安く、単一サーバーなら1日、冗長化クラスターでも最大1週間でセットアップできます。' },
            { type: 'plain-terms', text: 'チームメンバー全員が個別にAIサブスクリプションを支払う代わりに、企業は全員が接続する共有AIサーバーを1台運用できます -- 社内ファイルサーバーのようなものです。初期費用（ハードウェア購入）は高くなりますが、同じ利用量に対してクラウドAIプロバイダーに支払うよりも月々のコストははるかに安くなります。特に定期利用者が数人以上いる場合に効果的です。' },
          ],
          items: [
    ...
    zh: {
      ...
      heroImage: '/images/local-llm-setup-for-teams-overview-hero-zh.webp',
      seoTitle: '团队本地LLM服务器: 访问控制与成本追踪',
      intro: '**使用vLLM + nginx负载均衡器为5-20名团队成员部署共享本地LLM服务器。** 团队规模推理成本通常为$50/月（电力）对比云API $1,000+/月。本指南涵盖多用户访问、基于角色的权限、使用计量和成本分配。',
      metaDescription: '使用vLLM、nginx、访问控制和使用跟踪的团队本地LLM服务器。多用户设置、成本比较和基于角色的权限。',
      publishDate: '2026-04-05',
      dateModified: '2026-08-28',
      leadAnswerBlock: '**使用vLLM + nginx负载均衡器为5-20名团队成员部署共享本地LLM服务器。团队规模推理成本通常为$50/月（电力）对比云API $1,000+/月。**',
...
schema: {
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: '为5-20人团队搭建的共享本地LLM服务器，基于vLLM和nginx负载均衡器构建，电费成本约为每月50美元，而同等的云API使用成本则超过每月1,000美元；单台服务器一天即可搭建完成，冗余集群最多需要一周。' },
            { type: 'plain-terms', text: '与其让每位团队成员各自订阅AI服务，公司可以运行一台所有人共用的AI服务器——类似于公司文件服务器。前期成本（购买硬件）更高，但每月成本远低于向云端AI提供商为相同使用量付费，尤其是在常规用户超过几个人之后。' },
          ],
          items: [
    ...
    es: {
      ...
      publishDate: '2026-04-05',
      dateModified: '2026-08-28',
      leadAnswerBlock: '**Despliega un servidor LLM local compartido para 5-20 miembros de equipo usando vLLM + nginx load balancer. La inferencia a escala de equipo suele costar $50/mes (electricidad) frente a $1.000+/mes en APIs en la nube.**',
...
schema: {
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'Un servidor LLM local compartido para un equipo de 5 a 20 personas, construido sobre vLLM y un balanceador de carga nginx, cuesta aproximadamente $50/mes en electricidad frente a $1.000+/mes por un uso equivalente de APIs en la nube, con una configuración que toma un día para un solo servidor o hasta una semana para un clúster redundante.' },
            { type: 'plain-terms', text: 'En lugar de que cada miembro del equipo pague su propia suscripción de IA, una empresa puede ejecutar un único servidor de IA compartido al que todos se conectan, similar a un servidor de archivos corporativo. Cuesta más por adelantado (comprar el hardware), pero resulta mucho más barato al mes que pagar a un proveedor de IA en la nube por el mismo uso, especialmente una vez que hay más de un puñado de usuarios habituales.' },
          ],
          items: [
    ...
    ar: {
      ...
      heroImage: '/images/local-llm-setup-for-teams-overview-hero-ar.webp',
      seoTitle: 'خادم ⁨LLM⁩ محلي للفرق: ⁨vLLM⁩ و⁨nginx⁩ لـ⁨5⁩–⁨20⁩ مستخدمًا',
      intro: '**انشر خادم LLM محليًا مشتركًا لـ5-20 عضوًا في الفريق باستخدام vLLM + موازن أحمال nginx.** يكلّف الاستدلال على مستوى الفريق عادةً 50$ شهريًا (كهرباء) مقابل 1,000$+ شهريًا (واجهات API السحابية). يغطي هذا الدليل الوصول متعدد المستخدمين والأذونات القائمة على الأدوار وقياس الاستخدام وعزو التكاليف.',
      ...
      publishDate: '2026-04-05',
      dateModified: '2026-08-28',
      leadAnswerBlock: '**انشر خادم LLM محليًا مشتركًا لـ5-20 عضوًا في الفريق باستخدام vLLM + موازن أحمال nginx. يكلّف الاستدلال على مستوى الفريق عادةً 50$ شهريًا (كهرباء) مقابل 1,000$+ شهريًا في واجهات API السحابية.**',
...
schema: {
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'خادم LLM محلي مشترك لفريق من 5-20 شخصًا، مبني على vLLM وموازن أحمال nginx، يكلّف حوالي 50$ شهريًا في الكهرباء مقابل أكثر من 1,000$ شهريًا لاستخدام مكافئ من واجهات API السحابية، ويستغرق الإعداد يومًا واحدًا لخادم مفرد أو حتى أسبوع لعنقود متكرر.' },
            { type: 'plain-terms', text: 'بدلاً من أن يدفع كل عضو في الفريق اشتراك ذكاء اصطناعي خاصًا به، يمكن للشركة تشغيل خادم ذكاء اصطناعي واحد مشترك يتصل به الجميع -- على غرار خادم ملفات الشركة. يكلّف ذلك أكثر مقدمًا (شراء العتاد) لكنه يصبح أرخص بكثير شهريًا من الدفع لمزوّد ذكاء اصطناعي سحابي مقابل نفس الاستخدام، خاصة بمجرد وجود أكثر من عدد قليل من المستخدمين المنتظمين.' },
          ],
          items: [
    ...
    pt: {
      ...
      heroImage: '/images/local-llm-setup-for-teams-overview-hero-pt.webp',
      ...
      publishDate: '2026-04-05',
      dateModified: '2026-08-28',
      leadAnswerBlock: '**Faça deploy de um servidor LLM local compartilhado para 5–20 membros de equipe usando vLLM + nginx load balancer. A inferência em escala de equipe geralmente custa R$250/mês (eletricidade) frente a R$5.000+/mês em APIs na nuvem.**',
  ...
  ko: {
    ...
    heroImage: '/images/local-llm-setup-for-teams-overview-hero-ko.webp',
      seoTitle: '팀용 로컬 LLM 서버: 접근 제어 및 비용 추적',
      intro: '**vLLM + nginx 로드 밸런서를 사용하여 5~20명의 팀원을 위한 공유 로컬 LLM 서버를 구축하십시오.** 팀 규모의 추론 비용은 일반적으로 월 $50(전기료)인 반면 클라우드 API는 월 $1,000 이상입니다. 이 가이드에서는 다중 사용자 접근, 역할 기반 권한, 사용량 측정 및 비용 귀속을 다룹니다.',
      metaDescription: 'vLLM, nginx, 접근 제어 및 사용량 추적을 갖춘 팀용 로컬 LLM 서버 설정. 다중 사용자 설정 가이드, 비용 비교 및 역할 기반 권한.',
      publishDate: '2026-04-05',
      dateModified: '2026-08-28',
      leadAnswerBlock: '**vLLM + nginx 로드 밸런서를 사용하여 5~20명의 팀원을 위한 공유 로컬 LLM 서버를 구축하십시오. 팀 규모의 추론 비용은 일반적으로 월 $50(전기료)인 반면 클라우드 API는 월 $1,000 이상입니다.**',
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: '5~20명 규모의 팀을 위한 공유 로컬 LLM 서버는 vLLM과 nginx 로드 밸런서로 구축하면 전기료가 월 약 $50에 불과하며, 동등한 클라우드 API 사용 시 월 $1,000 이상이 드는 것과 비교됩니다. 단일 서버는 하루, 이중화 클러스터는 최대 일주일이면 구축할 수 있습니다.' },
            { type: 'plain-terms', text: '팀원 각자가 개별적으로 AI 구독료를 내는 대신, 회사는 모두가 접속하는 공유 AI 서버 하나를 운영할 수 있습니다 -- 사내 파일 서버와 비슷합니다. 초기 비용(하드웨어 구입)은 더 들지만, 동일한 사용량 기준으로 클라우드 AI 제공업체에 지불하는 것보다 월간 비용이 훨씬 저렴해집니다. 특히 정기 사용자가 몇 명을 넘어서면 더욱 그렇습니다.' },
          ],
          items: [
        ...
        'architecture': {
          ...
          content: [
            ...
            '- 간단한 설정, 단일 장애 지점. 프레임워크 선택에 대해서는 [개발자를 위한 최적의 로컬 LLM 스택](/ko/local-llms/local-llm-developer-stack)을 참조하십시오.',
            '- 비용: 하드웨어 $2,500 + 월 전기료 $50.',
            '**듀얼 GPU 클러스터(10~50명):**',
            '- 2× vLLM 인스턴스(GPU당 하나) + nginx 로드 밸런서.',
            '- 20명의 동시 사용자 처리(각 10 tok/s).',
            '- 자동 장애 조치(GPU 0 장애 시 GPU 1이 계속 실행됨). 자세한 내용은 [엔터프라이즈 로컬 LLM 확장](/ko/local-llms/scaling-local-llms-enterprise)을 참조하십시오.',
        ...
        'metering': {
          ...
          content: [
            '**추적:** 사용자당 하루 생성된 토큰 수. 팀 전체를 합산하여 총 비용을 산출합니다. 프라이버시 우선 측정에 대해서는 [민감한 데이터를 위한 프라이빗 로컬 LLM](/ko/local-llms/private-local-llm-sensitive-data)을 참조하십시오.',
        ...
        'relatedReading': {
          ...
          title: '관련 자료',
          items: [
            '[개발자를 위한 최적의 로컬 LLM 스택](/ko/local-llms/local-llm-developer-stack) — 프로덕션을 위한 오픈 소스 프레임워크 선택',
            '[민감한 데이터를 위한 프라이빗 로컬 LLM](/ko/local-llms/private-local-llm-sensitive-data) — 데이터 상주 및 규정 준수 요구 사항',
            '[로컬 LLM을 통한 엔터프라이즈 규정 준수](/ko/local-llms/enterprise-compliance-local-llms) — 팀을 위한 법적 및 규제 고려 사항',
            '[엔터프라이즈 로컬 LLM 확장](/ko/local-llms/scaling-local-llms-enterprise) — 다중 리전 배포 및 이중화',
          ],
        },
        'sources': {
          id: 'sources',
