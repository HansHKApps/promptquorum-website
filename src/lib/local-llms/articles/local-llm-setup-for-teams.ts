...
schema: {
      ...
      sections: {
        ...
        'relatedReading': {
          ...
          items: [
            '[Bester lokaler LLM-Stack für Entwickler](/local-llms/local-llm-developer-stack?lang=de) — Auswahl von Open-Source-Frameworks für Produktion',
            '[Private lokale LLM für sensible Daten](/local-llms/private-local-llm-sensitive-data?lang=de) — Datenresidenz und Compliance-Anforderungen',
            '[Enterprise Compliance mit lokalen LLMs](/local-llms/enterprise-compliance-local-llms?lang=de) — Rechtliche und regulatorische Überlegungen für Teams',
            '[Skalieren lokale LLMs Enterprise](/local-llms/scaling-local-llms-enterprise?lang=de) — Multi-Region Bereitstellungen und Redundanz',
          ],
        },
        'sources': {
          id: 'sources',
      ...
      sections: {
        ...
        'architecture': {
          ...
          content: [
            ...
            '- Setup simple, point de défaillance unique. Voir [meilleur stack LLM local](/local-llms/local-llm-developer-stack?lang=fr) pour les choix de framework.',
            ...
            '- Failover automatique (si GPU 0 tombe en panne, GPU 1 continue). En savoir plus dans [scaler les LLMs locaux en enterprise](/local-llms/scaling-local-llms-enterprise?lang=fr).',
        ...
        'auth': {
          ...
          content: [
            '**Auth simple (PME < 50 utilisateurs):** Clé API par utilisateur. L\'utilisateur envoie `Authorization: Bearer $API_KEY` dans l\'en-tête de la requête. Pour la conformité, voir [conformité enterprise avec LLMs locaux](/local-llms/enterprise-compliance-local-llms?lang=fr).',
        ...
        'metering': {
          ...
          content: [
            '**Tracking:** Tokens générés par utilisateur par jour. Somme sur l\'équipe pour le coût total. Voir [LLM local privé pour données sensibles](/local-llms/private-local-llm-sensitive-data?lang=fr) pour le metering en priorité sur la confidentialité.',
        ...
        'relatedReading': {
          ...
          items: [
            '[Meilleur stack LLM local pour développeurs](/local-llms/local-llm-developer-stack?lang=fr) — Choix de frameworks open-source pour la production',
            '[LLM local privé pour données sensibles](/local-llms/private-local-llm-sensitive-data?lang=fr) — Résidence des données et exigences de conformité',
            '[Conformité enterprise avec LLMs locaux](/local-llms/enterprise-compliance-local-llms?lang=fr) — Considérations légales et réglementaires pour les équipes',
            '[Scaler les LLMs locaux en enterprise](/local-llms/scaling-local-llms-enterprise?lang=fr) — Déploiements multi-régions et redondance',
          ],
        },
        'sources': {
          id: 'sources',
      ...
      sections: {
        ...
        'architecture': {
          ...
          content: [
            ...
            '- シンプルセットアップ、単一障害点。[最高ローカルLLMスタック](/local-llms/local-llm-developer-stack?lang=ja)を参照。',
            '- コスト: $2,500ハードウェア + $50/月電力。',
            '**デュアルGPUクラスタ（10～50ユーザー）:**',
            '- 2× vLLMインスタンス（GPU当たり1つ）+ nginxロードバランサー。',
            '- 20同時ユーザー処理（ユーザーあたり10 tok/s）。',
            '- 自動フェイルオーバー（GPU 0ダウン時もGPU 1は稼働）。詳細は[エンタープライズローカルLLMスケーリング](/local-llms/scaling-local-llms-enterprise?lang=ja)を参照。',
        ...
        'auth': {
          ...
          content: [
            '**シンプル認証（SMB < 50ユーザー）:** ユーザーあたりAPIキー。ユーザーがリクエストヘッダーで`Authorization: Bearer $API_KEY`を送信します。コンプライアンスについては[エンタープライズローカルLLMコンプライアンス](/local-llms/enterprise-compliance-local-llms?lang=ja)を参照。',
        ...
        'metering': {
          ...
          content: [
            '**トラッキング:** ユーザーあたり/日生成トークン。チーム全体で合計コストを計算。[プライベートローカルLLM機密データ用](/local-llms/private-local-llm-sensitive-data?lang=ja)を参照。',
        ...
        'relatedReading': {
          ...
          title: '関連資料',
          items: [
            '[開発者向け最高ローカルLLMスタック](/local-llms/local-llm-developer-stack?lang=ja) — プロダクション用オープンソースフレームワーク選択',
            '[機密データ用プライベートローカルLLM](/local-llms/private-local-llm-sensitive-data?lang=ja) — データレジデンスとコンプライアンス要件',
            '[ローカルLLMエンタープライズコンプライアンス](/local-llms/enterprise-compliance-local-llms?lang=ja) — チーム向け法務・規制対応',
            '[ローカルLLMエンタープライズスケーリング](/local-llms/scaling-local-llms-enterprise?lang=ja) — マルチリージョンデプロイと冗長性',
          ],
        },
        'sources': {
          id: 'sources',
      ...
      sections: {
        ...
        'architecture': {
          ...
          content: [
            ...
            '- 1× RTX 4090 + 64GB RAM + 1TB SSD。',
            '- 处理10个并发用户（每用户5令牌/秒）。',
            '- 简单设置，单一故障点。参见[最佳本地LLM堆栈](/local-llms/local-llm-developer-stack?lang=zh)了解框架选择。',
            '- 成本: $2,500硬件 + $50/月电力。',
            '**双GPU集群（10-50用户）:**',
            '- 2× vLLM实例（每个GPU一个）+ nginx负载均衡器。',
            '- 处理20个并发用户（每用户10令牌/秒）。',
            '- 自动故障转移（GPU 0故障时GPU 1继续）。详见[企业本地LLM扩展](/local-llms/scaling-local-llms-enterprise?lang=zh)。',
            '- 成本: $5,000硬件 + $100/月电力。',
            '**Redis缓存层（可选）:**',
            '- 缓存常见提示（系统消息、模板）。',
            '- 重复查询延迟减少30%。',
        ...
        'auth': {
          ...
          title: '如何设置用户认证和访问控制?',
          content: [
            '**简单认证（中小企业 < 50用户）:** 每用户API密钥。用户在请求标头中发送`Authorization: Bearer $API_KEY`。关于合规性，参见[企业本地LLM合规性](/local-llms/enterprise-compliance-local-llms?lang=zh)。',
        ...
        'metering': {
          ...
          title: '如何追踪成本分配和使用计量?',
          content: [
            '**追踪:** 每用户/日生成的令牌。跨团队汇总以获得总成本。参见[敏感数据专用本地LLM](/local-llms/private-local-llm-sensitive-data?lang=zh)了解隐私优先的计量。',
        ...
        'relatedReading': {
          ...
          title: '相关阅读',
          items: [
            '[开发者最佳本地LLM堆栈](/local-llms/local-llm-developer-stack?lang=zh) — 生产环境开源框架选择',
            '[敏感数据专用本地LLM](/local-llms/private-local-llm-sensitive-data?lang=zh) — 数据驻留和合规要求',
            '[企业本地LLM合规性](/local-llms/enterprise-compliance-local-llms?lang=zh) — 团队的法律和监管考虑',
            '[企业本地LLM扩展](/local-llms/scaling-local-llms-enterprise?lang=zh) — 多地域部署和冗余',
          ],
        },
        'sources': {
          id: 'sources',
