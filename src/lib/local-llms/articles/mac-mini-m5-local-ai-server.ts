  ...
  en: {
    ...
    sections: {
      ...
      hardware: {
        ...
        note: '★ Recommended for this use case. The 64GB memory ceiling is the deciding factor: running an LLM, Whisper STT, a RAG pipeline, and a voice assistant simultaneously needs all four models resident in memory at once, which the 32GB M6 cannot fit. Storage planning: Llama 3.3 8B Q4 ~5 GB per model, Whisper large-v3 ~3 GB, embedding model ~0.5 GB, ChromaDB with 10K docs ~2 GB. Typical 5-model setup: 50-80 GB. Minimum 512 GB SSD; 1 TB for power users. Apple has not published per-memory-tier pricing beyond each chip\'s starting price — check apple.com for exact configuration cost above the base tier.',
  ...
  de: {
    ...
    sections: {
      ...
      hardware: {
        ...
        note: '★ Für diesen Einsatzzweck empfohlen. Die 64-GB-Obergrenze ist entscheidend: LLM, Whisper STT, RAG-Pipeline und Sprachassistent gleichzeitig zu betreiben erfordert alle vier Modelle gleichzeitig im Speicher — das schafft der 32-GB-gedeckelte M6 nicht. Speicherplanung: Llama 3.3 8B Q4 ca. 5 GB pro Modell, Whisper large-v3 ca. 3 GB, Embedding-Modell ca. 0,5 GB, ChromaDB mit 10.000 Dokumenten ca. 2 GB. Typisches 5-Modell-Setup: 50–80 GB. Mindestens 512 GB SSD; 1 TB für Power-User. Apple hat noch keine Preise für Speicherkonfigurationen oberhalb der Basis veröffentlicht — genaue Konfigurationspreise auf apple.com prüfen.',
        image: '/images/mac-mini-m5-local-ai-server-memory-tier-capacity-en.svg',
  ...
  fr: {
    ...
    sections: {
      ...
      hardware: {
        ...
        note: '★ Recommandé pour cet usage. La limite de 64 Go est déterminante : faire tourner LLM, Whisper STT, pipeline RAG et assistant vocal simultanément exige que les quatre modèles résident en mémoire en même temps, ce que le M6 limité à 32 Go ne peut pas accueillir. Planification stockage : Llama 3.3 8B Q4 env. 5 Go par modèle, Whisper large-v3 env. 3 Go, modèle d\'embedding env. 0,5 Go, ChromaDB avec 10 000 documents env. 2 Go. Setup 5 modèles typique : 50–80 Go. SSD 512 Go minimum ; 1 To pour les utilisateurs avancés. Apple n\'a pas publié de tarifs par palier de mémoire au-delà du prix de base — vérifier le prix exact configuré sur apple.com.',
        image: '/images/mac-mini-m5-local-ai-server-memory-tier-capacity-en.svg',
  ...
  zh: {
    ...
    sections: {
      ...
      hardware: {
        ...
        note: '★针对此用途推荐。64GB内存上限是关键：同时运行LLM、Whisper STT、RAG管道和语音助手，需要四个模型同时常驻内存，而32GB上限的M6无法满足这一需求。存储规划：Llama 3.3 8B Q4约5GB/模型，Whisper large-v3约3GB，嵌入模型约0.5GB，含1万文档的ChromaDB约2GB。典型5模型配置：占用50–80GB。最低推荐512GB SSD；高级用户选1TB。Apple尚未公布基础款以上内存配置的具体定价——请前往apple.com查询实际配置价格。',
        image: '/images/mac-mini-m5-local-ai-server-memory-tier-capacity-en.svg',
  ...
  ar: {
    ...
    sections: {
      ...
      hardware: {
        ...
        note: '★ موصى به لهذا الاستخدام. سقف 64 GB هو العامل الحاسم: تشغيل LLM وWhisper STT وخط معالجة RAG ومساعد صوتي في آنٍ واحد يتطلب بقاء النماذج الأربعة في الذاكرة معًا في الوقت نفسه، وهو ما لا يستطيع M6 المحدود بـ 32 GB توفيره. تخطيط التخزين: Llama 3.3 8B Q4 ~5 GB لكل نموذج، Whisper large-v3 ~3 GB، نموذج embeddings ~0.5 GB، ChromaDB مع 10,000 مستند ~2 GB. تهيئة معتادة بـ 5 نماذج: 50–80 GB. الحد الأدنى 512 GB SSD؛ 1 TB للمستخدمين المتقدمين. لم تنشر Apple بعد أسعار تهيئات الذاكرة فوق الفئة الأساسية — راجع apple.com للسعر الدقيق حسب التهيئة.',
        image: '/images/mac-mini-m5-local-ai-server-memory-tier-capacity-en.svg',
  ...
  ko: {
    ...
    sections: {
      ...
      hardware: {
        ...
        note: '★ 이 용도에 권장. 64GB 메모리 상한이 결정적 요소입니다: LLM, Whisper STT, RAG 파이프라인, 음성 어시스턴트를 동시에 실행하려면 네 개 모델이 동시에 메모리에 상주해야 하며, 이는 32GB로 제한된 M6가 감당할 수 없습니다. 스토리지 계획: Llama 3.3 8B Q4 ~모델당 5 GB, Whisper large-v3 ~3 GB, 임베딩 모델 ~0.5 GB, 문서 10,000건의 ChromaDB ~2 GB. 일반적인 5모델 설정: 50-80 GB. 최소 512 GB SSD 권장, 고급 사용자는 1 TB. Apple은 기본형 이상의 메모리 구성 가격을 아직 공개하지 않았습니다 — 정확한 구성 가격은 apple.com에서 확인하십시오.',
        image: '/images/mac-mini-m5-local-ai-server-memory-tier-capacity-en.svg',
