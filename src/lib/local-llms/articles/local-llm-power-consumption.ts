    ...
    heroImage: '/images/local-llm-power-consumption-overview-hero-ko.webp',
      ...
      sections: {
        ...
        relatedReading: {
          ...
          title: '관련 읽을거리',
          items: [
            '[로컬 LLM 하드웨어 가이드 2026](/ko/local-llms/local-llm-hardware-guide-2026) — 완전한 하드웨어 설정 및 부품 선택.',
            '[로컬 LLM을 위한 최고의 GPU](/ko/local-llms/best-gpus-for-local-llms) — 전 등급 GPU 선택 가이드.',
            '[노트북 vs 데스크톱 로컬 LLM](/ko/local-llms/laptop-vs-desktop-local-llm) — 모바일 추론의 전력 및 열 트레이드오프.',
            '[로컬 LLM에 필요한 VRAM 용량은?](/ko/local-llms/how-much-vram-local-llm) — 모델별 메모리 요구 사항.',
            '[LLM 양자화 설명](/ko/local-llms/llm-quantization-explained) — 양자화를 통한 VRAM 및 전력 절감.',
            '[로컬 LLM에서 Mac vs Windows vs Linux](/ko/local-llms/mac-vs-windows-vs-linux-local-llm) — OS 전력 소비 및 효율성 비교.',
            '[로컬 AI 서버로서 Mac Mini M5](/ko/local-llms/mac-mini-m5-local-ai-server) — 연간 $35 전기 요금으로 상시 가동 LLM 스택: 전력 효율성 사례 연구.',
            '[M5 Pro vs M5 Max LLM 벤치마크 2026](/ko/local-llms/m5-pro-max-llm-benchmarks-2026) — 모든 Apple Silicon 칩 등급의 tok/sec와 함께 전력 소비 측정.',
            '[Apple Silicon의 Whisper 2026](/ko/local-llms/apple-silicon-whisper-metal-benchmark) — M5 Pro에서 전체 시스템 전력 ~30W로 실시간의 10배 속도로 음성 변환.',
          ],
        },
        sources: {
          id: 'sources',
