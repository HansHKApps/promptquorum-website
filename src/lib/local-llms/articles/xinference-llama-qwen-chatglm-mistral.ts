...
export const article: Partial<Record<Language, LLMArticle>> = {
  en: {
    ...
    heroImage: '/images/xinference-llama-qwen-chatglm-mistral-overview-hero-en.webp',
    ...
    leadAnswerBlock: '**Install Xinference with `pip install "xinference[all]"`, start it with `xinference-local`, then run `xinference launch --model-name llama-3.1-instruct --model-engine transformers --model-size-in-billions 8`.** Xinference natively supports Llama 3, Qwen2.5, ChatGLM4, Mistral, and 30+ other families — all served through an OpenAI-compatible API at localhost:9997.',
    quickAnswerTop: {
      en: {
        question: 'What models does Xinference support?',
        answer: 'Xinference natively supports Llama 3.3 (8B and 70B), Qwen2.5 (0.5B to 72B), ChatGLM4 (9B), Mistral Small v0.3, Mixtral 8x22B, and 30+ other families. Each launches with a single `xinference launch` command and serves via an OpenAI-compatible REST API at localhost:9997.',
        bullets: [
    ...
    sections: {
      tldr: {
        ...
        isTldr: true,
        items: [
          '**Xinference serves 30+ model families through one API** — Llama 3, Qwen2.5, ChatGLM4, Mistral, embedding models, and rerankers all share the same endpoint at localhost:9997/v1.',
          ...
          '**Qwen2.5 and ChatGLM4 are the best Xinference choices for CJK tasks** — both run in ~6–7 GB of VRAM and outperform comparable EN-only models on Chinese and Japanese benchmarks.',
      ...
      whatIs: {
        ...
        snippetBlocks: [
          { type: 'one-sentence', text: 'Xinference is an open-source inference server that natively supports Llama 3, Qwen2.5, ChatGLM4, Mistral, and 30+ other model families through a single OpenAI-compatible API.' },
      ...
      supportedModels: {
        ...
        snippetBlocks: [
          { type: 'one-sentence', text: 'Xinference natively supports Llama 3.3 (8B/70B), Qwen2.5 (7B/72B), ChatGLM4 9B, Mistral Small v0.3, and Mixtral 8x22B — each launchable with a single CLI command.' },
        ...
        faqs: [
          ...
          { q: 'Does Xinference support Qwen2.5?', a: 'Yes. Qwen2.5 Instruct is registered as `qwen2.5-instruct`. Sizes 0.5B, 1.5B, 3B, 7B, 14B, 32B, and 72B are all available. The 7B variant runs in ~6 GB of VRAM and handles Chinese, Japanese, Korean, and English with comparable quality to Llama 3.1 8B.' },
...
print(embedding.data[0].embedding[:5])`,
      ...
      vsAlternatives: {
        ...
        columns: ['Criterion', '[Xinference](https://github.com/xorbitsai/inference)', '[Ollama](https://ollama.com)', '[vLLM](https://github.com/vllm-project/vllm)'],
        snippetBlocks: [
      ...
      faqs: {
        ...
        faqs: [
            ...
            { q: 'What models does Xinference support in 2026?', a: 'Xinference registers 200+ model configurations. The most popular in 2026 are Llama 3.1 8B/70B Instruct, Qwen2.5 8B/72B Instruct, ChatGLM4 9B, Mistral Small Instruct v0.3, and Mixtral 8x22B Instruct. Run `xinference registrations --model-type LLM` to see the full list.' },
      ...
      relatedReading: {
        ...
        items: [
          '[Llama.cpp vs Ollama vs vLLM: Which Local Inference Engine?](/local-llms/llamacpp-vs-ollama-vs-vllm) -- Llama.cpp vs Ollama vs vLLM',
          ...
          '[OpenAI-Compatible API for Local LLMs](/local-llms/local-llm-openai-compatible-api) -- OpenAI-compatible API for local LLMs',
          '[Local LLM Developer Stack](/local-llms/local-llm-developer-stack) -- local LLM developer stack',
        ],
      },
    },
    schema: {
      ...
      'description': 'Xinference supports Llama 3, Qwen2.5, ChatGLM4, and Mistral out of the box. pip install, xi launch, done — OpenAI-compatible API at localhost:9997.',
    ...
    faqSchema: {
      ...
      'mainEntity': [
          ...
          {
            '@type': 'Question',
            'name': 'Does Xinference support Qwen2.5?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Yes. Qwen2.5 Instruct is registered as `qwen2.5-instruct`. Sizes 0.5B, 1.5B, 3B, 7B, 14B, 32B, and 72B are all available. The 7B variant runs in ~6 GB of VRAM and handles Chinese, Japanese, Korean, and English with comparable quality to Llama 3.1 8B.',
            },
          },
          {
            '@type': 'Question',
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Xinference registers 200+ model configurations. The most popular in 2026 are Llama 3.1 8B/70B Instruct, Qwen2.5 8B/72B Instruct, ChatGLM4 9B, Mistral Small Instruct v0.3, and Mixtral 8x22B Instruct. Run `xinference registrations --model-type LLM` to see the full list.',
            },
          },
          {
            '@type': 'Question',
    ...
    itemListSchema: {
      ...
      'itemListElement': [
        ...
        { '@type': 'ListItem', 'position': 4, 'name': 'Qwen2.5 72B Instruct', 'description': 'Alibaba — ~40 GB VRAM, large-scale CJK tasks via vLLM' },
  ...
  es: {
    ...
    sections: {
      tldr: {
        ...
        isTldr: true,
        items: [
          '**Xinference sirve más de 30 familias de modelos a través de una única API** — Llama 3, Qwen2.5, ChatGLM4, Mistral, modelos de embeddings y rerankers comparten el mismo endpoint en localhost:9997/v1.',
          ...
          '**Qwen2.5 y ChatGLM4 son las mejores opciones de Xinference para tareas CJK** — ambos funcionan con ~6–7 GB de VRAM y superan a modelos equivalentes solo en inglés en benchmarks de chino y japonés.',
      ...
      whatIs: {
        ...
        snippetBlocks: [
          { type: 'one-sentence', text: 'Xinference es un servidor de inferencia de código abierto que soporta de forma nativa Llama 3, Qwen2.5, ChatGLM4, Mistral y más de 30 familias de modelos a través de una única API compatible con OpenAI.' },
      ...
      supportedModels: {
        ...
        snippetBlocks: [
          { type: 'one-sentence', text: 'Xinference soporta de forma nativa Llama 3.3 (8B/70B), Qwen2.5 (7B/72B), ChatGLM4 9B, Mistral Small v0.3 y Mixtral 8x22B — cada uno lanzable con un único comando CLI.' },
        ...
        faqs: [
          ...
          { q: '¿Xinference soporta Qwen2.5?', a: 'Sí. Qwen2.5 Instruct está registrado como `qwen2.5-instruct`. Los tamaños 0,5B, 1,5B, 3B, 7B, 14B, 32B y 72B están disponibles. La variante 7B funciona con ~6 GB de VRAM y gestiona chino, japonés, coreano e inglés con una calidad comparable a Llama 3.1 8B.' },
...
xinference-local --host 0.0.0.0 --port 9997`,
      ...
      launchModels: {
        ...
        codeBlock: `# Llama 3.1 8B Instruct (GPU, backend transformers)
xinference launch \\
  --model-name llama-3.1-instruct \\
  --model-engine transformers \\
  --model-size-in-billions 8

# Llama 3.1 8B Instruct (CPU, cuantización Q4_K_M)
xinference launch \\
  --model-name llama-3.1-instruct \\
  --model-engine llama.cpp \\
  --model-size-in-billions 8 \\
...
print(embedding.data[0].embedding[:5])`,
      ...
      vsAlternatives: {
        ...
        columns: ['Criterio', '[Xinference](https://github.com/xorbitsai/inference)', '[Ollama](https://ollama.com)', '[vLLM](https://github.com/vllm-project/vllm)'],
        snippetBlocks: [
      ...
      relatedReading: {
        ...
        items: [
          '[Llama.cpp vs Ollama vs vLLM: Which Local Inference Engine?](/es/local-llms/llamacpp-vs-ollama-vs-vllm) -- Llama.cpp vs Ollama vs vLLM',
          '[Cómo instalar Ollama](/es/local-llms/how-to-install-ollama) -- how to install Ollama',
          '[Qwen Deployment Guide 2026](/es/local-llms/qwen-local-deployment-guide-2026) -- Qwen local deployment guide',
          '[API compatible con OpenAI para LLMs locales](/es/local-llms/local-llm-openai-compatible-api) -- OpenAI-compatible API for local LLMs',
          '[Stack de desarrollador LLM local](/es/local-llms/local-llm-developer-stack) -- local LLM developer stack',
        ],
      },
    },
    schema: {
      ...
      'description': 'Xinference soporta Llama 3, Qwen2.5, ChatGLM4 y Mistral de forma nativa. pip install, xi launch — API compatible con OpenAI disponible en localhost:9997.',
      'datePublished': '2026-05-23',
      'dateModified': '2026-08-28',
    ...
    faqSchema: {
      ...
      'mainEntity': [
          ...
          {
            '@type': 'Question',
            'name': '¿Xinference soporta Qwen2.5?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Sí. Qwen2.5 Instruct está registrado como `qwen2.5-instruct`. Los tamaños 0,5B, 1,5B, 3B, 7B, 14B, 32B y 72B están disponibles. La variante 7B funciona con ~6 GB de VRAM y gestiona chino, japonés, coreano e inglés con una calidad comparable a Llama 3.1 8B.',
            },
          },
          {
            '@type': 'Question',
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Xinference registra más de 200 configuraciones de modelos. Los más populares en 2026 son Llama 3.1 8B/70B Instruct, Qwen2.5 8B/72B Instruct, ChatGLM4 9B, Mistral Small Instruct v0.3 y Mixtral 8x22B Instruct. Ejecuta `xinference registrations --model-type LLM` para ver la lista completa.',
            },
          },
          {
            '@type': 'Question',
    ...
    itemListSchema: {
      ...
      'itemListElement': [
        ...
        { '@type': 'ListItem', 'position': 4, 'name': 'Qwen2.5 72B Instruct', 'description': 'Alibaba — ~40 GB VRAM, tareas CJK a gran escala via vLLM' },
  ...
  ar: {
    ...
    next_refresh_due: '2026-11-23',
    theme: 'Tools & Interfaces',
    heroImage: '/images/xinference-llama-qwen-chatglm-mistral-overview-hero-ar.webp',
    title: '⁨Xinference⁩: تشغيل ⁨Llama 3⁩ و⁨Qwen⁩ و⁨ChatGLM⁩ و⁨Mistral⁩ محليًا ⁨2026⁩',
    seoTitle: '⁨Xinference⁩: ⁨Llama 3⁩ و⁨Qwen⁩ و⁨ChatGLM⁩ و⁨Mistral⁩ محليًا',
    intro: 'Xinference (Xorbits Inference) هو إطار مفتوح المصدر يتيح تقديم Llama 3 وQwen2.5 وChatGLM4 وMistral وأكثر من 30 عائلة نماذج عبر واجهة API واحدة متوافقة مع OpenAI — يُثبَّت بأمر pip ويُطلَق باستدعاء CLI واحد. على عكس Ollama الموجّه لراحة المستخدم النهائي، صُمم Xinference للفرق التي تحتاج خدمة متعددة النماذج، ودعم عناقيد GPU، وembeddings/إعادة الترتيب جنبًا إلى جنب مع استدلال LLM. يغطي هذا الدليل عائلات النماذج المدعومة، والتثبيت، وأوامر الإطلاق لكل نموذج، وكيف يُقارن Xinference بـ Ollama وvLLM.',
    metaDescription: 'يدعم ⁨Xinference 200⁩+ نموذج منها ⁨Llama 3⁩ و⁨Qwen2.5⁩ و⁨ChatGLM4⁩ و⁨Mistral⁩ — ⁨pip install⁩ ثم ⁨xinference launch⁩، و⁨API⁩ متوافقة مع ⁨OpenAI⁩ على ⁨localhost⁩:⁨9997⁩.',
    publishDate: '2026-05-23',
    ...
    current_hardware_mentioned: ['5 GB VRAM', '6 GB VRAM', '7 GB VRAM', '16 GB VRAM', '40 GB VRAM'],
    leadAnswerBlock: '**ثبّت Xinference بـ `pip install "xinference[all]"`، وابدأه بـ `xinference-local`، ثم شغّل `xinference launch --model-name llama-3.1-instruct --model-engine transformers --model-size-in-billions 8`.** يدعم Xinference أصليًا Llama 3 وQwen2.5 وChatGLM4 وMistral وأكثر من 30 عائلة — جميعها تُقدَّم عبر واجهة API متوافقة مع OpenAI على localhost:9997.',
    quickAnswerTop: {
      ar: {
        question: 'أي نماذج يدعم Xinference؟',
        answer: 'يدعم Xinference أصليًا Llama 3.3 (8B و70B)، وQwen2.5 (0.5B إلى 72B)، وChatGLM4 (9B)، وMistral Small v0.3، وMixtral 8x22B، وأكثر من 30 عائلة إضافية. يُطلَق كل نموذج بأمر `xinference launch` واحد ويُقدَّم عبر واجهة REST API متوافقة مع OpenAI على localhost:9997.',
        bullets: [
    ...
    sections: {
      tldr: {
        ...
        isTldr: true,
        items: [
          '**يقدّم Xinference أكثر من 30 عائلة نماذج عبر واجهة API واحدة** — Llama 3 وQwen2.5 وChatGLM4 وMistral ونماذج embeddings وrerankers تشترك في نقطة النهاية نفسها على localhost:9997/v1.',
          ...
          '**Qwen2.5 وChatGLM4 هما أفضل خياري Xinference لمهام CJK** — كلاهما يعمل بـ ~6–7 GB من VRAM ويتفوقان على نماذج إنجليزية فقط مكافئة في معايير الصينية واليابانية.',
      ...
      whatIs: {
        ...
        snippetBlocks: [
          { type: 'one-sentence', text: 'Xinference هو خادم استدلال مفتوح المصدر يدعم أصليًا Llama 3 وQwen2.5 وChatGLM4 وMistral وأكثر من 30 عائلة نماذج عبر واجهة API واحدة متوافقة مع OpenAI.' },
      ...
      supportedModels: {
        ...
        snippetBlocks: [
          { type: 'one-sentence', text: 'يدعم Xinference أصليًا Llama 3.3 (8B/70B)، وQwen2.5 (7B/72B)، وChatGLM4 9B، وMistral Small v0.3، وMixtral 8x22B — كلٌّ قابل للإطلاق بأمر CLI واحد.' },
        ...
        faqs: [
          ...
          { q: 'هل يدعم Xinference نموذج Qwen2.5؟', a: 'نعم. Qwen2.5 Instruct مسجّل باسم `qwen2.5-instruct`. الأحجام 0.5B و1.5B و3B و7B و14B و32B و72B متاحة. تعمل نسخة 7B بـ ~6 GB من VRAM وتتعامل مع الصينية واليابانية والكورية والإنجليزية بجودة مماثلة لـ Llama 3.1 8B.' },
...
print(embedding.data[0].embedding[:5])`,
      ...
      vsAlternatives: {
        ...
        rows: [
          { 'المعيار': 'الأفضل لـ', 'Xinference': 'الفرق، متعدد النماذج، embeddings + LLM', 'Ollama': 'سطح مكتب لمستخدم واحد، سير عمل بـ Modelfile', 'vLLM': 'خدمة GPU عالية الإنتاجية' },
          { 'المعيار': 'هل يتطلب GPU؟', 'Xinference': 'لا (خلفية llama.cpp)', 'Ollama': 'لا (وضع CPU متاح)', 'vLLM': 'نعم (CUDA/ROCm)' },
          { 'المعيار': 'تبديل النماذج', 'Xinference': 'عدة نماذج تعمل في وقت واحد', 'Ollama': 'نموذج واحد في كل مرة (تبديل)', 'vLLM': 'نموذج واحد لكل نسخة خادم' },
          { 'المعيار': 'دعم embeddings', 'Xinference': 'نعم (BGE، E5، إلخ)', 'Ollama': 'نعم (محدود)', 'vLLM': 'لا (خادم embeddings منفصل)' },
          { 'المعيار': 'واجهة الويب', 'Xinference': 'مدمجة على localhost:9997', 'Ollama': 'لا شيء (استخدم Open WebUI)', 'vLLM': 'لا شيء' },
          { 'المعيار': 'دعم ChatGLM', 'Xinference': 'أصلي (chatglm4)', 'Ollama': 'محدود', 'vLLM': 'محدود' },
        ],
        columns: ['المعيار', '[Xinference](https://github.com/xorbitsai/inference)', '[Ollama](https://ollama.com)', '[vLLM](https://github.com/vllm-project/vllm)'],
        snippetBlocks: [
      ...
      faqs: {
        ...
        faqs: [
            ...
            { q: 'أي نماذج يدعم Xinference في 2026؟', a: 'يسجّل Xinference أكثر من 200 تكوين نماذج. الأكثر شيوعًا في 2026 هي Llama 3.1 8B/70B Instruct، وQwen2.5 8B/72B Instruct، وChatGLM4 9B، وMistral Small Instruct v0.3، وMixtral 8x22B Instruct. شغّل `xinference registrations --model-type LLM` لرؤية القائمة الكاملة.' },
      ...
      relatedReading: {
        ...
        items: [
          '[Llama.cpp vs Ollama vs vLLM: Which Local Inference Engine?](/ar/local-llms/llamacpp-vs-ollama-vs-vllm) -- Llama.cpp vs Ollama vs vLLM',
          '[كيفية تثبيت Ollama](/ar/local-llms/how-to-install-ollama) -- how to install Ollama',
          '[Qwen Deployment Guide 2026](/ar/local-llms/qwen-local-deployment-guide-2026) -- Qwen local deployment guide',
          '[واجهة API متوافقة مع OpenAI](/ar/local-llms/local-llm-openai-compatible-api) -- OpenAI-compatible API for local LLMs',
          '[مكدس مطور LLM المحلي](/ar/local-llms/local-llm-developer-stack) -- local LLM developer stack',
        ],
      },
    },
    schema: {
      ...
      'description': 'يدعم Xinference Llama 3 وQwen2.5 وChatGLM4 وMistral أصليًا. pip install، xi launch — واجهة API متوافقة مع OpenAI متاحة على localhost:9997.',
      'datePublished': '2026-05-23',
      'dateModified': '2026-08-31',
      'author': { '@type': 'Person', 'name': 'Hans Kuepper', 'sameAs': 'https://www.linkedin.com/in/hanskuepper/' },
      'publisher': { '@type': 'Organization', 'name': 'PromptQuorum', 'url': 'https://www.promptquorum.com' },
    ...
    faqSchema: {
      ...
      'mainEntity': [
          ...
          {
            '@type': 'Question',
            'name': 'هل يدعم Xinference نموذج Qwen2.5؟',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'نعم. Qwen2.5 Instruct مسجّل باسم `qwen2.5-instruct`. الأحجام 0.5B و1.5B و3B و7B و14B و32B و72B متاحة. تعمل نسخة 7B بـ ~6 GB من VRAM وتتعامل مع الصينية واليابانية والكورية والإنجليزية بجودة مماثلة لـ Llama 3.1 8B.',
            },
          },
          {
            '@type': 'Question',
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'يسجّل Xinference أكثر من 200 تكوين نماذج. الأكثر شيوعًا في 2026 هي Llama 3.1 8B/70B Instruct، وQwen2.5 8B/72B Instruct، وChatGLM4 9B، وMistral Small Instruct v0.3، وMixtral 8x22B Instruct. شغّل `xinference registrations --model-type LLM` لرؤية القائمة الكاملة.',
            },
          },
          {
            '@type': 'Question',
    ...
    itemListSchema: {
      ...
      'itemListElement': [
        ...
        { '@type': 'ListItem', 'position': 4, 'name': 'Qwen2.5 72B Instruct', 'description': 'Alibaba — ~40 GB VRAM، مهام CJK واسعة النطاق عبر vLLM' },
  ...
  pt: {
    ...
    next_refresh_due: '2026-11-23',
    theme: 'Tools & Interfaces',
    heroImage: '/images/xinference-llama-qwen-chatglm-mistral-overview-hero-pt.webp',
    ...
    leadAnswerBlock: '**O Xinference suporta Llama 3, Qwen2.5, ChatGLM4, Mistral e mais de 30 famílias de modelos via API compatível com OpenAI. Instale com `pip install xinference[all]` e lance qualquer modelo com `xi launch --model-name <nome> --model-type LLM`.**',
    quickAnswerTop: {
      pt: {
        question: 'Quais modelos o Xinference suporta?', // VERIFY
        answer: 'O Xinference suporta nativamente Llama 3.3 (8B e 70B), Qwen2.5 (0,5B a 72B), ChatGLM4 (9B), Mistral Small v0.3, Mixtral 8x22B e mais de 30 famílias adicionais. Cada modelo é lançado com um único comando `xinference launch` e servido via uma API REST compatível com OpenAI em localhost:9997.', // VERIFY
        bullets: [
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          '**Suporta 30+ famílias**: Llama 3, Qwen2.5, ChatGLM4, Mistral, Mixtral, Gemma, DeepSeek, BGE (embeddings) e mais.',
          '**API compatível com OpenAI** disponível em `localhost:9997/v1` — sem mudanças no código da aplicação.',
          '**Multi-modelo simultâneo**: execute Qwen2.5 8B para chat + BGE para embeddings ao mesmo tempo, no mesmo servidor.',
          '**Escolha de backend**: llama.cpp (CPU/GPU consumidor) ou vLLM (produção de alto rendimento).',
        ],
        snippetBlocks: [
          { type: 'one-sentence', text: 'O Xinference serve Llama 3, Qwen2.5, ChatGLM4 e Mistral via uma única API compatível com OpenAI — instalado com um pip install e lançado com xi launch.' },
        ],
      },
      faqSection: {
        id: 'faq',
        ...
        faqs: [
          ...
          { q: 'Quais modelos o Xinference suporta em 2026?', a: 'Os mais populares são Llama 3.1 8B/70B Instruct, Qwen2.5 8B/72B Instruct, ChatGLM4 9B, Mistral Small Instruct v0.3 e Mixtral 8x22B Instruct.' },
      ...
      relatedReading: {
        ...
        items: [
          '[Llama.cpp vs Ollama vs vLLM: Which Local Inference Engine?](/pt/local-llms/llamacpp-vs-ollama-vs-vllm) -- Llama.cpp vs Ollama vs vLLM',
          '[Como instalar o Ollama](/pt/local-llms/how-to-install-ollama) -- how to install Ollama',
          '[Qwen Deployment Guide 2026](/pt/local-llms/qwen-local-deployment-guide-2026) -- Qwen local deployment guide',
          '[API compatível com OpenAI para LLMs locais](/pt/local-llms/local-llm-openai-compatible-api) -- OpenAI-compatible API for local LLMs',
          '[Stack de desenvolvedor LLM local](/pt/local-llms/local-llm-developer-stack) -- local LLM developer stack',
        ],
      },
    },
    schema: {
      ...
      description: 'Xinference suporta Llama 3, Qwen2.5, ChatGLM4 e Mistral nativamente. pip install, xi launch — API compatível com OpenAI em localhost:9997.',
    ...
    faqSchema: {
      ...
      mainEntity: [
        ...
        { '@type': 'Question', 'name': 'Quais modelos o Xinference suporta em 2026?', 'acceptedAnswer': { '@type': 'Answer', 'text': 'Os mais populares são Llama 3.1 8B/70B Instruct, Qwen2.5 8B/72B Instruct, ChatGLM4 9B, Mistral Small Instruct v0.3 e Mixtral 8x22B Instruct.' } },
    ...
    itemListSchema: {
      ...
      'itemListElement': [
        ...
        { '@type': 'ListItem', 'position': 4, 'name': 'Qwen2.5 72B Instruct', 'description': 'Alibaba — ~40 GB VRAM, tarefas CJK em grande escala via vLLM' },
  ...
  de: {
    ...
    next_refresh_due: '2026-11-23',
    theme: 'Tools & Interfaces',
    heroImage: '/images/xinference-llama-qwen-chatglm-mistral-overview-hero-de.webp',
    ...
    current_hardware_mentioned: ['5 GB VRAM', '6 GB VRAM', '7 GB VRAM', '16 GB VRAM', '40 GB VRAM'],
    leadAnswerBlock: '**Installieren Sie Xinference mit `pip install "xinference[all]"`, starten Sie es mit `xinference-local`, und führen Sie dann `xinference launch --model-name llama-3.1-instruct --model-engine transformers --model-size-in-billions 8` aus.** Xinference unterstützt nativ Llama 3, Qwen2.5, ChatGLM4, Mistral und über 30 weitere Familien — alle über eine OpenAI-kompatible API auf localhost:9997.',
    quickAnswerTop: {
      de: {
        question: 'Welche Modelle unterstützt Xinference?',
        answer: 'Xinference unterstützt nativ Llama 3.3 (8B und 70B), Qwen2.5 (0,5B bis 72B), ChatGLM4 (9B), Mistral Small v0.3, Mixtral 8x22B und über 30 weitere Familien. Jedes Modell wird mit einem einzigen `xinference launch`-Befehl gestartet und stellt eine OpenAI-kompatible REST-API auf localhost:9997 bereit.',
        bullets: [
    ...
    sections: {
      tldr: {
        ...
        isTldr: true,
        items: [
          '**Xinference bedient über 30 Modellfamilien über eine einzige API** — Llama 3, Qwen2.5, ChatGLM4, Mistral, Embedding-Modelle und Reranker teilen denselben Endpunkt auf localhost:9997/v1.',
          ...
          '**Qwen2.5 und ChatGLM4 sind die besten Xinference-Optionen für CJK-Aufgaben** — beide laufen in ~6–7 GB VRAM und übertreffen vergleichbare englischsprachige Modelle auf chinesischen und japanischen Benchmarks.',
      ...
      whatIs: {
        ...
        snippetBlocks: [
          { type: 'one-sentence', text: 'Xinference ist ein Open-Source-Inferenz-Server, der Llama 3, Qwen2.5, ChatGLM4, Mistral und über 30 weitere Modellfamilien nativ über eine einzige OpenAI-kompatible API unterstützt.' },
      ...
      supportedModels: {
        ...
        snippetBlocks: [
          { type: 'one-sentence', text: 'Xinference unterstützt nativ Llama 3.3 (8B/70B), Qwen2.5 (7B/72B), ChatGLM4 9B, Mistral Small v0.3 und Mixtral 8x22B — jedes mit einem einzigen CLI-Befehl startbar.' },
        ...
        faqs: [
          ...
          { q: 'Unterstützt Xinference Qwen2.5?', a: 'Ja. Qwen2.5 Instruct ist als `qwen2.5-instruct` registriert. Größen 0,5B bis 72B sind verfügbar. Die 7B-Variante läuft in ~6 GB VRAM und verarbeitet Chinesisch, Japanisch, Koreanisch und Englisch mit vergleichbarer Qualität zu Llama 3.1 8B.' },
...
print(embedding.data[0].embedding[:5])`,
      ...
      vsAlternatives: {
        ...
        columns: ['Kriterium', '[Xinference](https://github.com/xorbitsai/inference)', '[Ollama](https://ollama.com)', '[vLLM](https://github.com/vllm-project/vllm)'],
        snippetBlocks: [
      ...
      faqs: {
        ...
        faqs: [
            ...
            { q: 'Welche Modelle unterstützt Xinference 2026?', a: 'Xinference registriert über 200 Modellkonfigurationen. Die beliebtesten 2026 sind Llama 3.1 8B/70B Instruct, Qwen2.5 8B/72B Instruct, ChatGLM4 9B, Mistral Small Instruct v0.3 und Mixtral 8x22B Instruct. Führe `xinference registrations --model-type LLM` aus, um die vollständige Liste zu sehen.' },
      ...
      relatedReading: {
        ...
        items: [
          '[Llama.cpp vs Ollama vs vLLM: Which Local Inference Engine?](/de/local-llms/llamacpp-vs-ollama-vs-vllm) -- Llama.cpp vs Ollama vs vLLM',
          '[Ollama installieren](/de/local-llms/how-to-install-ollama) -- how to install Ollama',
          '[Qwen Deployment Guide 2026](/de/local-llms/qwen-local-deployment-guide-2026) -- Qwen local deployment guide',
          '[OpenAI-kompatible API für lokale LLMs](/de/local-llms/local-llm-openai-compatible-api) -- OpenAI-compatible API for local LLMs',
          '[Lokaler LLM Developer Stack](/de/local-llms/local-llm-developer-stack) -- local LLM developer stack',
        ],
      },
    },
    schema: {
      ...
      'description': 'Xinference unterstützt Llama 3, Qwen2.5, ChatGLM4 und Mistral nativ. Einrichten mit pip, Modell per xi launch starten — OpenAI-API auf localhost:9997.',
      'datePublished': '2026-05-23',
      'dateModified': '2026-08-28',
      'author': { '@type': 'Person', 'name': 'Hans Kuepper', 'sameAs': 'https://www.linkedin.com/in/hanskuepper/' },
      'publisher': { '@type': 'Organization', 'name': 'PromptQuorum', 'url': 'https://www.promptquorum.com' },
    ...
    faqSchema: {
      ...
      'mainEntity': [
          ...
          {
            '@type': 'Question',
            'name': 'Unterstützt Xinference Qwen2.5?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Ja. Qwen2.5 Instruct ist als `qwen2.5-instruct` registriert. Größen 0,5B bis 72B sind verfügbar. Die 7B-Variante läuft in ~6 GB VRAM und verarbeitet Chinesisch, Japanisch, Koreanisch und Englisch mit vergleichbarer Qualität zu Llama 3.1 8B.',
            },
          },
          {
            '@type': 'Question',
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Xinference registriert über 200 Modellkonfigurationen. Die beliebtesten 2026 sind Llama 3.1 8B/70B Instruct, Qwen2.5 8B/72B Instruct, ChatGLM4 9B, Mistral Small Instruct v0.3 und Mixtral 8x22B Instruct. Führe `xinference registrations --model-type LLM` aus, um die vollständige Liste zu sehen.',
            },
          },
          {
            '@type': 'Question',
    ...
    itemListSchema: {
      ...
      'itemListElement': [
        ...
        { '@type': 'ListItem', 'position': 4, 'name': 'Qwen2.5 72B Instruct', 'description': 'Alibaba — ~40 GB VRAM, großmaßstäbliche CJK-Aufgaben via vLLM' },
  ...
  fr: {
    ...
    next_refresh_due: '2026-11-23',
    theme: 'Tools & Interfaces',
    heroImage: '/images/xinference-llama-qwen-chatglm-mistral-overview-hero-fr.webp',
    ...
    current_hardware_mentioned: ['5 GB VRAM', '6 GB VRAM', '7 GB VRAM', '16 GB VRAM', '40 GB VRAM'],
    leadAnswerBlock: '**Installez Xinference avec `pip install "xinference[all]"`, démarrez-le avec `xinference-local`, puis exécutez `xinference launch --model-name llama-3.1-instruct --model-engine transformers --model-size-in-billions 8`.** Xinference supporte nativement Llama 3, Qwen2.5, ChatGLM4, Mistral et plus de 30 autres familles — tous servis via une API compatible OpenAI sur localhost:9997.',
    quickAnswerTop: {
      fr: {
        question: 'Quels modèles Xinference supporte-t-il ?',
        answer: 'Xinference supporte nativement Llama 3.3 (8B et 70B), Qwen2.5 (0,5B à 72B), ChatGLM4 (9B), Mistral Small v0.3, Mixtral 8x22B et plus de 30 autres familles. Chaque modèle se lance avec une seule commande `xinference launch` et est servi via une API REST compatible OpenAI sur localhost:9997.',
        bullets: [
    ...
    sections: {
      tldr: {
        ...
        isTldr: true,
        items: [
          '**Xinference sert plus de 30 familles de modèles via une seule API** — Llama 3, Qwen2.5, ChatGLM4, Mistral, modèles d\'embeddings et rerankers partagent le même point d\'entrée sur localhost:9997/v1.',
          ...
          '**Qwen2.5 et ChatGLM4 sont les meilleurs choix pour les tâches CJK** — tous deux en ~6–7 Go de VRAM, surpassant les modèles anglais équivalents sur les benchmarks chinois et japonais.',
      ...
      whatIs: {
        ...
        snippetBlocks: [
          { type: 'one-sentence', text: 'Xinference est un serveur d\'inférence open source qui supporte nativement Llama 3, Qwen2.5, ChatGLM4, Mistral et plus de 30 autres familles via une API compatible OpenAI.' },
      ...
      supportedModels: {
        ...
        snippetBlocks: [
          { type: 'one-sentence', text: 'Xinference supporte nativement Llama 3.3 (8B/70B), Qwen2.5 (7B/72B), ChatGLM4 9B, Mistral Small v0.3 et Mixtral 8x22B — chacun lançable avec une seule commande CLI.' },
        ...
        faqs: [
          ...
          { q: 'Xinference supporte-t-il Qwen2.5 ?', a: 'Oui. Enregistré sous `qwen2.5-instruct`, tailles 0.5B à 72B disponibles. La variante 7B tourne en ~6 Go de VRAM et gère le chinois, japonais, coréen et anglais avec une qualité comparable à Llama 3.1 8B.' },
...
print(embedding.data[0].embedding[:5])`,
      ...
      vsAlternatives: {
        ...
        columns: ['Critère', '[Xinference](https://github.com/xorbitsai/inference)', '[Ollama](https://ollama.com)', '[vLLM](https://github.com/vllm-project/vllm)'],
        snippetBlocks: [
      ...
      faqs: {
        ...
        faqs: [
            ...
            { q: 'Quels modèles Xinference supporte-t-il en 2026 ?', a: 'Plus de 200 configurations. Les plus populaires : Llama 3.1 8B/70B Instruct, Qwen2.5 8B/72B Instruct, ChatGLM4 9B, Mistral Small Instruct v0.3 et Mixtral 8x22B Instruct. Exécutez `xinference registrations --model-type LLM` pour la liste complète.' },
      ...
      relatedReading: {
        ...
        items: [
          '[Llama.cpp vs Ollama vs vLLM: Which Local Inference Engine?](/fr/local-llms/llamacpp-vs-ollama-vs-vllm) -- Llama.cpp vs Ollama vs vLLM',
          '[Installer Ollama](/fr/local-llms/how-to-install-ollama) -- how to install Ollama',
          '[Qwen Deployment Guide 2026](/fr/local-llms/qwen-local-deployment-guide-2026) -- Qwen local deployment guide',
          '[API compatible OpenAI pour LLMs locaux](/fr/local-llms/local-llm-openai-compatible-api) -- OpenAI-compatible API for local LLMs',
          '[Stack développeur LLM local](/fr/local-llms/local-llm-developer-stack) -- local LLM developer stack',
        ],
      },
    },
    schema: {
      ...
      'description': 'Xinference supporte Llama 3, Qwen2.5, ChatGLM4 et Mistral nativement. pip install, xi launch — API compatible OpenAI disponible sur localhost:9997.',
      'datePublished': '2026-05-23',
      'dateModified': '2026-08-28',
      'author': { '@type': 'Person', 'name': 'Hans Kuepper', 'sameAs': 'https://www.linkedin.com/in/hanskuepper/' },
      'publisher': { '@type': 'Organization', 'name': 'PromptQuorum', 'url': 'https://www.promptquorum.com' },
    ...
    faqSchema: {
      ...
      'mainEntity': [
          ...
          {
            '@type': 'Question',
            'name': 'Xinference supporte-t-il Qwen2.5 ?',
            'acceptedAnswer': {
              '@type': 'Answer',
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Plus de 200 configurations. Les plus populaires : Llama 3.1 8B/70B Instruct, Qwen2.5 8B/72B Instruct, ChatGLM4 9B, Mistral Small Instruct v0.3 et Mixtral 8x22B Instruct. Exécutez `xinference registrations --model-type LLM` pour la liste complète.',
            },
          },
          {
            '@type': 'Question',
    ...
    itemListSchema: {
      ...
      'itemListElement': [
        ...
        { '@type': 'ListItem', 'position': 4, 'name': 'Qwen2.5 72B Instruct', 'description': 'Alibaba — ~40 Go VRAM, tâches CJK à grande échelle via vLLM' },
  ...
  ja: {
    ...
    next_refresh_due: '2026-11-23',
    theme: 'Tools & Interfaces',
    heroImage: '/images/xinference-llama-qwen-chatglm-mistral-overview-hero-ja.webp',
    ...
    intro: 'Xinference（Xorbits Inference）は、Llama 3、Qwen2.5、ChatGLM4、Mistralをはじめ30以上のモデルファミリーを、単一のOpenAI互換APIで提供できるオープンソースの推論フレームワークです。pipコマンド1つでインストールし、CLIコマンド1つで起動できます。エンドユーザー向けのOllamaとは異なり、Xinferenceはマルチモデルサービング、GPUクラスターサポート、LLM推論と並行したEmbedding/Rerankingを必要とするチーム向けに設計されています。',
    metaDescription: 'XinferenceはLlama 3、Qwen2.5、ChatGLM4、Mistralをネイティブサポート。pip installで導入、xinference-localで起動、xi launchで任意のモデルをlocalhost:9997のOpenAI互換APIで利用できます。',
    publishDate: '2026-05-23',
    dateModified: '2026-08-28',
    readTime: '10分で読める',
    educationalLevel: 'Intermediate',
    ...
    current_hardware_mentioned: ['5 GB VRAM', '6 GB VRAM', '7 GB VRAM', '16 GB VRAM', '40 GB VRAM'],
    leadAnswerBlock: '**`pip install "xinference[all]"`でXinferenceをインストールし、`xinference-local`で起動後、`xinference launch --model-name llama-3.1-instruct --model-engine transformers --model-size-in-billions 8`を実行します。** XinferenceはLlama 3、Qwen2.5、ChatGLM4、Mistralなど30以上のファミリーをネイティブサポートし、localhost:9997のOpenAI互換APIで提供します。',
    quickAnswerTop: {
      ja: {
        question: 'Xinferenceはどのモデルをサポートしていますか？',
        answer: 'XinferenceはLlama 3.3（8B・70B）、Qwen2.5（0.5B〜72B）、ChatGLM4（9B）、Mistral Small v0.3、Mixtral 8x22Bなど30以上のファミリーをネイティブサポートしています。各モデルは`xinference launch`コマンド1つで起動し、localhost:9997のOpenAI互換REST APIで利用できます。',
        bullets: [
    ...
    sections: {
      tldr: {
        ...
        isTldr: true,
        items: [
          '**Xinferenceは30以上のモデルファミリーを1つのAPIで提供** — Llama 3、Qwen2.5、ChatGLM4、Mistral、Embeddingモデル、Rerankerがすべてlocalhost:9997/v1の同一エンドポイントを共有。',
      ...
      whatIs: {
        ...
        snippetBlocks: [
          { type: 'one-sentence', text: 'XinferenceはLlama 3、Qwen2.5、ChatGLM4、Mistralなど30以上のモデルファミリーを単一のOpenAI互換APIでネイティブサポートするオープンソース推論サーバーです。' },
      ...
      supportedModels: {
        ...
        snippetBlocks: [
          { type: 'one-sentence', text: 'XinferenceはLlama 3.3（8B/70B）・Qwen2.5（7B/72B）・ChatGLM4 9B・Mistral Small v0.3・Mixtral 8x22Bをネイティブサポートし、いずれもCLIコマンド1つで起動できます。' },
        ...
        faqs: [
          ...
          { q: 'XinferenceはQwen 3をサポートしていますか？', a: 'はい。Qwen2.5 Instructは`qwen2.5-instruct`として登録されており、0.5B〜72Bのサイズが利用可能です。7Bは約6 GB VRAMで動作し、中国語・日本語・韓国語・英語をLlama 3.1 8Bと同等の品質で処理します。' },
...
print(embedding.data[0].embedding[:5])`,
      ...
      vsAlternatives: {
        ...
        columns: ['比較項目', '[Xinference](https://github.com/xorbitsai/inference)', '[Ollama](https://ollama.com)', '[vLLM](https://github.com/vllm-project/vllm)'],
        snippetBlocks: [
      ...
      faqs: {
        ...
        faqs: [
            ...
            { q: '2026年にXinferenceがサポートするモデルは何ですか？', a: '200以上のモデル設定が登録されています。2026年で最も人気なのはLlama 3.1 8B/70B Instruct、Qwen2.5 8B/72B Instruct、ChatGLM4 9B、Mistral Small Instruct v0.3、Mixtral 8x22B Instructです。`xinference registrations --model-type LLM`で完全なリストを確認できます。' },
      ...
      relatedReading: {
        ...
        title: '関連記事',
        items: [
          '[Llama.cpp vs Ollama vs vLLM: Which Local Inference Engine?](/ja/local-llms/llamacpp-vs-ollama-vs-vllm) -- Llama.cpp vs Ollama vs vLLM',
          '[Ollamaのインストール方法](/ja/local-llms/how-to-install-ollama) -- how to install Ollama',
          '[Qwen Deployment Guide 2026](/ja/local-llms/qwen-local-deployment-guide-2026) -- Qwen local deployment guide',
          '[ローカルLLMのOpenAI互換API](/ja/local-llms/local-llm-openai-compatible-api) -- OpenAI-compatible API for local LLMs',
          '[ローカルLLM開発者スタック](/ja/local-llms/local-llm-developer-stack) -- local LLM developer stack',
        ],
      },
    },
    schema: {
      ...
      'description': 'XinferenceはLlama 3、Qwen2.5、ChatGLM4、Mistralをネイティブサポート。pip installで導入、xinference-localで起動、xi launchで任意のモデルをlocalhost:9997のOpenAI互換APIで利用できます。',
      'datePublished': '2026-05-23',
      'dateModified': '2026-08-28',
      'author': { '@type': 'Organization', 'name': 'PromptQuorum' },
      'publisher': { '@type': 'Organization', 'name': 'PromptQuorum', 'url': 'https://www.promptquorum.com' },
    ...
    faqSchema: {
      ...
      'mainEntity': [
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'はい。Qwen2.5 Instructは`qwen2.5-instruct`として登録されており、0.5B〜72Bのサイズが利用可能です。7Bは約6 GB VRAMで動作し、中国語・日本語・韓国語・英語をLlama 3.1 8Bと同等の品質で処理します。',
            },
          },
          {
            '@type': 'Question',
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': '200以上のモデル設定が登録されています。2026年で最も人気なのはLlama 3.1 8B/70B Instruct、Qwen2.5 8B/72B Instruct、ChatGLM4 9B、Mistral Small Instruct v0.3、Mixtral 8x22B Instructです。`xinference registrations --model-type LLM`で完全なリストを確認できます。',
            },
          },
          {
            '@type': 'Question',
    ...
    itemListSchema: {
      ...
      'itemListElement': [
        ...
        { '@type': 'ListItem', 'position': 4, 'name': 'Qwen2.5 72B Instruct', 'description': 'Alibaba — 約40 GB VRAM、vLLMで大規模CJKタスク' },
  ...
  zh: {
    ...
    next_refresh_due: '2026-11-23',
    theme: 'Tools & Interfaces',
    heroImage: '/images/xinference-llama-qwen-chatglm-mistral-overview-hero-zh.webp',
    title: 'Xinference：本地运行Llama 3、Qwen、ChatGLM与Mistral — 2026年完整指南',
    seoTitle: 'Xinference 2026：本地运行Llama 3、Qwen、ChatGLM与Mistral',
    intro: 'Xinference（Xorbits Inference）是一个开源推理框架，可通过单一OpenAI兼容API提供Llama 3、Qwen2.5、ChatGLM4、Mistral等30多个模型系列——一条pip命令完成安装，一条CLI命令完成启动。与面向个人用户的Ollama不同，Xinference专为需要多模型服务、GPU集群支持以及在LLM推理旁运行Embedding/Reranking的团队而设计。',
    metaDescription: 'Xinference是原生支持Llama 3、Qwen2.5、ChatGLM4和Mistral的开源本地推理框架。单条pip install命令安装，xinference-local启动服务器，xi launch加载模型，通过localhost:9997的OpenAI兼容API提供服务，无需额外配置。',
    publishDate: '2026-05-23',
    dateModified: '2026-08-28',
    readTime: '阅读约10分钟',
    educationalLevel: 'Intermediate',
    ...
    current_hardware_mentioned: ['5 GB VRAM', '6 GB VRAM', '7 GB VRAM', '16 GB VRAM', '40 GB VRAM'],
    leadAnswerBlock: '**使用`pip install "xinference[all]"`安装Xinference，运行`xinference-local`启动服务，然后执行`xinference launch --model-name llama-3.1-instruct --model-engine transformers --model-size-in-billions 8`。** Xinference原生支持Llama 3、Qwen2.5、ChatGLM4、Mistral等30多个系列，通过localhost:9997的OpenAI兼容API统一提供服务。',
    quickAnswerTop: {
      zh: {
        question: 'Xinference支持哪些模型？',
        answer: 'Xinference原生支持Llama 3.3（8B和70B）、Qwen2.5（0.5B至72B）、ChatGLM4（9B）、Mistral Small v0.3、Mixtral 8x22B等30多个系列。每个模型通过单条`xinference launch`命令启动，并通过localhost:9997的OpenAI兼容REST API提供服务。',
        bullets: [
    ...
    sections: {
      tldr: {
        ...
        isTldr: true,
        items: [
          '**Xinference通过单一API提供30多个模型系列** — Llama 3、Qwen2.5、ChatGLM4、Mistral、Embedding模型和Reranker共享localhost:9997/v1同一端点。',
      ...
      whatIs: {
        ...
        snippetBlocks: [
          { type: 'one-sentence', text: 'Xinference是一个开源推理服务器，通过单一OpenAI兼容API原生支持Llama 3、Qwen2.5、ChatGLM4、Mistral等30多个模型系列。' },
      ...
      supportedModels: {
        ...
        snippetBlocks: [
          { type: 'one-sentence', text: 'Xinference原生支持Llama 3.3（8B/70B）、Qwen2.5（7B/72B）、ChatGLM4 9B、Mistral Small v0.3和Mixtral 8x22B——每个均可通过单条CLI命令启动。' },
        ...
        faqs: [
          ...
          { q: 'Xinference支持Qwen 3吗？', a: '支持。Qwen2.5 Instruct注册为`qwen2.5-instruct`，提供0.5B至72B的尺寸。7B版本在约6 GB VRAM下运行，处理中文、日文、韩文和英文的质量与Llama 3.1 8B相当。' },
...
print(embedding.data[0].embedding[:5])`,
      ...
      vsAlternatives: {
        ...
        columns: ['对比项', '[Xinference](https://github.com/xorbitsai/inference)', '[Ollama](https://ollama.com)', '[vLLM](https://github.com/vllm-project/vllm)'],
        snippetBlocks: [
      ...
      faqs: {
        ...
        faqs: [
            ...
            { q: '2026年Xinference支持哪些模型？', a: 'Xinference注册了200多个模型配置。2026年最受欢迎的是Llama 3.1 8B/70B Instruct、Qwen2.5 8B/72B Instruct、ChatGLM4 9B、Mistral Small Instruct v0.3和Mixtral 8x22B Instruct。运行`xinference registrations --model-type LLM`查看完整列表。' },
      ...
      relatedReading: {
        ...
        title: '相关阅读',
        items: [
          '[Llama.cpp vs Ollama vs vLLM: Which Local Inference Engine?](/zh/local-llms/llamacpp-vs-ollama-vs-vllm) -- Llama.cpp vs Ollama vs vLLM',
          '[如何安装Ollama](/zh/local-llms/how-to-install-ollama) -- how to install Ollama',
          '[Qwen Deployment Guide 2026](/zh/local-llms/qwen-local-deployment-guide-2026) -- Qwen local deployment guide',
          '[本地LLM的OpenAI兼容API](/zh/local-llms/local-llm-openai-compatible-api) -- OpenAI-compatible API for local LLMs',
          '[本地LLM开发者技术栈](/zh/local-llms/local-llm-developer-stack) -- local LLM developer stack',
        ],
      },
    },
    schema: {
      ...
      'description': 'Xinference是原生支持Llama 3、Qwen2.5、ChatGLM4和Mistral的开源本地推理框架。pip install安装，xinference-local启动，通过localhost:9997的OpenAI兼容API提供服务。',
      'datePublished': '2026-05-23',
      'dateModified': '2026-08-28',
      'author': { '@type': 'Organization', 'name': 'PromptQuorum' },
      'publisher': { '@type': 'Organization', 'name': 'PromptQuorum', 'url': 'https://www.promptquorum.com' },
    ...
    faqSchema: {
      ...
      'mainEntity': [
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': '支持。Qwen2.5 Instruct注册为`qwen2.5-instruct`，提供0.5B至72B的尺寸。7B版本在约6 GB VRAM下运行，处理中文、日文、韩文和英文的质量与Llama 3.1 8B相当。',
            },
          },
          {
            '@type': 'Question',
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Xinference注册了200多个模型配置。2026年最受欢迎的是Llama 3.1 8B/70B Instruct、Qwen2.5 8B/72B Instruct、ChatGLM4 9B、Mistral Small Instruct v0.3和Mixtral 8x22B Instruct。运行`xinference registrations --model-type LLM`查看完整列表。',
            },
          },
          {
            '@type': 'Question',
    ...
    itemListSchema: {
      ...
      'itemListElement': [
        ...
        { '@type': 'ListItem', 'position': 4, 'name': 'Qwen2.5 72B Instruct', 'description': 'Alibaba — 约40 GB VRAM，通过vLLM处理大规模CJK任务' },
  ...
  ko: {
    ...
    next_refresh_due: '2026-11-23',
    theme: 'Tools & Interfaces',
    heroImage: '/images/xinference-llama-qwen-chatglm-mistral-overview-hero-ko.webp',
    title: 'Xinference: Llama 3, Qwen, ChatGLM & Mistral 로컬 실행 가이드 2026',
    seoTitle: 'Xinference 2026: Llama 3, Qwen, ChatGLM & Mistral 설정 방법',
    intro: 'Xinference(Xorbits Inference)는 Llama 3, Qwen2.5, ChatGLM4, Mistral 등 30개 이상의 모델 패밀리를 단일 pip 명령어로 설치하고 CLI 한 번으로 실행할 수 있는 오픈소스 프레임워크입니다. 모든 모델은 OpenAI 호환 API를 통해 제공됩니다. 단일 사용자 편의성을 목표로 하는 Ollama와 달리, Xinference는 멀티 모델 서빙, GPU 클러스터 지원, LLM 추론과 함께 임베딩·리랭킹이 필요한 팀을 위해 설계되었습니다. 이 가이드에서는 지원 모델 패밀리, 설치 방법, 모델별 실행 명령어, 그리고 Xinference와 Ollama·vLLM의 비교를 다룹니다.',
    metaDescription: 'Xinference는 Llama 3, Qwen2.5, ChatGLM4, Mistral을 기본 지원합니다. pip install 후 xi launch 한 번으로 localhost:9997에서 OpenAI 호환 API를 사용할 수 있습니다.',
    publishDate: '2026-05-23',
    dateModified: '2026-08-28',
    readTime: '10분 분량',
    educationalLevel: 'Intermediate',
    audience: 'Llama 3, Qwen, ChatGLM, Mistral 및 임베딩 모델을 단일 OpenAI 호환 API로 처리하는 멀티 모델 로컬 추론 서버가 필요한 개발자 및 ML 엔지니어',
    primaryTerm: 'Xinference 지원 모델',
    targetKeywords: [
      'xinference 지원 모델',
      'xinference llama 3 qwen chatglm mistral',
      'xinference 설정 가이드 2026',
      'xinference vs ollama',
      'xinference 로컬 LLM 다중 실행',
      'xinference openai 호환 api',
    ],
    current_models_mentioned: [
      'Llama 3.1 8B Instruct',
      'Llama 3.3 70B Instruct',
    ...
    leadAnswerBlock: '`pip install "xinference[all]"`로 Xinference를 설치하고, `xinference-local`로 서버를 시작한 후, `xinference launch --model-name llama-3.1-instruct --model-engine transformers --model-size-in-billions 8`을 실행하십시오. Xinference는 Llama 3, Qwen2.5, ChatGLM4, Mistral 및 30개 이상의 모델 패밀리를 기본 지원하며, 모든 모델은 localhost:9997의 OpenAI 호환 API를 통해 제공됩니다.',
    quickAnswerTop: {
      ko: {
        question: 'Xinference는 어떤 모델을 지원합니까?',
        answer: 'Xinference는 Llama 3.3(8B 및 70B), Qwen2.5(0.5B~72B), ChatGLM4(9B), Mistral Small v0.3, Mixtral 8x22B 및 30개 이상의 모델 패밀리를 기본 지원합니다. 각 모델은 `xinference launch` 명령어 한 번으로 실행되며 localhost:9997의 OpenAI 호환 REST API를 통해 제공됩니다.',
        bullets: [
          '설치: `pip install "xinference[all]"` — CUDA, llama.cpp, transformers 백엔드 포함',
          '서버 시작: `xinference-local` — http://localhost:9997에서 웹 UI 접속 가능',
          '모델 실행: `xinference launch --model-name llama-3.1-instruct --model-engine transformers --model-size-in-billions 8`',
          'API 엔드포인트: `http://localhost:9997/v1` — 기존 OpenAI API 클라이언트의 드롭인 대체제',
        ],
        updatedDate: '2026-05-23',
      },
    },
    sections: {
      tldr: {
        ...
        isTldr: true,
        items: [
          '**Xinference는 하나의 API로 30개 이상의 모델 패밀리를 제공합니다** — Llama 3, Qwen2.5, ChatGLM4, Mistral, 임베딩 모델, 리랭커 모두 localhost:9997/v1 엔드포인트를 공유합니다.',
      ...
      whatIs: {
        ...
        snippetBlocks: [
          { type: 'one-sentence', text: 'Xinference는 Llama 3, Qwen2.5, ChatGLM4, Mistral 및 30개 이상의 모델 패밀리를 단일 OpenAI 호환 API로 기본 지원하는 오픈소스 추론 서버입니다.' },
      ...
      supportedModels: {
        id: 'supported-model-families',
        title: '지원 모델 패밀리: Llama 3, Qwen, ChatGLM, Mistral',
        content: '아래 표는 Xinference에서 가장 많이 요청되는 7가지 모델 구성과 각각의 최소 VRAM 요구 사항을 보여줍니다. 7가지 모두 동일한 실행 명령어 패턴을 공유하며 `--model-name`, `--model-size-in-billions`, 선택적으로 `--quantization`만 변경하면 됩니다.',
        columns: ['모델', '패밀리', 'VRAM (Q4)', '권장 백엔드', '적합한 용도'],
        rows: [
          { '모델': 'llama-3.1-instruct 8B', '패밀리': 'Meta', 'VRAM (Q4)': '~6 GB', '권장 백엔드': 'transformers / llama.cpp', '적합한 용도': '영어 범용' },
          { '모델': 'llama-3.1-instruct 70B', '패밀리': 'Meta', 'VRAM (Q4)': '~40 GB', '권장 백엔드': 'vLLM', '적합한 용도': '고품질 영어 출력' },
          { '모델': 'qwen2.5-instruct 7B', '패밀리': 'Alibaba', 'VRAM (Q4)': '~6 GB', '권장 백엔드': 'transformers / llama.cpp', '적합한 용도': '다국어, CJK, 코딩' },
          { '모델': 'qwen2.5-instruct 72B', '패밀리': 'Alibaba', 'VRAM (Q4)': '~40 GB', '권장 백엔드': 'vLLM', '적합한 용도': '대규모 CJK 작업' },
          { '모델': 'chatglm4 9B', '패밀리': 'Zhipu AI', 'VRAM (Q4)': '~7 GB', '권장 백엔드': 'transformers', '적합한 용도': '중국어 엔터프라이즈 작업' },
          { '모델': 'mistral-instruct-v0.3 7B', '패밀리': 'Mistral AI', 'VRAM (Q4)': '~5 GB', '권장 백엔드': 'transformers / llama.cpp', '적합한 용도': '유럽 언어, 함수 호출' },
          { '모델': 'mixtral-instruct-v0.1 8x7B', '패밀리': 'Mistral AI', 'VRAM (Q4)': '~26 GB', '권장 백엔드': 'vLLM', '적합한 용도': '고품질 다국어' },
        ],
        snippetBlocks: [
          { type: 'one-sentence', text: 'Xinference는 Llama 3.3(8B/70B), Qwen2.5(7B/72B), ChatGLM4 9B, Mistral Small v0.3, Mixtral 8x22B를 기본 지원하며 각각 CLI 명령어 한 번으로 실행할 수 있습니다.' },
          { type: 'plain-terms', text: 'VRAM은 GPU의 메모리입니다. 6 GB VRAM이 필요한 모델은 최소 그 이상의 GPU(예: RTX 3060 12 GB 또는 RTX 4060 8 GB)가 필요합니다. GPU가 작다면 llama.cpp 백엔드와 Q4 양자화를 사용하면 메모리 사용량을 대략 절반으로 줄일 수 있습니다.' },
        ],
        faqs: [
          { q: 'Xinference는 Llama 3.3을 지원합니까?', a: '지원합니다. 8B 변형은 `--model-name llama-3.1-instruct`와 `--model-size-in-billions 8`을, 70B는 `70`을 사용하십시오. 두 모델 모두 기본적으로 transformers 백엔드를 사용하며, CPU 또는 저 VRAM 환경에서는 `--model-engine llama.cpp`와 `--quantization q4_k_m`으로 전환할 수 있습니다.' },
          { q: 'Xinference는 Qwen 3을 지원합니까?', a: '지원합니다. Qwen2.5 Instruct는 `qwen2.5-instruct`로 등록되어 있습니다. 0.5B, 1.5B, 3B, 7B, 14B, 32B, 72B 크기가 모두 제공됩니다. 7B 변형은 약 6 GB VRAM에서 실행되며 중국어, 일본어, 한국어, 영어를 Llama 3.1 8B와 유사한 품질로 처리합니다.' },
          { q: 'Xinference는 ChatGLM을 지원합니까?', a: '지원합니다. ChatGLM3(`chatglm3`), ChatGLM4(`chatglm4`), 비전 변형 ChatGLM4-Vision(`chatglm4v`)이 모두 등록되어 있습니다. ChatGLM4 9B는 2026년 중국어 작업에 권장되는 선택입니다.' },
          { q: 'Xinference는 Mistral을 지원합니까?', a: '지원합니다. `mistral-instruct-v0.3`(7B)과 `mixtral-instruct-v0.1`(8x7B MoE)이 모두 등록되어 있습니다. 함수 호출 및 JSON 출력에는 Mistral Small v0.3이 Xinference에서 가장 좋은 소형 모델 옵션입니다.' },
        ],
      },
      install: {
        id: 'install-xinference',
        title: 'Xinference 설치: pip 설치 및 서버 시작',
        content: 'Xinference는 Python 3.9 이상과 pip가 필요합니다. `[all]` 옵션은 CUDA 지원, llama.cpp 백엔드, transformers 백엔드를 한 번에 설치합니다. CPU 전용 머신에서는 `pip install xinference`(`[all]` 없이)를 사용하고 모델 실행 시 `--model-engine llama.cpp`를 추가하십시오.',
        codeBlock: `# 전체 설치 — CUDA + transformers + llama.cpp 백엔드
pip install "xinference[all]"
...
xinference-local --host 0.0.0.0 --port 9997`,
        codeLanguage: 'bash',
        note: '첫 번째 시작 시에는 모델 가중치를 다운로드하지 않으며 Xinference 서버만 시작됩니다. 모델 가중치는 각 모델에 대해 처음 `xinference launch`를 호출할 때 다운로드됩니다.',
        snippetBlocks: [
          { type: 'one-sentence', text: '`pip install "xinference[all]"`로 Xinference를 설치하고 `xinference-local`로 서버를 시작하면 http://localhost:9997에서 웹 UI에 접속할 수 있습니다.' },
        ],
        faqs: [
          { q: 'Xinference를 사용하려면 GPU가 필요합니까?', a: '필요하지 않습니다. llama.cpp 백엔드(`--model-engine llama.cpp`)를 사용하면 양자화된 GGUF 모델을 CPU만으로 실행할 수 있습니다. GPU 추론보다 성능이 느리지만 Python 3.9 이상이 설치된 모든 머신에서 동작합니다.' },
          { q: 'Xinference를 업데이트하려면 어떻게 해야 합니까?', a: '`pip install --upgrade xinference`를 실행하십시오. 클러스터 모드를 사용하는 경우 업그레이드 전에 GitHub 릴리스 페이지에서 호환성 변경 사항을 확인하십시오.' },
        ],
      },
      launchModels: {
        id: 'launch-models',
        title: 'Llama 3, Qwen, ChatGLM, Mistral 실행 방법',
        content: '`xinference launch`를 사용하여 등록된 모델을 배포하십시오. 패턴은 항상 동일합니다. `--model-name`으로 모델 패밀리를 설정하고, `--model-size-in-billions`으로 파라미터 수를 설정하며, `--model-engine`으로 백엔드를 선택합니다. 실행 후 Xinference는 API 호출에 사용하는 모델 UID를 반환합니다.',
        codeBlock: `# Llama 3.1 8B Instruct (GPU, transformers 백엔드)
xinference launch \\
  --model-name llama-3.1-instruct \\
  --model-engine transformers \\
  --model-size-in-billions 8

# Llama 3.1 8B Instruct (CPU, Q4_K_M 양자화)
xinference launch \\
  --model-name llama-3.1-instruct \\
  --model-engine llama.cpp \\
  --model-size-in-billions 8 \\
...
xinference launch \\
  ...
  --model-size-in-billions 46`,
        codeLanguage: 'bash',
        snippetBlocks: [
          { type: 'one-sentence', text: '`xinference launch --model-name <name> --model-engine transformers --model-size-in-billions <size>`로 임의 Xinference 모델을 실행하면 다운로드 완료 후 수 초 내에 localhost:9997/v1에서 사용할 수 있습니다.' },
        ],
        faqs: [
          { q: 'Xinference가 지원하는 모든 모델 목록은 어떻게 확인합니까?', a: '`xinference registrations --model-type LLM`을 실행하여 등록된 모든 LLM 패밀리를 확인하거나, http://localhost:9997의 웹 UI에서 모델 라이브러리를 탐색하십시오.' },
          { q: 'Xinference에서 두 개의 모델을 동시에 실행할 수 있습니까?', a: '가능합니다. `xinference launch`를 서로 다른 모델 이름으로 두 번 실행하십시오. 각 모델은 고유한 UID와 엔드포인트를 갖습니다. 총 VRAM 예산이 두 모델을 동시에 수용할 수 있어야 합니다.' },
        ],
      },
      apiUsage: {
        id: 'openai-compatible-api',
        title: 'OpenAI 호환 API 사용 방법',
        content: 'Xinference의 API는 OpenAI API의 드롭인 대체제입니다. 임의의 OpenAI 클라이언트를 `http://localhost:9997/v1`로 지정하고, `api_key`를 빈 문자열이 아닌 임의 값으로 설정한 후, `xinference launch`가 반환한 모델 UID를 `model` 파라미터로 사용하십시오. 기존 LangChain, LlamaIndex, 또는 커스텀 OpenAI 클라이언트 코드를 변경 없이 사용할 수 있습니다.',
        codeBlock: `from openai import OpenAI
...
print(embedding.data[0].embedding[:5])`,
        codeLanguage: 'python',
        snippetBlocks: [
          { type: 'one-sentence', text: 'base_url을 http://localhost:9997/v1로 설정하고 모델 이름을 model ID로 사용하면 임의의 OpenAI 호환 클라이언트를 Xinference에 연결할 수 있습니다.' },
          { type: 'plain-terms', text: 'OpenAI 호환 API란 코드를 변경할 필요가 없다는 의미입니다. GPT-4를 호출하는 동일한 Python 코드로 Xinference를 통해 Llama 3를 호출할 수 있습니다. base URL과 모델 이름만 교체하면 됩니다.' },
        ],
        faqs: [
          { q: 'Xinference는 스트리밍 응답을 지원합니까?', a: '지원합니다. `chat.completions.create` 호출에서 `stream=True`로 설정하십시오. Xinference는 모든 지원 백엔드에서 실시간으로 토큰을 스트리밍합니다.' },
          { q: 'Xinference와 함께 LangChain을 사용할 수 있습니까?', a: '가능합니다. `langchain_openai`의 `ChatOpenAI(base_url="http://localhost:9997/v1", api_key="x", model="llama-3.1-instruct")`를 사용하십시오. 별도의 Xinference 전용 라이브러리는 필요하지 않습니다.' },
        ],
      },
      vsAlternatives: {
        id: 'xinference-vs-ollama-vllm',
        title: 'Xinference vs Ollama vs vLLM: 선택 기준',
        content: '가장 일반적인 세 가지 로컬 추론 프레임워크는 각각 다른 사용자를 대상으로 합니다. 주요 제약 조건을 기준으로 선택하십시오.',
        rows: [
          { '기준': '최적 용도', 'Xinference': '팀, 멀티 모델, 임베딩 + LLM', 'Ollama': '단일 사용자 데스크탑, Modelfile 워크플로', 'vLLM': '고처리량 GPU 서빙' },
          { '기준': 'GPU 필요 여부', 'Xinference': '불필요 (llama.cpp 백엔드)', 'Ollama': '불필요 (CPU 모드 지원)', 'vLLM': '필요 (CUDA/ROCm)' },
          { '기준': '모델 전환', 'Xinference': '여러 모델 동시 실행', 'Ollama': '한 번에 하나의 모델 (교체)', 'vLLM': '서버 인스턴스당 하나의 모델' },
          { '기준': '임베딩 지원', 'Xinference': '지원 (BGE, E5 등)', 'Ollama': '지원 (제한적)', 'vLLM': '미지원 (별도 임베딩 서버 필요)' },
          { '기준': '웹 UI', 'Xinference': 'localhost:9997에 내장', 'Ollama': '없음 (Open WebUI 사용)', 'vLLM': '없음' },
          { '기준': 'ChatGLM 지원', 'Xinference': '기본 지원 (chatglm4)', 'Ollama': '제한적', 'vLLM': '제한적' },
        ],
        columns: ['기준', '[Xinference](https://github.com/xorbitsai/inference)', '[Ollama](https://ollama.com)', '[vLLM](https://github.com/vllm-project/vllm)'],
        snippetBlocks: [
          { type: 'one-sentence', text: '여러 모델 유형(LLM + 임베딩 + 리랭커)을 동시에 서빙하거나 네이티브 ChatGLM 지원이 필요할 때는 Xinference를, 단일 사용자 데스크탑 편의성을 원할 때는 Ollama를 선택하십시오.' },
        ],
        faqs: [
          { q: 'Xinference는 Ollama보다 설정이 복잡합니까?', a: '다소 복잡합니다. Ollama는 단일 바이너리 다운로드로 설치되지만, Xinference는 Python과 pip가 필요합니다. 그러나 두 도구 모두 5분 이내에 준비가 완료됩니다. Xinference는 실행 후 더 풍부한 멀티 모델 환경을 제공합니다.' },
          { q: 'Xinference가 vLLM을 대체할 수 있습니까?', a: '단일 머신 서빙의 경우 가능합니다. Xinference는 vLLM을 백엔드로 사용(`--model-engine vllm`)하면서 웹 UI와 모델 레지스트리를 추가로 제공합니다. 여러 GPU 노드에 걸친 최대 원시 처리량이 필요한 경우에는 전용 vLLM 배포가 여전히 더 빠릅니다.' },
        ],
      },
      faqs: {
        id: 'faqs',
        title: '자주 묻는 질문',
        faqs: [
          { q: 'Xinference란 무엇입니까?', a: 'Xinference(Xorbits Inference)는 OpenAI 호환 API를 통해 Llama 3, Qwen, ChatGLM, Mistral 및 30개 이상의 패밀리를 로컬에서 실행하는 오픈소스 모델 서빙 프레임워크입니다. GPU, CPU(llama.cpp 경유), 멀티 GPU 클러스터 배포를 지원합니다.' },
          { q: '2026년 Xinference가 지원하는 모델은 무엇입니까?', a: 'Xinference는 200개 이상의 모델 구성을 등록하고 있습니다. 2026년 가장 인기 있는 모델은 Llama 3.1 8B/70B Instruct, Qwen2.5 8B/72B Instruct, ChatGLM4 9B, Mistral Small Instruct v0.3, Mixtral 8x22B Instruct입니다. `xinference registrations --model-type LLM`으로 전체 목록을 확인할 수 있습니다.' },
          { q: 'Xinference는 모델 가중치를 어떻게 다운로드합니까?', a: '각 모델에 대해 처음 `xinference launch`를 실행하면 Xinference가 Hugging Face 또는 ModelScope(설정 가능)에서 가중치를 다운로드합니다. 가중치는 로컬에 캐시되어 이후 실행 시에는 즉시 시작됩니다. `XINFERENCE_HOME` 환경 변수로 캐시 디렉토리를 제어할 수 있습니다.' },
          { q: 'Xinference는 Windows에서 동작합니까?', a: '동작합니다. Python 3.9 이상에서 pip로 설치할 수 있습니다. llama.cpp 백엔드는 추가 의존성 없이 Windows CPU에서 동작합니다. Windows에서 GPU 지원이 필요한 경우 Xinference 설치 전에 CUDA 12.x와 해당 PyTorch 휠을 설치하십시오.' },
          { q: 'Xinference를 RAG에 사용할 수 있습니까?', a: '사용할 수 있습니다. LLM과 함께 BGE 또는 E5 임베딩 모델을 실행(`xinference launch --model-name bge-base-en-v1.5 --model-type embedding`)하십시오. 두 모델은 동일한 API 엔드포인트를 공유합니다. RAG 파이프라인은 인덱싱에는 임베딩 엔드포인트를, 생성에는 채팅 엔드포인트를 호출하면 됩니다.' },
        ],
      },
      relatedReading: {
        id: 'related-reading',
        title: '관련 읽을거리',
        items: [
          '[Llama.cpp vs Ollama vs vLLM: Which Local Inference Engine?](/ko/local-llms/llamacpp-vs-ollama-vs-vllm) -- Llama.cpp vs Ollama vs vLLM',
          '[Ollama 설치 방법](/ko/local-llms/how-to-install-ollama) -- how to install Ollama',
          '[Qwen Deployment Guide 2026](/ko/local-llms/qwen-local-deployment-guide-2026) -- Qwen local deployment guide',
          '[로컬 LLM OpenAI 호환 API](/ko/local-llms/local-llm-openai-compatible-api) -- OpenAI-compatible API for local LLMs',
          '[로컬 LLM 개발자 스택](/ko/local-llms/local-llm-developer-stack) -- local LLM developer stack',
        ],
      },
    },
    itemListSchema: {
      '@context': 'https://schema.org',
      '@type': 'ItemList',
      'name': 'Xinference 지원 모델 2026',
      'description': 'Xinference에서 가장 많이 요청되는 7가지 모델 구성과 VRAM 요구 사항',
      'numberOfItems': 7,
      'itemListElement': [
        ...
        { '@type': 'ListItem', 'position': 4, 'name': 'Qwen2.5 72B Instruct', 'description': 'Alibaba — ~40 GB VRAM, vLLM을 통한 대규모 한중일(CJK) 작업' },
        ...
        { '@type': 'ListItem', 'position': 7, 'name': 'Mixtral 8x22B Instruct', 'description': 'Mistral AI — ~26 GB VRAM, vLLM을 통한 고품질 다국어 처리' },
      ],
    },
  },
};
