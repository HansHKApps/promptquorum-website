    ...
    ar: {
      ...
      sections: {
        ...
        protocols: {
          ...
          rows: [
            { 'الميزة': 'الميزة', 'WireGuard': 'WireGuard', 'OpenVPN': 'OpenVPN' },
            { 'الميزة': 'زمن الاستجابة', 'WireGuard': '~5 ميلي ثانية حمل إضافي', 'OpenVPN': '~15 ميلي ثانية حمل إضافي' },
            { 'الميزة': 'تعقيد الإعداد', 'WireGuard': 'بسيط (30 دقيقة)', 'OpenVPN': 'معقّد (ساعتان)' },
            { 'الميزة': 'حجم الشيفرة', 'WireGuard': '4,000 LOC (قابلة للتدقيق)', 'OpenVPN': '400K+ LOC (معقّد)' },
            { 'الميزة': 'المصادقة', 'WireGuard': 'مفتاح عام', 'OpenVPN': 'شهادات + مفاتيح' },
            { 'الميزة': 'النفق المنقسم', 'WireGuard': '✓ مدمج', 'OpenVPN': '✓ يتطلب تكوينًا' },
            { 'الميزة': 'VPN مؤسسية (AD/SAML)', 'WireGuard': 'محدود (مصادقة يدوية)', 'OpenVPN': 'أفضل (دعم RADIUS)' },
            { 'الميزة': 'التوصية', 'WireGuard': '**استخدم هذا**', 'OpenVPN': 'بديل إن لم يتوفر WireGuard' },
          ],
    ...
    pt: {
      ...
      sections: {
        ...
        protocols: {
          ...
          title: 'Protocolos VPN: WireGuard vs OpenVPN',
          rows: [
            { '0': 'Recurso', '1': 'WireGuard', '2': 'OpenVPN' },
            { '0': 'Latência', '1': '~5ms de overhead', '2': '~15ms de overhead' },
            { '0': 'Complexidade de configuração', '1': 'Simples (30 min)', '2': 'Complexa (2 horas)' },
            { '0': 'Tamanho do código', '1': '4.000 LOC (auditável)', '2': '400K+ LOC (complexo)' },
            { '0': 'Autenticação', '1': 'Chave pública', '2': 'Certificados + chaves' },
            { '0': 'Split tunneling', '1': '✓ Nativo', '2': '✓ Requer configuração' },
            { '0': 'VPN corporativa (AD/SAML)', '1': 'Limitado (autenticação manual)', '2': 'Melhor (suporte a RADIUS)' },
            { '0': 'Recomendação', '1': '**Use este**', '2': 'Alternativa se não houver WireGuard' },
          ],
          columns: ['Recurso', 'WireGuard', 'OpenVPN'],
        },
        setup: {
          id: 'setup',
        ...
        split: {
          id: 'split',
          title: 'Split Tunneling (acesso somente ao LLM, não à internet)',
          content: [
        ...
        troubleshooting: {
          id: 'troubleshooting',
          title: 'Solução de problemas de acesso remoto',
          content: [
        ...
        faqSection: {
          ...
          faqs: [
            ...
            { q: 'Posso usar uma VPN corporativa (Okta, Azure)?', a: 'Sim, é melhor para equipes grandes. Mas requer integração (RADIUS, SAML). O WireGuard é mais simples para equipes com menos de 20 usuários.' },
          ],
        },
        relatedReading: {
          id: 'related-reading',
          ...
          items: [
            ...
            '[Melhor stack de LLM local para desenvolvedores](/pt/local-llms/local-llm-developer-stack)',
          ],
        },
        sources: {
          id: 'sources',
      ...
      faqSchema: {
        ...
        inLanguage: 'pt-BR',
        mainEntity: [
          { '@type': 'Question', name: 'Posso acessar meu servidor LLM local remotamente sem VPN?', acceptedAnswer: { '@type': 'Answer', text: 'Sim, mas não é recomendado para uso empresarial. Você pode usar o Cloudflare Tunnel (sem exposição de IP público) ou um proxy reverso com autenticação. Para dados sensíveis, VPN + firewall é a arquitetura de menor risco.' } },
          { '@type': 'Question', name: 'WireGuard ou OpenVPN para acesso ao LLM local?', acceptedAnswer: { '@type': 'Answer', text: 'WireGuard para novos setups: menor latência (5-10 ms vs 15 ms), mais simples de configurar e auditar (4.000 vs 400.000+ linhas de código). OpenVPN se sua organização já tem infraestrutura OpenVPN existente.' } },
          { '@type': 'Question', name: 'Usar VPN afeta a velocidade de inferência do LLM?', acceptedAnswer: { '@type': 'Answer', text: 'A VPN afeta apenas a latência de rede (5-15 ms adicionais), não a velocidade de inferência do modelo. O throughput de tokens/seg é determinado pela GPU/CPU local, não pela rede. Para respostas longas, a diferença é imperceptível.' } },
          { '@type': 'Question', name: 'Como configurar split tunneling para o servidor LLM?', acceptedAnswer: { '@type': 'Answer', text: 'No WireGuard: adicione `AllowedIPs = 192.168.1.0/24` (substitua pelo IP do servidor LLM) em vez de `0.0.0.0/0`. Isso garante que apenas o tráfego para o servidor LLM passe pela VPN — o restante usa a conexão local do usuário.' } },
          { '@type': 'Question', name: 'A VPN para LLM local é compatível com LGPD?', acceptedAnswer: { '@type': 'Answer', text: 'A VPN em si não é um requisito da LGPD, mas garante que dados pessoais sejam transmitidos de forma segura entre o usuário remoto e o servidor. A LGPD e a ANPD exigem medidas técnicas adequadas para proteger dados em trânsito — a VPN com criptografia ponta-a-ponta atende a esse requisito.' } },
          { '@type': 'Question', name: 'Posso usar uma VPN corporativa (Okta, Azure)?', acceptedAnswer: { '@type': 'Answer', text: 'Sim, é melhor para equipes grandes. Mas requer integração (RADIUS, SAML). O WireGuard é mais simples para equipes com menos de 20 usuários.' } },
        ],
      },
    },
  ko: {
    ...
    heroImage: '/images/vpn-for-local-llm-users-overview-hero-ko.webp',
      ...
      toc: [
        ...
        { label: '분할 터널링 (LLM만 접속, 인터넷 제외)', anchor: '#split' },
        { label: '보안 강화', anchor: '#hardening' },
        { label: '원격 접속 문제 해결', anchor: '#troubleshooting' },
        { label: 'FAQ', anchor: '#faq' },
      ],
      sections: {
        tldr: {
        ...
        'protocols': {
          id: 'protocols',
          title: 'VPN 프로토콜: WireGuard vs OpenVPN',
          rows: [
            { '0': '항목', '1': 'WireGuard', '2': 'OpenVPN' },
            { '0': '지연 시간', '1': '~5ms 오버헤드', '2': '~15ms 오버헤드' },
            { '0': '설정 복잡도', '1': '간단 (30분)', '2': '복잡 (2시간)' },
            { '0': '코드 크기', '1': '4,000줄 (감사 가능)', '2': '400K+ 줄 (복잡)' },
            { '0': '인증 방식', '1': '공개 키', '2': '인증서 + 키' },
            { '0': '분할 터널링', '1': '✓ 기본 내장', '2': '✓ 설정 필요' },
            { '0': '기업 VPN (AD/SAML)', '1': '제한적 (수동 인증)', '2': '우수 (RADIUS 지원)' },
            { '0': '권장 여부', '1': '**이것을 사용하십시오**', '2': 'WireGuard 불가 시 대안' },
          ],
          columns: ['항목', 'WireGuard', 'OpenVPN'],
        },
        'setup': {
          id: 'setup',
          title: '설정: LLM 네트워크에 VPN 서버 구성',
          numberedItems: [
        ...
        'faqSection': {
          ...
          faqs: [
            ...
            { q: 'VPN 암호화가 추론 속도를 저하시킵니까?', a: '미미한 수준입니다 (5% 미만 영향). 최신 CPU는 Gbps 속도로 암호화/복호화가 가능합니다.' },
            { q: '분할 터널링을 사용해야 합니까?', a: '더 나은 UX를 위해 권장합니다. 데이터 유출을 위해 EDR (엔드포인트 탐지)로 모니터링하십시오.' },
            { q: 'VPN 키가 유출된 경우 어떻게 해야 합니까?', a: '즉시 해당 사용자의 키를 재생성하십시오. 기존 키는 무효화됩니다. 소급 접근은 없습니다.' },
            { q: '기업 VPN (Okta, Azure)을 사용할 수 있습니까?', a: '네, 대규모 팀에 더 적합합니다. 다만 통합 작업이 필요합니다 (RADIUS, SAML). WireGuard는 20명 미만 팀에 더 간단합니다.' },
          ],
        },
        'relatedReading': {
          id: 'related-reading',
