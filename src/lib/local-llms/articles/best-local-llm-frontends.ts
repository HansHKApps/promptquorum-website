      ...
      sections: {
        ...
        regionalContext: {
          ...
          content: [
            ...
            'For regulated EU sectors (healthcare, legal, finance): Open WebUI is the recommended frontend because it logs all conversations locally with exportable audit trails. German BSI guidance and French CNIL recommendations support locally-hosted AI tools as a privacy-preserving approach for high-risk processing when combined with appropriate access controls — consult your sector-specific DPA for binding requirements. Set up Open WebUI with authentication enabled (`WEBUI_AUTH=true` in Docker) and restrict access to authorized users only.',
            '**Japan (METI)**',
      ...
      sections: {
        ...
        regionalContext: {
          ...
          content: [
            ...
            'Für regulierte EU-Sektoren (Gesundheitswesen, Recht, Finanzen): Open WebUI ist das empfohlene Frontend, da es alle Konversationen lokal mit exportierbaren Audit-Trails protokolliert. BSI-Leitlinien und CNIL-Empfehlungen unterstützen lokal gehostete AI-Tools als datenschutzfreundlichen Ansatz für Hochrisikoverarbeitung in Verbindung mit angemessenen Zugriffskontrollen — für verbindliche Anforderungen konsultieren Sie Ihre sektorspezifische Datenschutzbehörde. Richten Sie Open WebUI mit aktivierter Authentifizierung (`WEBUI_AUTH=true` in Docker) ein und beschränken Sie den Zugriff auf autorisierte Benutzer.',
      ...
      sections: {
        ...
        regionalContext: {
          ...
          content: [
            ...
            'Para sectores regulados de la UE (salud, legal, finanzas): Open WebUI es el frontend recomendado porque registra todas las conversaciones localmente con registros de auditoría exportables. Las directrices del BSI alemán y las recomendaciones de la CNIL francesa apoyan las herramientas de IA alojadas localmente como enfoque de privacidad para el procesamiento de alto riesgo cuando se combinan con controles de acceso adecuados — consulte su autoridad de protección de datos sectorial para requisitos vinculantes. Configura Open WebUI con autenticación habilitada (`WEBUI_AUTH=true` en Docker) y restringe el acceso solo a usuarios autorizados.',
            '**Japón (METI)**',
  ...
  sections: {
    ...
    regionalContext: {
      ...
      content: [
        ...
        '규제된 EU 분야(의료, 법률, 금융)의 경우: Open WebUI는 내보낼 수 있는 감사 추적과 함께 모든 대화를 로컬로 기록하기 때문에 권장 프론트엔드입니다. 독일 BSI 지침과 프랑스 CNIL 권고사항은 적절한 접근 제어와 결합할 때 고위험 처리를 위한 로컬 호스팅 AI 도구를 개인정보 보호 접근 방식으로 지지합니다 — 구속력 있는 요구 사항은 해당 분야별 개인정보 보호 기관에 문의하십시오. 인증이 활성화된 상태로 Open WebUI를 설정하고(Docker에서 `WEBUI_AUTH=true`) 승인된 사용자만 접근을 제한하십시오.',
        '**일본 (METI)**',
      ...
      callouts: [
        { type: '⚠️ 경고', text: 'EU 규제 분야(의료, 법률, 금융)의 경우: Open WebUI의 기본 Docker 설정에는 인증이 없습니다. 내부 또는 외부 네트워크에 노출하기 전에 WEBUI_AUTH=true를 추가하십시오 — 이는 GDPR 제32조 기술적 조치에 필수입니다.' },
