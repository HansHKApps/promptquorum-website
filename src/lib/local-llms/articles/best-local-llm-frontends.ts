  ...
  sections: {
    ...
    regionalContext: {
      ...
      content: [
        ...
        '규제된 EU 분야(의료, 법률, 금융)의 경우: Open WebUI는 내보낼 수 있는 감사 추적과 함께 모든 대화를 로컬로 기록하기 때문에 권장 프론트엔드입니다. 독일 BSI와 프랑스 CNIL은 모두 적절한 접근 제어와 결합할 때 고위험 처리를 위한 로컬 호스팅 AI 도구를 수용합니다. 인증이 활성화된 상태로 Open WebUI를 설정하고(Docker에서 `WEBUI_AUTH=true`) 승인된 사용자만 접근을 제한하십시오.',
        '**일본 (METI)**',
