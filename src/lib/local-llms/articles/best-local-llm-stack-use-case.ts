...
export const article: Partial<Record<Language, LLMArticle>> = {
    en: {
      ...
      intro: '**The best local LLM stack depends on your workflow: writers need Ollama + OpenWebUI + Llama 3.3, developers need vLLM + Qwen2.5-Coder + IDE extension, researchers need LangGraph + vLLM. As of April 2026, no single tool excels at everything.** This guide maps 7 common use cases to their optimal stack (backend + UI + integrations) and hardware tiers (8–24 GB VRAM).',
      metaDescription: 'Coding: vLLM + Qwen2.5-Coder. Writing: Ollama + Llama 3.3. RAG: LlamaIndex + Qdrant. Agents: CrewAI. VRAM tiers 8–24 GB with setup commands. 2026.',
      twitterDescription: 'Local LLM stacks by use case: Coding → vLLM + Qwen2.5-Coder. Writing → Ollama + Llama 3.3. RAG → LlamaIndex + Qdrant. Agents → LangGraph + vLLM. April 2026 guide.',
      ...
      sections: {
        ...
        'hardware': {
          ...
          rows: [
            { 'Your Hardware': '4–8 GB VRAM (GTX 1660, RTX 3050)', 'Writing': 'Ollama + Phi-4 Mini', 'Coding': 'Ollama + Qwen2.5-Coder-1.5B', 'RAG': 'LlamaIndex + Phi-4 Mini', 'Agents': 'Not recommended' },
            { 'Your Hardware': '12 GB VRAM (RTX 3060, RTX 4070)', 'Writing': 'Ollama + Llama 3.2 8B', 'Coding': 'vLLM + Qwen2.5-Coder-7B', 'RAG': 'LlamaIndex + Llama 3.2 8B', 'Agents': 'LangGraph + Ollama (slower)' },
            { 'Your Hardware': '16 GB VRAM (RTX 4070 Ti, RTX 4080)', 'Writing': 'Ollama + Mistral Small 3.1', 'Coding': 'vLLM + Qwen2.5-Coder-14B', 'RAG': 'LlamaIndex + Mistral 3.1', 'Agents': 'LangGraph + vLLM' },
            { 'Your Hardware': '24 GB VRAM (RTX 3090, RTX 4090)', 'Writing': 'Ollama + Llama 3.3 70B Q4', 'Coding': 'vLLM + Qwen2.5-Coder-32B', 'RAG': 'LlamaIndex + Llama 3.3 70B', 'Agents': 'LangGraph + vLLM (fastest)' },
          ],
        },
        'writing': {
          id: 'writing',
        ...
        'coding': {
          id: 'coding',
          content: [
            '**Best stack: [vLLM](/local-llms/vllm-performance-guide) + Qwen2.5-Coder + IDE extension**',
            'Why this stack: Qwen2.5-Coder scores 82% on HumanEval (best open-source coding model, April 2026). vLLM is 3–5× faster than Ollama for batch inference. Native OpenAI API compatibility fits existing IDE tools. Streaming enabled for real-time suggestions.',
            '',
    ...
    print(f"=== {filename} ===\n{review}\n")`,
        ...
        'finetuning': {
          ...
          content: [
            ...
            '',
            '**Note (April 2026):** Meta deprecated Llama 3.3 for commercial fine-tuning. Fine-tune on Llama 3.2 (`meta-llama/Llama-3.2-1B` or larger) or Qwen2.5 (`Qwen/Qwen2.5-7B`) for Apache 2.0 / open-source license terms. Both support LoRA and load easily in Ollama.',
          ],
          numberedItems: [
        ...
        'faqSection': {
          ...
          faqs: [
            ...
            { q: 'What\'s the best local LLM for code review?', a: 'vLLM + Qwen2.5-Coder-7B-Instruct. Qwen2.5-Coder scores 82% on HumanEval (best open-source). vLLM processes 10 files in parallel. ~30–50 tok/sec on RTX 3060 12GB.' },
        ...
        'sources': {
          ...
          items: [
            ...
            '[Qwen2.5-Coder Technical Report](https://qwenlm.github.io/blog/qwen2.5-coder/) — Alibaba Qwen. 82% HumanEval score, specialized for coding tasks. Apache 2.0 licensed.',
...
schema: {
      ...
      faqSchema: {
        ...
        'mainEntity': [
          ...
          { '@type': 'Question', 'name': 'What\'s the best local LLM for code review?', 'acceptedAnswer': { '@type': 'Answer', 'text': 'vLLM + Qwen2.5-Coder-7B-Instruct. Qwen2.5-Coder scores 82% on HumanEval (best open-source). vLLM processes 10 files in parallel. ~30–50 tok/sec on RTX 3060 12GB.' } },
      ...
      itemListSchema: {
        ...
        'itemListElement': [
          ...
          { '@type': 'ListItem', 'position': 2, 'name': 'Software Development & Code Review', 'description': 'vLLM + Qwen2.5-Coder-7B-Instruct (RTX 3060 12GB) or -14B (RTX 4070+). Fastest for batch inference (3–5× faster than Ollama). 82% HumanEval score. OpenAI-compatible API, token streaming for real-time completions. Batch processing for multi-file code review.' },
          ...
          { '@type': 'ListItem', 'position': 6, 'name': 'Fine-Tuning & Research', 'description': 'HuggingFace Transformers + LoRA + Ollama/vLLM for inference. LoRA reduces VRAM footprint 10×. Train separately on dedicated hardware, serve on inference box. Llama 3.2 and Qwen2.5 recommended (Llama 3.3 deprecated as of April 2026).' },
...
for filename, review in reviews:
    print(f"=== {filename} ===\n{review}\n")`,
        ...
        'finetuning': {
          ...
          content: [
            ...
            '',
            '**Nota (abril 2026):** Meta deprecó Llama 3.3 para fine-tuning comercial. Haz fine-tuning en Llama 3.2 (`meta-llama/Llama-3.2-1B` o más grande) o Qwen2.5 (`Qwen/Qwen2.5-7B`) para términos de licencia Apache 2.0 / open-source. Ambos admiten LoRA y se cargan fácilmente en Ollama.',
          ],
          numberedItems: [
      ...
      itemListSchema: {
        ...
        'itemListElement': [
          ...
          { '@type': 'ListItem', 'position': 6, 'name': 'Fine-tuning e investigación', 'description': 'HuggingFace Transformers + LoRA + Ollama/vLLM para inferencia. LoRA reduce el uso de VRAM 10×. Entrena por separado en hardware dedicado, sirve en la máquina de inferencia. Se recomienda Llama 3.2 y Qwen2.5 (Llama 3.3 deprecated desde abril 2026).' },
    ...
    de: {
      ...
      intro: '**Der beste lokale LLM-Stack hängt von Ihrem Workflow ab: Autoren benötigen Ollama + OpenWebUI + Llama 3.3, Entwickler benötigen vLLM + Qwen2.5-Coder + IDE-Erweiterung, Forscher benötigen LangGraph + vLLM. Stand April 2026 überzeugt kein einzelnes Tool in allen Bereichen.** Dieser Leitfaden ordnet 7 gängige Anwendungsfälle ihrem optimalen Stack (Backend + UI + Integrationen) und Hardware-Stufen (8–24 GB VRAM) zu.',
      metaDescription: 'Lokaler LLM-Stack nach Anwendungsfall April 2026: Coding → vLLM + Qwen2.5-Coder. Schreiben → Ollama + Llama 3.3. RAG → LlamaIndex + Qdrant. 8–24 GB VRAM.',
      twitterDescription: 'Lokaler LLM-Stack nach Anwendungsfall: Coding → vLLM + Qwen2.5-Coder. Schreiben → Ollama + Llama 3.3. RAG → LlamaIndex + Qdrant. Agenten → LangGraph + vLLM. April 2026.',
      publishDate: '2026-04-05',
      dateModified: '2026-04-19',
      ...
      sections: {
        ...
        'hardware': {
          ...
          rows: [
            { 'Ihre Hardware': '4–8 GB VRAM (GTX 1660, RTX 3050)', 'Schreiben': 'Ollama + Phi-4 Mini', 'Coding': 'Ollama + Qwen2.5-Coder-1.5B', 'RAG': 'LlamaIndex + Phi-4 Mini', 'Agenten': 'Nicht empfohlen' },
            { 'Ihre Hardware': '12 GB VRAM (RTX 3060, RTX 4070)', 'Schreiben': 'Ollama + Llama 3.2 8B', 'Coding': 'vLLM + Qwen2.5-Coder-7B', 'RAG': 'LlamaIndex + Llama 3.2 8B', 'Agenten': 'LangGraph + Ollama (langsamer)' },
            { 'Ihre Hardware': '16 GB VRAM (RTX 4070 Ti, RTX 4080)', 'Schreiben': 'Ollama + Mistral Small 3.1', 'Coding': 'vLLM + Qwen2.5-Coder-14B', 'RAG': 'LlamaIndex + Mistral 3.1', 'Agenten': 'LangGraph + vLLM' },
            { 'Ihre Hardware': '24 GB VRAM (RTX 3090, RTX 4090)', 'Schreiben': 'Ollama + Llama 3.3 70B Q4', 'Coding': 'vLLM + Qwen2.5-Coder-32B', 'RAG': 'LlamaIndex + Llama 3.3 70B', 'Agenten': 'LangGraph + vLLM (schnellstes)' },
          ],
        },
        'writing': {
          id: 'writing',
    ...
    print(f"=== {filename} ===\n{review}\n")`,
        ...
        'finetuning': {
          ...
          content: [
            ...
            '',
            '**Hinweis (April 2026):** Meta hat Llama 3.3 für kommerzielles Fine-Tuning eingestellt. Fine-Tuning auf Llama 3.2 (`meta-llama/Llama-3.2-1B` oder größer) oder Qwen2.5 (`Qwen/Qwen2.5-7B`) für Apache 2.0 / Open-Source-Lizenzbedingungen durchführen. Beide unterstützen LoRA und werden in Ollama problemlos geladen.',
          ],
          numberedItems: [
        ...
        'faqSection': {
          ...
          faqs: [
            ...
            { q: 'Was ist der beste lokale LLM für Code-Review?', a: 'vLLM + Qwen2.5-Coder-7B-Instruct. Qwen2.5-Coder erzielt 82% auf HumanEval (bestes Open-Source-Coding-Modell). vLLM verarbeitet 10 Dateien parallel. Ca. 30–50 Tok/Sek auf RTX 3060 12 GB.' },
        ...
        'sources': {
          ...
          items: [
            ...
            '[Qwen2.5-Coder Technical Report](https://qwenlm.github.io/blog/qwen2.5-coder/) — Alibaba Qwen. 82% HumanEval-Score, spezialisiert für Coding-Aufgaben. Apache 2.0 lizenziert.',
...
schema: {
      ...
      faqSchema: {
        ...
        'mainEntity': [
          ...
          { '@type': 'Question', 'name': 'Was ist der beste lokale LLM für Code-Review?', 'acceptedAnswer': { '@type': 'Answer', 'text': 'vLLM + Qwen2.5-Coder-7B-Instruct. Qwen2.5-Coder erzielt 82% auf HumanEval (bestes Open-Source-Coding-Modell). vLLM verarbeitet 10 Dateien parallel. Ca. 30–50 Tok/Sek auf RTX 3060 12 GB.' } },
      ...
      itemListSchema: {
        ...
        'itemListElement': [
          ...
          { '@type': 'ListItem', 'position': 2, 'name': 'Softwareentwicklung & Code-Review', 'description': 'vLLM + Qwen2.5-Coder-7B-Instruct (RTX 3060 12 GB) oder -14B (RTX 4070+). 3–5-fach schneller als Ollama für Batch-Inferenz. 82% HumanEval-Score. OpenAI-kompatible API, Token-Streaming für Echtzeit-Codevervollständigungen.' },
          ...
          { '@type': 'ListItem', 'position': 6, 'name': 'Fine-Tuning & Forschung', 'description': 'HuggingFace Transformers + LoRA + Ollama/vLLM für Inferenz. LoRA reduziert VRAM-Bedarf um das 10-Fache. Separat trainieren, auf Inferenz-Box deployen. Llama 3.2 und Qwen2.5 empfohlen (Llama 3.3 ab April 2026 eingestellt).' },
    ...
    fr: {
      ...
      metaDescription: 'Stack LLM local par cas d\'usage avril 2026 : Coding → vLLM + Qwen2.5-Coder. Rédaction → Ollama + Llama 3.3. RAG → LlamaIndex + Qdrant. 8–24 Go VRAM. Guide complet.',
      twitterDescription: 'Stack LLM local par cas d\'usage : Coding → vLLM + Qwen2.5-Coder. Rédaction → Ollama + Llama 3.3. RAG → LlamaIndex + Qdrant. Agents → LangGraph + vLLM. Avril 2026.',
      publishDate: '2026-04-05',
      dateModified: '2026-04-19',
      ...
      sections: {
        ...
        'hardware': {
          ...
          rows: [
            { 'Votre matériel': '4–8 Go VRAM (GTX 1660, RTX 3050)', 'Rédaction': 'Ollama + Phi-4 Mini', 'Coding': 'Ollama + Qwen2.5-Coder-1.5B', 'RAG': 'LlamaIndex + Phi-4 Mini', 'Agents': 'Non recommandé' },
            { 'Votre matériel': '12 Go VRAM (RTX 3060, RTX 4070)', 'Rédaction': 'Ollama + Llama 3.2 8B', 'Coding': 'vLLM + Qwen2.5-Coder-7B', 'RAG': 'LlamaIndex + Llama 3.2 8B', 'Agents': 'LangGraph + Ollama (plus lent)' },
            { 'Votre matériel': '16 Go VRAM (RTX 4070 Ti, RTX 4080)', 'Rédaction': 'Ollama + Mistral Small 3.1', 'Coding': 'vLLM + Qwen2.5-Coder-14B', 'RAG': 'LlamaIndex + Mistral 3.1', 'Agents': 'LangGraph + vLLM' },
            { 'Votre matériel': '24 Go VRAM (RTX 3090, RTX 4090)', 'Rédaction': 'Ollama + Llama 3.3 70B Q4', 'Coding': 'vLLM + Qwen2.5-Coder-32B', 'RAG': 'LlamaIndex + Llama 3.3 70B', 'Agents': 'LangGraph + vLLM (le plus rapide)' },
          ],
        },
        'writing': {
          id: 'writing',
    ...
    print(f"=== {filename} ===\n{review}\n")`,
        ...
        'finetuning': {
          ...
          content: [
            ...
            '',
            '**Note (avril 2026) :** Meta a déprécié Llama 3.3 pour le fine-tuning commercial. Affinez sur Llama 3.2 (`meta-llama/Llama-3.2-1B` ou plus grand) ou Qwen2.5 (`Qwen/Qwen2.5-7B`) pour les termes de licence Apache 2.0 / open-source. Les deux supportent LoRA et se chargent facilement dans Ollama.',
          ],
          numberedItems: [
        ...
        'faqSection': {
          ...
          faqs: [
            ...
            { q: 'Quel est le meilleur LLM local pour la revue de code ?', a: 'vLLM + Qwen2.5-Coder-7B-Instruct. Qwen2.5-Coder obtient 82 % sur HumanEval. vLLM traite 10 fichiers en parallèle. ~30–50 tok/s sur RTX 3060 12 Go.' },
        ...
        'sources': {
          ...
          items: [
            ...
            '[Rapport technique Qwen2.5-Coder](https://qwenlm.github.io/blog/qwen2.5-coder/) — Alibaba Qwen. Score HumanEval 82 %, spécialisé pour le coding. Licence Apache 2.0.',
...
schema: {
      ...
      faqSchema: {
        ...
        'mainEntity': [
          ...
          { '@type': 'Question', 'name': 'Quel est le meilleur LLM local pour la revue de code ?', 'acceptedAnswer': { '@type': 'Answer', 'text': 'vLLM + Qwen2.5-Coder-7B-Instruct. Qwen2.5-Coder obtient 82 % sur HumanEval. vLLM traite 10 fichiers en parallèle. ~30–50 tok/s sur RTX 3060 12 Go.' } },
      ...
      itemListSchema: {
        ...
        'itemListElement': [
          ...
          { '@type': 'ListItem', 'position': 2, 'name': 'Développement logiciel & revue de code', 'description': 'vLLM + Qwen2.5-Coder-7B-Instruct (RTX 3060 12 Go) ou -14B (RTX 4070+). Plus rapide pour l\'inférence par lots (3–5× vs Ollama). Score HumanEval 82 %. API compatible OpenAI, streaming de tokens pour les complétions en temps réel.' },
          ...
          { '@type': 'ListItem', 'position': 6, 'name': 'Fine-tuning & recherche', 'description': 'HuggingFace Transformers + LoRA + Ollama/vLLM pour l\'inférence. LoRA réduit l\'empreinte VRAM par 10×. Entraînez séparément sur du matériel dédié, servez sur la machine d\'inférence. Llama 3.2 et Qwen2.5 recommandés (Llama 3.3 déprécié depuis avril 2026).' },
    ...
    ja: {
      ...
      intro: '**最適なローカルLLMスタックはワークフロー次第で決まる：ライティング→Ollama + OpenWebUI + Llama 3.3、開発→vLLM + Qwen2.5-Coder + IDE拡張、研究→LangGraph + vLLM。2026年4月現在、すべてのユースケースをカバーする単一ツールは存在しない。** 本ガイドは7つの主要ユースケースを最適スタック（backend + UI + インテグレーション）とハードウェア層（8–24 GB VRAM）にマッピングします。',
      metaDescription: 'ユースケース別ローカルLLMスタック2026年4月：コーディング→vLLM + Qwen2.5-Coder。ライティング→Ollama + Llama 3.3。RAG→LlamaIndex + Qdrant。VRAM 8–24 GB対応。完全セットアップガイド。',
      twitterDescription: 'ユースケース別ローカルLLMスタック：コーディング→vLLM + Qwen2.5-Coder。ライティング→Ollama + Llama 3.3。RAG→LlamaIndex + Qdrant。エージェント→LangGraph + vLLM。2026年4月版。',
      publishDate: '2026-04-05',
      dateModified: '2026-04-19',
      ...
      sections: {
        ...
        'hardware': {
          ...
          rows: [
            { 'お使いのハード': '4–8 GB VRAM（GTX 1660、RTX 3050）', 'ライティング': 'Ollama + Phi-4 Mini', 'コーディング': 'Ollama + Qwen2.5-Coder-1.5B', 'RAG': 'LlamaIndex + Phi-4 Mini', 'エージェント': '非推奨' },
            { 'お使いのハード': '12 GB VRAM（RTX 3060、RTX 4070）', 'ライティング': 'Ollama + Llama 3.2 8B', 'コーディング': 'vLLM + Qwen2.5-Coder-7B', 'RAG': 'LlamaIndex + Llama 3.2 8B', 'エージェント': 'LangGraph + Ollama（低速）' },
            { 'お使いのハード': '16 GB VRAM（RTX 4070 Ti、RTX 4080）', 'ライティング': 'Ollama + Mistral Small 3.1', 'コーディング': 'vLLM + Qwen2.5-Coder-14B', 'RAG': 'LlamaIndex + Mistral 3.1', 'エージェント': 'LangGraph + vLLM' },
            { 'お使いのハード': '24 GB VRAM（RTX 3090、RTX 4090）', 'ライティング': 'Ollama + Llama 3.3 70B Q4', 'コーディング': 'vLLM + Qwen2.5-Coder-32B', 'RAG': 'LlamaIndex + Llama 3.3 70B', 'エージェント': 'LangGraph + vLLM（最速）' },
          ],
        },
        'writing': {
          id: 'writing',
        ...
        'coding': {
          ...
          numberedItems: [
            ...
            'Qwen2.5-Coder-7BでvLLMサーバーを起動：`python -m vllm.entrypoints.openai.api_server --model Qwen/Qwen2.5-Coder-7B-Instruct --port 8000`',
            '16+ GB VRAMの場合は14Bを使用：`--model Qwen/Qwen2.5-Coder-14B-Instruct`',
    ...
    print(f"=== {filename} ===\n{review}\n")`,
        ...
        'finetuning': {
          ...
          content: [
            ...
            '',
            '**注意（2026年4月）：** MetaはLlama 3.3の商用ファインチューニングを非推奨。Llama 3.2（`meta-llama/Llama-3.2-1B`以上）またはQwen2.5（`Qwen/Qwen2.5-7B`）のApache 2.0ライセンスを選択。両方ともLoRAとOllama読み込みに対応。',
          ],
          numberedItems: [
        ...
        'faqSection': {
          ...
          faqs: [
            ...
            { q: 'コードレビューに最適なローカルLLMは？', a: 'vLLM + Qwen2.5-Coder-7B-Instruct。Qwen2.5-CoderはHumanEvalで８２％（オープンソース最高）。vLLMは10ファイルを並列分。RTX 3060 12GBで絀30–50 tok/s。' },
        ...
        'sources': {
          ...
          items: [
            ...
            '[Qwen2.5-Coder技術レポート](https://qwenlm.github.io/blog/qwen2.5-coder/) — Alibaba Qwen。HumanEval 82％、コーディング特化。Apache 2.0ライセンス。',
...
schema: {
      ...
      faqSchema: {
        ...
        'mainEntity': [
          ...
          { '@type': 'Question', 'name': 'コードレビューに最適なローカルLLMは？', 'acceptedAnswer': { '@type': 'Answer', 'text': 'vLLM + Qwen2.5-Coder-7B-Instruct。HumanEval 82％。vLLMは10ファイルを並列分。RTX 3060 12GBで絀30–50 tok/s。' } },
      ...
      itemListSchema: {
        ...
        'itemListElement': [
          ...
          { '@type': 'ListItem', 'position': 2, 'name': 'ソフトウェア開発 & コードレビュー', 'description': 'vLLM + Qwen2.5-Coder-7B-Instruct（RTX 3060 12 GB）または-14B（RTX 4070+）。バッチ推論に最高速（Ollamaの3–5倍）。HumanEval 82％。リアルタイム補完向けストリーミング仑8、OpenAI互换API。' },
          ...
          { '@type': 'ListItem', 'position': 6, 'name': 'ファインチューニング & 研究', 'description': 'HuggingFace Transformers + LoRA + Ollama/vLLM（推論）。LoRAはVRAMフットプリントを１０分の１に削減。専用ハードで別流トレーニング、推論マシンでserving。Llama 3.2とQwen2.5推奨（2026年4月以降Llama 3.3は非推奨）。' },
    ...
    zh: {
      ...
      intro: '**最佳本地LLM堆栈取决于具体工作流：内容写作→Ollama + OpenWebUI + Llama 3.3，软件开发→vLLM + Qwen2.5-Coder + IDE插件，程式研究→LangGraph + vLLM。2026年4月更新：无单一工具能涵盖所有场景。** 本指南将的7个常见用途与最佳堆栈（后端 + UI + 集成）及硬件配置（8–24 GB显存）一一对应。',
      metaDescription: '按用途本地LLM堆捨2026年4月：编程→vLLM+Qwen2.5-Coder，写作→Ollama+Llama 3.3，RAG→LlamaIndex+Qdrant。8-24GB显存。完整配置指南。',
      twitterDescription: '本地LLM堆栈：编程→vLLM+Qwen2.5-Coder，写作→Ollama+Llama 3.3，RAG→LlamaIndex+Qdrant，智能体→LangGraph+vLLM。2026年4月。',
      publishDate: '2026-04-05',
      dateModified: '2026-04-19',
      ...
      sections: {
        ...
        'hardware': {
          ...
          rows: [
            { '您的硬件': '4–8 GB显存（GTX 1660、RTX 3050）', '写作': 'Ollama + Phi-4 Mini', '编程': 'Ollama + Qwen2.5-Coder-1.5B', 'RAG': 'LlamaIndex + Phi-4 Mini', '智能体': '不推荐' },
            { '您的硬件': '12 GB显存（RTX 3060、RTX 4070）', '写作': 'Ollama + Llama 3.2 8B', '编程': 'vLLM + Qwen2.5-Coder-7B', 'RAG': 'LlamaIndex + Llama 3.2 8B', '智能体': 'LangGraph + Ollama（较慢）' },
            { '您的硬件': '16 GB显存（RTX 4070 Ti、RTX 4080）', '写作': 'Ollama + Mistral Small 3.1', '编程': 'vLLM + Qwen2.5-Coder-14B', 'RAG': 'LlamaIndex + Mistral 3.1', '智能体': 'LangGraph + vLLM' },
            { '您的硬件': '24 GB显存（RTX 3090、RTX 4090）', '写作': 'Ollama + Llama 3.3 70B Q4', '编程': 'vLLM + Qwen2.5-Coder-32B', 'RAG': 'LlamaIndex + Llama 3.3 70B', '智能体': 'LangGraph + vLLM（最快）' },
          ],
        },
        'writing': {
          id: 'writing',
    ...
    print(f"=== {filename} ===\n{review}\n")`,
        ...
        'finetuning': {
          ...
          content: [
            ...
            '',
            '**注意（2026年4月）：** Meta已对Llama 3.3商业微调不再推荐。建议在Llama 3.2（`meta-llama/Llama-3.2-1B`或更大）或Qwen2.5（`Qwen/Qwen2.5-7B`）上微调，遵循pache 2.0许可证。两者均支持LoRA且可在Ollama中加载。',
          ],
          numberedItems: [
        ...
        'faqSection': {
          ...
          faqs: [
            ...
            { q: '代码审查最佳本地LLM是什么？', a: 'vLLM + Qwen2.5-Coder-7B-Instruct。Qwen2.5-Coder在HumanEval得劆8剸2%（开源最佳）。vLLM并行处理10个文件。RTX 3060 12GB成逗30–50 tok/s。' },
        ...
        'sources': {
          ...
          items: [
            ...
            '[Qwen2.5-Coder技术报告](https://qwenlm.github.io/blog/qwen2.5-coder/) — 阿里巴巴通义千问。HumanEval 82%，代码专用。Apache 2.0许可。',
...
schema: {
      ...
      faqSchema: {
        ...
        'mainEntity': [
          ...
          { '@type': 'Question', 'name': '代码审查最佳本地LLM是什么？', 'acceptedAnswer': { '@type': 'Answer', 'text': 'vLLM + Qwen2.5-Coder-7B-Instruct。HumanEval 82%。vLLM并行处理10个文件。RTX 3060 12GB成逗30–50 tok/s。' } },
      ...
      itemListSchema: {
        ...
        'itemListElement': [
          ...
          { '@type': 'ListItem', 'position': 2, 'name': '软件开发 & 代码审查', 'description': 'vLLM + Qwen2.5-Coder-7B-Instruct（RTX 3060 12 GB）或-14B（RTX 4070+）。批量推理最快（Ollama的3–5倍）。HumanEval 82%。支持实时补全流式传输与OpenAI兼容API。' },
          ...
          { '@type': 'ListItem', 'position': 6, 'name': '微调 & 研究', 'description': 'HuggingFace Transformers + LoRA + Ollama/vLLM推理。LoRA将显存占用减小10倍。在层用硬件单独训练，在推理机器serving。推荐Llama 3.2和Qwen2.5（2026年4月起Llama 3.3已不推荐）。' },
