...
export const article: Partial<Record<Language, LLMArticle>> = {
    en: {
      ...
      leadAnswerBlock: '**Fine-tuning adapts a pre-trained model to your domain using LoRA (Low-Rank Adaptation) — add small adapter layers (0.4% of total weights) instead of retraining the entire model. A Llama 3.3 8B fine-tune requires 8 GB VRAM and 1–2 hours on consumer hardware using Unsloth (up to 2× faster than standard training).**',
...
schema: {
        ...
        'description': 'Fine-tune Llama 3.3 8B with LoRA on 8 GB VRAM in 1–2 hours using Unsloth (up to 2× faster). Only 500 examples needed. Full code, hyperparameters, and LoRA vs RAG decision matrix.',
    ...
    es: {
      ...
      metaDescription: 'Haz fine-tuning de Llama 3.3 8B con LoRA en 8 GB de VRAM en 1–2 horas con Unsloth (hasta 2× más rápido que el entrenamiento estándar, QLoRA). Mínimo 500 ejemplos. Código y guía LoRA vs RAG.',
      publishDate: '2026-04-04',
      ...
      schema: {
        ...
        'description': 'Haz fine-tuning de Llama 3.3 8B con LoRA en 8 GB de VRAM en 1–2 horas usando Unsloth (hasta 2× más rápido). Solo 500 ejemplos necesarios. Código completo, hiperparámetros y matriz de decisión LoRA vs RAG.',
    ...
    ar: {
      ...
      schema: {
        ...
        'description': 'اضبط Llama 3.3 8B بدقة بـ LoRA على 8 GB من VRAM في 1–2 ساعة باستخدام Unsloth (أسرع بـ 2× وفق المعايير الرسمية). 500 مثال فقط مطلوبة. كود كامل ومعاملات فائقة ومصفوفة قرار LoRA مقابل RAG.',
    ...
    pt: {
      ...
      metaDescription: 'Faça fine-tuning do Llama 3.3 8B com LoRA em 8 GB de VRAM em 1–2 horas com Unsloth (até 2× mais rápido que o treinamento padrão, QLoRA). Mínimo 500 exemplos. Código e guia LoRA vs RAG.',
      publishDate: '2026-04-04',
...
schema: {
        ...
        'description': 'Fine-Tunen Sie Llama 3.3 8B mit LoRA auf 8 GB VRAM in 1–2 Stunden mit Unsloth (bis zu 2× schneller). Benötigt nur 500 Beispiele. Vollständiger Code, Hyperparameter und LoRA vs RAG Entscheidungsmatrix.',
    ...
    fr: {
      ...
      metaDescription: 'Fine-tuner Llama 3.3 8B avec LoRA sur 8 Go de VRAM en 1–2 heures avec Unsloth (jusqu\'à 2× plus rapide). Nécessite seulement 500 exemples. Code complet, hyperparamètres et matrice de décision LoRA vs RAG.',
      publishDate: '2026-04-04',
...
schema: {
        ...
        'description': 'Fine-tuner Llama 3.3 8B avec LoRA sur 8 Go de VRAM en 1–2 heures avec Unsloth (jusqu\'à 2× plus rapide). Nécessite seulement 500 exemples. Code complet, hyperparamètres et matrice de décision LoRA vs RAG.',
    ...
    ja: {
      ...
      intro: 'ファインチューニングは、LoRA（Low-Rank Adaptation）を使用して事前学習済みモデルをドメインに適応させます。— 全体をリトレーニングする代わりに、小さなアダプター層（総重量の0.4%）を追加します。Llama 3.3 8B のファインチューニングには、Unsloth（標準トレーニングより最大2倍高速、[unsloth.ai 公式情報](https://github.com/unslothai/unsloth)）を使用して、コンシューマーハードウェアで 8GB VRAM と1～2時間が必要です。2026年4月現在、LoRA と QLoRA（4ビット量子化 LoRA）は Ollama、LM Studio、vLLM で本番対応です。',
      metaDescription: 'Unsloth（最大2倍高速）を使用して、Llama 3.3 8B を 8GB VRAM で 1～2時間でファインチューニングします。500例のみ必要。完全なコード、ハイパーパラメータ、および LoRA vs RAG 決定マトリックス。',
      publishDate: '2026-04-04',
      leadAnswerBlock: '**ファインチューニングは、LoRA（Low-Rank Adaptation）を使用して事前学習済みモデルをドメインに適応させます。— 全体をリトレーニングする代わりに、小さなアダプター層（総重量の0.4%）を追加します。Llama 3.3 8B のファインチューニングには、Unsloth（標準トレーニングより最大2倍高速）を使用して、コンシューマーハードウェアで 8GB VRAM と1～2時間が必要です。**',
...
schema: {
        ...
        'description': 'Unsloth（最大2倍高速）を使用して、Llama 3.3 8B を 8GB VRAM で 1～2時間でファインチューニングします。500例のみ必要。完全なコード、ハイパーパラメータ、および LoRA vs RAG 決定マトリックス。',
    ...
    zh: {
      ...
      intro: '微调通过 LoRA（低秩适应）将预训练模型适应到你的领域——只需添加小型适配器层（占总权重的 0.4%）而不是重新训练整个模型。使用 Unsloth（最快快 2 倍，[据 unsloth.ai 官方数据](https://github.com/unslothai/unsloth)）在 8GB VRAM 的消费级硬件上微调 Llama 3.3 8B 需要 1-2 小时。截至 2026 年 4 月，LoRA 和 QLoRA（4 位量化 LoRA）在 Ollama、LM Studio 和 vLLM 中已经生产就绪。',
      metaDescription: '使用 Unsloth（最快快 2 倍）在 8GB VRAM 上 1-2 小时内微调 Llama 3.3 8B。只需 500 个示例。完整代码、超参数和 LoRA vs RAG 决策矩阵。',
      publishDate: '2026-04-04',
      leadAnswerBlock: '**微调通过 LoRA（低秩适应）将预训练模型适应到你的领域——只需添加小型适配器层（占总权重的 0.4%）而不是重新训练整个模型。使用 Unsloth（最快快 2 倍）在 8GB VRAM 的消费级硬件上微调 Llama 3.3 8B 需要 1-2 小时。**',
...
schema: {
        ...
        'description': '使用 Unsloth（最快快 2 倍）在 8GB VRAM 上 1-2 小时内微调 Llama 3.3 8B。只需 500 个示例。完整代码、超参数和 LoRA vs RAG 决策矩阵。',
  ...
  ko: {
    ...
    heroImage: '/images/fine-tuning-local-llms-lora-overview-hero-ko.webp',
      ...
      intro: '파인튜닝은 LoRA(Low-Rank Adaptation)를 사용하여 사전 학습된 모델을 특정 도메인에 맞게 조정합니다 — 전체 모델을 재학습하는 대신, 소형 어댑터 레이어(전체 가중치의 0.4%)를 추가합니다. Llama 3.3 8B 파인튜닝은 Unsloth를 사용하면 8 GB VRAM과 1~2시간으로 소비자 하드웨어에서 가능합니다(표준 학습 대비 최대 2배 빠름, [unsloth.ai 공식 기준](https://github.com/unslothai/unsloth)). 2026년 4월 기준, LoRA와 QLoRA(4비트 양자화 LoRA)는 Ollama, LM Studio, vLLM에서 프로덕션 수준으로 지원됩니다.',
      metaDescription: 'Unsloth(최대 2배 빠른 QLoRA)를 사용하여 1~2시간 만에 8 GB VRAM으로 Llama 3.3 8B를 파인튜닝하십시오. 최소 500개 훈련 예제. 전체 코드 + LoRA vs RAG 의사결정 가이드.',
      publishDate: '2026-04-04',
      leadAnswerBlock: '**파인튜닝은 LoRA(Low-Rank Adaptation)를 사용하여 사전 학습된 모델을 특정 도메인에 맞게 조정합니다 — 전체 모델을 재학습하는 대신, 소형 어댑터 레이어(전체 가중치의 0.4%)를 추가합니다. Llama 3.3 8B 파인튜닝은 Unsloth를 사용하면 8 GB VRAM과 1~2시간으로 소비자 하드웨어에서 가능합니다(표준 학습 대비 최대 2배 빠름).**',
