      ...
      sections: {
        ...
        hardware: {
          ...
          content: [
            ...
            '**CPU-only (no GPU): best models for laptops without a dedicated GPU.** Phi-4 (14B, Q4) is the strongest reasoning model that runs at a usable 4–8 tok/s on a modern Intel or AMD laptop CPU with 16 GB RAM. Qwen3 1.7B and Llama 3.1 8B Q4 are also practical at 5–10 tok/s CPU-only. Apple Silicon (M-series) is the exception — the unified memory architecture runs at 45–120 tok/s on the same model sizes that a PC CPU would run at 5 tok/s, making it far more practical for CPU-only local inference.',
          ],
...
schema: {
    ...
    fr: {
      ...
      metaDescription: 'Local = données 100% privées, 0 € par token, 10x plus lent. Cloud = 50–150 tokens/s, 0,01–0,05 € par 1K tokens, accès web. Tableau comparatif complet + guide décisionnel.',
...
schema: {
    ...
    ja: {
      ...
      metaDescription: 'ローカル=100%プライバシー・無料・10倍遅い。クラウド=毎秒50–150トークン・1000トークン0.01–0.05ドル・ウェブ検索可。全比較表とAI使い分けガイド。',
...
schema: {
    ...
    zh: {
      ...
      metaDescription: '本地LLM=100%隐私、免费、速度慢10倍。云API=每秒50–150词元、1K词元0.01–0.05美元、支持网络搜索。完整对比表+选择指南。',
  ...
  ko: {
      ...
      sections: {
        ...
        hardware: {
          ...
          content: [
            '**유능한 로컬 모델(13B 이상)을 실행하려면 모든 사용자가 갖추지 못한 하드웨어가 필요합니다.** GPT-5.6 Luna 품질에 맞는 진정으로 유용한 로컬 LLM 경험을 위한 최소 조건은 16GB RAM과 현대적인 CPU 또는 Apple Silicon 칩입니다. 이는 현재 사용 중인 소비자용 노트북의 약 절반을 제외합니다. 자세한 내용과 VRAM 계산은 [로컬 LLM 하드웨어 가이드 2026](/ko/local-llms/local-llm-hardware-guide-2026)을 참조하십시오.',
        ...
        noInternet: {
          ...
          content: [
            ...
            '현재 정보가 필요한 작업(뉴스 요약, 최신 제품 비교, 실시간 데이터 분석)에는 클라우드 API가 실용적인 선택입니다. 전체 비교는 [로컬 LLM vs 클라우드 API](/ko/local-llms/local-llms-vs-cloud-apis)를 참조하십시오.',
          ],
        },
        whenRealtimeMatters: {
        ...
        setup: {
          ...
          content: [
            '**클라우드 API는 계정 생성, API 키 생성, HTTP 호출만 필요하며 일반적으로 총 5~10분이 소요됩니다.** 로컬 LLM은 추론 엔진([Ollama](/ko/local-llms/run-first-local-llm) 또는 LM Studio 등) 설치, 모델 파일 다운로드(2~50GB), GPU 오프로딩 구성, 드라이버 문제 해결이 필요합니다. [Ollama](https://ollama.com/)는 단일 바이너리 설치로 이를 줄여 수동 설치보다 과정을 단순화합니다.',
            ...
            '단계별 지침은 [Ollama 설치 방법](/ko/local-llms/run-first-local-llm)을 참조하고, 가장 일반적인 오류에 대한 수정사항은 [로컬 LLM 설정 문제 해결](/ko/local-llms/troubleshooting-local-llm-setup)을 참조하십시오. 전체 설정 시간 비교는 설정 시간: 로컬 vs 클라우드를 참조하십시오.',
          ],
          image: '/images/local-llm-limitations-setup-time-en.svg',
        ...
        whenCloud: {
          ...
          items: [
            '**최대 출력 품질이 필요한 경우** -- 법률 문서, 복잡한 코드 생성, 고급 연구 분석. GPT-5.6 또는 Claude Sonnet 5을 사용하십시오. 전체 비교는 [로컬 LLM vs 클라우드 API](/ko/local-llms/local-llms-vs-cloud-apis)를 참조하십시오.',
        ...
        commonMistakes: {
          ...
          items: [
            ...
            '**하드웨어 한계를 무시하는 것:** 유용한 모델에는 16GB RAM이 최소입니다. 그 이하에서는 품질이 크게 저하됩니다. 시작하기 전에 [하드웨어 요구사항](/ko/local-llms/local-llm-hardware-guide-2026)을 확인하십시오.',
        ...
        relatedReading: {
          ...
          title: '관련 읽기 자료',
          items: [
            '[로컬 LLM이 무엇이며 하드웨어에서 어떻게 실행되는지 이해하기](/ko/local-llms/what-are-local-llms) -- 핵심 개념, 아키텍처 및 클라우드 모델과의 주요 차이점',
            '[로컬 LLM vs 클라우드 API 비교: 각 방식의 사용 시기](/ko/local-llms/local-llms-vs-cloud-apis) -- 트레이드오프 분석, 비용 비교 및 결정 안내',
            '[제한된 하드웨어를 위한 입문자 친화적 최고의 로컬 LLM 모델 찾기](/ko/local-llms/best-beginner-local-llm-models) -- 모델 추천, VRAM 요구사항 및 설정 옵션',
            '[2026년 사용 가능한 최고의 로컬 LLM 탐색하기](/ko/local-llms/best-local-llms-2026) -- 상위 모델, 벤치마크 점수 및 성능 비교',
          ],
        },
      },
    },
