...
export const article: Partial<Record<Language, LLMArticle>> = {
  en: {
    ...
    current_models_mentioned: ['Llama 3.3 70B', 'DeepSeek-R1-Distill-Llama-70B', 'Qwen3 72B', 'DeepSeek 67B', 'Mixtral 8x22B', 'Llama 3.1 8B'],
...
for chunk in stream_generate(model, tokenizer, "Explain quantum computing", max_tokens=500):
  ...
  es: {
    ...
    publishDate: '2026-05-15',
    dateModified: '2026-08-26',
    current_models_mentioned: ['Llama 3.3 70B', 'Qwen3 72B', 'DeepSeek 67B', 'Mixtral 8x22B', 'Llama 3.1 8B'],
    current_hardware_mentioned: ['M5 Max', 'M5 Ultra', 'M4 Max', 'M3 Max', 'RTX 4090'],
...
def detect_recursion(threshold=900):
    ...
    return decorator`,
      ...
      alternatives: {
        ...
        note: 'Recomendaciones por caso de uso: Razonamiento general → Llama 3.3 70B Q5. Código → DeepSeek 67B. No inglés → Qwen3 72B. Preguntas sobre documentos → Command A+. Máxima velocidad → Mixtral 8x22B (MoE usa menos parámetros activos). (DeepSeek ha lanzado desde entonces DeepSeek-V4 —Flash/Pro— como nueva generación de pesos abiertos; R1/V3 siguen siendo válidos para ejecutar en local.)',
      },
      alternativesPull: {
        id: 'alternatives-pull',
...
def detect_recursion(threshold=900):
    ...
    return decorator`,
      ...
      alternatives: {
        ...
        note: 'توصيات حسب حالة الاستخدام: استدلال عام ← Llama 3.3 70B Q5. كود ← DeepSeek 67B. غير الإنجليزية ← Qwen3 72B. أسئلة عن المستندات ← Command A+. أقصى سرعة ← Mixtral 8x22B (يستخدم MoE معاملات نشطة أقل). (أصدرت DeepSeek منذ ذلك الحين DeepSeek-V4 — Flash/Pro — كجيل جديد بأوزان مفتوحة؛ يظل R1/V3 صالحًا للتشغيل محليًا.)',
      },
      alternativesPull: {
        id: 'alternatives-pull',
...
def detect_recursion(threshold=900):
    ...
    return decorator`,
      ...
      alternatives: {
        ...
        note: 'ユースケース別推奨：汎用推論 → Llama 3.3 70B Q5。コード → DeepSeek 67B。英語以外 → Qwen3 72B。文書Q&A → Command A+。最大速度 → Mixtral 8x22B（MoEはアクティブパラメーター数が少ない）。 (DeepSeekはその後、オープンウェイトの新世代としてDeepSeek-V4—Flash/Pro—をリリースしました。R1/V3は引き続きローカルで利用可能です。)',
      },
      alternativesPull: {
        id: 'alternatives-pull',
        title: '代替モデルのダウンロード',
...
def detect_recursion(threshold=900):
    ...
    return decorator`,
      ...
      alternatives: {
        ...
        note: '按用例推荐：通用推理 → Llama 3.3 70B Q5。代码 → DeepSeek 67B。非英语（含中文）→ Qwen3 72B。文档问答 → Command A+。最高速度 → Mixtral 8x22B（MoE使用的活跃参数更少）。 (DeepSeek 此后发布了开源权重新一代模型 DeepSeek-V4——Flash/Pro;R1/V3 仍可在本地正常使用。)',
      },
      alternativesPull: {
        id: 'alternatives-pull',
        title: '下载替代模型',
...
def detect_recursion(threshold=900):
    ...
    return decorator`,
      ...
      alternatives: {
        ...
        note: '용도별 권장사항: 범용 추론 → Llama 3.3 70B Q5. 코드 → DeepSeek 67B. 비영어권 → Qwen3 72B. 문서 Q&A → Command A+. 최대 속도 → Mixtral 8x22B (MoE는 더 적은 활성 파라미터를 사용). (DeepSeek는 이후 오픈 웨이트 신세대 모델인 DeepSeek-V4—Flash/Pro—를 출시했습니다. R1/V3는 계속 로컬에서 사용할 수 있습니다.)',
      },
      alternativesPull: {
        id: 'alternatives-pull',
        title: '대안 모델 다운로드',
