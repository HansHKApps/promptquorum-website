...
export const article: Partial<Record<Language, LLMArticle>> = {
    en: {
      ...
      heroImage: '/images/on-prem-air-gapped-local-llm-overview-hero-en.webp',
      dateModified: '2026-08-28',
      seoTitle: 'Air-Gapped Local LLM Deployment',
      intro: 'Air-gapped deployment means the LLM infrastructure is completely isolated from the internet. No internet connection, no external API calls, no data exfiltration risk. This is standard practice for government, military, and financial institutions handling classified or highly sensitive data.',
      ...
      leadAnswerBlock: '**Air-gapped deployment means the LLM infrastructure is completely isolated from the internet. No internet connection, no external API calls, no data exfiltration risk. This is standard practice for government, military, and financial institutions handling classified or highly sensitive data.**',
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'Air-gapped deployment means zero network connectivity between the LLM infrastructure and the internet or any external system -- models and updates move only via physical media or isolated internal networks.' },
            { type: 'plain-terms', text: 'An air-gapped AI server is completely disconnected from the internet, like a computer with no network cable and no WiFi. This is the highest level of data protection available: nothing can be hacked remotely because there is no remote connection to attack. The tradeoff is that updating the AI model takes manual work instead of an automatic download.' },
          ],
          items: [
            ...
            'Air-gapped deployment is standard practice for government agencies and defense contractors handling classified data.',
          ],
        },
        whatIsAirGapped: {
...
schema: {
    ...
    es: {
      ...
      heroImage: '/images/on-prem-air-gapped-local-llm-overview-hero-es.webp',
      seoTitle: 'Despliegue de LLM Local con Air-Gap',
      intro: 'El despliegue air-gapped significa que la infraestructura del LLM está completamente aislada de internet. Sin conexión a internet, sin llamadas a API externas, sin riesgo de exfiltración de datos. esto es el estándar para gobiernos, militares e instituciones financieras que manejan datos clasificados o altamente sensibles.',
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'El despliegue air-gapped implica cero conectividad de red entre la infraestructura del LLM e internet o cualquier sistema externo -- los modelos y las actualizaciones se transfieren solo mediante medios físicos o redes internas aisladas.' },
            { type: 'plain-terms', text: 'Un servidor de IA air-gapped está completamente desconectado de internet, como un ordenador sin cable de red ni WiFi. Es el nivel más alto de protección de datos disponible: nada puede ser hackeado remotamente porque no existe conexión remota que atacar. La contrapartida es que actualizar el modelo de IA requiere trabajo manual en lugar de una descarga automática.' },
          ],
          items: [
            ...
            'Air-gapped es una práctica estándar para agencias gubernamentales y contratistas de defensa que manejan datos clasificados.',
          ],
        },
        whatIsAirGapped: {
    ...
    ar: {
      ...
      heroImage: '/images/on-prem-air-gapped-local-llm-overview-hero-ar.webp',
      seoTitle: '⁨LLM⁩ محلي معزول (⁨Air-Gap⁩): نشر في بيئات مصنّفة',
      intro: 'النشر المعزول (air-gapped) يعني أن بنية LLM التحتية معزولة تمامًا عن الإنترنت. بلا اتصال بالإنترنت، بلا استدعاءات API خارجية، بلا خطر تسريب البيانات. هذا هو المعيار للحكومات والجهات العسكرية والمؤسسات المالية التي تتعامل مع بيانات مصنّفة أو شديدة الحساسية.',
      ...
      publishDate: '2026-04-04',
      leadAnswerBlock: '**النشر المعزول (air-gapped) يعني أن بنية LLM التحتية معزولة تمامًا عن الإنترنت. بلا اتصال بالإنترنت، بلا استدعاءات API خارجية، بلا خطر تسريب البيانات. هذا هو المعيار للحكومات والجهات العسكرية والمؤسسات المالية التي تتعامل مع بيانات مصنّفة أو شديدة الحساسية.**',
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'يعني النشر المعزول (air-gapped) انعدام أي اتصال شبكي بين بنية LLM التحتية والإنترنت أو أي نظام خارجي -- تُنقل النماذج والتحديثات فقط عبر وسائط فعلية أو شبكات داخلية معزولة.' },
            { type: 'plain-terms', text: 'خادم الذكاء الاصطناعي المعزول غير متصل بالإنترنت إطلاقًا، تمامًا مثل حاسوب بلا كابل شبكة وبلا WiFi. هذا أعلى مستوى حماية للبيانات متاح: لا يمكن اختراقه عن بُعد لأنه لا يوجد اتصال عن بُعد يمكن مهاجمته. الثمن هو أن تحديث نموذج الذكاء الاصطناعي يتطلب عملاً يدويًا بدلاً من تنزيل تلقائي.' },
          ],
          items: [
            ...
            'النشر المعزول ممارسة معيارية للجهات الحكومية ومقاولي الدفاع الذين يتعاملون مع بيانات مصنّفة.',
          ],
        },
        whatIsAirGapped: {
    ...
    pt: {
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'A implantação air-gapped significa zero conectividade de rede entre a infraestrutura do LLM e a internet ou qualquer sistema externo -- os modelos e as atualizações são transferidos apenas por mídia física ou redes internas isoladas.' },
            { type: 'plain-terms', text: 'Um servidor de IA air-gapped está completamente desconectado da internet, como um computador sem cabo de rede e sem WiFi. Esse é o nível mais alto de proteção de dados disponível: nada pode ser invadido remotamente porque não existe conexão remota para atacar. A contrapartida é que atualizar o modelo de IA exige trabalho manual em vez de um download automático.' },
          ],
          items: [
            ...
            'Air-gapped é uma prática padrão para órgãos governamentais e contratantes de defesa que lidam com dados classificados.',
          ],
        },
        whatIsAirGapped: {
    ...
    de: {
      ...
      heroImage: '/images/on-prem-air-gapped-local-llm-overview-hero-de.webp',
      seoTitle: 'Air-Gapped Local LLM Bereitstellung',
      intro: 'Air-Gapped-Bereitstellung bedeutet, dass die LLM-Infrastruktur vollständig vom Internet isoliert ist. Keine Internetverbindung, keine externen API-Aufrufe, kein Datenabfluss-Risiko. Dies ist Standardpraxis für Behörden, Militär und Finanzinstitute, die mit klassifizierten oder hochsensiblen Daten umgehen.',
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'Air-Gapped-Bereitstellung bedeutet null Netzwerkverbindung zwischen der LLM-Infrastruktur und dem Internet oder jedem externen System -- Modelle und Updates werden ausschließlich über physische Datenträger oder isolierte interne Netzwerke übertragen.' },
            { type: 'plain-terms', text: 'Ein Air-Gapped-KI-Server ist vollständig vom Internet getrennt, wie ein Computer ohne Netzwerkkabel und ohne WLAN. Das ist der höchste verfügbare Schutzgrad für Daten: Nichts kann aus der Ferne gehackt werden, da keine Fernverbindung existiert, die angegriffen werden könnte. Der Nachteil: Die Aktualisierung des KI-Modells erfordert manuelle Arbeit statt eines automatischen Downloads.' },
          ],
          items: [
            ...
            'Air-Gapped ist gängige Praxis für Behörden und Rüstungsunternehmen, die mit klassifizierten Daten arbeiten.',
          ],
        },
        whatIsAirGapped: {
    ...
    fr: {
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: "Le déploiement air-gapped signifie une connectivité réseau nulle entre l'infrastructure LLM et Internet ou tout système externe -- les modèles et les mises à jour ne circulent que via des supports physiques ou des réseaux internes isolés." },
            { type: 'plain-terms', text: "Un serveur d'IA air-gapped est totalement déconnecté d'Internet, comme un ordinateur sans câble réseau et sans WiFi. C'est le niveau de protection des données le plus élevé qui soit : rien ne peut être piraté à distance puisqu'il n'existe aucune connexion distante à attaquer. La contrepartie est que la mise à jour du modèle d'IA demande un travail manuel plutôt qu'un téléchargement automatique." },
          ],
          items: [
            ...
            'L\'air-gapped est une pratique standard pour les agences gouvernementales et les entrepreneurs de défense traitant des données classifiées.',
          ],
        },
        whatIsAirGapped: {
          id: 'what-is-air-gapped',
    ...
    ja: {
      ...
      heroImage: '/images/on-prem-air-gapped-local-llm-overview-hero-ja.webp',
      seoTitle: 'エアギャップローカルLLM デプロイメント',
      intro: 'エアギャップデプロイメントはLLMインフラをインターネットから完全に隔離することを意味します。インターネット接続なし、外部API呼び出しなし、データ流出リスクなし。これは機密またはきわめて機密データを扱う政府、軍事、金融機関の標準的な運用です。',
      metaDescription: 'エアギャップローカルLLMデプロイメント: 完全隔離アーキテクチャ、ネットワークセキュリティ、更新管理、機密データ処理。',
      publishDate: '2026-04-04',
      leadAnswerBlock: '**エアギャップデプロイメントはLLMインフラをインターネットから完全に隔離することを意味します。インターネット接続なし、外部API呼び出しなし、データ流出リスクなし。これは機密またはきわめて機密データを扱う政府、軍事、金融機関の標準的な運用です。**',
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'エアギャップ展開とは、LLMインフラとインターネットまたは外部システムとの間のネットワーク接続がゼロであることを意味します。モデルと更新は物理メディアまたは隔離された内部ネットワークを通じてのみ移動します。' },
            { type: 'plain-terms', text: 'エアギャップAIサーバーは、ネットワークケーブルもWiFiもないコンピューターのように、インターネットから完全に切断されています。これは利用可能な最高レベルのデータ保護です。攻撃対象となるリモート接続が存在しないため、リモートからハッキングされることはありません。その代償として、AIモデルの更新は自動ダウンロードではなく手動作業が必要になります。' },
          ],
          items: [
            ...
            'エアギャップは、機密データを扱う政府機関や防衛関連企業にとって標準的な運用です。',
          ],
        },
        'what-is-air-gapped': {
          id: 'what-is-air-gapped',
    ...
    zh: {
      ...
      heroImage: '/images/on-prem-air-gapped-local-llm-overview-hero-zh.webp',
      seoTitle: '气隙本地LLM部署: 隔离架构与安全指南',
      intro: '气隙部署是指LLM基础设施完全隔离于互联网。无互联网连接、无外部API调用、无数据泄露风险。这是政府、军事和金融机构处理分类或高度敏感数据的标准做法。',
      metaDescription: '了解如何在完全隔离的网络环境中部署本地LLM: 包括网络隔离架构设计、模型离线管理、安全更新流程和分类信息保护。适用于政府、军事、金融机构。',
      publishDate: '2026-04-04',
      leadAnswerBlock: '**气隙部署是指LLM基础设施完全隔离于互联网。无互联网连接、无外部API调用、无数据泄露风险。这是政府、军事和金融机构处理分类或高度敏感数据的标准做法。**',
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: '气隙部署意味着LLM基础设施与互联网或任何外部系统之间零网络连接——模型和更新只能通过物理介质或隔离的内部网络传输。' },
            { type: 'plain-terms', text: '气隙AI服务器完全与互联网断开连接，就像一台没有网线也没有WiFi的电脑。这是可用的最高级别数据保护：由于不存在可供攻击的远程连接，因此无法远程入侵。代价是更新AI模型需要手动操作，而不是自动下载。' },
          ],
          items: [
  ...
  ko: {
    ...
    heroImage: '/images/on-prem-air-gapped-local-llm-overview-hero-ko.webp',
      seoTitle: '에어갭 로컬 LLM 배포',
      intro: '에어갭 배포란 LLM 인프라가 인터넷으로부터 완전히 격리된 상태를 의미합니다. 인터넷 연결 없음, 외부 API 호출 없음, 데이터 유출 위험 없음. 이는 기밀 또는 고도로 민감한 데이터를 다루는 정부, 군사, 금융 기관의 표준 방식입니다.',
      metaDescription: '에어갭 로컬 LLM 배포: 완전 격리 아키텍처, 네트워크 보안, 업데이트 관리, 기밀 데이터 처리.',
      publishDate: '2026-04-04',
      leadAnswerBlock: '에어갭 배포란 LLM 인프라가 인터넷으로부터 완전히 격리된 상태를 의미합니다. 인터넷 연결 없음, 외부 API 호출 없음, 데이터 유출 위험 없음. 이는 기밀 또는 고도로 민감한 데이터를 다루는 정부, 군사, 금융 기관의 표준 방식입니다.',
      ...
      toc: [
        ...
        { label: '에어갭이란?', anchor: '#what-is-air-gapped' },
        { label: '아키텍처 설계', anchor: '#architecture' },
        { label: '네트워크 격리', anchor: '#network-isolation' },
        { label: '모델 및 데이터 관리', anchor: '#model-management' },
        { label: '업데이트 및 유지보수', anchor: '#updates' },
        { label: '보안 고려사항', anchor: '#security' },
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: '에어갭 배포란 LLM 인프라와 인터넷 또는 외부 시스템 간의 네트워크 연결이 전혀 없음을 의미합니다. 모델과 업데이트는 오직 물리적 매체나 격리된 내부 네트워크를 통해서만 이동합니다.' },
            { type: 'plain-terms', text: '에어갭 AI 서버는 네트워크 케이블도 와이파이도 없는 컴퓨터처럼 인터넷과 완전히 단절되어 있습니다. 이는 이용 가능한 최고 수준의 데이터 보호입니다. 공격할 수 있는 원격 연결이 존재하지 않기 때문에 원격으로 해킹될 수 없습니다. 대신 AI 모델을 업데이트하려면 자동 다운로드가 아닌 수동 작업이 필요합니다.' },
          ],
          items: [
            '에어갭 = 인터넷으로부터 완전 격리. 외부 시스템과의 네트워크 연결 제로.',
            '사용 사례: 정부(기밀), 군사(전시 상황), 금융(고보안), 의료(HIPAA 초민감).',
            '과제: 모델, 임베딩, 의존성의 자동 업데이트 불가. 수동 업데이트 절차 필요.',
            '아키텍처: 전용 하드웨어, 암호화 스토리지, 제한된 사용자 접근, 물리적 보안.',
            '**네트워크 격리:** 방화벽, 에어갭 어플라이언스, 네트워크 세분화로 외부 통신 차단.',
            '**모델 관리:** USB, 보안 매체, 내부 네트워크를 통해서만 모델 이전. 클라우드 접근 불가.',
            '**업데이트:** 수동 절차 — 오프라인에서 먼저 테스트 후, 보안 채널을 통해 배포.',
            '에어갭은 기밀 데이터를 다루는 정부 기관 및 국방 계약업체의 표준 관행입니다.',
          ],
        },
        whatIsAirGapped: {
          title: '에어갭이란 무엇입니까?',
          content: [
            ...
            '**완전한 격리:** WiFi 없음, 외부 네트워크로의 이더넷 없음, 인터넷 연결 기기와의 USB 연결 없음.',
            '**데이터 외부 유출 불가:** 추론은 로컬에서 실행되고, 결과는 로컬에 저장됩니다.',
            '**업데이트는 수동:** 모델 업데이트 자동 다운로드 불가. 업데이트는 물리적 매체(USB 드라이브, SD 카드) 또는 내부 네트워크 이전이 필요합니다.',
          ],
        },
        architecture: {
          ...
          numberedItems: [
            '**전용 하드웨어:** LLM 추론에만 사용되는 서버, 다른 용도 없음.',
            '**격리된 네트워크:** 기업 네트워크 또는 인터넷과의 연결 없음. 최대 별도 VLAN만 허용.',
            '**암호화 스토리지:** 모든 모델 파일, 데이터, 로그는 저장 시 암호화.',
            '**통제된 접근:** 권한 있는 인원만 접근 가능. 다중 인증 필수.',
            '**물리적 보안:** 잠금 서버실, 감시 카메라, 접근 로그.',
            '**이동식 매체 금지:** USB 포트 비활성화, CD/DVD 드라이브 제거.',
            '**로컬 모니터링:** 로그는 시스템 내에 유지되며, 외부 모니터링 서비스로 전송되지 않음.',
          ],
        },
        networkIsolation: {
          ...
          items: [
            '**물리적:** 에어갭 인프라를 위한 별도의 네트워크 하드웨어(스위치, 라우터).',
            '**방화벽:** 인바운드 및 아웃바운드 트래픽 차단. 클라우드 서비스, NTP, 외부 서버의 DNS 예외 없음.',
            '**소프트웨어:** 호스트 레벨 방화벽(iptables, Windows Firewall)을 보조 통제수단으로 사용.',
            '**모니터링:** 네트워크 트래픽 감사. 외부 연결 시도 시 즉시 로그 기록 및 플래그 지정.',
          ],
        },
        modelManagement: {
          ...
          numberedItems: [
            '인터넷 연결 머신(별도의 비기밀 환경)에서 모델 다운로드.',
            '모델 무결성 검증(체크섬, 디지털 서명).',
            '암호화된 USB 드라이브 또는 내부 파일 서버를 통해 에어갭 시스템으로 이전.',
            '검증: 이전 중 모델 무결성이 훼손되지 않았는지 테스트 실행.',
            '배포: 추론 엔진에 모델 로드.',
            '문서화: 배포된 모델, 버전, 날짜 기록.',
          ],
        },
        updates: {
          ...
          items: [
            '**모델 업데이트:** 위의 프로세스를 따릅니다. 분기별 또는 연간으로 업데이트를 예약합니다.',
            '**OS 패치:** 먼저 격리된 스테이징 환경에서 테스트한 후, 프로덕션에 배포합니다.',
            '**의존성:** 새 버전을 신중하게 평가합니다. 에어갭 시스템은 더 오래된 버전을 장기 운영합니다.',
            '**자동 업데이트 없음:** 자동 업데이트를 완전히 비활성화합니다. 모든 업데이트는 통제되고, 문서화되고, 감사됩니다.',
          ],
        },
        security: {
          ...
          items: [
            '**내부자 위협:** 접근 권한이 있는 직원이 USB를 통해 데이터를 복사할 수 있습니다. 투 퍼슨 인테그리티(two-person integrity)를 요구합니다.',
            '**공급망 공격:** 이전 중 훼손된 모델 또는 의존성. 체크섬과 디지털 서명을 검증합니다.',
            '**물리적 도난:** 모델과 데이터는 수백만 달러의 가치가 있습니다. 잠금 서버실, 감시 카메라, 침입 탐지를 요구합니다.',
            '**사회공학:** "믿어주세요, 모델을 업데이트해야 합니다." 공식적인 변경 통제 프로세스를 요구합니다.',
            '**로깅 부재:** 로그를 감사하지 않으면 내부자 활동을 탐지할 수 없습니다. 정기적인 외부 로그 감사를 요구합니다.',
          ],
        },
        commonMistakes: {
          title: '에어갭 배포의 흔한 실수',
          items: [
            ...
            '**변경 통제 없음:** 모델 업데이트가 비공식적으로 이루어지고 문서화되지 않습니다. 어떤 버전이 배포되었는지 파악할 수 없게 됩니다.',
            '**불량한 백업 전략:** 에어갭 시스템은 이중화 스토리지와 오프사이트 백업이 필요합니다. 단, 백업 자체도 에어갭 상태를 유지해야 합니다.',
            '**불충분한 로깅:** 에어갭 시스템은 포괄적인 감사(누가, 무엇을, 언제 접근했는지)가 필요합니다. 로그 없이는 침해가 탐지되지 않습니다.',
            '**물리적 보안만 신뢰:** 보안은 여러 레이어가 필요합니다: 격리, 암호화, 접근 통제, 감사 — 잠긴 문만으로는 부족합니다.',
          ],
        },
        faqSection: {
          id: 'faq',
          title: '에어갭 시스템에 대한 일반적인 질문',
          faqs: [
            {
              q: '에어갭 시스템에서 모델을 어떻게 업데이트합니까?',
              a: '암호화된 USB를 통한 수동 방식 또는 인터넷으로부터 격리된 내부 보안 네트워크를 통해 업데이트합니다. 모든 업데이트는 변경 통제 승인과 감사 추적이 필요합니다.',
            },
            {
              q: '에어갭 시스템에 클라우드 백업을 사용할 수 있습니까?',
              a: '아닙니다. 클라우드 백업은 인터넷 연결이 필요하므로 에어갭 원칙에 위배됩니다. 별도의 보안 시설에 보관되는 물리적 백업(암호화 하드 드라이브)을 사용합니다.',
            },
            {
              q: '에어갭은 모든 공격에 대해 완전히 안전합니까?',
              a: '대부분 그렇지만, 내부자 위협은 여전히 존재합니다. 에어갭은 원격 공격에는 안전하지만, 물리적 접근이나 내부자 접근은 보안을 훼손할 수 있습니다.',
            },
            {
              q: '에어갭 배포는 얼마나 비용이 듭니까?',
              a: '하드웨어 비용은 $50,000~$500,000(일반 온프레미스와 큰 차이 없음). 운영 비용(보안, 감사, 교육)은 수동 프로세스로 인해 5~10배 더 높습니다.',
            },
            {
              q: '에어갭 환경에서 표준 도구(Ollama, vLLM)를 사용할 수 있습니까?',
              a: '예. 두 도구 모두 인터넷 없이 작동합니다. 한 번 배포하면 인터넷 연결이 필요 없습니다. 모든 의존성이 오프라인으로 설치되었는지 확인하십시오.',
            },
            {
        ...
        sources: {
          ...
          title: '출처',
          items: [
            'NIST 사이버보안 프레임워크 -- nist.gov/cyberframework',
            'DoD 데이터 관리 -- defense.gov/News/Releases/',
