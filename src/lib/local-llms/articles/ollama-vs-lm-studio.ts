      ...
      sections: {
        ...
        api: {
          ...
          content: [
            ...
            'Ollama 또는 LM Studio가 모델을 서비스하면, 다음 결정은 어떤 코딩 하네스가 이를 구동하는지입니다. 세 가지 오픈소스 선택지와 워크플로우 차이에 대해서는 [Continue.dev vs Cline vs Aider](/ko/power-local-llm/continue-dev-vs-cline-vs-aider-local)를 참조하십시오.',
          ],
        ...
        relatedReading: {
          ...
          title: '관련 읽기',
          items: [
            '[Ollama 설치 방법](/ko/local-llms/how-to-install-ollama) — macOS, Windows, Linux에서 Ollama 설정 단계별 가이드.',
            '[LM Studio 설치 방법](/ko/local-llms/how-to-install-lm-studio) — LM Studio 전체 설치 및 첫 번째 모델 설정.',
            '[최고의 로컬 LLM 프론트엔드](/ko/local-llms/best-local-llm-frontends) — Ollama용 Open WebUI, Enchanted UI 및 기타 채팅 인터페이스 가이드.',
            '[로컬 LLM OpenAI 호환 API](/ko/local-llms/local-llm-openai-compatible-api) — Python, Node.js, JavaScript 앱을 로컬 모델에 연결하는 방법.',
            '[VS Code 및 Cursor를 사용한 로컬 LLM](/ko/local-llms/local-llms-with-vscode-cursor) — 에디터 내 완성을 위해 코드 에디터와 Ollama 통합.',
            '[로컬 LLM을 위한 프롬프트 엔지니어링](/ko/local-llms/prompt-engineering-for-local-models) — 두 도구를 통해 실행되는 모델을 위한 프롬프트 최적화.',
            '[체인-오브-생각 프롬프팅](/ko/prompt-engineering/chain-of-thought-prompting) — Ollama와 LM Studio 모두에서 출력을 향상시키는 추론 기법.',
            '[제약된 프롬프팅](/ko/prompt-engineering/constrained-prompting) — API 또는 채팅 인터페이스를 통한 출력 형식 제어.',
            '[Ollama vs LM Studio (Português)](/pt/local-llms/ollama-vs-lm-studio) — versão em português deste guia',
          ],
        },
        sources: {
