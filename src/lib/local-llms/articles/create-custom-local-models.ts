...
export const article: Partial<Record<Language, LLMArticle>> = {
    en: {
      ...
      intro: 'Creating custom local LLMs means fine-tuning an existing model or pre-training from scratch. Fine-tuning with LoRA is practical on consumer hardware: 500 examples, 8 GB VRAM, 1–2 hours, $100–500. Pre-training costs $50K–500K and requires 10B+ tokens — justified only for rare proprietary needs. This guide covers both approaches: the 7-step fine-tuning path with Unsloth, the decision matrix for fine-tuning vs pre-training vs RAG, and deployment to [Ollama](https://ollama.com/).',
      ...
      leadAnswerBlock: 'Creating custom local LLMs means fine-tuning an existing model or pre-training from scratch. Fine-tuning with LoRA is practical on consumer hardware: 500 examples, 8 GB VRAM, 1–2 hours, $100–500. Pre-training costs $50K–500K and requires 10B+ tokens — justified only for rare proprietary needs. This guide covers both approaches: the 7-step fine-tuning path with Unsloth, the decision matrix for fine-tuning vs pre-training vs RAG, and deployment to Ollama.',
...
schema: {
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'Creating a custom local LLM means either fine-tuning an existing model with LoRA (500 examples, 8 GB VRAM, 1-2 hours, $100-500) or pre-training from scratch ($50K-500K, 10B+ tokens) -- fine-tuning is the practical choice for almost everyone, while pre-training is justified only for rare proprietary needs.' },
            { type: 'plain-terms', text: "Most people who want a custom AI model should fine-tune, not pre-train. Fine-tuning takes an existing model and nudges it toward your specific data or style using a few hundred examples, on a single consumer GPU, in an afternoon. Pre-training builds a model from nothing and costs as much as a house -- it only makes sense if you have a truly unique need that no existing model can be adapted to meet." },
          ],
          items: [
            ...
            'Pre-training is rarely justified unless you need proprietary model.',
          ],
        },
        ftVsPt: {
        ...
        relatedReading: {
          ...
          items: [
            ...
            '[Quantization for Local Models](/local-llms/llm-quantization-explained) -- Reduce model size without losing quality.',
            '[Local LLM Hardware Guide 2026](/local-llms/local-llm-hardware-guide-2026) -- GPU, Apple Silicon, and CPU requirements.',
            'How to Evaluate Local LLMs -- Benchmarking and testing frameworks.',
    ...
    es: {
      ...
      publishDate: '2026-04-04',
      dateModified: '2026-08-28',
      leadAnswerBlock: 'Crear LLMs locales personalizados significa hacer fine-tuning de un modelo existente o pre-entrenar desde cero. El fine-tuning con LoRA es práctico en hardware de consumo: 500 ejemplos, 8 GB VRAM, 1–2 horas, $100–500. El pre-training cuesta $50K–500K y requiere más de 10B tokens — justificado solo para necesidades propietarias excepcionales. Esta guía cubre ambos enfoques: el camino de fine-tuning en 7 pasos con Unsloth, la matriz de decisión entre fine-tuning vs pre-training vs RAG, y el despliegue en Ollama.',
...
schema: {
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'Crear un LLM local personalizado significa hacer fine-tuning de un modelo existente con LoRA (500 ejemplos, 8 GB VRAM, 1-2 horas, $100-500) o pre-entrenar desde cero ($50K-500K, más de 10B tokens) -- el fine-tuning es la opción práctica para casi todos, mientras que el pre-training solo se justifica para necesidades propietarias excepcionales.' },
            { type: 'plain-terms', text: 'La mayoría de las personas que quieren un modelo de IA personalizado deberían hacer fine-tuning, no pre-training. El fine-tuning toma un modelo existente y lo orienta hacia tus datos o estilo específicos usando unos pocos cientos de ejemplos, en una sola GPU de consumo, en una tarde. El pre-training construye un modelo desde cero y cuesta tanto como una casa -- solo tiene sentido si tienes una necesidad realmente única que ningún modelo existente puede cubrir mediante adaptación.' },
          ],
          items: [
            ...
            'El pre-training rara vez está justificado a menos que necesites un modelo propietario.',
          ],
        },
        ftVsPt: {
          id: 'finetuning-vs-pretraining',
        ...
        relatedReading: {
          ...
          items: [
            ...
            '[Cuantización para Modelos Locales](/es/local-llms/llm-quantization-explained) -- Reduce el tamaño del modelo sin perder calidad.',
            '[Guía de Hardware para LLM Local 2026](/es/local-llms/local-llm-hardware-guide-2026) -- Requisitos de GPU, Apple Silicon y CPU.',
            'Cómo Evaluar LLMs Locales -- Frameworks de benchmarking y pruebas.',
    ...
    ar: {
      ...
      intro: 'إنشاء نماذج LLM محلية مخصصة يعني الضبط الدقيق لنموذج موجود أو التدريب المسبق من الصفر. الضبط الدقيق بـ LoRA عملي على العتاد الاستهلاكي: 500 مثال، 8 GB VRAM، 1–2 ساعة، 100–500 دولار. يكلّف التدريب المسبق 50 ألف–500 ألف دولار ويتطلب أكثر من 10 مليارات رمز — مبرَّر فقط للاحتياجات المملوكة الاستثنائية. يغطي هذا الدليل النهجين: مسار الضبط الدقيق بـ 7 خطوات مع Unsloth، ومصفوفة القرار بين الضبط الدقيق والتدريب المسبق وRAG، والنشر على [Ollama](https://ollama.com/).',
...
schema: {
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'إنشاء نموذج LLM محلي مخصص يعني إما الضبط الدقيق لنموذج موجود باستخدام LoRA (500 مثال، 8 GB VRAM، 1-2 ساعة، 100-500 دولار) أو التدريب المسبق من الصفر (50 ألف-500 ألف دولار، أكثر من 10 مليارات رمز) -- الضبط الدقيق هو الخيار العملي لمعظم الناس، بينما يُبرَّر التدريب المسبق فقط للاحتياجات المملوكة الاستثنائية.' },
            { type: 'plain-terms', text: 'معظم من يريدون نموذج ذكاء اصطناعي مخصص يجب أن يختاروا الضبط الدقيق، لا التدريب المسبق. يأخذ الضبط الدقيق نموذجاً موجوداً ويوجّهه نحو بياناتك أو أسلوبك المحدد باستخدام بضع مئات من الأمثلة، على بطاقة GPU استهلاكية واحدة، في فترة بعد ظهر واحدة. أما التدريب المسبق فيبني نموذجاً من الصفر ويكلّف بقدر ثمن منزل -- ولا معنى له إلا إذا كانت لديك حاجة فريدة حقاً لا يمكن لأي نموذج موجود تلبيتها عبر التكييف.' },
          ],
          items: [
            ...
            'نادراً ما يكون التدريب المسبق مبرَّراً ما لم تحتج نموذجاً مملوكاً.',
          ],
        },
        ftVsPt: {
          id: 'finetuning-vs-pretraining',
        ...
        regionalContext: {
          ...
          items: [
            ...
            '**الولايات المتحدة:** لا تنظيم فيدرالي لـ LLM (اعتباراً من أغسطس 2026). تختلف القواعد على مستوى الولايات؛ تركّز قوانين كاليفورنيا على شفافية الخوارزميات. للنماذج المالية أو الصحية، قد تفرض الجهات التنظيمية (SEC، FDA، CMS) متطلبات توثيق. فكّر في تدقيق التغييرات على النموذج.',
          ],
        },
        relatedReading: {
          id: 'related-reading',
          ...
          items: [
            ...
            '[التكميم للنماذج المحلية](/ar/local-llms/llm-quantization-explained) -- قلّل حجم النموذج دون فقدان الجودة.',
            '[دليل عتاد LLM المحلي 2026](/ar/local-llms/local-llm-hardware-guide-2026) -- متطلبات GPU وApple Silicon وCPU.',
            'كيف تقيّم نماذج LLM المحلية -- أطر المعايرة والاختبار.',
    ...
    pt: {
      ...
      publishDate: '2026-04-04',
      leadAnswerBlock: 'Criar LLMs locais personalizados significa fazer fine-tuning de um modelo existente ou pré-treinar do zero. O fine-tuning com LoRA é prático em hardware de consumo: 500 exemplos, 8 GB VRAM, 1–2 horas, $100–500. O pré-treinamento custa $50K–500K e requer mais de 10B de tokens — justificado apenas para necessidades proprietárias excepcionais.',
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'Criar um LLM local personalizado significa fazer fine-tuning de um modelo existente com LoRA (500 exemplos, 8 GB VRAM, 1-2 horas, $100-500) ou pré-treinar do zero ($50K-500K, mais de 10B tokens) -- o fine-tuning é a escolha prática para quase todos, enquanto o pré-treinamento só se justifica para necessidades proprietárias excepcionais.' },
            { type: 'plain-terms', text: 'A maioria das pessoas que quer um modelo de IA personalizado deveria fazer fine-tuning, não pré-treinamento. O fine-tuning pega um modelo existente e o direciona para seus dados ou estilo específicos usando algumas centenas de exemplos, em uma única GPU de consumo, em uma tarde. O pré-treinamento constrói um modelo do zero e custa tanto quanto uma casa -- só faz sentido se você tem uma necessidade verdadeiramente única que nenhum modelo existente pode atender por adaptação.' },
          ],
          items: [
...
schema: {
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'Ein individuelles lokales LLM zu erstellen bedeutet entweder Fine-Tuning eines bestehenden Modells mit LoRA (500 Beispiele, 8 GB VRAM, 1-2 Stunden, 100-500 €) oder Pre-Training von Grund auf (50.000-500.000 €, 10 Mrd.+ Tokens) -- Fine-Tuning ist für fast jeden die praktische Wahl, während Pre-Training nur für seltene proprietäre Bedürfnisse gerechtfertigt ist.' },
            { type: 'plain-terms', text: 'Die meisten, die ein individuelles KI-Modell wollen, sollten Fine-Tuning statt Pre-Training wählen. Fine-Tuning nimmt ein bestehendes Modell und lenkt es mit ein paar hundert Beispielen auf einer einzelnen Consumer-GPU an einem Nachmittag in Richtung der eigenen Daten oder des eigenen Stils. Pre-Training baut ein Modell komplett neu auf und kostet so viel wie ein Haus -- das ergibt nur Sinn, wenn ein wirklich einzigartiger Bedarf besteht, den kein vorhandenes Modell durch Anpassung erfüllen kann.' },
          ],
          items: [
            ...
            'Pre-Training ist selten begründet, außer Sie benötigen proprietäres Modell.',
          ],
        },
        ftVsPt: {
          id: 'finetuning-vs-pretraining',
        ...
        relatedReading: {
          ...
          items: [
            ...
            '[Quantisierung für lokale Modelle](/de/local-llms/llm-quantization-explained) -- Modellgröße ohne Qualitätsverlust reduzieren.',
            '[Lokales LLM Hardware-Handbuch 2026](/de/local-llms/local-llm-hardware-guide-2026) -- GPU-, Apple-Silicon- und CPU-Anforderungen.',
            'Wie man lokale LLMs evaluiert -- Benchmarking- und Test-Frameworks.',
    ...
    fr: {
      ...
      publishDate: '2026-04-04',
      dateModified: '2026-08-28',
      leadAnswerBlock: 'Créer des LLMs locaux personnalisés signifie affiner un modèle existant ou pré-entraîner à partir de zéro. Le fine-tuning avec LoRA est pratique sur du matériel grand public : 500 exemples, 8 GB VRAM, 1–2 heures, 100–500 €.',
...
schema: {
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: "Créer un LLM local personnalisé signifie soit affiner un modèle existant avec LoRA (500 exemples, 8 Go de VRAM, 1-2 heures, 100-500 €) soit pré-entraîner à partir de zéro (50 000-500 000 €, plus de 10 milliards de tokens) -- le fine-tuning est le choix pratique pour presque tout le monde, tandis que le pré-entraînement ne se justifie que pour des besoins propriétaires rares." },
            { type: 'plain-terms', text: "La plupart des gens qui veulent un modèle d'IA personnalisé devraient faire du fine-tuning, pas du pré-entraînement. Le fine-tuning prend un modèle existant et l'oriente vers vos données ou votre style spécifique avec quelques centaines d'exemples, sur un seul GPU grand public, en un après-midi. Le pré-entraînement construit un modèle à partir de rien et coûte autant qu'une maison -- cela n'a de sens que si vous avez un besoin vraiment unique qu'aucun modèle existant ne peut satisfaire par adaptation." },
          ],
          items: [
        ...
        regionalContext: {
          ...
          items: [
            ...
            '**États-Unis :** Pas régulation LLM fédérale (août 2026). Règles niveau état varient ; lois Californie focus transparence algorithmique. Pour modèles finance/santé, organismes réglementaires (SEC, FDA, CMS) peuvent imposer exigences documentation. Considérez pistes audit pour changements modèle.',
          ],
        },
        relatedReading: {
          id: 'related-reading',
          ...
          items: [
            ...
            '[Quantification Modèles Locaux](/fr/local-llms/llm-quantization-explained) -- Réduisez taille modèle sans perdre qualité.',
            '[Guide Matériel LLM Local 2026](/fr/local-llms/local-llm-hardware-guide-2026) -- GPU, Apple Silicon, exigences CPU.',
            'Comment Évaluer LLMs Locaux -- Benchmarking frameworks test.',
    ...
    ja: {
      ...
      intro: 'カスタム ローカル LLM を構築するとは、既存モデルを fine-tune するか、ゼロから pre-train することを意味します。LoRA による fine-tuning はコンシューマー ハードウェアで実用的です：500 サンプル、8 GB VRAM、1～2 時間、¥15,000～30,000。Pre-training には ¥6,000,000～60,000,000 かかり、100 億+ トークンが必要です—稀な独自ニーズのみ正当化されます。このガイドは両方のアプローチをカバーしています：Unsloth を使用した 7 ステップ fine-tuning パス、fine-tuning vs. pre-training vs. RAG の決定マトリックス、および Ollama への デプロイメント。',
      metaDescription: 'Llama 3.1 8BをLoRAで微調整：500例、8GB VRAM、1–2時間、$100–500。事前訓練：10B+トークン、数週間、$50K–500K。判断マトリクスとOllamaデプロイ手順付き。',
      publishDate: '2026-04-04',
      dateModified: '2026-08-28',
      leadAnswerBlock: 'カスタム ローカル LLM を構築するとは、既存モデルを fine-tune するか、ゼロから pre-train することを意味します。LoRA による fine-tuning はコンシューマー ハードウェアで実用的です：500 サンプル、8 GB VRAM、1～2 時間、¥15,000～30,000。',
...
schema: {
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'カスタムローカルLLMを作成するには、LoRAで既存モデルをfine-tuningする（500サンプル、8GB VRAM、1〜2時間、$100〜500）か、ゼロからpre-trainingする（5万〜50万ドル、100億トークン以上）方法があります。ほぼ全員にとってfine-tuningが実用的な選択肢であり、pre-trainingは稀な独自ニーズにのみ正当化されます。' },
            { type: 'plain-terms', text: 'カスタムAIモデルを求めるほとんどの人は、pre-trainingではなくfine-tuningをすべきです。Fine-tuningは既存モデルを取り、数百のサンプルを使って、1台のコンシューマー向けGPUで午後のうちにあなた固有のデータやスタイルに近づけます。Pre-trainingはゼロからモデルを構築し、家一軒分の費用がかかります -- 既存のモデルを調整しても満たせない本当に独自のニーズがある場合にのみ意味があります。' },
          ],
          items: [
            ...
            '2026 年 8 月時点では、proprietary モデルが必要でない限り pre-training はほぼ正当化されません。',
          ],
        },
        ftVsPt: {
          id: 'finetuning-vs-pretraining',
        ...
        regionalContext: {
          ...
          items: [
            ...
            '**米国：** 連邦 LLM 規制なし（2026 年 8 月現在）。州レベルのルールは異なります；カリフォルニア法は アルゴリズム透明性に焦点。金融/医療モデルについては、規制機関（SEC、FDA、CMS）がドキュメンテーション要件を課す可能性。モデル変更の監査証跡を検討。',
          ],
        },
        relatedReading: {
          id: 'related-reading',
          ...
          items: [
            ...
            '[ローカルモデルの量子化](/ja/local-llms/llm-quantization-explained) -- 品質を失わないモデルサイズ削減。',
            '[ローカル LLM ハードウェアガイド 2026](/ja/local-llms/local-llm-hardware-guide-2026) -- GPU、Apple Silicon、CPU 要件。',
            'ローカル LLM を評価する方法 -- ベンチマークとテストフレームワーク。',
    ...
    zh: {
      ...
      intro: '创建自定义本地 LLM 意味着微调现有模型或从零开始预训练。使用 LoRA 的微调在消费级硬件上可行：500 个样本、8GB VRAM、1-2 小时、$100-500。预训练成本为 $50,000-500,000，需要 100 亿+ tokens——仅对罕见的专有需求合理。本指南涵盖两种方法：使用 Unsloth 的 7 步微调路径、微调 vs. 预训练 vs. RAG 决策矩阵以及 Ollama 部署。',
      metaDescription: '用LoRA微调Llama 3.1 8B：500个样本，8GB显存，1–2小时，$100–500。预训练：10B+令牌，数周，$50K–500K。含决策矩阵与Ollama部署指南。',
      publishDate: '2026-04-04',
      dateModified: '2026-08-28',
      leadAnswerBlock: '创建自定义本地 LLM 意味着微调现有模型或从零开始预训练。使用 LoRA 的微调在消费级硬件上可行：500 个样本、8GB VRAM、1-2 小时、$100-500。',
...
schema: {
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: '定制本地LLM意味着要么用LoRA对现有模型进行微调（500个样本、8GB显存、1-2小时、100-500美元），要么从零开始预训练（5万-50万美元、100亿以上tokens）——对几乎所有人来说，微调都是实用的选择，而预训练只在罕见的专有需求下才合理。' },
            { type: 'plain-terms', text: '大多数想要定制AI模型的人应该选择微调，而不是预训练。微调是拿一个现成模型，用几百个样本，在一块消费级GPU上，花一个下午的时间让它贴近你的特定数据或风格。预训练则是从零构建一个模型，成本堪比一套房子——只有当你有真正独特、任何现有模型都无法通过调整满足的需求时，才值得这样做。' },
          ],
          items: [
            ...
            '截至 2026 年 8 月，除非需要专有模型，否则预训练很少正当。',
          ],
        },
        ftVsPt: {
          id: 'finetuning-vs-pretraining',
        ...
        relatedReading: {
          ...
          items: [
            ...
            '[本地模型量化](/zh/local-llms/llm-quantization-explained) -- 在不损失质量的情况下减小模型大小。',
  ...
  ko: {
    ...
    heroImage: '/images/create-custom-local-models-overview-hero-ko.webp',
      ...
      intro: '커스텀 로컬 LLM을 만드는 방법은 기존 모델을 파인튜닝하거나 처음부터 사전학습하는 두 가지가 있습니다. LoRA를 활용한 파인튜닝은 소비자용 하드웨어에서도 실용적입니다: 예시 500개, VRAM 8 GB, 1~2시간, $100~500. 사전학습은 $5만~50만이 소요되며 100억 개 이상의 토큰이 필요합니다 — 특수한 독점 목적에만 정당화됩니다. 이 가이드에서는 두 가지 접근 방식을 모두 다룹니다: Unsloth를 활용한 7단계 파인튜닝 경로, 파인튜닝 vs 사전학습 vs RAG 의사결정 매트릭스, 그리고 [Ollama](https://ollama.com/) 배포 방법.',
      ...
      publishDate: '2026-04-04',
      dateModified: '2026-08-28',
      leadAnswerBlock: '커스텀 로컬 LLM을 만드는 방법은 기존 모델을 파인튜닝하거나 처음부터 사전학습하는 두 가지가 있습니다. LoRA를 활용한 파인튜닝은 소비자용 하드웨어에서도 실용적입니다: 예시 500개, VRAM 8 GB, 1~2시간, $100~500. 사전학습은 $5만~50만이 소요되며 100억 개 이상의 토큰이 필요합니다 — 특수한 독점 목적에만 정당화됩니다. 이 가이드에서는 Unsloth를 활용한 7단계 파인튜닝 경로, 파인튜닝 vs 사전학습 vs RAG 의사결정 매트릭스, 그리고 Ollama 배포 방법을 다룹니다.',
      ...
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: '커스텀 로컬 LLM을 만드는 방법은 LoRA로 기존 모델을 파인튜닝하거나(예시 500개, VRAM 8GB, 1~2시간, $100~500) 처음부터 사전학습하는(5만~50만 달러, 100억+ 토큰) 방법이 있습니다 -- 거의 모든 사람에게 파인튜닝이 실용적인 선택이며, 사전학습은 특수한 독점적 필요에만 정당화됩니다.' },
            { type: 'plain-terms', text: '커스텀 AI 모델을 원하는 대부분의 사람은 사전학습이 아니라 파인튜닝을 해야 합니다. 파인튜닝은 기존 모델을 가져와 몇백 개의 예시만으로 소비자용 GPU 한 대에서 오후 한나절 만에 여러분의 특정 데이터나 스타일에 맞게 조정합니다. 사전학습은 모델을 처음부터 만드는 것으로 집 한 채 값이 듭니다 -- 기존 모델을 조정해서는 충족할 수 없는 정말 독특한 필요가 있을 때만 의미가 있습니다.' },
          ],
          items: [
            ...
            '독점 모델이 필요한 경우가 아니면 사전학습은 거의 정당화되지 않습니다.',
          ],
        },
        ftVsPt: {
          id: 'finetuning-vs-pretraining',
        ...
        regionalContext: {
          ...
          items: [
            ...
            '**미국:** 연방 LLM 규정 없음 (2026년 8월 기준). 주 수준 규정은 다양하며, 캘리포니아 법률은 알고리즘 투명성에 초점을 맞춥니다. 금융/의료 모델의 경우 규제 기관(SEC, FDA, CMS)이 문서화 요구사항을 부과할 수 있습니다. 모델 변경에 대한 감사 추적을 고려하십시오.',
          ],
        },
        relatedReading: {
          id: 'related-reading',
          title: '관련 읽기',
          items: [
            '[LoRA로 로컬 LLM 파인튜닝](/ko/local-llms/fine-tuning-local-llms-lora) -- 효율적인 파인튜닝 단계별 가이드.',
            '[2026년 최고의 로컬 LLM](/ko/local-llms/best-local-llms-2026) -- 파인튜닝을 위한 권장 기본 모델.',
            '[로컬 모델 양자화](/ko/local-llms/llm-quantization-explained) -- 품질 손실 없이 모델 크기 줄이기.',
            '[로컬 LLM 하드웨어 가이드 2026](/ko/local-llms/local-llm-hardware-guide-2026) -- GPU, Apple Silicon 및 CPU 요구사항.',
            '로컬 LLM 평가 방법 -- 벤치마킹 및 테스트 프레임워크.',
