    ...
    en: {
      ...
      sections: {
        ...
        telemetry: {
          ...
          rows: [
            { 'Tool': 'Ollama', 'Telemetry Default': 'None collected', 'How to Confirm/Disable': 'Check github.com/ollama/ollama -- no analytics code' },
            { 'Tool': 'LM Studio', 'Telemetry Default': 'Anonymous analytics enabled', 'How to Confirm/Disable': 'Settings → Privacy → uncheck "Send anonymous usage data"' },
            { 'Tool': 'Jan AI', 'Telemetry Default': 'None -- explicitly disabled', 'How to Confirm/Disable': 'Open source -- audit github.com/janhq/jan' },
            { 'Tool': 'GPT4All', 'Telemetry Default': 'Opt-in only at first launch', 'How to Confirm/Disable': 'Settings → Privacy → disable usage analytics' },
          ],
  ...
  es: {
    ...
    sections: {
      ...
      telemetry: {
        ...
        rows: [
          { 'Herramienta': 'Ollama', 'Telemetría por defecto': 'Ninguna', 'Cómo confirmar/deshabilitar': 'Verifica github.com/ollama/ollama -- sin código de análisis' },
          { 'Herramienta': 'LM Studio', 'Telemetría por defecto': 'Análisis anónimos habilitados', 'Cómo confirmar/deshabilitar': 'Configuración → Privacidad → desmarca "Enviar datos de uso anónimos"' },
          { 'Herramienta': 'Jan AI', 'Telemetría por defecto': 'Ninguna -- explícitamente deshabilitada', 'Cómo confirmar/deshabilitar': 'Código abierto -- audita github.com/janhq/jan' },
          { 'Herramienta': 'GPT4All', 'Telemetría por defecto': 'Solo opt-in en el primer lanzamiento', 'Cómo confirmar/deshabilitar': 'Configuración → Privacidad → deshabilita análisis de uso' },
        ],
  ...
  ar: {
    ...
    sections: {
      ...
      telemetry: {
        ...
        rows: [
          { 'Herramienta': 'Ollama', 'Telemetría por defecto': 'لا شيء', 'Cómo confirmar/deshabilitar': 'تحقق من github.com/ollama/ollama — بلا شيفرة تحليلات' },
          { 'Herramienta': 'LM Studio', 'Telemetría por defecto': 'تحليلات مجهولة مُفعّلة', 'Cómo confirmar/deshabilitar': 'الإعدادات → الخصوصية → ألغِ تحديد "إرسال بيانات استخدام مجهولة"' },
          { 'Herramienta': 'Jan AI', 'Telemetría por defecto': 'لا شيء — مُعطّلة صراحةً', 'Cómo confirmar/deshabilitar': 'مفتوح المصدر — دقّق github.com/janhq/jan' },
          { 'Herramienta': 'GPT4All', 'Telemetría por defecto': 'اشتراك اختياري فقط عند الإطلاق الأول', 'Cómo confirmar/deshabilitar': 'الإعدادات → الخصوصية → عطّل تحليلات الاستخدام' },
        ],
        columns: ['Herramienta', 'Telemetría por defecto', 'Cómo confirmar/deshabilitar'],
  ...
  de: {
    ...
    sections: {
      ...
      telemetry: {
        ...
        rows: [
          { 'Tool': 'Ollama', 'Telemetrie Standard': 'Keine erfasst', 'Überprüfung/Deaktivieren': 'github.com/ollama/ollama -- kein Analytics-Code' },
          { 'Tool': 'LM Studio', 'Telemetrie Standard': 'Anonyme Analytics aktiviert', 'Überprüfung/Deaktivieren': 'Einstellungen → Datenschutz → „Anonyme Nutzungsdaten senden" deaktivieren' },
          { 'Tool': 'Jan AI', 'Telemetrie Standard': 'Keine -- explizit deaktiviert', 'Überprüfung/Deaktivieren': 'Open Source -- überprüfen Sie github.com/janhq/jan' },
          { 'Tool': 'GPT4All', 'Telemetrie Standard': 'Nur Opt-in beim ersten Start', 'Überprüfung/Deaktivieren': 'Einstellungen → Datenschutz → Nutzungs-Analytics deaktivieren' },
        ],
  ...
  fr: {
    ...
    sections: {
      ...
      telemetry: {
        ...
        rows: [
          { 'Outil': 'Ollama', 'Télémétrie Défaut': 'Aucune', 'Vérification/Désactiver': 'github.com/ollama/ollama -- pas de code analytics' },
          { 'Outil': 'LM Studio', 'Télémétrie Défaut': 'Analytics anon activée', 'Vérification/Désactiver': 'Paramètres → Confidentialité → décocher « Données d\'utilisation »' },
          { 'Outil': 'Jan AI', 'Télémétrie Défaut': 'Aucune -- explicitement désactivée', 'Vérification/Désactiver': 'Open source -- auditez github.com/janhq/jan' },
          { 'Outil': 'GPT4All', 'Télémétrie Défaut': 'Opt-in au premier lancement', 'Vérification/Désactiver': 'Paramètres → Confidentialité → désactiver analytics' },
        ],
  ...
  ja: {
    ...
    sections: {
      ...
      telemetry: {
        ...
        rows: [
          { 'ツール': 'Ollama', 'テレメトリデフォルト': 'なし', '確認/無効化方法': 'github.com/ollama/ollama -- アナリティクスコードなし' },
          { 'ツール': 'LM Studio', 'テレメトリデフォルト': '匿名アナリティクス有効', '確認/無効化方法': '設定 → プライバシー → 「匿名使用データを送信」のチェックを外す' },
          { 'ツール': 'Jan AI', 'テレメトリデフォルト': 'なし--明示的に無効', '確認/無効化方法': 'オープンソース -- github.com/janhq/janを監査' },
          { 'ツール': 'GPT4All', 'テレメトリデフォルト': '初回起動時のオプトインのみ', '確認/無効化方法': '設定 → プライバシー → 使用分析を無効化' },
        ],
  ...
  zh: {
    ...
    sections: {
      ...
      telemetry: {
        ...
        rows: [
          { '工具': 'Ollama', '遥测默认': '不收集', '验证/禁用方式': '检查github.com/ollama/ollama -- 没有分析代码' },
          { '工具': 'LM Studio', '遥测默认': '匿名分析已启用', '验证/禁用方式': '设置 → 隐私 → 取消勾选"发送匿名使用数据"' },
          { '工具': 'Jan AI', '遥测默认': '无 -- 明确禁用', '验证/禁用方式': '开源 -- 审计github.com/janhq/jan' },
          { '工具': 'GPT4All', '遥测默认': '首次启动时仅选择性加入', '验证/禁用方式': '设置 → 隐私 → 禁用使用分析' },
        ],
  ...
  ko: {
      ...
      sections: {
        ...
        telemetry: {
          ...
          rows: [
            { 'Tool': 'Ollama', 'Telemetry Default': '수집하지 않음', 'How to Confirm/Disable': 'github.com/ollama/ollama 확인 -- 분석 코드 없음' },
            { 'Tool': 'LM Studio', 'Telemetry Default': '익명 분석 활성화됨', 'How to Confirm/Disable': '설정 → 개인정보 → "익명 사용 데이터 전송" 체크 해제' },
            { 'Tool': 'Jan AI', 'Telemetry Default': '없음 -- 명시적으로 비활성화됨', 'How to Confirm/Disable': '오픈 소스 -- github.com/janhq/jan 감사' },
            { 'Tool': 'GPT4All', 'Telemetry Default': '첫 실행 시 옵트인만', 'How to Confirm/Disable': '설정 → 개인정보 → 사용 분석 비활성화' },
          ],
          columns: ['Tool', 'Telemetry Default', 'How to Confirm/Disable'],
