...
export const article: Partial<Record<Language, LLMArticle>> = {
    en: {
      ...
      metaDescription: 'Best sub-4B local LLM models for 4-8 GB RAM: Phi-4 Mini (68% MMLU), Gemma 4 E2B (60% MMLU Pro, 128K context), Qwen2.5 (coding), Llama 3.2 (general).',
    ...
    ar: {
      ...
      sections: {
        ...
        comparisonTable: {
          ...
          imageCaption: 'مستويات الأداء: تُظهر درجات MMLU وHumanEval أن Phi-4 Mini يتصدّر في الاستدلال والبرمجة، وGemma 2 الأسرع على CPU، وQwen2.5 يتفوق في البرمجة.',
          rows: [
...
schema: {
 ...
 items: ['**Bestes Reasoning im kleinen Format**: Phi-4 Mini 3.8B -- 68% MMLU, 70% HumanEval, läuft auf 4 GB RAM.', '**Bestes kleines/Edge-Modell**: Gemma 4 E2B -- MMLU Pro 60%, 128K Kontext, ~2 GB RAM, Apache 2.0. Ersetzt das ältere Gemma 2 2B.', '**Bestes kleines Codierungsmodell**: Qwen2.5 3B -- 65% HumanEval bei ~2 GB RAM.', '**Beste Universal-3B**: Llama 3.2 3B -- beste Community-Unterstützung, 128K Kontext, 2,5 GB RAM.', 'Kein Sub-2B-Modell produziert Ausgabequalität, die für professionelle Aufgaben geeignet ist. Verwenden Sie 3B+ für echte Arbeit.'] },
        ...
        llama32_1b: { id: 'llama-3-2-1b', title: 'Llama 3.2 1B -- Absolute Mindestanforderung für nützliche Ausgabe', content: 'Llama 3.2 1B benötigt nur 1,3 GB RAM und generiert 60-90 tok/sek auf CPU. Die Ausgabequalität ist marginal: es bewältigt sehr einfache Klassifizierung, kämpft aber mit kohärenten mehrsätzigen Antworten. Verwenden Sie Llama 3.2 1B nur, wenn RAM die bindende Einschränkung ist oder zum Testen von Tool-Integrationen.' },
...
schema: {
      ...
      sections: {
        ...
        llama32_1b: {
          ...
          content: 'Llama 3.2 1Bはわずか1.3 GBのRAMを必要とし、CPUで60～90トークン/秒を生成します。最も高速なローカル実行可能モデルです。出力品質は限界的です：非常にシンプルな分類とキーワード抽出を処理しますが、一貫した複数文の応答に苦労します。RAMが本当に制約（3 GB未満）である場合、またはツール統合をテストする場合のみ、Llama 3.2 1Bを使用してください。',
        },
        comparisonTable: {
          id: 'comparison-table',
...
schema: {
      ...
      sections: {
        ...
        llama32_1b: {
          ...
          content: 'Llama 3.2 1B仅需1.3 GB RAM，在CPU上生成60-90令牌/秒，是最快的本地运行模型。输出质量是边界的：它处理非常简单的分类和关键词提取，但在连贯的多句响应上苦恼。仅在RAM是真正约束（可用3 GB以下）或测试工具集成时使用Llama 3.2 1B。',
        },
        comparisonTable: {
          id: 'comparison-table',
  ...
  ko: {
      ...
      sections: {
        ...
        llama32_1b: {
          ...
          content: 'Llama 3.2 1B는 RAM 1.3 GB만 필요하며 CPU에서 초당 60~90 토큰을 생성합니다 -- 로컬에서 실행 가능한 가장 빠른 모델입니다. 출력 품질은 한계 수준입니다: 매우 간단한 분류와 키워드 추출은 처리하지만 일관성 있는 여러 문장 응답에는 어려움을 겪습니다. Llama 3.2 1B는 RAM이 실제로 제약 요인인 경우(가용 RAM 3 GB 미만)나 도구 통합 테스트 목적으로만 사용하십시오.',
        },
        comparisonTable: {
          id: 'comparison-table',
          title: '전체 비교: 4B 미만 최고 소형 로컬 LLM',
          image: '/images/small-local-llm-models-performance-tier-hero-ko.webp',
          imageCaption: '성능 등급: MMLU와 HumanEval 점수는 Phi-4 Mini가 추론과 코딩에서 선두임을 보여줍니다. Gemma 2는 CPU에서 가장 빠르며, Qwen2.5은 코딩에서 탁월합니다.',
          rows: [
