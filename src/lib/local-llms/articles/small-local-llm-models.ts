...
export const article: Partial<Record<Language, LLMArticle>> = {
    en: {
      ...
      intro: 'Small local LLMs (1B-4B parameters) run on machines with 4-8 GB RAM. The best small models in 2026 are Microsoft Phi-4 Mini 3.8B (best reasoning), Google Gemma 4 E2B (best small/edge, 128K context), Qwen2.5 3B (best coding), and Meta Llama 3.2 3B (best general use). Gemma 4 E2B (Apache 2.0) replaced the older Gemma 2 2B in August 2026 -- same ~2 GB footprint, but 128K context instead of 8K and a newer benchmark suite (MMLU Pro, LiveCodeBench v6, GPQA Diamond) instead of the original MMLU.',
      metaDescription: 'Best small & smallest local LLMs for 4-8 GB RAM in 2026: Phi-4 Mini 3.8B (68% MMLU), Gemma 4 E2B, Qwen2.5 3B (coding), Llama 3.2 3B/1B (smallest).',
      heroImage: '/images/small-local-llm-models-performance-tier-hero-en.webp',
      ...
      toc: [
        ...
        { label: '#2 Gemma 4 E2B -- Best Small/Edge Model', anchor: '#gemma-4-e2b' },
...
schema: {
      ...
      itemListSchema: {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Gemma 4 E2B', description: 'MMLU Pro 60%, LiveCodeBench v6 44%, GPQA Diamond 43.4%. ~2 GB RAM. 128K context. Apache 2.0. ollama pull gemma4:e2b' },
      ...
      faqSchema: {
        ...
        mainEntity: [
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Some do. Qwen2.5 3B supports function calling and JSON mode. Llama 3.2 3B has basic tool use support. Check each model\'s own documentation before building a pipeline that depends on structured output, since support varies by version.',
            },
          },
          {
            '@type': 'Question',
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Qwen2.5 3B supports 29 languages natively including Chinese, Japanese, Korean, and Arabic. Phi-4 Mini is primarily English-optimized. For non-English tasks at the small model scale, Qwen2.5 3B is the clear choice. See [Qwen vs Llama vs Mistral multilingual comparison](/local-llms/qwen-vs-llama-vs-mistral) for a full language comparison.',
            },
          },
          {
            '@type': 'Question',
          ...
          {
            '@type': 'Question',
            'name': 'What is the smallest local LLM that is actually usable?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Around 1B parameters is the floor for coherent output, and Llama 3.2 1B is the smallest model here worth running for real work. Below that, tiny models will load and generate on almost anything, but the text stops holding together across a few sentences, so they are better treated as autocomplete than as an assistant. If your constraint is disk and RAM rather than quality, the smallest usable local LLM with a small footprint is a sub-2B model at Q4; if you have 4GB or more to spend, a 4B model is a large step up in coherence for very little extra memory.',
            },
          },
          {
            '@type': 'Question',
            'name': 'What is the smallest LLM model you can run locally?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': "Llama 3.2 1B is the smallest Llama model and the smallest practical option in this comparison -- it needs only 1.3 GB of RAM and runs at 60-90 tok/sec on CPU, faster than any other model on this page. It handles short classification and keyword-extraction tasks but breaks down on longer, multi-step output. If you want a small model (sometimes called a 'mini LLM') that's actually usable day-to-day, treat Llama 3.2 1B as the floor for hardware, not for quality -- step up to Gemma 4 E2B or a 3B model like Qwen2.5 3B or Llama 3.2 3B for real work.",
            },
          },
        ],
      },
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'Small local LLMs (1B-4B parameters) run on 4-8 GB RAM machines at 30-70 tokens/sec, with Phi-4 Mini 3.8B leading reasoning, Gemma 4 E2B best for small/edge devices with 128K context, Qwen2.5 3B best for coding, and Llama 3.2 3B best for general use.' },
        ...
        gemma2: {
          id: 'gemma-4-e2b',
          title: 'Gemma 4 E2B -- Best Small/Edge Local LLM',
          content: [
          ...
          rows: [
            { 'Spec': 'MMLU Pro', 'Value': '60%' },
            { 'Spec': 'LiveCodeBench v6', 'Value': '44%' },
            { 'Spec': 'GPQA Diamond', 'Value': '43.4%' },
        ...
        llama32_1b: {
          ...
          content: 'Llama 3.2 1B is the smallest Llama model Meta publishes, requiring only 1.3 GB of RAM and generating 60-90 tok/sec on CPU -- the fastest locally-runnable model on this page. Output quality is marginal: it handles very simple classification and keyword extraction but struggles with coherent multi-sentence responses. Use Llama 3.2 1B only when RAM is genuinely the binding constraint (under 3 GB available) or for testing tool integrations.',
        },
        comparisonTable: {
          id: 'comparison-table',
          ...
          imageCaption: 'Performance tiers: Phi-4 Mini leads on reasoning and coding, Gemma 4 E2B leads on context length (128K) at the smallest RAM footprint, Qwen2.5 excels at coding.',
          rows: [
            ...
            { 'Model': 'Gemma 4 E2B', 'MMLU': '—', 'HumanEval': '—', 'RAM': '2 GB', 'Context': '128K', 'Best For': 'Edge/SBC (MMLU Pro 60%)' },
        ...
        faqSection: {
          ...
          faqs: [
            ...
            { q: 'Do small models support function calling?', a: 'Some do. Qwen2.5 3B supports function calling and JSON mode. Llama 3.2 3B has basic tool use support. Check each model\'s own documentation before building a pipeline that depends on structured output, since support varies by version.' },
            { q: 'Which small model is best for languages other than English?', a: 'Qwen2.5 3B supports 29 languages natively including Chinese, Japanese, Korean, and Arabic. Phi-4 Mini is primarily English-optimized. For non-English tasks at the small model scale, Qwen2.5 3B is the clear choice. See [Qwen vs Llama vs Mistral multilingual comparison](/local-llms/qwen-vs-llama-vs-mistral) for a full language comparison.' },
            ...
            { q: 'What is the smallest LLM model you can run locally?', a: "Llama 3.2 1B is the smallest Llama model and the smallest practical option in this comparison -- it needs only 1.3 GB of RAM and runs at 60-90 tok/sec on CPU, faster than any other model on this page. It handles short classification and keyword-extraction tasks but breaks down on longer, multi-step output. If you want a small model (sometimes called a 'mini LLM') that's actually usable day-to-day, treat Llama 3.2 1B as the floor for hardware, not for quality -- step up to Gemma 4 E2B or a 3B model like Qwen2.5 3B or Llama 3.2 3B for real work." },
          ],
        },
        sources: {
          id: 'sources',
    ...
    es: {
      ...
      intro: 'Los LLMs locales pequeños (1B-4B parámetros) funcionan en máquinas con 4-8 GB de RAM y producen 30-70 tokens/seg en CPU -- suficientemente rápido para chat en tiempo real. Los mejores modelos pequeños en 2026 son Microsoft Phi-4 Mini 3.8B (mejor razonamiento), Google Gemma 4 E2B (el más rápido), Qwen2.5 3B (mejor para código) y Meta Llama 3.2 3B (mejor uso general).',
      metaDescription: 'Mejores modelos LLM locales sub-4B para 4-8 GB de RAM: Phi-4 Mini (68% MMLU), Gemma 4 E2B (60% MMLU Pro, 128K contexto), Qwen2.5 (código), Llama 3.2 (general).',
      heroImage: '/images/small-local-llm-models-performance-tier-hero-es.webp',
      publishDate: '2026-04-04',
      ...
      toc: [
        ...
        { label: '#2 Gemma 4 E2B -- El más rápido en CPU', anchor: '#gemma-4-e2b' },
        { label: '#3 Qwen2.5 3B -- Mejor para código', anchor: '#qwen2-5-3b' },
      ...
      itemListSchema: {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Gemma 4 E2B', description: 'MMLU Pro 60%, LiveCodeBench v6 44%, GPQA Diamond 43,4%. ~2 GB RAM. Contexto 128K. Apache 2.0. ollama pull gemma4:e2b' },
          { '@type': 'ListItem', position: 3, name: 'Qwen2.5 3B', description: 'MMLU 62%, HumanEval 65%. 2 GB de RAM. 29 idiomas de forma nativa. Compatible con llamadas a funciones. ollama run qwen2.5:3b' },
      ...
      faqSchema: {
        ...
        mainEntity: [
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Algunos sí. Qwen2.5 3B admite llamadas a funciones y modo JSON. Llama 3.2 3B tiene soporte básico de uso de herramientas. Gemma 4 E2B no admite llamadas a funciones. Consulta la documentación del modelo antes de crear un pipeline que dependa de salida estructurada.',
            },
          },
          {
            '@type': 'Question',
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Qwen2.5 3B admite 29 idiomas de forma nativa, incluyendo chino, japonés, coreano y árabe. Gemma 4 E2B y Phi-4 Mini están optimizados principalmente para inglés. Para tareas en otros idiomas a escala de modelo pequeño, Qwen2.5 3B es la elección clara. Consulta la [comparación multilingüe Qwen vs Llama vs Mistral](/es/local-llms/qwen-vs-llama-vs-mistral) para una comparación completa de idiomas.',
            },
          },
          {
            '@type': 'Question',
          ...
          {
            '@type': 'Question',
            'name': '¿Cuál es el LLM local más pequeño que realmente es usable?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Alrededor de 1B de parámetros es el límite inferior para obtener resultados coherentes, y Llama 3.2 1B es el modelo más pequeño de esta lista que vale la pena ejecutar para trabajo real. Por debajo de eso, los modelos diminutos cargan y generan texto con casi cualquier hardware, pero el texto deja de mantener coherencia después de unas pocas frases, así que es mejor tratarlos como autocompletado que como un asistente. Si tu limitación es el disco y la RAM más que la calidad, el LLM local usable más pequeño con una huella reducida es un modelo sub-2B en Q4; si tienes 4GB o más disponibles, un modelo 4B supone un salto grande en coherencia por muy poca RAM adicional.',
            },
          },
          {
            '@type': 'Question',
            'name': '¿Cuál es el LLM más pequeño que puedes ejecutar localmente?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': "Llama 3.2 1B es el modelo Llama más pequeño y la opción más práctica de esta comparación: necesita solo 1,3 GB de RAM y funciona a 60-90 tok/seg en CPU, más rápido que cualquier otro modelo de esta página. Maneja tareas breves de clasificación y extracción de palabras clave, pero se rompe con salidas más largas o de varios pasos. Si buscas un modelo pequeño (a veces llamado 'mini LLM') que sea realmente usable en el día a día, trata Llama 3.2 1B como el límite de hardware, no de calidad -- pasa a Gemma 4 E2B o a un modelo 3B como Qwen2.5 3B o Llama 3.2 3B para trabajo real.",
            },
          },
        ],
      },
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'Los LLM locales pequeños (1B-4B de parámetros) funcionan en equipos con 4-8 GB de RAM a 30-70 tokens/seg, con Phi-4 Mini 3.8B a la cabeza en razonamiento, Gemma 4 E2B ideal para dispositivos pequeños/edge con contexto de 128K, Qwen2.5 3B mejor para programación y Llama 3.2 3B mejor para uso general.' },
          ...
          items: [
            ...
            'Ningún modelo sub-2B produce calidad de salida adecuada para tareas profesionales. Usa 3B o más para trabajo real.',
          ],
        },
        whatIsSmall: {
          id: 'what-is-small-llm',
        ...
        phi4mini: {
          ...
          content: [
            ...
            'Phi-4 Mini es la opción recomendada para usuarios que principalmente necesitan razonamiento (matemáticas, lógica, explicaciones paso a paso) o asistencia con código en hardware con 4-6 GB de RAM.',
          ],
          rows: [
        ...
        gemma2: {
          ...
          content: [
            ...
            'La ventana de contexto de 128K es una mejora importante frente a los 8K del antiguo Gemma 2 2B -- ahora iguala a Phi-4 Mini y Llama 3.2 3B. La velocidad en CPU varía según el hardware y la cuantización (Google reporta ~7,6 tok/seg en un Raspberry Pi 5; informes independientes en CPU x86 van de 8 a 48 tok/seg) -- consulta cifras actuales para tu dispositivo. Licencia Apache 2.0.',
          ],
          rows: [
            { 'Especificación': 'MMLU Pro', 'Valor': '60%' },
            { 'Especificación': 'LiveCodeBench v6', 'Valor': '44%' },
            { 'Especificación': 'GPQA Diamond', 'Valor': '43,4%' },
        ...
        qwen25_3b: {
          id: 'qwen2-5-3b',
          title: 'Qwen2.5 3B -- Mejor modelo pequeño para tareas de código',
          content: [
        ...
        llama32_1b: {
          ...
          content: 'Llama 3.2 1B es el modelo Llama más pequeño que publica Meta: requiere solo 1,3 GB de RAM y genera 60-90 tok/seg en CPU -- el modelo ejecutable localmente más rápido de esta página. La calidad de salida es marginal: maneja clasificaciones muy simples y extracción de palabras clave, pero tiene dificultades con respuestas coherentes de varias frases. Usa Llama 3.2 1B solo cuando la RAM es genuinamente la restricción principal (menos de 3 GB disponibles) o para probar integraciones de herramientas.',
        },
        comparisonTable: {
          id: 'comparison-table',
          ...
          imageCaption: 'Niveles de rendimiento: Phi-4 Mini lidera en razonamiento y código, Gemma 4 E2B lidera en longitud de contexto (128K) con la menor huella de RAM, Qwen2.5 destaca en código.',
          rows: [
            { 'Modelo': 'Phi-4 Mini 3.8B', 'MMLU': '68%', 'HumanEval': '70%', 'RAM': '2,5 GB', 'Contexto': '128K', 'Mejor para': 'Razonamiento, código' },
            { 'Modelo': 'Qwen2.5 3B', 'MMLU': '62%', 'HumanEval': '65%', 'RAM': '2 GB', 'Contexto': '128K', 'Mejor para': 'Código, multilingüe' },
            { 'Modelo': 'Llama 3.2 3B', 'MMLU': '58%', 'HumanEval': '60%', 'RAM': '2,5 GB', 'Contexto': '128K', 'Mejor para': 'Uso general, primer modelo' },
            { 'Modelo': 'Gemma 4 E2B', 'MMLU': '—', 'HumanEval': '—', 'RAM': '2 GB', 'Contexto': '128K', 'Mejor para': 'Edge/borde (MMLU Pro 60%)' },
        ...
        relatedReading: {
          ...
          items: [
            ...
            '[Mejores LLMs locales para código](/es/local-llms/best-local-llms-for-coding) -- modelos específicos de código a escala 7B+ cuando Qwen2.5 3B no es suficiente para tareas complejas',
        ...
        faqSection: {
          ...
          faqs: [
            { q: '¿Cuál es el LLM local más pequeño que produce resultados útiles?', a: 'El mínimo práctico para resultados útiles es un modelo 3B con cuantización Q4_K_M. Los modelos por debajo de 2B parámetros (Llama 3.2 1B, Gemma 4 E2B) producen frases coherentes pero tienen dificultades con instrucciones de varios pasos, respuestas más largas y razonamiento complejo. Para tareas como resumen y preguntas y respuestas simples, Gemma 4 E2B es usable. Para cualquier cosa más compleja, comienza con un modelo 3B.' },
            ...
            { q: '¿Los modelos pequeños admiten llamadas a funciones?', a: 'Algunos sí. Qwen2.5 3B admite llamadas a funciones y modo JSON. Llama 3.2 3B tiene soporte básico de uso de herramientas. Gemma 4 E2B no admite llamadas a funciones. Consulta la documentación del modelo antes de crear un pipeline que dependa de salida estructurada.' },
            { q: '¿Qué modelo pequeño es mejor para idiomas distintos del inglés?', a: 'Qwen2.5 3B admite 29 idiomas de forma nativa, incluyendo chino, japonés, coreano y árabe. Gemma 4 E2B y Phi-4 Mini están optimizados principalmente para inglés. Para tareas en otros idiomas a escala de modelo pequeño, Qwen2.5 3B es la elección clara. Consulta la [comparación multilingüe Qwen vs Llama vs Mistral](/es/local-llms/qwen-vs-llama-vs-mistral) para una comparación completa de idiomas.' },
            ...
            { q: '¿Cuál es el LLM local más pequeño que realmente es usable?', a: 'Alrededor de 1B de parámetros es el límite inferior para obtener resultados coherentes, y Llama 3.2 1B es el modelo más pequeño de esta lista que vale la pena ejecutar para trabajo real. Por debajo de eso, los modelos diminutos cargan y generan texto con casi cualquier hardware, pero el texto deja de mantener coherencia después de unas pocas frases, así que es mejor tratarlos como autocompletado que como un asistente. Si tu limitación es el disco y la RAM más que la calidad, el LLM local usable más pequeño con una huella reducida es un modelo sub-2B en Q4; si tienes 4GB o más disponibles, un modelo 4B supone un salto grande en coherencia por muy poca RAM adicional.' },
            { q: '¿Cuál es el LLM más pequeño que puedes ejecutar localmente?', a: "Llama 3.2 1B es el modelo Llama más pequeño y la opción más práctica de esta comparación: necesita solo 1,3 GB de RAM y funciona a 60-90 tok/seg en CPU, más rápido que cualquier otro modelo de esta página. Maneja tareas breves de clasificación y extracción de palabras clave, pero se rompe con salidas más largas o de varios pasos. Si buscas un modelo pequeño (a veces llamado 'mini LLM') que sea realmente usable en el día a día, trata Llama 3.2 1B como el límite de hardware, no de calidad -- pasa a Gemma 4 E2B o a un modelo 3B como Qwen2.5 3B o Llama 3.2 3B para trabajo real." },
          ],
        },
        sources: {
          id: 'sources',
    ...
    ar: {
      freshness_tier: 'semi_annual',
      theme: 'Best Models',
      title: 'نماذج ⁨LLM⁩ المحلية الصغيرة: أفضل نماذج أقل من ⁨4B⁩ للأجهزة منخفضة الذاكرة في ⁨2026⁩',
      ...
      metaDescription: 'أفضل ⁨5⁩ نماذج ⁨LLM⁩ محلية أقل من ⁨4B⁩ في ⁨2026⁩: ⁨Phi-4 Mini⁩ (⁨68% MMLU⁩، ⁨2.5 GB⁩)، ⁨Gemma 4 E2B⁩ (⁨60% MMLU Pro⁩، سياق ⁨128K⁩)، ⁨Qwen2.5⁩ (البرمجة)، ⁨Llama 3.2⁩ (عام).',
      heroImage: '/images/small-local-llm-models-performance-tier-hero-ar.webp',
      publishDate: '2026-04-04',
      ...
      educationalLevel: 'Beginner',
      primaryTerm: 'نموذج LLM محلي صغير',
      toc: [
        { label: 'النقاط الرئيسية', anchor: '#key-takeaways' },
        { label: 'ما هو نموذج LLM المحلي "الصغير"؟', anchor: '#what-is-small-llm' },
        { label: 'أي نموذج يجب أن تستخدم؟', anchor: '#model-selection-guide' },
        { label: '#1 Phi-4 Mini 3.8B -- أفضل استدلال', anchor: '#phi-4-mini' },
        { label: '#2 Gemma 4 E2B -- الأسرع على CPU', anchor: '#gemma-4-e2b' },
        { label: '#3 Qwen2.5 3B -- الأفضل للبرمجة', anchor: '#qwen2-5-3b' },
        { label: '#4 Llama 3.2 3B -- الأفضل للاستخدام العام', anchor: '#llama-3-2-3b' },
        { label: '#5 Llama 3.2 1B -- الحد الأدنى المطلق', anchor: '#llama-3-2-1b' },
        { label: 'جدول مقارنة كامل', anchor: '#comparison-table' },
        { label: 'دليل التكميم', anchor: '#quantization-guide' },
      ...
      schema: {
        ...
        proficiencyLevel: 'Beginner',
        about: [
          { '@type': 'Thing', name: 'نموذج LLM محلي صغير' },
          { '@type': 'Thing', name: 'Phi-4 Mini' },
          { '@type': 'Thing', name: 'Gemma 4 E2B' },
          { '@type': 'Thing', name: 'Llama 3.2 3B' },
          { '@type': 'Thing', name: 'الاستدلال بذاكرة منخفضة' },
        ],
        speakable: { '@type': 'SpeakableSpecification', cssSelector: ['.article-intro', '.key-takeaways'] },
      },
      itemListSchema: {
        '@context': 'https://schema.org',
        '@type': 'ItemList',
        name: 'أفضل نماذج LLM المحلية الصغيرة أقل من 4B معاملات 2026',
        numberOfItems: 5,
        itemListElement: [
          { '@type': 'ListItem', position: 1, name: 'Phi-4 Mini 3.8B', description: 'MMLU 68%، HumanEval 70%. 2.5 GB من RAM. 30-50 tok/ثانية على CPU. سياق 128K. أفضل استدلال. ollama run phi4-mini' },
          { '@type': 'ListItem', position: 2, name: 'Gemma 4 E2B', description: 'MMLU Pro 60%، LiveCodeBench v6 44%، GPQA Diamond 43.4%. ~2 GB RAM. سياق 128K. Apache 2.0. ollama pull gemma4:e2b' },
          { '@type': 'ListItem', position: 3, name: 'Qwen2.5 3B', description: 'MMLU 62%، HumanEval 65%. 2 GB من RAM. 29 لغة بشكل أصلي. يدعم استدعاء الدوال. ollama run qwen2.5:3b' },
          { '@type': 'ListItem', position: 4, name: 'Llama 3.2 3B', description: 'MMLU 58%، HumanEval 60%. 2.5 GB من RAM. سياق 128K. أكبر دعم مجتمعي. ollama run llama3.2:3b' },
          { '@type': 'ListItem', position: 5, name: 'Llama 3.2 1B', description: 'MMLU 32%. 1.3 GB من RAM. 60-90 tok/ثانية على CPU. أدنى متطلب RAM مطلق. ollama run llama3.2:1b' },
        ],
      },
      faqSchema: {
        '@context': 'https://schema.org',
        ...
        mainEntity: [
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'بعضها نعم. يدعم Qwen2.5 3B استدعاء الدوال ووضع JSON. يملك Llama 3.2 3B دعمًا أساسيًا لاستخدام الأدوات. لا يدعم Gemma 4 E2B استدعاء الدوال. راجع وثائق النموذج قبل بناء مسار يعتمد على مخرجات منظّمة.',
            },
          },
          {
            '@type': 'Question',
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'يدعم Qwen2.5 3B 29 لغة بشكل أصلي، بما في ذلك الصينية واليابانية والكورية والعربية. Gemma 4 E2B وPhi-4 Mini محسّنان أساسًا للإنجليزية. لمهام اللغات الأخرى على نطاق النموذج الصغير، Qwen2.5 3B هو الخيار الواضح. راجع [مقارنة Qwen vs Llama vs Mistral متعددة اللغات](/ar/local-llms/qwen-vs-llama-vs-mistral) لمقارنة كاملة للغات.',
            },
          },
          {
            '@type': 'Question',
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'نعم. يحصل Phi-4 Mini على 70% في HumanEval مقابل 60% لـ Llama 3.2 3B -- فجوة كبيرة قدرها 10 نقاط على هذا النطاق. للمساعدة في البرمجة على أجهزة بذاكرة 4-6 GB، Phi-4 Mini هو الخيار الموصى به. للبرمجة بلغات متعددة (غير Python)، Qwen2.5 3B بنسبة 65% في HumanEval منافس لـ Phi-4 Mini ويدعم أيضًا استدعاء الدوال.',
            },
          },
          {
            '@type': 'Question',
          ...
          {
            '@type': 'Question',
            'name': 'ما أصغر نموذج LLM محلي قابل للاستخدام فعليًا؟',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'الحد الأدنى لإنتاج نص متماسك هو نحو 1B من المعاملات، وLlama 3.2 1B هو أصغر نموذج في هذه القائمة يستحق التشغيل لعمل حقيقي. أما النماذج الأصغر من ذلك فتُحمَّل وتولّد نصًا على أي عتاد تقريبًا، لكن النص يفقد تماسكه بعد بضع جمل، لذا يُفضَّل التعامل معها كإكمال تلقائي لا كمساعد. إذا كان القيد هو مساحة التخزين وRAM لا الجودة، فأصغر نموذج LLM محلي قابل للاستخدام بحجم صغير هو نموذج أقل من 2B بتكميم Q4؛ وإذا توفّر لديك 4GB أو أكثر، فإن نموذج 4B يمثل قفزة كبيرة في التماسك مقابل زيادة طفيفة جدًا في RAM.',
            },
          },
          {
            '@type': 'Question',
            'name': 'ما أصغر نموذج LLM يمكنك تشغيله محليًا؟',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Llama 3.2 1B هو أصغر نموذج Llama والخيار الأكثر عملية في هذه المقارنة -- يحتاج فقط إلى 1.3 GB من RAM ويعمل بسرعة 60-90 tok/ثانية على CPU، وهو أسرع نموذج في هذه الصفحة. يتعامل مع مهام تصنيف قصيرة واستخراج كلمات مفتاحية، لكنه يتعثر مع مخرجات أطول ومتعددة الخطوات. إذا كنت تريد نموذجًا صغيرًا (يُعرف أحيانًا بـ mini LLM) قابلًا للاستخدام فعليًا يوميًا، فاعتبر Llama 3.2 1B الحد الأدنى للعتاد لا للجودة -- انتقل إلى Gemma 4 E2B أو نموذج 3B مثل Qwen2.5 3B أو Llama 3.2 3B للعمل الحقيقي.',
            },
          },
        ],
      },
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: 'تعمل نماذج LLM المحلية الصغيرة (1B-4B معلمة) على أجهزة بذاكرة 4-8 GB بسرعة 30-70 token/ثانية، مع تصدّر Phi-4 Mini 3.8B للاستدلال، وGemma 4 E2B الأفضل للأجهزة الصغيرة/الطرفية بسياق 128K، وQwen2.5 3B الأفضل للبرمجة، وLlama 3.2 3B الأفضل للاستخدام العام.' },
        ...
        whatIsSmall: {
          ...
          content: [
            ...
            'فجوة الجودة بين نموذج 3B ونموذج 7B كبيرة -- تعادل تقريبًا الفجوة بين GPT-4o mini وGPT-5.5. للمستخدمين بذاكرة 8 GB، يكاد يكون نموذج 7B بـ Q4_K_M الخيار الأفضل دائمًا إذا توفّر للجهاز هامش. راجع [أفضل نماذج LLM المحلية للمبتدئين](/ar/local-llms/best-beginner-local-llm-models) للتوصيات بشأن 7B.',
          ],
        },
        modelSelectionGuide: {
          id: 'model-selection-guide',
        ...
        phi4mini: {
          ...
          rows: [
            { 'المواصفة': 'MMLU', 'القيمة': '68%' },
            { 'المواصفة': 'HumanEval', 'القيمة': '70%' },
            { 'المواصفة': 'RAM (Q4_K_M)', 'القيمة': '~2.5 GB' },
            { 'المواصفة': 'السياق', 'القيمة': '128K token' },
            { 'المواصفة': 'السرعة على CPU', 'القيمة': '30-50 tok/ثانية' },
        ...
        gemma2: {
          ...
          rows: [
            { 'المواصفة': 'MMLU Pro', 'القيمة': '60%' },
            { 'المواصفة': 'LiveCodeBench v6', 'القيمة': '44%' },
            { 'المواصفة': 'GPQA Diamond', 'القيمة': '43.4%' },
            { 'المواصفة': 'RAM (Q4_K_M)', 'القيمة': '~2 GB' },
            { 'المواصفة': 'السياق', 'القيمة': '128K token' },
        ...
        qwen25_3b: {
          id: 'qwen2-5-3b',
          title: 'Qwen2.5 3B -- أفضل نموذج صغير لمهام البرمجة',
          content: [
          ...
          rows: [
            { 'المواصفة': 'MMLU', 'القيمة': '62%' },
            { 'المواصفة': 'HumanEval', 'القيمة': '65%' },
            { 'المواصفة': 'RAM (Q4_K_M)', 'القيمة': '~2 GB' },
            { 'المواصفة': 'السياق', 'القيمة': '128K token' },
            { 'المواصفة': 'السرعة على CPU', 'القيمة': '25-40 tok/ثانية' },
            { 'المواصفة': 'أمر Ollama', 'القيمة': 'ollama run qwen2.5:3b' },
          ],
          columns: ['المواصفة', 'القيمة'],
        },
        llama32_3b: {
          ...
          rows: [
            { 'المواصفة': 'MMLU', 'القيمة': '58%' },
            { 'المواصفة': 'RAM (Q4_K_M)', 'القيمة': '~2.5 GB' },
            { 'المواصفة': 'السياق', 'القيمة': '128K token' },
            { 'المواصفة': 'السرعة على CPU', 'القيمة': '25-45 tok/ثانية' },
        ...
        comparisonTable: {
          ...
          imageCaption: 'مستويات الأداء: يتصدّر Phi-4 Mini الاستدلال والبرمجة، ويتصدّر Gemma 4 E2B طول السياق (128K) بأصغر بصمة RAM، ويتفوق Qwen2.5 في البرمجة.',
          rows: [
            { 'النموذج': 'Phi-4 Mini 3.8B', 'MMLU': '68%', 'HumanEval': '70%', 'RAM': '2.5 GB', 'السياق': '128K', 'الأفضل لـ': 'الاستدلال، البرمجة' },
            { 'النموذج': 'Qwen2.5 3B', 'MMLU': '62%', 'HumanEval': '65%', 'RAM': '2 GB', 'السياق': '128K', 'الأفضل لـ': 'البرمجة، متعدد اللغات' },
            { 'النموذج': 'Llama 3.2 3B', 'MMLU': '58%', 'HumanEval': '60%', 'RAM': '2.5 GB', 'السياق': '128K', 'الأفضل لـ': 'استخدام عام، أول نموذج' },
            { 'النموذج': 'Gemma 4 E2B', 'MMLU': '—', 'HumanEval': '—', 'RAM': '2 GB', 'السياق': '128K', 'الأفضل لـ': 'الأجهزة الطرفية (MMLU Pro 60%)' },
            { 'النموذج': 'Llama 3.2 1B', 'MMLU': '32%', 'HumanEval': '28%', 'RAM': '1.3 GB', 'السياق': '128K', 'الأفضل لـ': 'أدنى RAM مطلق' },
          ],
        ...
        relatedReading: {
          ...
          items: [
            ...
            '[أفضل نماذج LLM المحلية للمبتدئين](/ar/local-llms/best-beginner-local-llm-models) -- توصيات 7B عند توفّر 8 GB من RAM وحاجة الجودة لتجاوز نماذج 3B',
            '[شغّل أول نموذج LLM محلي](/ar/local-llms/run-first-local-llm) -- دليل خطوة بخطوة لتنزيل وتشغيل أي نموذج في هذه الصفحة باستخدام Ollama في أقل من 10 دقائق',
            '[LLM محلي على حاسوب محمول](/ar/local-llms/local-llm-on-laptop) -- الإدارة الحرارية وإدارة البطارية للاستدلال المستدام على عتاد محدود',
            '[شرح تكميم LLM](/ar/local-llms/llm-quantization-explained) -- لماذا Q4_K_M هو الافتراضي ومتى تفكر في Q3_K_M لقيود RAM القصوى',
            '[أفضل نماذج LLM المحلية للبرمجة](/ar/local-llms/best-local-llms-for-coding) -- نماذج خاصة بالبرمجة على نطاق 7B+ عندما لا يكفي Qwen2.5 3B للمهام المعقدة',
            '[استكشاف أخطاء إعداد LLM المحلي وإصلاحها](/ar/local-llms/troubleshooting-local-llm-setup) -- صحّح أخطاء OOM، والاستدلال البطيء، وأعطال تحميل النماذج الشائعة على الأجهزة منخفضة الذاكرة',
        ...
        faqSection: {
          ...
          faqs: [
            { q: 'ما أصغر نموذج LLM محلي ينتج نتائج مفيدة؟', a: 'الحد الأدنى العملي لنتائج مفيدة هو نموذج 3B بتكميم Q4_K_M. النماذج أقل من 2B معاملات (Llama 3.2 1B، Gemma 4 E2B) تنتج جملًا متماسكة لكنها تواجه صعوبة مع التعليمات متعددة الخطوات، والاستجابات الأطول، والاستدلال المعقد. لمهام مثل التلخيص والأسئلة والأجوبة البسيطة، Gemma 4 E2B قابل للاستخدام. لأي شيء أكثر تعقيدًا، ابدأ بنموذج 3B.' },
            ...
            { q: 'هل تدعم النماذج الصغيرة استدعاء الدوال؟', a: 'بعضها نعم. يدعم Qwen2.5 3B استدعاء الدوال ووضع JSON. يملك Llama 3.2 3B دعمًا أساسيًا لاستخدام الأدوات. لا يدعم Gemma 4 E2B استدعاء الدوال. راجع وثائق النموذج قبل بناء مسار يعتمد على مخرجات منظّمة.' },
            { q: 'أي نموذج صغير أفضل للغات غير الإنجليزية؟', a: 'يدعم Qwen2.5 3B 29 لغة بشكل أصلي، بما في ذلك الصينية واليابانية والكورية والعربية. Gemma 4 E2B وPhi-4 Mini محسّنان أساسًا للإنجليزية. لمهام اللغات الأخرى على نطاق النموذج الصغير، Qwen2.5 3B هو الخيار الواضح. راجع [مقارنة Qwen vs Llama vs Mistral متعددة اللغات](/ar/local-llms/qwen-vs-llama-vs-mistral) لمقارنة كاملة للغات.' },
            ...
            { q: 'ما أصغر نموذج LLM محلي قابل للاستخدام فعليًا؟', a: 'الحد الأدنى لإنتاج نص متماسك هو نحو 1B من المعاملات، وLlama 3.2 1B هو أصغر نموذج في هذه القائمة يستحق التشغيل لعمل حقيقي. أما النماذج الأصغر من ذلك فتُحمَّل وتولّد نصًا على أي عتاد تقريبًا، لكن النص يفقد تماسكه بعد بضع جمل، لذا يُفضَّل التعامل معها كإكمال تلقائي لا كمساعد. إذا كان القيد هو مساحة التخزين وRAM لا الجودة، فأصغر نموذج LLM محلي قابل للاستخدام بحجم صغير هو نموذج أقل من 2B بتكميم Q4؛ وإذا توفّر لديك 4GB أو أكثر، فإن نموذج 4B يمثل قفزة كبيرة في التماسك مقابل زيادة طفيفة جدًا في RAM.' },
            { q: 'ما أصغر نموذج LLM يمكنك تشغيله محليًا؟', a: 'Llama 3.2 1B هو أصغر نموذج Llama والخيار الأكثر عملية في هذه المقارنة -- يحتاج فقط إلى 1.3 GB من RAM ويعمل بسرعة 60-90 tok/ثانية على CPU، وهو أسرع نموذج في هذه الصفحة. يتعامل مع مهام تصنيف قصيرة واستخراج كلمات مفتاحية، لكنه يتعثر مع مخرجات أطول ومتعددة الخطوات. إذا كنت تريد نموذجًا صغيرًا (يُعرف أحيانًا بـ mini LLM) قابلًا للاستخدام فعليًا يوميًا، فاعتبر Llama 3.2 1B الحد الأدنى للعتاد لا للجودة -- انتقل إلى Gemma 4 E2B أو نموذج 3B مثل Qwen2.5 3B أو Llama 3.2 3B للعمل الحقيقي.' },
          ],
        },
        sources: {
          id: 'sources',
    ...
    pt: {
      ...
      intro: 'Os LLMs locais pequenos (1B-4B parâmetros) funcionam em máquinas com 4-8 GB de RAM e produzem 30-70 tokens/seg na CPU -- rápido o suficiente para chat em tempo real. Os melhores modelos pequenos em 2026 são Microsoft Phi-4 Mini 3.8B (melhor raciocínio), Google Gemma 4 E2B (o mais rápido), Qwen2.5 3B (melhor para código) e Meta Llama 3.2 3B (melhor uso geral).',
      metaDescription: 'Melhores modelos LLM locais sub-4B para 4-8 GB de RAM: Phi-4 Mini (68% MMLU), Gemma 4 E2B (60% MMLU Pro, contexto 128K), Qwen2.5 (código), Llama 3.2 (geral).',
      heroImage: '/images/small-local-llm-models-performance-tier-hero-pt.webp',
      publishDate: '2026-04-04',
      ...
      toc: [
        ...
        { label: '#2 Gemma 4 E2B -- O mais rápido na CPU', anchor: '#gemma-4-e2b' },
        { label: '#3 Qwen2.5 3B -- Melhor para código', anchor: '#qwen2-5-3b' },
      ...
      itemListSchema: {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Gemma 4 E2B', description: 'MMLU Pro 60%, LiveCodeBench v6 44%, GPQA Diamond 43,4%. ~2 GB RAM. Contexto 128K. Apache 2.0. ollama pull gemma4:e2b' },
          { '@type': 'ListItem', position: 3, name: 'Qwen2.5 3B', description: 'MMLU 62%, HumanEval 65%. 2 GB de RAM. 29 idiomas nativamente. Compatível com chamadas de função. ollama run qwen2.5:3b' },
        ...
        mainEntity: [
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'O mínimo prático para resultados úteis é um modelo 3B com quantização Q4_K_M. Os modelos abaixo de 2B parâmetros (Llama 3.2 1B, Gemma 4 E2B) produzem frases coerentes, mas têm dificuldade com instruções de várias etapas, respostas mais longas e raciocínio complexo. Para tarefas como resumo e perguntas e respostas simples, o Gemma 4 E2B é usável. Para qualquer coisa mais complexa, comece com um modelo 3B.',
            },
          },
          {
            '@type': 'Question',
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Alguns sim. O Qwen2.5 3B suporta chamadas de função e modo JSON. O Llama 3.2 3B tem suporte básico ao uso de ferramentas. O Gemma 4 E2B não suporta chamadas de função. Consulte a documentação do modelo antes de criar um pipeline que dependa de saída estruturada.',
            },
          },
          {
            '@type': 'Question',
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'O Qwen2.5 3B suporta 29 idiomas nativamente, incluindo chinês, japonês, coreano e árabe. O Gemma 4 E2B e o Phi-4 Mini são otimizados principalmente para inglês. Para tarefas em outros idiomas em escala de modelo pequeno, o Qwen2.5 3B é a escolha clara. Consulte a [comparação multilíngue Qwen vs Llama vs Mistral](/pt/local-llms/qwen-vs-llama-vs-mistral) para uma comparação completa de idiomas.',
            },
          },
          {
            '@type': 'Question',
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Sim. O Phi-4 Mini obtém 70% no HumanEval ante 60% do Llama 3.2 3B -- uma diferença significativa de 10 pontos nessa escala. Para assistência com código em máquinas de 4-6 GB de RAM, o Phi-4 Mini é a opção recomendada. Para código em vários idiomas (não Python), o Qwen2.5 3B com 65% no HumanEval é competitivo com o Phi-4 Mini e também suporta chamadas de função.',
            },
          },
          {
            '@type': 'Question',
          ...
          {
            '@type': 'Question',
            'name': 'Qual é o menor LLM local que é realmente usável?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Cerca de 1B de parâmetros é o piso para uma saída coerente, e o Llama 3.2 1B é o menor modelo desta lista que vale a pena rodar para trabalho de verdade. Abaixo disso, modelos minúsculos carregam e geram texto em quase qualquer hardware, mas o texto deixa de se manter coerente depois de poucas frases, então é melhor tratá-los como autocompletar do que como um assistente. Se sua restrição é disco e RAM em vez de qualidade, o menor LLM local usável com pegada reduzida é um modelo sub-2B em Q4; se você tem 4GB ou mais disponíveis, um modelo 4B representa um salto grande em coerência por muito pouca RAM extra.',
            },
          },
          {
            '@type': 'Question',
            'name': 'Qual é o menor modelo de LLM que você pode rodar localmente?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'O Llama 3.2 1B é o menor modelo Llama e a opção mais prática desta comparação -- precisa de apenas 1,3 GB de RAM e roda a 60-90 tok/seg na CPU, mais rápido que qualquer outro modelo desta página. Ele dá conta de tarefas curtas de classificação e extração de palavras-chave, mas falha em saídas mais longas e de várias etapas. Se você quer um modelo pequeno (às vezes chamado de "mini LLM") que seja realmente usável no dia a dia, trate o Llama 3.2 1B como o piso de hardware, não de qualidade -- suba para o Gemma 4 E2B ou um modelo 3B como Qwen2.5 3B ou Llama 3.2 3B para trabalho de verdade.',
            },
          },
        ],
      },
      sections: {
        tldr: {
          ...
          items: [
            ...
            '**Melhor modelo pequeno para código**: Qwen2.5 3B -- 65% HumanEval com ~2 GB de RAM.',
            '**Melhor 3B de uso geral**: Llama 3.2 3B -- maior suporte da comunidade, contexto de 128K, 2,5 GB de RAM.',
            'Nenhum modelo sub-2B produz qualidade de saída adequada para tarefas profissionais. Use 3B ou mais para trabalho de verdade.',
          ],
        },
        whatIsSmall: {
          id: 'what-is-small-llm',
          ...
          content: [
            ...
            'Os modelos pequenos são adequados para: resumo rápido, perguntas e respostas simples, explicação de trechos de código, tradução de textos curtos e tarefas de classificação. Eles não são adequados para raciocínio de várias etapas, geração de código complexo nem redação de documentos longos e coerentes.',
        ...
        phi4mini: {
          ...
          content: [
            ...
            'O Phi-4 Mini é a opção recomendada para usuários que precisam principalmente de raciocínio (matemática, lógica, explicações passo a passo) ou de assistência com código em hardware com 4-6 GB de RAM.',
          ],
          rows: [
        ...
        gemma2: {
          ...
          content: [
            ...
            'A janela de contexto de 128K é uma grande melhoria em relação aos 8K do antigo Gemma 2 2B -- agora iguala Phi-4 Mini e Llama 3.2 3B. A velocidade na CPU varia conforme o hardware e a quantização (o Google reporta ~7,6 tok/seg em um Raspberry Pi 5; relatos independentes em CPUs x86 variam de 8 a 48 tok/seg) -- verifique números atuais para seu dispositivo. Licença Apache 2.0.',
          ],
          rows: [
            { 'Especificação': 'MMLU Pro', 'Valor': '60%' },
            { 'Especificação': 'LiveCodeBench v6', 'Valor': '44%' },
            { 'Especificação': 'GPQA Diamond', 'Valor': '43,4%' },
        ...
        qwen25_3b: {
          id: 'qwen2-5-3b',
          title: 'Qwen2.5 3B -- Melhor modelo pequeno para tarefas de código',
          content: [
            '**O Qwen2.5 3B obtém 65% no HumanEval -- 5 pontos percentuais acima do Llama 3.2 3B -- tornando-o a melhor opção para tarefas de código em escala 3B.** Inclui modo JSON e suporte a chamadas de função, e lida nativamente com 29 idiomas.',
            'Para tarefas que não são de código em inglês, o Llama 3.2 3B e o Phi-4 Mini produzem prosa mais natural. Escolha o Qwen2.5 3B especificamente quando o código ou a saída multilíngue é o caso de uso principal.',
          ],
          rows: [
        ...
        llama32_1b: {
          ...
          content: 'O Llama 3.2 1B é o menor modelo Llama que a Meta publica: exige apenas 1,3 GB de RAM e gera 60-90 tok/seg na CPU -- o modelo executável localmente mais rápido desta página. A qualidade da saída é marginal: dá conta de classificações muito simples e extração de palavras-chave, mas tem dificuldade com respostas coerentes de várias frases. Use o Llama 3.2 1B só quando a RAM for genuinamente a restrição principal (menos de 3 GB disponíveis) ou para testar integrações de ferramentas.',
        },
        comparisonTable: {
          id: 'comparison-table',
          ...
          imageCaption: 'Níveis de desempenho: o Phi-4 Mini lidera em raciocínio e código, o Gemma 4 E2B lidera em contexto (128K) com a menor pegada de RAM, o Qwen2.5 se destaca em código.',
          rows: [
            { 'Modelo': 'Phi-4 Mini 3.8B', 'MMLU': '68%', 'HumanEval': '70%', 'RAM': '2,5 GB', 'Contexto': '128K', 'Melhor para': 'Raciocínio, código' },
            { 'Modelo': 'Qwen2.5 3B', 'MMLU': '62%', 'HumanEval': '65%', 'RAM': '2 GB', 'Contexto': '128K', 'Melhor para': 'Código, multilíngue' },
            { 'Modelo': 'Llama 3.2 3B', 'MMLU': '58%', 'HumanEval': '60%', 'RAM': '2,5 GB', 'Contexto': '128K', 'Melhor para': 'Uso geral, primeiro modelo' },
            { 'Modelo': 'Gemma 4 E2B', 'MMLU': '—', 'HumanEval': '—', 'RAM': '2 GB', 'Contexto': '128K', 'Melhor para': 'Edge (MMLU Pro 60%)' },
        ...
        relatedReading: {
          ...
          items: [
            ...
            '[Melhores LLMs locais para código](/pt/local-llms/best-local-llms-for-coding) -- modelos específicos de código em escala 7B+ quando o Qwen2.5 3B não basta para tarefas complexas',
        ...
        faqSection: {
          ...
          faqs: [
            { q: 'Qual é o menor LLM local que produz resultados úteis?', a: 'O mínimo prático para resultados úteis é um modelo 3B com quantização Q4_K_M. Os modelos abaixo de 2B parâmetros (Llama 3.2 1B, Gemma 4 E2B) produzem frases coerentes, mas têm dificuldade com instruções de várias etapas, respostas mais longas e raciocínio complexo. Para tarefas como resumo e perguntas e respostas simples, o Gemma 4 E2B é usável. Para qualquer coisa mais complexa, comece com um modelo 3B.' },
            ...
            { q: 'Os modelos pequenos suportam chamadas de função?', a: 'Alguns sim. O Qwen2.5 3B suporta chamadas de função e modo JSON. O Llama 3.2 3B tem suporte básico ao uso de ferramentas. O Gemma 4 E2B não suporta chamadas de função. Consulte a documentação do modelo antes de criar um pipeline que dependa de saída estruturada.' },
            { q: 'Qual modelo pequeno é melhor para idiomas diferentes do inglês?', a: 'O Qwen2.5 3B suporta 29 idiomas nativamente, incluindo chinês, japonês, coreano e árabe. O Gemma 4 E2B e o Phi-4 Mini são otimizados principalmente para inglês. Para tarefas em outros idiomas em escala de modelo pequeno, o Qwen2.5 3B é a escolha clara. Consulte a [comparação multilíngue Qwen vs Llama vs Mistral](/pt/local-llms/qwen-vs-llama-vs-mistral) para uma comparação completa de idiomas.' },
            ...
            { q: 'O Phi-4 Mini é melhor que o Llama 3.2 3B para código?', a: 'Sim. O Phi-4 Mini obtém 70% no HumanEval ante 60% do Llama 3.2 3B -- uma diferença significativa de 10 pontos nessa escala. Para assistência com código em máquinas de 4-6 GB de RAM, o Phi-4 Mini é a opção recomendada. Para código em vários idiomas (não Python), o Qwen2.5 3B com 65% no HumanEval é competitivo com o Phi-4 Mini e também suporta chamadas de função.' },
            ...
            { q: 'Qual é o menor LLM local que é realmente usável?', a: 'Cerca de 1B de parâmetros é o piso para uma saída coerente, e o Llama 3.2 1B é o menor modelo desta lista que vale a pena rodar para trabalho de verdade. Abaixo disso, modelos minúsculos carregam e geram texto em quase qualquer hardware, mas o texto deixa de se manter coerente depois de poucas frases, então é melhor tratá-los como autocompletar do que como um assistente. Se sua restrição é disco e RAM em vez de qualidade, o menor LLM local usável com pegada reduzida é um modelo sub-2B em Q4; se você tem 4GB ou mais disponíveis, um modelo 4B representa um salto grande em coerência por muito pouca RAM extra.' },
            { q: 'Qual é o menor modelo de LLM que você pode rodar localmente?', a: 'O Llama 3.2 1B é o menor modelo Llama e a opção mais prática desta comparação -- precisa de apenas 1,3 GB de RAM e roda a 60-90 tok/seg na CPU, mais rápido que qualquer outro modelo desta página. Ele dá conta de tarefas curtas de classificação e extração de palavras-chave, mas falha em saídas mais longas e de várias etapas. Se você quer um modelo pequeno (às vezes chamado de "mini LLM") que seja realmente usável no dia a dia, trate o Llama 3.2 1B como o piso de hardware, não de qualidade -- suba para o Gemma 4 E2B ou um modelo 3B como Qwen2.5 3B ou Llama 3.2 3B para trabalho de verdade.' },
          ],
        },
        sources: {
          id: 'sources',
    ...
    de: {
      ...
      metaDescription: 'Best Sub-4B Modelle für 4-8 GB RAM in 2026: Phi-4 Mini (68% MMLU), Gemma 4 E2B (60% MMLU Pro, 128K Kontext), Qwen2.5 (Codierung), Llama 3.2.',
      heroImage: '/images/small-local-llm-models-performance-tier-hero-de.webp',
      intro: 'Kleine lokale LLMs (1B-4B Parameter) laufen auf Maschinen mit 4-8 GB RAM und erzeugen 30-70 Token/Sek auf CPU -- schnell genug für Echtzeit-Chat. Die besten kleinen Modelle in 2026 sind Microsoft Phi-4 Mini 3.8B (bestes Reasoning), Google Gemma 4 E2B (schnellste), Qwen2.5 3B (beste Codierung) und Meta Llama 3.2 3B (beste allgemeine Nutzung).',
      publishDate: '2026-04-04',
      freshness_tier: 'semi_annual',
      next_refresh_due: '2027-03-02',
      leadAnswerBlock: '**Kleine lokale LLMs (1-4B Parameter) laufen auf Rechnern mit 4-8 GB RAM und liefern 30-70 Token/Sekunde auf der CPU — schnell genug für Chat in Echtzeit.**',
      readTime: '8 min',
      educationalLevel: 'Beginner',
      toc: [
        { label: 'Das Wichtigste', anchor: '#key-takeaways' },
        { label: 'Was ist ein „kleines" lokales LLM?', anchor: '#what-is-small-llm' },
        { label: 'Welches Modell sollten Sie verwenden?', anchor: '#model-selection-guide' },
        { label: '#1 Phi-4 Mini 3.8B -- Bestes Reasoning', anchor: '#phi-4-mini' },
        { label: '#2 Gemma 4 E2B -- Bestes kleines/Edge-Modell', anchor: '#gemma-4-e2b' },
        { label: '#3 Qwen2.5 3B -- Beste Codierung', anchor: '#qwen2-5-3b' },
        { label: '#4 Llama 3.2 3B -- Beste allgemeine Nutzung', anchor: '#llama-3-2-3b' },
        { label: '#5 Llama 3.2 1B -- Absolutes Minimum', anchor: '#llama-3-2-1b' },
        { label: 'Vollständige Vergleichstabelle', anchor: '#comparison-table' },
        { label: 'Quantisierungs-Leitfaden', anchor: '#quantization-guide' },
...
schema: {
        ...
        description: 'Beste kleine lokale LLMs 2026 : Phi-4 Mini (Reasoning), Gemma 4 E2B (128K Kontext, Edge), Qwen2.5 (Codierung), Llama 3.2 (allgemeine Verwendung). Vollständiger Vergleich für RAM-arme Hardware.',
        datePublished: '2026-04-04',
        dateModified: '2026-09-02',
      ...
      itemListSchema: {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Gemma 4 E2B', description: 'MMLU Pro 60%, LiveCodeBench v6 44%, GPQA Diamond 43,4%. ~2 GB RAM. 128K Kontext. Apache 2.0. ollama pull gemma4:e2b' },
          { '@type': 'ListItem', position: 3, name: 'Qwen2.5 3B', description: 'MMLU 62%, HumanEval 65%. 2 GB RAM. 29 Sprachen nativ. Funktionsaufrufe unterstützt. ollama run qwen2.5:3b' },
      ...
      faqSchema: {
        ...
        '@type': 'FAQPage',
        mainEntity: [
          { '@type': 'Question', name: 'Was ist das kleinste lokale LLM mit nützlicher Ausgabe?', acceptedAnswer: { '@type': 'Answer', text: '3B bei Q4_K_M ist das praktische Minimum. Modelle unter 2B (Llama 3.2 1B, Gemma 4 E2B) erzeugen kohärente einzelne Sätze, kämpfen aber mit mehrstufigen Anweisungen und Reasoning. Für Zusammenfassung und F&A ist Gemma 4 E2B brauchbar. Für komplexere Aufgaben: beginnen Sie mit 3B.' } },
          { '@type': 'Question', name: 'Kann ein 3B-Modell auf einem Telefon laufen?', acceptedAnswer: { '@type': 'Answer', text: 'Ja -- Llama 3.2 1B und 3B sind für mobile Bereitstellung konzipiert. Meta bietet optimierte Builds für iOS und Android. Inferenz auf modernem Telefon erzeugt 15-30 tok/sek für 1B. LM Studio und Ollama laufen nicht auf iOS/Android -- mobil erfordert separate Frameworks.' } },
          { '@type': 'Question', name: 'Sind kleine Modelle für Zusammenfassung geeignet?', acceptedAnswer: { '@type': 'Answer', text: 'Ja -- Zusammenfassung ist stärkster Anwendungsfall für kleine Modelle. Gemma 4 E2B und Llama 3.2 3B produzieren zuverlässig genaue Zusammenfassungen von Texten bis zu ~4.000 Wörtern. Für längere Dokumente verwenden Sie Modell mit großem Kontext wie Phi-4 Mini oder Llama 3.2 3B (beide 128K).' } },
          { '@type': 'Question', name: 'Wie viel schneller ist 2B als 7B auf gleicher Hardware?', acceptedAnswer: { '@type': 'Answer', text: 'Hängt stark von Hardware und Quantisierung ab -- Google berichtet ~7,6 Token/Sek (Decode) für Gemma 4 E2B auf einem Raspberry Pi 5, unabhängige x86-Tests liegen bei 8-48 Token/Sek. Auf GPU verengt sich Vorteil, da GPU-Durchsatz weniger durch Modellgröße begrenzt ist. Unterschied ist auf reinen CPU-Maschinen deutlichst.' } },
          { '@type': 'Question', name: 'Unterstützen kleine Modelle Funktionsaufrufe?', acceptedAnswer: { '@type': 'Answer', text: 'Einige tun dies. Qwen2.5 3B unterstützt Funktionsaufrufe und JSON. Llama 3.2 3B hat grundlegende Werkzeugunterstützung. Gemma 4 E2B unterstützt keine Funktionsaufrufe. Überprüfen Sie Modelldokumentation vor Pipeline-Aufbau.' } },
          { '@type': 'Question', name: 'Welches kleine Modell ist beste für nicht-englische Sprachen?', acceptedAnswer: { '@type': 'Answer', text: 'Qwen2.5 3B unterstützt nativ 29 Sprachen (Chinesisch, Japanisch, Koreanisch, Arabisch). Gemma 4 E2B und Phi-4 Mini sind hauptsächlich auf Englisch optimiert. Für nicht-englische Aufgaben bei kleinen Modellen ist Qwen2.5 3B klare Wahl.' } },
          { '@type': 'Question', name: 'Unterschied zwischen Phi-4 Mini und Llama 3.2 3B?', acceptedAnswer: { '@type': 'Answer', text: 'Phi-4 Mini übertrifft bei Reasoning, Mathematik, Codierung (68% vs 58% MMLU, 70% vs 60% HumanEval) bei fast identischem RAM (2,5 GB). Für Alltags-Aufgaben ist Lücke spürbar aber nicht dramatisch. Llama 3.2 3B hat breitere Support. Wählen Sie Phi-4 Mini für Reasoning; Llama für Chat.' } },
          { '@type': 'Question', name: 'Kann ich zwei kleine Modelle gleichzeitig laufen lassen?', acceptedAnswer: { '@type': 'Answer', text: 'Ja, wenn RAM zulässt. Zwei 3B-Modelle bei Q4_K_M verwenden ~5 GB kombiniert -- auf 8 GB Maschine mit schlankem OS möglich. Ollama lädt standardmäßig ein Modell pro Prozess. Laufen Sie zwei Ollama-Instanzen auf verschiedenen Ports (OLLAMA_HOST=:11434 und :11435) parallel.' } },
          { '@type': 'Question', name: 'Funktionieren kleine Modelle für RAG?', acceptedAnswer: { '@type': 'Answer', text: 'Ja für einfaches RAG. Llama 3.2 3B und Phi-4 Mini beantworten Fragen über abgerufene Dokumentchunks zuverlässig. Für RAG über große Wissensdatenbanken mit Multi-Hop-Reasoning sind 7B+-Modelle konsistenter. GPT4All LocalDocs nutzt 3B für Dokument-Q&A.' } },
          { '@type': 'Question', name: 'Ist Phi-4 Mini besser als Llama 3.2 3B für Codierung?', acceptedAnswer: { '@type': 'Answer', text: 'Ja. Phi-4 Mini erreicht 70% auf HumanEval vs 60% für Llama 3.2 3B -- aussagekräftige 10-Punkte-Lücke. Für Code-Hilfe auf 4-6 GB RAM ist Phi-4 Mini empfohlen. Für mehrsprachige Codierung (nicht-Python) ist Qwen2.5 3B mit 65% HumanEval wettbewerbsfähig und unterstützt Funktionsaufrufe.' } },
          { '@type': 'Question', name: 'Q8_0-Quantisierung statt Q4_K_M verwenden', acceptedAnswer: { '@type': 'Answer', text: 'Q8_0 benötigt fast doppelt so viel RAM wie Q4_K_M bei minimaler Qualitätsverbesserung im kleinen Maßstab. Ein Llama 3.2 3B-Modell mit Q8_0 benötigt ~3,8 GB RAM statt ~2,5 GB bei Q4_K_M. Auf einer 4-GB-Maschine kann Q8_0 Swap-Nutzung auslösen und die Inferenz um das 3-5-fache verlangsamen. Verwenden Sie für Sub-4B-Modelle immer Q4_K_M als Standard.' } },
          { '@type': 'Question', name: 'Ein Basismodell statt der Instruct-Variante ausführen', acceptedAnswer: { '@type': 'Answer', text: 'Basismodelle (z. B. llama3.2:3b-text) sind Checkpoints vor dem Fine-Tuning, die darauf trainiert sind, das nächste Token in einem Text vorherzusagen. Sie folgen keinen Anweisungen. Wenn Sie ein Basismodell fragen „Was ist 2+2?", vervollständigt es den Satz möglicherweise wie ein Quiz, anstatt „4" zu antworten. Verwenden Sie immer die Instruct-Variante: llama3.2:3b (Ollama verwendet standardmäßig Instruct für benannte Modelle).' } },
          { '@type': 'Question', name: '7B-Modellqualität von einem 3B-Modell erwarten', acceptedAnswer: { '@type': 'Answer', text: 'Ein 3B-Modell mit 68% MMLU (Phi-4 Mini) schneidet bei allgemeinen Aufgaben ähnlich ab wie ein GPT-3.5 Mini aus dem Jahr 2023. Komplexe Reasoning-Ketten, lange Texte und differenzierte Codegenerierung liefern eine spürbar niedrigere Qualität als ein 7B-Modell. Wenn die Ausgabequalität nicht ausreicht, wechseln Sie zu einem 7B-Modell -- der RAM-Unterschied beträgt ~2 GB (2,5 GB → 4,5 GB).' } },
          { '@type': 'Question', name: 'Was ist das kleinste lokale LLM, das tatsächlich brauchbar ist?', acceptedAnswer: { '@type': 'Answer', text: 'Etwa 1B Parameter sind die Untergrenze für kohärente Ausgaben, und Llama 3.2 1B ist das kleinste Modell in dieser Liste, das sich für echte Arbeit lohnt. Darunter laden und generieren winzige Modelle zwar auf fast jeder Hardware, aber der Text verliert nach wenigen Sätzen seine Kohärenz -- solche Modelle eignen sich eher als Autovervollständigung denn als Assistent. Wenn Speicherplatz und RAM statt Qualität die Einschränkung sind, ist das kleinste brauchbare lokale LLM mit geringem Fußabdruck ein Sub-2B-Modell bei Q4; mit 4 GB oder mehr ist ein 4B-Modell ein großer Kohärenzsprung für sehr wenig zusätzlichen RAM.' } },
          { '@type': 'Question', name: 'Was ist das kleinste LLM-Modell, das Sie lokal ausführen können?', acceptedAnswer: { '@type': 'Answer', text: 'Llama 3.2 1B ist das kleinste Llama-Modell und die praktikabelste Option in diesem Vergleich -- es benötigt nur 1,3 GB RAM und läuft mit 60-90 tok/sek auf der CPU, schneller als jedes andere Modell auf dieser Seite. Es bewältigt kurze Klassifizierungs- und Stichwort-Extraktionsaufgaben, scheitert aber an längeren, mehrstufigen Ausgaben. Wenn Sie ein kleines Modell (manchmal „Mini-LLM" genannt) suchen, das im Alltag wirklich brauchbar ist, betrachten Sie Llama 3.2 1B als Hardware-Untergrenze, nicht als Qualitäts-Untergrenze -- steigen Sie für echte Arbeit auf Gemma 4 E2B oder ein 3B-Modell wie Qwen2.5 3B oder Llama 3.2 3B um.' } },
        ],
      },
      sections: {
 ...
 items: ['**Bestes Reasoning im kleinen Format**: Phi-4 Mini 3.8B -- 68% MMLU, 70% HumanEval, läuft auf 4 GB RAM.', '**Bestes kleines/Edge-Modell**: Gemma 4 E2B -- MMLU Pro 60%, 128K Kontext, ~2 GB RAM, Apache 2.0. Ersetzt das ältere Gemma 2 2B.', '**Bestes kleines Codierungsmodell**: Qwen2.5 3B -- 65% HumanEval bei ~2 GB RAM.', '**Beste Universal-3B**: Llama 3.2 3B -- beste Community-Unterstützung, 128K Kontext, 2,5 GB RAM.', 'Kein Sub-2B-Modell produziert Ausgabequalität, die für professionelle Aufgaben geeignet ist. Verwenden Sie 3B+ für echte Arbeit.'] },
        whatIsSmall: { id: 'what-is-small-llm', title: 'Was ist ein „kleines" lokales LLM und wann sollten Sie eines verwenden?', content: ['Ein kleines lokales LLM ist typischerweise definiert als ein Modell mit weniger als 4 Milliarden Parametern. Bei Q4_K_M-Quantisierung benötigen diese Modelle 1,5-3 GB RAM -- gut innerhalb der Einschränkungen von Einstiegs-Laptops mit 4-8 GB Gesamtspeicher.', 'Kleine Modelle sind geeignet für: schnelle Zusammenfassung, einfache F&A, Erklärung von Code-Snippets, Übersetzung kurzer Texte und Klassifizierungsaufgaben. Sie sind nicht geeignet für mehrstufiges Reasoning, komplexe Code-Generierung oder das Verfassen langer kohärenter Dokumente.', 'Die Qualitätslücke zwischen einem 3B- und 7B-Modell ist erheblich -- ungefähr der Lücke zwischen GPT-4o mini und GPT-5.5 entsprechend. Für Benutzer mit 8 GB RAM ist ein 7B-Modell bei Q4_K_M fast immer die bessere Wahl. Siehe [Beste Anfänger-Lokale-LLM-Modelle](/local-llms/best-beginner-local-llm-models) für 7B-Empfehlungen.'] },
        modelSelectionGuide: { id: 'model-selection-guide', title: 'Welches Modell sollten Sie verwenden? Kurzer Entscheidungsleitfaden', image: '/images/small-llm-decision-tree-mockup.svg', imageCaption: 'Entscheidungsbaum: Wählen Sie nach Priorität (Reasoning, Geschwindigkeit oder Codierung). Im Zweifel Llama 3.2 3B als Standard.' },
        phi4mini: { id: 'phi-4-mini', title: 'Phi-4 Mini 3.8B -- Beste Reasoning-Leistung in der Sub-4B-Klasse', content: ['Microsoft Phi-4 Mini erreicht 68% bei MMLU und 70% bei HumanEval -- Werte, die viele vor 2025 veröffentlichte 7B-Modelle übertreffen. Dies ist möglich, weil Phi-4 Mini auf einem kuratierten synthetischen Datensatz trainiert wurde, der auf Reasoning und Problemlösung fokussiert ist.', 'Phi-4 Mini ist die empfohlene Wahl für Benutzer, die hauptsächlich Reasoning oder Code-Unterstützung auf Hardware mit 4-6 GB RAM benötigen.'], rows: [{ 'Spec': 'MMLU', 'Value': '68%' }, { 'Spec': 'HumanEval', 'Value': '70%' }, { 'Spec': 'RAM (Q4_K_M)', 'Value': '~2,5 GB' }, { 'Spec': 'Kontext', 'Value': '128K Token' }, { 'Spec': 'CPU-Geschwindigkeit', 'Value': '30-50 tok/sek' }, { 'Spec': 'Ollama-Befehl', 'Value': 'ollama run phi4-mini' }], columns: ['Spec', 'Value'] },
        gemma2: { id: 'gemma-4-e2b', title: 'Gemma 4 E2B -- Bestes kleines Modell/Edge-Modell', content: ['Google Gemma 4 E2B ist die aktuelle Generation kleiner Modelle und ersetzt das ältere Gemma 2 2B. Es erreicht 60% auf MMLU Pro (ein anspruchsvollerer Benchmark als das ursprüngliche MMLU), 44% auf LiveCodeBench v6 und 43,4% auf GPQA Diamond, laut Googles eigener Modellkarte. Es hat 2,3B effektive Parameter (5,1B inklusive Embeddings) und passt in ~2 GB RAM.', 'Das 128K-Kontextfenster ist eine deutliche Verbesserung gegenüber den 8K des alten Gemma 2 2B -- es entspricht nun Phi-4 Mini und Llama 3.2 3B. Die CPU-Geschwindigkeit variiert je nach Hardware und Quantisierung (Google berichtet ~7,6 Token/Sek Decode auf einem Raspberry Pi 5; unabhängige x86-Berichte reichen von 8-48 Token/Sek) -- aktuelle Werte für Ihr Gerät prüfen. Apache-2.0-Lizenz.'], rows: [{ 'Spec': 'MMLU Pro', 'Value': '60%' }, { 'Spec': 'LiveCodeBench v6', 'Value': '44%' }, { 'Spec': 'GPQA Diamond', 'Value': '43,4%' }, { 'Spec': 'RAM (Q4_K_M)', 'Value': '~2 GB' }, { 'Spec': 'Kontext', 'Value': '128K Token' }, { 'Spec': 'Ollama-Befehl', 'Value': 'ollama pull gemma4:e2b' }], columns: ['Spec', 'Value'] },
        qwen25_3b: { id: 'qwen2-5-3b', title: 'Qwen2.5 3B -- Bestes kleines Modell für Codierungsaufgaben', content: ['Qwen2.5 3B erreicht 65% bei HumanEval -- 5 Prozentpunkte über Llama 3.2 3B -- und ist beste Wahl für Codierungsaufgaben im 3B-Format. Es umfasst JSON-Modus und Funktionsaufruf-Unterstützung und verarbeitet nativ 29 Sprachen.', 'Für nicht-Codierungs-Aufgaben im Englischen erzeugen Llama 3.2 3B und Phi-4 Mini natürlichere Prosa. Wählen Sie Qwen2.5 3B speziell für Codierung oder mehrsprachige Aufgaben.'], rows: [{ 'Spec': 'MMLU', 'Value': '62%' }, { 'Spec': 'HumanEval', 'Value': '65%' }, { 'Spec': 'RAM (Q4_K_M)', 'Value': '~2 GB' }, { 'Spec': 'Kontext', 'Value': '128K Token' }, { 'Spec': 'CPU-Geschwindigkeit', 'Value': '25-40 tok/sek' }, { 'Spec': 'Ollama-Befehl', 'Value': 'ollama run qwen2.5:3b' }], columns: ['Spec', 'Value'] },
        llama32_3b: { id: 'llama-3-2-3b', title: 'Llama 3.2 3B -- Bestes Universal-Kleinmodell', content: ['Meta Llama 3.2 3B ist das am weitesten dokumentierte und von der Community unterstützte 3B-Modell. Es erreicht 58% bei MMLU und 60% bei HumanEval, aber hat breiteste Werkzeugunterstützung, die meisten Fine-Tunes und größte Sammlung von Community-Guides.', 'Das 128K-Kontextfenster eignet sich für Zusammenfassung von Dokumenten mittlerer Länge. Für ein erstes kleines Modell bleibt Llama 3.2 3B die sicherste Wahl aufgrund vorhersehbaren Verhaltens und umfangreicher Dokumentation.'], rows: [{ 'Spec': 'MMLU', 'Value': '58%' }, { 'Spec': 'RAM (Q4_K_M)', 'Value': '~2,5 GB' }, { 'Spec': 'Kontext', 'Value': '128K Token' }, { 'Spec': 'CPU-Geschwindigkeit', 'Value': '25-45 tok/sek' }, { 'Spec': 'Ollama-Befehl', 'Value': 'ollama run llama3.2:3b' }], columns: ['Spec', 'Value'] },
        llama32_1b: { id: 'llama-3-2-1b', title: 'Llama 3.2 1B -- Absolute Mindestanforderung für nützliche Ausgabe', content: 'Llama 3.2 1B ist das kleinste Llama-Modell, das Meta veröffentlicht: Es benötigt nur 1,3 GB RAM und generiert 60-90 tok/sek auf CPU -- das schnellste lokal ausführbare Modell auf dieser Seite. Die Ausgabequalität ist marginal: es bewältigt sehr einfache Klassifizierung, kämpft aber mit kohärenten mehrsätzigen Antworten. Verwenden Sie Llama 3.2 1B nur, wenn RAM die bindende Einschränkung ist oder zum Testen von Tool-Integrationen.' },
        comparisonTable: { id: 'comparison-table', title: 'Vollständiger Vergleich: Beste kleine lokale LLMs unter 4B Parametern', image: '/images/small-local-llm-models-performance-tier-hero-de.webp', imageCaption: 'Leistungsstufen: Phi-4 Mini führt bei Reasoning und Codierung, Gemma 4 E2B führt bei der Kontextlänge (128K) mit dem kleinsten RAM-Bedarf, Qwen2.5 glänzt beim Codieren.', rows: [{ 'Modell': 'Phi-4 Mini 3.8B', 'MMLU': '68%', 'HumanEval': '70%', 'RAM': '2,5 GB', 'Kontext': '128K', 'Beste für': 'Reasoning, Codierung' }, { 'Modell': 'Qwen2.5 3B', 'MMLU': '62%', 'HumanEval': '65%', 'RAM': '2 GB', 'Kontext': '128K', 'Beste für': 'Codierung, mehrsprachig' }, { 'Modell': 'Llama 3.2 3B', 'MMLU': '58%', 'HumanEval': '60%', 'RAM': '2,5 GB', 'Kontext': '128K', 'Beste für': 'Allgemein, erstes Modell' }, { 'Modell': 'Gemma 4 E2B', 'MMLU': '—', 'HumanEval': '—', 'RAM': '2 GB', 'Kontext': '128K', 'Beste für': 'Edge (MMLU Pro 60%)' }, { 'Modell': 'Llama 3.2 1B', 'MMLU': '32%', 'HumanEval': '28%', 'RAM': '1,3 GB', 'Kontext': '128K', 'Beste für': 'Minimales RAM' }], columns: ['Modell', 'MMLU', 'HumanEval', 'RAM', 'Kontext', 'Beste für'] },
        regionalContext: { id: 'regional-context', title: 'Kleine lokale LLMs nach Region', content: ['**EU / DSGVO:** Für EU-Profis auf begrenzter Hardware -- Feldarbeit, air-gapped Umgebungen, ältere Enterprise-Laptops -- bieten kleine lokale Modelle DSGVO-konforme Inferenz ohne Datengress. Ein Phi-4 Mini 3.8B auf Standard-Corporate-Laptop (8 GB RAM) behält alle Texte auf Gerät unter DSGVO-Artikel 5. Für BSI-Compliance: Phi-4 Mini (Microsoft, MIT-Lizenz) und Llama 3.2 3B (Meta, Llama Community-Lizenz) bieten versionierte Modell-Identifikatoren via Ollama-Tags. Mistral bietet derzeit kein Sub-4B-Modell. Für EU-Organisationen mit EU-Präferenz sind Optionen begrenzt, bis Mistral Sub-4B freigibt.', '**Japan (METI):** Für japanischsprachige Aufgaben ist Qwen2.5 3B das einzige Modell mit nativer japanischer Tokenisierung. Llama 3.2 3B verarbeitet Japanisch mit niedrigerer Token-Effizienz. Für japanische Zusammenfassung oder Übersetzung: `ollama run qwen2.5:3b`. Der Geschwindigkeitsvorteil ist relevant für japanischen Enterprise-Einsatz: 25-40 tok/sek auf CPU bietet angemessene Echtzeit-Antwort auf Standard-Office-Hardware.', '**China:** Qwen2.5 3B (Alibaba, Apache 2.0) ist natürliche Wahl für chinesischsprachige Bereitstellung. Native chinesische Tokenisierung verarbeitet Mandarin-Text 30-40% effizienter als Llama. Für IoT und Edge unter Chinas Datensicherheitsgesetz: `ollama run qwen2.5:3b` läuft auf jedem Linux-Gerät mit 4 GB RAM ohne externe API-Aufrufe.'] },
        commonMistakes: { id: 'common-mistakes', title: 'Häufige Fehler beim Ausführen kleiner lokaler LLMs', items: ['**Verwendung von Q8_0 statt Q4_K_M:** Q8_0 benötigt fast doppelt so viel RAM mit minimaler Qualitätsverbesserung. Ein Llama 3.2 3B mit Q8_0 benötigt ~3,8 GB RAM vs ~2,5 GB für Q4_K_M. Auf 4 GB Maschinen kann Q8_0 Swap-Nutzung und 3-5× langsamere Inferenz auslösen. Verwenden Sie immer Q4_K_M für Sub-4B-Modelle.', '**Ausführung eines Basis-Modells statt Instruct-Variante:** Basis-Modelle (z. B. `llama3.2:3b-text`) folgen keine Anweisungen. Wenn Sie ein Basis-Modell „Was ist 2+2?" fragen, kann es den Satz als Quiz vervollständigen statt „4" zu antworten. Verwenden Sie immer: `llama3.2:3b` (Ollama greift standardmäßig zu Instruct).', '**Erwartung von 7B-Qualität aus 3B-Modell:** Ein 3B-Modell mit 68% MMLU (Phi-4 Mini) funktioniert ähnlich wie GPT-4o mini aus 2023. Komplexe Reasoning, lange Texte und differenzierter Code haben niedrigere Qualität. Für bessere Qualität: aktualisieren Sie auf 7B (RAM-Unterschied: ~2 GB mehr).'] },
        quantizationGuide: { id: 'quantization-guide', title: 'Quantisierung verstehen: RAM- vs. Qualitäts-Kompromiss', image: '/images/small-local-llm-models-quantization-tradeoff-hero-de.webp', imageCaption: 'Quantisierungs-Kompromiss: Q4_K_M (2,5 GB, -0,5% Qualität) ist die empfohlene Standardeinstellung. Q8_0 nutzt 3,8 GB ohne Qualitätsgewinn. Q3_K_M (1,8 GB, -1,8% Verlust) für extreme RAM-Einschränkungen.' },
        relatedReading: { id: 'related-reading', title: 'Weiterführende Lektüre', items: ['[Edge-AI-Hardware fuer lokale LLMs 2026](/de/power-local-llm/edge-ai-hardware-for-local-llms) -- Was wirklich in ein Serienprodukt passt: Leistungsbudget, thermisches Design, Joule pro Token statt Desktop-Tokens/Sek.', '[Beste Anfänger-Lokale-LLM-Modelle](/local-llms/best-beginner-local-llm-models) -- 7B-Empfehlungen wenn 8 GB RAM verfügbar', '[Ihr erstes lokales LLM ausführen](/local-llms/run-first-local-llm) -- Schritt-für-Schritt-Anleitung mit Ollama unter 10 Minuten', '[Lokales LLM auf Laptop](/local-llms/local-llm-on-laptop) -- Wärmeverwaltung und Batterielebensdauer auf begrenzter Hardware', '[LLM-Quantisierung erklärt](/local-llms/llm-quantization-explained) -- warum Q4_K_M Standard ist und Q3_K_M für extreme RAM-Einschränkungen', '[Beste lokale LLMs zum Codieren](/local-llms/best-local-llms-for-coding) -- Codierungs-Modelle bei 7B+ wenn Qwen2.5 3B nicht ausreicht', '[Lokales LLM-Setup beheben](/local-llms/troubleshooting-local-llm-setup) -- OOM-Fehler, langsame Inferenz und Ladefehler auf RAM-armen Maschinen', '[Beste 7B-Modelle für Consumer-Hardware](/de/local-llms/best-7b-models-consumer-hardware) -- beste 7B-Modelle für Consumer-Hardware'] },
        faqSection: { id: 'faq', title: 'Häufig gestellte Fragen', faqs: [{ q: 'Was ist das kleinste lokale LLM mit nützlicher Ausgabe?', a: '3B bei Q4_K_M ist das praktische Minimum. Modelle unter 2B (Llama 3.2 1B, Gemma 4 E2B) erzeugen kohärente einzelne Sätze, kämpfen aber mit mehrstufigen Anweisungen und Reasoning. Für Zusammenfassung und F&A ist Gemma 4 E2B brauchbar. Für komplexere Aufgaben: beginnen Sie mit 3B.' }, { q: 'Kann ein 3B-Modell auf einem Telefon laufen?', a: 'Ja -- Llama 3.2 1B und 3B sind für mobile Bereitstellung konzipiert. Meta bietet optimierte Builds für iOS und Android. Inferenz auf modernem Telefon erzeugt 15-30 tok/sek für 1B. LM Studio und Ollama laufen nicht auf iOS/Android -- mobil erfordert separate Frameworks.' }, { q: 'Sind kleine Modelle für Zusammenfassung geeignet?', a: 'Ja -- Zusammenfassung ist stärkster Anwendungsfall für kleine Modelle. Gemma 4 E2B und Llama 3.2 3B produzieren zuverlässig genaue Zusammenfassungen von Texten bis zu ~4.000 Wörtern. Für längere Dokumente verwenden Sie Modell mit großem Kontext wie Phi-4 Mini oder Llama 3.2 3B (beide 128K).' }, { q: 'Wie viel schneller ist 2B als 7B auf gleicher Hardware?', a: 'Hängt stark von Hardware und Quantisierung ab -- Google berichtet ~7,6 Token/Sek (Decode) für Gemma 4 E2B auf einem Raspberry Pi 5, unabhängige x86-Tests liegen bei 8-48 Token/Sek. Auf GPU verengt sich Vorteil, da GPU-Durchsatz weniger durch Modellgröße begrenzt ist. Unterschied ist auf reinen CPU-Maschinen deutlichst.' }, { q: 'Unterstützen kleine Modelle Funktionsaufrufe?', a: 'Einige tun dies. Qwen2.5 3B unterstützt Funktionsaufrufe und JSON. Llama 3.2 3B hat grundlegende Werkzeugunterstützung. Gemma 4 E2B unterstützt keine Funktionsaufrufe. Überprüfen Sie Modelldokumentation vor Pipeline-Aufbau.' }, { q: 'Welches kleine Modell ist beste für nicht-englische Sprachen?', a: 'Qwen2.5 3B unterstützt nativ 29 Sprachen (Chinesisch, Japanisch, Koreanisch, Arabisch). Gemma 4 E2B und Phi-4 Mini sind hauptsächlich auf Englisch optimiert. Für nicht-englische Aufgaben bei kleinen Modellen ist Qwen2.5 3B klare Wahl.' }, { q: 'Unterschied zwischen Phi-4 Mini und Llama 3.2 3B?', a: 'Phi-4 Mini übertrifft bei Reasoning, Mathematik, Codierung (68% vs 58% MMLU, 70% vs 60% HumanEval) bei fast identischem RAM (2,5 GB). Für Alltags-Aufgaben ist Lücke spürbar aber nicht dramatisch. Llama 3.2 3B hat breitere Support. Wählen Sie Phi-4 Mini für Reasoning; Llama für Chat.' }, { q: 'Kann ich zwei kleine Modelle gleichzeitig laufen lassen?', a: 'Ja, wenn RAM zulässt. Zwei 3B-Modelle bei Q4_K_M verwenden ~5 GB kombiniert -- auf 8 GB Maschine mit schlankem OS möglich. Ollama lädt standardmäßig ein Modell pro Prozess. Laufen Sie zwei Ollama-Instanzen auf verschiedenen Ports (OLLAMA_HOST=:11434 und :11435) parallel.' }, { q: 'Funktionieren kleine Modelle für RAG?', a: 'Ja für einfaches RAG. Llama 3.2 3B und Phi-4 Mini beantworten Fragen über abgerufene Dokumentchunks zuverlässig. Für RAG über große Wissensdatenbanken mit Multi-Hop-Reasoning sind 7B+-Modelle konsistenter. GPT4All LocalDocs nutzt 3B für Dokument-Q&A.' }, { q: 'Ist Phi-4 Mini besser als Llama 3.2 3B für Codierung?', a: 'Ja. Phi-4 Mini erreicht 70% auf HumanEval vs 60% für Llama 3.2 3B -- aussagekräftige 10-Punkte-Lücke. Für Code-Hilfe auf 4-6 GB RAM ist Phi-4 Mini empfohlen. Für mehrsprachige Codierung (nicht-Python) ist Qwen2.5 3B mit 65% HumanEval wettbewerbsfähig und unterstützt Funktionsaufrufe.' }, { q: 'Q8_0-Quantisierung statt Q4_K_M verwenden', a: 'Q8_0 benötigt fast doppelt so viel RAM wie Q4_K_M bei minimaler Qualitätsverbesserung im kleinen Maßstab. Ein Llama 3.2 3B-Modell mit Q8_0 benötigt ~3,8 GB RAM statt ~2,5 GB bei Q4_K_M. Auf einer 4-GB-Maschine kann Q8_0 Swap-Nutzung auslösen und die Inferenz um das 3-5-fache verlangsamen. Verwenden Sie für Sub-4B-Modelle immer Q4_K_M als Standard.' }, { q: 'Ein Basismodell statt der Instruct-Variante ausführen', a: 'Basismodelle (z. B. llama3.2:3b-text) sind Checkpoints vor dem Fine-Tuning, die darauf trainiert sind, das nächste Token in einem Text vorherzusagen. Sie folgen keinen Anweisungen. Wenn Sie ein Basismodell fragen „Was ist 2+2?", vervollständigt es den Satz möglicherweise wie ein Quiz, anstatt „4" zu antworten. Verwenden Sie immer die Instruct-Variante: llama3.2:3b (Ollama verwendet standardmäßig Instruct für benannte Modelle).' }, { q: '7B-Modellqualität von einem 3B-Modell erwarten', a: 'Ein 3B-Modell mit 68% MMLU (Phi-4 Mini) schneidet bei allgemeinen Aufgaben ähnlich ab wie ein GPT-3.5 Mini aus dem Jahr 2023. Komplexe Reasoning-Ketten, lange Texte und differenzierte Codegenerierung liefern eine spürbar niedrigere Qualität als ein 7B-Modell. Wenn die Ausgabequalität nicht ausreicht, wechseln Sie zu einem 7B-Modell -- der RAM-Unterschied beträgt ~2 GB (2,5 GB → 4,5 GB).' }, { q: 'Was ist das kleinste lokale LLM, das tatsächlich brauchbar ist?', a: 'Etwa 1B Parameter sind die Untergrenze für kohärente Ausgaben, und Llama 3.2 1B ist das kleinste Modell in dieser Liste, das sich für echte Arbeit lohnt. Darunter laden und generieren winzige Modelle zwar auf fast jeder Hardware, aber der Text verliert nach wenigen Sätzen seine Kohärenz -- solche Modelle eignen sich eher als Autovervollständigung denn als Assistent. Wenn Speicherplatz und RAM statt Qualität die Einschränkung sind, ist das kleinste brauchbare lokale LLM mit geringem Fußabdruck ein Sub-2B-Modell bei Q4; mit 4 GB oder mehr ist ein 4B-Modell ein großer Kohärenzsprung für sehr wenig zusätzlichen RAM.' }, { q: 'Was ist das kleinste LLM-Modell, das Sie lokal ausführen können?', a: 'Llama 3.2 1B ist das kleinste Llama-Modell und die praktikabelste Option in diesem Vergleich -- es benötigt nur 1,3 GB RAM und läuft mit 60-90 tok/sek auf der CPU, schneller als jedes andere Modell auf dieser Seite. Es bewältigt kurze Klassifizierungs- und Stichwort-Extraktionsaufgaben, scheitert aber an längeren, mehrstufigen Ausgaben. Wenn Sie ein kleines Modell (manchmal „Mini-LLM" genannt) suchen, das im Alltag wirklich brauchbar ist, betrachten Sie Llama 3.2 1B als Hardware-Untergrenze, nicht als Qualitäts-Untergrenze -- steigen Sie für echte Arbeit auf Gemma 4 E2B oder ein 3B-Modell wie Qwen2.5 3B oder Llama 3.2 3B um.' }] },
        sources: { id: 'sources', title: 'Quellen', items: ['Hugging Face Open LLM Leaderboard -- open-llm-leaderboard.hf.space (MMLU und HumanEval Werte)', 'Microsoft Phi-4 Technical Report -- microsoft.com/en-us/research/publication/phi-4-technical-report/', 'Meta Llama 3.2 Model Card -- huggingface.co/meta-llama/Llama-3.2-3B-Instruct', 'Google Gemma 4 Model Card -- huggingface.co/google/gemma-4-e2b-it'] },
      },
    },
    fr: {
      theme: 'Best Models',
      ...
      metaDescription: 'Top 5 modèles sub-4B pour 4-8 GB RAM en 2026 : Phi-4 Mini (68% MMLU), Gemma 4 E2B (60% MMLU Pro, contexte 128K), Qwen2.5 (codage), Llama 3.2.',
      heroImage: '/images/small-local-llm-models-performance-tier-hero-fr.webp',
      intro: 'Les petits LLMs locaux (1B-4B paramètres) fonctionnent sur des machines avec 4-8 GB RAM et produisent 30-70 tokens/sec sur CPU -- assez rapide pour le chat en temps réel. Les meilleurs petits modèles en 2026 sont Microsoft Phi-4 Mini 3.8B (meilleur reasoning), Google Gemma 4 E2B (plus rapide), Qwen2.5 3B (meilleur codage), et Meta Llama 3.2 3B (meilleur usage général).',
      publishDate: '2026-04-04',
      freshness_tier: 'semi_annual',
      next_refresh_due: '2027-03-02',
      leadAnswerBlock: '**Les petits LLM locaux (1 à 4 milliards de paramètres) tournent sur des machines dotées de 4 à 8 Go de RAM et produisent 30 à 70 tokens/seconde sur CPU — assez rapide pour discuter en temps réel.**',
      readTime: '8 min',
      educationalLevel: 'Beginner',
      toc: [
        { label: 'Points clés', anchor: '#key-takeaways' },
        { label: 'Qu\'est-ce qu\'un "petit" LLM local ?', anchor: '#what-is-small-llm' },
        { label: 'Quel modèle choisir ?', anchor: '#model-selection-guide' },
        { label: '#1 Phi-4 Mini 3.8B -- Meilleur raisonnement', anchor: '#phi-4-mini' },
        { label: '#2 Gemma 4 E2B -- Meilleur petit modèle/edge', anchor: '#gemma-4-e2b' },
        { label: '#3 Qwen2.5 3B -- Meilleur pour le codage', anchor: '#qwen2-5-3b' },
        { label: '#4 Llama 3.2 3B -- Meilleur usage général', anchor: '#llama-3-2-3b' },
        { label: '#5 Llama 3.2 1B -- Minimum absolu', anchor: '#llama-3-2-1b' },
        { label: 'Tableau comparatif complet', anchor: '#comparison-table' },
        { label: 'Guide de quantification', anchor: '#quantization-guide' },
...
schema: {
        ...
        description: 'Meilleurs petits LLMs locaux 2026 : Phi-4 Mini (reasoning), Gemma 4 E2B (contexte 128K, edge), Qwen2.5 (codage), Llama 3.2 (utilisation générale). Comparaison complète pour RAM faible.',
        datePublished: '2026-04-04',
        dateModified: '2026-09-02',
      ...
      itemListSchema: {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Gemma 4 E2B', description: 'MMLU Pro 60 %, LiveCodeBench v6 44 %, GPQA Diamond 43,4 %. ~2 Go RAM. Contexte 128K. Apache 2.0. ollama pull gemma4:e2b' },
          { '@type': 'ListItem', position: 3, name: 'Qwen2.5 3B', description: 'MMLU 62%, HumanEval 65%. 2 Go de RAM. 29 langues nativement. Appel de fonction supporté. ollama run qwen2.5:3b' },
      ...
      faqSchema: {
        ...
        mainEntity: [
          {
            '@type': 'Question',
            'name': 'Quel est le plus petit LLM local qui produit une sortie utile ?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': "Le minimum pratique pour une sortie utile est un modèle 3B à quantification Q4_K_M. Les modèles sous 2B paramètres (Llama 3.2 1B, Gemma 4 E2B) produisent des phrases cohérentes mais peinent avec les instructions multi-étapes, les réponses plus longues et le raisonnement complexe. Pour les tâches comme la synthèse et le Q&R simple, Gemma 4 E2B est utilisable. Pour tout ce qui est plus complexe, commencez par un modèle 3B.",
            },
          },
          {
            '@type': 'Question',
            'name': 'Puis-je exécuter un modèle 3B sur un téléphone ?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': "Oui -- Llama 3.2 1B et 3B sont spécifiquement conçus pour le déploiement mobile sur appareil. Meta fournit des builds optimisés pour iOS (via MLC LLM) et Android. L'inférence sur un téléphone moderne (Snapdragon 8 Gen 3 ou Apple A17 Pro) produit 15-30 tok/sec pour les modèles 1B. LM Studio et Ollama ne fonctionnent pas actuellement sur iOS ou Android -- le mobile nécessite des frameworks séparés.",
            },
          },
          {
            '@type': 'Question',
            'name': 'Les petits modèles sont-ils bons pour la synthèse ?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': "Oui -- la synthèse est l'un des cas d'usage les plus forts pour les petits modèles. Gemma 4 E2B et Llama 3.2 3B produisent régulièrement des résumés précis de textes jusqu'à ~4 000 mots (leur limite de contexte pratique pour une sortie de qualité). Pour les documents plus longs, utilisez un modèle avec une grande fenêtre de contexte comme Phi-4 Mini ou Llama 3.2 3B (tous deux 128K tokens).",
            },
          },
          {
            '@type': 'Question',
            'name': "Combien de fois plus rapide est un modèle 2B qu'un modèle 7B sur le même matériel ?",
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Cela dépend fortement du matériel et de la quantification -- Google rapporte ~7,6 tok/sec (décodage) pour Gemma 4 E2B sur un Raspberry Pi 5, tandis que des tests indépendants sur CPU x86 vont de 8 à 48 tok/sec. Sur GPU, l\'avantage de vitesse se réduit car le débit GPU est moins contraint par la taille du modèle. La différence de vitesse est la plus notable sur les machines CPU uniquement.',
            },
          },
          {
            '@type': 'Question',
            'name': 'Les petits modèles supportent-ils l\'appel de fonction ?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': "Certains oui. Qwen2.5 3B supporte l'appel de fonction et le mode JSON. Llama 3.2 3B a un support basique d'utilisation d'outils. Gemma 4 E2B ne supporte pas l'appel de fonction. Vérifiez la documentation du modèle avant de construire un pipeline qui dépend d'une sortie structurée.",
            },
          },
          {
            '@type': 'Question',
            'name': 'Quel petit modèle est le meilleur pour les langues autres que l\'anglais ?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': "Qwen2.5 3B supporte nativement 29 langues dont le chinois, le japonais, le coréen et l'arabe. Gemma 4 E2B et Phi-4 Mini sont principalement optimisés pour l'anglais. Pour les tâches non-anglaises à l'échelle des petits modèles, Qwen2.5 3B est le choix évident. Voir [comparaison multilingue Qwen vs Llama vs Mistral](/fr/local-llms/qwen-vs-llama-vs-mistral) pour une comparaison complète des langues.",
            },
          },
          {
            '@type': 'Question',
            'name': 'Quelle est la différence entre Phi-4 Mini et Llama 3.2 3B pour les tâches quotidiennes ?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': "Phi-4 Mini surpasse Llama 3.2 3B sur le raisonnement, les mathématiques et le codage (68% vs 58% MMLU, 70% vs 60% HumanEval) avec pratiquement la même RAM (2,5 Go chacun). Pour les tâches quotidiennes -- Q&R, synthèse, explications simples -- l'écart de qualité est notable mais pas dramatique. Llama 3.2 3B bénéficie d'un support communautaire plus large et de plus de fine-tunings disponibles. Choisissez Phi-4 Mini pour le raisonnement structuré ; Llama 3.2 3B pour le chat général et la compatibilité.",
            },
          },
          {
            '@type': 'Question',
            'name': 'Puis-je exécuter deux petits modèles simultanément ?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': "Oui, si la RAM totale le permet. Deux modèles 3B en Q4_K_M utilisent ~5 Go combinés -- faisable sur une machine de 8 Go avec un OS léger. Ollama charge un modèle à la fois par processus par défaut. Exécutez deux instances Ollama sur des ports différents (OLLAMA_HOST=:11434 et OLLAMA_HOST=:11435) pour servir deux modèles en parallèle. C'est utile pour les tests A/B des sorties.",
            },
          },
          {
            '@type': 'Question',
            'name': 'Les petits modèles fonctionnent-ils pour le RAG (génération augmentée par récupération) ?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': "Oui pour le RAG simple. Llama 3.2 3B et Phi-4 Mini peuvent répondre de manière fiable aux questions sur des fragments de documents récupérés. Pour le RAG sur de grandes bases de connaissances nécessitant un raisonnement multi-saut, les modèles 7B+ performent plus régulièrement. La fonctionnalité LocalDocs de GPT4All utilise un modèle 3B pour le Q&R de documents et fonctionne bien pour les collections de documents personnels.",
            },
          },
          {
            '@type': 'Question',
            'name': 'Phi-4 Mini est-il meilleur que Llama 3.2 3B pour le codage ?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': "Oui. Phi-4 Mini obtient 70% sur HumanEval contre 60% pour Llama 3.2 3B -- un écart significatif de 10 points à cette échelle. Pour l'assistance au codage sur des machines de 4-6 Go de RAM, Phi-4 Mini est le choix recommandé. Pour le codage multilingue (non-Python), Qwen2.5 3B à 65% HumanEval est compétitif avec Phi-4 Mini tout en supportant l'appel de fonction.",
            },
          },
          {
            '@type': 'Question',
            'name': 'Utiliser la quantification Q8_0 au lieu de Q4_K_M',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': "Q8_0 nécessite près du double de RAM par rapport à Q4_K_M pour une amélioration de qualité minime à cette échelle. Un modèle Llama 3.2 3B en Q8_0 nécessite environ 3,8 Go de RAM contre environ 2,5 Go pour Q4_K_M. Sur une machine à 4 Go, Q8_0 peut déclencher l'utilisation du swap et ralentir l'inférence de 3 à 5 fois. Utilisez toujours Q4_K_M comme réglage par défaut pour les modèles sub-4B.",
            },
          },
          {
            '@type': 'Question',
            'name': "Exécuter un modèle de base au lieu de la variante instruct",
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': "Les modèles de base (par exemple llama3.2:3b-text) sont des points de contrôle antérieurs au fine-tuning, entraînés à prédire le prochain token d'un texte. Ils ne suivent pas les instructions. Si vous demandez à un modèle de base « Combien font 2+2 ? », il peut compléter la phrase comme un quiz plutôt que de répondre « 4 ». Utilisez toujours la variante instruct : llama3.2:3b (Ollama utilise l'instruct par défaut pour les modèles nommés).",
            },
          },
          {
            '@type': 'Question',
            'name': "Attendre d'un modèle 3B la qualité d'un modèle 7B",
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': "Un modèle 3B à 68% MMLU (Phi-4 Mini) performe de façon similaire à un GPT-4o mini d'ère 2023 sur les tâches générales. Les chaînes de raisonnement complexes, l'écriture longue et la génération de code nuancé produiront une qualité nettement inférieure à celle d'un modèle 7B. Si la qualité de sortie est insuffisante, passez à un modèle 7B -- la différence de RAM est d'environ 2 Go (2,5 Go → 4,5 Go).",
            },
          },
          {
            '@type': 'Question',
            'name': 'Quel est le plus petit LLM local réellement utilisable ?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': "Environ 1 milliard de paramètres est le plancher pour une sortie cohérente, et Llama 3.2 1B est le plus petit modèle de cette liste qui mérite d'être utilisé pour un vrai travail. En dessous, les modèles minuscules se chargent et génèrent du texte sur presque tout matériel, mais le texte cesse d'être cohérent au bout de quelques phrases -- mieux vaut donc les traiter comme de l'autocomplétion que comme un assistant. Si votre contrainte est le disque et la RAM plutôt que la qualité, le plus petit LLM local utilisable avec une faible empreinte est un modèle sub-2B en Q4 ; avec 4 Go ou plus disponibles, un modèle 4B représente un saut important en cohérence pour très peu de RAM supplémentaire.",
            },
          },
          {
            '@type': 'Question',
            'name': 'Quel est le plus petit modèle LLM que vous pouvez exécuter localement ?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': "Llama 3.2 1B est le plus petit modèle Llama et l'option la plus pratique de cette comparaison -- il ne nécessite que 1,3 Go de RAM et tourne à 60-90 tok/sec sur CPU, plus rapide que tout autre modèle de cette page. Il gère des tâches courtes de classification et d'extraction de mots-clés, mais peine sur des sorties plus longues et multi-étapes. Si vous cherchez un petit modèle (parfois appelé « mini LLM ») réellement utilisable au quotidien, considérez Llama 3.2 1B comme le plancher matériel, pas le plancher qualité -- passez à Gemma 4 E2B ou à un modèle 3B comme Qwen2.5 3B ou Llama 3.2 3B pour un vrai travail.",
            },
          },
        ],
      },
 ...
 snippetBlocks: [
    { type: 'one-sentence', text: "Les petits LLM locaux (1B-4B paramètres) tournent sur des machines avec 4-8 Go de RAM à 30-70 tokens/sec, Phi-4 Mini 3.8B en tête pour le raisonnement, Gemma 4 E2B idéal pour les petits appareils/edge avec un contexte de 128K, Qwen2.5 3B meilleur pour le code, et Llama 3.2 3B meilleur pour l'usage général." },
 ...
 items: ['**Meilleur raisonnement à petite échelle** : Phi-4 Mini 3.8B -- 68% MMLU, 70% HumanEval, fonctionne sur 4 Go de RAM.', '**Meilleur petit modèle/edge** : Gemma 4 E2B -- MMLU Pro 60 %, contexte 128K, ~2 Go RAM, Apache 2.0. Remplace l\'ancien Gemma 2 2B.', '**Meilleur petit modèle de codage** : Qwen2.5 3B -- 65% HumanEval avec environ 2 Go de RAM.', '**Meilleur usage général 3B** : Llama 3.2 3B -- meilleur support communautaire, contexte 128K, 2,5 Go de RAM.', "Aucun modèle sub-2B ne produit une qualité de sortie adaptée aux tâches professionnelles. Utilisez 3B ou plus pour un vrai travail."] },
        whatIsSmall: { id: 'what-is-small-llm', title: 'Qu\'est-ce qu\'un "petit" LLM local et quand devriez-vous en utiliser un ?', content: ['**Un petit LLM local est généralement défini comme un modèle avec moins de 4 milliards de paramètres.** Avec la quantification Q4_K_M, ces modèles nécessitent 1,5-3 Go de RAM -- bien dans les contraintes des ordinateurs portables d\'entrée de gamme avec 4-8 Go de mémoire totale.', "Les petits modèles conviennent pour : la synthèse rapide, le Q&R simple, l'explication de fragments de code, la traduction de textes courts et les tâches de classification. Ils ne conviennent pas au raisonnement multi-étapes, à la génération de code complexe ou à la rédaction de documents longs et cohérents.", "L'écart de qualité entre un modèle 3B et 7B est significatif -- environ l'équivalent de l'écart entre GPT-4o mini et GPT-5.5. Pour les utilisateurs avec 8 Go de RAM, un modèle 7B en Q4_K_M est presque toujours le meilleur choix si la machine en a les capacités. Voir [Meilleurs modèles LLM locaux pour débutants](/local-llms/best-beginner-local-llm-models) pour les recommandations 7B."] },
        modelSelectionGuide: { id: 'model-selection-guide', title: 'Quel modèle devriez-vous utiliser ? Guide de décision rapide', image: '/images/small-llm-decision-tree-mockup.svg', imageCaption: "Arbre de décision : choisir par priorité (raisonnement, vitesse ou codage). Par défaut Llama 3.2 3B si vous n'êtes pas sûr." },
        phi4mini: { id: 'phi-4-mini', title: 'Phi-4 Mini 3.8B -- Meilleures performances de raisonnement dans la classe sub-4B', content: ["**Microsoft Phi-4 Mini atteint 68% sur MMLU et 70% sur HumanEval -- des scores qui dépassent beaucoup de modèles 7B publiés avant 2025.** C'est possible parce que Phi-4 Mini a été entraîné sur un ensemble de données synthétiques sélectionné, axé sur le raisonnement et la résolution de problèmes, plutôt que sur du texte web généraliste.", "Phi-4 Mini est le choix recommandé pour les utilisateurs qui ont principalement besoin de raisonnement (mathématiques, logique, explications pas à pas) ou d'assistance au codage sur du matériel avec 4-6 Go de RAM."], rows: [{ 'Spec': 'MMLU', 'Value': '68%' }, { 'Spec': 'HumanEval', 'Value': '70%' }, { 'Spec': 'RAM (Q4_K_M)', 'Value': '~2,5 Go' }, { 'Spec': 'Contexte', 'Value': '128K tokens' }, { 'Spec': 'Vitesse CPU', 'Value': '30-50 tok/sec' }, { 'Spec': 'Commande Ollama', 'Value': 'ollama run phi4-mini' }], columns: ['Spec', 'Value'] },
        gemma2: { id: 'gemma-4-e2b', title: 'Gemma 4 E2B -- Meilleur modèle petit/edge', content: ["**Google Gemma 4 E2B est la génération actuelle de petit modèle, remplaçant l'ancien Gemma 2 2B.** Il obtient 60 % sur MMLU Pro (un benchmark plus exigeant que le MMLU original), 44 % sur LiveCodeBench v6 et 43,4 % sur GPQA Diamond, selon la fiche modèle officielle de Google. Il a 2,3 Mrd de paramètres effectifs (5,1 Mrd embeddings inclus) et tient dans ~2 Go de RAM.", "La fenêtre de contexte de 128K est une amélioration majeure par rapport aux 8K de l'ancien Gemma 2 2B -- elle égale désormais Phi-4 Mini et Llama 3.2 3B. La vitesse CPU varie selon le matériel et la quantification (Google rapporte ~7,6 tok/sec en décodage sur un Raspberry Pi 5 ; des rapports indépendants sur CPU x86 vont de 8 à 48 tok/sec) -- vérifiez les chiffres actuels pour votre appareil. Licence Apache 2.0."], rows: [{ 'Spec': 'MMLU Pro', 'Value': '60 %' }, { 'Spec': 'LiveCodeBench v6', 'Value': '44 %' }, { 'Spec': 'GPQA Diamond', 'Value': '43,4 %' }, { 'Spec': 'RAM (Q4_K_M)', 'Value': '~2 Go' }, { 'Spec': 'Contexte', 'Value': '128K tokens' }, { 'Spec': 'Commande Ollama', 'Value': 'ollama pull gemma4:e2b' }], columns: ['Spec', 'Value'] },
        qwen25_3b: { id: 'qwen2-5-3b', title: 'Qwen2.5 3B -- Meilleur petit modèle pour les tâches de codage', content: ["**Qwen2.5 3B obtient 65% sur HumanEval -- 5 points de pourcentage au-dessus de Llama 3.2 3B -- ce qui en fait le meilleur choix pour les tâches de codage à l'échelle 3B.** Il inclut le mode JSON et le support d'appel de fonction, et gère nativement 29 langues.", "Pour les tâches non-codage en anglais, Llama 3.2 3B et Phi-4 Mini produisent une prose plus naturelle. Choisissez Qwen2.5 3B spécifiquement quand le codage ou la sortie multilingue est le cas d'usage principal."], rows: [{ 'Spec': 'MMLU', 'Value': '62%' }, { 'Spec': 'HumanEval', 'Value': '65%' }, { 'Spec': 'RAM (Q4_K_M)', 'Value': '~2 Go' }, { 'Spec': 'Contexte', 'Value': '128K tokens' }, { 'Spec': 'Vitesse CPU', 'Value': '25-40 tok/sec' }, { 'Spec': 'Commande Ollama', 'Value': 'ollama run qwen2.5:3b' }], columns: ['Spec', 'Value'] },
        llama32_3b: { id: 'llama-3-2-3b', title: 'Llama 3.2 3B -- Meilleur petit modèle polyvalent', content: ["**Meta Llama 3.2 3B est le modèle 3B le plus documenté et le mieux supporté par la communauté.** Il obtient 58% sur MMLU et 60% sur HumanEval -- légèrement en dessous de Phi-4 Mini -- mais possède le support d'outils le plus large, le plus grand nombre de fine-tunings disponibles et la plus grande collection de guides communautaires.", "La fenêtre de contexte 128K est la même que les modèles Llama 3.x plus grands, ce qui le rend adapté à la synthèse de documents de longueur moyenne. Pour un premier petit modèle, Llama 3.2 3B reste le choix le plus sûr en raison de son comportement prévisible et de sa documentation exhaustive."], rows: [{ 'Spec': 'MMLU', 'Value': '58%' }, { 'Spec': 'RAM (Q4_K_M)', 'Value': '~2,5 Go' }, { 'Spec': 'Contexte', 'Value': '128K tokens' }, { 'Spec': 'Vitesse CPU', 'Value': '25-45 tok/sec' }, { 'Spec': 'Commande Ollama', 'Value': 'ollama run llama3.2:3b' }], columns: ['Spec', 'Value'] },
        llama32_1b: { id: 'llama-3-2-1b', title: 'Llama 3.2 1B -- Minimum absolu pour une sortie utile', content: "Llama 3.2 1B est le plus petit modèle Llama publié par Meta : il nécessite seulement 1,3 Go de RAM et génère 60-90 tok/sec sur CPU -- le modèle exécutable localement le plus rapide de cette page. La qualité de sortie est marginale : il gère la classification très simple et l'extraction de mots-clés, mais peine avec des réponses multi-phrases cohérentes. Utilisez Llama 3.2 1B uniquement quand la RAM est véritablement la contrainte principale (moins de 3 Go disponibles) ou pour tester les intégrations d'outils." },
        comparisonTable: { id: 'comparison-table', title: 'Comparaison complète : Meilleurs petits LLMs locaux sous 4B paramètres', image: '/images/small-local-llm-models-performance-tier-hero-fr.webp', imageCaption: 'Niveaux de performance : Phi-4 Mini domine le raisonnement et le codage, Gemma 4 E2B domine la longueur de contexte (128K) avec l\'empreinte RAM la plus réduite, Qwen2.5 excelle en codage.', rows: [{ 'Modèle': 'Phi-4 Mini 3.8B', 'MMLU': '68%', 'HumanEval': '70%', 'RAM': '2,5 Go', 'Contexte': '128K', 'Meilleur pour': 'Raisonnement, codage' }, { 'Modèle': 'Qwen2.5 3B', 'MMLU': '62%', 'HumanEval': '65%', 'RAM': '2 Go', 'Contexte': '128K', 'Meilleur pour': 'Codage, multilingue' }, { 'Modèle': 'Llama 3.2 3B', 'MMLU': '58%', 'HumanEval': '60%', 'RAM': '2,5 Go', 'Contexte': '128K', 'Meilleur pour': 'Usage général, premier modèle' }, { 'Modèle': 'Gemma 4 E2B', 'MMLU': '—', 'HumanEval': '—', 'RAM': '2 Go', 'Contexte': '128K', 'Meilleur pour': 'Edge (MMLU Pro 60 %)' }, { 'Modèle': 'Llama 3.2 1B', 'MMLU': '32%', 'HumanEval': '28%', 'RAM': '1,3 Go', 'Contexte': '128K', 'Meilleur pour': 'RAM minimum absolu' }], columns: ['Modèle', 'MMLU', 'HumanEval', 'RAM', 'Contexte', 'Meilleur pour'] },
        quantizationGuide: { id: 'quantization-guide', title: 'Comprendre la quantification : compromis RAM vs qualité', image: '/images/small-local-llm-models-quantization-tradeoff-hero-fr.webp', imageCaption: 'Compromis de quantification : Q4_K_M (2,5 Go, -0,5% qualité) est le défaut recommandé. Q8_0 utilise 3,8 Go sans gain de qualité. Q3_K_M (1,8 Go, -1,8% perte) pour les contraintes RAM extrêmes.' },
        regionalContext: { id: 'regional-context', title: 'Petits LLMs locaux par région', content: ["**UE / RGPD :** Pour les professionnels européens sur du matériel contraint -- travail terrain, environnements air-gap, anciens ordinateurs portables d'entreprise -- les petits modèles locaux fournissent une inférence conforme RGPD sans sortie de données. Un Phi-4 Mini 3.8B fonctionnant sur un portable corporate standard (8 Go de RAM) maintient tous les textes traités sur l'appareil conformément à l'article 5 du RGPD (minimisation des données). Pour la documentation de conformité BSI : Phi-4 Mini (Microsoft, licence MIT) et Llama 3.2 3B (Meta, licence communautaire Llama) fournissent des identifiants de modèles versionnés via leurs tags Ollama. Mistral n'offre actuellement pas de modèle sub-4B. Pour les organisations préférant un modèle d'origine européenne, les options sont limitées jusqu'à ce que Mistral publie une variante sub-4B.", "**Japon (METI) :** Pour les tâches en langue japonaise au niveau des petits modèles, Qwen2.5 3B est le seul modèle de cette comparaison avec une tokenisation japonaise native. Llama 3.2 3B gère le japonais mais avec une efficacité de tokenisation inférieure. Pour la synthèse ou la traduction japonaise avec RAM contraint : `ollama run qwen2.5:3b`. La vitesse de 25-40 tok/sec sur CPU fournit une réponse en temps réel adéquate pour les interfaces de chat sur du matériel de bureau standard.", "**Chine :** Qwen2.5 3B (Alibaba, Apache 2.0) est le choix naturel pour le déploiement de petits modèles en langue chinoise. La tokenisation chinoise native traite le texte mandarin 30-40% plus efficacement que Llama pour un nombre de paramètres équivalent. Pour les déploiements IoT et edge sous la loi chinoise sur la sécurité des données (数据安全法) : `ollama run qwen2.5:3b` fonctionne sur n'importe quel appareil Linux avec 4 Go de RAM et traite tout le texte sur l'appareil sans appels API externes."] },
        commonMistakes: { id: 'common-mistakes', title: 'Quelles sont les erreurs courantes lors de l\'exécution de petits LLMs locaux ?', items: ["**Utiliser la quantification Q8_0 au lieu de Q4_K_M :** Q8_0 nécessite presque le double de RAM de Q4_K_M pour une amélioration de qualité minimale à petite échelle. Un modèle Llama 3.2 3B en Q8_0 nécessite ~3,8 Go de RAM contre ~2,5 Go pour Q4_K_M. Sur une machine de 4 Go, Q8_0 peut déclencher l'utilisation du swap et ralentir l'inférence de 3-5×. Utilisez toujours Q4_K_M comme défaut pour les modèles sub-4B.", "**Exécuter un modèle de base au lieu de la variante instruct :** Les modèles de base (ex. `llama3.2:3b-text`) sont des points de contrôle pré-fine-tuning entraînés à prédire le prochain token dans le texte. Ils ne suivent pas les instructions. Quand vous demandez à un modèle de base «Combien font 2+2 ?», il peut compléter la phrase comme un quiz plutôt que répondre «4». Utilisez toujours la variante instruct : `llama3.2:3b` (Ollama utilise instruct par défaut pour les modèles nommés).", "**S'attendre à une qualité de modèle 7B d'un modèle 3B :** Un modèle 3B à 68% MMLU (Phi-4 Mini) performe comme un GPT-4o mini de l'ère 2023 sur les tâches générales. Les chaînes de raisonnement complexes, la rédaction longue et la génération de code nuancé produiront une qualité nettement inférieure à un modèle 7B. Si la qualité de sortie est insuffisante, passez à un modèle 7B -- la différence de RAM est ~2 Go (2,5 Go → 4,5 Go)."] },
        relatedReading: { id: 'related-reading', title: 'Lectures complémentaires', items: ['[Silicium IA embarquee pour LLM locaux 2026](/fr/power-local-llm/edge-ai-hardware-for-local-llms) -- Ce qui tient vraiment dans un produit industrialise : budget de puissance, conception thermique, joules par token.', '[Meilleurs modèles LLM locaux pour débutants](/fr/local-llms/best-beginner-local-llm-models) -- recommandations 7B quand 8 Go de RAM sont disponibles et que la qualité doit dépasser les 3B', '[Exécuter votre premier LLM local](/fr/local-llms/run-first-local-llm) -- guide étape par étape pour télécharger et exécuter n\'importe quel modèle de cette page avec Ollama en moins de 10 minutes', '[LLM local sur ordinateur portable](/fr/local-llms/local-llm-on-laptop) -- gestion thermique et batterie pour une inférence soutenue sur matériel contraint', '[Quantification LLM expliquée](/fr/local-llms/llm-quantization-explained) -- pourquoi Q4_K_M est le défaut et quand considérer Q3_K_M pour les contraintes RAM extrêmes', '[Meilleurs LLMs locaux pour le codage](/fr/local-llms/best-local-llms-for-coding) -- modèles spécifiques au codage à 7B+ quand Qwen2.5 3B ne suffit pas pour les tâches complexes', '[Dépannage de la configuration LLM local](/fr/local-llms/troubleshooting-local-llm-setup) -- corriger les erreurs OOM, l\'inférence lente et les échecs de chargement courants sur les machines à faible RAM', '[Meilleurs modèles 7B pour les GPU grand public](/fr/local-llms/best-7b-models-consumer-hardware) -- meilleurs modèles 7B pour hardware grand public'] },
        faqSection: { id: 'faq', title: 'Questions fréquentes sur les petits modèles LLM locaux', faqs: [
            { q: 'Quel est le plus petit LLM local qui produit une sortie utile ?', a: "Le minimum pratique pour une sortie utile est un modèle 3B à quantification Q4_K_M. Les modèles sous 2B paramètres (Llama 3.2 1B, Gemma 4 E2B) produisent des phrases cohérentes mais peinent avec les instructions multi-étapes, les réponses plus longues et le raisonnement complexe. Pour les tâches comme la synthèse et le Q&R simple, Gemma 4 E2B est utilisable. Pour tout ce qui est plus complexe, commencez par un modèle 3B." },
            { q: 'Puis-je exécuter un modèle 3B sur un téléphone ?', a: "Oui -- Llama 3.2 1B et 3B sont spécifiquement conçus pour le déploiement mobile sur appareil. Meta fournit des builds optimisés pour iOS (via MLC LLM) et Android. L'inférence sur un téléphone moderne (Snapdragon 8 Gen 3 ou Apple A17 Pro) produit 15-30 tok/sec pour les modèles 1B. LM Studio et Ollama ne fonctionnent pas actuellement sur iOS ou Android -- le mobile nécessite des frameworks séparés." },
            { q: 'Les petits modèles sont-ils bons pour la synthèse ?', a: "Oui -- la synthèse est l'un des cas d'usage les plus forts pour les petits modèles. Gemma 4 E2B et Llama 3.2 3B produisent régulièrement des résumés précis de textes jusqu'à ~4 000 mots (leur limite de contexte pratique pour une sortie de qualité). Pour les documents plus longs, utilisez un modèle avec une grande fenêtre de contexte comme Phi-4 Mini ou Llama 3.2 3B (tous deux 128K tokens)." },
            { q: "Combien de fois plus rapide est un modèle 2B qu'un modèle 7B sur le même matériel ?", a: 'Cela dépend fortement du matériel et de la quantification -- Google rapporte ~7,6 tok/sec (décodage) pour Gemma 4 E2B sur un Raspberry Pi 5, tandis que des tests indépendants sur CPU x86 vont de 8 à 48 tok/sec. Sur GPU, l\'avantage de vitesse se réduit car le débit GPU est moins contraint par la taille du modèle. La différence de vitesse est la plus notable sur les machines CPU uniquement.' },
            { q: 'Les petits modèles supportent-ils l\'appel de fonction ?', a: "Certains oui. Qwen2.5 3B supporte l'appel de fonction et le mode JSON. Llama 3.2 3B a un support basique d'utilisation d'outils. Gemma 4 E2B ne supporte pas l'appel de fonction. Vérifiez la documentation du modèle avant de construire un pipeline qui dépend d'une sortie structurée." },
            { q: 'Quel petit modèle est le meilleur pour les langues autres que l\'anglais ?', a: "Qwen2.5 3B supporte nativement 29 langues dont le chinois, le japonais, le coréen et l'arabe. Gemma 4 E2B et Phi-4 Mini sont principalement optimisés pour l'anglais. Pour les tâches non-anglaises à l'échelle des petits modèles, Qwen2.5 3B est le choix évident. Voir [comparaison multilingue Qwen vs Llama vs Mistral](/fr/local-llms/qwen-vs-llama-vs-mistral) pour une comparaison complète des langues." },
            { q: 'Quelle est la différence entre Phi-4 Mini et Llama 3.2 3B pour les tâches quotidiennes ?', a: "Phi-4 Mini surpasse Llama 3.2 3B sur le raisonnement, les mathématiques et le codage (68% vs 58% MMLU, 70% vs 60% HumanEval) avec pratiquement la même RAM (2,5 Go chacun). Pour les tâches quotidiennes -- Q&R, synthèse, explications simples -- l'écart de qualité est notable mais pas dramatique. Llama 3.2 3B bénéficie d'un support communautaire plus large et de plus de fine-tunings disponibles. Choisissez Phi-4 Mini pour le raisonnement structuré ; Llama 3.2 3B pour le chat général et la compatibilité." },
            { q: 'Puis-je exécuter deux petits modèles simultanément ?', a: "Oui, si la RAM totale le permet. Deux modèles 3B en Q4_K_M utilisent ~5 Go combinés -- faisable sur une machine de 8 Go avec un OS léger. Ollama charge un modèle à la fois par processus par défaut. Exécutez deux instances Ollama sur des ports différents (OLLAMA_HOST=:11434 et OLLAMA_HOST=:11435) pour servir deux modèles en parallèle. C'est utile pour les tests A/B des sorties." },
            { q: 'Les petits modèles fonctionnent-ils pour le RAG (génération augmentée par récupération) ?', a: "Oui pour le RAG simple. Llama 3.2 3B et Phi-4 Mini peuvent répondre de manière fiable aux questions sur des fragments de documents récupérés. Pour le RAG sur de grandes bases de connaissances nécessitant un raisonnement multi-saut, les modèles 7B+ performent plus régulièrement. La fonctionnalité LocalDocs de GPT4All utilise un modèle 3B pour le Q&R de documents et fonctionne bien pour les collections de documents personnels." },
            { q: 'Phi-4 Mini est-il meilleur que Llama 3.2 3B pour le codage ?', a: "Oui. Phi-4 Mini obtient 70% sur HumanEval contre 60% pour Llama 3.2 3B -- un écart significatif de 10 points à cette échelle. Pour l'assistance au codage sur des machines de 4-6 Go de RAM, Phi-4 Mini est le choix recommandé. Pour le codage multilingue (non-Python), Qwen2.5 3B à 65% HumanEval est compétitif avec Phi-4 Mini tout en supportant l'appel de fonction." },
            { q: 'Utiliser la quantification Q8_0 au lieu de Q4_K_M', a: "Q8_0 nécessite près du double de RAM par rapport à Q4_K_M pour une amélioration de qualité minime à cette échelle. Un modèle Llama 3.2 3B en Q8_0 nécessite environ 3,8 Go de RAM contre environ 2,5 Go pour Q4_K_M. Sur une machine à 4 Go, Q8_0 peut déclencher l'utilisation du swap et ralentir l'inférence de 3 à 5 fois. Utilisez toujours Q4_K_M comme réglage par défaut pour les modèles sub-4B." },
            { q: "Exécuter un modèle de base au lieu de la variante instruct", a: "Les modèles de base (par exemple llama3.2:3b-text) sont des points de contrôle antérieurs au fine-tuning, entraînés à prédire le prochain token d'un texte. Ils ne suivent pas les instructions. Si vous demandez à un modèle de base « Combien font 2+2 ? », il peut compléter la phrase comme un quiz plutôt que de répondre « 4 ». Utilisez toujours la variante instruct : llama3.2:3b (Ollama utilise l'instruct par défaut pour les modèles nommés)." },
            { q: "Attendre d'un modèle 3B la qualité d'un modèle 7B", a: "Un modèle 3B à 68% MMLU (Phi-4 Mini) performe de façon similaire à un GPT-4o mini d'ère 2023 sur les tâches générales. Les chaînes de raisonnement complexes, l'écriture longue et la génération de code nuancé produiront une qualité nettement inférieure à celle d'un modèle 7B. Si la qualité de sortie est insuffisante, passez à un modèle 7B -- la différence de RAM est d'environ 2 Go (2,5 Go → 4,5 Go)." },
            { q: 'Quel est le plus petit LLM local réellement utilisable ?', a: "Environ 1 milliard de paramètres est le plancher pour une sortie cohérente, et Llama 3.2 1B est le plus petit modèle de cette liste qui mérite d'être utilisé pour un vrai travail. En dessous, les modèles minuscules se chargent et génèrent du texte sur presque tout matériel, mais le texte cesse d'être cohérent au bout de quelques phrases -- mieux vaut donc les traiter comme de l'autocomplétion que comme un assistant. Si votre contrainte est le disque et la RAM plutôt que la qualité, le plus petit LLM local utilisable avec une faible empreinte est un modèle sub-2B en Q4 ; avec 4 Go ou plus disponibles, un modèle 4B représente un saut important en cohérence pour très peu de RAM supplémentaire." },
            { q: 'Quel est le plus petit modèle LLM que vous pouvez exécuter localement ?', a: "Llama 3.2 1B est le plus petit modèle Llama et l'option la plus pratique de cette comparaison -- il ne nécessite que 1,3 Go de RAM et tourne à 60-90 tok/sec sur CPU, plus rapide que tout autre modèle de cette page. Il gère des tâches courtes de classification et d'extraction de mots-clés, mais peine sur des sorties plus longues et multi-étapes. Si vous cherchez un petit modèle (parfois appelé « mini LLM ») réellement utilisable au quotidien, considérez Llama 3.2 1B comme le plancher matériel, pas le plancher qualité -- passez à Gemma 4 E2B ou à un modèle 3B comme Qwen2.5 3B ou Llama 3.2 3B pour un vrai travail." },
          ] },
        sources: { id: 'sources', title: 'Sources', items: ['Hugging Face Open LLM Leaderboard -- open-llm-leaderboard.hf.space (scores MMLU et HumanEval)', 'Microsoft Phi-4 Technical Report -- microsoft.com/en-us/research/publication/phi-4-technical-report/', 'Meta Llama 3.2 Model Card -- huggingface.co/meta-llama/Llama-3.2-3B-Instruct', 'Google Gemma 4 Model Card -- huggingface.co/google/gemma-4-e2b-it'] },
      },
    },
    ja: {
      theme: 'Best Models',
      ...
      metaDescription: '4～8GB RAM向け最高5つの小型ローカルLLMモデル2026年完全比較：Phi-4 Mini（68% MMLU）、Gemma 4 E2B（MMLU Pro 60%、128Kコンテキスト）、Qwen2.5（コーディング）、Llama 3.2。',
      heroImage: '/images/small-local-llm-models-performance-tier-hero-ja.webp',
      intro: '小規模ローカルLLM（1B～4Bパラメータ）は4～8GB RAMのマシンで動作し、CPU上で30～70トークン/秒を生成します。リアルタイムチャットに十分な速度です。2026年の最高の小規模モデルはMicrosoft Phi-4 Mini 3.8B（最高の推論）、Google Gemma 4 E2B（最速）、Qwen2.5 3B（最高のコーディング）、Meta Llama 3.2 3B（最高の汎用）です。',
      publishDate: '2026-04-04',
      freshness_tier: 'semi_annual',
      next_refresh_due: '2027-03-02',
      leadAnswerBlock: '**小規模なローカルLLM（1B〜4Bパラメータ）は4〜8 GBのRAMを積んだマシンで動作し、CPUだけで毎秒30〜70トークンを生成します。リアルタイムのチャットに十分な速度です。**',
      readTime: '8分',
      educationalLevel: 'Beginner',
      toc: [
        { label: '重要ポイント', anchor: '#key-takeaways' },
        { label: '「小規模」ローカルLLMとは？', anchor: '#what-is-small-llm' },
        { label: 'どのモデルを使うべきか？', anchor: '#model-selection-guide' },
        { label: '#1 Phi-4 Mini 3.8B -- 最高の推論', anchor: '#phi-4-mini' },
        { label: '#2 Gemma 4 E2B -- 最良の小型/エッジモデル', anchor: '#gemma-4-e2b' },
        { label: '#3 Qwen2.5 3B -- 最高のコーディング', anchor: '#qwen2-5-3b' },
        { label: '#4 Llama 3.2 3B -- 最高の汎用モデル', anchor: '#llama-3-2-3b' },
        { label: '#5 Llama 3.2 1B -- 絶対的な最小構成', anchor: '#llama-3-2-1b' },
        { label: '完全な比較表', anchor: '#comparison-table' },
        { label: '量子化ガイド', anchor: '#quantization-guide' },
...
schema: {
        ...
        description: '最高の小規模ローカルLLM 2026：Phi-4 Mini（推論）、Gemma 4 E2B（128Kコンテキスト、エッジ）、Qwen2.5（コーディング）、Llama 3.2（一般用途）。RAM制限されたマシン向けの完全な比較。',
        datePublished: '2026-04-04',
        dateModified: '2026-09-02',
      ...
      itemListSchema: {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Gemma 4 E2B', description: 'MMLU Pro 60%、LiveCodeBench v6 44%、GPQA Diamond 43.4%。~2 GB RAM。128Kコンテキスト。Apache 2.0。ollama pull gemma4:e2b' },
          { '@type': 'ListItem', position: 3, name: 'Qwen2.5 3B', description: 'MMLU 62%、HumanEval 65%。2 GB RAM。29言語をネイティブサポート。関数呼び出し対応。ollama run qwen2.5:3b' },
      ...
      faqSchema: {
        ...
        '@type': 'FAQPage',
        mainEntity: [
          { '@type': 'Question', name: '有用な出力を生成する最小のローカルLLMは何ですか？', acceptedAnswer: { '@type': 'Answer', text: '有用な出力の実用的な最小値はQ4_K_M量子化での3Bモデルです。2B以下のパラメータを持つモデル（Llama 3.2 1B、Gemma 4 E2B）は一貫した単一文を生成しますが、複数ステップの指示と複雑な推論に苦労しています。要約やシンプルなQ&Aなどのタスクの場合、Gemma 4 E2Bは使用可能です。より複雑な場合は、3Bモデルから始めてください。' } },
          { '@type': 'Question', name: '3Bモデルを電話で実行できますか？', acceptedAnswer: { '@type': 'Answer', text: 'はい -- Llama 3.2 1Bと3Bは、オンデバイスのモバイル展開用に設計されています。Metaは、iOS（MLC LLM経由）およびAndroid向けの最適化されたビルドを提供します。モダンフォン（Snapdragon 8 Gen 3またはApple A17 Pro）での推論は、1Bモデルで15～30トークン/秒を生成します。LM StudioとOllamaは現在iOSまたはAndroidで動作しません。モバイルには別のフレームワークが必要です。' } },
          { '@type': 'Question', name: '小規模モデルは要約に適していますか？', acceptedAnswer: { '@type': 'Answer', text: 'はい -- 要約は小規模モデルの最強のユースケースの1つです。Gemma 4 E2BとLlama 3.2 3Bは、~4,000語までのテキスト（品質出力の実用的なコンテキスト制限）の正確な要約を確実に生成します。より長い文書の場合は、Phi-4 MiniまたはLlama 3.2 3B（両方で128Kトークン）のような大きなコンテキストウィンドウを持つモデルを使用してください。' } },
          { '@type': 'Question', name: '2Bモデルは同じハードウェアで7Bモデルより何倍速いですか？', acceptedAnswer: { '@type': 'Answer', text: 'ハードウェアと量子化に大きく依存します -- GoogleはRaspberry Pi 5でのGemma 4 E2Bのデコード速度を約7.6トークン/秒と報告しており、独立系のx86 CPUテストでは8～48トークン/秒の幅があります。GPUではスピード利点は狭くなります。スピード差はCPUのみのマシンで最も顕著です。' } },
          { '@type': 'Question', name: '小規模モデルは関数呼び出しをサポートしていますか？', acceptedAnswer: { '@type': 'Answer', text: 'いくつかサポートしています。Qwen2.5 3Bは関数呼び出しとJSONモードをサポートしています。Llama 3.2 3Bは基本的なツール使用をサポートしています。Gemma 4 E2Bは関数呼び出しをサポートしていません。構造化された出力に依存するパイプラインを構築する前に、モデルのドキュメントを確認してください。' } },
          { '@type': 'Question', name: '英語以外の言語に最適な小規模モデルはどれですか？', acceptedAnswer: { '@type': 'Answer', text: 'Qwen2.5 3Bは、中国語、日本語、韓国語、アラビア語を含む29言語をネイティブにサポートしています。Gemma 4 E2BとPhi-4 Miniは主に英語最適化されています。小規模モデルスケールでの非英語タスクの場合、Qwen2.5 3Bが明確な選択肢です。[Qwen vs Llama vs Mistral 多言語比較](/local-llms/qwen-vs-llama-vs-mistral)で完全な言語比較を参照してください。' } },
          { '@type': 'Question', name: '日常的なタスクでのPhi-4 MiniとLlama 3.2 3Bの違いは何ですか？', acceptedAnswer: { '@type': 'Answer', text: 'Phi-4 Miniはほぼ同じRAM（各2.5 GB）で推論、数学、コーディングでLlama 3.2 3Bを上回ります（68%対58% MMLU、70%対60% HumanEval）。日常的なタスク -- Q&A、要約、シンプルな説明 -- では、品質ギャップは顕著ですが劇的ではありません。Llama 3.2 3Bはより広いコミュニティサポートとより多くのファインチューンが利用可能です。構造化された推論にはPhi-4 Miniを選択し、汎用チャットと互換性にはLlama 3.2 3Bを選択してください。' } },
          { '@type': 'Question', name: '2つの小規模モデルを同時に実行できますか？', acceptedAnswer: { '@type': 'Answer', text: 'はい、合計RAMが許可する場合。Q4_K_Mで2つの3Bモデルは~5 GB組み合わされて使用します。リーンなOSを備えた8 GBマシンでは実行可能です。Ollamaはデフォルトでプロセスごとに一度に1つのモデルをロードします。異なるポート（OLLAMA_HOST=:11434とOLLAMA_HOST=:11435）で2つのOllamaインスタンスを実行して、2つのモデルを並行して提供してください。これは出力をA/Bテストするのに役立ちます。' } },
          { '@type': 'Question', name: '小規模モデルはRAG（検索拡張生成）に機能しますか？', acceptedAnswer: { '@type': 'Answer', text: 'シンプルなRAGの場合ははい。Llama 3.2 3BとPhi-4 Miniは、取得した文書チャンク上の質問に確実に答えることができます。複数ホップの推論が必要な大規模な知識ベース上のRAGの場合、7B+モデルはより一貫してパフォーマンスします。GPT4AllのLocalDocsフィーチャーはドキュメントQ&Aに3Bモデルを使用し、個人ドキュメント集合に対してうまく動作します。' } },
          { '@type': 'Question', name: 'Phi-4 MiniはコーディングでLlama 3.2 3Bより優れていますか？', acceptedAnswer: { '@type': 'Answer', text: 'はい。Phi-4 Miniはこのスケールで意味のある10ポイントギャップでLlama 3.2 3Bの60%対70% HumanEvalスコアを達成します。4～6 GB RAMマシンでのコーディング支援の場合、Phi-4 Miniが推奨される選択肢です。多言語コーディング（非Python）の場合、65% HumanEvalのQwen2.5 3Bはphi-4 Miniと競争力があり、関数呼び出しもサポートしています。' } },
          { '@type': 'Question', name: 'Q4_K_Mの代わりにQ8_0量子化を使用する場合', acceptedAnswer: { '@type': 'Answer', text: 'Q8_0は、この規模では品質向上がわずかであるにもかかわらず、Q4_K_Mのほぼ2倍のRAMを必要とします。Q8_0のLlama 3.2 3BモデルはQ4_K_Mの約2.5 GBに対して約3.8 GBのRAMが必要です。4 GBのマシンでは、Q8_0がスワップ使用を引き起こし、推論を3～5倍遅くする可能性があります。Sub-4Bモデルには常にQ4_K_Mをデフォルトとして使用してください。' } },
          { '@type': 'Question', name: 'instructバリアントの代わりにベースモデルを実行する場合', acceptedAnswer: { '@type': 'Answer', text: 'ベースモデル（例：llama3.2:3b-text）はファインチューニング前のチェックポイントで、テキスト中の次のトークンを予測するように学習されています。指示には従いません。ベースモデルに「2+2は何ですか？」と尋ねると、「4」と答える代わりに、クイズのように文を完成させることがあります。常にinstructバリアントを使用してください：llama3.2:3b（Ollamaは名前付きモデルに対してデフォルトでinstructを使用します）。' } },
          { '@type': 'Question', name: '3Bモデルに7Bモデル相当の品質を期待する場合', acceptedAnswer: { '@type': 'Answer', text: 'MMLU 68%の3Bモデル（Phi-4 Mini）は、一般的なタスクにおいて2023年頃のGPT-3.5 Miniと同程度の性能です。複雑な推論の連鎖、長文の執筆、繊細なコード生成では、7Bモデルよりも明らかに品質が低くなります。出力品質が不十分な場合は、7Bモデルにアップグレードしてください -- RAMの差は約2 GB（2.5 GB → 4.5 GB）です。' } },
          { '@type': 'Question', name: '実際に使える最小のローカルLLMは何ですか？', acceptedAnswer: { '@type': 'Answer', text: '一貫した出力の下限は約1Bパラメータで、実務で動かす価値があるこのリストの中で最も小さいモデルはLlama 3.2 1Bです。それより小さいモデルはほぼどんなハードウェアでもロードして生成できますが、数文でテキストの一貫性が崩れるため、アシスタントというよりオートコンプリートとして扱う方が適しています。品質よりディスクとRAMの制約が優先される場合、フットプリントが小さく実用的な最小のローカルLLMはQ4量子化のsub-2Bモデルです。4GB以上使える場合は、4Bモデルがわずかな追加RAMで一貫性を大きく向上させます。' } },
          { '@type': 'Question', name: 'ローカルで実行できる最小のLLMモデルは何ですか？', acceptedAnswer: { '@type': 'Answer', text: 'Llama 3.2 1BはMetaが公開する最小のLlamaモデルであり、この比較の中で最も実用的な選択肢です。必要なRAMはわずか1.3 GBで、CPUで60～90トークン/秒動作します -- このページで最も高速なモデルです。短い分類やキーワード抽出タスクは処理できますが、長く複数ステップの出力には対応できません。日常的に実際に使える小型モデル（「ミニLLM」と呼ばれることもあります）をお探しなら、Llama 3.2 1Bはハードウェアの下限であって品質の下限ではないと考えてください -- 実務にはGemma 4 E2Bや、Qwen2.5 3BやLlama 3.2 3Bのような3Bモデルにステップアップしてください。' } },
        ],
      },
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: '小型ローカルLLM（1B〜4Bパラメータ）は4〜8GBのRAMを搭載したマシンで秒間30〜70トークンで動作し、推論ではPhi-4 Mini 3.8Bが優れ、128Kコンテキストを持つ小型・エッジ用途にはGemma 4 E2B、コーディングにはQwen2.5 3B、汎用にはLlama 3.2 3Bが最適です。' },
        ...
        modelSelectionGuide: {
          id: 'model-selection-guide',
          title: 'どのモデルを使うべきか？簡単な決定ガイド',
          image: '/images/small-llm-decision-tree-mockup.svg',
          imageCaption: '決定木：優先事項（推論、速度、コーディング）で選択します。迷った場合はLlama 3.2 3Bを既定にしてください。',
        },
        phi4mini: {
          id: 'phi-4-mini',
        ...
        gemma2: {
          id: 'gemma-4-e2b',
          title: 'Gemma 4 E2B -- 最良の小型/エッジモデル',
          content: [
          ...
          rows: [
            ...
            { 'Spec': 'RAM (Q4_K_M)', 'Value': '~2 GB' },
            { 'Spec': 'コンテキスト', 'Value': '128Kトークン' },
            { 'Spec': 'Ollamaコマンド', 'Value': 'ollama pull gemma4:e2b' },
          ],
          columns: ['Spec', 'Value'],
        },
        qwen25_3b: {
        ...
        llama32_1b: {
          ...
          content: 'Llama 3.2 1BはMetaが公開する最小のLlamaモデルです。わずか1.3 GBのRAMを必要とし、CPUで60～90トークン/秒を生成します -- このページで最も高速なローカル実行可能モデルです。出力品質は限界的です：非常にシンプルな分類とキーワード抽出を処理しますが、一貫した複数文の応答に苦労します。RAMが本当に制約（3 GB未満）である場合、またはツール統合をテストする場合のみ、Llama 3.2 1Bを使用してください。',
        },
        comparisonTable: {
          id: 'comparison-table',
          title: '完全比較：4B未満の最高の小規模ローカルLLM',
          image: '/images/small-local-llm-models-performance-tier-hero-ja.webp',
          imageCaption: 'パフォーマンス階層: Phi-4 Miniは推論とコーディングで先頭、Gemma 4 E2Bは最小のRAMフットプリントでコンテキスト長（128K）をリード、Qwen2.5はコーディングで優れています。',
          rows: [
            ...
            { 'モデル': 'Gemma 4 E2B', 'MMLU': '—', 'HumanEval': '—', 'RAM': '2 GB', 'コンテキスト': '128K', '最適用途': 'エッジ用途（MMLU Pro 60%）' },
        ...
        quantizationGuide: {
          id: 'quantization-guide',
          title: '量子化を理解する：RAMと品質のトレードオフ',
          image: '/images/small-local-llm-models-quantization-tradeoff-hero-ja.webp',
          imageCaption: '量子化のトレードオフ：Q4_K_M（2.5 GB、品質-0.5%）が推奨デフォルトです。Q8_0は3.8 GBを使用しても品質向上はありません。極端なRAM制約にはQ3_K_M（1.8 GB、品質-1.8%）。',
        },
        relatedReading: {
          id: 'related-reading',
          title: '関連資料',
          ...
          faqs: [
            {
              ...
              a: '有用な出力の実用的な最小値はQ4_K_M量子化での3Bモデルです。2B以下のパラメータを持つモデル（Llama 3.2 1B、Gemma 4 E2B）は一貫した単一文を生成しますが、複数ステップの指示と複雑な推論に苦労しています。要約やシンプルなQ&Aなどのタスクの場合、Gemma 4 E2Bは使用可能です。より複雑な場合は、3Bモデルから始めてください。',
            },
            {
              q: '3Bモデルを電話で実行できますか？',
            ...
            {
              q: '小規模モデルは要約に適していますか？',
              a: 'はい -- 要約は小規模モデルの最強のユースケースの1つです。Gemma 4 E2BとLlama 3.2 3Bは、~4,000語までのテキスト（品質出力の実用的なコンテキスト制限）の正確な要約を確実に生成します。より長い文書の場合は、Phi-4 MiniまたはLlama 3.2 3B（両方で128Kトークン）のような大きなコンテキストウィンドウを持つモデルを使用してください。',
            },
            {
              q: '2Bモデルは同じハードウェアで7Bモデルより何倍速いですか？',
              a: 'ハードウェアと量子化に大きく依存します -- GoogleはRaspberry Pi 5でのGemma 4 E2Bのデコード速度を約7.6トークン/秒と報告しており、独立系のx86 CPUテストでは8～48トークン/秒の幅があります。GPUではスピード利点は狭くなります。スピード差はCPUのみのマシンで最も顕著です。',
            },
            {
              q: '小規模モデルは関数呼び出しをサポートしていますか？',
              a: 'いくつかサポートしています。Qwen2.5 3Bは関数呼び出しとJSONモードをサポートしています。Llama 3.2 3Bは基本的なツール使用をサポートしています。Gemma 4 E2Bは関数呼び出しをサポートしていません。構造化された出力に依存するパイプラインを構築する前に、モデルのドキュメントを確認してください。',
            },
            {
              q: '英語以外の言語に最適な小規模モデルはどれですか？',
              a: 'Qwen2.5 3Bは、中国語、日本語、韓国語、アラビア語を含む29言語をネイティブにサポートしています。Gemma 4 E2BとPhi-4 Miniは主に英語最適化されています。小規模モデルスケールでの非英語タスクの場合、Qwen2.5 3Bが明確な選択肢です。[Qwen vs Llama vs Mistral 多言語比較](/local-llms/qwen-vs-llama-vs-mistral)で完全な言語比較を参照してください。',
            },
            {
            ...
            {
              q: 'Q4_K_Mの代わりにQ8_0量子化を使用する場合',
              a: 'Q8_0は、この規模では品質向上がわずかであるにもかかわらず、Q4_K_Mのほぼ2倍のRAMを必要とします。Q8_0のLlama 3.2 3BモデルはQ4_K_Mの約2.5 GBに対して約3.8 GBのRAMが必要です。4 GBのマシンでは、Q8_0がスワップ使用を引き起こし、推論を3～5倍遅くする可能性があります。Sub-4Bモデルには常にQ4_K_Mをデフォルトとして使用してください。',
            },
            {
              q: 'instructバリアントの代わりにベースモデルを実行する場合',
              a: 'ベースモデル（例：llama3.2:3b-text）はファインチューニング前のチェックポイントで、テキスト中の次のトークンを予測するように学習されています。指示には従いません。ベースモデルに「2+2は何ですか？」と尋ねると、「4」と答える代わりに、クイズのように文を完成させることがあります。常にinstructバリアントを使用してください：llama3.2:3b（Ollamaは名前付きモデルに対してデフォルトでinstructを使用します）。',
            },
            {
              q: '3Bモデルに7Bモデル相当の品質を期待する場合',
              a: 'MMLU 68%の3Bモデル（Phi-4 Mini）は、一般的なタスクにおいて2023年頃のGPT-3.5 Miniと同程度の性能です。複雑な推論の連鎖、長文の執筆、繊細なコード生成では、7Bモデルよりも明らかに品質が低くなります。出力品質が不十分な場合は、7Bモデルにアップグレードしてください -- RAMの差は約2 GB（2.5 GB → 4.5 GB）です。',
            },
            {
              q: '実際に使える最小のローカルLLMは何ですか？',
              a: '一貫した出力の下限は約1Bパラメータで、実務で動かす価値があるこのリストの中で最も小さいモデルはLlama 3.2 1Bです。それより小さいモデルはほぼどんなハードウェアでもロードして生成できますが、数文でテキストの一貫性が崩れるため、アシスタントというよりオートコンプリートとして扱う方が適しています。品質よりディスクとRAMの制約が優先される場合、フットプリントが小さく実用的な最小のローカルLLMはQ4量子化のsub-2Bモデルです。4GB以上使える場合は、4Bモデルがわずかな追加RAMで一貫性を大きく向上させます。',
            },
            {
              q: 'ローカルで実行できる最小のLLMモデルは何ですか？',
              a: 'Llama 3.2 1BはMetaが公開する最小のLlamaモデルであり、この比較の中で最も実用的な選択肢です。必要なRAMはわずか1.3 GBで、CPUで60～90トークン/秒動作します -- このページで最も高速なモデルです。短い分類やキーワード抽出タスクは処理できますが、長く複数ステップの出力には対応できません。日常的に実際に使える小型モデル（「ミニLLM」と呼ばれることもあります）をお探しなら、Llama 3.2 1Bはハードウェアの下限であって品質の下限ではないと考えてください -- 実務にはGemma 4 E2Bや、Qwen2.5 3BやLlama 3.2 3Bのような3Bモデルにステップアップしてください。',
            },
          ],
        },
        sources: {
    ...
    zh: {
      ...
      seoTitle: '本地小模型推荐 2026：5大小型LLM排行与对比',
      metaDescription: '本地小模型推荐与排行：Phi-4 Mini（68% MMLU）、Gemma 4 E2B（MMLU Pro 60%，128K上下文）、Qwen2.5（编码最优）、Llama 3.2（通用）。4-8GB 内存即可运行，含性能对比。',
      heroImage: '/images/small-local-llm-models-performance-tier-hero-zh.webp',
      intro: '小型本地LLM（1B-4B参数）在4-8GB RAM的机器上运行，在CPU上生成30-70个令牌/秒----足够用于实时聊天。2026年最好的小型模型是Microsoft Phi-4 Mini 3.8B（最佳推理）、Google Gemma 4 E2B（最快）、Qwen2.5 3B（最佳编码）和Meta Llama 3.2 3B（最佳通用用途）。',
      publishDate: '2026-04-04',
      freshness_tier: 'semi_annual',
      next_refresh_due: '2027-03-02',
      ...
      toc: [
        ...
        { label: '什么是"小型"本地LLM？', anchor: '#what-is-small-llm' },
        { label: '应该使用哪个模型？', anchor: '#model-selection-guide' },
        { label: '#1 Phi-4 Mini 3.8B -- 最佳推理', anchor: '#phi-4-mini' },
        { label: '#2 Gemma 4 E2B -- 最佳小型/边缘模型', anchor: '#gemma-4-e2b' },
        { label: '#3 Qwen2.5 3B -- 最佳编码', anchor: '#qwen2-5-3b' },
        { label: '#4 Llama 3.2 3B -- 最佳通用模型', anchor: '#llama-3-2-3b' },
        { label: '#5 Llama 3.2 1B -- 绝对最小配置', anchor: '#llama-3-2-1b' },
        { label: '完整对比表', anchor: '#comparison-table' },
        { label: '量化指南', anchor: '#quantization-guide' },
        { label: '区域情况', anchor: '#regional-context' },
...
schema: {
        ...
        description: '2026年最佳小型本地LLM：Phi-4 Mini（推理）、Gemma 4 E2B（128K上下文，边缘设备）、Qwen2.5（编码）、Llama 3.2（通用）。RAM受限机器的完整比较。',
        datePublished: '2026-04-04',
        dateModified: '2026-09-02',
      ...
      itemListSchema: {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Gemma 4 E2B', description: 'MMLU Pro 60%，LiveCodeBench v6 44%，GPQA Diamond 43.4%。~2GB RAM。128K上下文。Apache 2.0。ollama pull gemma4:e2b' },
          { '@type': 'ListItem', position: 3, name: 'Qwen2.5 3B', description: 'MMLU 62%，HumanEval 65%。2GB RAM。原生支持29种语言。支持函数调用。ollama run qwen2.5:3b' },
      ...
      faqSchema: {
        ...
        '@type': 'FAQPage',
        mainEntity: [
          { '@type': 'Question', name: '生成有用输出的最小本地LLM是什么？', acceptedAnswer: { '@type': 'Answer', text: '有用输出的实际最小值是Q4_K_M量化下的3B模型。2B以下参数的模型生成连贯的单句但在多步骤指令和复杂推理上苦恼。对于摘要和简单问答等任务，Gemma 4 E2B是可用的。对于更复杂的任务，从3B模型开始。' } },
          { '@type': 'Question', name: '我可以在电话上运行3B模型吗？', acceptedAnswer: { '@type': 'Answer', text: '可以----Llama 3.2 1B和3B专为设备上移动部署而设计。Meta为iOS（通过MLC LLM）和Android提供优化的构建。在现代手机上的推理为1B模型生成15-30令牌/秒。LM Studio和Ollama目前不在iOS或Android上运行----移动需要单独的框架。' } },
          { '@type': 'Question', name: '小型模型适合摘要吗？', acceptedAnswer: { '@type': 'Answer', text: '是的----摘要是小型模型最强的用例之一。Gemma 4 E2B和Llama 3.2 3B可靠地生成高达〜4,000字的文本的准确摘要。对于较长的文档，使用具有大上下文窗口的模型，如Phi-4 Mini或Llama 3.2 3B（均128K令牌）。' } },
          { '@type': 'Question', name: '2B模型在相同硬件上比7B模型快多少倍？', acceptedAnswer: { '@type': 'Answer', text: '很大程度上取决于硬件和量化方式——Google官方数据显示Gemma 4 E2B在Raspberry Pi 5上的解码速度约为7.6令牌/秒，而独立的x86 CPU测试范围为8-48令牌/秒。在GPU上，速度优势缩小。速度差在仅CPU的机器上最明显。' } },
          { '@type': 'Question', name: '小型模型支持函数调用吗？', acceptedAnswer: { '@type': 'Answer', text: '有些支持。Qwen2.5 3B支持函数调用和JSON模式。Llama 3.2 3B有基本的工具使用支持。Gemma 4 E2B不支持函数调用。在构建依赖于结构化输出的管道之前检查模型文档。' } },
          { '@type': 'Question', name: '英文以外的语言最好的小型模型是什么？', acceptedAnswer: { '@type': 'Answer', text: 'Qwen2.5 3B原生支持29种语言，包括中文、日文、韩文和阿拉伯文。Gemma 4 E2B和Phi-4 Mini主要是英文优化的。对于小型模型规模的非英文任务，Qwen2.5 3B是明确的选择。有关完整的语言比较，请参阅[Qwen vs Llama vs Mistral 多语言比较](/local-llms/qwen-vs-llama-vs-mistral)。' } },
          { '@type': 'Question', name: '日常任务中Phi-4 Mini和Llama 3.2 3B之间的区别是什么？', acceptedAnswer: { '@type': 'Answer', text: 'Phi-4 Mini以几乎相同的RAM（每个2.5 GB）在推理、数学和编码上超过Llama 3.2 3B（68%对58% MMLU，70%对60% HumanEval）。对于日常任务----问答、摘要、简单解释----质量差距明显但不引人注目。Llama 3.2 3B拥有更广泛的社区支持和更多可用的微调。为结构化推理选择Phi-4 Mini；为通用聊天和兼容性选择Llama 3.2 3B。' } },
          { '@type': 'Question', name: '我可以同时运行两个小型模型吗？', acceptedAnswer: { '@type': 'Answer', text: '可以，如果总RAM允许的话。两个3B模型在Q4_K_M下组合使用〜5 GB----在具有精益OS的8 GB机器上可行。Ollama默认按进程一次加载一个模型。在不同的端口（OLLAMA_HOST=:11434和OLLAMA_HOST=:11435）上运行两个Ollama实例以并行提供两个模型。这对于A/B测试输出很有用。' } },
          { '@type': 'Question', name: '小型模型适合RAG（检索增强生成）吗？', acceptedAnswer: { '@type': 'Answer', text: '对于简单RAG是的。Llama 3.2 3B和Phi-4 Mini可以可靠地回答检索到的文档块上的问题。对于需要多跳推理的大型知识库上的RAG，7B+模型执行更加一致。GPT4All的LocalDocs功能为文档问答使用3B模型，对个人文档集合效果很好。' } },
          { '@type': 'Question', name: 'Phi-4 Mini在编码上比Llama 3.2 3B更好吗？', acceptedAnswer: { '@type': 'Answer', text: '是的。Phi-4 Mini在此规模达到60%对70% HumanEval得分，与Llama 3.2 3B相比有意义的10点差距。对于4-6 GB RAM机器上的编码协助，Phi-4 Mini是推荐选择。对于多语言编码（非Python），65% HumanEval的Qwen2.5 3B与Phi-4 Mini竞争，同时也支持函数调用。' } },
          { '@type': 'Question', name: '使用Q8_0量化代替Q4_K_M', acceptedAnswer: { '@type': 'Answer', text: '在小规模下，Q8_0的RAM需求几乎是Q4_K_M的两倍，但质量提升微乎其微。使用Q8_0的Llama 3.2 3B模型需要约3.8 GB RAM，而Q4_K_M仅需约2.5 GB。在4 GB的机器上，Q8_0可能触发交换分区使用，使推理速度慢3-5倍。对于Sub-4B模型，请始终使用Q4_K_M作为默认设置。' } },
          { '@type': 'Question', name: '运行基础模型而非指令变体', acceptedAnswer: { '@type': 'Answer', text: '基础模型（例如llama3.2:3b-text）是微调前的检查点，经过训练用于预测文本中的下一个token，不遵循指令。当你问基础模型"2+2等于多少？"时，它可能像做填空题一样补全句子，而不是回答"4"。请始终使用指令变体：llama3.2:3b（Ollama对具名模型默认使用指令版本）。' } },
          { '@type': 'Question', name: '期望3B模型达到7B模型的质量', acceptedAnswer: { '@type': 'Answer', text: 'MMLU得分68%的3B模型（Phi-4 Mini）在一般任务上的表现与2023年前后的GPT-3.5 Mini相近。复杂的推理链、长篇写作和精细的代码生成，其质量会明显低于7B模型。如果输出质量不够，请升级到7B模型——RAM差异约为2 GB（2.5 GB → 4.5 GB）。' } },
          { '@type': 'Question', name: '实际可用的最小本地LLM是什么？', acceptedAnswer: { '@type': 'Answer', text: '约1B参数是连贯输出的下限，Llama 3.2 1B是本文中值得用于实际工作的最小模型。低于这个规模，微型模型几乎可以在任何硬件上加载并生成文本，但输出在几句话后就失去连贯性，更适合当作自动补全而非助手使用。如果限制因素是磁盘和RAM而非质量，占用小且实用的最小本地LLM是Q4量化的sub-2B模型；如果有4GB以上可用空间，4B模型能以极少的额外RAM换来连贯性的大幅提升。' } },
          { '@type': 'Question', name: '你可以在本地运行的最小LLM模型是什么？', acceptedAnswer: { '@type': 'Answer', text: 'Llama 3.2 1B是Meta发布的最小Llama模型，也是本次比较中最实用的选择——仅需1.3 GB RAM，在CPU上以60-90令牌/秒运行，比本页其他任何模型都快。它能处理简短的分类和关键词提取任务，但在更长、多步骤的输出上会失效。如果你想要一个日常真正可用的小模型（有时称为mini LLM），把Llama 3.2 1B当作硬件下限，而非质量下限——实际工作请升级到Gemma 4 E2B，或Qwen2.5 3B、Llama 3.2 3B等3B模型。' } },
        ],
      },
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: '小型本地LLM（1B-4B参数）可在4-8 GB内存的设备上以每秒30-70个令牌的速度运行，Phi-4 Mini 3.8B在推理方面领先，Gemma 4 E2B最适合支持128K上下文的小型/边缘设备，Qwen2.5 3B最适合编程，Llama 3.2 3B最适合通用用途。' },
          ...
          items: [
            ...
            '2B以下的模型无法生成适合专业任务的输出质量。对于实际工作，使用3B或更大的模型。',
          ],
        },
        whatIsSmall: {
          id: 'what-is-small-llm',
          ...
          content: [
            ...
            '小型模型适合：快速摘要、简单问答、代码片段解释、短文本翻译和分类任务。不适合多步推理、复杂代码生成或长篇一致的文档编写。',
        ...
        modelSelectionGuide: {
          id: 'model-selection-guide',
          title: '应该使用哪个模型？快速决策指南',
          image: '/images/small-llm-decision-tree-mockup.svg',
          imageCaption: '决策树：按优先级选择（推理、速度或编码）。如不确定，默认选择Llama 3.2 3B。',
        },
        phi4mini: {
          id: 'phi-4-mini',
          ...
          content: [
            ...
            'Phi-4 Mini是需要推理（数学、逻辑、分步解释）或在4-6 GB RAM硬件上进行编码协助的用户的推荐选择。',
          ],
          rows: [
            { 'Spec': 'MMLU', 'Value': '68%' },
            { 'Spec': 'HumanEval', 'Value': '70%' },
        ...
        gemma2: {
          id: 'gemma-4-e2b',
          title: 'Gemma 4 E2B -- 最佳小型/边缘模型',
          content: [
            '**Google Gemma 4 E2B是当前一代小型模型，取代了较旧的Gemma 2 2B。** 根据Google官方模型卡，它在MMLU Pro（比原始MMLU更难的基准测试）上得分60%，在LiveCodeBench v6上得分44%，在GPQA Diamond上得分43.4%。有效参数为2.3B（含嵌入层为5.1B），可在约2GB RAM中运行。',
            '128K的上下文窗口相较旧款Gemma 2 2B的8K是重大提升——现已与Phi-4 Mini和Llama 3.2 3B持平。CPU速度因硬件和量化方式而异（Google官方数据显示在Raspberry Pi 5上解码约7.6令牌/秒；独立的x86 CPU测试报告范围为8-48令牌/秒）——请查阅您设备的最新数据。采用Apache 2.0许可证。',
          ],
          rows: [
            { 'Spec': 'MMLU Pro', 'Value': '60%' },
            { 'Spec': 'LiveCodeBench v6', 'Value': '44%' },
            ...
            { 'Spec': 'RAM (Q4_K_M)', 'Value': '~2 GB' },
            { 'Spec': '上下文', 'Value': '128K令牌' },
            { 'Spec': 'Ollama命令', 'Value': 'ollama pull gemma4:e2b' },
          ],
          columns: ['Spec', 'Value'],
        },
        qwen25_3b: {
        ...
        llama32_1b: {
          ...
          content: 'Llama 3.2 1B是Meta发布的最小Llama模型，仅需1.3 GB RAM，在CPU上生成60-90令牌/秒——是本页最快的本地运行模型。输出质量是边界的：它处理非常简单的分类和关键词提取，但在连贯的多句响应上苦恼。仅在RAM是真正约束（可用3 GB以下）或测试工具集成时使用Llama 3.2 1B。',
        },
        comparisonTable: {
          id: 'comparison-table',
          title: '完整比较：4B以下最好的小型本地LLM',
          image: '/images/small-local-llm-models-performance-tier-hero-zh.webp',
          imageCaption: '性能层级：Phi-4 Mini在推理和编码方面领先，Gemma 4 E2B以最小的RAM占用领先于上下文长度（128K），Qwen2.5在编码方面表现出色。',
          rows: [
            ...
            { '模型': 'Gemma 4 E2B', 'MMLU': '—', 'HumanEval': '—', 'RAM': '2 GB', '上下文': '128K', '最适合': '边缘设备（MMLU Pro 60%）' },
        ...
        regionalContext: {
          ...
          content: [
            ...
            '**其他地区：** 对于英文用例，Phi-4 Mini 3.8B在基于CPU的系统上提供最佳推理质量。当成本优化和速度很重要时，Gemma 4 E2B以最小的内存提供可扩展的推理基础。',
          ],
        },
        commonMistakes: {
          id: 'common-mistakes',
        ...
        quantizationGuide: {
          id: 'quantization-guide',
          title: '理解量化：RAM与质量的权衡',
          image: '/images/small-local-llm-models-quantization-tradeoff-hero-zh.webp',
          imageCaption: '量化权衡：Q4_K_M（2.5 GB，质量-0.5%）是推荐的默认选项。Q8_0使用3.8 GB但没有质量提升。极端RAM限制下可选Q3_K_M（1.8 GB，质量-1.8%）。',
        },
        relatedReading: {
          id: 'related-reading',
          title: '相关阅读',
        ...
        faqSection: {
          ...
          faqs: [
            {
              q: '生成有用输出的最小本地LLM是什么？',
              a: '有用输出的实际最小值是Q4_K_M量化下的3B模型。2B以下参数的模型生成连贯的单句但在多步骤指令和复杂推理上苦恼。对于摘要和简单问答等任务，Gemma 4 E2B是可用的。对于更复杂的任务，从3B模型开始。',
            },
            {
              q: '我可以在电话上运行3B模型吗？',
            ...
            {
              q: '小型模型适合摘要吗？',
              a: '是的----摘要是小型模型最强的用例之一。Gemma 4 E2B和Llama 3.2 3B可靠地生成高达〜4,000字的文本的准确摘要。对于较长的文档，使用具有大上下文窗口的模型，如Phi-4 Mini或Llama 3.2 3B（均128K令牌）。',
            },
            {
              q: '2B模型在相同硬件上比7B模型快多少倍？',
              a: '很大程度上取决于硬件和量化方式——Google官方数据显示Gemma 4 E2B在Raspberry Pi 5上的解码速度约为7.6令牌/秒，而独立的x86 CPU测试范围为8-48令牌/秒。在GPU上，速度优势缩小。速度差在仅CPU的机器上最明显。',
            },
            {
              q: '小型模型支持函数调用吗？',
              a: '有些支持。Qwen2.5 3B支持函数调用和JSON模式。Llama 3.2 3B有基本的工具使用支持。Gemma 4 E2B不支持函数调用。在构建依赖于结构化输出的管道之前检查模型文档。',
            },
            {
              q: '英文以外的语言最好的小型模型是什么？',
              a: 'Qwen2.5 3B原生支持29种语言，包括中文、日文、韩文和阿拉伯文。Gemma 4 E2B和Phi-4 Mini主要是英文优化的。对于小型模型规模的非英文任务，Qwen2.5 3B是明确的选择。有关完整的语言比较，请参阅[Qwen vs Llama vs Mistral 多语言比较](/local-llms/qwen-vs-llama-vs-mistral)。',
            },
            {
            ...
            {
              q: '使用Q8_0量化代替Q4_K_M',
              a: '在小规模下，Q8_0的RAM需求几乎是Q4_K_M的两倍，但质量提升微乎其微。使用Q8_0的Llama 3.2 3B模型需要约3.8 GB RAM，而Q4_K_M仅需约2.5 GB。在4 GB的机器上，Q8_0可能触发交换分区使用，使推理速度慢3-5倍。对于Sub-4B模型，请始终使用Q4_K_M作为默认设置。',
            },
            {
              q: '运行基础模型而非指令变体',
              a: '基础模型（例如llama3.2:3b-text）是微调前的检查点，经过训练用于预测文本中的下一个token，不遵循指令。当你问基础模型"2+2等于多少？"时，它可能像做填空题一样补全句子，而不是回答"4"。请始终使用指令变体：llama3.2:3b（Ollama对具名模型默认使用指令版本）。',
            },
            {
              q: '期望3B模型达到7B模型的质量',
              a: 'MMLU得分68%的3B模型（Phi-4 Mini）在一般任务上的表现与2023年前后的GPT-3.5 Mini相近。复杂的推理链、长篇写作和精细的代码生成，其质量会明显低于7B模型。如果输出质量不够，请升级到7B模型——RAM差异约为2 GB（2.5 GB → 4.5 GB）。',
            },
            {
              q: '实际可用的最小本地LLM是什么？',
              a: '约1B参数是连贯输出的下限，Llama 3.2 1B是本文中值得用于实际工作的最小模型。低于这个规模，微型模型几乎可以在任何硬件上加载并生成文本，但输出在几句话后就失去连贯性，更适合当作自动补全而非助手使用。如果限制因素是磁盘和RAM而非质量，占用小且实用的最小本地LLM是Q4量化的sub-2B模型；如果有4GB以上可用空间，4B模型能以极少的额外RAM换来连贯性的大幅提升。',
            },
            {
              q: '你可以在本地运行的最小LLM模型是什么？',
              a: 'Llama 3.2 1B是Meta发布的最小Llama模型，也是本次比较中最实用的选择——仅需1.3 GB RAM，在CPU上以60-90令牌/秒运行，比本页其他任何模型都快。它能处理简短的分类和关键词提取任务，但在更长、多步骤的输出上会失效。如果你想要一个日常真正可用的小模型（有时称为mini LLM），把Llama 3.2 1B当作硬件下限，而非质量下限——实际工作请升级到Gemma 4 E2B，或Qwen2.5 3B、Llama 3.2 3B等3B模型。',
            },
          ],
        },
        sources: {
          ...
          items: [
            ...
            'Google Gemma 4 模型卡 -- huggingface.co/google/gemma-4-e2b-it',
          ],
        },
      },
    },
  ko: {
      freshness_tier: 'semi_annual',
      theme: 'Best Models',
      title: '소형 로컬 LLM 모델: 2026년 저용량 RAM 기기를 위한 최고의 4B 미만 모델',
      seoTitle: '소형 로컬 LLM 모델 2026: 상위 5개 4B 미만 순위',
      intro: '소형 로컬 LLM(1B~4B 파라미터)은 RAM 4~8GB 기기에서 실행되며 CPU에서 초당 30~70 토큰을 생성합니다 -- 실시간 채팅에 충분한 속도입니다. 2026년 최고의 소형 모델은 Microsoft Phi-4 Mini 3.8B(최고 추론 성능), Google Gemma 4 E2B(가장 빠름), Qwen2.5 3B(최고 코딩 성능), Meta Llama 3.2 3B(최고 범용 성능)입니다.',
      metaDescription: 'RAM 4~8GB용 4B 미만 최고 로컬 LLM 모델: Phi-4 Mini(MMLU 68%), Gemma 4 E2B(MMLU Pro 60%, 컨텍스트 128K), Qwen2.5(코딩), Llama 3.2(범용).',
      heroImage: '/images/small-local-llm-models-performance-tier-hero-ko.webp',
      publishDate: '2026-04-04',
      ...
      readTime: '8분 읽기',
      educationalLevel: 'Beginner',
      primaryTerm: '소형 로컬 LLM',
      toc: [
        { label: '핵심 요약', anchor: '#key-takeaways' },
        { label: '"소형" 로컬 LLM이란?', anchor: '#what-is-small-llm' },
        { label: '어떤 모델을 사용해야 합니까?', anchor: '#model-selection-guide' },
        { label: '#1 Phi-4 Mini 3.8B -- 최고 추론 성능', anchor: '#phi-4-mini' },
        { label: '#2 Gemma 4 E2B -- CPU에서 가장 빠름', anchor: '#gemma-4-e2b' },
        { label: '#3 Qwen2.5 3B -- 코딩에 최적', anchor: '#qwen2-5-3b' },
        { label: '#4 Llama 3.2 3B -- 최고 범용 모델', anchor: '#llama-3-2-3b' },
        { label: '#5 Llama 3.2 1B -- 절대 최소 사양', anchor: '#llama-3-2-1b' },
        { label: '전체 비교 표', anchor: '#comparison-table' },
        { label: '양자화 가이드', anchor: '#quantization-guide' },
        { label: '지역별 현황', anchor: '#regional-context' },
      ...
      schema: {
        ...
        proficiencyLevel: 'Beginner',
        about: [
          { '@type': 'Thing', name: '소형 로컬 LLM' },
          { '@type': 'Thing', name: 'Phi-4 Mini' },
          { '@type': 'Thing', name: 'Gemma 4 E2B' },
          { '@type': 'Thing', name: 'Llama 3.2 3B' },
          { '@type': 'Thing', name: '저용량 RAM 추론' },
        ],
      },
      itemListSchema: {
        '@context': 'https://schema.org',
        '@type': 'ItemList',
        name: '2026년 4B 미만 최고 소형 로컬 LLM 모델',
        numberOfItems: 5,
        itemListElement: [
          { '@type': 'ListItem', position: 1, name: 'Phi-4 Mini 3.8B', description: 'MMLU 68%, HumanEval 70%. RAM 2.5 GB. CPU 초당 30~50 토큰. 컨텍스트 128K. 최고 추론 성능. ollama run phi4-mini' },
          { '@type': 'ListItem', position: 2, name: 'Gemma 4 E2B', description: 'MMLU Pro 60%, LiveCodeBench v6 44%, GPQA Diamond 43.4%. RAM ~2 GB. 컨텍스트 128K. Apache 2.0. ollama pull gemma4:e2b' },
          { '@type': 'ListItem', position: 3, name: 'Qwen2.5 3B', description: 'MMLU 62%, HumanEval 65%. RAM 2 GB. 29개 언어 기본 지원. 함수 호출 지원. ollama run qwen2.5:3b' },
          { '@type': 'ListItem', position: 4, name: 'Llama 3.2 3B', description: 'MMLU 58%, HumanEval 60%. RAM 2.5 GB. 컨텍스트 128K. 커뮤니티 지원 가장 광범위. ollama run llama3.2:3b' },
          { '@type': 'ListItem', position: 5, name: 'Llama 3.2 1B', description: 'MMLU 32%. RAM 1.3 GB. CPU 초당 60~90 토큰. 최소 RAM 요구 사양. ollama run llama3.2:1b' },
        ],
      },
      faqSchema: {
        '@context': 'https://schema.org',
        ...
        mainEntity: [
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': '유용한 출력을 위한 실질적인 최소 사양은 Q4_K_M 양자화의 3B 모델입니다. 2B 미만 파라미터 모델(Llama 3.2 1B, Gemma 4 E2B)은 일관성 있는 단일 문장을 생성하지만 다단계 지시, 긴 응답, 복잡한 추론에 어려움을 겪습니다. 요약이나 간단한 Q&A와 같은 작업에는 Gemma 4 E2B가 사용 가능합니다. 더 복잡한 작업에는 3B 모델부터 시작하십시오.',
            },
          },
          {
            '@type': 'Question',
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': '일부 모델은 지원합니다. Qwen2.5 3B는 함수 호출과 JSON 모드를 지원합니다. Llama 3.2 3B는 기본적인 도구 사용을 지원합니다. Gemma 4 E2B는 함수 호출을 지원하지 않습니다. 구조화된 출력에 의존하는 파이프라인을 구축하기 전에 모델 문서를 확인하십시오.',
            },
          },
          {
            '@type': 'Question',
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Qwen2.5 3B는 중국어, 일본어, 한국어, 아랍어를 포함하여 29개 언어를 기본으로 지원합니다. Gemma 4 E2B와 Phi-4 Mini는 주로 영어에 최적화되어 있습니다. 소형 모델 규모에서 비영어 작업에는 Qwen2.5 3B가 명확한 선택입니다. 전체 언어 비교는 [Qwen vs Llama vs Mistral 다국어 비교](/ko/local-llms/qwen-vs-llama-vs-mistral)를 참조하십시오.',
            },
          },
          {
            '@type': 'Question',
          ...
          },
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': '예. Phi-4 Mini는 HumanEval에서 70%를 기록하는 반면 Llama 3.2 3B는 60%입니다 -- 이 규모에서 의미 있는 10포인트 차이입니다. RAM 4~6 GB 기기에서 코딩 지원에는 Phi-4 Mini가 권장됩니다. 다국어 코딩(Python 외)의 경우 HumanEval 65%의 Qwen2.5 3B가 함수 호출도 지원하면서 Phi-4 Mini와 경쟁력 있는 성능을 보입니다.',
            },
          },
          {
            '@type': 'Question',
          ...
          {
            '@type': 'Question',
            'name': '실제로 사용 가능한 가장 작은 로컬 LLM은 무엇입니까?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': '일관된 출력을 위한 최소 기준은 약 1B 파라미터이며, 이 목록에서 실제 작업에 사용할 가치가 있는 가장 작은 모델은 Llama 3.2 1B입니다. 그 이하의 초소형 모델은 거의 모든 하드웨어에서 로드되고 텍스트를 생성하지만, 몇 문장이 지나면 일관성이 무너지므로 어시스턴트보다는 자동 완성 기능으로 취급하는 것이 적절합니다. 품질보다 디스크와 RAM이 제약 조건이라면, 작은 용량으로 실용적인 가장 작은 로컬 LLM은 Q4 양자화의 sub-2B 모델입니다. 4GB 이상 여유가 있다면 4B 모델이 아주 적은 추가 RAM으로 일관성을 크게 향상시킵니다.',
            },
          },
          {
            '@type': 'Question',
            'name': '로컬에서 실행할 수 있는 가장 작은 LLM 모델은 무엇입니까?',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Llama 3.2 1B는 Meta가 공개한 가장 작은 Llama 모델이자 이 비교에서 가장 실용적인 선택입니다 -- RAM 1.3 GB만 필요하며 CPU에서 초당 60~90 토큰으로 동작하여 이 페이지의 다른 어떤 모델보다 빠릅니다. 짧은 분류 및 키워드 추출 작업은 처리하지만, 더 길고 여러 단계로 이루어진 출력에서는 한계를 보입니다. 일상적으로 실제로 사용할 수 있는 소형 모델(미니 LLM이라고도 불립니다)을 원한다면, Llama 3.2 1B를 품질의 하한이 아니라 하드웨어의 하한으로 취급하십시오 -- 실제 작업에는 Gemma 4 E2B나 Qwen2.5 3B, Llama 3.2 3B 같은 3B 모델로 단계를 올리십시오.',
            },
          },
        ],
      },
      sections: {
        tldr: {
          ...
          isTldr: true,
          snippetBlocks: [
            { type: 'one-sentence', text: '소형 로컬 LLM(1B-4B 파라미터)은 4-8GB RAM 기기에서 초당 30-70토큰으로 동작하며, 추론에서는 Phi-4 Mini 3.8B가 앞서고, 128K 컨텍스트를 지원하는 소형/엣지 기기에는 Gemma 4 E2B, 코딩에는 Qwen2.5 3B, 범용에는 Llama 3.2 3B가 가장 적합합니다.' },
        ...
        phi4mini: {
          ...
          rows: [
            ...
            { 'Spec': 'HumanEval', 'Value': '70%' },
            { 'Spec': 'RAM (Q4_K_M)', 'Value': '~2.5 GB' },
            { 'Spec': '컨텍스트', 'Value': '128K 토큰' },
            { 'Spec': 'CPU 속도', 'Value': '초당 30~50 토큰' },
        ...
        qwen25_3b: {
          id: 'qwen2-5-3b',
          title: 'Qwen2.5 3B -- 코딩 작업에 최적인 소형 모델',
          content: [
          ...
          rows: [
            ...
            { 'Spec': 'RAM (Q4_K_M)', 'Value': '~2 GB' },
            { 'Spec': '컨텍스트', 'Value': '128K 토큰' },
            { 'Spec': 'CPU 속도', 'Value': '초당 25~40 토큰' },
            { 'Spec': 'Ollama 명령어', 'Value': 'ollama run qwen2.5:3b' },
          ],
          columns: ['Spec', 'Value'],
        },
        llama32_3b: {
        ...
        llama32_1b: {
          ...
          content: 'Llama 3.2 1B는 Meta가 공개한 가장 작은 Llama 모델로, RAM 1.3 GB만 필요하며 CPU에서 초당 60~90 토큰을 생성합니다 -- 이 페이지에서 로컬로 실행 가능한 가장 빠른 모델입니다. 출력 품질은 한계 수준입니다: 매우 간단한 분류와 키워드 추출은 처리하지만 일관성 있는 여러 문장 응답에는 어려움을 겪습니다. Llama 3.2 1B는 RAM이 실제로 제약 요인인 경우(가용 RAM 3 GB 미만)나 도구 통합 테스트 목적으로만 사용하십시오.',
        },
        comparisonTable: {
          id: 'comparison-table',
          title: '전체 비교: 4B 미만 최고 소형 로컬 LLM',
          image: '/images/small-local-llm-models-performance-tier-hero-ko.webp',
          imageCaption: '성능 등급: Phi-4 Mini는 추론과 코딩에서 선두이고, Gemma 4 E2B는 가장 작은 RAM 사용량으로 컨텍스트 길이(128K)에서 앞서며, Qwen2.5는 코딩에서 탁월합니다.',
          rows: [
            { '모델': 'Phi-4 Mini 3.8B', 'MMLU': '68%', 'HumanEval': '70%', 'RAM': '2.5 GB', '컨텍스트': '128K', '최적 용도': '추론, 코딩' },
            { '모델': 'Qwen2.5 3B', 'MMLU': '62%', 'HumanEval': '65%', 'RAM': '2 GB', '컨텍스트': '128K', '최적 용도': '코딩, 다국어' },
            { '모델': 'Llama 3.2 3B', 'MMLU': '58%', 'HumanEval': '60%', 'RAM': '2.5 GB', '컨텍스트': '128K', '최적 용도': '범용, 첫 모델' },
            { '모델': 'Gemma 4 E2B', 'MMLU': '—', 'HumanEval': '—', 'RAM': '2 GB', '컨텍스트': '128K', '최적 용도': '엣지 (MMLU Pro 60%)' },
            { '모델': 'Llama 3.2 1B', 'MMLU': '32%', 'HumanEval': '28%', 'RAM': '1.3 GB', '컨텍스트': '128K', '최적 용도': '절대 최소 RAM' },
          ],
        ...
        relatedReading: {
          ...
          items: [
            ...
            '[코딩에 최적인 로컬 LLM](/ko/local-llms/best-local-llms-for-coding) -- Qwen2.5 3B가 복잡한 작업에 충분하지 않을 때의 7B 이상 규모 코딩 특화 모델',
        ...
        faqSection: {
          ...
          faqs: [
            { q: '유용한 출력을 생성하는 가장 작은 로컬 LLM은 무엇입니까?', a: '유용한 출력을 위한 실질적인 최소 사양은 Q4_K_M 양자화의 3B 모델입니다. 2B 미만 파라미터 모델(Llama 3.2 1B, Gemma 4 E2B)은 일관성 있는 단일 문장을 생성하지만 다단계 지시, 긴 응답, 복잡한 추론에 어려움을 겪습니다. 요약이나 간단한 Q&A와 같은 작업에는 Gemma 4 E2B가 사용 가능합니다. 더 복잡한 작업에는 3B 모델부터 시작하십시오.' },
            ...
            { q: '소형 모델은 함수 호출을 지원합니까?', a: '일부 모델은 지원합니다. Qwen2.5 3B는 함수 호출과 JSON 모드를 지원합니다. Llama 3.2 3B는 기본적인 도구 사용을 지원합니다. Gemma 4 E2B는 함수 호출을 지원하지 않습니다. 구조화된 출력에 의존하는 파이프라인을 구축하기 전에 모델 문서를 확인하십시오.' },
            { q: '영어 외 언어에 가장 적합한 소형 모델은 무엇입니까?', a: 'Qwen2.5 3B는 중국어, 일본어, 한국어, 아랍어를 포함하여 29개 언어를 기본으로 지원합니다. Gemma 4 E2B와 Phi-4 Mini는 주로 영어에 최적화되어 있습니다. 소형 모델 규모에서 비영어 작업에는 Qwen2.5 3B가 명확한 선택입니다. 전체 언어 비교는 [Qwen vs Llama vs Mistral 다국어 비교](/ko/local-llms/qwen-vs-llama-vs-mistral)를 참조하십시오.' },
            ...
            { q: '코딩에서 Phi-4 Mini가 Llama 3.2 3B보다 우수합니까?', a: '예. Phi-4 Mini는 HumanEval에서 70%를 기록하는 반면 Llama 3.2 3B는 60%입니다 -- 이 규모에서 의미 있는 10포인트 차이입니다. RAM 4~6 GB 기기에서 코딩 지원에는 Phi-4 Mini가 권장됩니다. 다국어 코딩(Python 외)의 경우 HumanEval 65%의 Qwen2.5 3B가 함수 호출도 지원하면서 Phi-4 Mini와 경쟁력 있는 성능을 보입니다.' },
            ...
            { q: '실제로 사용 가능한 가장 작은 로컬 LLM은 무엇입니까?', a: '일관된 출력을 위한 최소 기준은 약 1B 파라미터이며, 이 목록에서 실제 작업에 사용할 가치가 있는 가장 작은 모델은 Llama 3.2 1B입니다. 그 이하의 초소형 모델은 거의 모든 하드웨어에서 로드되고 텍스트를 생성하지만, 몇 문장이 지나면 일관성이 무너지므로 어시스턴트보다는 자동 완성 기능으로 취급하는 것이 적절합니다. 품질보다 디스크와 RAM이 제약 조건이라면, 작은 용량으로 실용적인 가장 작은 로컬 LLM은 Q4 양자화의 sub-2B 모델입니다. 4GB 이상 여유가 있다면 4B 모델이 아주 적은 추가 RAM으로 일관성을 크게 향상시킵니다.' },
            { q: '로컬에서 실행할 수 있는 가장 작은 LLM 모델은 무엇입니까?', a: 'Llama 3.2 1B는 Meta가 공개한 가장 작은 Llama 모델이자 이 비교에서 가장 실용적인 선택입니다 -- RAM 1.3 GB만 필요하며 CPU에서 초당 60~90 토큰으로 동작하여 이 페이지의 다른 어떤 모델보다 빠릅니다. 짧은 분류 및 키워드 추출 작업은 처리하지만, 더 길고 여러 단계로 이루어진 출력에서는 한계를 보입니다. 일상적으로 실제로 사용할 수 있는 소형 모델(미니 LLM이라고도 불립니다)을 원한다면, Llama 3.2 1B를 품질의 하한이 아니라 하드웨어의 하한으로 취급하십시오 -- 실제 작업에는 Gemma 4 E2B나 Qwen2.5 3B, Llama 3.2 3B 같은 3B 모델로 단계를 올리십시오.' },
          ],
        },
        sources: {
          id: 'sources',
