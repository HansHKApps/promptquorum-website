...
schema: {
      ...
      sections: {
        ...
        relatedReading: { id: 'related-reading', title: 'Lectures complémentaires', items: ['[Comment installer Ollama](/local-llms/how-to-install-ollama?lang=fr) -- Installation et configuration', '[Comment installer LM Studio](/local-llms/how-to-install-lm-studio?lang=fr) -- Alternative GUI', '[Meilleurs modèles LLM locaux pour débutants](/local-llms/best-beginner-local-llm-models?lang=fr) -- Recommandations de modèles selon votre matériel', '[Que sont les LLMs locaux ?](/local-llms/what-are-local-llms?lang=fr) -- Concepts de base', '[Dépannage de l\'installation LLM local](/local-llms/troubleshooting-local-llm-setup?lang=fr) -- Corriger l\'inférence lente et les erreurs courantes', '[API compatible OpenAI pour LLM local](/local-llms/local-llm-openai-compatible-api?lang=fr) -- Connecter Ollama à Python et d\'autres applications', 'Votre premier LLM local fonctionne. Apprenez maintenant à le prompter efficacement : [qu\'est-ce que le prompt engineering](https://www.promptquorum.com/prompt-engineering/what-is-prompt-engineering?lang=fr) enseigne les bases.'] },
      },
    },
    ja: {
      theme: '入門',
...
schema: {
      ...
      sections: {
        ...
        nextSteps: { title: '最初の実行後の次のステップ', content: '動作するローカルLLMができたので、何ができるか探索しましょう。ハードウェアに最適なモデルを理解するには、[初心者向けベストローカルLLMモデル](/local-llms/best-beginner-local-llm-models?lang=ja)をご覧ください。ノートパソコン固有のパフォーマンスについては[ノートパソコンでローカルLLMを実行する方法](/local-llms/local-llm-on-laptop?lang=ja)をご覧ください。' },
        ...
        relatedReading: { id: 'related-reading', title: '関連資料', items: ['[Ollamaのインストール方法](/local-llms/how-to-install-ollama?lang=ja) -- インストールとセットアップ', '[LM Studioのインストール方法](/local-llms/how-to-install-lm-studio?lang=ja) -- GUIの代替', '[初心者向けベストローカルLLMモデル](/local-llms/best-beginner-local-llm-models?lang=ja) -- ハードウェアに基づくモデル推奨', '[ローカルLLMとは？](/local-llms/what-are-local-llms?lang=ja) -- 基本概念', '[ローカルLLMセットアップのトラブルシューティング](/local-llms/troubleshooting-local-llm-setup?lang=ja) -- 遅い推論やGPU未検出の修正', '[ローカルLLM OpenAI互換API](/local-llms/local-llm-openai-compatible-api?lang=ja) -- OllamaをPythonやその他のアプリに接続', 'ローカルLLMが動作しました。次は効果的なプロンプトを学びましょう：[プロンプトエンジニアリングとは](https://www.promptquorum.com/prompt-engineering/what-is-prompt-engineering?lang=ja)で基礎を学べます。'] },
      },
    },
    zh: {
      theme: '入门',
...
schema: {
      ...
      sections: {
        ...
        beyondTerminal: { id: 'beyond-the-terminal', title: '如何在终端之外使用本地LLM？', content: 'Ollama终端聊天适合测试，但大多数实际用例需要更好的界面：', items: ['**Open WebUI**：Ollama的全功能Web界面。用Docker运行：`docker run -d -p 3000:8080 --add-host=host.docker.internal:host-gateway ghcr.io/open-webui/open-webui:main`。访问http://localhost:3000。', '**LM Studio**：如果您更喜欢桌面GUI，[如何安装LM Studio](/local-llms/how-to-install-lm-studio?lang=zh)涵盖完整设置。', '**API集成**：`localhost:11434`的Ollama API与OpenAI SDK兼容。任何接受OpenAI基础URL的应用程序都可以连接到本地模型。', '**VS Code / Cursor**：Continue.dev等扩展连接到Ollama，直接在编辑器中提供本地AI编码辅助。'] },
        ...
        nextSteps: { title: '第一次运行后的下一步', content: '现在您有了一个可用的本地LLM，探索它能做什么。要了解哪些模型最适合您的硬件，请参阅[初学者最佳本地LLM模型](/local-llms/best-beginner-local-llm-models?lang=zh)。有关笔记本电脑专用性能技巧，请参阅[如何在笔记本电脑上运行本地LLM](/local-llms/local-llm-on-laptop?lang=zh)。' },
        ...
        relatedReading: { id: 'related-reading', title: '相关阅读', items: ['[如何安装Ollama](/local-llms/how-to-install-ollama?lang=zh) -- 安装和设置', '[如何安装LM Studio](/local-llms/how-to-install-lm-studio?lang=zh) -- GUI替代方案', '[初学者最佳本地LLM模型](/local-llms/best-beginner-local-llm-models?lang=zh) -- 基于硬件的模型推荐', '[什么是本地LLM？](/local-llms/what-are-local-llms?lang=zh) -- 核心概念', '[本地LLM设置故障排除](/local-llms/troubleshooting-local-llm-setup?lang=zh) -- 修复慢推理和常见错误', '[本地LLM OpenAI兼容API](/local-llms/local-llm-openai-compatible-api?lang=zh) -- 将Ollama连接到Python和其他应用程序', '您的第一个本地LLM正在运行。现在学习如何有效地提示它：[什么是提示工程](https://www.promptquorum.com/prompt-engineering/what-is-prompt-engineering?lang=zh)教授基础知识。'] },
      },
    },
  };
