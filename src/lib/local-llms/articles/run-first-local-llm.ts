      ...
      sections: {
        ...
        whatToExpect: {
          ...
          content: [
            ...
            '**Quality** from `llama3.2:3b` is noticeably lower than GPT-4o or Claude Opus 4.8 on complex tasks. For summarization, simple Q&A, and code explanation, the output is useful. For multi-step reasoning or long-form writing, upgrade to an 8B or 13B model.',
      ...
      sections: {
        ...
        whatToExpect: {
          ...
          content: [
            ...
            '**Calidad** de `llama3.2:3b` notablemente inferior a GPT-4o o Claude Opus 4.8 en tareas complejas. Para resúmenes, preguntas y respuestas simples y explicación de código, el resultado es útil. Para razonamiento en múltiples pasos o escritura de formato largo, actualiza a un modelo 8B o 13B.',
...
schema: {
      ...
      sections: {
        ...
        whatToExpect: { id: 'what-to-expect', title: '预期效果：速度、质量与限制', content: ['**速度**因硬件而异。2023款笔记本电脑（无GPU）：3B模型预计15-25个令牌/秒，8B模型预计8-15个令牌/秒。Apple M3 Pro：8B模型50-80个令牌/秒。NVIDIA RTX 4070 Ti：8B模型90-130个令牌/秒。', '**质量**：`llama3.2:3b`在复杂任务上明显低于GPT-4o或Claude Opus 4.8。对于摘要、简单问答和代码解释，输出有实用价值。对于多步推理或长篇写作，升级到8B或13B模型。', '**上下文窗口**：`llama3.2:3b`在Ollama中默认支持128K令牌。实际上，单次对话超过约16K令牌后质量会下降。', '**首次响应延迟**：`ollama run`后的首次响应包含模型加载时间（5-30秒）。同一会话中的后续响应更快。'] },
