...
import { EDGE_AI_PUBLISHED_SLUGS, EDGE_AI_HUB_PUBLISHED } from '@/lib/edge-ai/published'
...
export const metadata = contentMetadata as unknown as {
  ...
  'edge-ai': Record<string, ClusterMetadata>
...
export const EXCLUDED_PATH_PREFIXES = [
  ...
  '/edge-ai', // Edge AI cluster: gated by EDGE_AI_PUBLISHED_SLUGS/EDGE_AI_HUB_PUBLISHED; published paths are whitelisted back in below
]
...
export function getImagesForPage(path: string): Array<{ url: string; title?: string }> {
  ...
  if (path.startsWith('/edge-ai/')) {
    const slug = path.replace('/edge-ai/', '')
    const key = EDGE_AI_SLUG_TO_KEY[slug]
    if (key) return extractImagesFromArticle(metadata['edge-ai'], key)
  }
...
export function availableLangsForPath(path: string): readonly string[] | null {
  const clusters: Array<[string, Record<string, ClusterMetadata>, Record<string, string>]> = [
    ...
    ['/edge-ai/', metadata['edge-ai'], EDGE_AI_SLUG_TO_KEY],
  ]
...
const EDGE_AI_PUBLISHED_PATHS: ReadonlySet<string> = new Set([
  ...(EDGE_AI_HUB_PUBLISHED ? ['/edge-ai'] : []),
  ...Array.from(EDGE_AI_PUBLISHED_SLUGS).map(slug => `/edge-ai/${slug}`),
])
...
export function isExcluded(path: string): boolean {
  ...
  if (EDGE_AI_PUBLISHED_PATHS.has(path)) return false
