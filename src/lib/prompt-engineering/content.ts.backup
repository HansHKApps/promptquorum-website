    ...
    ja: {
      ...
      sections: {
        ...
        whyMatters: {
          ...
          content: [
            '5ブロックモデルは、OpenAI・Google・Anthropic・独立した実践者からのプロンプトエンジニアリングガイダンスにわたる収束したコンセンサスを反映しています。役割・指示・例・制約・出力形式は、2023年以降に発表されたすべての主要フレームワークに — 異なる名称で — 登場します。これは偶然ではありません：確率的なモデルが有用で一貫した結果を生み出すために必要な最小限の情報です。',
        ...
        block2: {
          ...
          items: [
            ...
            '✅ 強い：「非技術系マネージャー向けのプロンプトエンジニアリングのメリットについて、150語のLinkedIn投稿を書いてください」',
            '❌ 弱い：「このデータを分析してください」',
            '✅ 強い：「このデータセットの上位3つのトレンドを特定し、収益への影響度の高い順に並べてください」',
          ],
        },
        ...
        templateNote: {
          content: 'このテンプレートはGPT-4o・Claude・Gemini・OllamaやLM Studio経由のローカルLLMで機能します。ブロックの順序は推奨であり、厳格なルールではありません — ただし、役割とコンテキストを最初に、出力形式を最後に配置することが、すべての主要モデルで最も一般的で信頼性の高い配置です。',
        },
