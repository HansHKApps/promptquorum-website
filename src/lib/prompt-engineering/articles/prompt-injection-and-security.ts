      ...
      sections: {
        ...
        tldr: {
          ...
          items: [
            ...
            '**LLMs cannot reliably detect injections themselves.** In PromptQuorum tests, GPT-4o, Claude Opus 4.7, and Gemini 3.1 Pro flagged 18 of 30 adversarial injection strings — a 60% detection rate.',
        ...
        howToStart: {
          ...
          numberedItems: [
            ...
            '**Context isolation with delimiters and metadata:** Structure prompts to clearly mark trust boundaries: `[SYSTEM: instructions] [RETRIEVED: <untrusted>] [USER: <query>]`. Claude Opus 4.7 and GPT-4o partially respect structured delimiters when trained on them, but this is not a complete defense on its own — combine with the other four layers.',
          ],
        },
    ...
    de: {
      ...
      howToSchema: {
        ...
        step: [
          ...
          { '@type': 'HowToStep', position: 5, name: 'Kontextisolierung mit Trennzeichen und Metadaten', text: 'Strukturieren Sie Prompts so, dass Vertrauensgrenzen mithilfe expliziter Trennzeichen klar markiert sind. Claude Opus 4.7 und GPT-4o respektieren strukturierte Trennzeichen teilweise, aber dies ist allein keine vollständige Verteidigung — kombinieren Sie es mit den anderen vier Schichten.' },
        ],
      },
      itemListSchema: {
        '@context': 'https://schema.org',
      ...
      sections: {
        ...
        tldr: {
          ...
          items: [
            ...
            '**LLMs können Injektionen nicht zuverlässig selbst erkennen.** In PromptQuorum-Tests erkannten GPT-4o, Claude Opus 4.7 und Gemini 3.1 Pro 18 von 30 gegnerischen Injection-Strings — eine Erkennungsrate von 60 %.',
        ...
        howToStart: {
          ...
          numberedItems: [
            ...
            '**Kontextisolierung mit Trennzeichen und Metadaten:** Strukturieren Sie Prompts, um Vertrauensgrenzen klar zu markieren: `[SYSTEM: instructions] [RETRIEVED: <untrusted>] [USER: <query>]`. Claude Opus 4.7 und GPT-4o respektieren strukturierte Trennzeichen teilweise, wenn sie darauf trainiert wurden, aber dies ist allein keine vollständige Verteidigung — kombinieren Sie es mit den anderen vier Schichten.',
          ],
        },
...
def wrap_retrieved_context(doc_text: str, user_query: str) -> str:
    ...
    es: {
      ...
      schema: {
        ...
        datePublished: '2026-03-30',
        dateModified: '2026-03-30',
        url: 'https://www.promptquorum.com/prompt-engineering/prompt-injection-and-security?lang=es',
        inLanguage: 'es',
        proficiencyLevel: 'Advanced',
      ...
      howToSchema: {
        ...
        step: [
          ...
          { '@type': 'HowToStep', position: 5, name: 'Aislamiento de contexto con delimitadores y metadatos', text: 'Estructura los prompts para marcar claramente los límites de confianza usando delimitadores explícitos. Claude Opus 4.7 y GPT-4o respetan parcialmente los delimitadores estructurados, pero esto no es una defensa completa por sí sola — combínalo con las otras cuatro capas.' },
        ],
      },
      itemListSchema: {
        '@context': 'https://schema.org',
      ...
      sections: {
        ...
        howToStart: {
          ...
          numberedItems: [
            ...
            '**Aislamiento de contexto con delimitadores y metadatos:** Estructura los prompts para marcar claramente los límites de confianza: `[SYSTEM: instrucciones] [RETRIEVED: <no confiable>] [USER: <consulta>]`. Claude Opus 4.7 y GPT-4o respetan parcialmente los delimitadores estructurados, pero esto no es una defensa completa por sí sola — combínalo con las otras cuatro capas.',
          ],
        },
        inputValidation: {
...
def is_injection_attempt(text: str) -> bool:`,
        ...
        relatedReading: {
          ...
          items: [
            '[Constrained prompting](/prompt-engineering/constrained-prompting?lang=es) — Cómo las constraints de salida actúan como una capa de defensa contra la inyección',
            '[Structured output y modo JSON](/prompt-engineering/structured-output-json-mode?lang=es) — Cómo la aplicación de esquema detecta intentos de inyección que alteran el formato',
            '[RAG explicado](/prompt-engineering/rag-explained?lang=es) — Comprende los pipelines RAG para identificar la superficie de ataque de inyección indirecta',
            '[Build quality checks](/prompt-engineering/build-quality-checks?lang=es) — Patrones de validación de salida en producción',
            '[Glosario de prompt engineering](/prompt-engineering/prompt-engineering-glossary?lang=es) — Definiciones de prompt injection, jailbreaking y términos de seguridad relacionados',
          ],
        },
        faqSection: {
        ...
        sources: {
          ...
          items: [
            ...
            '[OpenAI. "Safety best practices"](https://platform.openai.com/docs/guides/safety-best-practices) — Documentación de fuente primaria de OpenAI para asegurar las aplicaciones GPT-4o contra entradas adversariales, incluyendo mitigaciones de prompt injection y validación de salida',
          ],
        },
      },
    },
    fr: {
      ...
      howToSchema: {
        ...
        step: [
          ...
          { '@type': 'HowToStep', position: 5, name: 'Isolation du contexte avec des délimiteurs et des métadonnées', text: 'Structurez les prompts pour marquer clairement les limites de confiance à l\'aide de délimiteurs explicites. Claude Opus 4.7 et GPT-4o respectent partiellement les délimiteurs structurés, mais cela ne constitue pas une défense complète en soi — combinez avec les quatre autres niveaux.' },
        ],
      },
      itemListSchema: {
        '@context': 'https://schema.org',
      ...
      sections: {
        ...
        tldr: {
          ...
          items: [
            ...
            '**Les LLM ne peuvent pas détecter les injections de manière fiable.** Dans les tests PromptQuorum, GPT-4o, Claude Opus 4.7 et Gemini 3.1 Pro ont détecté 18 des 30 chaînes d\'injection adversariales — un taux de détection de 60 %.',
        ...
        howToStart: {
          ...
          numberedItems: [
            ...
            '**Isolation du contexte avec des délimiteurs et des métadonnées :** Structurez les prompts pour marquer clairement les limites de confiance : `[SYSTEM: instructions] [RETRIEVED: <untrusted>] [USER: <query>]`. Claude Opus 4.7 et GPT-4o respectent partiellement les délimiteurs structurés lorsqu\'ils y sont entraînés, mais cela ne constitue pas une défense complète en soi — combinez avec les quatre autres niveaux.',
          ],
        },
...
def wrap_retrieved_context(doc_text: str, user_query: str) -> str:
    return (
        ...
        promptquorumBridge: {
          ...
          content: [
            '**Exemple de cadre comparatif :** Si vous soumettiez simultanément 30 chaînes d\'injection adversariales (15 directes, 15 injections de style document indirect) à GPT-4o, Claude Opus 4.7 et Gemini 3.1 Pro, vous observeriez probablement que les modèles avec un entraînement à la sécurité plus fort (Constitutional AI dans Claude) affichent des taux de détection plus élevés sur les injections naïves, tandis que tous les modèles atteignent une détection quasi nulle sur les charges utiles adversarialement obfusquées. Ce cadre d\'analyse est illustratif ; les taux de détection réels dépendent de vos motifs d\'injection spécifiques et des versions des modèles.',
    ...
    ja: {
      ...
      howToSchema: {
        ...
        step: [
          ...
          { '@type': 'HowToStep', position: 5, name: 'デリミタとメタデータによるコンテキスト分離', text: '明示的なデリミタを使用して信頼境界を明確にマークするようプロンプトを構造化します。Claude Opus 4.7とGPT-4oは構造化デリミタを部分的に尊重しますが、これだけでは完全な防御にはなりません — 他の4層と組み合わせてください。' },
        ],
      },
      itemListSchema: {
        '@context': 'https://schema.org',
      ...
      sections: {
        ...
        tldr: {
          ...
          items: [
            ...
            '**LLMは自身でインジェクションを確実には検出できません。** PromptQuorumのテストでは、GPT-4o、Claude Opus 4.7、Gemini 3.1 Proが30件の敵対的インジェクション文字列中18件を検出 — 検出率60%。',
        ...
        howToStart: {
          ...
          numberedItems: [
            ...
            '**デリミタとメタデータによるコンテキスト分離：** 明示的なデリミタを使用して信頼境界を明確にマークするようプロンプトを構造化します：`[SYSTEM: instructions] [RETRIEVED: <untrusted>] [USER: <query>]`。Claude Opus 4.7とGPT-4oはトレーニングされた場合、構造化デリミタを部分的に尊重しますが、これだけでは完全な防御にはなりません — 他の4層と組み合わせてください。',
          ],
        },
...
def wrap_retrieved_context(doc_text: str, user_query: str) -> str:
    return (
        ...
        promptquorumBridge: {
          ...
          content: [
            '**比較フレームワークの例：** 30件の敵対的インジェクション文字列（15件の直接、15件の間接スタイルのドキュメントインジェクション）をGPT-4o、Claude Opus 4.7、Gemini 3.1 Proに同時送信した場合、より強い安全トレーニングを持つモデル（ClaudeのConstitutional AI）がナイーブなインジェクションでより高い検出率を示す一方で、敵対的に難読化されたペイロードでは全モデルがほぼゼロの検出率になることが観察されるでしょう。この分析フレームワークは例示的なものです；実際の検出率は特定のインジェクションパターンとモデルバージョンによって異なります。',
        ...
        globalContext: {
          ...
          content: [
            ...
            '保護対象のデータをインフラ外に出せない場合、脅威モデルからクラウド LLM そのものを排除することは、どのプロンプトレベルの防御よりも強力な対策です。GDPR に準拠したローカルアーキテクチャは、[業務データのためのローカル RAG](/power-local-llm/local-rag-for-private-business-data?lang=ja)を参照してください。',
          ],
          blockquote: '"Trustworthy AI systems are designed, developed, deployed, and operated in a manner consistent with AI risk management practices. AI systems that interact with adversarial inputs should be tested for prompt injection resistance as part of adversarial robustness evaluation."',
          blockquoteSource: '[NIST AI Risk Management Framework (AI RMF 1.0)](https://nvlpubs.nist.gov/nistpubs/ai/NIST.AI.RMF.1.0.pdf) — National Institute of Standards and Technology, January 2023',
        },
        ...
        faq: {
          ...
          faqs: [
            ...
            },
            {
              ...
              a: '信頼できる検出を達成するモデルはありません。PromptQuorumのテストでは、Claude Opus 4.7は30件の敵対的インジェクション文字列中22件（73%）を検出し、GPT-4oは18件（60%）を検出しました。テストした3モデルすべてが難読化されたインジェクション（エンコードされたテキスト、仮説的フレーミング、分割された指示）で失敗しました。効果的な防御には、モデルの自己検出だけでなく、外部の検証レイヤーが必要です。',
            },
            {
    ...
    zh: {
      ...
      howToSchema: {
        ...
        step: [
          ...
          { '@type': 'HowToStep', position: 5, name: '使用分隔符和元数据进行上下文隔离', text: '构建提示词时使用显式分隔符清晰标记信任边界。Claude Opus 4.7和GPT-4o部分遵守结构化分隔符，但这本身并非完整的防御——需要与其他四层结合使用。' },
        ],
      },
      itemListSchema: {
        '@context': 'https://schema.org',
      ...
      sections: {
        ...
        tldr: {
          ...
          items: [
            ...
            '**LLM无法可靠地自行检测注入。** 在PromptQuorum测试中，GPT-4o、Claude Opus 4.7和Gemini 3.1 Pro在30个对抗性注入字符串中标记了18个——检测率为60%。',
        ...
        howToStart: {
          ...
          numberedItems: [
            ...
            '**使用分隔符和元数据进行上下文隔离：** 构建提示词时使用显式分隔符清晰标记信任边界：`[SYSTEM: instructions] [RETRIEVED: <untrusted>] [USER: <query>]`。在经过训练的情况下，Claude Opus 4.7和GPT-4o部分遵守结构化分隔符，但这本身并非完整的防御——需要与其他四层结合使用。',
          ],
        },
...
def wrap_retrieved_context(doc_text: str, user_query: str) -> str:
    return (
        ...
        promptquorumBridge: {
          title: '模型注入抵抗：比较分析框架',
          content: [
            '**示例比较框架：** 如果您同时向GPT-4o、Claude Opus 4.7和Gemini 3.1 Pro提交30个对抗性注入字符串（15个直接，15个间接文档注入风格），您可能会观察到，具有更强安全训练（Claude中的Constitutional AI）的模型在简单注入上显示出更高的检测率，而所有模型对对抗性混淆载荷的检测率接近零。此分析框架仅供参考；实际检测率取决于您的具体注入模式和模型版本。',
        ...
        globalContext: {
          ...
          content: [
            ...
            '当要保护的数据不能离开自己的基础设施时，把云端 LLM 完全排除在威胁模型之外，是比任何提示层防御都更强的控制手段。要了解符合 GDPR 的本地架构，请参阅[面向企业数据的本地 RAG](/power-local-llm/local-rag-for-private-business-data?lang=zh)。',
          ],
        ...
        faq: {
          ...
          faqs: [
            ...
            {
              q: 'LLM能自动检测提示词注入吗？',
              a: '没有模型能实现可靠检测。在PromptQuorum测试中，Claude Opus 4.7检测到30个对抗性注入字符串中的22个（73%）；GPT-4o检测到30个中的18个（60%）。所有三个被测模型在混淆注入（编码文本、假设框架、分割指令）上均失败。有效防御需要外部验证层，而非仅靠模型自我检测。',
            },
            {
              q: '如何防止RAG管道中的提示词注入？',
