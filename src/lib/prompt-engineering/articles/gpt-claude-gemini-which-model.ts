...
export const article: Partial<Record<Language, PEArticle>> = {
  ...
  ko: {
    ...
    faqSchema: {
      ...
      'inLanguage': 'ko',
      'mainEntity': [
        { '@type': 'Question', 'name': '구독을 하나만 결제할 수 있다면 무엇을 선택해야 합니까?', 'acceptedAnswer': { '@type': 'Answer', 'text': 'Claude Opus 5으로 시작하십시오. 작문, 추론, 코드 전반에서 품질이 가장 높습니다. 주된 필요가 도구 통합과 멀티모달이라면 GPT-5.6를 선택하십시오. Google Workspace 중심 팀이고 비용이 중요하다면 Gemini를 선택하십시오. 사용자가 중국 본토에 있다면 선택지가 없으며 DeepSeek 또는 Baidu ERNIE를 사용해야 합니다(지연 시간과 컴플라이언스상 필수).' } },
        { '@type': 'Question', 'name': '모델 선택은 얼마나 자주 재검토해야 합니까?', 'acceptedAnswer': { '@type': 'Answer', 'text': '분기마다 검토하십시오. 3~4개월마다 새 모델이 출시되고 리더보드 순위도 바뀝니다. PromptQuorum으로 가장 중요한 작업을 최신 모델에서 다시 테스트하십시오. 6개월 전에 최적이던 선택이 지금도 최적이라는 보장은 없습니다.' } },
        { '@type': 'Question', 'name': '하나의 제품이나 에이전트에서 여러 모델을 함께 사용할 수 있습니까?', 'acceptedAnswer': { '@type': 'Answer', 'text': '가능하며, 그렇게 하는 편이 좋습니다. 작업 유형별로 모델을 배분하십시오. 작문은 Claude, 검색은 Gemini, 에이전트는 GPT입니다. 조건 분기를 사용하십시오. 작문 작업이면 Claude, 검색 작업이면 Gemini와 같은 식입니다. 실제 프로덕션 시스템이 바로 이렇게 동작합니다.' } },
        { '@type': 'Question', 'name': '벤더 종속은 어떻게 바라봐야 합니까?', 'acceptedAnswer': { '@type': 'Answer', 'text': '벤더 종속은 시스템이 특정 모델의 API 형식, 고유 기능, 가격 체계에 의존할 때 발생합니다. 대응 방법은 네 가지입니다. (1) 모델 간에 공통으로 통하는 표준 프롬프트 구조를 사용하십시오. (2) 여러 제공업체를 지원하는 추상화 계층(PromptQuorum 등)을 사용하십시오. (3) 여러 모델에서 정기적으로 테스트하여 벤더별 편차를 감지하십시오. (4) 핵심 시스템에는 로컬 모델(Ollama, LM Studio)을 폴백으로 두십시오.' } },
        { '@type': 'Question', 'name': '오픈소스 로컬 모델은 어디에 해당합니까?', 'acceptedAnswer': { '@type': 'Answer', 'text': '로컬 모델(Llama 4 Scout, Qwen3.6, Mistral Small 4, Ollama 또는 LM Studio를 통한 기타 모델)은 다음에 최적입니다: 고용량 반복 작업(분류, 요약, 추출), 비공개 데이터(API 호출 없음), 비용에 민감한 업무, 그리고 API 비용을 확정하기 전 테스트입니다. 품질에서 프런티어 모델에 미치지 못하지만 프라이버시와 비용에서 우수합니다. 프런티어급 추론이 필요 없는 80%의 작업에 사용하십시오.' } },
        ...
        { '@type': 'Question', 'name': '가장 정확한 AI 모델은 무엇입니까?', 'acceptedAnswer': { '@type': 'Answer', 'text': '모든 작업에서 가장 정확한 단일 모델은 없습니다. 작문과 구조화된 분석에서는 Claude Opus 5이 앞서고, 도구 통합형 추론에서는 GPT-5.6가 앞서며, 실시간 웹 그라운딩을 활용한 장문 리서치에서는 Gemini 3.1 Pro가 앞섭니다. 정확도는 작업에 따라 달라지며, 유일하게 믿을 만한 검증은 실제 프롬프트를 모든 모델에서 실행하고 결과를 측정하는 것입니다.' } },
