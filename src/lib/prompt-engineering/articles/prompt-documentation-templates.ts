...
const OG_SLUG = keyToSlug('prompt-documentation-templates');
...
export const article: Partial<Record<Language, PEArticle>> = {
    ...
    sections: {
      ...
      six_templates: {
        ...
        numberedItems: [
          ...
          { title: 'API Config Block', whyItMatters: 'Purpose: record the model parameters used in production. Fields: model (e.g., GPT-5.6, Claude Sonnet 5), temperature, max tokens, top_p, stop sequences, system prompt version, user prompt version. Use when: a prompt is deployed to production. Storage: deployment config file, referenced in version block.' },
        ],
        callouts: [
  ...
  ar: {
    ...
    seoTitle: 'قوالب توثيق البرومبتات: ⁨6⁩ تنسيقات لتوثيق الفرق',
    ...
    metaDescription: '⁨6⁩ قوالب لتوثيق البرومبتات: بطاقة السطر الواحد، كتلة الإصدار، رأس مجموعة الاختبار، سجل القرارات، تهيئة ⁨API⁩. أقل من ⁨10⁩ دقائق.',
    ...
    sections: {
      ...
      six_templates: {
        ...
        numberedItems: [
          ...
          { title: 'كتلة تهيئة API', whyItMatters: 'الغرض: تسجيل معاملات النموذج المستخدمة في الإنتاج. الحقول: النموذج (مثلاً GPT-5.6، Claude Sonnet 5)، درجة الحرارة، الحد الأقصى للرموز، top_p، تسلسلات الإيقاف، إصدار أمر النظام، إصدار أمر المستخدم. متى تستخدمه: عند نشر أمر في الإنتاج. التخزين: ملف تهيئة النشر، مشار إليه من كتلة الإصدار.' },
        ],
        callouts: [
  ...
  pt: {
    ...
    schema: {
      '@context': 'https://schema.org',
      '@type': 'TechArticle',
      url: 'https://www.promptquorum.com/pt/prompt-engineering/prompt-documentation-templates',
      inLanguage: 'pt-BR',
    ...
    sections: {
      ...
      six_templates: {
        ...
        numberedItems: [
          ...
          { title: 'Bloco de Config API', whyItMatters: 'Propósito: registrar os parâmetros do modelo usados em produção. Campos: modelo (por exemplo, GPT-5.6, Claude Sonnet 5), temperatura, max tokens, top_p, stop sequences, versão do system prompt, versão do user prompt. Use quando: um prompt é implantado em produção. Armazenamento: arquivo de configuração de implantação, referenciado no bloco de versão.' },
        ],
        callouts: [
  ...
  sections: {
    ...
    six_templates: {
      ...
      numberedItems: [
        ...
        { title: 'API Config Block', whyItMatters: '목적: 프로덕션에서 사용된 모델 파라미터를 기록합니다. 필드: 모델(예: GPT-5.6, Claude Sonnet 5), temperature, max tokens, top_p, stop sequences, 시스템 프롬프트 버전, 사용자 프롬프트 버전. 사용 시기: 프롬프트가 프로덕션에 배포될 때. 저장: 배포 구성 파일, Version Block에서 참조.' },
      ],
      callouts: [
