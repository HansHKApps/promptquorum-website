...
export const article: Partial<Record<Language, PEArticle>> = {
    en: {
      ...
      sections: {
        ...
        howPQHelps: {
          ...
          content: [
            '**PromptQuorum is a multi-model AI dispatch tool that lets you test zero shot and few shot prompts across multiple providers in one place.** You can send the same instruction-only prompt and the same example-augmented prompt to models such as GPT-5.5, Claude Opus 4.8, and Gemini 3.1 Pro side by side.',
  ...
  sections: {
    ...
    howPQHelps: {
      ...
      content: [
        '**PromptQuorum ist ein Multi-Model-AI-Dispatch-Tool, mit dem Sie Zero-Shot- und Few-Shot-Prompts an mehreren Anbietern an einem Ort testen können.** Sie können denselben Anweisungs-Only-Prompt und denselben beispielerweiterten Prompt nebeneinander an Modelle wie GPT-5.5, Claude Opus 4.8 und Gemini 3.1 Pro senden.',
  ...
  educationalLevel: 'Intermediate',
  primaryTerm: 'Zero-Shot y Few-Shot Prompting',
  ...
  sections: {
    ...
    keyDifferences: {
      ...
      rows: [
        ...
        { 'Dimensión': 'Velocidad de configuración', 'Zero-Shot': 'Muy rápida; no requiere selección de ejemplos', 'Few-Shot': 'Más lenta; los ejemplos deben seleccionarse y mantenerse' },
        { 'Dimensión': 'Requisitos de datos', 'Zero-Shot': 'No se necesitan ejemplos etiquetados', 'Few-Shot': 'Requiere al menos algunos ejemplos etiquetados' },
        { 'Dimensión': 'Precisión en tareas específicas', 'Zero-Shot': 'A menudo menor o más genérica', 'Few-Shot': 'Generalmente mayor y más consistente en dominios específicos' },
        { 'Dimensión': 'Escalabilidad entre tareas', 'Zero-Shot': 'Muy escalable, fácil de agregar nuevas tareas', 'Few-Shot': 'Menos escalable; cada tarea puede necesitar sus propios ejemplos' },
      ],
    },
    whenToUseZeroShot: {
  ...
  schema: {
    '@context': 'https://schema.org',
    '@type': 'TechArticle',
    headline: 'الطلبات Zero-Shot مقابل Few-Shot',
    description: 'مقارنة عملية بين الطلبات Zero-Shot وFew-Shot، ومتى تستخدم كل أسلوب، وكيف يساعدك PromptQuorum على اختيار الاستراتيجية الصحيحة.',
    datePublished: '2026-03-26',
    dateModified: '2026-03-26',
    keywords: ['Zero-Shot Prompting', 'Few-Shot Prompting', 'هندسة الطلبات', 'PromptQuorum'],
    author: { '@type': 'Person', name: 'Hans Kuepper', url: 'https://www.promptquorum.com/about' },
    publisher: { '@type': 'Organization', name: 'PromptQuorum', url: 'https://www.promptquorum.com' },
  ...
  sections: {
    ...
    whatIsFewShot: {
      title: 'ما هي الطلبات Few-Shot؟',
      content: [
        '**تُضيف الطلبات Few-Shot عددًا صغيرًا من أمثلة المدخلات والمخرجات إلى التعليمات حتى يستطيع النموذج استنتاج نمط المهمة من عروض ملموسة.** عمليًا، يعني Few-Shot عادةً ما بين مثالين وعشرة أمثلة.',
        'تعمل هذه الأمثلة كمجموعة تدريب مصغّرة داخل الطلب، توجّه النموذج في كيفية تفسير المهام الغامضة والتنسيقات المتخصصة واللغة الخاصة بالمجال. تكون الطلبات Few-Shot مفيدة بشكل خاص عندما تحتاج إلى أسلوب أو مخطط أو سلوك محدد لا تستطيع التعليمات العامة التقاطه.',
      ],
    },
    keyDifferences: {
      title: 'الفروقات الرئيسية: Zero-Shot مقابل Few-Shot',
      content: [
        '**تختلف الطلبات Zero-Shot وFew-Shot أساسًا في جهد الإعداد ودقة المهام المتخصصة وقابلية التوسع عبر حالات الاستخدام المتعددة.** كلاهما يعتمد على النموذج الأساسي ذاته، لكنهما يتبادلان جهد تصميم الأمثلة مقابل توافق أفضل مع المهمة.',
      ],
      columns: ['البُعد', 'Zero-Shot', 'Few-Shot'],
      rows: [
        { 'البُعد': 'أمثلة في الطلب', 'Zero-Shot': 'لا يوجد', 'Few-Shot': '٢–١٠+ أمثلة تمثيلية' },
        { 'البُعد': 'سرعة الإعداد', 'Zero-Shot': 'سريعة جدًا؛ لا تحتاج إلى اختيار أمثلة', 'Few-Shot': 'أبطأ؛ يجب اختيار الأمثلة وصيانتها' },
        { 'البُعد': 'متطلبات البيانات', 'Zero-Shot': 'لا تحتاج إلى أمثلة مُعلَّمة', 'Few-Shot': 'تتطلب بعض الأمثلة المُعلَّمة على الأقل' },
        { 'البُعد': 'دقة المهام المتخصصة', 'Zero-Shot': 'غالبًا أقل دقة أو أعم', 'Few-Shot': 'عادةً أعلى دقة وأكثر اتساقًا في المجالات المتخصصة' },
        { 'البُعد': 'قابلية التوسع بين المهام', 'Zero-Shot': 'قابل للتوسع بدرجة عالية، سهل إضافة مهام جديدة', 'Few-Shot': 'أقل قابلية للتوسع؛ قد تحتاج كل مهمة إلى أمثلتها الخاصة' },
      ],
    },
    whenToUseZeroShot: {
      title: 'متى تستخدم Zero-Shot؟',
      content: [
        '**استخدم الطلبات Zero-Shot عندما تحتاج إلى السرعة وليس لديك أمثلة مُعلَّمة ومهمتك عامة بصورة معقولة.** يعمل هذا النمط بشكل جيد كمرور أول أو خط أساس.',
        'سيناريوهات Zero-Shot الشائعة:',
      ],
      items: [
        'الأسئلة والأجوبة العامة والتلخيصات البسيطة وتصنيف المشاعر الأساسي.',
        'التجريب السريع عندما لا تزال تستكشف شكل المهمة.',
        'المجالات أو اللغات الجديدة التي لا تملك فيها أمثلة منظّمة.',
      ],
    },
    whenToUseFewShot: {
      title: 'متى تستخدم Few-Shot؟',
      content: [
        '**استخدم الطلبات Few-Shot عندما تكون المهمة متخصصة أو حساسة للتنسيق أو عالية المخاطر، ويمكنك تقديم أمثلة جيدة.** في هذه الحالات، تُحسّن الأمثلة الموثوقية بشكل ملحوظ مقارنةً بالتعليمات الخالصة.',
        'سيناريوهات Few-Shot الشائعة:',
      ],
      items: [
        'التصنيف أو الاستخراج الخاص بالمجال (القانوني والطبي والمالي) حيث تهمّ التسميات والصياغة الدقيقة.',
        'المهام ذات المخططات الصارمة، كاستخراج JSON منظّم من نص فوضوي.',
        'المهام متعددة اللغات أو الترجمة حيث تساعد أمثلة قليلة لكل لغة في التعامل مع التعابير الاصطلاحية والأسلوب.',
      ],
    },
    example: {
      title: 'مثال: طلب Zero-Shot مقابل Few-Shot',
      content: [
        '**يتضح الفرق العملي بين Zero-Shot وFew-Shot بجلاء عند مقارنة الطلبات للمهمة ذاتها.** هنا نصنّف تذاكر الدعم حسب النية.',
        '**[طلب سيء – بلا هيكل]**',
        '"انظر إلى تذكرة الدعم هذه وأخبرني عم تتحدث."',
        '**[طلب Zero-Shot]**',
        '"صنّف تذكرة الدعم التالية ضمن إحدى هذه الفئات: `billing_issue` أو `login_problem` أو `feature_request` أو `bug_report` أو `other`. التذكرة: "حاولت إعادة تعيين كلمة المرور ثلاث مرات اليوم والرابط يقول دائمًا إنه انتهت صلاحيته." أعِد اسم الفئة فقط."',
        '**[طلب Few-Shot]**',
        '"صنّف كل تذكرة دعم ضمن إحدى هذه الفئات: `billing_issue` أو `login_problem` أو `feature_request` أو `bug_report` أو `other`. أعِد اسم الفئة فقط. مثال ١: التذكرة: "لقد فُرِضت عليّ رسوم مرتين هذا الشهر على نفس الاشتراك." التسمية: `billing_issue` مثال ٢: التذكرة: "كلما نقرت على \'تصدير التقرير\' لا يحدث شيء حتى بعد تحديث الصفحة." التسمية: `bug_report` مثال ٣: التذكرة: "هل يمكنكم إضافة دعم لتصدير التقارير مباشرةً إلى Google Sheets؟" التسمية: `feature_request` الآن صنّف هذه التذكرة: "حاولت إعادة تعيين كلمة المرور ثلاث مرات اليوم والرابط يقول دائمًا إنه انتهت صلاحيته."',
        'يعرض إصدار Few-Shot النمط بشكل صريح، مما يُحسّن عادةً جودة التصنيف للتذاكر الغامضة أو كثيرة الضوضاء.',
      ],
    },
    howPQHelps: {
      title: 'كيف يساعدك PromptQuorum على الاختيار',
      content: [
        '**PromptQuorum هو أداة إرسال ذكاء اصطناعي متعددة النماذج تتيح لك اختبار الطلبات Zero-Shot وFew-Shot عبر مزودين متعددين في مكان واحد.** يمكنك إرسال الطلب ذاته بتعليمات فقط والطلب ذاته مع أمثلة إلى نماذج مثل GPT-5.5 وClaude Opus 4.8 وGemini 3.1 Pro في وقتٍ واحد.',
        'داخل PromptQuorum، يمكنك:',
      ],
      items: [
        'البدء بطلبات Zero-Shot باستخدام أطر عمل مثل Single Step وRTF وCO-STAR للحصول على خطوط أساس سريعة.',
        'الترقية إلى طلبات Few-Shot بدمج أمثلة تمثيلية في أطر مثل SPECS أو دليل الطلبات من Google عندما تحتاج إلى مزيد من التحكم.',
        'حفظ نسختَي Zero-Shot وFew-Shot كقوالب ومقارنة الدقة والكمون وتكاليف الرموز بين النماذج بمرور الوقت.',
      ],
    },
    howToStart: {
      title: 'كيف تختار بين الطلبات Zero-Shot وFew-Shot؟',
      numberedItems: [
        '**للمهام الروتينية والبسيطة، ابدأ بـ Zero-Shot (بدون أمثلة).** مثال: "صنّف هذه المراجعة على أنها إيجابية أو سلبية." إذا كانت الدقة كافية، فإن Zero-Shot أسرع وأقل تكلفة.',
        '**عندما يكون أداء Zero-Shot منخفضًا (< ٨٠٪ دقة أو جودة)، أضِف ٢–٥ أمثلة Few-Shot.** أرِ النموذج ٢–٣ مراجعات إيجابية و٢–٣ سلبية مع التسميات الصحيحة. يُعلّم Few-Shot بالأمثلة.',
        '**للمهام ذات التمييزات الدقيقة أو الأنماط النادرة، أضِف ٥–١٠ أمثلة (Few-Shot+).** إذا كانت مهمتك تتطلب اكتشاف السخرية أو التحيز الضار أو الفروق الدقيقة الخاصة بالمجال، تساعد المزيد من الأمثلة.',
        '**اختَر أمثلة تغطي نطاق المدخلات التي تتوقعها.** إذا كنت تصنّف مراجعات المنتجات، أدرج أمثلة متحمسة وفاترة وسلبية. لا تعرض الحالات السهلة فقط.',
  ...
  schema: {
    ...
    author: { '@type': 'Person', name: 'Hans Kuepper', url: 'https://www.promptquorum.com/about' },
    publisher: { '@type': 'Organization', name: 'PromptQuorum', url: 'https://www.promptquorum.com' },
    url: 'https://www.promptquorum.com/pt/pt/prompt-engineering/zero-shot-vs-few-shot',
    inLanguage: 'pt-BR',
  },
  sections: {
    whatIsZeroShot: {
  ...
  sections: {
    ...
    注意点: {
      ...
      items: [
        ...
        'モデルによってゼロショット vs. フューショットの反応が異なる場合があります。GPT-5.5はゼロショットで堅牢ですが、Gemini 3.1 Proはいくつかのドメインでフューショットの恩恵をより受けることがあります。',
      ],
    },
    howPQHelps: {
      title: 'PromptQuorumがどのように選択を支援するか',
      content: [
        '**PromptQuorumは、複数のプロバイダー間でゼロショットとフューショットプロンプトをテストできるマルチモデルAIディスパッチツールです。** 指示のみのプロンプトと例を拡張したプロンプトの両方を、GPT-5.5、Claude Opus 4.8、Gemini 3.1 Proなどのモデルに並行して送信できます。',
  ...
  sections: {
    ...
    howPQHelps: {
      ...
      content: [
        '**PromptQuorum 是一款多模型 AI 分派工具，让您在一个地方跨多个提供商测试零样本和少样本提示。** 您可以将相同的仅指令提示和相同的示例增强提示并行发送到 GPT-5.5、Claude Opus 4.8 和 Gemini 3.1 Pro 等模型。',
    ...
    实用建议: {
      ...
      items: [
        ...
        '在生产环境中部署前测试性能差异。',
        '比较不同模型间的零样本 vs. 少样本性能——GPT-5.5 可能在零样本中表现出色，而 Gemini 3.1 Pro 可能从少样本获益更多。',
      ],
    },
    howToStart: {
      title: '如何在零样本和少样本间选择',
