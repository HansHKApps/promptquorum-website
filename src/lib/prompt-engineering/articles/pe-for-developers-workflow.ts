...
export const article: Partial<Record<Language, PEArticle>> = {
  ...
  ar: {
    ...
    seoTitle: 'سير عمل البرومبت للمطوّرين: إعداد IDE وCI/CD',
  ...
  ko: {
    ...
    ogDescription: '개발자 프롬프트 워크플로우: Cursor 또는 VS Code IDE, 30초 로컬 테스트 루프, Git 버전 관리, GitHub Actions CI/CD 게이트, 프로덕션 품질 모니터링.',
    twitterDescription: '작성 → 30초 내 테스트 → 버전 관리 → CI/CD 게이트 → 모니터링. 5단계 개발자 프롬프트 워크플로우. Cursor + Promptfoo + Git.',
    publishDate: '2026-05-02',
    readTime: '12분 읽기',
    educationalLevel: 'Advanced',
    primaryTerm: '프롬프트 엔지니어링 워크플로우',
    leadAnswerBlock: '**개발자에게는 별도의 툴 생태계가 아닌, 버전 관리, CI/CD, 로컬 테스트 등 기존 개발 프로세스에 맞는 프롬프트 엔지니어링 워크플로우가 필요합니다.** 워크플로우는 5단계로 구성됩니다: 작성, 로컬 테스트, 버전 관리, CI/CD 게이트, 프로덕션 모니터링.',
    quickFacts: [
      'Promptfoo를 사용한 로컬 프롬프트 테스트 루프는 30초 미만입니다: 작성, 3개 입력으로 테스트, 기준선과 비교, 커밋.',
      '프롬프트를 /prompts 디렉터리에 .txt 또는 .ts 파일로 저장하십시오. [task]-[version].txt 형식으로 명명하십시오 (예: customer-support-v3.txt).',
      'CI/CD 게이트 임계값: 85% 통과율로 시작하고, 안정적인 테스트 3개월 후 95%로 상향하십시오.',
      'Cursor는 TypeScript/Python 개발자에게 권장되는 IDE입니다. 오픈소스/로컬 모델 요구사항에는 VS Code + Continue.dev를 사용하십시오.',
      '프로덕션에서 응답별로 프롬프트 식별자, 모델, 지연 시간, 토큰 수, 품질 점수를 기록하십시오.',
      '24시간 롤링 윈도우 기준 품질 점수가 10% 이상 하락하면 알림을 발송하십시오.',
    ],
    toc: [
      { label: '프롬프트 엔지니어링을 위한 IDE 설정', anchor: 'ide_setup' },
      { label: '로컬 프롬프트 테스트 루프', anchor: 'local_test_loop' },
      { label: '버전 관리에 프롬프트 저장하기', anchor: 'version_control' },
      { label: '프롬프트용 CI/CD 게이트', anchor: 'cicd_gates' },
      { label: '프롬프트 프로덕션 모니터링', anchor: 'production_monitoring' },
    ...
    schema: {
      '@context': 'https://schema.org',
      '@type': 'TechArticle',
      headline: '개발자를 위한 프롬프트 엔지니어링 워크플로우: IDE 설정, 테스트 및 CI/CD 통합',
      description: '개발자 프롬프트 엔지니어링 워크플로우: IDE 설정, 로컬 테스트 루프, Git 버전 관리, CI/CD 게이트, 프로덕션 모니터링.',
      url: 'https://www.promptquorum.com/ko/prompt-engineering/pe-for-developers-workflow',
      datePublished: '2026-05-02',
      author: { '@type': 'Person', name: 'Hans Kuepper', sameAs: 'https://www.promptquorum.com/about' },
      publisher: { '@type': 'Organization', name: 'PromptQuorum', url: 'https://www.promptquorum.com', logo: { '@type': 'ImageObject', url: 'https://www.promptquorum.com/logo.svg' } },
      image: { '@type': 'ImageObject', url: 'https://www.promptquorum.com/api/og/pe-for-developers-workflow', width: 1200, height: 630 },
      ...
      proficiencyLevel: 'Advanced',
      about: [
        { '@type': 'Thing', name: '프롬프트 엔지니어링 워크플로우' },
        { '@type': 'Thing', name: 'LLM용 CI/CD' },
    ...
    sections: {
      tldr: {
        ...
        title: 'TL;DR',
        isTldr: true,
        content: '개발자 프롬프트 엔지니어링 워크플로우는 5단계로 구성됩니다: Cursor 또는 VS Code에서 작성, 30초 미만으로 Promptfoo 로컬 테스트 루프 실행, /prompts에 .txt 또는 .ts 파일로 프롬프트 버전 관리, 85% 미만 통과율을 차단하는 GitHub Actions 워크플로우로 CI/CD에서 변경사항 게이팅, 그리고 10% 하락 시 알림을 통한 프로덕션 품질 모니터링. 여러 모델 간 테스트에는 PromptQuorum을 활용하십시오.',
      },
      ide_setup: {
        id: 'ide-setup',
        title: '프롬프트 엔지니어링을 위한 IDE 설정',
        snippets: [
          { type: 'in-one-sentence', text: 'Cursor와 Continue.dev가 탑재된 VS Code는 대부분의 개발자 프롬프트 엔지니어링 요구사항을 충족하는 두 가지 IDE로, Cursor는 클라우드 API 워크플로우에, Continue.dev는 오픈소스 및 로컬 모델 요구사항에 적합합니다.' },
          { type: 'in-plain-terms', text: '이미 가장 많은 시간을 보내는 IDE를 선택하십시오. TypeScript 또는 Python을 사용하고 클라우드 API(OpenAI, Anthropic, Google)를 호출한다면 Cursor가 가장 적은 마찰을 제공합니다. 모델을 로컬에서 실행해야 하거나 오픈소스 요구사항이 있다면 Continue.dev가 탑재된 VS Code가 적합합니다.' },
        ],
        content: [
      ...
      local_test_loop: {
        ...
        callouts: [
          { type: 'warning', label: '기준선 비교는 선택사항이 아닙니다', text: '기준선과 비교하지 않으면, 절대 임계값이 낮을 경우 엣지 케이스에서 품질이 저하된 프롬프트도 테스트를 "통과"할 수 있습니다. 항상 마지막으로 배포된 버전과 비교하십시오.' },
        ],
      },
      version_control: {
        id: 'version-control',
        title: '버전 관리에 프롬프트 저장하기',
        content: [
        ...
        callouts: [
          { type: 'insight', label: '프롬프트는 코드입니다', text: '프롬프트 파일을 코드 파일과 동일한 규율로 취급하십시오: PR 검토, 작성자 명시, 시맨틱 버전 관리, 그리고 절대 삭제하지 말고 /prompts/archive/로 이동하십시오.' },
        ],
      },
      cicd_gates: {
        id: 'cicd-gates',
        title: '프롬프트용 CI/CD 게이트',
        content: [
      ...
      faq: {
        ...
        title: '자주 묻는 질문',
        faqs: [
          { q: '프롬프트 엔지니어링에 가장 적합한 IDE는 무엇입니까?', a: 'Cursor는 주로 TypeScript 또는 Python으로 작업하며 프롬프트 파일을 일급 시민으로 취급하는 네이티브 AI 통합을 원하는 개발자에게 권장되는 IDE입니다. 로컬 모델 지원, 오픈소스 요구사항, 또는 Cursor에서 잘 지원되지 않는 언어 생태계에서 작업하는 경우 Continue.dev가 탑재된 VS Code를 권장합니다.' },
          { q: '버전 관리에 프롬프트를 어떻게 저장해야 합니까?', a: '리포지터리 루트의 /prompts 디렉터리에 .txt 또는 .ts 파일로 프롬프트를 저장하십시오. [task]-[version].txt 명명 규칙을 사용하십시오 (예: customer-support-v3.txt). 프롬프트 변경에는 컨벤셔널 커밋 메시지 형식을 사용하십시오 (feat:, fix:, refactor:). 프로덕션에 배포된 모든 버전에 Git 태그를 추가하십시오.' },
          { q: '프롬프트용 CI/CD 게이트를 어떻게 설정합니까?', a: '모든 풀 리퀘스트에서 Promptfoo 또는 Braintrust를 테스트 스위트에 실행하는 GitHub Actions 워크플로우 단계를 추가하십시오. 통과율이 임계값 이하로 떨어지면 빌드를 실패시키도록 단계를 구성하십시오 — 85%로 시작하고 3개월간 안정적인 테스트 후 95%로 상향하십시오. 통과율 임계값을 버전 관리되도록 리포지터리의 구성 파일에 저장하십시오.' },
      ...
      related_reading: {
        ...
        items: [
          ...
          { title: '최고의 프롬프트 테스트 도구', url: '/prompt-engineering/best-prompt-testing-tools' },
      ...
      sources: {
        ...
        title: '출처',
        items: [
          { title: 'Cursor IDE 문서', url: 'https://docs.cursor.com' },
          { title: 'Continue.dev 문서 (GitHub)', url: 'https://github.com/continuedev/continue' },
          { title: 'Promptfoo CLI 문서', url: 'https://www.promptfoo.dev/docs' },
        ],
      },
    },
  },
