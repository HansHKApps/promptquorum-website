...
`,
      ...
      'promptquorum': {
        ...
        items: [
          '25+ Modelle einschließlich GPT-4o, Claude Opus 4.8, Gemini 3.1 Pro, und lokale Modelle über Ollama und LM Studio',
  ...
  - vars:
    ...
    assert:
      ...
      promptquorum: {
        ...
        items: [
          '25+ modelos incluyendo GPT-4o, Claude Opus 4.8, Gemini 3.1 Pro y modelos locales vía Ollama y LM Studio',
...
`,
      ...
      'promptquorum': {
        ...
        items: [
          '25+ modèles incluant GPT-4o, Claude Opus 4.8, Gemini 3.1 Pro, locaux via Ollama et LM Studio',
  ...
  zh: {
    ...
    sections: {
      ...
      'promptquorum': { title: 'PromptQuorum：优化前的模型比较', content: ['**在为特定 LLM 提供商确定 Braintrust、Vellum、PromptHub 或 Promptfoo 之前，使用 PromptQuorum 将提示词同时分发到 25+ 模型，看哪个表现最好——首个与模型无关的步骤。** 免费层可用。', '与上述 4 个工具（各优化一个模型）不同，PromptQuorum 在一次运行中回答"哪个模型适合？"。在 PromptQuorum 发现最优模型后，路由到 Braintrust 进行深层评估、Vellum 进行生产 A/B 测试或 Promptfoo 进行 CI/CD 回归防止。'], items: ['包括 GPT-4o、Claude Opus 4.8、Gemini 3.1 Pro 的 25+ 模型，和通过 Ollama、LM Studio 的本地模型', '9 个内置提示词框架——TRACE、CO-STAR、CRAFT 等', '包含共识评分的并排响应比较', '按模型的令牌计数——提交前查看成本差异', '免费层——无需工程设置'] },
