  ...
  ja: {
    ...
    sections: {
      ...
      whyReview: {
        ...
        content: [
          '**レビューなしのプロンプトは本番環境で3倍の失敗率を示します。** APIにデプロイされたとき、ライブデータに対して実行されるとき、または本番環境トラフィックにスケーリングされるとき、分離された状態で機能するプロンプトが破損します。手動コードレビューは構文エラーを検出します。プロンプトレビューはロジックエラー、不足しているコンテキスト、自動テストだけでは検出できない[幻覚の出荷](/prompt-engineering/ai-hallucinations-how-to-stop?lang=ja)を検出します。',
