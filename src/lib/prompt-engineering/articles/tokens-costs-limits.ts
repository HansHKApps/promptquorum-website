...
export const article: Partial<Record<Language, PEArticle>> = {
  ...
  ko: {
      ...
      sections: {
        ...
        tokenCounting: {
          title: '토큰 계산 방식',
          content: '**API 호출의 모든 요소 — 시스템 프롬프트, 대화 기록, 새 메시지, 파일, 모델의 출력 — 은 할당량에서 토큰을 소비합니다.** 작은 메시지로 시작한 대화가 5번의 주고받기 후에 갑자기 비용이 높아지는 이유가 여기에 있습니다. 축적된 모든 것에 대해 비용을 지불합니다. [시스템 프롬프트와 사용자 프롬프트](/prompt-engineering/system-prompt-vs-user-prompt)의 차이를 이해하는 것이 중요합니다. 두 가지 모두 매 호출마다 청구되기 때문입니다.',
          items: [
