...
export const article: Partial<Record<Language, PEArticle>> = {
    en: {
      ...
      schema: {
        ...
        keywords: ['multimodal prompting', 'vision-language models', 'image prompting', 'GPT-5.6', 'Claude Opus 5', 'Gemini 3.5 Pro', 'prompt engineering', 'image analysis', 'text-to-image generation', 'image editing'],
      ...
      sections: {
        ...
        howToStart: {
          ...
          numberedItems: [
            ...
            '**Test across multiple models with PromptQuorum.** GPT-5.6 excels at scene understanding, Claude Opus 5 at text extraction, Gemini 3.5 Pro at long contexts. One model is rarely best for all tasks—compare outputs to find your best fit.',
  ...
  ko: {
    ...
    sections: {
      ...
      howVisionWorks: {
        ...
        content: [
          '**GPT-5.6, Claude Opus 5, Gemini 3.5 Pro와 같은 비전-언어 모델은 시각적 인코더를 사용하여 이미지를 고차원 벡터(임베딩)로 변환한 다음, 공유된 의미 공간에서 텍스트 토큰과 함께 해당 임베딩을 처리합니다.** 이 접근 방식은 VLM에게 여러 작업에 걸쳐 명확한 강점을 부여합니다. 객체를 식별하고 텍스트를 읽으며 공간적 관계를 이해하고 여러 이미지에 걸쳐 콘텐츠를 추론합니다. Gemini 3.5 Pro는 최대 100만 토큰을 지원하여 GPT-5.6의 128k 컨텍스트 창보다 긴 멀티모달 시퀀스를 분석할 수 있습니다. [컨텍스트 창 한계](/prompt-engineering/context-windows-explained-why-ai-forgets)를 이해하면 긴 이미지 설명이나 다중 이미지 시퀀스 작업 시 잘림을 방지하는 프롬프트를 구성하는 데 도움이 됩니다.',
      ...
      promptquorumHelps: {
        ...
        content: [
          '**PromptQuorum은 GPT-5.6, Claude Opus 5, Gemini 3.5 Pro 및 기타 모델에 걸쳐 멀티모달 프롬프트를 동시에 테스트할 수 있는 다중 모델 프롬프트 디스패치 플랫폼입니다.** **PromptQuorum에서 테스트됨 — 세 모델에 걸쳐 30개의 제품 이미지 프롬프트 디스패치:** GPT-5.6는 30건 중 22건에서 가장 구조화된 출력을 반환했으며, Claude Opus 5은 30건 중 25건에서 텍스트 추출에서 가장 높은 정확도를 달성했고, Gemini 3.5 Pro는 30건 중 18건에서 가장 많은 맥락적 세부 사항을 포착했습니다 — 서로 다른 모델이 서로 다른 이미지 분석 작업에 탁월함을 보여줍니다. Consensus Scoring은 모든 다중 모델 불일치에서 이상값 응답을 식별했습니다.',
      ...
      howToStart: {
        ...
        numberedItems: [
          ...
          '**PromptQuorum으로 여러 모델에 걸쳐 테스트하십시오.** GPT-5.6는 장면 이해에, Claude Opus 5은 텍스트 추출에, Gemini 3.5 Pro는 긴 컨텍스트에 뛰어납니다. 한 모델이 모든 작업에서 최고인 경우는 드뭅니다 — 출력을 비교하여 최적의 모델을 찾으십시오.',
        ...
        faqs: [
          {
            ...
            a: '단일 최고 모델은 없습니다. GPT-5.6는 일반적인 장면 이해와 복잡한 추론에 뛰어납니다. Claude Opus 5은 문서 분석과 텍스트 추출에 정확합니다. Gemini 3.5 Pro는 더 긴 멀티모달 컨텍스트(100만 토큰)를 처리합니다. PromptQuorum을 사용하여 특정 작업에 대해 세 모델 모두를 테스트하십시오.',
          },
          {
          ...
          },
          {
            ...
            a: 'GPT-5.6, Claude Opus 5, Gemini 3.5 Pro는 JPEG, PNG, GIF, WebP를 허용합니다. 대부분은 최대 20 MB의 이미지를 지원합니다. 특정 제한은 모델마다 다릅니다. 현재 세부 사항은 OpenAI 및 Anthropic 문서를 확인하십시오.',
          },
          {
