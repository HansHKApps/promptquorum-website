...
export const article: Record<Language, PEArticle> = {
  en: {
    ...
    metaDescription: 'Build a prompt regression test suite with golden sets, edge cases, and adversarial inputs. Block deployment if pass rate drops >5%. Tools: Promptfoo and Braintrust.',
  ...
  de: {
    ...
    metaDescription: 'Erstellen Sie eine Prompt-Regressionstestsuite mit Golden Sets, Edge Cases und adversariellen Eingaben. Deployment blockieren, wenn die Bestehensquote um mehr als 5% sinkt. Tools: Promptfoo und Braintrust.',
    ...
    faqSchema: {
      ...
      mainEntity: [
        ...
        {
          '@type': 'Question',
          name: 'Was ist ein Golden Test Set?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'Ein Golden Test Set ist eine feste Sammlung von Eingabe-/Ausgabe-Paaren, bei denen die erwartete Ausgabe manuell als korrekt verifiziert wurde. Es ist der Maßstab, den Ihr Prompt konsistent erfüllen muss. Beginnen Sie mit 10–20 Paaren aus echtem Produktions-Traffic.',
          },
        },
        {
          '@type': 'Question',
          name: 'Woran erkenne ich, ob eine Prompt-Regression signifikant ist?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'Eine Regression ist signifikant, wenn die Bestehensquote im Golden Test Set um mehr als 5% vom Baseline sinkt, wenn ein adversarieller Test, der zuvor bestanden hat, jetzt fehlschlägt, oder wenn die Ausgabeformat-Compliance bei mehr als 2 von 10 Testfällen sinkt.',
          },
        },
        {
          '@type': 'Question',
          name: 'Kann ich PromptQuorum für Regressionstests verwenden?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'Ja. PromptQuorum sendet Prompts gleichzeitig an mehrere Modelle, was es gut geeignet für Multi-Modell-Regressionstests macht. Sie können ein Testset parallel gegen GPT-4o, Claude 4.6 Sonnet und Gemini 2.5 Pro ausführen und Bestehensquoten modellübergreifend vergleichen.',
          },
        },
      ],
    },
  ...
  fr: {
    ...
    metaDescription: 'Créez une suite de tests de régression de prompts avec des golden sets, des cas limites et des entrées adversariales. Bloquez le déploiement si le taux de réussite chute de plus de 5%. Outils : Promptfoo et Braintrust.',
    ...
    faqSchema: {
      ...
      mainEntity: [
        ...
        {
          '@type': 'Question',
          name: 'Qu\'est-ce qu\'un golden test set ?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'Un golden test set est une collection fixe de paires entrée/sortie où la sortie attendue a été vérifiée manuellement comme correcte. Il représente le standard que votre prompt doit constamment respecter. Commencez avec 10–20 paires issues du trafic réel en production.',
          },
        },
        {
          '@type': 'Question',
          name: 'Comment savoir si une régression de prompt est significative ?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'Une régression est significative si le taux de réussite sur votre golden test set chute de plus de 5% par rapport à la baseline, si un test adversarial qui passait précédemment échoue maintenant, ou si la conformité du format de sortie chute sur plus de 2 cas de test sur 10.',
          },
        },
        {
          '@type': 'Question',
          name: 'Puis-je utiliser PromptQuorum pour les tests de régression ?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'Oui. PromptQuorum envoie des prompts à plusieurs modèles simultanément, ce qui le rend adapté aux tests de régression multi-modèles. Vous pouvez exécuter un jeu de tests en parallèle contre GPT-4o, Claude 4.6 Sonnet et Gemini 2.5 Pro et comparer les taux de réussite entre modèles.',
          },
        },
      ],
    },
  ...
  ja: {
    ...
    faqSchema: {
      ...
      mainEntity: [
        ...
        {
          '@type': 'Question',
          name: 'ゴールデンテストセットとは何ですか？',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'ゴールデンテストセットは、期待出力が手動で正しいと確認された固定の入力/出力ペアのコレクションです。プロンプトが一貫して満たすべきベンチマークです。実際の本番トラフィックから10〜20ペアから始めてください。',
          },
        },
        {
          '@type': 'Question',
          name: 'プロンプト回帰が重大かどうかはどうやって判断しますか？',
          acceptedAnswer: {
            '@type': 'Answer',
            text: '回帰は、ゴールデンテストセットの合格率がベースラインから5%以上低下した場合、以前に合格した敵対的テストが失敗した場合、または出力形式の準拠が10件中2件以上のテストケースで低下した場合に重大です。',
          },
        },
        {
          '@type': 'Question',
          name: 'PromptQuorumを回帰テストに使用できますか？',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'はい。PromptQuorumは複数のモデルにプロンプトを同時に送信するため、マルチモデル回帰テストに適しています。GPT-4o、Claude 4.6 Sonnet、Gemini 2.5 Proに対して並行してテストセットを実行し、モデル固有の回帰を検出できます。',
          },
        },
      ],
    },
  ...
  zh: {
    ...
    metaDescription: '使用黄金集、边界情况和对抗性输入构建提示词回归测试套件。如果通过率下降超过5%，阻止部署。工具：Promptfoo和Braintrust。',
    ...
    faqSchema: {
      ...
      mainEntity: [
        ...
        },
        {
          ...
          acceptedAnswer: {
            '@type': 'Answer',
            text: '黄金测试集是期望输出已经手动验证为正确的固定输入/输出对集合。它代表您的提示词必须一致满足的基准。从实际生产流量中选取10-20对开始。',
          },
        },
        {
          '@type': 'Question',
          ...
          acceptedAnswer: {
            '@type': 'Answer',
            text: '如果黄金测试集的通过率从基线下降超过5%、之前通过的对抗性测试现在失败、或输出格式合规性在10个测试用例中超过2个下降，则回归重大。使用绝对阈值，而不仅仅是相对阈值。',
          },
        },
        {
          '@type': 'Question',
          name: '我可以使用PromptQuorum进行回归测试吗？',
          acceptedAnswer: {
            '@type': 'Answer',
            text: '可以。PromptQuorum将提示词同时分发给多个模型，非常适合多模型回归测试。您可以并行地对GPT-4o、Claude 4.6 Sonnet和Gemini 2.5 Pro运行测试集，比较各模型的通过率以检测模型特定的回归。',
          },
        },
      ],
    },
