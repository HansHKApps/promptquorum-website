...
export const article: Partial<Record<Language, PEArticle>> = {
  ...
  zh: {
    ...
    leadAnswerBlock: '**三种提示词模式无需改动 API 即可将结构化输出可靠性提升到 95% 或以上：将 schema 嵌入提示词、向模型展示一个有效的输出示例、以及为类型、格式和 null 处理添加字段级指令。** 这些模式在 GPT-5.5、Claude 4.6 Sonnet 和 Gemini 2.5 Pro 上均有效，无论是否使用原生 JSON mode。',
    publishDate: '2026-04-30',
    dateModified: '2026-04-30',
    readTime: '阅读约9分钟',
    educationalLevel: 'Intermediate',
    ...
    sections: {
      ...
      tldr: {
        ...
        items: [
          ...
          '每个结构化输出提示词都至少在 2 个模型上测试 —— 一个在 GPT-5.5 上以 95% 通过的提示词，若没有与模型无关的指令，可能在 Claude 4.6 Sonnet 上只有 70% 通过',
        ],
      },
      whatMakesReliable: {
        id: 'what-makes-reliable',
        ...
        content: [
          ...
          '**三种提示词层面的干预能持续弥合可靠性差距。嵌入 schema 让输出结构无歧义。单个有效的输出示例消除格式歧义。字段级指令消除类型和 null 处理错误。三者结合，可在 GPT-5.5、Claude 4.6 Sonnet 和 Gemini 2.5 Pro 上将结构化输出可靠性提升到 95%+ —— 无论是否使用原生 JSON mode。**',
        ],
      ...
      testingReliability: {
        ...
        numberedItems: [
          ...
          '**在第二个模型上验证提示词。** 用同一提示词对第二个模型运行完整的 20 用例集。在 GPT-5.5 上达到 95%+ 但在 Claude 4.6 Sonnet 上仅 70% 的提示词是依赖于模型的。要么添加足够明确、能在两者上都通过的指令，要么记录该提示词已针对哪个模型验证，并且不经重新测试就不要切换。',
        ],
        callouts: [
          ...
          { type: 'pro-tip', label: '使用 PromptQuorum 进行多模型对比', text: 'PromptQuorum 会同时用你的 20 用例测试集对 GPT-5.5、Claude 4.6 Sonnet 和 Gemini 2.5 Pro 运行，并并排显示字段级通过率。这能在一次运行中识别出依赖于模型的失败，而不是分三次。' },
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
        ...
        mistakes: [
          ...
          },
          {
            ...
            problem: '结构化输出可靠性在不同模型间差异显著 —— 由于在 schema 约束上的指令遵循行为不同，一个在 GPT-5.5 上 95% 的提示词可能在 Claude 4.6 Sonnet 上只有 70%',
      ...
      faqSection: {
        ...
        faqs: [
          ...
          },
          {
            ...
            a: '会。把必填字段放在最前，可选或可空字段放在最后。模型在决定包含什么时会更看重靠前的 schema 元素。当模型对某个值不确定时，排在前面的可空字段比排在后面的必填字段更容易被遗漏。这种排序效应在 GPT-5.5 和 Claude 4.6 Sonnet 上是一致的。',
          },
        ],
      },
      relatedReading: {
        ...
        title: '相关阅读',
        items: [
          '[结构化输出与 JSON Mode：何时以及如何使用](/zh/prompt-engineering/structured-output-and-json-mode) —— 面向 GPT-5.5、Claude 和 Gemini 的 API 层 JSON mode 配置，附模型合规性对照表',
          ...
          '[如何跨模型测试提示词](/zh/prompt-engineering/how-to-test-prompts-across-models) —— 用同一提示词对 GPT-5.5、Claude 4.6 Sonnet 和 Gemini 2.5 Pro 运行，以发现特定于模型的失败',
  ...
  ko: {
    ...
    leadAnswerBlock: 'API 변경 없이 구조화 출력 신뢰도를 95% 이상으로 높이는 세 가지 프롬프트 패턴이 있습니다: 스키마를 프롬프트에 직접 임베딩하기, 유효한 출력 예시 하나 제시하기, 타입·포맷·null 처리에 대한 필드 수준 지시사항 추가하기. 이 패턴들은 네이티브 JSON 모드 사용 여부에 관계없이 GPT-5.5, Claude 4.6 Sonnet, Gemini 2.5 Pro 모두에서 작동합니다.',
    publishDate: '2026-04-30',
    dateModified: '2026-04-30',
    readTime: '9분 읽기',
    educationalLevel: 'Intermediate',
    ...
    sections: {
      ...
      tldr: {
        ...
        items: [
          ...
          '모든 구조화 출력 프롬프트를 최소 2개 모델로 테스트하십시오 — GPT-5.5에서 95%를 통과하는 프롬프트가 모델에 무관한 지시사항 없이는 Claude 4.6 Sonnet에서 70%로 떨어질 수 있습니다.',
        ],
      },
      whatMakesReliable: {
        id: 'what-makes-reliable',
        ...
        content: [
          ...
          '세 가지 프롬프트 수준 개입이 신뢰도 격차를 일관되게 해소합니다. 스키마 임베딩은 출력 구조를 명확하게 합니다. 유효한 출력 예시 하나가 포맷 모호성을 제거합니다. 필드 수준 지시사항이 타입 및 null 처리 오류를 없앱니다. 이 세 가지를 합치면 네이티브 JSON 모드 사용 여부에 관계없이 GPT-5.5, Claude 4.6 Sonnet, Gemini 2.5 Pro에서 구조화 출력 신뢰도를 95% 이상으로 높입니다.',
        ],
      ...
      testingReliability: {
        ...
        numberedItems: [
          ...
          '**두 번째 모델로 프롬프트를 검증하십시오.** 동일한 프롬프트로 두 번째 모델에서 전체 20개 케이스를 실행하십시오. GPT-5.5에서 95%+이지만 Claude 4.6 Sonnet에서 70%인 프롬프트는 모델 종속적입니다. 두 모델 모두에서 통과할 만큼 명시적인 지시사항을 추가하거나, 어떤 모델로 검증되었는지 문서화하고 재테스트 없이 전환하지 마십시오.',
        ],
        callouts: [
          ...
          { type: 'pro-tip', label: '다중 모델 비교를 위해 PromptQuorum 사용', text: 'PromptQuorum은 20개 케이스 테스트 세트를 GPT-5.5, Claude 4.6 Sonnet, Gemini 2.5 Pro에서 동시에 실행하고 필드 수준 통과율을 나란히 보여줍니다. 이를 통해 세 번 대신 한 번의 실행으로 모델 종속 실패를 파악할 수 있습니다.' },
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
        ...
        mistakes: [
          ...
          },
          {
            ...
            problem: '구조화 출력 신뢰도는 모델마다 크게 다릅니다 — 스키마 제약에 대한 다른 지시사항 따르기 동작 때문에 GPT-5.5에서 95%인 프롬프트가 Claude 4.6 Sonnet에서 70%로 실패할 수 있습니다.',
      ...
      faqSection: {
        ...
        faqs: [
          ...
          },
          {
            ...
            a: '예. 필수 필드를 먼저, 선택적 또는 nullable 필드를 마지막에 배치하십시오. 모델은 무엇을 포함할지 결정할 때 스키마의 앞 요소에 더 많은 가중치를 부여합니다. 첫 번째로 나열된 nullable 필드는 모델이 값에 대해 불확실할 때 나중에 나열된 필수 필드보다 생략될 가능성이 높습니다. 이 순서 효과는 GPT-5.5와 Claude 4.6 Sonnet 모두에서 일관됩니다.',
          },
        ],
      },
      relatedReading: {
