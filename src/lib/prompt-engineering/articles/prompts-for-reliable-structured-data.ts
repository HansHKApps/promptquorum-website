...
export const article: Partial<Record<Language, PEArticle>> = {
  en: {
    ...
    specific_year: 2026,
    theme: 'Techniques',
    heroImage: '/images/prompts-for-reliable-structured-data-overview-hero-en.webp',
    ...
    sections: {
      ...
      tldr: {
        ...
        items: [
          ...
          'Test every structured output prompt against at least 2 models — a prompt that passes at 95% on GPT-5.6 may fail at 70% on Claude Sonnet 5 without model-agnostic instructions',
        ],
      },
      whatMakesReliable: {
      ...
      testingReliability: {
        ...
        numberedItems: [
          ...
          '**Validate the prompt on a second model.** Run the full 20-case set against a second model using the same prompt. A prompt at 95%+ on GPT-5.6 but 70% on Claude Sonnet 5 is model-dependent. Either add instructions explicit enough to pass on both, or document which model the prompt is validated for and do not switch without re-testing.',
        ],
        callouts: [
          ...
          { type: 'pro-tip', label: 'Use PromptQuorum for multi-model comparison', text: 'PromptQuorum runs your 20-case test set against GPT-5.6, Claude Sonnet 5, and Gemini 3.1 Pro simultaneously and shows field-level pass rates side-by-side. This identifies model-dependent failures in one run instead of three.' },
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
        ...
        mistakes: [
          ...
          },
          {
            ...
            problem: 'Structured output reliability varies significantly across models — a prompt at 95% on GPT-5.6 can fail at 70% on Claude Sonnet 5 due to different instruction-following behavior on schema constraints',
      ...
      relatedReading: {
        ...
        items: [
          '[Structured Output and JSON Mode: When and How to Use It](/prompt-engineering/structured-output-and-json-mode) — API-level JSON mode configuration for GPT-5.6, Claude, and Gemini with a model compliance table',
          ...
          '[How To Test Prompts Across Models](/prompt-engineering/how-to-test-prompts-across-models) — running the same prompt against GPT-5.6, Claude Sonnet 5, and Gemini 3.1 Pro to find model-specific failures',
  ...
  de: {
    ...
    theme: 'Techniken',
    heroImage: '/images/prompts-for-reliable-structured-data-overview-hero-de.webp',
    ...
    leadAnswerBlock: '**Drei Prompt-Muster erreichen 95% oder mehr strukturierte Output-Zuverlässigkeit ohne API-Änderungen: Schema im Prompt einbetten, dem Modell ein gültiges Ausgabebeispiel zeigen und feldspezifische Anweisungen für Typ, Format und null-Behandlung hinzufügen.** Diese Muster funktionieren über GPT-5.6, Claude Sonnet 5 und Gemini 3.1 Pro, mit oder ohne nativen JSON-Modus.',
    publishDate: '2026-04-30',
    dateModified: '2026-08-31',
    ...
    sections: {
      ...
      testingReliability: {
        ...
        callouts: [
          ...
          { type: 'pro-tip', label: 'Nutzen Sie PromptQuorum für Multi-Modell-Vergleich', text: 'PromptQuorum führt Ihr 20-Fall-Test-Set gegen GPT-5.6, Claude Sonnet 5 und Gemini 3.1 Pro gleichzeitig aus und zeigt feldspezifische Pass-Raten Seite an Seite. Dies identifiziert modellabhängige Fehler in einem Durchlauf statt drei.' },
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
        ...
        mistakes: [
          ...
          },
          {
            ...
            problem: 'Strukturierte Output-Zuverlässigkeit variiert erheblich zwischen Modellen — ein Prompt bei 95% auf GPT-5.6 kann bei 70% auf Claude Sonnet 5 scheitern aufgrund unterschiedlichen Instruction-Folgens bei Schema-Konstraints',
      ...
      relatedReading: {
        ...
        items: [
          '[Strukturierte Ausgabe und JSON-Modus: Wann und wie Sie es nutzen](/de/prompt-engineering/structured-output-and-json-mode) — API-Level-JSON-Modus-Konfiguration für GPT-5.6, Claude und Gemini mit Modell-Konformitäts-Tabelle',
          ...
          '[Wie Sie Prompts über Modelle hinweg testen](/de/prompt-engineering/how-to-test-prompts-across-models) — Ausführen des gleichen Prompts gegen GPT-5.6, Claude Sonnet 5 und Gemini 3.1 Pro, um modellabhängige Fehler zu finden',
  ...
  es: {
    ...
    specific_year: 2026,
    theme: 'Techniques',
    heroImage: '/images/prompts-for-reliable-structured-data-overview-hero-es.webp',
    ...
    sections: {
      ...
      tldr: {
        ...
        items: [
          ...
          'Prueba cada prompt de salida estructurada contra al menos 2 modelos — un prompt que aprueba al 95% en GPT-5.6 puede fallar al 70% en Claude Sonnet 5 sin instrucciones agnósticas al modelo',
        ],
      },
      whatMakesReliable: {
        id: 'what-makes-reliable',
      ...
      testingReliability: {
        ...
        numberedItems: [
          ...
          '**Valida el prompt en un segundo modelo.** Ejecuta el conjunto completo de 20 casos contra un segundo modelo usando el mismo prompt. Un prompt al 95%+ en GPT-5.6 pero al 70% en Claude Sonnet 5 es dependiente del modelo.',
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
        ...
        mistakes: [
          ...
          { mistake: 'Probar solo contra el modelo en el que se desarrolló el prompt', problem: 'La fiabilidad de salida estructurada varía significativamente entre modelos — un prompt al 95% en GPT-5.6 puede fallar al 70% en Claude Sonnet 5', fix: 'Ejecuta cada prompt de salida estructurada contra al menos 2 modelos antes de tratarlo como agnóstico al modelo.' },
      ...
      faqSection: {
        ...
        faqs: [
          ...
          { q: '¿El orden de los campos en el esquema afecta la fiabilidad de la salida estructurada?', a: 'Sí. Coloca los campos requeridos primero y los campos opcionales o nullable al final. Los modelos ponderan más los elementos del esquema anteriores al decidir qué incluir. Este efecto de ordenamiento es consistente en GPT-5.6 y Claude Sonnet 5.' },
        ],
      },
      relatedReading: {
        id: 'related-reading',
        ...
        items: [
          '[Salida estructurada y modo JSON: cuándo y cómo usarlo](/es/prompt-engineering/structured-output-and-json-mode) — Configuración del modo JSON a nivel de API para GPT-5.6, Claude y Gemini con tabla de cumplimiento del modelo',
          ...
          '[Cómo probar prompts en diferentes modelos](/es/prompt-engineering/how-to-test-prompts-across-models) — ejecutar el mismo prompt en GPT-5.6, Claude Sonnet 5 y Gemini 3.1 Pro para encontrar fallos específicos del modelo',
  ...
  ar: {
    ...
    specific_year: 2026,
    theme: 'Techniques',
    heroImage: '/images/prompts-for-reliable-structured-data-overview-hero-ar.webp',
    ...
    sections: {
      ...
      tldr: {
        ...
        items: [
          ...
          'اختبر كل موجّه مخرجات منظمة على الأقل على نموذجين — موجّه ينجح بنسبة 95% في GPT-5.6 قد يفشل بنسبة 70% في Claude Sonnet 5 بدون تعليمات مستقلة عن النموذج',
        ],
      },
      whatMakesReliable: {
        id: 'what-makes-reliable',
      ...
      testingReliability: {
        ...
        numberedItems: [
          ...
          '**تحقق من الموجّه على نموذج ثانٍ.** شغّل مجموعة الـ20 حالة الكاملة على نموذج ثانٍ باستخدام نفس الموجّه. موجّه بنسبة 95%+ في GPT-5.6 لكن 70% في Claude Sonnet 5 يعتمد على النموذج.',
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
        ...
        mistakes: [
          ...
          { mistake: 'الاختبار فقط على النموذج الذي طُوّر الموجّه عليه', problem: 'تتفاوت موثوقية المخرجات المنظمة بشكل كبير بين النماذج — موجّه بنسبة 95% في GPT-5.6 قد يفشل بنسبة 70% في Claude Sonnet 5', fix: 'شغّل كل موجّه مخرجات منظمة على الأقل على نموذجين قبل التعامل معه كمستقل عن النموذج.' },
      ...
      faqSection: {
        ...
        faqs: [
          ...
          { q: 'هل يؤثر ترتيب الحقول في المخطط على موثوقية المخرجات المنظمة؟', a: 'نعم. ضع الحقول المطلوبة أولاً والحقول الاختيارية أو القابلة للإلغاء في النهاية. النماذج تُعطي وزناً أكبر لعناصر المخطط السابقة عند تحديد ما تضمّنه. تأثير الترتيب هذا متسق في GPT-5.6 و Claude Sonnet 5.' },
        ],
      },
      relatedReading: {
        id: 'related-reading',
        ...
        items: [
          '[المخرجات المنظمة ووضع JSON: متى وكيف تستخدمها](/ar/prompt-engineering/structured-output-and-json-mode) — إعداد وضع JSON على مستوى API لـ GPT-5.6 و Claude و Gemini مع جدول امتثال النماذج',
          ...
          '[كيفية اختبار الموجّهات على نماذج مختلفة](/ar/prompt-engineering/how-to-test-prompts-across-models) — تشغيل نفس الموجّه في GPT-5.6 و Claude Sonnet 5 و Gemini 3.1 Pro لاكتشاف الإخفاقات الخاصة بالنموذج',
  ...
  pt: {
    ...
    specific_year: 2026,
    theme: 'Techniques',
    heroImage: '/images/prompts-for-reliable-structured-data-overview-hero-pt.webp',
    ...
    schema: {
      ...
      datePublished: '2026-04-30',
      dateModified: '2026-08-31',
      url: 'https://www.promptquorum.com/pt/prompt-engineering/prompts-for-reliable-structured-data',
      inLanguage: 'pt-BR',
      proficiencyLevel: 'Intermediate',
      about: [
    ...
    sections: {
      ...
      tldr: {
        ...
        items: [
          ...
          'Teste cada prompt de saída estruturada contra pelo menos 2 modelos — um prompt aprovado a 95% no GPT-5.6 pode falhar a 70% no Claude Sonnet 5 sem instruções agnósticas ao modelo',
        ],
      },
      whatMakesReliable: {
        id: 'what-makes-reliable',
      ...
      testingReliability: {
        ...
        numberedItems: [
          ...
          '**Valide o prompt em um segundo modelo.** Execute o conjunto completo de 20 casos contra um segundo modelo usando o mesmo prompt. Um prompt a 95%+ no GPT-5.6 mas a 70% no Claude Sonnet 5 é dependente do modelo.',
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
      ...
      faqSection: {
        ...
        faqs: [
          ...
          { q: 'A ordem dos campos no esquema afeta a confiabilidade da saída estruturada?', a: 'Sim. Coloque os campos obrigatórios primeiro e os campos opcionais ou anuláveis por último. Os modelos ponderam mais os elementos anteriores do esquema ao decidir o que incluir. Esse efeito de ordenamento é consistente no GPT-5.6 e no Claude Sonnet 5.' },
        ],
      },
      relatedReading: {
        id: 'related-reading',
        ...
        items: [
          '[Saída estruturada e modo JSON: quando e como usar](/pt/prompt-engineering/structured-output-and-json-mode) — Configuração do modo JSON a nível de API para GPT-5.6, Claude e Gemini com tabela de conformidade do modelo',
          ...
          '[Como testar prompts em diferentes modelos](/pt/prompt-engineering/how-to-test-prompts-across-models) — executar o mesmo prompt no GPT-5.6, Claude Sonnet 5 e Gemini 3.1 Pro para encontrar falhas específicas do modelo',
  ...
  fr: {
    ...
    next_refresh_due: '2026-10-30',
    theme: 'Techniques',
    heroImage: '/images/prompts-for-reliable-structured-data-overview-hero-fr.webp',
    ...
    sections: {
      ...
      testingReliability: {
        ...
        numberedItems: [
          ...
          '**Valider le prompt sur un second modèle.** Exécuter l\'ensemble complet de 20 cas sur un second modèle avec le même prompt. Un prompt à 95%+ sur GPT-5.6 mais 70% sur Claude Sonnet 5 est dépendant du modèle. Soit ajouter des instructions assez explicites pour réussir sur les deux, soit documenter quel modèle le prompt est validé pour et ne pas changer sans retesting.',
        ],
        callouts: [
          ...
          { type: 'pro-tip', label: 'Utiliser PromptQuorum pour comparaison multi-modèle', text: 'PromptQuorum exécute votre ensemble de test de 20 cas sur GPT-5.6, Claude Sonnet 5 et Gemini 3.1 Pro simultanément et affiche les taux de réussite par champ côte à côte. Cela identifie les défaillances dépendantes du modèle en un run au lieu de trois.' },
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
        ...
        mistakes: [
          ...
          },
          {
            ...
            problem: 'La fiabilité de sortie structurée varie significativement entre les modèles — un prompt à 95% sur GPT-5.6 peut échouer à 70% sur Claude Sonnet 5 à cause du suivi d\'instruction différent sur les contraintes de schéma',
      ...
      relatedReading: {
        ...
        items: [
          '[Sortie Structurée et JSON-mode: Quand et Comment l\'Utiliser](/fr/prompt-engineering/structured-output-and-json-mode) — Configuration JSON-mode au niveau API pour GPT-5.6, Claude et Gemini avec tableau de conformité du modèle',
          ...
          '[Comment Tester les Prompts sur les Modèles](/fr/prompt-engineering/how-to-test-prompts-across-models) — Exécution du même prompt sur GPT-5.6, Claude Sonnet 5 et Gemini 3.1 Pro pour trouver les défaillances dépendantes du modèle',
  ...
  ja: {
    ...
    next_refresh_due: '2026-10-30',
    theme: 'テクニック',
    heroImage: '/images/prompts-for-reliable-structured-data-overview-hero-ja.webp',
    ...
    leadAnswerBlock: '**3つのプロンプトパターンはAPI変更なしで95%以上の信頼性を達成します: スキーマをプロンプトに埋め込む、モデルに1つの有効な出力例を見せる、タイプ、形式、null処理のフィールドレベルの指示を追加する。** これらのパターンはGPT-5.6、Claude Sonnet 5、Gemini 3.1 Proで動作し、ネイティブJSON-modeの有無は問いません。',
    publishDate: '2026-04-30',
    dateModified: '2026-08-31',
    readTime: '9分で読める',
    educationalLevel: 'Intermediate',
    ...
    sections: {
      ...
      testingReliability: {
        ...
        numberedItems: [
          ...
          '**2番目のモデルで有効にします。** 2番目のモデルに対して完全な20ケースセットを実行します。GPT-5.6で95%+、Claude Sonnet 5で70%のプロンプトはモデル依存です。両方で成功するように指示をより明確にするか、プロンプトが検証されたモデルを文書化し、再テストなしで変更しないでください。',
        ],
        callouts: [
          ...
          { type: 'pro-tip', label: 'マルチモデル比較でPromptQuorumを使用', text: 'PromptQuorumは20ケースセットをGPT-5.6、Claude Sonnet 5、Gemini 3.1 Proで同時にテストし、フィールドごとのパスレートを並べて表示します。これにより、モデル依存の失敗が1回の実行で識別され、3回ではなく。' },
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
        ...
        mistakes: [
          ...
          },
          {
            ...
            problem: '構造化出力信頼性はモデル間で大きく異なります — GPT-5.6で95%のプロンプトはClaude Sonnet 5で70%失敗することができ、スキーマ制約でのさまざまな指示フォローが原因で',
      ...
      relatedReading: {
        ...
        title: '参考資料',
        items: [
          '[構造化出力とJSON-mode: いつ、どのように使用するか](/ja/prompt-engineering/structured-output-and-json-mode) — GPT-5.6、Claude、Geminiのためのapi-levelJSON-mode構成と、モデル準拠テーブル',
          ...
          '[モデル全体でプロンプトをテストする方法](/ja/prompt-engineering/how-to-test-prompts-across-models) — GPT-5.6、Claude Sonnet 5、Gemini 3.1 Pro全体で同じプロンプトを実行して、モデル依存の失敗を検出',
  ...
  zh: {
    ...
    next_refresh_due: '2026-10-30',
    theme: '技术',
    heroImage: '/images/prompts-for-reliable-structured-data-overview-hero-zh.webp',
    ...
    ogDescription: 'JSON mode 能阻止格式错误的 JSON，却无法阻止缺失字段或错误类型。三种提示词技巧 —— 嵌入 schema、输出示例、字段指令 —— 无需改动 API 配置即可将可靠性提升到 95%+。',
    twitterDescription: 'JSON mode 修复的是语法，而非 schema 合规性。提示词内嵌 schema + 一个输出示例 + 字段指令 → 95%+ 的结构化输出可靠性。无需改动 API。',
    intro: '**大多数结构化输出失败都发生在合法的 JSON 内部 —— 必填字段缺失、日期被格式化为普通字符串、列举值拼写错误、可空字段返回空字符串而非 null。** API 层的 JSON mode 和 tool_use 能消除无法解析的输出，却对 schema 合规性失败无能为力。三种提示词技巧能修复 JSON mode 遗留的问题。',
    leadAnswerBlock: '**三种提示词模式无需改动 API 即可将结构化输出可靠性提升到 95% 或以上：将 schema 嵌入提示词、向模型展示一个有效的输出示例、以及为类型、格式和 null 处理添加字段级指令。** 这些模式在 GPT-5.6、Claude Sonnet 5 和 Gemini 3.1 Pro 上均有效，无论是否使用原生 JSON mode。',
    publishDate: '2026-04-30',
    dateModified: '2026-08-31',
    readTime: '阅读约9分钟',
    educationalLevel: 'Intermediate',
    audience: '使用 LLM API 构建生产工作流的开发者',
    primaryTerm: 'prompts for reliable structured data',
    aboutTopics: ['结构化输出提示', 'JSON Schema 设计', '提示词可靠性测试'],
    quickFacts: [
    ...
    toc: [
      ...
      { label: '提示词设计决定结构化输出的可靠性',                       anchor: '#what-makes-reliable' },
      { label: '直接把 schema 嵌入提示词',                             anchor: '#schema-in-prompt' },
      { label: '给模型看一个有效的输出示例',                            anchor: '#example-anchoring' },
      { label: '写字段级指令，而不只是 schema',                        anchor: '#field-instructions' },
      { label: '根据任务和强制方式选择 JSON、YAML 或 CSV',            anchor: '#format-selection' },
      { label: '让模型修复自己格式错误的输出',                          anchor: '#failure-recovery' },
      { label: '数组、列举和可空字段的提示词模式',                      anchor: '#data-type-patterns' },
      { label: '衡量你的提示词的结构化输出可靠性',                      anchor: '#testing-reliability' },
      { label: '5 个常见的结构化输出提示词错误',                        anchor: '#common-mistakes' },
    ...
    schema: {
      '@context': 'https://schema.org',
      '@type': 'TechArticle',
      headline: '可靠结构化数据的提示词：3 种技巧（2026）',
      description: 'JSON mode 只强制 JSON 语法，而非字段完整性。嵌入 schema、输出示例和字段指令可在不改动 API 的情况下，将可靠性提升到 95%+。',
      author: { '@type': 'Organization', name: 'PromptQuorum' },
      publisher: { '@type': 'Organization', name: 'PromptQuorum', url: 'https://www.promptquorum.com' },
      datePublished: '2026-04-30',
      dateModified: '2026-08-31',
      url: 'https://www.promptquorum.com/zh/prompt-engineering/prompts-for-reliable-structured-data',
      inLanguage:       'zh',
      proficiencyLevel: 'Intermediate',
      about: [
        { '@type': 'Thing', name: '结构化输出提示' },
        { '@type': 'Thing', name: 'JSON Schema 设计' },
        { '@type': 'Thing', name: '提示词可靠性测试' },
      ],
      audience:  { '@type': 'Audience', audienceType: '使用 LLM API 构建生产工作流的开发者' },
      speakable: { '@type': 'SpeakableSpecification', cssSelector: ['.article-intro', '.key-takeaways'] },
    },
    itemListSchema: {
      '@context': 'https://schema.org',
      '@type': 'ItemList',
      name: '结构化输出格式对比：JSON、YAML、XML、CSV、Markdown',
      inLanguage: 'zh',
      numberOfItems: 5,
      itemListElement: [
        {
          ...
          position: 1,
          name: 'JSON',
          description: '最适合 API、数据库和类型安全的消费方。在 API 强制下可靠性最高（99%+）。无强制时：提示词内嵌 schema 可达 80–85%。需要闭合花括号和转义序列，在自由生成中会提高语法错误率。',
        },
        {
          '@type': 'ListItem',
          position: 2,
          name: 'YAML',
          description: '最适合无 API 强制时输出人类可读的配置和文档。由于语法更简单（无花括号、无尾随逗号），模型在提示词内嵌 schema 时可达 88–92% 的可靠性。送入期望 JSON 的系统前需要先转换。',
        },
        {
          '@type': 'ListItem',
          position: 3,
          name: 'XML',
          description: '最适合文档转换和遗留系统集成。模型在提示词内嵌 schema 时可生成 85–90% 正确的 XML。对简单的键值数据过于冗长；更适合层级化的文档结构。',
        },
        {
          '@type': 'ListItem',
          position: 4,
          name: 'CSV',
          description: '最适合导出到电子表格或数据管道的扁平表格数据。由于行列结构简单，扁平数据的可靠性最高（95%+）。不适合嵌套或层级化数据。',
        },
        {
          '@type': 'ListItem',
          position: 5,
          name: 'Markdown 表格',
          description: '最适合带表格段落的报告和人类可读输出。简单表格可靠性高。若无解析器，不适合机器可读的下游处理。',
        },
      ],
    },
    sections: {
      tldrCallout: {
        callouts: [
          { type: 'tldr', label: 'TL;DR', text: 'JSON mode 只强制 JSON 语法，而非 schema 合规性 —— 缺失字段、错误类型和无效列举值都需要在提示词层面修复。三种技巧可弥合这一差距：(1) 把 schema 作为 JSON 模板直接嵌入提示词，(2) 包含一个有效的输出示例，(3) 为每个字段添加一条涵盖类型、格式和 null 处理的指令。部署前目标是在 20 个用例的测试集上达到 95%+ 的通过率。对于没有 API 强制的自由提示词，用 YAML 代替 JSON —— 模型产生的语法错误更少。' },
        ],
      },
      tldr: {
        id: 'key-takeaways',
      ...
      whatMakesReliable: {
        ...
        content: [
          ...
          '**三种提示词层面的干预能持续弥合可靠性差距。嵌入 schema 让输出结构无歧义。单个有效的输出示例消除格式歧义。字段级指令消除类型和 null 处理错误。三者结合，可在 GPT-5.6、Claude Sonnet 5 和 Gemini 3.1 Pro 上将结构化输出可靠性提升到 95%+ —— 无论是否使用原生 JSON mode。**',
        ],
        columns: ['失败类型', '提示词中的成因', '提示词修复方法'],
        rows: [
          { '失败类型': '必填字段缺失', '提示词中的成因': '模型从自然语言描述中推断该字段为可选', '提示词修复方法': '为每个必填字段明确标注："title [REQUIRED]"，或单独列出必填字段' },
          { '失败类型': '错误的数据类型', '提示词中的成因': '字段名含糊且无类型标注', '提示词修复方法': '在提示词中添加类型标注："amount (integer, not string)"' },
          { '失败类型': '无效的列举值', '提示词中的成因': '列举未完整列出 —— 模型编造一个看似合理的值', '提示词修复方法': '明确列出所有列举值："status: one of \'active\', \'inactive\', \'pending\'"' },
          { '失败类型': 'null 与空字符串混淆', '提示词中的成因': '没有区分 null 与 "" 的指令', '提示词修复方法': '添加："Return null if unknown. Never return empty string for unknown values."' },
          { '失败类型': '多余的未声明字段', '提示词中的成因': '模型添加了 schema 之外的"有用"上下文', '提示词修复方法': '添加："Return only the fields specified. Do not add fields not listed in the schema."' },
        ],
        tableFormat: true,
        snippets: [
          { type: 'in-one-sentence', text: '结构化输出可靠性是指模型响应中可解析、包含所有必填字段、使用正确数据类型且列举值有效的比例 —— JSON mode 只保证这四项中的第一项。' },
          { type: 'in-plain-terms', text: '把 JSON mode 想象成拼写检查：它能抓出语法错误，却抓不出含义错误。一份文档可以通过拼写检查却依然是错的。只依赖 JSON mode 的提示词就像通过了拼写检查的文档 —— 结构合法，却可能不完整或类型错误。' },
        ],
        callouts: [
          { type: 'key-point', label: 'JSON mode 还不够', text: '即便使用 API 强制的 JSON mode，仍然需要提示词内嵌 schema、字段指令和输出示例。JSON mode 与提示词 schema 设计是互补的 —— 而非二选一。JSON mode 防止语法失败；提示词设计防止合规性失败。' },
        ],
      },
      schemaInPrompt: {
        id: 'schema-in-prompt',
        title: '直接把 schema 嵌入提示词',
        content: [
        ...
        promptExamples: [
          {
            bad: '从下面的文本中提取订单详情，并以 JSON 形式返回。包括订单 ID、客户姓名、总金额、所订商品和订单状态。\n\nText: {{text}}',
            good: '从下面的文本中提取订单详情，并返回与以下确切 schema 匹配的 JSON：\n\n{\n  "order_id": "string",\n  "customer_name": "string",\n  "total_amount": 0.00,\n  "status": "string",\n  "items": [\n    {\n      "name": "string",\n      "quantity": 0,\n      "unit_price": 0.00\n    }\n  ]\n}\n\n只返回有效的 JSON。不要包含 JSON 对象之外的任何文本。\n\nText: {{text}}',
            badLabel: '仅自然语言描述',
            goodLabel: '以 JSON 模板嵌入 schema',
          },
        ],
        callouts: [
          { type: 'pro-tip', label: '使用 TypeScript 风格的类型标注', text: '对于无法使用 JSON mode 的提示词，在 schema 模板内以注释形式添加 TypeScript 风格的类型标注：`"amount": 0 // float, USD, 2 decimal places`。这能在 schema 结构内部提供类型信息，而无需单独的字段指令部分。' },
          { type: 'key-point', label: '字段顺序很重要', text: '在 schema 模板中先列必填字段，其次是可选字段，最后是可空字段。模型在决定包含什么时会更看重靠前的元素 —— 排在最前的可空字段，在模型对其值不确定时更容易被遗漏。' },
        ],
      },
      exampleAnchoring: {
        id: 'example-anchoring',
        ...
        promptExamples: [
          {
            bad: '从下面的评论中提取产品详情，并返回符合此 schema 的 JSON：\n{\n  "product_name": "string",\n  "rating": 0,\n  "sentiment": "string",\n  "key_features": ["string"]\n}\n\nReview: {{review}}',
            good: '从下面的评论中提取产品详情，并返回符合此 schema 的 JSON：\n{\n  "product_name": "string",\n  "rating": 0,\n  "sentiment": "string",\n  "key_features": ["string"]\n}\n\nExample output:\n{\n  "product_name": "WH-1000XM5 Headphones",\n  "rating": 4,\n  "sentiment": "positive",\n  "key_features": ["noise cancellation", "30-hour battery", "comfortable fit"]\n}\n\nReview: {{review}}',
            badLabel: '仅 schema —— 无输出示例',
            goodLabel: 'schema + 一个真实的输出示例',
          },
        ],
        callouts: [
          { type: 'pro-tip', label: '一个示例通常就够了', text: '只有当你的数据会因输入条件而具有明显不同的结构时，第二个示例才有价值 —— 例如某些字段会根据产品类型有条件地出现。超过两个示例后，对大多数结构化输出任务而言，提示词长度的成本会超过可靠性收益。' },
          { type: 'warning', label: '避免琐碎的占位值', text: '以 "foo"、"bar"、"test" 或 `0` 作为占位符的示例，会让模型以为这些是有效值。请使用能代表你真实数据的值 —— 真实的产品名、真实的评分、真实的日期字符串。' },
        ],
      },
      fieldInstructions: {
        id: 'field-instructions',
        ...
        columns: ['字段类型', '指令模式', '示例指令'],
        rows: [
          { '字段类型': 'String', '指令模式': '格式、最大长度、禁用字符', '示例指令': '"title (string, max 100 characters, no HTML tags)"' },
          { '字段类型': 'Number', '指令模式': '整数还是浮点数、精度、单位', '示例指令': '"price (float, exactly 2 decimal places, USD, no currency symbol)"' },
          { '字段类型': 'Date', '指令模式': '格式、时区', '示例指令': '"created_at (string, ISO 8601: YYYY-MM-DDTHH:MM:SSZ, UTC timezone)"' },
          { '字段类型': 'Enum', '指令模式': '逐字列出所有允许值', '示例指令': '"status (string, exactly one of: \'active\', \'inactive\', \'pending\')"' },
          { '字段类型': 'Boolean', '指令模式': '仅 true/false —— 拒绝 yes/no/1/0', '示例指令': '"is_verified (boolean, true or false only — not 1/0 or yes/no)"' },
          { '字段类型': 'Nullable', '指令模式': '何时返回 null、空字符串还是省略', '示例指令': '"description (string or null — return null if unknown, empty string if known to be blank)"' },
          { '字段类型': 'Array', '指令模式': '最少/最多项数、项类型、空数组处理', '示例指令': '"tags (array of strings, 0–5 items, return [] if none — never return null)"' },
        ],
        tableFormat: true,
        promptExamples: [
          {
            bad: 'Return JSON with these fields:\n{\n  "invoice_id": ...,\n  "amount": ...,\n  "due_date": ...,\n  "status": ...,\n  "line_items": [...]\n}',
            good: 'Return JSON with these fields:\n{\n  "invoice_id": "string",\n  "amount": 0.00,\n  "due_date": "YYYY-MM-DD",\n  "status": "string",\n  "line_items": [{"description": "string", "quantity": 0, "unit_price": 0.00}]\n}\n\nField requirements:\n- invoice_id: string, format INV-XXXXXX (e.g. INV-004821)\n- amount: float, 2 decimal places, USD total including tax\n- due_date: string, ISO 8601 date (YYYY-MM-DD), not a datetime\n- status: string, exactly one of: \'paid\', \'unpaid\', \'overdue\', \'cancelled\'\n- line_items: array of objects, 1 or more items, return [] if no line items found\n- If any field cannot be determined, return null for that field',
            badLabel: '仅 schema —— 无字段指令',
            goodLabel: 'schema + 字段级指令',
          },
        ],
        callouts: [
          { type: 'key-point', label: '何时添加字段指令', text: '在以下情况添加字段指令：(1) 字段有特定格式要求（ISO 日期、货币精度），(2) 字段是列举，(3) 字段可空且 null 与空字符串的区分很重要，或 (4) 你的测试集显示该字段在超过 10% 的用例中失败。对于像 "title" 或 "name" 这样简单、无歧义的字符串字段，可跳过字段指令。' },
        ],
      },
      formatSelection: {
        id: 'format-selection',
        title: 'API 用 JSON，提示词用 YAML，表格数据用 CSV',
        content: [
        ...
        columns: ['格式', '无 API 强制时的可靠性', '适用场景', '避免场景'],
        rows: [
          { '格式': 'JSON', '无 API 强制时的可靠性': '提示词内嵌 schema 时 80–85%', '适用场景': 'API、数据库、类型安全的消费方', '避免场景': '无 API 强制且涉及复杂嵌套时' },
          { '格式': 'YAML', '无 API 强制时的可靠性': '提示词内嵌 schema 时 88–92%', '适用场景': '人类可读输出、配置型数据、无 API 强制', '避免场景': '下游系统需要 JSON 且没有转换步骤时' },
          { '格式': 'XML', '无 API 强制时的可靠性': '提示词内嵌 schema 时 85–90%', '适用场景': '文档转换、遗留系统集成', '避免场景': '简单的键值数据（XML 带来不必要的冗长）' },
          { '格式': 'CSV', '无 API 强制时的可靠性': '扁平数据 95%+', '适用场景': '表格数据、电子表格导出、数据管道', '避免场景': '数据具有嵌套或层级结构时' },
          { '格式': 'Markdown 表格', '无 API 强制时的可靠性': '简单表格时较高', '适用场景': '报告、文档、人类可读的表格输出', '避免场景': '需要机器可读的下游处理时' },
        ],
        tableFormat: true,
        items: [
        ...
        callouts: [
          { type: 'warning', label: 'YAML 转 JSON 的成本', text: '如果你为提高提示词可靠性使用 YAML，而下游处理需要 JSON，请在管道中加入一个转换步骤。Python 的 yaml.safe_load() 和 Node.js 的 js-yaml 一行代码即可完成。在团队全面采用 YAML 之前，先把这一点纳入架构考量。' },
        ],
      },
      failureRecovery: {
        id: 'failure-recovery',
        title: '让模型修复自己格式错误的输出',
        content: [
        ...
        promptExamples: [
          {
            bad: '你返回了无效的输出。请重试并返回有效的 JSON。\n\n{{original_prompt}}',
            good: '你之前的响应未通过验证。只修复下面列出的错误，并返回更正后的 JSON。\n\nExpected schema:\n{\n  "invoice_id": "string",\n  "amount": 0.00,\n  "status": "string"\n}\n\nYour previous response:\n{\n  "invoice_id": null,\n  "amount": "150.00",\n  "status": "PAID"\n}\n\nValidation errors:\n- invoice_id is null but is a required string field — extract it from the input\n- amount is a string ("150.00") but must be a float (150.00)\n- status must be lowercase: use \'paid\', not \'PAID\'\n\nReturn only the corrected JSON object.',
            badLabel: '模糊重试 —— 无错误上下文',
            goodLabel: '包含 schema、输出和具体错误的更正提示词',
          },
        ],
        callouts: [
          { type: 'key-point', label: '当更正两次失败时，修复基础提示词', text: '如果更正提示词在第二次尝试时仍无法产生有效输出，那么问题出在基础提示词，而非输入数据。停止重试，诊断失败模式：哪个字段失败、在什么输入条件下失败。添加一条字段指令或修改 schema，从源头上防止失败。' },
          { type: 'warning', label: '更正提示词会增加延迟和成本', text: '每条更正提示词都会使该次调用的 API 成本和延迟翻倍。仅将更正提示词用于边界情况失败（少于 10% 的输出）。如果你的结构化输出提示词失败率超过 10%，请修复基础提示词，而不是在生产中构建更正循环。' },
        ],
      },
      dataTypePatterns: {
        itemHeadings: true,
        ...
        columns: ['数据类型', '常见失败', '可预防的提示词模式'],
        rows: [
          { '数据类型': '数组（0 项）', '常见失败': '模型返回 null 而非 []', '可预防的提示词模式': '"Return an empty array [] if no items are present. Never return null for array fields."' },
          { '数据类型': '数组（1 项及以上）', '常见失败': '只找到一项时，模型返回单个对象而非数组', '可预防的提示词模式': '"Always return an array, even when there is only one item. Single items must be wrapped: [{...}]"' },
          { '数据类型': '列举（2–5 个值）', '常见失败': '模型缩写或编造类似的值', '可预防的提示词模式': '"status: exactly one of: \'active\', \'inactive\', \'pending\' — no abbreviations or variants"' },
          { '数据类型': '列举（6 个及以上值）', '常见失败': '模型编造清单之外的值', '可预防的提示词模式': '用编号列表列出所有值，然后："Use only values from the list above. Do not abbreviate or combine values."' },
          { '数据类型': '可空字段', '常见失败': '模型返回 "" 而非 null，或完全省略该字段', '可预防的提示词模式': '"Return null if the value is unknown. Return empty string \'\' only if the field is known to be blank. Always include the field — do not omit it."' },
          { '数据类型': '整数还是浮点数', '常见失败': '期望整数时模型返回浮点数，或两者都返回字符串', '可预防的提示词模式': '"score (integer — no decimal places, e.g. 4 not 4.0)" or "price (float — exactly 2 decimal places, e.g. 12.99 not 13)"' },
          { '数据类型': '嵌套对象', '常见失败': '模型把嵌套对象压平成扁平键（例如 "address.city" 而非 {"address": {"city": ...}}）', '可预防的提示词模式': '在 schema 模板中以正确缩进展示完整的嵌套结构。用自然语言描述嵌套常常会被压平成扁平键。' },
        ],
        tableFormat: true,
        callouts: [
          { type: 'warning', label: 'null、undefined 与省略', text: 'JSON 没有 undefined 值，但模型有时表现得好像有 —— 当它们认为值未知时，会完全省略某个字段，而不是返回 null。如果下游代码使用 obj.hasOwnProperty() 或类似检查，被省略的字段与值为 null 的字段是不同的。添加："Always include every field in the schema, even if the value is null."' },
          { type: 'pro-tip', label: '嵌套列举需要额外的明确性', text: '嵌套对象内部的列举比顶层列举更容易被拼写错误或缩写。如果你在嵌套对象内有一个列举，请在 schema 模板中该字段出现的位置附近重复该指令，而不仅仅放在通用的字段规则部分。' },
        ],
      },
      testingReliability: {
        id: 'testing-reliability',
        title: '衡量你的结构化输出提示词的可靠性',
        content: [
        ...
        callouts: [
          { type: 'key-point', label: '在 temperature 0 下运行测试', text: '在 temperature 0 下运行结构化输出测试集，以获得确定、可重复的结果。在 temperature 0 下通过的提示词是设计上可靠 —— 而非运气。只有当提示词在确定性条件下达到 95%+ 通过后，才提高 temperature，然后在新的 temperature 下重新运行测试集，以确认可靠性依然成立。' },
          { type: 'pro-tip', label: '使用 PromptQuorum 进行多模型对比', text: 'PromptQuorum 会同时用你的 20 用例测试集对 GPT-5.6、Claude Sonnet 5 和 Gemini 3.1 Pro 运行，并并排显示字段级通过率。这能在一次运行中识别出依赖于模型的失败，而不是分三次。' },
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
        title: '5 个常见的结构化输出提示词错误',
        content: '**五个最常见的结构化输出提示词错误都会产生相同的症状 —— 间歇性或系统性失败 —— 但需要不同的修复方法。在添加指令之前先诊断你犯的是哪种错误，能节省时间。**',
        mistakes: [
          {
            mistake: '用自然语言描述 schema，而不是嵌入它',
            problem: '自然语言描述是含糊的 —— "a list of items" 可能指数组、逗号分隔的字符串或编号列表；"the total" 可能是字符串或浮点数',
            fix: '把预期的 schema 作为 JSON 模板直接嵌入提示词。模板通过其结构而非散文描述来展示字段名、嵌套深度和值类型。',
          },
          {
            mistake: '没有指明如何处理缺失或未知的值',
            problem: '模型会为未知字段编造看似合理的值，而不是返回 null —— 日期变成 "unknown"，金额变成 0，缺失的 ID 变成 "N/A" —— 它们都通不过类型验证',
          ...
          {
            mistake: '只在你开发提示词所用的模型上测试',
            problem: '结构化输出可靠性在不同模型间差异显著 —— 由于在 schema 约束上的指令遵循行为不同，一个在 GPT-5.6 上 95% 的提示词可能在 Claude Sonnet 5 上只有 70%',
            fix: '在把每个结构化输出提示词当作与模型无关之前，至少在 2 个模型上运行它。使用 PromptQuorum 或直接 API 调用，一步[跨模型测试提示词](/zh/prompt-engineering/how-to-test-prompts-across-models)。',
          },
          {
            mistake: '用完全相同的提示词重试失败的输出',
            problem: '在 temperature 0 下重试失败的提示词每次都会产生相同的失败。在更高 temperature 下，它会产生各不相同但仍然失败的输出 —— 错误不同，根因相同',
            fix: '使用一条带有具体验证错误和格式错误输出的更正提示词，或诊断失败模式（哪个字段、哪种输入类型）并向基础提示词添加一条有针对性的字段指令。',
          },
          {
            mistake: '把 JSON mode 当作完整的结构化输出解决方案',
            problem: 'JSON mode 能防止无法解析的输出，却防止不了 schema 合规性失败 —— 使用 JSON mode 的模型仍可能返回缺字段、错类型和无效列举值的合法 JSON，它们都通不过下游验证',
            fix: '即便使用 API 强制的 JSON mode，也始终包含提示词内嵌 schema 和字段指令。API 配置参见[结构化输出与 JSON Mode](/zh/prompt-engineering/structured-output-and-json-mode) —— 本指南涵盖的是提示词层面的补充。',
          },
        ],
      },
      faqSection: {
        id: 'faq',
        title: '常见问题',
        content: '**关于结构化输出提示的最常见问题，涵盖 JSON mode 与提示词设计的边界、该包含多少个示例，以及如何系统性地改进一个失败的提示词。**',
        faqs: [
          {
            q: 'JSON mode 是否让提示词内嵌 schema 变得多余？',
            a: '不会。JSON mode 强制的是可解析的 JSON 语法，而非 schema 合规性。使用 JSON mode 的模型仍可能返回缺失必填字段、使用错误数据类型或包含无效列举值的合法 JSON。提示词内嵌 schema 和字段指令解决的是 schema 合规性失败；JSON mode 只防止无法解析的输出。两种方法是互补的，而非二选一。',
          },
          {
            q: '我应该在提示词中包含多少个输出示例？',
            a: '一个示例通常就足够，并带来最大的可靠性提升。只有当你的数据会因输入条件而具有明显不同的结构时，第二个示例才有价值 —— 例如某些字段会根据输入类型有条件地成为必填。超过两个示例后，对大多数结构化输出任务而言，提示词长度的成本会超过可靠性收益。',
          },
          {
            q: '在没有 API 强制的情况下，结构化输出该用 JSON 还是 YAML？',
            a: '当在没有 API 强制的情况下生成，且输出不需要被期望 JSON 的系统解析时，使用 YAML。由于 YAML 不需要闭合花括号、转义序列或尾随逗号跟踪，模型在其中产生的语法错误更少。当输出直接送入需要 JSON 的 API、数据库或下游系统时，使用 JSON。无论哪种格式，都始终解析并验证。',
          },
          {
            q: '改进一个结构化输出通过率为 70% 的提示词，最快的方法是什么？',
            a: '在字段层面运行测试集，而不只是整体。找出单独通过率最低的字段，添加一条涵盖类型、格式和 null 处理的明确指令，然后重新运行。一条有针对性的字段指令通常能将整体通过率提高 5–15 个百分点。重复直到达到 95% 或更高。',
          },
          {
            q: '如何从没有原生 JSON mode 的模型获得可靠的结构化输出？',
            a: '把完整的 JSON schema 作为模板嵌入提示词，包含一个有效的输出示例，添加字段级指令，并在 temperature 0 下运行。解析并验证每一个输出；对任何验证失败发送一条更正提示词。设计良好的提示词在没有原生 JSON mode 的情况下，也能在大多数模型上以 temperature 0 达到 85–92% 的可靠性。',
          },
          {
            q: '结构化输出提示词合适的测试集规模是多少？',
            a: '至少 20 个用例：10 个正常路径输入（典型、格式良好的数据）、5 个边界情况（异常值、缺失可选字段、长输入），以及 5 个对抗性输入（可能误导模型的值、嵌在字段值中的指令、含糊类型）。这个规模能识别出最常见的失败类别，又不会耗费过多的搭建时间。',
          },
          {
            q: '我应该在什么时候使用更正提示词，而不是修复基础提示词？',
            a: '当失败罕见 —— 少于 10% 的输出 —— 且由异常的边界情况输入引起时，使用更正提示词。当失败是系统性的时，修复基础提示词：同一字段缺失，或同一类型错误在多个测试用例中出现。更正提示词会为每次失败增加延迟和 API 成本；更好的基础提示词能彻底防止失败。',
          },
          {
            q: 'schema 中字段的顺序会影响结构化输出的可靠性吗？',
            a: '会。把必填字段放在最前，可选或可空字段放在最后。模型在决定包含什么时会更看重靠前的 schema 元素。当模型对某个值不确定时，排在前面的可空字段比排在后面的必填字段更容易被遗漏。这种排序效应在 GPT-5.6 和 Claude Sonnet 5 上是一致的。',
          },
        ],
      },
      relatedReading: {
        ...
        title: '相关阅读',
        items: [
          '[结构化输出与 JSON Mode：何时以及如何使用](/zh/prompt-engineering/structured-output-and-json-mode) —— 面向 GPT-5.6、Claude 和 Gemini 的 API 层 JSON mode 配置，附模型合规性对照表',
          '[结构化输出的最佳工具（2026）](/zh/prompt-engineering/best-tools-structured-output-json) —— 面向结构化抽取工作流对比 Instructor、Outlines、Pydantic AI 和 LangChain',
          '[如何控制输出：格式、Temperature 与受约束解码](/zh/prompt-engineering/control-the-output) —— 受约束解码的机制、面向结构化任务的 temperature 和 top-p、停止序列',
          '[如何评估提示词质量：指标、测试与清单](/zh/prompt-engineering/how-to-evaluate-prompt-quality) —— 20 用例测试集的构建、二元通过/失败评分，以及 LLM-as-judge 评分标准',
          '[如何跨模型测试提示词](/zh/prompt-engineering/how-to-test-prompts-across-models) —— 用同一提示词对 GPT-5.6、Claude Sonnet 5 和 Gemini 3.1 Pro 运行，以发现特定于模型的失败',
          '[零样本与少样本提示](/zh/prompt-engineering/zero-shot-vs-few-shot) —— 何时向提示词添加示例，以及针对不同任务类型该包含多少个',
        ],
      },
      sources: {
        id: 'sources',
        title: '来源',
        items: [
          '[OpenAI Structured Outputs 文档](https://platform.openai.com/docs/guides/structured-outputs) —— OpenAI API 中 response_format 和 JSON mode 的技术规范',
          '[Anthropic 工具使用文档](https://docs.anthropic.com/en/docs/build-with-claude/tool-use) —— Claude 的 tool_use 参数如何在 API 层强制结构化输出',
          '[Google Gemini GenerationConfig 文档](https://ai.google.dev/api/generate-content#v1beta.GenerationConfig) —— Gemini 用于原生 JSON 输出的 responseMimeType 配置',
          '[BAML 基准测试：结构化输出准确性的权衡](https://docs.boundaryml.com/benchmarks) —— 关于各模型受约束与不受约束生成之间可靠性差异的证据',
          '[NIST AI 风险管理框架](https://www.nist.gov/artificial-intelligence/ai-risk-management-framework) —— 面向生产系统中 AI 输出验证的治理原则',
        ],
      },
    },
  },
  ko: {
    ...
    specific_year: 2026,
    theme: 'Techniques',
    heroImage: '/images/prompts-for-reliable-structured-data-overview-hero-ko.webp',
    ...
    ogDescription: 'JSON 모드는 잘못된 JSON을 차단하지만 누락된 필드나 잘못된 타입은 막지 못합니다. 스키마 임베딩, 출력 예시, 필드 지시사항 세 가지 프롬프트 기법으로 API 설정 변경 없이 신뢰도를 95% 이상으로 높이십시오.',
    twitterDescription: 'JSON 모드는 구문을 수정하지만 스키마 준수는 보장하지 않습니다. 스키마-인-프롬프트 + 출력 예시 1개 + 필드 지시사항 → 구조화 출력 신뢰도 95% 이상. API 변경 불필요.',
    intro: '**구조화 출력 실패의 대부분은 유효한 JSON 내부에서 발생합니다 — 필수 필드 누락, 날짜를 일반 문자열로 포맷, enum 값 오타, nullable 필드에 null 대신 빈 문자열 반환.** API 수준의 JSON 모드와 tool_use는 파싱 불가 출력을 제거하지만 스키마 준수 실패에는 아무런 효과가 없습니다. 세 가지 프롬프트 기법이 JSON 모드가 해결하지 못하는 문제를 수정합니다.',
    leadAnswerBlock: 'API 변경 없이 구조화 출력 신뢰도를 95% 이상으로 높이는 세 가지 프롬프트 패턴이 있습니다: 스키마를 프롬프트에 직접 임베딩하기, 유효한 출력 예시 하나 제시하기, 타입·포맷·null 처리에 대한 필드 수준 지시사항 추가하기. 이 패턴들은 네이티브 JSON 모드 사용 여부에 관계없이 GPT-5.6, Claude Sonnet 5, Gemini 3.1 Pro 모두에서 작동합니다.',
    publishDate: '2026-04-30',
    dateModified: '2026-08-31',
    readTime: '9분 읽기',
    educationalLevel: 'Intermediate',
    audience: 'LLM API로 프로덕션 워크플로를 구축하는 개발자',
    primaryTerm: '신뢰할 수 있는 구조화 데이터를 위한 프롬프트',
    aboutTopics: ['구조화 출력 프롬프팅', 'JSON 스키마 설계', '프롬프트 신뢰도 테스트'],
    quickFacts: [
      'OpenAI response_format, Anthropic tool_use 등 JSON 모드 API는 파싱 가능한 JSON을 강제하지만 필드 존재 여부, 올바른 데이터 타입, 유효한 enum 값은 보장하지 않습니다 — 스키마 준수 실패는 프롬프트 수준의 제어가 필요합니다.',
      'API 강제 JSON 모드 없이 스키마-인-프롬프트만 사용하는 모델은 구조화 출력 신뢰도 80–85%를 달성하며, 유효한 출력 예시 하나를 추가하면 90–92%로 높아집니다.',
      'enum 필드의 허용 값이 5개를 초과하는 경우 모든 허용 값을 프롬프트에 명시적으로 나열해야 합니다 — enum 목록이 프롬프트에 없으면 모델이 그럴듯하지만 범위 밖의 값을 만들어냅니다.',
      '20개 케이스 테스트 세트(정상 경로 10개, 엣지 케이스 5개, 적대적 입력 5개)로 프로덕션 배포 전 가장 흔한 구조화 출력 프롬프트 실패를 파악하기에 충분합니다.',
    ],
    toc: [
      { label: '핵심 내용',                                              anchor: '#key-takeaways' },
      { label: '프롬프트 설계가 구조화 출력 신뢰도를 결정합니다',          anchor: '#what-makes-reliable' },
      { label: '프롬프트에 스키마를 직접 임베딩하기',                     anchor: '#schema-in-prompt' },
      { label: '유효한 출력 예시 하나 제시하기',                          anchor: '#example-anchoring' },
      { label: '고위험 출력을 위한 필드 수준 지시사항 작성',               anchor: '#field-instructions' },
      { label: '작업과 강제 방식에 따라 JSON, YAML, CSV 선택',            anchor: '#format-selection' },
      { label: '잘못된 출력을 모델 스스로 수정하게 하기',                  anchor: '#failure-recovery' },
      { label: '배열, enum, nullable 필드를 위한 프롬프트 패턴',           anchor: '#data-type-patterns' },
      { label: '프롬프트의 구조화 출력 신뢰도 측정',                       anchor: '#testing-reliability' },
      { label: '구조화 출력 프롬프트의 5가지 흔한 실수',                   anchor: '#common-mistakes' },
    ...
    schema: {
      '@context': 'https://schema.org',
      '@type': 'TechArticle',
      headline: '신뢰할 수 있는 구조화 데이터를 위한 프롬프트: 3가지 기법 (2026)',
      description: 'JSON 모드는 JSON 구문만 강제할 뿐 필드 완성도는 보장하지 않습니다. 스키마 임베딩, 출력 예시, 필드 지시사항으로 API 변경 없이 신뢰도를 95% 이상으로 높일 수 있습니다.',
      author:    { '@type': 'Person', name: 'Hans Kuepper', sameAs: 'https://www.linkedin.com/in/hanskuepper/' },
      publisher: { '@type': 'Organization', name: 'PromptQuorum', url: 'https://www.promptquorum.com' },
      datePublished: '2026-04-30',
      dateModified: '2026-08-31',
      ...
      proficiencyLevel: 'Intermediate',
      about: [
        { '@type': 'Thing', name: '구조화 출력 프롬프팅' },
        { '@type': 'Thing', name: 'JSON 스키마 설계' },
        { '@type': 'Thing', name: '프롬프트 신뢰도 테스트' },
      ],
      audience:  { '@type': 'Audience', audienceType: 'LLM API로 프로덕션 워크플로를 구축하는 개발자' },
      speakable: { '@type': 'SpeakableSpecification', cssSelector: ['.article-intro', '.key-takeaways'] },
    },
    itemListSchema: {
      '@context': 'https://schema.org',
      '@type': 'ItemList',
      name: '구조화 출력 포맷 비교: JSON, YAML, XML, CSV, Markdown',
      inLanguage: 'ko',
      numberOfItems: 5,
      itemListElement: [
        {
          ...
          position: 1,
          name: 'JSON',
          description: 'API, 데이터베이스, 타입 안전 소비자에게 최적입니다. API 강제 시 가장 높은 신뢰도(99%+). 강제 없이: 스키마-인-프롬프트로 80–85%. 닫는 중괄호와 이스케이프 시퀀스가 필요하여 자유형 생성에서 구문 오류율이 높아집니다.',
        },
        {
          '@type': 'ListItem',
          position: 2,
          name: 'YAML',
          description: 'API 강제 없이 사람이 읽기 쉬운 설정 및 문서 출력에 최적입니다. 더 간단한 구문(중괄호 없음, 후행 쉼표 없음) 덕분에 스키마-인-프롬프트로 88–92%의 신뢰도를 달성합니다. JSON을 요구하는 시스템에 공급하기 전 변환이 필요합니다.',
        },
        {
          '@type': 'ListItem',
          position: 3,
          name: 'XML',
          description: '문서 변환 및 레거시 시스템 통합에 최적입니다. 스키마-인-프롬프트로 85–90%의 정확한 XML을 생성합니다. 단순 키-값 데이터에는 장황하며 계층적 문서 구조에 더 적합합니다.',
        },
        {
          '@type': 'ListItem',
          position: 4,
          name: 'CSV',
          description: '스프레드시트나 데이터 파이프라인으로 내보내는 단순 테이블 데이터에 최적입니다. 단순한 행-열 구조 덕분에 단순 데이터에서 가장 높은 신뢰도(95%+)를 보입니다. 중첩 또는 계층적 데이터에는 적합하지 않습니다.',
        },
        {
          '@type': 'ListItem',
          position: 5,
          name: 'Markdown 테이블',
          description: '테이블 섹션이 포함된 보고서 및 사람이 읽기 쉬운 출력에 최적입니다. 단순 테이블에서 높은 신뢰도를 보입니다. 파서 없이는 기계 판독 가능한 다운스트림 처리에 적합하지 않습니다.',
        },
      ],
    },
    sections: {
      ...
      tldr: {
        ...
        title: '핵심 내용',
        items: [
          'JSON 모드는 잘못된 JSON을 막지만 스키마 준수 실패는 막지 못합니다 — 필수 필드 누락, 잘못된 데이터 타입, 유효하지 않은 enum 값은 프롬프트 수준의 수정이 필요합니다.',
          '자연어 설명이 아닌 JSON 템플릿으로 스키마를 프롬프트에 임베딩하십시오 — 템플릿 임베딩은 예상 구조를 명확하게 하여 필드 누락을 줄입니다.',
          '프롬프트에 유효한 출력 예시 하나를 추가하십시오 — 구체적인 예시 하나가 스키마만 있는 프롬프트보다 통과율을 5–8 퍼센트 포인트 높입니다.',
          '필수 필드마다 데이터 타입, 허용 포맷, null 처리, enum 값을 포함하는 필드 지시사항을 작성하십시오 — 필드 지시사항은 타입 오류를 유발하는 모호성을 제거합니다.',
          'API 강제 없이 자유형 프롬프팅에는 JSON 대신 YAML을 사용하십시오 — 더 간단한 구문 덕분에 모델이 YAML에서 구문 오류를 더 적게 생성합니다.',
          '구조화 출력 프롬프트를 프로덕션에 배포하기 전 20개 케이스 테스트 세트에서 95% 이상의 통과율을 목표로 하십시오; 95% 미만이면 다운스트림 실패에 복구 프로세스가 필요합니다.',
          '모든 구조화 출력 프롬프트를 최소 2개 모델로 테스트하십시오 — GPT-5.6에서 95%를 통과하는 프롬프트가 모델에 무관한 지시사항 없이는 Claude Sonnet 5에서 70%로 떨어질 수 있습니다.',
        ],
      },
      whatMakesReliable: {
        id: 'what-makes-reliable',
        title: '프롬프트 설계가 구조화 출력 신뢰도를 결정합니다',
        content: [
          ...
          '세 가지 프롬프트 수준 개입이 신뢰도 격차를 일관되게 해소합니다. 스키마 임베딩은 출력 구조를 명확하게 합니다. 유효한 출력 예시 하나가 포맷 모호성을 제거합니다. 필드 수준 지시사항이 타입 및 null 처리 오류를 없앱니다. 이 세 가지를 합치면 네이티브 JSON 모드 사용 여부에 관계없이 GPT-5.6, Claude Sonnet 5, Gemini 3.1 Pro에서 구조화 출력 신뢰도를 95% 이상으로 높입니다.',
        ],
        columns: ['실패 유형', '프롬프트에서의 원인', '프롬프트 수정'],
        rows: [
          { '실패 유형': '필수 필드 누락', '프롬프트에서의 원인': '모델이 자연어 설명에서 필드가 선택적이라고 추론함', '프롬프트 수정': '각 필수 필드를 명시적으로 표시: "title [REQUIRED]" 또는 필수 필드를 별도로 나열' },
          { '실패 유형': '잘못된 데이터 타입', '프롬프트에서의 원인': '타입 어노테이션 없는 모호한 필드명', '프롬프트 수정': '프롬프트에 타입 어노테이션 추가: "amount (integer, not string)"' },
          { '실패 유형': '유효하지 않은 enum 값', '프롬프트에서의 원인': 'enum이 완전히 나열되지 않아 모델이 그럴듯한 값을 만들어냄', '프롬프트 수정': '모든 enum 값을 명시적으로 나열: "status: one of \'active\', \'inactive\', \'pending\'"' },
          { '실패 유형': 'null vs 빈 문자열 혼동', '프롬프트에서의 원인': 'null과 ""를 구분하는 지시사항 없음', '프롬프트 수정': '"알 수 없으면 null을 반환하십시오. 알 수 없는 값에 빈 문자열을 절대 반환하지 마십시오." 추가' },
          { '실패 유형': '선언되지 않은 추가 필드', '프롬프트에서의 원인': '모델이 스키마에 없는 유용한 컨텍스트를 추가함', '프롬프트 수정': '"지정된 필드만 반환하십시오. 스키마에 나열되지 않은 필드를 추가하지 마십시오." 추가' },
        ],
        tableFormat: true,
        snippets: [
          { type: 'in-one-sentence', text: '구조화 출력 신뢰도는 파싱 가능하고, 모든 필수 필드를 포함하며, 올바른 데이터 타입을 사용하고, 유효한 enum 값을 가진 모델 응답의 비율입니다 — JSON 모드는 이 네 가지 중 첫 번째만 보장합니다.' },
          { type: 'in-plain-terms', text: 'JSON 모드를 맞춤법 검사기라고 생각하십시오: 구문 오류는 잡지만 의미 오류는 잡지 못합니다. 문서는 맞춤법 검사를 통과하고도 여전히 틀릴 수 있습니다. JSON 모드에만 의존하는 프롬프트는 맞춤법 검사를 통과한 문서와 같습니다 — 구조적으로 유효하지만 잠재적으로 불완전하거나 잘못된 타입일 수 있습니다.' },
        ],
        callouts: [
          { type: 'key-point', label: 'JSON 모드만으로는 충분하지 않습니다', text: 'API 강제 JSON 모드를 사용할 때도 스키마-인-프롬프트, 필드 지시사항, 출력 예시가 필요합니다. JSON 모드와 프롬프트 스키마 설계는 상호 보완적이며 대안이 아닙니다. JSON 모드는 구문 실패를 방지하고 프롬프트 설계는 준수 실패를 방지합니다.' },
        ],
      },
      schemaInPrompt: {
        id: 'schema-in-prompt',
        title: '프롬프트에 스키마를 직접 임베딩하기',
        content: [
        ...
        promptExamples: [
          {
            bad: '다음 텍스트에서 주문 세부 정보를 추출하여 JSON으로 반환하십시오. 주문 ID, 고객명, 총 금액, 주문 항목, 주문 상태를 포함하십시오.\n\n텍스트: {{text}}',
            good: '다음 텍스트에서 주문 세부 정보를 추출하여 다음 스키마와 정확히 일치하는 JSON으로 반환하십시오:\n\n{\n  "order_id": "string",\n  "customer_name": "string",\n  "total_amount": 0.00,\n  "status": "string",\n  "items": [\n    {\n      "name": "string",\n      "quantity": 0,\n      "unit_price": 0.00\n    }\n  ]\n}\n\n유효한 JSON만 반환하십시오. JSON 객체 외부에 텍스트를 포함하지 마십시오.\n\n텍스트: {{text}}',
            badLabel: '자연어 설명만 사용',
            goodLabel: 'JSON 템플릿으로 스키마 임베딩',
          },
        ],
        callouts: [
          { type: 'pro-tip', label: 'TypeScript 스타일 타입 어노테이션 사용', text: 'JSON 모드를 사용할 수 없는 프롬프트에서는 스키마 템플릿 내부에 TypeScript 스타일 타입 어노테이션을 주석으로 추가하십시오: `"amount": 0 // float, USD, 소수점 2자리`. 이렇게 하면 별도의 필드 지시사항 섹션 없이도 스키마 구조 내에 타입 정보를 제공할 수 있습니다.' },
          { type: 'key-point', label: '필드 순서가 중요합니다', text: '스키마 템플릿에서 필수 필드를 먼저, 선택적 필드를 다음에, nullable 필드를 마지막에 나열하십시오. 모델은 무엇을 포함할지 결정할 때 앞에 나온 요소에 더 많은 가중치를 부여합니다 — 첫 번째로 나열된 nullable 필드는 모델이 값에 대해 불확실할 때 나중에 나열된 필수 필드보다 누락될 가능성이 높습니다.' },
        ],
      },
      exampleAnchoring: {
        id: 'example-anchoring',
        title: '유효한 출력 예시 하나 제시하기',
        content: [
        ...
        promptExamples: [
          {
            bad: '아래 리뷰에서 제품 세부 정보를 추출하여 이 스키마의 JSON으로 반환하십시오:\n{\n  "product_name": "string",\n  "rating": 0,\n  "sentiment": "string",\n  "key_features": ["string"]\n}\n\n리뷰: {{review}}',
            good: '아래 리뷰에서 제품 세부 정보를 추출하여 이 스키마의 JSON으로 반환하십시오:\n{\n  "product_name": "string",\n  "rating": 0,\n  "sentiment": "string",\n  "key_features": ["string"]\n}\n\n출력 예시:\n{\n  "product_name": "WH-1000XM5 헤드폰",\n  "rating": 4,\n  "sentiment": "positive",\n  "key_features": ["노이즈 캔슬링", "30시간 배터리", "편안한 착용감"]\n}\n\n리뷰: {{review}}',
            badLabel: '스키마만 — 출력 예시 없음',
            goodLabel: '스키마 + 실제적인 출력 예시 하나',
          },
        ],
        callouts: [
          { type: 'pro-tip', label: '예시 하나로 보통 충분합니다', text: '두 번째 예시는 입력 조건에 따라 데이터 구조가 의미 있게 달라지는 경우에만 가치가 있습니다 — 예를 들어 제품 유형에 따라 특정 필드가 조건부로 존재하는 경우. 두 예시를 초과하면 대부분의 구조화 출력 작업에서 프롬프트 길이 비용이 신뢰도 이득을 초과합니다.' },
          { type: 'warning', label: '사소한 플레이스홀더 값을 피하십시오', text: '"foo", "bar", "test" 또는 `0`을 플레이스홀더로 사용하는 예시는 모델에게 이것들이 유효한 값이라고 가르칩니다. 실제 데이터를 대표하는 값을 사용하십시오 — 실제 제품명, 실제적인 평점, 실제 날짜 문자열.' },
        ],
      },
      fieldInstructions: {
        id: 'field-instructions',
        title: '고위험 출력을 위한 필드 수준 지시사항 작성',
        content: [
        ...
        columns: ['필드 타입', '지시사항 패턴', '예시 지시사항'],
        rows: [
          { '필드 타입': '문자열', '지시사항 패턴': '포맷, 최대 길이, 금지 문자', '예시 지시사항': '"title (string, 최대 100자, HTML 태그 없음)"' },
          { '필드 타입': '숫자', '지시사항 패턴': '정수 vs 부동소수점, 정밀도, 단위', '예시 지시사항': '"price (float, 소수점 정확히 2자리, USD, 통화 기호 없음)"' },
          { '필드 타입': '날짜', '지시사항 패턴': '포맷, 타임존', '예시 지시사항': '"created_at (string, ISO 8601: YYYY-MM-DDTHH:MM:SSZ, UTC 타임존)"' },
          { '필드 타입': 'Enum', '지시사항 패턴': '모든 허용 값을 정확히 나열', '예시 지시사항': '"status (string, 정확히 다음 중 하나: \'active\', \'inactive\', \'pending\')"' },
          { '필드 타입': 'Boolean', '지시사항 패턴': 'true/false만 — yes/no/1/0 거부', '예시 지시사항': '"is_verified (boolean, true 또는 false만 — 1/0이나 yes/no 불가)"' },
          { '필드 타입': 'Nullable', '지시사항 패턴': 'null vs 빈 문자열 vs 생략 시 처리', '예시 지시사항': '"description (string 또는 null — 알 수 없으면 null, 빈 것으로 알려진 경우 빈 문자열)"' },
          { '필드 타입': '배열', '지시사항 패턴': '최소/최대 항목, 항목 타입, 빈 배열 처리', '예시 지시사항': '"tags (문자열 배열, 0–5개 항목, 없으면 [] 반환 — null 절대 불가)"' },
        ],
        tableFormat: true,
        promptExamples: [
          {
            bad: '다음 필드가 있는 JSON을 반환하십시오:\n{\n  "invoice_id": ...,\n  "amount": ...,\n  "due_date": ...,\n  "status": ...,\n  "line_items": [...]\n}',
            good: '다음 필드가 있는 JSON을 반환하십시오:\n{\n  "invoice_id": "string",\n  "amount": 0.00,\n  "due_date": "YYYY-MM-DD",\n  "status": "string",\n  "line_items": [{"description": "string", "quantity": 0, "unit_price": 0.00}]\n}\n\n필드 요구사항:\n- invoice_id: string, 형식 INV-XXXXXX (예: INV-004821)\n- amount: float, 소수점 2자리, 세금 포함 USD 합계\n- due_date: string, ISO 8601 날짜 (YYYY-MM-DD), datetime 아님\n- status: string, 정확히 다음 중 하나: \'paid\', \'unpaid\', \'overdue\', \'cancelled\'\n- line_items: 객체 배열, 1개 이상, 항목 없으면 [] 반환\n- 결정할 수 없는 필드는 null 반환',
            badLabel: '스키마만 — 필드 지시사항 없음',
            goodLabel: '스키마 + 필드 수준 지시사항',
          },
        ],
        callouts: [
          { type: 'key-point', label: '필드 지시사항 추가 시점', text: '다음 경우에 필드 지시사항을 추가하십시오: (1) 특정 포맷 요구사항이 있는 필드(ISO 날짜, 통화 정밀도), (2) enum 필드, (3) nullable이며 null/빈 문자열 구분이 중요한 필드, (4) 테스트 세트에서 해당 필드가 10% 이상 실패하는 경우. "title"이나 "name"처럼 단순하고 명확한 문자열 필드에는 필드 지시사항을 건너뛰십시오.' },
        ],
      },
      formatSelection: {
        id: 'format-selection',
        title: 'API에는 JSON, 프롬프트에는 YAML, 테이블 데이터에는 CSV 선택',
        content: [
        ...
        columns: ['포맷', 'API 강제 없는 신뢰도', '최적 용도', '피해야 할 경우'],
        rows: [
          { '포맷': 'JSON', 'API 강제 없는 신뢰도': '스키마-인-프롬프트로 80–85%', '최적 용도': 'API, 데이터베이스, 타입 안전 소비자', '피해야 할 경우': 'API 강제 없이 복잡한 중첩이 포함된 경우' },
          { '포맷': 'YAML', 'API 강제 없는 신뢰도': '스키마-인-프롬프트로 88–92%', '최적 용도': '사람이 읽기 쉬운 출력, 설정 스타일 데이터, API 강제 없는 경우', '피해야 할 경우': '다운스트림 시스템이 변환 단계 없이 JSON을 요구하는 경우' },
          { '포맷': 'XML', 'API 강제 없는 신뢰도': '스키마-인-프롬프트로 85–90%', '최적 용도': '문서 변환, 레거시 시스템 통합', '피해야 할 경우': '단순 키-값 데이터 (XML이 불필요하게 장황해짐)' },
          { '포맷': 'CSV', 'API 강제 없는 신뢰도': '단순 데이터에서 95%+', '최적 용도': '테이블 데이터, 스프레드시트 내보내기, 데이터 파이프라인', '피해야 할 경우': '데이터에 중첩 또는 계층적 구조가 있는 경우' },
          { '포맷': 'Markdown 테이블', 'API 강제 없는 신뢰도': '단순 테이블에서 높음', '최적 용도': '보고서, 문서, 사람이 읽기 쉬운 테이블 출력', '피해야 할 경우': '기계 판독 가능한 다운스트림 처리가 필요한 경우' },
        ],
        tableFormat: true,
        items: [
        ...
        callouts: [
          { type: 'warning', label: 'YAML-JSON 변환 비용', text: '프롬프트 신뢰도를 위해 YAML을 사용하고 다운스트림 처리에 JSON이 필요하다면 파이프라인에 변환 단계를 추가하십시오. Python의 yaml.safe_load()와 Node.js의 js-yaml이 한 줄로 처리합니다. 팀 전체에 YAML을 적용하기 전 이를 아키텍처에 반영하십시오.' },
        ],
      },
      failureRecovery: {
        id: 'failure-recovery',
        title: '잘못된 출력을 모델 스스로 수정하게 하기',
        content: [
        ...
        promptExamples: [
          {
            bad: '유효하지 않은 출력을 반환하셨습니다. 다시 시도하여 유효한 JSON을 반환하십시오.\n\n{{original_prompt}}',
            good: '이전 응답이 유효성 검사에 실패했습니다. 아래 나열된 오류만 수정하여 올바른 JSON을 반환하십시오.\n\n예상 스키마:\n{\n  "invoice_id": "string",\n  "amount": 0.00,\n  "status": "string"\n}\n\n이전 응답:\n{\n  "invoice_id": null,\n  "amount": "150.00",\n  "status": "PAID"\n}\n\n유효성 검사 오류:\n- invoice_id가 null이지만 필수 문자열 필드입니다 — 입력에서 추출하십시오.\n- amount가 문자열("150.00")이지만 float(150.00)이어야 합니다.\n- status는 소문자여야 합니다: \'PAID\' 대신 \'paid\' 사용\n\n수정된 JSON 객체만 반환하십시오.',
            badLabel: '모호한 재시도 — 오류 컨텍스트 없음',
            goodLabel: '스키마, 출력, 구체적 오류가 포함된 수정 프롬프트',
          },
        ],
        callouts: [
          { type: 'key-point', label: '수정이 두 번 실패하면 기본 프롬프트를 수정하십시오', text: '수정 프롬프트가 두 번째 시도에서도 유효한 출력을 생성하지 못하면 문제는 입력 데이터가 아닌 기본 프롬프트에 있습니다. 재시도를 중단하고 실패 패턴을 진단하십시오: 어떤 필드가 어떤 입력 조건에서 실패하는지. 소스에서 실패를 방지하기 위해 필드 지시사항이나 스키마 변경을 추가하십시오.' },
          { type: 'warning', label: '수정 프롬프트는 지연 시간과 비용을 추가합니다', text: '각 수정 프롬프트는 해당 호출의 API 비용과 지연 시간을 두 배로 늘립니다. 수정 프롬프트는 엣지 케이스 실패(출력의 10% 미만)에만 사용하십시오. 구조화 출력 프롬프트가 10% 이상 실패한다면 프로덕션에 수정 루프를 구축하는 대신 기본 프롬프트를 수정하십시오.' },
        ],
      },
      dataTypePatterns: {
        id: 'data-type-patterns',
        title: '배열, enum, nullable 필드를 위한 프롬프트 패턴',
        content: '**배열, enum, nullable 필드는 스키마-인-프롬프트만으로는 방지할 수 없는 구조화 출력 실패의 세 가지 가장 흔한 원인입니다. 각각은 프롬프트에서 특정 지시사항 패턴이 필요합니다.**',
        columns: ['데이터 타입', '흔한 실패', '방지하는 프롬프트 패턴'],
        rows: [
          { '데이터 타입': '배열 (0개 항목)', '흔한 실패': '모델이 [] 대신 null 반환', '방지하는 프롬프트 패턴': '"항목이 없으면 빈 배열 []을 반환하십시오. 배열 필드에 null을 절대 반환하지 마십시오."' },
          { '데이터 타입': '배열 (1개 이상)', '흔한 실패': '항목이 하나만 발견되면 배열 대신 단일 객체 반환', '방지하는 프롬프트 패턴': '"항목이 하나뿐이더라도 항상 배열을 반환하십시오. 단일 항목은 래핑되어야 합니다: [{...}]"' },
          { '데이터 타입': 'Enum (2–5개 값)', '흔한 실패': '모델이 유사한 값을 약어로 쓰거나 만들어냄', '방지하는 프롬프트 패턴': '"status: 정확히 다음 중 하나: \'active\', \'inactive\', \'pending\' — 약어나 변형 불가"' },
          { '데이터 타입': 'Enum (6개 이상 값)', '흔한 실패': '모델이 목록에 없는 값을 만들어냄', '방지하는 프롬프트 패턴': '모든 값을 번호 목록으로 나열한 다음: "위 목록의 값만 사용하십시오. 값을 약어로 쓰거나 결합하지 마십시오."' },
          { '데이터 타입': 'Nullable 필드', '흔한 실패': '모델이 null 대신 "" 반환하거나 필드를 완전히 생략', '방지하는 프롬프트 패턴': '"값을 알 수 없으면 null을 반환하십시오. 빈 것으로 알려진 경우에만 빈 문자열 \'\'을 반환하십시오. 항상 필드를 포함하십시오 — 생략하지 마십시오."' },
          { '데이터 타입': '정수 vs 부동소수점', '흔한 실패': '정수가 필요한 곳에 부동소수점 반환, 또는 둘 다 문자열', '방지하는 프롬프트 패턴': '"score (integer — 소수점 없음, 예: 4.0이 아닌 4)" 또는 "price (float — 소수점 정확히 2자리, 예: 13이 아닌 12.99)"' },
          { '데이터 타입': '중첩 객체', '흔한 실패': '모델이 중첩 객체를 플랫 키로 축소 (예: {"address": {"city": ...}} 대신 "address.city")', '방지하는 프롬프트 패턴': '스키마 템플릿에 적절한 들여쓰기로 완전한 중첩 구조를 보여주십시오. 중첩에 대한 자연어 설명은 빈번하게 플랫 키로 축소됩니다.' },
        ],
        tableFormat: true,
        callouts: [
          { type: 'warning', label: 'null vs undefined vs 생략', text: 'JSON에는 undefined 값이 없지만 모델은 때때로 있는 것처럼 동작합니다 — null을 반환하는 대신 값이 알 수 없다고 생각할 때 필드를 완전히 생략합니다. 다운스트림 코드가 obj.hasOwnProperty() 같은 검사를 사용한다면 생략된 필드는 null 필드와 다릅니다. "null이더라도 스키마의 모든 필드를 항상 포함하십시오."를 추가하십시오.' },
          { type: 'pro-tip', label: '중첩된 enum은 더 구체적인 지시가 필요합니다', text: '중첩 객체 안의 enum은 최상위 enum보다 오타가 나거나 약어로 쓰일 가능성이 높습니다. 중첩 객체 안에 enum이 있다면 일반 필드 규칙 섹션뿐만 아니라 스키마 템플릿에서 해당 필드가 나타나는 곳 가까이에 지시사항을 반복하십시오.' },
        ],
      },
      testingReliability: {
        id: 'testing-reliability',
        title: '프롬프트의 구조화 출력 신뢰도 측정',
        content: [
        ...
        numberedItems: [
          '**모든 스키마 필드에 대한 통과/실패 기준을 정의하십시오.** 각 필드에 대해: 타입이 올바른지, 필수 필드가 존재하는지, enum 값이 허용 목록에 있는지, 날짜 포맷이 필요한 패턴과 일치하는지. 시각적 검사가 아닌 프로그래밍적 검사로 작성하십시오. 이 단계가 테스트 오라클을 생성합니다.',
          '**20개 케이스 테스트 세트를 구축하십시오.** 정상 경로 입력 10개(일반적이고 잘 형성된 데이터), 엣지 케이스 5개(선택적 필드 누락, 긴 텍스트, 특이한 값, 다국어 콘텐츠), 적대적 입력 5개(필드 값에 포함된 지시사항, 극단적인 날짜, 모호한 타입). 실제 데이터 도메인에서 현실적인 입력을 사용하십시오.',
          '**temperature 0에서 실행하고 필드별 통과/실패를 기록하십시오.** 결정론적이고 반복 가능한 결과를 위해 temperature 0에서 20개 케이스를 모두 실행하십시오. 전체 결과뿐만 아니라 각 테스트 케이스에서 각 필드가 통과하는지 실패하는지 기록하십시오. 필드 수준 실패 패턴이 어떤 지시사항이 누락되었는지 파악합니다.',
          '**가장 낮은 통과율 필드를 수정하고 재테스트하십시오.** 필드 지시사항 하나를 추가하거나 강화하십시오: 타입, 포맷, null 처리, enum 값. 20개 케이스를 모두 다시 실행하십시오. 타겟된 지시사항 추가 하나가 일반적으로 전체 통과율을 5–15 퍼센트 포인트 높입니다. 전체 통과율이 95% 이상에 도달할 때까지 반복하십시오.',
          '**두 번째 모델로 프롬프트를 검증하십시오.** 동일한 프롬프트로 두 번째 모델에서 전체 20개 케이스를 실행하십시오. GPT-5.6에서 95%+이지만 Claude Sonnet 5에서 70%인 프롬프트는 모델 종속적입니다. 두 모델 모두에서 통과할 만큼 명시적인 지시사항을 추가하거나, 어떤 모델로 검증되었는지 문서화하고 재테스트 없이 전환하지 마십시오.',
        ],
        callouts: [
          { type: 'key-point', label: 'temperature 0에서 테스트를 실행하십시오', text: '결정론적이고 반복 가능한 결과를 얻으려면 temperature 0에서 구조화 출력 테스트 세트를 실행하십시오. temperature 0에서 통과하는 프롬프트는 설계상 신뢰할 수 있습니다 — 운이 좋은 것이 아닙니다. 프롬프트가 결정론적으로 95%+를 통과한 후에만 temperature를 높이고, 그 다음 신뢰도가 유지되는지 확인하기 위해 새 temperature에서 테스트 세트를 다시 실행하십시오.' },
          { type: 'pro-tip', label: '다중 모델 비교를 위해 PromptQuorum 사용', text: 'PromptQuorum은 20개 케이스 테스트 세트를 GPT-5.6, Claude Sonnet 5, Gemini 3.1 Pro에서 동시에 실행하고 필드 수준 통과율을 나란히 보여줍니다. 이를 통해 세 번 대신 한 번의 실행으로 모델 종속 실패를 파악할 수 있습니다.' },
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
        title: '구조화 출력 프롬프트의 5가지 흔한 실수',
        content: '**구조화 출력 프롬프트의 5가지 가장 흔한 실수는 모두 동일한 증상 — 간헐적 또는 체계적 실패 — 을 만들어내지만 다른 수정이 필요합니다. 지시사항을 추가하기 전에 어떤 실수를 했는지 진단하면 시간을 절약할 수 있습니다.**',
        mistakes: [
          {
            mistake: '스키마를 임베딩하는 대신 자연어로 설명',
            problem: '자연어 설명은 모호합니다 — "항목 목록"은 배열, 쉼표로 구분된 문자열, 또는 번호 목록을 의미할 수 있으며 "합계"는 문자열이나 부동소수점 모두가 될 수 있습니다.',
            fix: '예상 스키마를 JSON 템플릿으로 프롬프트에 직접 임베딩하십시오. 템플릿은 산문 설명이 아닌 구조를 통해 필드명, 중첩 깊이, 값 타입을 보여줍니다.',
          },
          {
            mistake: '누락되거나 알 수 없는 값 처리 방법을 지정하지 않음',
            problem: '모델은 알 수 없는 필드에 null을 반환하는 대신 그럴듯한 값을 만들어냅니다 — 날짜는 "unknown"이 되고, 금액은 0이 되고, 누락된 ID는 "N/A"가 됩니다 — 이 중 어느 것도 타입 유효성 검사를 통과하지 못합니다.',
          ...
          },
          {
            ...
            problem: 'temperature 0에서 재시도되는 실패 프롬프트는 매번 같은 실패를 생성합니다. 더 높은 temperature에서는 다양하지만 여전히 실패하는 출력을 생성합니다 — 다른 오류, 같은 근본 원인.',
      ...
      faqSection: {
        id: 'faq',
        title: '자주 묻는 질문',
        content: '**구조화 출력 프롬프팅에 관한 가장 흔한 질문들은 JSON 모드와 프롬프트 설계의 경계, 포함할 예시 수, 실패하는 프롬프트를 체계적으로 개선하는 방법을 다룹니다.**',
        faqs: [
          {
            q: 'JSON 모드가 있으면 스키마-인-프롬프트가 불필요한가요?',
            a: '아닙니다. JSON 모드는 파싱 가능한 JSON 구문을 강제하지만 스키마 준수는 보장하지 않습니다. JSON 모드를 사용하는 모델은 여전히 필수 필드가 누락되거나, 잘못된 데이터 타입을 사용하거나, 유효하지 않은 enum 값을 포함하는 유효한 JSON을 반환할 수 있습니다. 스키마-인-프롬프트와 필드 지시사항은 스키마 준수 실패를 다루고, JSON 모드는 파싱 불가 출력만 방지합니다. 두 접근 방식은 상호 보완적이며 대안이 아닙니다.',
          },
          {
            q: '프롬프트에 출력 예시를 몇 개나 포함해야 하나요?',
            a: '예시 하나로 보통 충분하며 가장 큰 신뢰도 향상을 가져옵니다. 두 번째 예시는 입력 조건에 따라 데이터 구조가 의미 있게 달라지는 경우에만 가치가 있습니다 — 예를 들어 입력 유형에 따라 특정 필드가 조건부로 필요한 경우. 두 예시를 초과하면 대부분의 구조화 출력 작업에서 프롬프트 길이 비용이 신뢰도 이득을 초과합니다.',
          },
          {
            q: 'API 강제 없이 구조화 출력에 JSON 또는 YAML을 사용해야 하나요?',
            a: 'API 강제 없이 생성하고 출력을 JSON을 필요로 하는 시스템에서 파싱할 필요가 없는 경우 YAML을 사용하십시오. 닫는 중괄호, 이스케이프 시퀀스, 후행 쉼표 추적이 필요 없기 때문에 모델이 YAML에서 구문 오류를 더 적게 생성합니다. 출력이 JSON을 필요로 하는 API, 데이터베이스, 다운스트림 시스템으로 직접 들어가는 경우 JSON을 사용하십시오. 포맷에 관계없이 항상 파싱하고 유효성을 검사하십시오.',
          },
          {
            q: '구조화 출력 통과율이 70%인 프롬프트를 가장 빠르게 개선하는 방법은 무엇인가요?',
            a: '전체가 아닌 필드 수준에서 테스트 세트를 실행하십시오. 개별 통과율이 가장 낮은 필드를 찾고, 타입, 포맷, null 처리를 다루는 명시적인 지시사항 하나를 추가한 다음 재실행하십시오. 타겟된 필드 지시사항 하나가 일반적으로 전체 통과율을 5–15 퍼센트 포인트 높입니다. 95% 이상에 도달할 때까지 반복하십시오.',
          },
          {
            q: '네이티브 JSON 모드 없이 모델에서 신뢰할 수 있는 구조화 출력을 얻으려면 어떻게 해야 하나요?',
            a: '프롬프트에 전체 JSON 스키마를 템플릿으로 임베딩하고, 유효한 출력 예시 하나를 포함하고, 필드 수준 지시사항을 추가하고, temperature 0에서 실행하십시오. 모든 출력을 파싱하고 유효성을 검사하며, 유효성 검사 실패에 대해 수정 프롬프트를 전송하십시오. 잘 설계된 프롬프트는 네이티브 JSON 모드 없이 temperature 0에서 대부분의 모델에서 85–92%의 신뢰도를 달성합니다.',
          },
          {
            q: '구조화 출력 프롬프트의 올바른 테스트 세트 크기는 얼마인가요?',
            a: '최소 20개: 정상 경로 입력 10개(일반적이고 잘 형성된 데이터), 엣지 케이스 5개(특이한 값, 선택적 필드 누락, 긴 입력), 적대적 입력 5개(모델을 오도할 수 있는 값, 필드 값에 포함된 지시사항, 모호한 타입). 이 크기는 과도한 설정 시간 없이 가장 흔한 실패 범주를 파악합니다.',
          },
          {
            q: '수정 프롬프트를 사용해야 할 때와 기본 프롬프트를 수정해야 할 때는 언제인가요?',
            a: '실패가 드물 때 — 출력의 10% 미만 — 그리고 특이한 엣지 케이스 입력으로 인한 경우 수정 프롬프트를 사용하십시오. 실패가 체계적인 경우 — 여러 테스트 케이스에서 동일한 필드가 누락되거나 동일한 타입 오류가 나타나는 경우 — 기본 프롬프트를 수정하십시오. 수정 프롬프트는 실패마다 지연 시간과 API 비용을 추가하며, 더 나은 기본 프롬프트는 실패를 완전히 방지합니다.',
          },
          {
            q: '스키마에서 필드 순서가 구조화 출력 신뢰도에 영향을 미치나요?',
            a: '예. 필수 필드를 먼저, 선택적 또는 nullable 필드를 마지막에 배치하십시오. 모델은 무엇을 포함할지 결정할 때 스키마의 앞 요소에 더 많은 가중치를 부여합니다. 첫 번째로 나열된 nullable 필드는 모델이 값에 대해 불확실할 때 나중에 나열된 필수 필드보다 생략될 가능성이 높습니다. 이 순서 효과는 GPT-5.6와 Claude Sonnet 5 모두에서 일관됩니다.',
          },
        ],
      },
      relatedReading: {
        ...
        items: [
          ...
          '[여러 모델에서 프롬프트를 테스트하는 방법](/ko/prompt-engineering/how-to-test-prompts-across-models) — 모델별 실패를 찾기 위해 GPT-5.6, Claude Sonnet 5, Gemini 3.1 Pro에서 동일한 프롬프트 실행',
      ...
      sources: {
        ...
        title: '출처',
        items: [
          '[OpenAI 구조화 출력 문서](https://platform.openai.com/docs/guides/structured-outputs) — OpenAI API의 response_format 및 JSON 모드 기술 사양',
          '[Anthropic tool use 문서](https://docs.anthropic.com/en/docs/build-with-claude/tool-use) — Claude의 tool_use 파라미터가 API 수준에서 구조화 출력을 강제하는 방법',
          '[Google Gemini GenerationConfig 문서](https://ai.google.dev/api/generate-content#v1beta.GenerationConfig) — 네이티브 JSON 출력을 위한 Gemini의 responseMimeType 설정',
          '[BAML 벤치마크: 구조화 출력 정확도 트레이드오프](https://docs.boundaryml.com/benchmarks) — 여러 모델에서 제약 생성과 비제약 생성 간의 신뢰도 차이에 대한 증거',
          '[NIST AI 위험 관리 프레임워크](https://www.nist.gov/artificial-intelligence/ai-risk-management-framework) — 프로덕션 시스템에서 AI 출력 유효성 검사를 위한 거버넌스 원칙',
        ],
      },
    },
  },
