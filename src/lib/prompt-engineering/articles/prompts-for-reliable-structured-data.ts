...
export const article: Partial<Record<Language, PEArticle>> = {
    ...
    sections: {
      ...
      testingReliability: {
        ...
        callouts: [
          ...
          { type: 'pro-tip', label: 'Use PromptQuorum for multi-model comparison', text: 'PromptQuorum runs your 20-case test set against GPT-5.6, Claude Sonnet 5, and Gemini 2.5 Pro simultaneously and shows field-level pass rates side-by-side. This identifies model-dependent failures in one run instead of three.' },
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[How To Test Prompts Across Models](/prompt-engineering/how-to-test-prompts-across-models) — running the same prompt against GPT-5.6, Claude Sonnet 5, and Gemini 2.5 Pro to find model-specific failures',
  ...
  de: {
    ...
    leadAnswerBlock: '**Drei Prompt-Muster erreichen 95% oder mehr strukturierte Output-Zuverlässigkeit ohne API-Änderungen: Schema im Prompt einbetten, dem Modell ein gültiges Ausgabebeispiel zeigen und feldspezifische Anweisungen für Typ, Format und null-Behandlung hinzufügen.** Diese Muster funktionieren über GPT-5.6, Claude Sonnet 5 und Gemini 2.5 Pro, mit oder ohne nativen JSON-Modus.',
    publishDate: '2026-04-30',
    dateModified: '2026-07-13',
    ...
    sections: {
      ...
      testingReliability: {
        ...
        callouts: [
          ...
          { type: 'pro-tip', label: 'Nutzen Sie PromptQuorum für Multi-Modell-Vergleich', text: 'PromptQuorum führt Ihr 20-Fall-Test-Set gegen GPT-5.6, Claude Sonnet 5 und Gemini 2.5 Pro gleichzeitig aus und zeigt feldspezifische Pass-Raten Seite an Seite. Dies identifiziert modellabhängige Fehler in einem Durchlauf statt drei.' },
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[Wie Sie Prompts über Modelle hinweg testen](/de/prompt-engineering/how-to-test-prompts-across-models) — Ausführen des gleichen Prompts gegen GPT-5.6, Claude Sonnet 5 und Gemini 2.5 Pro, um modellabhängige Fehler zu finden',
    ...
    sections: {
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[Cómo probar prompts en diferentes modelos](/es/prompt-engineering/how-to-test-prompts-across-models) — ejecutar el mismo prompt en GPT-5.6, Claude Sonnet 5 y Gemini 2.5 Pro para encontrar fallos específicos del modelo',
    ...
    sections: {
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[كيفية اختبار الموجّهات على نماذج مختلفة](/ar/prompt-engineering/how-to-test-prompts-across-models) — تشغيل نفس الموجّه في GPT-5.6 و Claude Sonnet 5 و Gemini 2.5 Pro لاكتشاف الإخفاقات الخاصة بالنموذج',
    ...
    sections: {
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[Como testar prompts em diferentes modelos](/pt/prompt-engineering/how-to-test-prompts-across-models) — executar o mesmo prompt no GPT-5.6, Claude Sonnet 5 e Gemini 2.5 Pro para encontrar falhas específicas do modelo',
    ...
    sections: {
      ...
      testingReliability: {
        ...
        callouts: [
          ...
          { type: 'pro-tip', label: 'Utiliser PromptQuorum pour comparaison multi-modèle', text: 'PromptQuorum exécute votre ensemble de test de 20 cas sur GPT-5.6, Claude Sonnet 5 et Gemini 2.5 Pro simultanément et affiche les taux de réussite par champ côte à côte. Cela identifie les défaillances dépendantes du modèle en un run au lieu de trois.' },
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[Comment Tester les Prompts sur les Modèles](/fr/prompt-engineering/how-to-test-prompts-across-models) — Exécution du même prompt sur GPT-5.6, Claude Sonnet 5 et Gemini 2.5 Pro pour trouver les défaillances dépendantes du modèle',
  ...
  ja: {
    ...
    leadAnswerBlock: '**3つのプロンプトパターンはAPI変更なしで95%以上の信頼性を達成します: スキーマをプロンプトに埋め込む、モデルに1つの有効な出力例を見せる、タイプ、形式、null処理のフィールドレベルの指示を追加する。** これらのパターンはGPT-5.6、Claude Sonnet 5、Gemini 2.5 Proで動作し、ネイティブJSON-modeの有無は問いません。',
    publishDate: '2026-04-30',
    dateModified: '2026-07-13',
    readTime: '9分で読める',
    educationalLevel: 'Intermediate',
    ...
    sections: {
      ...
      testingReliability: {
        ...
        callouts: [
          ...
          { type: 'pro-tip', label: 'マルチモデル比較でPromptQuorumを使用', text: 'PromptQuorumは20ケースセットをGPT-5.6、Claude Sonnet 5、Gemini 2.5 Proで同時にテストし、フィールドごとのパスレートを並べて表示します。これにより、モデル依存の失敗が1回の実行で識別され、3回ではなく。' },
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[モデル全体でプロンプトをテストする方法](/ja/prompt-engineering/how-to-test-prompts-across-models) — GPT-5.6、Claude Sonnet 5、Gemini 2.5 Pro全体で同じプロンプトを実行して、モデル依存の失敗を検出',
  ...
  zh: {
    ...
    leadAnswerBlock: '**三种提示词模式无需改动 API 即可将结构化输出可靠性提升到 95% 或以上：将 schema 嵌入提示词、向模型展示一个有效的输出示例、以及为类型、格式和 null 处理添加字段级指令。** 这些模式在 GPT-5.6、Claude Sonnet 5 和 Gemini 2.5 Pro 上均有效，无论是否使用原生 JSON mode。',
    publishDate: '2026-04-30',
    dateModified: '2026-07-13',
    readTime: '阅读约9分钟',
    educationalLevel: 'Intermediate',
    ...
    sections: {
      ...
      testingReliability: {
        ...
        callouts: [
          ...
          { type: 'pro-tip', label: '使用 PromptQuorum 进行多模型对比', text: 'PromptQuorum 会同时用你的 20 用例测试集对 GPT-5.6、Claude Sonnet 5 和 Gemini 2.5 Pro 运行，并并排显示字段级通过率。这能在一次运行中识别出依赖于模型的失败，而不是分三次。' },
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[如何跨模型测试提示词](/zh/prompt-engineering/how-to-test-prompts-across-models) —— 用同一提示词对 GPT-5.6、Claude Sonnet 5 和 Gemini 2.5 Pro 运行，以发现特定于模型的失败',
  ...
  ko: {
    ...
    leadAnswerBlock: 'API 변경 없이 구조화 출력 신뢰도를 95% 이상으로 높이는 세 가지 프롬프트 패턴이 있습니다: 스키마를 프롬프트에 직접 임베딩하기, 유효한 출력 예시 하나 제시하기, 타입·포맷·null 처리에 대한 필드 수준 지시사항 추가하기. 이 패턴들은 네이티브 JSON 모드 사용 여부에 관계없이 GPT-5.6, Claude Sonnet 5, Gemini 2.5 Pro 모두에서 작동합니다.',
    publishDate: '2026-04-30',
    dateModified: '2026-07-13',
    readTime: '9분 읽기',
    educationalLevel: 'Intermediate',
    ...
    sections: {
      ...
      testingReliability: {
        ...
        callouts: [
          ...
          { type: 'pro-tip', label: '다중 모델 비교를 위해 PromptQuorum 사용', text: 'PromptQuorum은 20개 케이스 테스트 세트를 GPT-5.6, Claude Sonnet 5, Gemini 2.5 Pro에서 동시에 실행하고 필드 수준 통과율을 나란히 보여줍니다. 이를 통해 세 번 대신 한 번의 실행으로 모델 종속 실패를 파악할 수 있습니다.' },
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
