...
export const article: Record<Language, PEArticle> = {
  ...
  ja: {
    ...
    howToSchema: {
      '@context': 'https://schema.org',
      '@type': 'HowTo',
      name: '構造化出力信頼性を測定および改善する方法',
      inLanguage: 'ja',
      totalTime: 'PT2H',
      step: [
        {
          '@type': 'HowToStep',
          name: 'スキーマの各フィールドの合格/不合格基準を定義する',
          text: 'スキーマの各フィールドについて、プログラム的に実行される二項チェックを記述します: タイプが正しい、必須フィールドが存在する、列挙値が許可されたリストにある、日付形式が期待されたパターンに一致する。これらのチェックはテストオラクルです。',
        },
        {
          '@type': 'HowToStep',
          name: '20ケースのテストセットを構築する',
          text: '20個のテスト入力を作成します: 10個のhappy-path (典型的、整形式)、5個のエッジケース (不足しているオプションフィールド、長いテキスト、異常な値)、5個の敵対的 (フィールド値に埋め込まれた指示、極端な日付、曖昧なタイプ)。',
        },
        {
          '@type': 'HowToStep',
          name: '温度0で実行し、フィールドごとの成功率を記録する',
          text: '確定的な結果を得るために、温度0でプロンプトに対して20ケースをすべて実行します。全体ではなく、フィールドごとの成功/不合格を記録します。フィールドレベルの失敗は、どの指示が不足しているか、または曖昧かを正確に識別します。',
        },
        {
          '@type': 'HowToStep',
          name: '成功率が最も低いフィールドを修正し、再度テストする',
          text: '失敗が最も多いフィールドを特定します。その指示を追加または強化します: タイプ、許可された値、形式、null処理。20ケースを再度実行します。繰り返すまで、全体的な成功率が95%以上に達します。',
        },
        {
          '@type': 'HowToStep',
          name: '2番目のモデルでプロンプトを検証する',
          text: '2番目のモデルに対して同じ20ケースセットを実行します。GPT-4oで95%以上、Claude 4.6 Sonnetで70%のプロンプトはモデル依存です。どちらでも成功するように指示をより明確にするか、プロンプトがどのモデルに対して検証されているかを文書化します。',
        },
      ],
    },
    faqSchema: {
      ...
      mainEntity: [
        {
          '@type': 'Question',
          name: 'JSON-modeはプロンプト内のスキーマを不要にしますか?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'いいえ。JSON-modeは解析可能なJSON、スキーマ準拠を強制します。JSON-modeを使用するモデルでも、不足しているフィールド、間違ったデータ型、または無効な列挙値を含む有効なJSONを返すことができます。スキーマとフィールド指示は、スキーマ準拠の失敗に対処します; JSON-modeは解析不可能な出力を防ぐだけです。2つのアプローチは代替ではなく補完的です。',
          },
        },
        {
          '@type': 'Question',
          name: 'プロンプトに含める出力例はいくつですか?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: '1つの例は通常十分で、最大の信頼性獲得を提供します。2番目の例は、入力条件に応じてデータが意味の異なる構造を持つ場合にのみ価値があります — 例えば、特定のフィールドが入力型に基づいて条件付きで必須の場合。2つ以上の例を超えて、プロンプトの長さコストは、ほとんどの構造化出力タスクの信頼性メリットを超えます。',
          },
        },
        {
          '@type': 'Question',
          name: 'フィールドレベルの指示は信頼性を95%を超えることができますか?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'はい、ただし収益逓減です。既に信頼できるフィールド (95%以上個別に) の追加のフィールド指示は、全体的な率を1–2ポイント以上上げることはめったにありません。<85%成功率のフィールドに集中してください; そこで1指示あたり5–15ポイントの改善が見えます。95%の全体に達した後、利益は限界です; 残りの境界ケースに修正プロンプトを使用してください。',
          },
        },
        {
          '@type': 'Question',
          name: '同じプロンプトを異なる温度でテストすべきですか?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'はい。確定的で再現可能な結果を得るために、まず温度0でテストします。プロンプトが温度0で95%以上に達した場合のみ、温度を上げて再度テストします。温度0で95%、温度1で60%のプロンプトは温度に敏感です — より明確な指示を追加するか、温度0に留まる必要があります。',
          },
        },
        {
          '@type': 'Question',
          name: '95%成功率は本番環境に「十分」ですか?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'ほとんどの本番システムではい、注意事項付きです。1日あたり100の構造化出力リクエストの95%信頼性で、約5日のエラーが発生します; 1000/日で約50です。85%未満 — 修正ループが必須です。99%以上は非クリティカルデータに不要です。対象: 下流の許容度に応じて95–97%。',
          },
        },
        {
          '@type': 'Question',
          name: 'スキーマ内のフィールド順序は構造化出力信頼性に影響しますか?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'はい。必須フィールドを最初に配置し、オプションまたはnullableフィールドを最後に配置します。モデルは、含めるかどうかを決定する際に、スキーマの初期要素をより重く重み付けます。最初にリストされているnullableフィールドは、モデルが値について不確実な場合、後でリストされている必須フィールドより省略される可能性が高くなります。この順序効果はGPT-4oとClaude 4.6 Sonnet全体で一貫しています。',
          },
        },
        {
          '@type': 'Question',
          name: '構造化出力を使用する際、日本語でのベストプラクティスはありますか?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'はい。日本の開発チームは国際的なベストプラクティスと同じアプローチを適用します。スキーマ埋め込み、出力例、フィールド指示は言語に依存しません。ただし、プロンプトの指示では、です/ます形式を使用して敬意を払い、日本の読者向けにテクニカルタームを英語 (JSON、API、GPU) のまま使用することをお勧めします。',
          },
        },
        {
          '@type': 'Question',
          name: 'スキーマなしで構造化出力プロンプトをテストできますか?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: '技術的にはい、ただし手間がかかります。20ケースを手動でテストし、結果をスプレッドシートに記録し、フィールドごとの成功を計算できます。ただし、自動化 — PythonまたはNode.jsスクリプト — により、より速く反復でき、プロンプト改訂間の変更を追跡できます。可能であれば自動化から始めてください。',
          },
        },
        {
          '@type': 'Question',
          name: 'プロンプトが15%以上失敗する場合はどうしますか?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'パターンを診断します: どのフィールドが失敗し、どの入力条件で。そのフィールドの特定の指示を追加または強化します。再度テストします。本番環境で修正ループを使用しないでください — コストが倍になります。ソースでプロンプトを修正してください。パターンを特定できない場合は、まったく異なるアプローチを検討してください: API JSON-mode強制、別のLLM、または少し変更されたタスク。',
          },
        },
        {
          '@type': 'Question',
          name: 'PromptQuorumはこのワークフローにどのように役立ちますか?',
          acceptedAnswer: {
            '@type': 'Answer',
            text: 'PromptQuorumは20ケースセットをGPT-4o、Claude 4.6 Sonnet、Gemini 2.5 Proで自動的にテストし、フィールドごとの成功率を並べて表示します。これにより、モデル依存の失敗が1回の実行で特定され、3回の実行ではなく識別されます。どのモデルにどの問題があり、どのフィールドが強化された指示が必要かがすぐわかります。',
          },
        },
      ],
    },
