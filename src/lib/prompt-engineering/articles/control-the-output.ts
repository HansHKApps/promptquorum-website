...
export const article: Partial<Record<Language, PEArticle>> = {
      ...
      sections: {
        ...
        definition: {
          ...
          content: [
            ...
            'The two-stage approach — letting Claude Opus 4.7 (Anthropic) or GPT-4o (OpenAI) reason freely in Stage 1, then feeding output into a small specialist structuring model (Osmosis-Structure-0.6B, trained on 500K synthetic unstructured → structured transformations) in Stage 2 — achieves format guarantees without the reasoning quality penalty of constrained decoding.',
        ...
        promptStructure: {
          ...
          content: [
            'Explicit output schema instructions — placed at the start of the system prompt for Claude Opus 4.7 and immediately before user content for GPT-4o — produce structured output compliance rates of 85–95% without the reasoning quality penalty of native constrained decoding.',
        ...
        reasoningTradeoff: {
          ...
          content: [
            ...
            'The production-grade solution for systems requiring both reasoning depth and format guarantees: (1) **Stage 1** — Send to GPT-4o or Claude Opus 4.7 without constraints: "Analyse this, reason step by step, explain your logic." (2) **Stage 2** — Feed Stage 1 output to a small specialist model (Osmosis-Structure-0.6B or GPT-4o-mini with `strict: true`): "Extract the key data from this analysis and return it in this exact JSON schema."',
        ...
        promptquorumTest: {
          ...
          content: [
            'Tested in [PromptQuorum](https://www.promptquorum.com/) — 30 output control prompts dispatched across three models: Claude Opus 4.7 achieved 93% JSON compliance using XML-tagged format instructions without constrained decoding. GPT-4o achieved 89% compliance using numbered format rules. Gemini 3.1 Pro achieved 91% compliance with schema stated at both start and end. All three models produced shorter, less complete reasoning when `strict: true` constrained decoding was enabled — consistent with the 2.26-point accuracy drop observed on the BFCL benchmark.',
          ],
        },
        ...
        tldr: {
          ...
          items: [
            ...
            'Claude Opus 4.7 achieves 93% JSON compliance with XML-tagged format prompts; GPT-4o achieves 89% with numbered format rules — both without constrained decoding',
          ],
        },
      ...
      sections: {
        ...
        tldr: {
          ...
          items: [
            ...
            'Claude Opus 4.7 erreicht 93 % JSON-Compliance mit XML-getaggten Format-Prompts; GPT-4o erreicht 89 % mit nummerierten Format-Regeln — beide ohne Constrained Decoding',
          ],
        },
    ...
    es: {
      ...
      seoTitle: 'Controla el formato de salida de la IA y el cumplimiento de schema (2026)',
      metaDescription: 'Controla la salida de LLMs con modo JSON y constrained decoding. Alcanza el 100 % de cumplimiento de schema usando pipelines de dos etapas sin sacrificar calidad de razonamiento. Guía de producción para GPT, Claude y Gemini.',
      ...
      schema: {
        ...
        url: 'https://www.promptquorum.com/prompt-engineering/control-the-output?lang=es',
        inLanguage: 'es',
        datePublished: '2026-03-24',
        dateModified: '2026-04-29',
        author: { '@type': 'Person', name: 'Hans Kuepper', sameAs: 'https://www.promptquorum.com/about' },
        publisher: { '@type': 'Organization', name: 'PromptQuorum', url: 'https://www.promptquorum.com', logo: { '@type': 'ImageObject', url: 'https://www.promptquorum.com/logo.svg' } },
        image: { '@type': 'ImageObject', url: 'https://www.promptquorum.com/api/og/control-the-output?lang=es', width: 1200, height: 630 },
      ...
      sections: {
        ...
        goodPrompt: {
          title: '¿Cómo es un buen prompt de salida estructurada (Claude Opus 4.7)?',
          content: ['**Buen prompt — Claude Opus 4.7**'],
          blockquote: '<output_format>\nReturn only this JSON object, no prose:\n{\n  "sentiment": "positive" | "neutral" | "negative",\n  "key_issues": ["string"],  // max 3 items\n  "urgency": "low" | "medium" | "high",\n  "confidence": 0.0–1.0\n}\n</output_format>\n\n<task>Analyse the following customer review.</task>\n\n<review>[REVIEW TEXT HERE]</review>',
        },
        promptOutcome: {
          content: [
        ...
        relatedReading: {
          ...
          items: [
            '[¿Qué es el prompt engineering?](/prompt-engineering/what-is-prompt-engineering?lang=es) — principios fundamentales detrás del diseño de instrucciones de IA estructuradas',
            '[Temperature y Top-P explicados](/prompt-engineering/temperature-and-top-p?lang=es) — análisis profundo de los dos parámetros primarios de aleatoriedad',
            '[Escribe mejor código con IA](/prompt-engineering/write-better-code-with-ai?lang=es) — aplicando técnicas de control de salida en workflows de generación de código',
            '[Tool use y function calling](/prompt-engineering/tool-use-and-function-calling?lang=es) — salida estructurada vía definiciones de herramientas y schemas de función',
            '[Tokens y economía de tokens](/prompt-engineering/tokens-costs-limits-economics-of-ai-prompting?lang=es) — comprensión de los costos de tokens para constrained decoding y pipelines de dos etapas',
            '[Manejo de errores en aplicaciones LLM](/prompt-engineering/error-handling-llm?lang=es) — detección y recuperación de salida malformada en sistemas de producción',
          ],
        },
        howToStart: {
      ...
      sections: {
        ...
        tldr: {
          ...
          items: [
            ...
            'Claude Opus 4.7 atteint 93 % de conformité JSON avec des prompts de format XML ; GPT-4o atteint 89 % avec des règles numérotées — tous deux sans decoding contraint',
          ],
        },
    ...
    ja: {
      ...
      sections: {
        ...
        promptquorumTest: {
          title: 'トップモデルの出力制御比較',
          content: [
            '[PromptQuorum](https://www.promptquorum.com/)でテスト済み — 30件の出力制御プロンプトを3モデルに分散：Claude Opus 4.7は制約デコードなしのXMLタグ付きフォーマット指示で93%のJSON準拠を達成。GPT-4oは番号付きフォーマットルールで89%の準拠を達成。Gemini 3.1 Proはスキーマを冒頭と末尾の両方に指定すると91%の準拠を達成。`strict: true`の制約デコードが有効な場合、3モデルすべてがより短く完全性の低い推論を生成しました — BFCLベンチマークで観察された2.26ポイントの精度低下と一致します。',
          ],
        },
        ...
        tldr: {
          ...
          items: [
            ...
            'Claude Opus 4.7はXMLタグ付きフォーマットプロンプトで93%のJSON準拠を達成；GPT-4oは番号付きフォーマットルールで89% — どちらも制約デコードなし',
          ],
        },
    ...
    zh: {
      ...
      sections: {
        ...
        promptquorumTest: {
          ...
          content: [
            '在[PromptQuorum](https://www.promptquorum.com/)中测试——30个输出控制Prompt分发到三个模型：Claude Opus 4.7使用XML标签格式指令（不启用受限解码）达到93% JSON合规率。GPT-4o使用编号格式规则达到89%合规率。Gemini 3.1 Pro在开头和结尾均说明Schema的情况下达到91%合规率。启用`strict: true`受限解码后，三个模型的推理更短、更不完整——与BFCL基准上观察到的2.26百分点准确率下降一致。',
          ],
        },
