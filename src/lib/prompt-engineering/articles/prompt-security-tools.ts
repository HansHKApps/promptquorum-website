...
export const article: Partial<Record<Language, PEArticle>> = {
  en: {
    ...
    quickFacts: [
      ...
      'PromptQuorum runs the same attack probes across GPT-5.6, Claude Sonnet 5, and Gemini 3.1 Pro to detect model-specific vulnerabilities.',
    ],
    toc: [
    ...
    sections: {
      ...
      tools: {
        ...
        content: [
          ...
          'PromptQuorum runs the same set of attack probes across multiple models simultaneously (e.g., GPT-5.6, Claude Sonnet 5, Gemini 3.1 Pro). This identifies which models are more susceptible to specific attack patterns and helps you make model selection decisions based on security behavior, not just output quality.',
        ],
        callouts: [
  ...
  de: {
    ...
    quickFacts: [
      ...
      'PromptQuorum führt dieselben Angriffstests über GPT-5.6, Claude Sonnet 5 und Gemini 3.1 Pro durch, um modellspezifische Schwachstellen zu erkennen.',
    ],
    toc: [
    ...
    sections: {
      ...
      tools: {
        ...
        content: [
          ...
          'PromptQuorum führt dieselben Angriffstests über mehrere Modelle (GPT-5.6, Claude Sonnet 5, Gemini 3.1 Pro) durch. Dies identifiziert, welche Modelle anfälliger für spezifische Angriffsmuster sind, und hilft bei Modellauswahl-Entscheidungen auf Basis des Sicherheitsverhaltens.',
        ],
        callouts: [
  ...
  es: {
    ...
    quickFacts: [
      ...
      'PromptQuorum ejecuta los mismos probes de ataque en GPT-5.6, Claude Sonnet 5 y Gemini 3.1 Pro para detectar vulnerabilidades específicas de cada modelo.',
    ],
    toc: [
  ...
  pt: {
    ...
    quickFacts: [
      ...
      'PromptQuorum executa os mesmos probes de ataque no GPT-5.6, Claude Sonnet 5 e Gemini 3.1 Pro para detectar vulnerabilidades específicas de cada modelo.',
    ],
    toc: [
    ...
    sections: {
      ...
      tools: {
        ...
        content: [
          ...
          'PromptQuorum executa o mesmo conjunto de probes de ataque em múltiplos modelos simultaneamente (por exemplo, GPT-5.6, Claude Sonnet 5, Gemini 3.1 Pro). Isso identifica quais modelos são mais suscetíveis a padrões de ataque específicos e ajuda você a tomar decisões de seleção de modelos com base no comportamento de segurança, não apenas na qualidade da saída.',
        ],
        callouts: [
  ...
  fr: {
    ...
    quickFacts: [
      ...
      'PromptQuorum exécute les mêmes sondes d\'attaque sur GPT-5.6, Claude Sonnet 5 et Gemini 3.1 Pro pour détecter les vulnérabilités spécifiques aux modèles.',
    ],
    toc: [
    ...
    sections: {
      ...
      tools: {
        ...
        content: [
          ...
          'PromptQuorum exécute les mêmes sondes d\'attaque sur plusieurs modèles (GPT-5.6, Claude Sonnet 5, Gemini 3.1 Pro) simultanément pour identifier lesquels sont plus susceptibles à des schémas d\'attaque spécifiques.',
        ],
        callouts: [
  ...
  ja: {
    ...
    quickFacts: [
      ...
      'PromptQuorumはGPT-5.6、Claude Sonnet 5、Gemini 3.1 Proで同じ攻撃プローブを実行し、モデル固有の脆弱性を検出。',
    ],
    toc: [
    ...
    sections: {
      ...
      tools: {
        ...
        content: [
          ...
          'PromptQuorumは複数のモデル（GPT-5.6、Claude Sonnet 5、Gemini 3.1 Pro）で同じ攻撃プローブを実行し、どのモデルが特定の攻撃パターンに対してより脆弱かを特定します。',
        ],
        callouts: [
  ...
  zh: {
    ...
    quickFacts: [
      ...
      'PromptQuorum在GPT-5.6、Claude Sonnet 5和Gemini 3.1 Pro上运行相同的攻击探针，检测特定模型漏洞。',
    ],
    toc: [
    ...
    sections: {
      ...
      tools: {
        ...
        content: [
          ...
          'PromptQuorum在多个模型（GPT-5.6、Claude Sonnet 5、Gemini 3.1 Pro）上同时运行相同的攻击探针，识别哪些模型对特定攻击模式更容易受到攻击。',
        ],
        callouts: [
  ...
  ko: {
    ...
    ogDescription: '프롬프트 인젝션 취약점 탐지: 직접·간접 공격, Garak·PyRIT 도구, 4계층 방어 스택. PromptQuorum으로 다중 모델 테스트.',
    twitterDescription: '프롬프트 인젝션은 OWASP LLM01 — 보안 위험 1위. 자동화 테스트에 Garak, 레드팀에 PyRIT, 4계층 방어 스택.',
    publishDate: '2026-05-02',
    readTime: '11분 분량',
    educationalLevel: 'Advanced',
    primaryTerm: 'Prompt Injection',
    leadAnswerBlock: '**프롬프트 인젝션은 공격자가 사용자 제공 입력에 지침을 삽입하여 system prompt를 무력화하고 모델 동작을 변경하는 공격입니다.** LLM 애플리케이션에서 가장 일반적인 보안 취약점이며 입력에 완전히 의존하는 유일한 취약점입니다.',
    quickFacts: [
      ...
      'PromptQuorum은 GPT-5.6, Claude Sonnet 5, Gemini 3.1 Pro에서 동일한 공격 프로브를 실행하여 모델별 취약점을 탐지합니다.',
    ],
    toc: [
      { label: '프롬프트 인젝션이란 무엇인가', anchor: 'what_is_injection' },
      { label: '직접 인젝션: 패턴과 탐지', anchor: 'direct_injection' },
      { label: '간접 인젝션: 데이터가 공격이 될 때', anchor: 'indirect_injection' },
      { label: '프롬프트 보안 테스트 도구', anchor: 'tools' },
      { label: '입력 위생 처리와 출력 검증 패턴', anchor: 'defenses' },
    ...
    schema: {
      '@context': 'https://schema.org',
      '@type': 'TechArticle',
      headline: '프롬프트 보안 테스트: 인젝션 취약점 탐지 도구와 방법',
      description: '프롬프트 인젝션 테스트: 직접 vs 간접 인젝션, 도구, 입력 위생 처리, 출력 검증, OWASP LLM Top 10 커버리지.',
      url: 'https://www.promptquorum.com/ko/prompt-engineering/prompt-security-tools',
      datePublished: '2026-05-02',
      author: { '@type': 'Person', name: 'Hans Kuepper', sameAs: 'https://www.linkedin.com/in/hanskuepper/' },
      publisher: { '@type': 'Organization', name: 'PromptQuorum', url: 'https://www.promptquorum.com' },
    ...
    sections: {
      tldr: {
        ...
        title: '핵심 요점',
        isTldr: true,
        content: '프롬프트 인젝션은 OWASP LLM01 — LLM 보안 위험 1위입니다. 직접 인젝션(공격자가 사용자 입력을 제어)과 간접 인젝션(공격자가 RAG 문서와 같은 데이터 소스를 오염) 모두 테스트하십시오. 40개 이상의 공격 패턴에 대한 자동화 커버리지에 Garak, 다중 턴 레드팀에 PyRIT를 사용하십시오. 방어에는 4계층이 필요합니다: 입력 필터링, 출력 스키마 적용, 권한 분리, 지침 격리. 여러 모델에서 동일한 공격 프로브를 실행하려면 PromptQuorum을 사용하십시오.',
      },
      what_is_injection: {
        id: 'what-is-injection',
        title: '프롬프트 인젝션이란 무엇인가',
        snippets: [
          { type: 'in-one-sentence', text: '프롬프트 인젝션은 공격자가 사용자 제공 입력에 지침을 삽입하여 system prompt를 무력화하고 모델 동작을 변경하는 공격입니다.' },
          { type: 'in-plain-terms', text: '누군가에게 작성 양식을 줬는데 그 사람이 여백에 다른 모든 것을 무시하라는 지침을 적는 것을 상상하십시오. 프롬프트 인젝션은 LLM에 동일한 작업을 수행합니다: 공격자가 사용자 입력(또는 LLM이 읽는 문서)에 명령을 삽입하여 의도된 동작을 무력화합니다.' },
        ],
        content: [
        ...
        callouts: [
          { type: 'warning', label: 'OWASP LLM Top 10 #1', text: '프롬프트 인젝션은 LLM01 — LLM 애플리케이션에서 가장 일반적이고 높은 영향의 취약점이기 때문에 첫 번째로 분류됩니다. 외부 입력을 허용하는 모든 LLM 애플리케이션이 노출되어 있습니다.' },
        ],
      },
      direct_injection: {
        id: 'direct-injection',
      ...
      tools: {
        ...
        content: [
          ...
          'PromptQuorum은 여러 모델에서 동시에 동일한 공격 프로브 세트를 실행합니다(예: GPT-5.6, Claude Sonnet 5, Gemini 3.1 Pro). 이를 통해 특정 공격 패턴에 더 취약한 모델을 식별하고 출력 품질만이 아닌 보안 동작을 기반으로 모델 선택 결정을 내릴 수 있습니다.',
        ],
        callouts: [
      ...
      defenses: {
        ...
        callouts: [
          { type: 'insight', label: '심층 방어는 필수', text: '어떤 단일 계층도 프롬프트 인젝션을 막지 못합니다. 차단 목록만으로는 패러프레이징으로 우회됩니다; 스키마 적용만으로는 데이터 유출을 방지하지 못합니다. 네 계층 모두 동시에 활성화되어야 합니다.' },
        ],
      },
      common_mistakes: {
        id: 'common-mistakes',
        title: '프롬프트 보안 테스트의 일반적인 실수',
        mistakes: [
          { mistake: '직접 인젝션만 테스트', problem: '검색된 문서를 통한 간접 인젝션이 프로덕션에서 더 일반적이며 테스트되지 않습니다', fix: '간접 인젝션 경로를 테스트하십시오: RAG 문서, API 응답, 사용자 제어 메타데이터 필드' },
          { mistake: '출력 스키마 적용 없음', problem: '비구조화 출력은 무제한 인젝션 표면을 만듭니다', fix: '모든 자동화 파이프라인에 출력 스키마(JSON 모드, Zod/Pydantic 검증)를 적용하십시오' },
          { mistake: '정적 차단 목록만', problem: '차단 목록은 새 패턴을 놓치고 인코딩 변형으로 우회됩니다', fix: '차단 목록을 시맨틱 의도 탐지와 권한 분리와 결합하십시오' },
          { mistake: '권한 분리 없음', problem: '모델에 쓰기/실행 접근이 있으면 성공적인 인젝션으로 되돌릴 수 없는 피해가 발생할 수 있습니다', fix: '최소 권한 원칙 적용: 검색 모델에는 읽기 전용, 도구를 사용하는 모델에는 별도의 실행 환경' },
        ],
      },
      key_takeaways: {
        id: 'key-takeaways',
      ...
      faq: {
        ...
        title: '자주 묻는 질문',
        faqs: [
          { q: '프롬프트 인젝션이란 무엇입니까?', a: '프롬프트 인젝션은 공격자가 사용자 제공 입력에 지침을 삽입하여 system prompt를 무력화하고 모델 동작을 변경하는 공격입니다. OWASP LLM Top 10에서 LLM01로 분류됩니다 — LLM 애플리케이션에서 가장 높은 우선순위 위험.' },
          { q: '직접 인젝션과 간접 인젝션의 차이점은 무엇입니까?', a: '직접 인젝션: 공격자가 사용자 입력 필드를 제어하고 무력화 지침을 직접 삽입합니다. 간접 인젝션: 공격자가 LLM이 읽는 데이터 소스를 오염(웹 페이지, 문서, 데이터베이스 레코드)하고 악의적인 지침이 프롬프트 실행 중에 검색됩니다. 공격 표면에 애플리케이션이 읽는 모든 외부 데이터 소스가 포함되기 때문에 간접 인젝션은 방지하기 더 어렵습니다.' },
          { q: '프롬프트 보안 테스트에 어떤 도구를 사용할 수 있습니까?', a: 'Garak은 수십 가지 공격 패턴을 포함하는 LLM을 위한 무료 오픈 소스 적대적 프로브 라이브러리입니다. PyRIT는 구조화된 공격 오케스트레이션이 있는 Microsoft의 오픈 소스 레드팀 툴킷입니다. PromptQuorum은 특정 공격 패턴에 더 취약한 모델을 식별하기 위해 여러 모델에서 동일한 공격 프로브를 실행합니다.' },
          { q: 'RAG 파이프라인에서 간접 인젝션을 어떻게 방지합니까?', a: '네 가지 방어: (1) 입력 필터링 — 프롬프트에 포함하기 전에 검색된 콘텐츠를 검증하고 위생 처리하십시오. (2) 출력 스키마 적용 — 스키마 외부의 출력을 생성하는 삽입된 지침을 모델이 따를 수 없도록 엄격한 출력 형식을 정의하십시오. (3) 권한 분리 — LLM의 기능을 특정 작업으로 제한하십시오(작업이 요구하는 것 이상의 도구 접근 없음). (4) 지침 격리 — 시스템 지침과 검색된 데이터 사이에 명확한 구분자를 사용하십시오.' },
          { q: 'OWASP LLM01이란 무엇입니까?', a: 'OWASP LLM01은 OWASP LLM Top 10(2025)의 첫 번째 항목입니다: 프롬프트 인젝션. 직접 인젝션(공격자 제어 입력)과 간접 인젝션(검색된 콘텐츠 또는 도구 출력의 악의적인 지침)을 포함합니다. 가장 일반적이고 높은 영향의 LLM 취약점이기 때문에 첫 번째로 분류됩니다.' },
          { q: 'Garak은 몇 가지 공격 패턴을 테스트합니까?', a: 'Garak(버전 0.9+)은 프롬프트 인젝션, 탈옥, 데이터 추출, 환각 유발, 독성 우회를 포함한 40개 이상의 공격 프로브를 포함합니다. `garak --list-probes`를 실행하여 전체 목록을 확인하십시오. Garak은 오픈 소스이며 무료입니다; 모든 LLM API 엔드포인트에 대해 CLI를 통해 실행하십시오.' },
          { q: 'Garak과 PyRIT의 차이점은 무엇입니까?', a: 'Garak은 고정된 공격 프로브 라이브러리를 실행하고 합격/불합격 결과를 보고하는 자동화 스캐너입니다. PyRIT(Microsoft의 Python Risk Identification Toolkit)는 단일 턴 프로브가 탐지할 수 없는 취약점을 찾기 위해 여러 턴에 걸쳐 모델과 대화하는 공격자를 시뮬레이션하는 다중 턴 레드팀 오케스트레이터입니다. 체계적인 커버리지에는 Garak을, 심층에는 PyRIT를 사용하십시오.' },
        ],
      },
      related_reading: {
        id: 'related-reading',
  ...
  ar: {
    ...
    quickFacts: [
      ...
      'PromptQuorum يُشغّل probes الهجوم ذاتها على GPT-5.6 وClaude Sonnet 5 وGemini 3.1 Pro للكشف عن الثغرات الخاصة بكل نموذج.',
    ],
    toc: [
    ...
    sections: {
      ...
      tools: {
        ...
        content: [
          ...
          'PromptQuorum يُشغّل مجموعة probes الهجوم ذاتها على نماذج متعددة (GPT-5.6 وClaude Sonnet 5 وGemini 3.1 Pro) في آنٍ واحد للتعرف على أي النماذج أكثر عرضة لأنماط هجوم معينة.',
        ],
        callouts: [
