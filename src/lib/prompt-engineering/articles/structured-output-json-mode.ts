...
export const article: Partial<Record<Language, PEArticle>> = {
      ...
      faqSchema: {
        ...
        mainEntity: [
          ...
          },
          {
            ...
            acceptedAnswer: {
              '@type': 'Answer',
              text: 'No. OpenAI GPT-5.6 and Claude support native JSON mode. Older models and open-source LLMs may require prompt-based enforcement (e.g., specifying the schema in instructions and examples).',
            },
          },
          {
            '@type': 'Question',
      ...
      sections: {
        ...
        keyTakeaways: {
          ...
          items: [
            ...
            '**Model Dependent**: Native JSON mode available in GPT-5.6, Claude, Gemini; older/open-source models need prompt engineering.',
        ...
        whatIsStructured: {
          ...
          snippets: [
            { type: 'in-one-sentence', text: 'Structured output means requiring the model to follow a fixed schema — a list, a table, or JSON with named fields and types — so downstream tools can parse the result without manual cleanup.' },
            { type: 'in-plain-terms', text: 'Free-form text is fine for a human to read and miserable for a program to use. A schema turns the answer into something a database can accept directly, instead of something you have to unpick with string matching every time.' },
          ],
          content: [
        ...
        modelComparison: {
          ...
          rows: [
            { 'Model': 'OpenAI GPT-5.6', 'Native JSON Mode': 'Yes (enforced)', 'Prompt-Only Compliance': 'Not needed', 'Notes': 'Industry standard for JSON mode; 99%+ success rate.' },
            { 'Model': 'Anthropic Claude Sonnet 5', 'Native JSON Mode': 'Yes (enforced)', 'Prompt-Only Compliance': 'Not needed', 'Notes': 'Excellent JSON compliance; supports complex nested structures.' },
        ...
        howToStart: {
          ...
          numberedItems: [
            '**For data extraction and machine-readable outputs, use JSON mode (available in OpenAI GPT-5.6, Anthropic Claude, Google Gemini, and others).** This guarantees the model returns valid JSON, not prose. Example: Extract product info as JSON with keys: name, price, description, rating.',
        ...
        faq: {
          ...
          faqs: [
            ...
            },
            {
              ...
              a: 'No. OpenAI GPT-5.6, Anthropic Claude Sonnet 5, and Google Gemini support native JSON mode. Older models and open-source LLMs may require prompt-based enforcement (e.g., specifying the schema in instructions and providing examples).',
            },
            {
    ...
    de: {
      theme: 'Techniques', heroImage: '/images/structured-output-json-mode-overview-hero-de.webp',
      ...
      faqSchema: {
        ...
        mainEntity: [
          ...
          { '@type': 'Question', name: 'Unterstützen alle LLMs den JSON-Modus?', acceptedAnswer: { '@type': 'Answer', text: 'Nein. OpenAI GPT-5.6, Anthropic Claude Sonnet 5 und Google Gemini unterstützen nativen JSON-Modus. Ältere Modelle und Open-Source-LLMs benötigen möglicherweise Prompt-basierte Erzwingung.' } },
      ...
      sections: {
        ...
        keyTakeaways: {
          ...
          items: [
            ...
            'Der JSON-Modus erzwingt valide JSON-Ausgabe und wird nativ von OpenAI GPT-5.6, Anthropic Claude und Google Gemini unterstützt; ältere Modelle benötigen Prompt-basierte Erzwingung.',
        ...
        whatIsStructured: {
          ...
          snippets: [
            { type: 'in-one-sentence', text: 'Strukturierte Ausgabe bedeutet, das Modell auf ein festes Schema zu verpflichten — eine Liste, eine Tabelle oder JSON mit benannten Feldern und Typen —, damit nachgelagerte Werkzeuge das Ergebnis ohne manuelle Nacharbeit verarbeiten können.' },
            { type: 'in-plain-terms', text: 'Fließtext ist für Menschen angenehm und für Programme mühsam. Ein Schema macht aus der Antwort etwas, das eine Datenbank direkt annimmt, statt etwas, das Sie jedes Mal mit Zeichenkettensuche auseinandernehmen müssen.' },
          ],
          content: [
        ...
        modelComparison: {
          ...
          rows: [
            { 'Modell': 'OpenAI GPT-5.6', 'Nativer JSON-Modus': 'Ja (erzwungen)', 'Nur-Prompt-Compliance': 'Nicht erforderlich', 'Hinweise': 'Industriestandard für JSON-Modus; 99%+ Erfolgsrate.' },
            { 'Modell': 'Anthropic Claude Sonnet 5', 'Nativer JSON-Modus': 'Ja (erzwungen)', 'Nur-Prompt-Compliance': 'Nicht erforderlich', 'Hinweise': 'Hervorragende JSON-Compliance; unterstützt komplexe verschachtelte Strukturen.' },
        ...
        howToStart: {
          ...
          numberedItems: [
            '**Für Datenextraktion und maschinenlesbare Ausgaben den JSON-Modus (verfügbar in OpenAI GPT-5.6, Anthropic Claude, Google Gemini und anderen) verwenden.** Dies garantiert, dass das Modell valides JSON zurückgibt. Beispiel: Produktinformationen als JSON mit Schlüsseln extrahieren: name, price, description, rating.',
    ...
    es: {
      theme: 'Techniques', heroImage: '/images/structured-output-json-mode-overview-hero-es.webp',
      ...
      howToSchema: {
        ...
        step: [
          { '@type': 'HowToStep', position: 1, name: 'Elegir el modo JSON', text: 'Para extracción de datos y salidas legibles por máquinas, usa el modo JSON disponible en OpenAI GPT-5.6, Anthropic Claude, Google Gemini y otros proveedores principales. Esto garantiza que el modelo devuelva JSON válido, no prosa.' },
      ...
      faqSchema: {
        ...
        mainEntity: [
          ...
          { '@type': 'Question', name: '¿Todos los LLMs admiten el modo JSON?', acceptedAnswer: { '@type': 'Answer', text: 'No. OpenAI GPT-5.6, Anthropic Claude Sonnet 5 y Google Gemini admiten el modo JSON nativo. Los modelos más antiguos y los LLMs de código abierto pueden requerir aplicación basada en prompts.' } },
      ...
      sections: {
        ...
        keyTakeaways: {
          ...
          items: [
            ...
            '**Dependiente del modelo**: el modo JSON nativo está disponible en GPT-5.6, Claude, Gemini; los modelos más antiguos o de código abierto necesitan prompt engineering.',
        ...
        whatIsStructured: {
          ...
          snippets: [
            { type: 'in-one-sentence', text: 'La salida estructurada consiste en exigir al modelo que siga un esquema fijo — una lista, una tabla o JSON con campos y tipos definidos — para que las herramientas posteriores puedan procesar el resultado sin limpieza manual.' },
            { type: 'in-plain-terms', text: 'El texto libre se lee bien y se programa fatal. Un esquema convierte la respuesta en algo que una base de datos acepta directamente, en lugar de algo que tienes que desmontar con búsquedas de texto cada vez.' },
          ],
          content: [
        ...
        modelComparison: {
          ...
          rows: [
            { 'Modelo': 'OpenAI GPT-5.6', 'Modo JSON nativo': 'Sí (impuesto)', 'Cumplimiento solo con prompt': 'No necesario', 'Notas': 'Estándar de la industria para el modo JSON; tasa de éxito > 99%.' },
            { 'Modelo': 'Anthropic Claude Sonnet 5', 'Modo JSON nativo': 'Sí (impuesto)', 'Cumplimiento solo con prompt': 'No necesario', 'Notas': 'Excelente cumplimiento JSON; admite estructuras anidadas complejas.' },
        ...
        faq: {
          ...
          faqs: [
            ...
            { q: '¿Todos los LLMs admiten el modo JSON?', a: 'No. OpenAI GPT-5.6, Anthropic Claude Sonnet 5 y Google Gemini admiten el modo JSON nativo. Los modelos más antiguos y los LLMs de código abierto pueden requerir aplicación basada en prompts.' },
    ...
    ar: {
      theme: 'Techniques', heroImage: '/images/structured-output-json-mode-overview-hero-ar.webp',
      ...
      howToSchema: {
        ...
        step: [
          { '@type': 'HowToStep', position: 1, name: 'اختيار وضع JSON', text: 'لاستخراج البيانات والمخرجات القابلة للقراءة الآلية، استخدم وضع JSON المتاح في OpenAI GPT-5.6 وAnthropic Claude وGoogle Gemini وغيرهم من المزودين الرئيسيين. يضمن هذا أن يُعيد النموذج JSON صالحاً وليس نثراً.' },
      ...
      faqSchema: {
        ...
        mainEntity: [
          {
            '@type': 'Question',
            'name': 'ما الفرق بين المخرجات المهيكلة ووضع JSON؟',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'المخرجات المهيكلة هي الفئة الأوسع لطلب إعادة البيانات بصيغة ثابتة (قوائم أو جداول أو أزواج مفتاح-قيمة أو JSON). وضع JSON هو نوع أكثر صرامة يُلزم بمخرج JSON صالح، غالباً بضمانات على مستوى واجهة برمجة التطبيقات من مزود النموذج.',
            },
          },
          {
            '@type': 'Question',
            'name': 'هل تدعم جميع النماذج اللغوية الكبيرة وضع JSON؟',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'لا. OpenAI GPT-5.6 وAnthropic Claude Sonnet 5 وGoogle Gemini تدعم وضع JSON الأصيل. النماذج الأقدم والنماذج مفتوحة المصدر قد تتطلب تطبيقاً قائماً على البرومبت.',
            },
          },
          {
            '@type': 'Question',
            'name': 'كيف أُلزم بردود JSON فقط بدون وضع JSON الأصيل؟',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'استخدم هندسة البرومبت: (1) صرّح صراحةً بـ"أعد JSON صالحاً فقط"، (2) وفّر مخططاً مفصلاً وأمثلة، (3) أضف تعليمة "لا تُدرج أي نص خارج JSON". معدلات النجاح تتحسن بشكل ملحوظ مع أمثلة جيدة.',
            },
          },
          {
            '@type': 'Question',
            'name': 'ماذا يحدث إذا أعاد النموذج JSON غير صالح؟',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'تحقق من JSON من جهتك باستخدام محلل. إذا فشل، أعد المحاولة بطلب أكثر وضوحاً أو العودة إلى الاستخراج اليدوي. مع هندسة برومبت جيدة وأمثلة مخططات، معدلات الفشل منخفضة (عادةً < 5٪ للبرومبتات المصممة جيداً).',
            },
          },
          {
            '@type': 'Question',
            'name': 'هل يمكن استخدام المخرجات المهيكلة للمستندات المعقدة؟',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'نعم. قسّم المهام المعقدة إلى خطوات: استخرج الحقول الرئيسية أولاً، ثم تحقق، ثم حوّل اختيارياً في الأنظمة اللاحقة. تقسيم المستندات الكبيرة ومعالجتها بشكل منفصل عادةً يُحسّن الموثوقية ويُقلّل استخدام الرموز.',
            },
          },
          {
            '@type': 'Question',
            'name': 'كيف أتعامل مع البيانات المفقودة أو الغامضة في المخرجات المهيكلة؟',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'عرّف سلوك الاحتياطي في مخططك: استخدم سلاسل فارغة أو قيم null أو علامة خاصة مثل "مجهول". أضف التعليمة: "إذا كانت القيمة غامضة أو مفقودة، استخدم null بدلاً من التخمين."',
            },
          },
          {
            '@type': 'Question',
            'name': 'هل وضع JSON متأثر بالامتثال التنظيمي (GDPR وما شابه)؟',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'وضع JSON في حد ذاته محايد. ومع ذلك، المخرجات المهيكلة مفيدة للامتثال لأنها تُتيح تتبع البيانات المستخرجة والمحوّلة والمسجلة بشكل منهجي، وهو أمر بالغ الأهمية لسجلات التدقيق.',
            },
          },
          {
            '@type': 'Question',
            'name': 'كيف أختبر البرومبتات في وضع JSON؟',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'اختبر بمدخلات متنوعة: حالات حافة وبيانات غامضة وأمثلة من الواقع. حلّل المخرج وتحقق: (1) JSON صالح، (2) مخطط صحيح، (3) أنواع بيانات متوقعة. استهدف معدل نجاح ≥95٪ قبل النشر في الإنتاج.',
            },
          },
          {
            '@type': 'Question',
            'name': 'هل يمكن إعادة استخدام مخططات المخرجات المهيكلة عبر النماذج المختلفة؟',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'نعم، مع الحذر. عرّف مخططك مرة واحدة واختبره على نماذج مختلفة؛ قد تحتاج إلى ضبط البرومبتات للنماذج الأقدم أو الأصغر. وثّق أي اختلافات خاصة بالنموذج ومعدلات النجاح.',
            },
          },
          {
            '@type': 'Question',
            'name': 'ما تكلفة الأداء لوضع JSON؟',
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'ضئيلة. وضع JSON الأصيل (OpenAI وAnthropic وGoogle) له تأثير أداء ضئيل جداً. التطبيق بالبرومبت فقط قد يُضيف زمن استجابة 5–10٪ بسبب تكاليف شرح المخطط، لكن مكاسب الأمان عادةً تُبرر ذلك.',
            },
          },
        ],
      },
      ...
      sections: {
        ...
        keyTakeaways: {
          ...
          items: [
            ...
            '**تعتمد على النموذج**: وضع JSON الأصيل متاح في GPT-5.6 وClaude وGemini؛ النماذج الأقدم أو مفتوحة المصدر تحتاج هندسة برومبت.',
        ...
        whatIsStructured: {
          ...
          snippets: [
            { type: 'in-one-sentence', text: 'تعني المخرجات المهيكلة إلزام النموذج باتّباع مخطط ثابت — قائمة أو جدول أو JSON بحقول وأنواع محددة — كي تتمكن الأدوات اللاحقة من معالجة النتيجة دون تنظيف يدوي.' },
            { type: 'in-plain-terms', text: 'النص الحر مريح للقراءة البشرية ومرهق للبرمجة. المخطط يحوّل الإجابة إلى شيء تقبله قاعدة البيانات مباشرة، بدل شيء تضطر لتفكيكه في كل مرة بمطابقة النصوص.' },
          ],
          content: [
        ...
        modelComparison: {
          ...
          rows: [
            { 'النموذج': 'OpenAI GPT-5.6', 'وضع JSON الأصيل': 'نعم (مُلزَم)', 'الامتثال بالبرومبت فقط': 'غير ضروري', 'ملاحظات': 'معيار الصناعة لوضع JSON؛ معدل نجاح > 99٪.' },
            { 'النموذج': 'Anthropic Claude Sonnet 5', 'وضع JSON الأصيل': 'نعم (مُلزَم)', 'الامتثال بالبرومبت فقط': 'غير ضروري', 'ملاحظات': 'امتثال JSON ممتاز؛ يدعم البنى المتداخلة المعقدة.' },
        ...
        faq: {
          ...
          faqs: [
            ...
            { q: 'هل تدعم جميع النماذج اللغوية الكبيرة وضع JSON؟', a: 'لا. OpenAI GPT-5.6 وAnthropic Claude Sonnet 5 وGoogle Gemini تدعم وضع JSON الأصيل. النماذج الأقدم والنماذج مفتوحة المصدر قد تتطلب تطبيقاً قائماً على البرومبت.' },
    ...
    pt: {
      theme: 'Techniques', heroImage: '/images/structured-output-json-mode-overview-hero-pt.webp',
      ...
      sections: {
        ...
        whatIsStructured: {
          ...
          snippets: [
            { type: 'in-one-sentence', text: 'Saída estruturada significa exigir que o modelo siga um schema fixo — uma lista, uma tabela ou JSON com campos e tipos nomeados — para que ferramentas downstream processem o resultado sem limpeza manual.' },
            { type: 'in-plain-terms', text: 'Texto livre é ótimo para uma pessoa ler e péssimo para um programa usar. Um schema transforma a resposta em algo que um banco de dados aceita direto, em vez de algo que você precisa desmontar com busca de texto toda vez.' },
          ],
          content: [
        ...
        whyItMatters: {
          title: 'Por que saída estruturada e JSON mode importam',
          content: [
        ...
        practicesAddendum: {
          content: [
            'Se ainda houver problemas de formatação, você pode adicionar uma instrução simples como "Se estiver em dúvida, deixe o campo como string vazia em vez de adivinhar." A saída estruturada funciona melhor combinada com [RAG (geração aumentada por recuperação)](/pt/prompt-engineering/rag-explained) para checagem dos dados extraídos. Quando os dados extraídos precisam permanecer em infraestrutura privada, os mesmos padrões de JSON mode se conectam a um vector store on-premise — veja [RAG local para dados de negócio](/pt/power-local-llm/local-rag-for-private-business-data) para o modelo de implantação em conformidade com a LGPD.',
          ],
        },
        modelComparison: {
          ...
          rows: [
            { 'Modelo': 'OpenAI GPT-5.6', 'Modo JSON nativo': 'Sim (imposto)', 'Conformidade apenas com prompt': 'Não necessário', 'Notas': 'Padrão da indústria para modo JSON.' },
            { 'Modelo': 'Anthropic Claude Sonnet 5', 'Modo JSON nativo': 'Sim (imposto)', 'Conformidade apenas com prompt': 'Não necessário', 'Notas': 'Excelente conformidade JSON.' },
        ...
        example: {
          title: 'Exemplo: texto livre versus JSON estruturado',
          content: [
            ...
            '"Você é um assistente de suporte ao cliente. Leia o e-mail do cliente abaixo e extraia as informações principais para um objeto JSON. Requisitos: gere apenas JSON válido, com chaves e valores de string entre aspas duplas. Não inclua explicações nem texto extra fora do JSON. Se um valor estiver ausente, use uma string vazia. Schema JSON: {\n  "issue_type": "string",\n  "urgency": "low | medium | high",\n  "summary": "string (max 25 words)",\n  "customer_sentiment": "negative | neutral | positive"\n} E-mail do cliente: [cole o texto do e-mail aqui]"',
        ...
        regulatedEnvironments: {
          title: 'Saída estruturada em ambientes regulados',
          content: [
        ...
        whenToUseJsonMode: {
          title: 'Quando usar JSON mode em vez das alternativas',
          content: [
        ...
        whenToUseStructuredOutput: {
          title: 'Quando você deve usar saída estruturada?',
          content: [
    ...
    fr: { theme: 'Techniques', heroImage: '/images/structured-output-json-mode-overview-hero-fr.webp', title: 'Sortie structuree dans les LLM : mode JSON, exemples et utilisation', intro: 'La sortie structuree et le mode JSON transforment les resultats libres des modeles linguistiques en formats fiables et lisibles par machine qui s\'integrent parfaitement aux bases de donnees, aux API et aux workflows d\'automatisation. Apprenez a concevoir des prompts qui forcent une sortie JSON valide, comparez le mode JSON avec les appels de fonction et les prompts de schema, et decidez quelle methode convient a votre cas d\'usage.', publishDate: '2026-03-26', dateModified: '2026-07-13', readTime: '10 min de lecture', seoTitle: 'Sortie structuree et mode JSON dans les LLM : utilisation, exemples et comparaison', metaDescription: 'Apprenez quand utiliser la sortie structuree, le mode JSON ou les appels de fonction. Comparez la conformite JSON par modele, voyez des exemples reels et maitrisez la conception de schemas pour les API.', educationalLevel: 'Intermediate', schema: { '@context': 'https://schema.org', '@type': 'TechArticle', headline: 'Sortie structuree et mode JSON : obtenir des donnees utilisables de l\'IA', description: 'Maitrisez la sortie structuree et le mode JSON dans les prompts. Apprenez a concevoir des schemas, a imposer un JSON valide, a comparer la conformite des modeles et a eviter les erreurs courantes dans les systemes de production.', datePublished: '2026-03-26', dateModified: '2026-07-13', keywords: ['sortie structuree', 'mode JSON', 'ingenierie des prompts', 'conception de schemas', 'sortie lisible par machine', 'validation JSON', 'modeles de prompts'], author: { '@type': 'Person', name: 'Hans Kuepper', sameAs: 'https://www.linkedin.com/in/hanskuepper/' }, publisher: { '@type': 'Organization', name: 'PromptQuorum', url: 'https://www.promptquorum.com' } }, howToSchema: { '@context': 'https://schema.org', '@type': 'HowTo', name: 'Comment utiliser la sortie structuree et le mode JSON', step: [ { '@type': 'HowToStep', position: 1, name: 'Choisir le mode JSON', text: 'Pour l\'extraction de donnees et les sorties lisibles par machine, utilisez le mode JSON disponible dans OpenAI GPT-5.6, Anthropic Claude, Google Gemini et autres fournisseurs majeurs. Cela garantit que le modele renvoie un JSON valide, pas du texte.' }, { '@type': 'HowToStep', position: 2, name: 'Definir votre schema', text: 'Definissez explicitement votre schema JSON, y compris les noms de champs, les types de donnees et les contraintes.' }, { '@type': 'HowToStep', position: 3, name: 'Fournir un exemple de sortie', text: 'Fournissez un exemple de la structure JSON exacte que vous voulez. Les exemples sont plus puissants que les descriptions de schemas seules.' }, { '@type': 'HowToStep', position: 4, name: 'Gerer les structures imbriquees', text: 'Pour les objets dans les tableaux, soyez explicite sur la hierarchie. Fournissez un exemple JSON complet.' }, { '@type': 'HowToStep', position: 5, name: 'Valider la sortie JSON', text: 'Validez la sortie JSON avant la use en aval. Analysez et verifiez le JSON valide, le schema correct, et les types de donnees attendus.' } ] }, faqSchema: { '@context': 'https://schema.org', '@type': 'FAQPage', mainEntity: [
        ...
        whenToUseJsonMode: {
          title: 'Quand utiliser le mode JSON plutôt qu\'une alternative',
          content: [
        ...
        whenToUseStructuredOutput: {
          title: 'Quand faut-il utiliser la sortie structurée ?',
          content: [
        ...
        howToStart: {
          title: 'Comment utiliser la sortie structurée et le mode JSON',
          numberedItems: [
            '**Pour l\'extraction de données et les sorties lisibles par une machine, utilisez le mode JSON (disponible dans OpenAI GPT-5.6, Anthropic Claude, Google Gemini et d\'autres).** Il garantit que le modèle renvoie du JSON valide, et non de la prose. Exemple : extraire les informations produit en JSON avec les clés name, price, description, rating.',
            '**Définissez explicitement votre schéma JSON, avec les noms de champs, les types de données et les contraintes.** Exemple : { "name": string, "price": number (≥ 0), "in_stock": boolean, "tags": array of strings }.',
            '**Fournissez un exemple de la structure JSON exacte que vous attendez.** Exemple : { "issue": "memory leak", "severity": "critical", "suggested_fix": "...", "code_snippet": "..." }. Les exemples sont plus efficaces que les descriptions de schéma.',
            '**Pour les structures imbriquées (objets dans des tableaux), soyez explicite sur la hiérarchie.** Fournissez un exemple JSON complet, tableaux imbriqués compris. Voir l\'exemple ci-dessous.',
            '**Validez la sortie JSON avant de l\'utiliser dans les systèmes en aval.** Parsez le JSON renvoyé et vérifiez : (1) la syntaxe JSON est valide, (2) tous les champs requis sont présents, (3) les types de données correspondent aux attentes. Gérez proprement les erreurs de parsing.',
          ],
        },
        howToStep4Code: {
          content: [
    ...
    ja: { theme: 'テクニック', heroImage: '/images/structured-output-json-mode-overview-hero-ja.webp', title: 'LLMの構造化出力：JSONモード、サンプル、使用タイミング', intro: '構造化出力とJSONモードは、言語モデルの出力を信頼性の高い機械可読フォーマットに変換し、データベース、API、自動化ワークフローにシームレスに統合できます。有効なJSONを強制するプロンプトの設計方法、JSONモード対関数呼び出し対スキーマプロンプティングの比較、およびユースケースに適した方法の選択方法を学びます。', publishDate: '2026-03-26', dateModified: '2026-07-13', readTime: '10分読む', seoTitle: 'LLMの構造化出力とJSONモード：使用時期、サンプル、比較', metaDescription: '構造化出力、JSONモード、関数呼び出しをいつ使用するかを学びます。モデル別のJSON準拠を比較し、実例を確認し、APIのスキーマ設計をマスターしてください。', educationalLevel: 'Intermediate', schema: { '@context': 'https://schema.org', '@type': 'TechArticle', headline: '構造化出力とJSONモード：AIから使用可能なデータを取得', description: 'プロンプトで構造化出力とJSONモードをマスターします。スキーマ設計、有効なJSONの強制、モデル準拠の比較、および本番システムでの一般的なエラーの回避方法を学びます。', datePublished: '2026-03-26', dateModified: '2026-07-13', keywords: ['構造化出力', 'JSONモード', 'プロンプトエンジニアリング', 'スキーマ設計', '機械可読出力', 'JSON検証', 'プロンプトテンプレート'], author: { '@type': 'Person', name: 'Hans Kuepper', sameAs: 'https://www.linkedin.com/in/hanskuepper/' }, publisher: { '@type': 'Organization', name: 'PromptQuorum', url: 'https://www.promptquorum.com' } }, howToSchema: { '@context': 'https://schema.org', '@type': 'HowTo', name: '構造化出力とJSONモードの使用方法', step: [ { '@type': 'HowToStep', position: 1, name: 'JSONモードを選択', text: 'データ抽出と機械可読出力の場合、OpenAI GPT-5.6、Anthropic Claude、Google Geminiおよびその他の主要プロバイダで利用可能なJSONモードを使用します。これにより、モデルが有効なJSONを返すことが保証されます。' }, { '@type': 'HowToStep', position: 2, name: 'スキーマを定義', text: 'JSONスキーマを明示的に定義します。フィールド名、データタイプ、および制約を含めます。' }, { '@type': 'HowToStep', position: 3, name: '出力例を提供', text: '必要なJSON構造の例を提供します。例はスキーマの説明だけより強力です。' }, { '@type': 'HowToStep', position: 4, name: 'ネストされた構造を処理', text: '配列内のオブジェクトの場合、階層に関して明示的にします。完全なJSON例を提供します。' }, { '@type': 'HowToStep', position: 5, name: 'JSON出力を検証', text: 'ダウンストリームシステムで使用する前にJSON出力を検証します。有効なJSON、正しいスキーマ、期待されるデータタイプを確認します。' } ] }, faqSchema: { '@context': 'https://schema.org', '@type': 'FAQPage', mainEntity: [ { '@type': 'Question', name: '構造化出力とJSONモードの違いは何ですか？', acceptedAnswer: { '@type': 'Answer', text: '構造化出力はモデルに固定フォーマットでデータを返すよう要求する広いカテゴリです。JSONモードはより厳密なバリアントで、有効なJSON出力を強制します。' } }, { '@type': 'Question', name: 'すべてのLLMがJSONモードをサポートしていますか？', acceptedAnswer: { '@type': 'Answer', text: 'いいえ。OpenAI GPT-5.6、Anthropic Claude Sonnet 5、Google Geminiがネイティブjsonモードをサポートします。古いモデルとオープンソースLLMはプロンプトベースの適用が必要です。' } }, { '@type': 'Question', name: 'ネイティブJSONモードなしでJSON応答のみを強制するにはどうすればよいですか？', acceptedAnswer: { '@type': 'Answer', text: 'プロンプトエンジニアリングを使用します：「有効なJSONのみを出力」を明示的に宣言し、詳細なスキーマと例を提供します。' } }, { '@type': 'Question', name: 'モデルが無効なJSONを返す場合はどうなりますか？', acceptedAnswer: { '@type': 'Answer', text: 'JSONをサイドで検証します。失敗した場合、より明確なプロンプトでリクエストを再試行するか、手動抽出に戻ります。' } }, { '@type': 'Question', name: '複雑なドキュメントに構造化出力を使用できますか？', acceptedAnswer: { '@type': 'Answer', text: 'はい。複雑なタスクをステップに分割します：最初に主要フィールドを抽出し、次に検証し、オプションでダウンストリームシステムに変換します。' } }, { '@type': 'Question', name: '構造化出力で欠落しているまたは曖昧なデータを処理するにはどうすればよいですか？', acceptedAnswer: { '@type': 'Answer', text: 'スキーマでフォールバック動作を定義します：空の文字列、null値、または「不明」などの特別なマーカーを使用します。' } }, { '@type': 'Question', name: 'JSONモードは規制遵守（GDPR、CCPA）の影響を受けますか？', acceptedAnswer: { '@type': 'Answer', text: 'JSONモード自体は中立的です。ただし、構造化出力は、抽出、変換、記録されたデータを体系的に追跡できるため、コンプライアンスに有益です。' } }, { '@type': 'Question', name: 'JSONモードプロンプトをテストするにはどうすればよいですか？', acceptedAnswer: { '@type': 'Answer', text: '多様な入力でテストします：エッジケース、曖昧なデータ、実世界の例。有効なJSON、正しいスキーマ、予想されるデータタイプを確認します。' } }, { '@type': 'Question', name: 'さまざまなモデル全体で構造化出力スキーマを再利用できますか？', acceptedAnswer: { '@type': 'Answer', text: 'はい、注意深く。スキーマを一度定義し、モデル全体でテストします。古いモデルや小さいモデルのプロンプト調整が必要な場合があります。' } }, { '@type': 'Question', name: 'JSONモードのパフォーマンスコストは何ですか？', acceptedAnswer: { '@type': 'Answer', text: '最小限。ネイティブJSONモード（OpenAI、Anthropic、Google）はパフォーマンスへの影響がわずかです。' } } ] }, sections: { definition: { content: [ '**構造化出力は、言語モデルを強制して、定義済みフォーマット（JSONなど）でデータを返す方法です。** 自由形式のテキストとは異なり、ダウンストリームツールが手動クリーンアップなしで処理できる厳密なフィールド名、データタイプ、スキーマを強制します。' ] }, exampleBlock: { content: [ 'JSON形式での構造化出力の簡単な例を次に示します。' ], codeBlock: '{\n  "task": "summarize",\n  "title": "Quick AI Guide",\n  "summary": "This article explains structured output and JSON mode.",\n  "key_points": ["JSON enforces format", "Reduces parsing errors", "Enables automation"],\n  "audience_level": "intermediate",\n  "confidence": 0.95\n}', codeLanguage: 'json' }, keyTakeaways: { isTldr: true, content: [ '**重要なポイント：構造化出力について知っておくべきことが：**' ], items: [ '**信頼性の向上** : 構造化出力は、厳密なスキーマを強制することで、パースエラーと手動データクリーンアップを削減します。', '**自動化の有効化** : JSONモードは、抽出されたフィールド（優先度、カテゴリ、緊急度）に基づいて条件付きロジックをトリガーします。', '**API対応** : 再フォーマットなしでデータベース、CRM、ビジネスシステムへの直接統合。', '**モデル依存** : ネイティブJSONモードはGPT-5.6、Claude、Geminiで利用可能です。古いモデル/オープンソースモデルはプロンプトエンジニアリングが必要です。', '**決定論的タスクに最適** : API、自動化、データパイプライン。クリエイティブライティングは避けてください。', '**検証が必要** : ダウンストリーム使用前にJSON出力を常に検証および検証してください。', '**モデル全体でスケール** : スキーマを一度定義します。テストし、相違点を文書化します。' ] }, whatIsStructured: { title: '構造化出力とは', snippets: [{ type: 'in-one-sentence', text: '構造化出力とは、リスト・テーブル・JSON のように項目名と型が定まったスキーマにモデルを従わせることであり、後続のツールが手作業の整形なしに結果を処理できるようにする手法です。' }, { type: 'in-plain-terms', text: '自由な文章は人が読むには快適でも、プログラムが扱うには厄介です。スキーマを与えると、回答はデータベースがそのまま受け取れる形になります。毎回文字列を探して切り分ける必要はなくなります。' } ], content: [ '**構造化出力とは、モデルに固定スキーマ（リスト、テーブル、JSONなど）に従うよう要求することです。** 自由形式の段落の代わりに、フィールド、タイプ、許可された値を定義します。', '構造化出力はいくつかの形式をとることができます：' ], items: [ '固定数のアイテムを含むブレットリスト。', '特定の列を持つMarkdownテーブル。', '単純な属性のキーと値のペア。', '事前定義されたキーを持つ完全なJSONオブジェクトまたは配列。' ] }, structuredGoal: { content: [ '目標は常に同じです：あいまいな説明を予測可能な形に変換することです。' ] }, whatIsJSON: { title: 'JSONモードとは', content: [ '**JSONモードは、モデルが有効なJSONのみを返すよう指示または構成される厳密な構造化出力バリアントです。** JSONモードでは、モデルが出力するすべてが追加のクリーンアップなしでJSONとして解析可能である必要があります。', '典型的なJSONスキーマは次のようになります：' ], codeBlock: '{\n  "title": "string",\n  "summary": "string",\n  "tags": ["string"],\n  "priority": "low | medium | high"\n}', codeLanguage: 'json' }, jsonModeApproach: { content: [ 'このスキーマをプロンプトに反映し、モデルにそれを入力するよう要求します。一部のプラットフォームはJSON のみの応答を強制する特別な設定またはAPIも提供します。' ] }, whyItMatters: { title: '構造化出力とJSONモードが重要な理由', content: [ '**構造化出力とJSONモードが重要な理由は、言語モデルを単なるチャットアシスタントではなく、より大きなシステムのコンポーネントに変換できるためです。** 出力が予測可能な場合、以下を実行できます：' ], items: [ 'データベース、CRM、分析ツールに結果を直接供給します。', 'モデル出力フィールド（優先度、ステータス、信頼度）に基づいてアクションをトリガーします。', 'カード、テーブル、ダッシュボードにモデル結果を表示するUIを構築します。' ] }, debuggingBenefit: {
        ...
        howToStart: {
          title: '構造化出力とJSONモードの使い方',
          numberedItems: [
            '**データ抽出や機械可読な出力には、JSONモードを使ってください（OpenAI GPT-5.6、Anthropic Claude、Google Geminiなどで利用可能です）。** これにより、モデルが散文ではなく有効なJSONを返すことが保証されます。例：製品情報をname、price、description、ratingのキーを持つJSONとして抽出する。',
            '**JSONスキーマを明示的に定義し、フィールド名、データ型、制約を含めてください。** 例：{ "name": string, "price": number (≥ 0), "in_stock": boolean, "tags": array of strings }。',
            '**欲しいJSON構造そのものの例を示してください。** 例：{ "issue": "memory leak", "severity": "critical", "suggested_fix": "...", "code_snippet": "..." }。例はスキーマの説明よりも強力です。',
            '**入れ子構造（配列内のオブジェクト）では、階層を明示してください。** 入れ子の配列を含む完全なJSONの例を示します。下記の例をご覧ください。',
            '**下流システムで使う前にJSON出力を検証してください。** 返ってきたJSONをパースし、次を確認します。（1）JSON構文として有効か、（2）必須フィールドがすべて存在するか、（3）データ型が期待どおりか。パースエラーは適切に処理してください。',
          ],
        },
        howToStep4Code: {
          content: [
        ...
        relatedReading: { title: '関連読み物', items: [ '[制約付きプロンプト](/ja/prompt-engineering/constrained-prompting) — 特定の出力フォーマットを強制します。', '[SPECSフレームワーク](/ja/prompt-engineering/specs-framework) — 仕様重視のプロンプト。', '[RAG説明](/ja/prompt-engineering/rag-explained) — 構造化抽出とデータ取得を組み合わせます。', '[思考の連鎖](/ja/prompt-engineering/chain-of-thought-prompting) — ステップバイステップで理由を述べます。', '[プロンプトテンプレート](/ja/prompt-engineering/prompt-documentation-templates) — 再利用可能なテンプレート。', '[ゼロショット対フューショット](/ja/prompt-engineering/zero-shot-vs-few-shot) — 例がJSON準拠を改善するとき。', '[信頼性の高い構造化データのプロンプト](/ja/prompt-engineering/prompts-for-reliable-structured-data)' ] }, faq: { title: 'よくある質問', faqs: [
            { q: '構造化出力とJSONモードの違いは何ですか？', a: '構造化出力はより広いカテゴリです。JSONモードはより厳密なバリアントです。' },
            { q: 'すべてのLLMがJSONモードをサポートしていますか？', a: 'いいえ。OpenAI GPT-5.6、Anthropic Claude Sonnet 5、Google Geminiがサポートしています。' },
            { q: 'ネイティブJSONモードなしでJSON応答のみを強制するにはどうすればよいですか？', a: 'プロンプトエンジニアリング：「有効なJSONのみ」を宣言し、スキーマと例を提供します。' },
            { q: 'モデルが無効なJSONを返す場合はどうなりますか？', a: 'JSONをサイドで検証します。失敗した場合は再試行するか、手動で戻ります。' },
            { q: '複雑なドキュメントに構造化出力を使用できますか？', a: 'はい。複雑なタスクをステップに分割します。' },
            { q: '欠落しているまたは曖昧なデータを処理するにはどうすればよいですか？', a: 'スキーマでフォールバック動作を定義します。' },
            { q: 'JSONモードは規制遵守に影響を受けますか？', a: 'JSONモード自体は中立的です。しかし構造化出力はコンプライアンスに有益です。' },
            { q: 'JSONモードプロンプトをテストするにはどうすればよいですか？', a: '多様な入力でテストします。本番前に95%以上の成功率を目指します。' },
            { q: 'さまざまなモデル全体でスキーマを再利用できますか？', a: 'はい、注意深く。スキーマを定義してテストします。' },
            { q: 'JSONモードのパフォーマンスコストは何ですか？', a: '最小限。ネイティブJSONモードはわずかな影響です。' },
            { q: 'Comment gerer les donnees manquantes ou ambigues dans les sorties structurees ?', a: 'Definissez le comportement de secours dans votre schema : utilisez des chaines vides, des valeurs null ou un marqueur special comme "inconnu".' },
          ] }, sources: { title: 'ソース', items: [ '[OpenAI JSONモードドキュメント](https://platform.openai.com/docs/guides/json-mode) — 公式ガイド。', '[Anthropicガイド](https://docs.anthropic.com/claude/reference/getting-started-with-the-api) — ドキュメント。', '[Google Gemini API](https://ai.google.dev/gemini-2/docs/prompt-engineering/structured-output-and-json-mode) — ネイティブJSONサポート。', '[JSON Schemaスペック](https://json-schema.org/specification.html) — 標準仕様。' ] } } },
  ko: {
    theme: 'Techniques', heroImage: '/images/structured-output-json-mode-overview-hero-ko.webp',
    ...
    howToSchema: {
      ...
      step: [
        { '@type': 'HowToStep', position: 1, name: 'JSON 모드 선택', text: '데이터 추출 및 기계 판독 가능한 출력의 경우 OpenAI GPT-5.6, Anthropic Claude, Google Gemini 및 기타 주요 제공업체에서 사용할 수 있는 JSON 모드를 사용하십시오. 이렇게 하면 모델이 산문이 아닌 유효한 JSON을 반환하게 됩니다.' },
    ...
    faqSchema: {
      ...
      mainEntity: [
        ...
        { '@type': 'Question', name: '모든 LLM이 JSON 모드를 지원합니까?', acceptedAnswer: { '@type': 'Answer', text: '아닙니다. OpenAI GPT-5.6, Anthropic Claude Sonnet 5 및 Google Gemini는 기본 JSON 모드를 지원합니다. 이전 모델 및 오픈 소스 LLM은 프롬프트 기반 적용이 필요할 수 있습니다.' } },
    ...
    sections: {
      ...
      whatIsStructured: {
        ...
        title: '구조화된 출력이란',
        snippets: [
          { type: 'in-one-sentence', text: '구조화된 출력이란 리스트, 표, 또는 필드명과 타입이 정해진 JSON처럼 고정된 스키마를 모델이 따르도록 요구하여, 후속 도구가 수작업 정리 없이 결과를 처리할 수 있게 하는 방식입니다.' },
          { type: 'in-plain-terms', text: '자유로운 문장은 사람이 읽기에는 좋지만 프로그램이 다루기에는 고약합니다. 스키마를 지정하면 답변이 데이터베이스가 그대로 받아들일 수 있는 형태가 되며, 매번 문자열을 찾아 뜯어낼 필요가 없어집니다.' },
        ],
        content: [
      ...
      modelComparison: {
        ...
        rows: [
          { '모델': 'OpenAI GPT-5.6', '기본 JSON 모드': '예 (강제)', '프롬프트 전용 준수': '불필요', '비고': 'JSON 모드의 업계 표준; 99%+ 성공률.' },
          { '모델': 'Anthropic Claude Sonnet 5', '기본 JSON 모드': '예 (강제)', '프롬프트 전용 준수': '불필요', '비고': '우수한 JSON 준수; 복잡한 중첩 구조 지원.' },
      ...
      faq: {
        ...
        faqs: [
          ...
          { q: '모든 LLM이 JSON 모드를 지원합니까?', a: '아닙니다. OpenAI GPT-5.6, Anthropic Claude Sonnet 5 및 Google Gemini는 기본 JSON 모드를 지원합니다. 이전 모델 및 오픈 소스 LLM은 프롬프트 기반 적용이 필요할 수 있습니다(예: 지시사항과 예시에 스키마 지정).' },
  ...
  zh: { theme: '技术', heroImage: '/images/structured-output-json-mode-overview-hero-zh.webp', title: 'LLM中的结构化输出：JSON模式、示例及使用时机', intro: '结构化输出和JSON模式将语言模型的输出转变为可靠的机器可读格式，可以无缝集成到数据库、API和自动化工作流中。学习如何设计强制有效JSON输出的提示词，比较JSON模式与函数调用与架构提示词，并确定哪种方法适合您的用例。', publishDate: '2026-03-26', dateModified: '2026-07-13', readTime: '10分钟阅读', seoTitle: 'LLM中的结构化输出和JSON模式：何时使用、示例和比较', metaDescription: '学习何时使用结构化输出、JSON模式或函数调用。按模型比较JSON合规性，查看真实示例，掌握API的架构设计。', educationalLevel: 'Intermediate', schema: { '@context': 'https://schema.org', '@type': 'TechArticle', headline: '结构化输出和JSON模式：从AI获取可用数据', description: '掌握提示词中的结构化输出和JSON模式。学习架构设计、强制有效JSON、比较模型合规性以及避免生产系统中的常见错误。', datePublished: '2026-03-26', dateModified: '2026-07-13', keywords: ['结构化输出', 'JSON模式', '提示词工程', '架构设计', '机器可读输出', 'JSON验证', '提示词模板'], author: { '@type': 'Person', name: 'Hans Kuepper', sameAs: 'https://www.linkedin.com/in/hanskuepper/' }, publisher: { '@type': 'Organization', name: 'PromptQuorum', url: 'https://www.promptquorum.com' } }, howToSchema: { '@context': 'https://schema.org', '@type': 'HowTo', name: '如何使用结构化输出和JSON模式', step: [ { '@type': 'HowToStep', position: 1, name: '选择JSON模式', text: '对于数据提取和机器可读输出，请使用OpenAI GPT-5.6、Anthropic Claude、Google Gemini及其他主要提供商提供的JSON模式。这可确保模型返回有效的JSON，而不是文本。' }, { '@type': 'HowToStep', position: 2, name: '定义架构', text: '显式定义JSON架构。包括字段名称、数据类型和约束。' }, { '@type': 'HowToStep', position: 3, name: '提供输出示例', text: '提供您想要的确切JSON结构的示例。示例比架构描述本身更强大。' }, { '@type': 'HowToStep', position: 4, name: '处理嵌套结构', text: '对于数组中的对象，明确说明层次结构。提供完整的JSON示例。' }, { '@type': 'HowToStep', position: 5, name: '验证JSON输出', text: '在下游使用前验证JSON输出。检查有效的JSON、正确的架构和预期的数据类型。' } ] }, faqSchema: { '@context': 'https://schema.org', '@type': 'FAQPage', mainEntity: [ { '@type': 'Question', name: '结构化输出和JSON模式之间有什么区别？', acceptedAnswer: { '@type': 'Answer', text: '结构化输出是要求模型以固定格式返回数据的更广泛类别。JSON模式是更严格的变体，强制有效的JSON输出。' } }, { '@type': 'Question', name: '所有LLM都支持JSON模式吗？', acceptedAnswer: { '@type': 'Answer', text: '否。OpenAI GPT-5.6、Anthropic Claude Sonnet 5和Google Gemini支持原生JSON模式。较旧的模型和开源LLM可能需要基于提示词的应用。' } }, { '@type': 'Question', name: '在没有原生JSON模式的情况下如何强制仅JSON响应？', acceptedAnswer: { '@type': 'Answer', text: '使用提示词工程：明确声明"仅输出有效的JSON"，提供详细的架构和示例。' } }, { '@type': 'Question', name: '如果模型返回无效的JSON会发生什么？', acceptedAnswer: { '@type': 'Answer', text: '在您的一方验证JSON。如果失败，请用更清晰的提示词重试请求或回到手动提取。' } }, { '@type': 'Question', name: '我可以将结构化输出用于复杂的文档吗？', acceptedAnswer: { '@type': 'Answer', text: '是的。将复杂任务分解成步骤：首先提取关键字段，然后验证，然后可选地转换为下游系统。' } }, { '@type': 'Question', name: '我如何处理结构化输出中缺失或模糊的数据？', acceptedAnswer: { '@type': 'Answer', text: '在架构中定义后备行为：使用空字符串、null值或特殊标记（如"未知"）。' } }, { '@type': 'Question', name: 'JSON模式是否受到监管合规性（GDPR、CCPA）的影响？', acceptedAnswer: { '@type': 'Answer', text: 'JSON模式本身是中立的。但是，结构化输出对合规性有益，因为它可以系统地跟踪提取、转换和记录的数据。' } }, { '@type': 'Question', name: '我如何测试JSON模式提示词？', acceptedAnswer: { '@type': 'Answer', text: '使用不同的输入进行测试：边界案例、模糊数据和真实示例。验证有效的JSON、正确的架构和预期的数据类型。' } }, { '@type': 'Question', name: '我可以在不同的模型中重复使用结构化输出架构吗？', acceptedAnswer: { '@type': 'Answer', text: '可以，但要谨慎。定义一次架构，在模型中测试。您可能需要为较旧或较小的模型调整提示词。' } }, { '@type': 'Question', name: 'JSON模式的性能成本是什么？', acceptedAnswer: { '@type': 'Answer', text: '最少。原生JSON模式（OpenAI、Anthropic、Google）的性能影响可以忽略不计。' } } ] }, sections: { definition: { content: [ '**结构化输出是强制语言模型以预定义格式（如JSON）返回数据的一种方法。** 它不同于自由形式文本，在于它强制严格的字段名称、数据类型和架构，下游工具可以毫不费力地处理。' ] }, exampleBlock: { content: [ '以下是JSON格式的结构化输出的简单示例：' ], codeBlock: '{\n  "task": "summarize",\n  "title": "Quick AI Guide",\n  "summary": "This article explains structured output and JSON mode.",\n  "key_points": ["JSON enforces format", "Reduces parsing errors", "Enables automation"],\n  "audience_level": "intermediate",\n  "confidence": 0.95\n}', codeLanguage: 'json' }, keyTakeaways: { isTldr: true, content: [ '**关键点：关于结构化输出您需要了解的内容：**' ], items: [ '**提高可靠性** : 结构化输出通过强制严格的架构来减少解析错误和手动数据清理。', '**启用自动化** : JSON模式基于提取的字段（优先级、类别、紧急性）触发条件逻辑。', '**API就绪** : 直接集成到数据库、CRM和业务系统，无需重新格式化。', '**取决于模型** : 原生JSON模式在GPT-5.6、Claude、Gemini中可用。旧型号/开源模式需要提示词工程。', '**最适合确定性任务** : API、自动化、数据管道。避免创意写作。', '**需要验证** : 始终在下游使用前验证和检查JSON输出。', '**跨模型扩展** : 定义一次架构；测试和记录差异。' ] }, whatIsStructured: { title: '什么是结构化输出', snippets: [{ type: 'in-one-sentence', text: '结构化输出是指要求模型遵循固定的架构——列表、表格，或带有字段名与类型的 JSON——使下游工具无需人工清洗即可处理结果。' }, { type: 'in-plain-terms', text: '自由文本适合人读，却让程序很难处理。有了架构，回答就变成数据库可以直接接收的东西，而不是每次都要靠字符串匹配去拆解的一段文字。' } ], content: [ '**结构化输出意味着要求模型遵循固定的架构，例如列表、表格或JSON。** 而不是自由形式的段落，您定义字段、类型和允许的值。', '结构化输出可以采用多种形式：' ], items: [ '具有固定项数的项目符号列表。', '具有特定列的Markdown表格。', '简单属性的键值对。', '具有预定义键的完整JSON对象或数组。' ] }, structuredGoal: { content: [ '目标始终是相同的：将模糊的描述转换为可预测的形式。' ] }, whatIsJSON: { title: '什么是JSON模式', content: [ '**JSON模式是结构化输出的更严格变体，其中模型被指示或配置为仅返回有效的JSON。** 在JSON模式中，模型输出的所有内容都应该可以解析为JSON而无需额外的清理。', '典型的JSON架构可能如下所示：' ], codeBlock: '{\n  "title": "string",\n  "summary": "string",\n  "tags": ["string"],\n  "priority": "low | medium | high"\n}', codeLanguage: 'json' }, jsonModeApproach: { content: [ '您在提示词中反映该架构，然后要求模型填充它。某些平台也提供强制仅JSON响应的特殊设置或API。' ] }, whyItMatters: { title: '为什么结构化输出和JSON模式很重要', content: [ '**结构化输出和JSON模式很重要，因为它们使您能够将语言模型转变为更大系统的组件，而不仅仅是聊天助手。** 当输出是可预测的时，您可以：' ], items: [ '直接将结果提供给数据库、CRM或分析工具。', '基于模型输出字段（优先级、状态、信心）触发自动化。', '构建显示卡片、表格或仪表板中模型结果的用户界面。' ] }, debuggingBenefit: {
          content: [
            '它们还让提示词更容易调试。如果结构出错，你就知道问题出在提示词或架构上，而不是某个含糊的"质量"维度。',
          ],
        },
        jsonVsFunctionCalling: { title: 'JSON模式与函数调用与架构提示词', content: [ '**存在三种从LLM获取结构化输出的方法。每种都有不同的优缺点。**' ], items: [ '**JSON模式** : 模型仅输出有效的JSON。最佳用途：数据提取、分类、摘要。', '**函数调用** : 模型选择要调用的函数并以JSON提供参数。最佳用途：API集成、工具使用、代理工作流。', '**架构提示词** : 显式指令+要求模型遵循架构的示例。最佳用途：灵活性、开源模型、自定义格式。' ] }, example: { title: '示例：自由文本与结构化JSON', content: [ '**当您比较免费文本提示词和结构化JSON提示词完成相同任务时，差异变得明显。** 在这里，我们对客户电子邮件进行分类和汇总。', '**[错误的提示词]**', '"阅读此客户电子邮件并总结他们想要什么。"', '**[正确的提示词 - JSON模式]**', '"您是一名客户支持助理。"', '"正确"版本定义架构、有效值和仅JSON要求。' ] }, bestPractices: { title: '结构化输出和JSON模式的最佳做法', content: [ '**要获得可靠的结构化输出，您在提示词中需要明确、一致和严格。** 当抽取的数据不能离开自有基础设施时，同样的 JSON 模式模式也可以直接对接本地向量存储——要查看符合 GDPR 的部署模板，请参阅[面向企业数据的本地 RAG](/zh/power-local-llm/local-rag-for-private-business-data)。' ], items: [ '显示您期望的确切架构。', '声明仅应返回JSON或结构。', '使用简短、明确的键名称。', '当任务复杂或敏感时添加有效输出的示例。', '对于嵌套结构，逐步构建并使用真实输入进行测试。' ] }, practicesAddendum: {
          content: [
            '如果仍然出现格式问题，可以加一条简单的指令，例如"如果不确定，请把该字段留为空字符串，不要猜测。"结构化输出与[RAG（检索增强生成）](/zh/prompt-engineering/rag-explained)结合使用效果最好，可用于核查抽取出的数据。当抽取的数据必须留在自有基础设施内时，同样的JSON模式可以直接对接本地部署的向量存储——符合GDPR的部署模板请参阅[面向企业数据的本地RAG](/zh/power-local-llm/local-rag-for-private-business-data)。',
          ],
        },
        modelComparison: { title: '模型比较：按提供商的JSON合规性', content: [ '**不同的模型对原生JSON模式支持的级别不同。** 截至2026年4月，以下是主要提供商的排名：' ], columns: [ '模型', '原生JSON模式', '仅提示词合规', '备注' ], rows: [ { '模型': 'OpenAI GPT-5.6', '原生JSON模式': '是（强制）', '仅提示词合规': '不需要', '备注': 'JSON模式的行业标准。' }, { '模型': 'Anthropic Claude Sonnet 5', '原生JSON模式': '是（强制）', '仅提示词合规': '不需要', '备注': 'JSON合规性优秀。' }, { '模型': 'Google Gemini 2.0', '原生JSON模式': '是（强制）', '仅提示词合规': '不需要', '备注': '原生JSON支持。' }, { '模型': 'Meta Llama 3.3（70B）', '原生JSON模式': '部分', '仅提示词合规': '强烈推荐', '备注': '开源。' }, { '模型': 'Mistral Large', '原生JSON模式': '部分', '仅提示词合规': '推荐', '备注': 'JSON行为良好。' }, { '模型': '旧GPT-3.5、Claude 2', '原生JSON模式': '否', '仅提示词合规': '必需', '备注': '需要强大的工程。' }, { '模型': '小型开源模型（<13B）', '原生JSON模式': '否', '仅提示词合规': '示例需要', '备注': '需要详细的架构。' } ] }, regulatedEnvironments: {
          title: '受监管环境中的结构化输出',
          content: [
