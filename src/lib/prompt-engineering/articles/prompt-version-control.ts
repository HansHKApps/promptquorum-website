      ...
      faq: {
        ...
        faqs: [
          ...
          },
          {
            ...
            a: 'プロンプトのバージョン管理は、モデルに送信するテキスト指示への変更を追跡します。モデルのバージョン管理は、アプリケーションが呼び出すAIのバージョン（GPT-5.5・Claude Sonnet 4.6・Llama 4）を追跡します。両方に別個のバージョン管理が必要です。対象モデルを変更する場合、プロンプトテキストが同一でもMAJORバンプとして扱ってください。異なるモデルは同じプロンプトに対して異なる応答を返します。',
          },
          {
      ...
      faq: {
        ...
        faqs: [
          ...
          },
          {
            ...
            a: 'Prompt 版本控制追踪发送给模型的文本指令的变更。模型版本控制追踪应用程序调用的 AI 版本（GPT-5.5、Claude Sonnet 4.6、Llama 4）。两者都需要独立的版本控制。切换目标模型时，即使 Prompt 文本相同，也应视为 MAJOR 版本升级——不同模型对相同 Prompt 的响应可能不同。',
          },
          {
