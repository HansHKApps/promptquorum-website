    ...
    de: {
      ...
      sections: {
        ...
        workflow: {
          ...
          items: [
            '**Modellauswahl:** GPT-5.5, Claude Opus 4.7 und Gemini 1.5 Pro reagieren unterschiedlich auf denselben Prompt. Das richtige Modell für die Aufgabe zu wählen ist Teil des Engineering-Prozesses. Mistral AI (Europa) und Qwen (China) folgen denselben Prompting-Prinzipien, erfordern aber möglicherweise angepassste Ausgabeformat-Spezifikationen aufgrund von Unterschieden im Instruction-Following-Verhalten. Der gleiche strukturierte Prompt funktioniert weltweit in allen großen Modellfamilien → [Fundamentals: GPT, Claude or Gemini? How to Pick the Right Model]',
    ...
    fr: {
      ...
      sections: {
        ...
        workflow: {
          ...
          items: [
            '**Sélection du modèle :** GPT-5.5, Claude Opus 4.7 et Gemini 1.5 Pro réagissent différemment au même prompt. Choisir le bon modèle pour la tâche fait partie du processus d\'ingénierie → [Fundamentals: GPT, Claude or Gemini? How to Pick the Right Model]',
    ...
    ja: {
      ...
      sections: {
        ...
        workflow: {
          ...
          items: [
            '**モデルの選択：** GPT-5.5、Claude Opus 4.7、Gemini 1.5 Proは同じプロンプトに対して異なる反応を示します。タスクに適したモデルを選ぶことも、エンジニアリングプロセスの一部です → [Fundamentals: GPT, Claude or Gemini? How to Pick the Right Model]',
    ...
    zh: {
      ...
      sections: {
        ...
        workflow: {
          ...
          items: [
            '**模型选择：** GPT-5.5、Claude Opus 4.7 和 Gemini 1.5 Pro 对相同提示词的响应各有不同。为任务选择合适的模型是工程流程的一部分。Mistral AI（欧洲）和阿里云 Qwen（中国）遵循相同的提示工程原则，但由于指令跟随行为的差异，可能需要调整输出格式规范。相同的结构化提示在全球所有主要模型族中都有效 → [Fundamentals: GPT, Claude or Gemini? How to Pick the Right Model]',
