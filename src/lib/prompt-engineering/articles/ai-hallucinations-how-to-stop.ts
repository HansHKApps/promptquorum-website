    ...
    en: {
      ...
      sections: {
        ...
        definition: {
          ...
          snippets: [
            { type: 'in-one-sentence', text: 'Hallucinations are fluent false statements that language models generate because they predict text patterns rather than retrieve facts from a reliable store.' },
            { type: 'in-plain-terms', text: 'The model is not lying — it has no idea it is wrong. It is finishing your sentence the way the training data suggests, and a confident-sounding wrong answer scores just as well as a right one. That is why fabricated sources read exactly like real ones.' },
          ],
          content: [
    ...
    de: {
      ...
      sections: {
        definition: {
          ...
          snippets: [
            { type: 'in-one-sentence', text: 'Halluzinationen sind flüssig formulierte Falschaussagen, die Sprachmodelle erzeugen, weil sie Textmuster vorhersagen, statt Fakten aus einer verlässlichen Quelle abzurufen.' },
            { type: 'in-plain-terms', text: 'Das Modell lügt nicht — es weiß schlicht nicht, dass es falsch liegt. Es vervollständigt Ihren Satz so, wie es die Trainingsdaten nahelegen, und eine selbstbewusst klingende falsche Antwort schneidet dabei genauso gut ab wie eine richtige. Deshalb lesen sich erfundene Quellen exakt wie echte.' },
          ],
          content: [
    ...
    es: {
      ...
      sections: {
        definition: {
          ...
          snippets: [
            { type: 'in-one-sentence', text: 'Las alucinaciones son afirmaciones falsas pero fluidas que los modelos de lenguaje generan porque predicen patrones de texto en lugar de recuperar hechos de una fuente fiable.' },
            { type: 'in-plain-terms', text: 'El modelo no miente: simplemente no sabe que se equivoca. Está completando tu frase como sugieren los datos de entrenamiento, y una respuesta incorrecta que suena segura puntúa igual de bien que una correcta. Por eso las fuentes inventadas se leen exactamente como las reales.' },
          ],
          content: [
    ...
    ar: {
      ...
      sections: {
        definition: {
          ...
          snippets: [
            { type: 'in-one-sentence', text: 'الهلوسات هي عبارات خاطئة لكنها سلسة الصياغة، تنتجها نماذج اللغة لأنها تتنبأ بأنماط النص بدلاً من استرجاع الحقائق من مصدر موثوق.' },
            { type: 'in-plain-terms', text: 'النموذج لا يكذب — فهو ببساطة لا يعرف أنه مخطئ. إنه يُكمل جملتك بالطريقة التي توحي بها بيانات التدريب، والإجابة الخاطئة التي تبدو واثقة تحصل على تقييم جيد تماماً كالإجابة الصحيحة. لهذا تبدو المصادر الملفَّقة مطابقة تماماً للمصادر الحقيقية.' },
          ],
          content: [
    ...
    pt: {
      ...
      sections: {
        definition: {
          ...
          snippets: [
            { type: 'in-one-sentence', text: 'Alucinações são afirmações falsas, mas fluentes, que os modelos de linguagem geram porque preveem padrões de texto em vez de recuperar fatos de uma fonte confiável.' },
            { type: 'in-plain-terms', text: 'O modelo não está mentindo: ele simplesmente não sabe que errou. Ele está completando a sua frase do jeito que os dados de treinamento sugerem, e uma resposta errada que soa segura pontua tão bem quanto uma certa. É por isso que fontes inventadas se leem exatamente como as reais.' },
          ],
          content: [
    ...
    fr: {
      ...
      sections: {
        definition: {
          ...
          snippets: [
            { type: 'in-one-sentence', text: 'Les hallucinations sont des affirmations fausses mais fluides que les modèles de langage produisent parce qu\'ils prédisent des motifs de texte au lieu de récupérer des faits dans une source fiable.' },
            { type: 'in-plain-terms', text: 'Le modèle ne ment pas : il ignore simplement qu\'il se trompe. Il complète votre phrase comme le suggèrent ses données d\'entraînement, et une réponse fausse mais assurée obtient un aussi bon score qu\'une réponse juste. D\'où des sources inventées qui se lisent exactement comme les vraies.' },
          ],
          content: [
    ...
    ja: {
      ...
      sections: {
        definition: {
          ...
          snippets: [
            { type: 'in-one-sentence', text: 'ハルシネーションとは、言語モデルが信頼できる情報源から事実を取得するのではなく、テキストのパターンを予測することによって生成する、流暢な虚偽の記述です。' },
            { type: 'in-plain-terms', text: 'モデルは嘘をついているのではなく、自分が間違っていることに気づいていません。学習データが示すとおりに文を続けているだけで、自信ありげな誤答も正答と同じように高く評価されます。だからこそ、捏造された出典が本物とまったく同じように読めるのです。' },
          ],
          content: [
    ...
    zh: {
      ...
      sections: {
        definition: {
          title: '什么是AI幻觉？',
          snippets: [
            { type: 'in-one-sentence', text: '幻觉是指语言模型生成的流畅但错误的陈述，其成因是模型在预测文本模式，而不是从可靠来源检索事实。' },
            { type: 'in-plain-terms', text: '模型不是在撒谎，它根本不知道自己错了。它只是按训练数据的提示把你的句子补完，而一个听起来笃定的错误答案，得分和正确答案一样高。这正是编造的出处读起来和真实出处别无二致的原因。' },
          ],
          content: [
  ...
  ko: {
    ...
    sections: {
      definition: {
        title: 'AI 환각이란 무엇입니까?',
        snippets: [
          { type: 'in-one-sentence', text: '환각이란 언어 모델이 신뢰할 수 있는 출처에서 사실을 검색하는 대신 텍스트 패턴을 예측함으로써 생성하는, 유창하지만 거짓인 진술입니다.' },
          { type: 'in-plain-terms', text: '모델은 거짓말을 하는 것이 아니라 자신이 틀렸다는 사실을 모릅니다. 학습 데이터가 시사하는 대로 문장을 이어 갈 뿐이며, 확신에 찬 오답도 정답과 똑같이 높은 점수를 받습니다. 조작된 출처가 실제 출처와 똑같이 읽히는 이유가 여기에 있습니다.' },
        ],
        content: [
