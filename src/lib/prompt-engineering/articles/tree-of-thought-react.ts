...
Réponse finale : [conclusion synthétisée]`,
  ...
  ja: {
    ...
    quickFacts: [
      ...
      '**ToT 向き最高モデル：** Claude Opus 4.7 (extended thinking)、GPT-5.5 (reasoning mode)、Gemini 3.1 Pro (Deep Think)',
    ...
    faqSchema: {
      ...
      mainEntity: [
        ...
        { '@type': 'Question', name: '2026年でもReActを手動でフォーマットする必要がありますか？', acceptedAnswer: { '@type': 'Answer', text: 'ネイティブツール使用を持つ frontier モデル (GPT-5.5、Claude Opus 4.7、Gemini 3.1 Pro) では不要です。これらのモデルはループを自動的に実装します。手動フォーマットは open-weights モデルや学習コンテキストで有用です。' } },
        ...
        { '@type': 'Question', name: 'どのモデルがTree-of-Thoughtに最適ですか？', acceptedAnswer: { '@type': 'Answer', text: 'Claude Opus 4.7 (extended thinking)、GPT-5.5 (reasoning mode)、Gemini 3.1 Pro (Deep Think) は ToT を最も自然に処理します。' } },
...
Final Answer: [総合結論]`,
      ...
      react2026: {
        ...
        content: [
          '「元々の ReAct 論文 (2023)」は手動の Thought/Action/Observation フォーマットをプロンプティングパターンとして提案しました。2026年、GPT-5.5、Claude Opus 4.7/Sonnet 4.6、Gemini 3.1 Pro は Function Calling / native tools を通じて ReAct ループを自動実装します。',
      ...
      howToStart: {
        ...
        numberedItems: [
          ...
          'PromptQuorum で両パターンを GPT-5.5、Claude Opus 4.7、Gemini 3.1 Pro で並べてテスト',
        ],
      },
      inPromptQuorum: {
        title: 'PromptQuorum で',
        content: 'ToT と ReAct パターンを GPT-5.5、Claude Opus 4.7/Sonnet 4.6、Gemini 3.1 Pro で並べてテスト。トークンコスト、出力品質、レイテンシを測定。複数 API にデータ露出なし。',
      },
      relatedReading: {
        title: '関連資料',
        items: [
      ...
      faqSection: {
        ...
        faqs: [
          ...
          { q: '2026年でもReActを手動でフォーマットする必要がありますか？', a: 'Native tool use 付き frontier モデル (GPT-5.5、Claude Opus 4.7、Gemini 3.1 Pro) では不要です。これらはループを自動実装します。手動 `Thought:/Action:/Observation:` フォーマットは open-weights モデルや教育・シミュレーション目的で有用です。' },
          ...
          { q: 'どのモデルがTree-of-Thoughtに最適ですか？', a: 'Extended Thinking / Reasoning モード付きモデルが ToT を最も自然に処理：Claude Opus 4.7 (extended thinking)、GPT-5.5 (reasoning mode)、Gemini 3.1 Pro (Deep Think)。これらは明示的なプロンプトレベル ToT なしで内部的に複数ブランチを探索できます。' },
  ...
  zh: {
    ...
    quickFacts: [
      ...
      '**ToT 最佳模型：** Claude Opus 4.7（扩展思维）、GPT-5.5（推理模式）、Gemini 3.1 Pro（深度思维）',
    ...
    faqSchema: {
      ...
      mainEntity: [
        ...
        { '@type': 'Question', name: '2026 年还需要手动格式化 ReAct 吗？', acceptedAnswer: { '@type': 'Answer', text: '在具有原生工具使用的前沿模型（GPT-5.5、Claude Opus 4.7、Gemini 3.1 Pro）上不需要。这些模型自动实现循环。手动格式化对于没有工具使用的开源权重模型或教学目的仍然有用。' } },
        ...
        { '@type': 'Question', name: '哪些模型最适合树思考？', acceptedAnswer: { '@type': 'Answer', text: '具有扩展思维/推理模式的模型最自然地处理 ToT：Claude Opus 4.7（扩展思维）、GPT-5.5（推理模式）和 Gemini 3.1 Pro（深度思维）。这些模型可以在内部探索多个分支。' } },
    ...
    sections: {
      ...
      react2026: { id: 'react-2026', title: '2026 年的 ReAct：从提示模式到内置行为', content: ['原始 ReAct 论文（2023）提议手动 Thought/Action/Observation 格式作为提示模式。2026 年，GPT-5.5、Claude Opus 4.7/Sonnet 4.6 和 Gemini 3.1 Pro 通过 Function Calling/原生工具自动实现 ReAct 循环。', '模型决定何时调用工具、接收结果、继续推理——无需手动格式化。只需定义可用工具；模型管理循环。', '**手动格式化仍然相关的情况：**', '1. **开源权重模型**：没有原生工具使用（Mistral、旧 LLaMA 变体、Qwen）', '2. **教育/调试上下文**：希望看到完整追踪', '3. **模拟工具交互**：没有真实 API 连接', '**生产 ReAct：** 自主 AI 编码工具（Claude Code、OpenAI Codex、Cursor）是大规模 productionized ReAct，其中"行动"= 文件编辑和测试运行。'], callouts: [{ type: 'warning', label: '警告', text: '树思考提示可以生成标准提示 3-5 倍的输出 token。在 $0.025/1M 输出 token（Claude Opus 4.7），生成 5,000 token 的复杂 ToT 每次运行成本约为 $0.125。相应进行预算。' }] },
      ...
      howToStart: { id: 'how-to-start', title: '入门方法', numberedItems: ['战略/计划 → 使用带明确分支数和评估标准的 ToT', '带工具的研究/调试 → 使用 ReAct（或只在前沿模型上使用原生工具使用）', '组合：计划阶段用 ToT，选定分支内的执行用 ReAct', '在 PromptQuorum 上跨 GPT-5.5、Claude Opus 4.7、Gemini 3.1 Pro 并行测试两种模式'] },
      inPromptQuorum: { title: '在 PromptQuorum 中', content: '跨 GPT-5.5、Claude Opus 4.7/Sonnet 4.6、Gemini 3.1 Pro 并行测试 ToT 和 ReAct 模式。测量 token 成本、输出质量和延迟，无需向多个 API 暴露数据。' },
