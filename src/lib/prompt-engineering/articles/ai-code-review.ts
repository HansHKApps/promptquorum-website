  ...
  ko: {
    ...
    faqSchema: {
      ...
      mainEntity: [
        ...
        },
        {
          ...
          acceptedAnswer: { '@type': 'Answer', text: 'Claude Sonnet 4.6는 가장 포괄적인 보안 분석을 생성합니다 — SQL 인젝션 벡터, 누락된 입력 정제, 인증 엣지 케이스를 식별합니다. GPT-5.5는 가장 실행 가능한 수정 제안을 생성합니다 — 설명 대신 구체적인 수정 코드. 보안 리뷰의 경우 세 가지 모두 실행하고 수렴하는 결과를 높은 신뢰도 문제로 취급하십시오.' },
        },
        {
          '@type': 'Question',
    ...
    sections: {
      ...
      toolComparison: {
        ...
        imageCaption: 'AI 코드 리뷰 도구 비교: PromptQuorum은 GPT-5.5 + Claude를 동시에 디스패치합니다 — 두 모델이 개별 모델보다 다른 버그 클래스를 감지합니다.',
      },
      ...
      contextWindow: {
        ...
        rows: [
          { '모델': 'GPT-5.5 (OpenAI)', '컨텍스트 윈도우': '1M 토큰', '코드 줄 수 (근사)': '~75만 줄', '사용 사례': '전체 프로젝트 PR 리뷰' },
          { '모델': 'Claude Sonnet 4.6 (Anthropic)', '컨텍스트 윈도우': '1M 토큰', '코드 줄 수 (근사)': '~75만 줄', '사용 사례': '멀티 파일 보안 리뷰' },
      ...
      faq: {
        ...
        faqs: [
          ...
          },
          {
            ...
            a: 'Claude Sonnet 4.6는 가장 포괄적인 보안 분석을 생성합니다 — SQL 인젝션 벡터, 누락된 입력 정제, 인증 엣지 케이스를 식별합니다. GPT-5.5는 가장 실행 가능한 수정 제안을 생성합니다 — 설명 대신 구체적인 수정 코드. 세 가지 프론티어 모델 모두 이제 1M 토큰 (~75만 줄의 코드) 컨텍스트 윈도우를 지원합니다. 보안 리뷰의 경우 세 가지 모두 실행하고 수렴하는 결과를 높은 신뢰도 문제로 취급하십시오.',
          },
          {
