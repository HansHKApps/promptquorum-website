...
export const article: Record<Language, PEArticle> = {
  ...
  zh: {
    ...
    dateModified: '2026-05-03',
    readTime: '阅读约9分钟',
    seoTitle: 'Google提示词指南2026：5大核心原则',
    ...
    next_refresh_due: '2026-09-24',
    leadAnswerBlock: '**Google指南优先考虑明确性而非技巧：明确定义任务、受众和约束。5大中核原则—明确性、结构、例子、角色、约束—在所有现代模型（Gemini 3.1 Pro、GPT-5.5、Claude Opus 4.7）上有效，生成更可预测、可复用的提示词。**',
    quickFacts: [
      '**来源:** Google DeepMind将指南作为研究支持的最佳实践发布（2024–2026）',
      '**5大中核原则:** 明确性、结构、例子、角色、约束（CSERC）',
      '**模型无关:** 在Gemini 3.1 Pro、GPT-5.5、Claude Opus 4.7、Ollama本地模型上同等有效',
    ...
    faqSchema: {
      ...
      inLanguage: 'zh',
      mainEntity: [
        { '@type': 'Question', name: 'Google指南只适用于Gemini吗？', acceptedAnswer: { '@type': 'Answer', text: '不是。原则是通用的，对GPT-5.5、Claude和所有现代模型都同样有效。Gemini是主要例子，但核心思想是普遍的。' } },
    ...
    sections: {
      tldr: { id: 'key-takeaways', title: '核心要点', isTldr: true, items: ['Google指南将重点放在清晰性而非技巧上：明确定义任务、受众和约束。', '5大中核原则（清晰性、结构、例子、角色、约束）在所有现代模型（Gemini 3.1 Pro、GPT-5.5、Claude Opus 4.7）上一致有效。', '明确的约束和示例比冗长说明更有力。写得好的例子值得10行说明。', '角色提示词（"你是…"）引导语气和细节程度，对创意任务特别有用。', '逐步推理（"step-by-step think"）强制结构化思维，减少错误，但增加token使用。', '对于高风险决策多样化输入和模型测试；一个提示词在一个例子上有效不等于在所有情况下有效。', '在PromptQuorum中并排测试所有框架，比较Gemini 3.1 Pro、GPT-5.5和Claude Opus 4.7的输出。'] },
      ...
      howPqImplements: { id: 'how-pq-implements', title: 'PromptQuorum如何实现它', content: 'PromptQuorum将Google指南集成为一个内置框架。创建提示词时，用户可选择"Google"模板，工具会：1) 提示输入清晰任务定义、受众和约束；2) 从库生成例子；3) 应用格式规则；4) 跨Gemini 3.1 Pro、GPT-5.5和Claude Opus 4.7测试输出。这使Google原则可操作化，不需要用户手动构造提示词。' },
      ...
      howToStart: { id: 'how-to-start', title: '开始使用', numberedItems: ['**明确定义。** 避免模糊。明确说出任务、受众、想要什么。包括任何重要约束。', '**提供例子。** 对于任何涉及风格、格式或复杂判断的任务，包括1–3个输入-输出对。', '**请求结构化思考。** 对于复杂问题，要求模型在回答前"thinking step by step"。', '**定义输出格式。** 说"以JSON/表格/Markdown返回"并给出示范。', '**在PromptQuorum中测试。** 对提示词运行A/B对比；跨Gemini 3.1 Pro、GPT-5.5、Claude Opus 4.7测试相同提示词，看哪个模型最一致。'] },
      ...
      sources: { id: 'sources', title: '来源', items: [
        ...
        'OpenAI & Anthropic API文档（2026） — GPT-5.5、Claude Opus 4.7、开源模型的提示词工程最佳实践',
