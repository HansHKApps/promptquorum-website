...
export const article: Record<Language, PEArticle> = {
    ...
    de: {
      ...
      seoTitle: 'Prompt-Evaluierungsmetriken: Pass Rate, BLEU & LLM-as-Judge',
      metaDescription: 'Die richtige Evaluierungsmetrik je Ausgabetyp wählen: Pass Rate für JSON, BLEU für Übersetzungen, semantische Ähnlichkeit für Paraphrasen, LLM-as-Judge für Freitext. April 2026.',
      ogDescription: 'BLEU-Score auf JSON-Prompts verwenden? Dieser Score bedeutet nichts. Die richtige Evaluierungsmetrik hängt vollständig vom Ausgabetyp ab — wer falsch wählt, bekommt falsches Vertrauen, das echte Produktionsfehler verbirgt.',
      twitterDescription: 'Pass Rate für JSON. BLEU für Übersetzung. Semantische Ähnlichkeit für Paraphrase. LLM-as-Judge für Freitext. Wähle die richtige Metrik oder bekomme falsches Vertrauen.',
    ...
    fr: {
      ...
      seoTitle: 'Métriques d\'évaluation de prompts : BLEU et LLM-as-Judge',
      metaDescription: 'Choisissez la bonne métrique selon le type de sortie : taux de réussite pour JSON, BLEU pour les traductions, similarité sémantique pour les reformulations, LLM-as-Judge pour le texte libre.',
      ogDescription: 'Utiliser BLEU sur des prompts JSON ? Ce score ne signifie rien. La bonne métrique dépend entièrement du type de sortie — choisir mal cache les vrais problèmes en production.',
      twitterDescription: 'Taux de réussite pour JSON. BLEU pour traduction. Similarité sémantique pour reformulation. LLM-as-Judge pour texte libre. Choisissez bien ou ayez faux confiance.',
    ...
    ja: {
      ...
      seoTitle: 'プロンプト評価指標：合格率・BLEUスコア・LLM-as-Judge',
      metaDescription: '出力タイプ別に最適なプロンプト評価指標を選択する方法：JSON用は合格率、翻訳用はBLEUスコア、言い換えには意味的類似度、自由記述にはLLM-as-Judgeを使用。回帰検出の実践フレームワーク。2026年4月。',
      ogDescription: 'JSONプロンプトにBLEUを使ってませんか？そのスコアは意味がありません。正しい評価指標は出力タイプに完全に依存します。選択を間違えると、本番環境の問題を隠す間違った信頼を生み出します。',
      twitterDescription: 'JSONは合格率。翻訳はBLEU。言い換えは意味的類似度。自由記述はLLM-as-Judge。正しい指標を選ばないと間違った信頼になります。',
    ...
    zh: {
      ...
      seoTitle: '提示词评估指标：通过率、BLEU与LLM-as-Judge',
      metaDescription: '按输出类型选择正确的提示词评估指标：JSON用通过率、翻译用BLEU分数、改写任务用语义相似度、自由文本用LLM-as-Judge。附回归检测框架。2026年4月。',
      ogDescription: '在JSON提示词上使用BLEU？那个分数毫无意义。正确的评估指标完全取决于你的输出类型——选错了会产生虚假信心，掩盖真正的生产问题。',
      twitterDescription: 'JSON用通过率。翻译用BLEU。改写用语义相似度。自由文本用LLM-as-Judge。选择正确的指标或者得到虚假信心。',
