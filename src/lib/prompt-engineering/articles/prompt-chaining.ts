...
export const article: Partial<Record<Language, PEArticle>> = {
    en: {
      ...
      theme: 'Techniques',
      heroImage: '/images/prompt-chaining-overview-hero-en.webp',
    ...
    de: {
      freshness_tier: 'semi_annual',
      theme: 'Techniques',
      heroImage: '/images/prompt-chaining-overview-hero-de.webp',
    ...
    es: {
      freshness_tier: 'semi_annual',
      theme: 'Techniques',
      heroImage: '/images/prompt-chaining-overview-hero-es.webp',
    ...
    ar: {
      freshness_tier: 'semi_annual',
      theme: 'Techniques',
      heroImage: '/images/prompt-chaining-overview-hero-ar.webp',
    ...
    pt: {
      theme: 'Techniques',
      heroImage: '/images/prompt-chaining-overview-hero-pt.webp',
    ...
    fr: {
      freshness_tier: 'semi_annual',
      theme: 'Techniques',
      heroImage: '/images/prompt-chaining-overview-hero-fr.webp',
  ...
  freshness_tier: 'semi_annual',
  theme: 'Techniques',
      heroImage: '/images/prompt-chaining-overview-hero-ja.webp',
...
zh: {
      freshness_tier: 'semi_annual',
      theme: 'Techniques',
  heroImage: '/images/prompt-chaining-overview-hero-zh.webp',
      ...
      sections: {
        ...
        faqSection: {
          ...
          faqs: [
            {
              q: '提示词链接和单个复杂提示词有什么区别？',
              a: '单个复杂提示词试图一次性完成所有工作（分析、规划、生成、审查）。提示词链接将这些拆分为多个步骤。单一提示词更简单，但在处理复杂任务时可靠性较低。链条更透明、更易测试，但需要更多设置和API调用。',
            },
            {
              q: '一个提示词链应该包含多少个步骤？',
              a: '最有效的链条通常有3-5个步骤。每个步骤应足够简单，能装入一个清晰的提示词中（指令少于500个token）。超过7个步骤通常意味着过度设计。请自问：这个步骤是否增加了价值，还是可以与前一步骤合并？',
            },
            {
              q: '什么时候应该使用提示词链接而不是微调？',
              a: '当您想将复杂任务分解为可管理的阶段时，使用链接。当单个模型在某项任务（如分类）上系统性表现不佳且您拥有训练数据时，使用微调。两者并不对立——您可以将经过微调的模型链接在一起。',
            },
            {
              q: '如果链条中的某一步骤失败会怎样？',
              a: '整个链条通常会停止。为应对这种情况，请在每个步骤后添加验证以尽早捕获错误。实施回退机制（例如"如果JSON解析失败，用更简单的指令重试"）。也可以选择将失败情况转交人工审查，而不是直接中断。',
            },
          ],
        },
      },
  ...
  ko: {
      freshness_tier: 'semi_annual',
      theme: '기법',
    heroImage: '/images/prompt-chaining-overview-hero-ko.webp',
      ...
      sections: {
        ...
        relatedReading: {
          title: '관련 자료',
          items: [
            '[사고 연쇄 프롬프팅](/ko/prompt-engineering/chain-of-thought-prompting) — CoT는 단일 프롬프트 추론이며, 체이닝은 여러 프롬프트를 순서화합니다.',
            '[Tree-of-Thought and ReAct](/ko/prompt-engineering/tree-of-thought-and-react) — ReAct는 특정 체이닝 패턴(추론 → 행동 → 관찰 루프)입니다.',
            '[제약 프롬프팅](/ko/prompt-engineering/constrained-prompting) — 신뢰할 수 있는 핸드오프를 위해 각 체인 단계의 출력 형식을 제한합니다.',
            '[페르소나 프롬프팅](/ko/prompt-engineering/persona-prompting) — 각 체인 단계에서 서로 다른 페르소나 사용(분석가 → 작가 → 편집자).',
            '[RTF 프레임워크](/ko/prompt-engineering/rtf-framework) — 역할-작업-형식은 개별 체인 단계에 자연스럽게 매핑됩니다.',
            '[토큰, 비용 및 제한](/ko/prompt-engineering/tokens-costs-limits-economics-of-ai-prompting) — 체인은 토큰을 2~5배 더 사용하며, 비용 영향이 있습니다.',
            '[GPT, Claude, 또는 Gemini?](/ko/prompt-engineering/gpt-claude-or-gemini-how-to-pick-the-right-model) — 서로 다른 체인 단계에서 서로 다른 모델 사용.',
          ],
        },
      },
    },
