    ...
    sections: {
      ...
      tldr: {
        ...
        items: [
          ...
          'System-Prompts machen etwa 70 % der Verhaltenskonsistenz aus basierend auf PromptQuorum-Tests über GPT-4o, Claude Opus 4.8 und Gemini 3.1 Pro; User-Prompts formen spezifische Ausgaben',
      ...
      whereTheyLive: {
        ...
        content: [
          '**System-Prompts leben in der Anwendungsschicht; User-Prompts leben in der Interaktionsschicht.** Wenn du GPT-4o über die OpenAI-API aufrufst, akzeptiert der Endpoint zwei separate Eingaben: `system` (die persistenten Anweisungen) und `messages` (per-Anfrage-Benutzereingabe). Dasselbe gilt für Claude Opus 4.8 über die Anthropic-API, Gemini 3.1 Pro über die Google-API und jedes lokale LLM, das über [Ollama](/prompt-engineering/context-windows-explained-why-ai-forgets) oder LM Studio läuft.',
          'Alle Modelle unterstützen das System + User-Prompt-Muster:',
        ],
        items: [
          '**Modell-Schicht:** Das Basis-LLM (GPT-4o, Claude Opus 4.8, Gemini 3.1 Pro, LLaMA 3.1, Mistral Large) — alle akzeptieren sowohl System- als auch User-Prompts',
      ...
      promptquorumDifference: {
        ...
        items: [
          ...
          '**2. Du kannst Verhalten nicht über Modelle replizieren.** Wenn du einen System-Prompt-Design entdeckst, der gut für Claude Opus 4.8 funktioniert, kannst du ihn nicht zu GPT-4o kopieren, weil du Claudes System-Prompt nie gesehen hast. Du musst ihn reverse-engineern.',
      ...
      promptquorum: {
        ...
        content: [
          '**PromptQuorum ist das einzige Tool, das dir den System-Prompt UND User-Prompt für jedes Modell nebeneinander zeigt.** Wenn du einen Prompt über PromptQuorum gleichzeitig an GPT-4o, Claude Opus 4.8 und Gemini 3.1 Pro sendest, siehst du:',
        ],
        items: [
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[Prompt-Engineering-Geschichte](/prompt-engineering/prompt-engineering-history) — wie Prompt Engineering sich von GPT-3 zu GPT-4o und Claude Opus 4.8 entwickelt hat',
        ],
      },
    },
  },
    ...
    sections: {
      ...
      tldr: {
        ...
        items: [
          ...
          'Les prompts système représentent environ 70 % de la cohérence comportementale selon les tests PromptQuorum sur GPT-4o, Claude Opus 4.8 et Gemini 3.1 Pro ; les prompts utilisateur façonnent les sorties spécifiques',
      ...
      promptquorumDifference: {
        ...
        items: [
          ...
          '**2. Tu ne peux pas répliquer le comportement entre les modèles.** Si tu découvres un design de prompt système qui fonctionne bien pour Claude Opus 4.8, tu ne peux pas le copier à GPT-4o parce que tu n\'as jamais vu le prompt système de Claude. Tu dois le rétro-ingéniérer.',
      ...
      promptquorum: {
        ...
        content: [
          '**PromptQuorum est le seul outil qui te montre le prompt système ET le prompt utilisateur pour chaque modèle côte à côte.** Lorsque tu envoies un prompt via PromptQuorum simultanément à GPT-4o, Claude Opus 4.8 et Gemini 3.1 Pro, tu vois :',
        ],
        items: [
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[Histoire de l\'ingénierie des prompts](/prompt-engineering/prompt-engineering-history) — comment l\'ingénierie des prompts a évolué de GPT-3 à GPT-4o et Claude Opus 4.8',
        ],
      },
    },
  },
    ...
    sections: {
      ...
      tldr: {
        ...
        items: [
          ...
          'システムプロンプトはGPT-4o、Claude Opus 4.8、Gemini 3.1 Proにおける動作一貫性の約70%を占めます（PromptQuorumテストに基づく）；ユーザープロンプトは特定の出力を形成します',
      ...
      whereTheyLive: {
        ...
        content: [
          '**システムプロンプトはアプリケーション層に存在し、ユーザープロンプトはインタラクション層に存在します。** OpenAI APIを通じてGPT-4oを呼び出すと、エンドポイントは2つの別の入力を受け入れます：`system`（永続的な指示）と`messages`（リクエストごとのユーザー入力）。これはAnthropic APIを通じたClaude Opus 4.8、Google APIを通じたGemini 3.1 Pro、[Ollama](/prompt-engineering/context-windows-explained-why-ai-forgets)またはLM Studioで実行されるローカルLLMでも同じです。',
          'すべてのモデルはシステム+ユーザープロンプトパターンをサポートしています：',
        ],
        items: [
          '**モデル層：** ベースLLM（GPT-4o、Claude Opus 4.8、Gemini 3.1 Pro、LLaMA 3.1、Mistral Large）—すべてシステムとユーザープロンプトを受け入れます',
      ...
      promptquorumDifference: {
        ...
        items: [
          ...
          '**2.モデル間で動作を複製できません。** Claude Opus 4.8で機能するシステムプロンプト設計を発見した場合、ClaudeのシステムプロンプトをGPT-4oにコピーすることはできません（見たことがないため）。リバースエンジニアリングが必要です。',
      ...
      promptquorum: {
        ...
        content: [
          '**PromptQuorumは、各モデルのシステムプロンプトとユーザープロンプトの両方を並べて表示する唯一のツールです。** PromptQuorumを通じてGPT-4o、Claude Opus 4.8、Gemini 3.1 Proにプロンプトを同時に送信すると、次が表示されます：',
        ],
        items: [
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[プロンプトエンジニアリングの歴史](/prompt-engineering/prompt-engineering-history)—プロンプトエンジニアリングがGPT-3からGPT-4oとClaude Opus 4.8へどのように進化したか',
        ],
      },
    },
  },
    ...
    sections: {
      ...
      tldr: {
        ...
        items: [
          ...
          '根据 PromptQuorum 在 GPT-4o、Claude Opus 4.8 和 Gemini 3.1 Pro 上的测试，系统提示占行为一致性的约 70%；用户提示塑造特定输出',
      ...
      whereTheyLive: {
        ...
        content: [
          '**系统提示位于应用层；用户提示位于交互层。** 通过 OpenAI API 调用 GPT-4o 时，端点接受两个独立输入：`system`（持久指令）和 `messages`（每请求用户输入）。Claude Opus 4.8 通过 Anthropic API、Gemini 3.1 Pro 通过 Google API，以及通过 [Ollama](/prompt-engineering/context-windows-explained-why-ai-forgets) 或 LM Studio 运行的任何本地 LLM 都是如此。',
          '所有模型都支持系统+用户提示模式：',
        ],
        items: [
          '**模型层：** 基础 LLM（GPT-4o、Claude Opus 4.8、Gemini 3.1 Pro、LLaMA 3.1、Mistral Large）——所有都接受系统和用户提示',
      ...
      promptquorumDifference: {
        ...
        items: [
          ...
          '**2. 你无法跨模型复制行为。** 如果你发现一个对 Claude Opus 4.8 有效的系统提示设计，你无法将其复制到 GPT-4o，因为你从未看过 Claude 的系统提示。你必须对其进行逆向工程。',
      ...
      promptquorum: {
        ...
        content: [
          '**PromptQuorum 是唯一向您并排显示每个模型的系统提示和用户提示的工具。** 当您通过 PromptQuorum 同时向 GPT-4o、Claude Opus 4.8 和 Gemini 3.1 Pro 发送提示时，您会看到：',
        ],
        items: [
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[提示工程历史](/prompt-engineering/prompt-engineering-history)——提示工程如何从 GPT-3 演变到 GPT-4o 和 Claude Opus 4.8',
        ],
      },
    },
  },
