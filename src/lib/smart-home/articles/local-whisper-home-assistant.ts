  ...
  en: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        faqs: [
          { q: 'What is the Wyoming protocol?', a: "Wyoming is the lightweight network protocol Home Assistant's Assist pipeline uses to talk to voice add-ons — speech-to-text (like the Whisper add-on), text-to-speech, and wake-word detection. It lets the add-on run as its own service, even on a separate, more powerful machine, and register itself with Assist automatically instead of requiring manual API wiring. You don't configure Wyoming directly — installing the Whisper add-on exposes a Wyoming endpoint that Assist detects and lets you select in your pipeline." },
  ...
  ar: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        faqs: [
          { q: 'ما هو بروتوكول Wyoming؟', a: 'Wyoming هو بروتوكول الشبكة خفيف الوزن الذي يستخدمه خط أنابيب Assist في Home Assistant للتواصل مع إضافات الصوت — تحويل الكلام إلى نص (مثل إضافة Whisper)، وتحويل النص إلى كلام، واكتشاف كلمة التنبيه. يتيح للإضافة العمل كخدمة مستقلة، حتى على جهاز منفصل وأقوى، وتسجّل نفسها تلقائيًا لدى Assist بدلاً من الحاجة إلى ربط API يدوي. لا تقوم بإعداد Wyoming مباشرة — تثبيت إضافة Whisper يعرض نقطة نهاية Wyoming التي يكتشفها Assist ويمكنك اختيارها في خط الأنابيب الخاص بك.' },
  ...
  de: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        faqs: [
          { q: 'Was ist das Wyoming-Protokoll?', a: 'Wyoming ist das schlanke Netzwerkprotokoll, mit dem die Assist-Pipeline von Home Assistant mit Sprach-Add-ons kommuniziert — Speech-to-Text (wie das Whisper-Add-on), Text-to-Speech und Wake-Word-Erkennung. Es ermöglicht dem Add-on, als eigener Dienst zu laufen, sogar auf einer separaten, leistungsstärkeren Maschine, und sich automatisch bei Assist zu registrieren, statt eine manuelle API-Verkabelung zu benötigen. Sie konfigurieren Wyoming nicht direkt — die Installation des Whisper-Add-ons stellt einen Wyoming-Endpunkt bereit, den Assist erkennt und den Sie in Ihrer Pipeline auswählen können.' },
  ...
  es: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        faqs: [
          { q: '¿Qué es el protocolo Wyoming?', a: 'Wyoming es el protocolo de red ligero que usa el pipeline Assist de Home Assistant para hablar con los complementos de voz: reconocimiento de voz (como el complemento Whisper), síntesis de voz y detección de palabra de activación. Permite que el complemento funcione como su propio servicio, incluso en una máquina separada más potente, y se registre automáticamente en Assist en lugar de requerir una conexión manual por API. No configuras Wyoming directamente: instalar el complemento Whisper expone un endpoint Wyoming que Assist detecta y que puedes seleccionar en tu pipeline.' },
  ...
  ko: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        title: '자주 묻는 질문',
        faqs: [
          { q: 'Wyoming 프로토콜이란 무엇입니까?', a: 'Wyoming은 Home Assistant의 Assist 파이프라인이 음성 애드온과 통신할 때 사용하는 경량 네트워크 프로토콜입니다 — Whisper 애드온과 같은 음성 인식, 음성 합성, 웨이크워드 감지 등에 사용됩니다. 이를 통해 애드온은 별도의 더 강력한 기기에서도 독립적인 서비스로 실행될 수 있으며, 수동 API 연결 없이 Assist에 자동으로 등록됩니다. Wyoming을 직접 설정할 필요는 없습니다 — Whisper 애드온을 설치하면 Wyoming 엔드포인트가 노출되고, Assist가 이를 감지하여 파이프라인에서 선택할 수 있게 됩니다.' },
  ...
  fr: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        faqs: [
          { q: 'Qu\'est-ce que le protocole Wyoming ?', a: "Wyoming est le protocole réseau léger que le pipeline Assist de Home Assistant utilise pour communiquer avec les modules complémentaires vocaux — reconnaissance vocale (comme le module Whisper), synthèse vocale et détection de mot de réveil. Il permet au module de fonctionner comme son propre service, même sur une machine séparée plus puissante, et de s'enregistrer automatiquement auprès d'Assist plutôt que de nécessiter un câblage API manuel. Vous ne configurez pas Wyoming directement : l'installation du module Whisper expose un point de terminaison Wyoming qu'Assist détecte et que vous pouvez sélectionner dans votre pipeline." },
  ...
  ja: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        title: 'よくある質問',
        faqs: [
          { q: 'Wyomingプロトコルとは何ですか？', a: 'Wyomingは、Home AssistantのAssistパイプラインが音声アドオン（Whisperアドオンのような音声認識、音声合成、ウェイクワード検出）と通信するために使う軽量なネットワークプロトコルです。これにより、アドオンは別の、より高性能なマシン上でも独自のサービスとして動作でき、手動でAPIを接続する必要なくAssistに自動的に登録されます。Wyomingを直接設定することはありません——Whisperアドオンをインストールすると、AssistがWyomingエンドポイントを検出し、パイプラインで選択できるようになります。' },
  ...
  pt: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        faqs: [
          { q: 'O que é o protocolo Wyoming?', a: 'Wyoming é o protocolo de rede leve que o pipeline Assist do Home Assistant usa para se comunicar com os complementos de voz — reconhecimento de voz (como o complemento Whisper), síntese de voz e detecção de palavra de ativação. Ele permite que o complemento rode como seu próprio serviço, mesmo em uma máquina separada e mais potente, e se registre automaticamente no Assist em vez de exigir conexão manual via API. Você não configura o Wyoming diretamente — instalar o complemento Whisper expõe um endpoint Wyoming que o Assist detecta e que você pode selecionar no seu pipeline.' },
  ...
  zh: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        title: '常见问题',
        faqs: [
          { q: 'Wyoming 协议是什么？', a: 'Wyoming 是 Home Assistant 的 Assist 语音管线用来与语音附加组件通信的轻量级网络协议——包括语音转文本（如 Whisper 附加组件）、文本转语音以及唤醒词检测。它让附加组件可以作为独立服务运行，甚至运行在另一台更强大的设备上，并自动向 Assist 注册，而无需手动配置 API 连接。你不需要直接配置 Wyoming——安装 Whisper 附加组件会暴露一个 Wyoming 端点，Assist 会自动检测到它，你可以在管线中选择使用。' },
