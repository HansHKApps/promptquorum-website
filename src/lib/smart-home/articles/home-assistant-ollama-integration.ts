  ...
  en: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        faqs: [
          { q: 'What is Home-LLM (acon96) and how is it different from the Ollama integration?', a: 'Home-LLM ([github.com/acon96/home-llm](https://github.com/acon96/home-llm)) is a separate, community-built HACS custom integration with its own small models fine-tuned specifically for Home Assistant device control, rather than a general-purpose chat model. The Ollama integration this guide covers is the official, built-in path: point Home Assistant at any model Ollama is already serving. Home-LLM is a purpose-built alternative worth evaluating if you want a model trained specifically for structured device commands, but it requires installing a separate HACS component rather than using the integration that ships with Home Assistant.' },
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[System Prompt vs User Prompt: What\'s the Difference](/prompt-engineering/system-prompt-vs-user-prompt-whats-the-difference) — understand the system prompt you configure when wiring the Ollama conversation agent',
        ],
      },
    },
    schema: {
  ...
  ar: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        title: 'أسئلة شائعة',
        faqs: [
          { q: 'ما هو Home-LLM (acon96) وكيف يختلف عن تكامل Ollama؟', a: 'Home-LLM ([github.com/acon96/home-llm](https://github.com/acon96/home-llm)) هو تكامل HACS مجتمعي منفصل بنماذج صغيرة خاصة به مضبوطة دقيقًا خصيصًا للتحكم في أجهزة Home Assistant، وليس نموذج دردشة عام الغرض. أما تكامل Ollama الذي يغطيه هذا الدليل فهو المسار الرسمي المدمج: وجّه Home Assistant إلى أي نموذج يقدّمه Ollama بالفعل. Home-LLM بديل مصمم خصيصًا يستحق التقييم إذا كنت تريد نموذجًا مدربًا خصيصًا على أوامر تحكم منظمة، لكنه يتطلب تثبيت مكوّن HACS منفصل بدلاً من استخدام التكامل المدمج مع Home Assistant.' },
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[الفرق بين System Prompt وUser Prompt](/ar/prompt-engineering/system-prompt-vs-user-prompt-whats-the-difference) — افهم الـ system prompt الذي تضبطه عند إعداد عميل محادثة Ollama',
        ],
      },
    },
    schema: {
  ...
  de: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        faqs: [
          { q: 'Was ist Home-LLM (acon96) und wie unterscheidet es sich von der Ollama-Integration?', a: 'Home-LLM ([github.com/acon96/home-llm](https://github.com/acon96/home-llm)) ist eine separate, von der Community entwickelte HACS-Custom-Integration mit eigenen kleinen Modellen, die speziell für die Gerätesteuerung in Home Assistant feinabgestimmt wurden — kein allgemeines Chat-Modell. Die in diesem Leitfaden behandelte Ollama-Integration ist der offizielle, integrierte Weg: Home Assistant wird auf ein beliebiges, bereits von Ollama bereitgestelltes Modell verwiesen. Home-LLM ist eine zweckgebundene Alternative, die sich lohnt, wenn Sie ein speziell auf strukturierte Gerätebefehle trainiertes Modell möchten — erfordert aber die Installation einer separaten HACS-Komponente statt der mitgelieferten Integration.' },
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[System Prompt vs. User Prompt: der Unterschied](/de/prompt-engineering/system-prompt-vs-user-prompt-whats-the-difference) – verstehen Sie den System Prompt, den Sie beim Einrichten des Ollama-Konversations-Agenten konfigurieren',
        ],
      },
    },
    schema: {
  ...
  es: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        faqs: [
          { q: '¿Qué es Home-LLM (acon96) y en qué se diferencia de la integración de Ollama?', a: 'Home-LLM ([github.com/acon96/home-llm](https://github.com/acon96/home-llm)) es una integración HACS independiente, creada por la comunidad, con sus propios modelos pequeños ajustados específicamente para el control de dispositivos en Home Assistant, no un modelo de chat de propósito general. La integración de Ollama que cubre esta guía es la vía oficial e integrada: apunta Home Assistant a cualquier modelo que Ollama ya esté sirviendo. Home-LLM es una alternativa especializada que vale la pena evaluar si quieres un modelo entrenado específicamente para comandos de control estructurados, pero requiere instalar un componente HACS aparte en lugar de usar la integración incluida.' },
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[System Prompt vs User Prompt: la diferencia](/es/prompt-engineering/system-prompt-vs-user-prompt-whats-the-difference) — comprende el system prompt que configuras al instalar el agente de conversación de Ollama',
        ],
      },
    },
    schema: {
  ...
  ko: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        title: '자주 묻는 질문',
        faqs: [
          { q: 'Home-LLM(acon96)이란 무엇이며 Ollama 통합과 어떻게 다릅니까?', a: 'Home-LLM([github.com/acon96/home-llm](https://github.com/acon96/home-llm))은 범용 채팅 모델이 아니라 Home Assistant 기기 제어에 특화되어 파인튜닝된 자체 소형 모델을 갖춘 별도의 커뮤니티 제작 HACS 커스텀 통합입니다. 이 가이드에서 다루는 Ollama 통합은 공식 내장 방식으로, Home Assistant를 Ollama가 이미 서빙 중인 모델로 연결합니다. Home-LLM은 구조화된 기기 명령을 위해 특별히 학습된 모델을 원한다면 검토할 가치가 있는 전용 대안이지만, 기본 제공 통합 대신 별도의 HACS 구성 요소를 설치해야 합니다.' },
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[시스템 프롬프트 vs 사용자 프롬프트: 차이점](/ko/prompt-engineering/system-prompt-vs-user-prompt-whats-the-difference) — Ollama 대화 에이전트를 설정할 때 구성하는 시스템 프롬프트 이해',
        ],
      },
    },
    schema: {
  ...
  fr: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        faqs: [
          { q: "Qu'est-ce que Home-LLM (acon96) et en quoi diffère-t-il de l'intégration Ollama ?", a: "Home-LLM ([github.com/acon96/home-llm](https://github.com/acon96/home-llm)) est une intégration HACS communautaire distincte, dotée de ses propres petits modèles finement ajustés spécifiquement pour le contrôle des appareils Home Assistant, plutôt qu'un modèle de chat généraliste. L'intégration Ollama couverte par ce guide est la voie officielle et intégrée : elle pointe Home Assistant vers n'importe quel modèle déjà servi par Ollama. Home-LLM est une alternative spécialisée qui mérite d'être évaluée si vous voulez un modèle entraîné spécifiquement pour des commandes d'appareils structurées, mais elle nécessite l'installation d'un composant HACS séparé plutôt que d'utiliser l'intégration livrée avec Home Assistant." },
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[System Prompt vs User Prompt : la différence](/fr/prompt-engineering/system-prompt-vs-user-prompt-whats-the-difference) — comprenez le system prompt que vous configurez lors de la mise en place de l\'agent de conversation Ollama',
        ],
      },
    },
    schema: {
  ...
  ja: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        title: 'よくある質問',
        faqs: [
          { q: 'Home-LLM（acon96）とは何ですか？Ollama連携とどう違いますか？', a: 'Home-LLM（github.com/acon96/home-llm）は、汎用チャットモデルではなく、Home Assistantのデバイス制御専用にファインチューニングされた独自の小型モデルを持つ、コミュニティ製の別のHACSカスタム統合です。本ガイドで扱うOllama連携は公式の組み込み方式で、Ollamaが既に提供しているモデルにHome Assistantを接続します。Home-LLMは、構造化されたデバイスコマンド専用に訓練されたモデルが欲しい場合に検討する価値のある専用の代替手段ですが、標準搭載の連携ではなく別途HACSコンポーネントのインストールが必要です。' },
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[システムプロンプト vs ユーザープロンプト：その違い](/ja/prompt-engineering/system-prompt-vs-user-prompt-whats-the-difference) — Ollama会話エージェント設定時に構成するシステムプロンプトを理解する',
        ],
      },
    },
    schema: {
  ...
  pt: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        faqs: [
          { q: 'O que é o Home-LLM (acon96) e como ele difere da integração Ollama?', a: 'O Home-LLM ([github.com/acon96/home-llm](https://github.com/acon96/home-llm)) é uma integração HACS separada, feita pela comunidade, com seus próprios modelos pequenos ajustados especificamente para o controle de dispositivos no Home Assistant, e não um modelo de chat de propósito geral. A integração Ollama coberta neste guia é o caminho oficial e nativo: aponta o Home Assistant para qualquer modelo que o Ollama já esteja servindo. O Home-LLM é uma alternativa especializada que vale a pena avaliar se você quer um modelo treinado especificamente para comandos de dispositivos estruturados, mas exige instalar um componente HACS separado em vez de usar a integração já incluída.' },
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[System Prompt vs User Prompt: a diferença](/pt/prompt-engineering/system-prompt-vs-user-prompt-whats-the-difference) — entenda o system prompt que você configura ao instalar o agente de conversa do Ollama',
        ],
      },
    },
    schema: {
  ...
  zh: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        title: '常见问题',
        faqs: [
          { q: 'Home-LLM（acon96）是什么？它与Ollama集成有什么区别？', a: 'Home-LLM（github.com/acon96/home-llm）是一个独立的社区维护HACS自定义集成，拥有专为Home Assistant设备控制微调的自有小型模型，而非通用聊天模型。本指南介绍的Ollama集成是官方内置方案：让Home Assistant连接到Ollama已经在提供的任意模型。如果你想要一个专门针对结构化设备指令训练的模型，Home-LLM是值得评估的专用替代方案，但需要安装单独的HACS组件，而不是使用系统自带的集成。' },
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[系统提示词 vs 用户提示词：了解其中区别](/zh/prompt-engineering/system-prompt-vs-user-prompt-whats-the-difference) — 了解配置Ollama对话代理时所设置的系统提示词',
        ],
      },
    },
    schema: {
