  ...
  de: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        faqs: [
          { q: 'Was ist Home-LLM (acon96) und wie unterscheidet es sich von der Ollama-Integration?', a: 'Home-LLM (github.com/acon96/home-llm) ist eine separate, von der Community entwickelte HACS-Custom-Integration mit eigenen kleinen Modellen, die speziell für die Gerätesteuerung in Home Assistant feinabgestimmt wurden — kein allgemeines Chat-Modell. Die in diesem Leitfaden behandelte Ollama-Integration ist der offizielle, integrierte Weg: Home Assistant wird auf ein beliebiges, bereits von Ollama bereitgestelltes Modell verwiesen. Home-LLM ist eine zweckgebundene Alternative, die sich lohnt, wenn Sie ein speziell auf strukturierte Gerätebefehle trainiertes Modell möchten — erfordert aber die Installation einer separaten HACS-Komponente statt der mitgelieferten Integration.' },
  ...
  es: {
    ...
    sections: {
      ...
      faqSection: {
        ...
        faqs: [
          { q: '¿Qué es Home-LLM (acon96) y en qué se diferencia de la integración de Ollama?', a: 'Home-LLM (github.com/acon96/home-llm) es una integración HACS independiente, creada por la comunidad, con sus propios modelos pequeños ajustados específicamente para el control de dispositivos en Home Assistant, no un modelo de chat de propósito general. La integración de Ollama que cubre esta guía es la vía oficial e integrada: apunta Home Assistant a cualquier modelo que Ollama ya esté sirviendo. Home-LLM es una alternativa especializada que vale la pena evaluar si quieres un modelo entrenado específicamente para comandos de control estructurados, pero requiere instalar un componente HACS aparte en lugar de usar la integración incluida.' },
