...
export const article: Partial<Record<Language, LLMArticle>> = {
  en: {
    ...
    intro:
      '[Pocket AI - No Internet](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702), made by developer Faisal at [Dataweal](https://mypocketai.app/), is a free iPhone app that runs open-weight language models directly on the device using [llama.cpp](https://github.com/ggml-org/llama.cpp) with Metal acceleration. There is no account, no server round trip, and once a model is downloaded the app works in airplane mode. The app itself is free; a one-time $4.99 Pro unlock adds on-device Whisper voice input, with no subscription involved. This review covers what the app does, what it costs, what hardware it needs, and — because the developer disclosed it directly and unprompted — what its current App Store rating looks like and why.',
    metaDescription:
      'Pocket AI - No Internet review 2026: free iPhone app for offline, on-device AI chat via llama.cpp, $4.99 one-time Pro unlock for Whisper voice input, no subscription. Pricing, requirements, and developer-disclosed rating context.',
    twitterDescription:
    ...
    leadAnswerBlock:
      '**Pocket AI - No Internet is a free iPhone app that runs open-weight AI models entirely on-device via [llama.cpp](https://github.com/ggml-org/llama.cpp) with Metal acceleration — no account, no server, and full offline use once a model is downloaded.** Made by developer Faisal at [Dataweal](https://mypocketai.app/), the app\'s catalogue spans 28 models across the Qwen, Gemma, Llama, Mistral, Phi, and DeepSeek families, per the developer. The app is free to use for text chat; a one-time $4.99 Pro unlock adds on-device Whisper voice input, with no subscription. It requires an iPhone 12 or newer, which the developer attributes to inference being memory-bound. Readers who want a comparison point should also read the [Private LLM review](/power-local-llm/private-llm-review) or [PocketPal AI review](/power-local-llm/pocketpal-ai-review).',
    quickAnswerTop: {
      en: {
        question: 'Is Pocket AI - No Internet worth downloading?',
        answer:
          'Yes, if you want a free, no-account iPhone app that runs open-weight AI models fully offline via llama.cpp, and you own an iPhone 12 or newer. The $4.99 Pro unlock is optional and only adds on-device Whisper voice input — text chat is free. Skip it if you need a larger, more mature curated model library or Android/desktop support; compare it against Private LLM or PocketPal AI first.',
        bullets: [
          'Free to download and use for text chat; no account or sign-in required.',
          'One-time $4.99 Pro unlock adds on-device Whisper voice dictation — no subscription.',
          ...
          'As of the developer\'s first outreach to PromptQuorum (before September 8, 2026), the App Store listing showed a 2.8 rating from 6 ratings — the developer attributes the low rating count to the app not prompting users to rate it until an in-app rating prompt shipped on September 4, 2026. This is the developer\'s own explanation, not independently verified.',
        ],
    ...
    toc: [
      ...
      { label: 'App Store Rating: What the Developer Disclosed', anchor: 'app-store-rating' },
    ...
    sections: {
      tldr: {
        ...
        snippetBlocks: [
          {
            type: 'one-sentence',
            text: 'Pocket AI - No Internet is a free iPhone app by developer Faisal at Dataweal that runs open-weight AI models (Qwen, Gemma, Llama, Mistral, Phi, DeepSeek) entirely on-device via llama.cpp with Metal acceleration, with a one-time $4.99 Pro unlock for on-device Whisper voice input and no subscription.',
          },
          {
            type: 'plain-terms',
        ...
        items: [
          'Price: free to [download](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) and use for text chat; a one-time $4.99 Pro unlock adds on-device Whisper voice input, with no subscription.',
          ...
          'App Store rating: 2.8 from 6 ratings as of the developer\'s first email to PromptQuorum (before September 8, 2026); the developer attributes the low count to the app not prompting for ratings until an in-app prompt shipped on September 4, 2026 — this is the developer\'s own explanation, not independently verified, and the current rating may differ from this figure.',
        ],
      },
      whatIsPocketAI: {
      ...
      howToGetStarted: {
        ...
        numberedItems: [
          ...
          },
          {
            ...
            whyItMatters: 'A one-time $4.99 in-app purchase unlocks on-device Whisper voice input for dictation, with no subscription. This is optional — text chat works without it.',
          },
        ],
      ...
      fromTheMaker: {
        ...
        content: [
          'PromptQuorum asked Faisal, the developer behind Pocket AI at Dataweal, to describe the app and the reasoning behind its pricing and platform choices. The following is presented as the developer\'s own words, lightly formatted for readability, not as PromptQuorum\'s independent editorial assessment:',
        ],
        blockquote:
        ...
        blockquoteSource: 'Faisal, developer, Dataweal',
      },
      appStoreRating: {
        id: 'app-store-rating',
        title: 'App Store Rating: What the Developer Disclosed',
        content: [
          '**As of the developer\'s first outreach to PromptQuorum, before September 8, 2026, the Pocket AI App Store listing showed a 2.8 rating from 6 ratings.** The developer disclosed this figure directly, unprompted, along with an explanation for it: the app had never prompted users to leave a rating until an in-app rating prompt shipped on September 4, 2026.',
          'This is the developer\'s own account of why the rating count and average look the way they do — PromptQuorum has not independently verified the cause, and a rating average based on 6 ratings is a very small sample that can move sharply with just one or two additional reviews in either direction. Readers should check the [current App Store listing](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) directly for the up-to-date rating and count, since both will have changed since this figure was recorded.',
          'PromptQuorum is including this figure specifically because the developer volunteered it rather than because it was requested — a low rating count with an explained cause is more useful context for readers than no rating information at all, provided it is clearly labeled as developer-attributed rather than independently confirmed.',
        ],
      },
      tradeOffs: {
        id: 'tradeoffs',
        ...
        rows: [
          {
            ...
            'Limitation / caveat': 'Voice input requires the $4.99 Pro unlock; it is not included free.',
          },
          {
            'Benefit': 'One-time $4.99 Pro unlock',
        ...
        rows: [
          {
            ...
            'Price': 'Free; $4.99 one-time Pro unlock for voice input',
      ...
      whoShouldUse: {
        ...
        items: [
          ...
          '**Readers who specifically want to avoid a subscription.** Both the free tier and the $4.99 Pro unlock are one-time — there is no recurring charge either way.',
          '**Users who want on-device voice input without a subscription.** The Pro unlock is a single $4.99 purchase for Whisper-based dictation that the developer states runs locally.',
      ...
      whoShouldNotUse: {
        ...
        items: [
          ...
          '**Readers who need an established track record before trusting an app.** Pocket AI is newer to the App Store than alternatives like Private LLM, and as of the developer\'s own disclosure had only 6 ratings before September 8, 2026 — readers who weight review volume heavily should factor that in.',
        ...
        faqs: [
          {
            ...
            a: 'Yes. The app is free to download and use for text chat with any model in its catalogue. A one-time $4.99 in-app purchase unlocks on-device Whisper voice input; there is no subscription for either tier.',
          },
          {
          ...
          {
            q: 'What does the $4.99 Pro unlock include?',
            a: 'The one-time $4.99 Pro unlock adds on-device Whisper voice input for dictation. It does not add extra models or other features beyond voice input, per the developer\'s own description. There is no subscription.',
          },
          {
            q: 'Why does Pocket AI have such a low App Store rating?',
            a: 'As of the developer\'s first outreach to PromptQuorum, before September 8, 2026, the App Store listing showed a 2.8 rating from 6 ratings. The developer attributes this to the app not prompting users to leave a rating until an in-app rating prompt shipped on September 4, 2026. This is the developer\'s own explanation, not independently verified by PromptQuorum, and the current rating may have changed — check the [App Store listing](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) directly for the up-to-date figure.',
          },
          {
      ...
      verdict: {
        ...
        title: 'Verdict',
        content:
          'Pocket AI - No Internet delivers on a straightforward promise: a free iPhone app that runs open-weight models entirely on-device via llama.cpp, with no account and no server round trip, and a genuinely optional one-time $4.99 unlock for voice input rather than a subscription. The 28-model catalogue across the Qwen, Gemma, Llama, Mistral, Phi, and DeepSeek families is a reasonable starting point for a free app, and the developer\'s own reasoning for the iPhone 12 minimum — memory-bound inference — is a plausible, specific explanation rather than a vague hardware requirement. What keeps this review cautious rather than enthusiastic is the app\'s youth: it is closed-source, so privacy and offline claims rest on the developer\'s own word rather than independent code review, and the developer\'s own disclosure of a 2.8 rating from just 6 ratings (before September 8, 2026) reflects a track record that is still being built, however well-explained the low rating count is. Readers who want a free, no-subscription, on-device chat app with a plausible privacy story should try it; readers who weight an established App Store track record and independently auditable source code more heavily should start with [PocketPal AI](/power-local-llm/pocketpal-ai-review) or budget for [Private LLM](/power-local-llm/private-llm-review) instead.',
      },
      sources: {
        id: 'sources',
        title: 'Sources',
        items: [
          '[Pocket AI - No Internet on the App Store](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) — price, platform compatibility, and rating.',
          '[Pocket AI official site](https://mypocketai.app/) — product overview.',
          'Direct correspondence with Faisal, developer at Dataweal, provided to PromptQuorum before September 8, 2026 — app description, model catalogue count, pricing model, hardware requirement reasoning, open-source status, and App Store rating context.',
        ],
      },
      relatedReading: {
        id: 'related-reading',
  ...
  de: {
    ...
    intro:
      '[Pocket AI - No Internet](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702), entwickelt von Faisal bei [Dataweal](https://mypocketai.app/), ist eine kostenlose iPhone-App, die Open-Weight-Sprachmodelle direkt auf dem Gerät über [llama.cpp](https://github.com/ggml-org/llama.cpp) mit Metal-Beschleunigung ausführt. Es gibt kein Konto und keine Kommunikation mit einem Server, und sobald ein Modell heruntergeladen ist, funktioniert die App im Flugmodus. Die App selbst ist kostenlos; ein einmaliger Kauf für 4,99 $ schaltet als "Pro"-Funktion Spracheingabe per On-Device-Whisper frei — ohne Abonnement. Dieser Test behandelt, was die App leistet, was sie kostet, welche Hardware sie benötigt und — weil der Entwickler es direkt und unaufgefordert mitgeteilt hat — wie die aktuelle App-Store-Bewertung aussieht und warum.',
    metaDescription:
      'Pocket AI - No Internet Test 2026: kostenlose iPhone-App für Offline-KI-Chat auf dem Gerät via llama.cpp, 4,99 $ Einmalkauf für Whisper-Spracheingabe, kein Abo. Preise, Anforderungen und vom Entwickler offengelegter Bewertungskontext.',
    twitterDescription:
    ...
    quickAnswerTop: {
      ...
      de: {
        ...
        bullets: [
          ...
          'Zum Zeitpunkt der ersten Kontaktaufnahme des Entwicklers mit PromptQuorum (vor dem 8. September 2026) zeigte der App-Store-Eintrag eine Bewertung von 2,8 aus 6 Bewertungen — der Entwickler führt die niedrige Bewertungszahl darauf zurück, dass die App erst seit einem In-App-Bewertungshinweis am 4. September 2026 überhaupt nach Bewertungen fragt. Dies ist die eigene Erklärung des Entwicklers, nicht unabhängig verifiziert.',
        ],
        updatedDate: '2026-09-11',
      },
    },
    toc: [
      ...
      { label: 'App-Store-Bewertung: Was der Entwickler offengelegt hat', anchor: 'app-store-rating' },
    ...
    sections: {
      tldr: {
        ...
        snippetBlocks: [
          {
            type: 'one-sentence',
            text: 'Pocket AI - No Internet ist eine kostenlose iPhone-App von Entwickler Faisal bei Dataweal, die Open-Weight-KI-Modelle (Qwen, Gemma, Llama, Mistral, Phi, DeepSeek) vollständig auf dem Gerät über llama.cpp mit Metal-Beschleunigung ausführt, mit einem einmaligen 4,99-$-Pro-Kauf für On-Device-Whisper-Spracheingabe und ohne Abonnement.',
          },
          {
            type: 'plain-terms',
        ...
        items: [
          'Preis: kostenlos zum [Herunterladen](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) und für Text-Chat nutzbar; ein einmaliger 4,99-$-Pro-Kauf schaltet On-Device-Whisper-Spracheingabe frei, ohne Abonnement.',
          ...
          'App-Store-Bewertung: 2,8 aus 6 Bewertungen zum Zeitpunkt der ersten E-Mail des Entwicklers an PromptQuorum (vor dem 8. September 2026); der Entwickler führt die niedrige Zahl darauf zurück, dass die App erst seit einem In-App-Bewertungshinweis am 4. September 2026 nach Bewertungen fragt — dies ist die eigene Erklärung des Entwicklers, nicht unabhängig verifiziert, und die aktuelle Bewertung kann abweichen.',
        ],
      },
      whatIsPocketAI: {
        id: 'what-is-pocket-ai',
      ...
      howToGetStarted: {
        ...
        numberedItems: [
          ...
          },
          {
            ...
            whyItMatters: 'Ein einmaliger In-App-Kauf für 4,99 $ schaltet On-Device-Whisper-Spracheingabe für Diktate frei, ohne Abonnement. Dies ist optional — Text-Chat funktioniert auch ohne.',
          },
        ],
      ...
      fromTheMaker: {
        ...
        blockquoteSource: 'Faisal, Entwickler, Dataweal',
      },
      appStoreRating: {
        id: 'app-store-rating',
        title: 'App-Store-Bewertung: Was der Entwickler offengelegt hat',
        content: [
          '**Zum Zeitpunkt der ersten Kontaktaufnahme des Entwicklers mit PromptQuorum, vor dem 8. September 2026, zeigte der App-Store-Eintrag von Pocket AI eine Bewertung von 2,8 aus 6 Bewertungen.** Der Entwickler legte diese Zahl direkt und unaufgefordert offen, zusammen mit einer Erklärung dafür: Die App hatte Nutzer nie zu einer Bewertung aufgefordert, bis ein In-App-Bewertungshinweis am 4. September 2026 eingeführt wurde.',
          'Dies ist die eigene Darstellung des Entwicklers, warum Bewertungszahl und Durchschnitt so aussehen — PromptQuorum hat die Ursache nicht unabhängig überprüft, und ein Bewertungsdurchschnitt auf Basis von 6 Bewertungen ist eine sehr kleine Stichprobe, die sich mit nur ein oder zwei weiteren Bewertungen in beide Richtungen stark verschieben kann. Leser sollten den [aktuellen App-Store-Eintrag](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) direkt prüfen, um die aktuelle Bewertung und Anzahl zu sehen, da sich beide seit der Erfassung dieser Zahl geändert haben dürften.',
          'PromptQuorum nennt diese Zahl konkret, weil der Entwickler sie freiwillig mitgeteilt hat und nicht, weil sie angefragt wurde — eine niedrige Bewertungszahl mit erklärter Ursache ist für Leser hilfreicher als gar keine Bewertungsinformation, sofern sie klar als vom Entwickler stammend gekennzeichnet ist und nicht als unabhängig bestätigt.',
        ],
      },
      tradeOffs: {
        id: 'tradeoffs',
        ...
        rows: [
          {
            ...
            'Einschränkung / Hinweis': 'Spracheingabe erfordert den 4,99-$-Pro-Kauf; sie ist nicht kostenlos enthalten.',
          },
          {
            'Vorteil': 'Einmaliger 4,99-$-Pro-Kauf',
        ...
        rows: [
          {
            ...
            'Preis': 'Kostenlos; 4,99 $ einmaliger Pro-Kauf für Spracheingabe',
      ...
      whoShouldUse: {
        ...
        items: [
          ...
          '**Leser, die gezielt ein Abonnement vermeiden möchten.** Sowohl die kostenlose Stufe als auch der 4,99-$-Pro-Kauf sind einmalig — in beiden Fällen keine wiederkehrende Kosten.',
          '**Nutzer, die Spracheingabe auf dem Gerät ohne Abonnement möchten.** Der Pro-Kauf ist ein einmaliger Kauf für 4,99 $ für Whisper-basiertes Diktat, das laut Entwickler lokal läuft.',
      ...
      whoShouldNotUse: {
        ...
        items: [
          ...
          '**Leser, die vor dem Vertrauen in eine App eine etablierte Erfolgsbilanz benötigen.** Pocket AI ist neuer im App Store als Alternativen wie Private LLM und hatte laut eigener Offenlegung des Entwicklers vor dem 8. September 2026 nur 6 Bewertungen — Leser, die viel Wert auf die Bewertungsmenge legen, sollten das berücksichtigen.',
        ...
        faqs: [
          {
            ...
            a: 'Ja. Die App ist kostenlos zum Herunterladen und für Text-Chat mit jedem Modell der Bibliothek nutzbar. Ein einmaliger In-App-Kauf für 4,99 $ schaltet On-Device-Whisper-Spracheingabe frei; für keine der beiden Stufen gibt es ein Abonnement.',
          },
          {
          ...
          {
            q: 'Was ist im 4,99-$-Pro-Kauf enthalten?',
            a: 'Der einmalige 4,99-$-Pro-Kauf schaltet On-Device-Whisper-Spracheingabe für Diktate frei. Laut eigener Beschreibung des Entwicklers fügt er keine weiteren Modelle oder Funktionen über die Spracheingabe hinaus hinzu. Es gibt kein Abonnement.',
          },
          {
            q: 'Warum hat Pocket AI eine so niedrige App-Store-Bewertung?',
            a: 'Zum Zeitpunkt der ersten Kontaktaufnahme des Entwicklers mit PromptQuorum, vor dem 8. September 2026, zeigte der App-Store-Eintrag eine Bewertung von 2,8 aus 6 Bewertungen. Der Entwickler führt dies darauf zurück, dass die App Nutzer erst seit einem In-App-Bewertungshinweis am 4. September 2026 zur Bewertung aufforderte. Dies ist die eigene Erklärung des Entwicklers, nicht unabhängig von PromptQuorum verifiziert, und die aktuelle Bewertung kann sich geändert haben — prüfen Sie den [App-Store-Eintrag](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) direkt für die aktuelle Zahl.',
          },
          {
          ...
          },
          {
            ...
            a: 'Pocket AI ist kostenlos mit optionalem einmaligem 4,99-$-Sprach-Unlock, quellcodegeschlossen und neuer im App Store. [Private LLM](/de/power-local-llm/private-llm-review) ist von Anfang an ein 4,99-$-Einmalkauf mit größerer, etablierterer Bibliothek von 140+ Modellen. [PocketPal AI](/de/power-local-llm/pocketpal-ai-review) ist kostenlos und Open Source, erfordert aber das manuelle Beschaffen von GGUF-Modelldateien. Wählen Sie danach, ob Ihnen Kosten, Größe und Reife der Modellbibliothek oder Quellcode-Transparenz am wichtigsten sind.',
          },
        ],
      },
      verdict: {
        ...
        title: 'Fazit',
        content:
          'Pocket AI - No Internet hält ein einfaches Versprechen: eine kostenlose iPhone-App, die Open-Weight-Modelle vollständig auf dem Gerät über llama.cpp ausführt, ohne Konto und ohne Serverkommunikation, mit einem echt optionalen einmaligen 4,99-$-Unlock für Spracheingabe statt eines Abonnements. Die 28-Modell-Bibliothek über die Familien Qwen, Gemma, Llama, Mistral, Phi und DeepSeek ist für eine kostenlose App ein vernünftiger Ausgangspunkt, und die eigene Begründung des Entwicklers für das iPhone-12-Minimum — speicherlimitierte Inferenz — ist eine plausible, konkrete Erklärung statt einer vagen Hardware-Anforderung. Was diesen Test eher vorsichtig als begeistert hält, ist das junge Alter der App: Sie ist quellcodegeschlossen, sodass Datenschutz- und Offline-Angaben auf dem Wort des Entwicklers beruhen und nicht auf unabhängiger Code-Prüfung, und die eigene Offenlegung des Entwicklers einer Bewertung von 2,8 aus nur 6 Bewertungen (vor dem 8. September 2026) spiegelt eine Erfolgsbilanz wider, die noch aufgebaut wird — so gut die niedrige Bewertungszahl auch erklärt ist. Leser, die eine kostenlose Chat-App ohne Abonnement mit plausibler Datenschutz-Story auf dem Gerät möchten, sollten sie ausprobieren; Leser, denen eine etablierte App-Store-Erfolgsbilanz und unabhängig prüfbarer Quellcode wichtiger sind, sollten stattdessen mit [PocketPal AI](/de/power-local-llm/pocketpal-ai-review) beginnen oder für [Private LLM](/de/power-local-llm/private-llm-review) budgetieren.',
      },
      sources: {
        id: 'sources',
        title: 'Quellen',
        items: [
          '[Pocket AI - No Internet im App Store](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) — Preis, Plattformkompatibilität und Bewertung.',
          '[Offizielle Pocket-AI-Website](https://mypocketai.app/) — Produktübersicht.',
          'Direkte Korrespondenz mit Faisal, Entwickler bei Dataweal, PromptQuorum vor dem 8. September 2026 mitgeteilt — App-Beschreibung, Anzahl der Modellbibliothek, Preismodell, Begründung der Hardware-Anforderung, Open-Source-Status und Kontext zur App-Store-Bewertung.',
        ],
      },
      relatedReading: {
        id: 'related-reading',
  ...
  fr: {
    ...
    intro:
      '[Pocket AI - No Internet](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702), développée par Faisal chez [Dataweal](https://mypocketai.app/), est une application iPhone gratuite qui exécute des modèles de langage à poids ouverts directement sur l\'appareil via [llama.cpp](https://github.com/ggml-org/llama.cpp) avec accélération Metal. Aucun compte, aucun aller-retour vers un serveur, et une fois qu\'un modèle est téléchargé, l\'application fonctionne en mode avion. L\'application elle-même est gratuite ; un déblocage Pro à 4,99 $ payable une seule fois ajoute la saisie vocale Whisper sur l\'appareil, sans abonnement. Cet avis couvre ce que fait l\'application, son coût, la configuration matérielle requise et — parce que le développeur l\'a communiqué directement et spontanément — l\'état actuel de sa note sur l\'App Store et pourquoi.',
    metaDescription:
      'Avis Pocket AI - No Internet 2026 : application iPhone gratuite pour chat IA hors ligne sur l\'appareil via llama.cpp, déblocage Pro à 4,99 $ payable une fois pour la saisie vocale Whisper, sans abonnement. Prix, configuration requise et contexte de notation communiqué par le développeur.',
    twitterDescription:
    ...
    quickAnswerTop: {
      ...
      fr: {
        ...
        bullets: [
          ...
          'Au moment du premier contact du développeur avec PromptQuorum (avant le 8 septembre 2026), la fiche App Store affichait une note de 2,8 sur 6 avis — le développeur attribue ce faible nombre d\'avis au fait que l\'application n\'a jamais invité les utilisateurs à noter avant qu\'une invite intégrée ne soit déployée le 4 septembre 2026. C\'est l\'explication du développeur lui-même, non vérifiée indépendamment.',
        ],
        updatedDate: '2026-09-11',
      },
    },
    toc: [
      ...
      { label: 'Note App Store : ce que le développeur a révélé', anchor: 'app-store-rating' },
    ...
    sections: {
      tldr: {
        ...
        snippetBlocks: [
          {
            type: 'one-sentence',
            text: 'Pocket AI - No Internet est une application iPhone gratuite développée par Faisal chez Dataweal qui exécute des modèles d\'IA à poids ouverts (Qwen, Gemma, Llama, Mistral, Phi, DeepSeek) entièrement sur l\'appareil via llama.cpp avec accélération Metal, avec un déblocage Pro à 4,99 $ (paiement unique) pour la saisie vocale Whisper sur l\'appareil et sans abonnement.',
          },
          {
            type: 'plain-terms',
        ...
        items: [
          'Prix : gratuite à [télécharger](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) et à utiliser pour le chat textuel ; un déblocage Pro à 4,99 $ (paiement unique) ajoute la saisie vocale Whisper sur l\'appareil, sans abonnement.',
          ...
          'Note App Store : 2,8 sur 6 avis au moment du premier e-mail du développeur à PromptQuorum (avant le 8 septembre 2026) ; le développeur attribue ce faible nombre au fait que l\'application n\'invitait pas à noter avant une invite intégrée déployée le 4 septembre 2026 — c\'est l\'explication du développeur lui-même, non vérifiée indépendamment, et la note actuelle peut différer de ce chiffre.',
        ],
      },
      whatIsPocketAI: {
        id: 'what-is-pocket-ai',
      ...
      howToGetStarted: {
        ...
        numberedItems: [
          ...
          },
          {
            ...
            whyItMatters: 'Un achat intégré unique de 4,99 $ débloque la saisie vocale Whisper sur l\'appareil pour la dictée, sans abonnement. C\'est facultatif — le chat textuel fonctionne sans cela.',
          },
        ],
      ...
      fromTheMaker: {
        ...
        content: [
          'PromptQuorum a demandé à Faisal, le développeur de Pocket AI chez Dataweal, de décrire l\'application et le raisonnement derrière ses choix de prix et de plateforme. Ce qui suit est présenté comme les propres mots du développeur, légèrement mis en forme pour la lisibilité, et non comme une évaluation éditoriale indépendante de PromptQuorum :',
        ],
        blockquote:
        ...
        blockquoteSource: 'Faisal, développeur, Dataweal',
      },
      appStoreRating: {
        id: 'app-store-rating',
        title: 'Note App Store : ce que le développeur a révélé',
        content: [
          '**Au moment du premier contact du développeur avec PromptQuorum, avant le 8 septembre 2026, la fiche App Store de Pocket AI affichait une note de 2,8 sur 6 avis.** Le développeur a révélé ce chiffre directement, sans qu\'on le lui demande, accompagné d\'une explication : l\'application n\'avait jamais invité les utilisateurs à laisser un avis avant qu\'une invite intégrée ne soit déployée le 4 septembre 2026.',
          'C\'est la propre explication du développeur quant à l\'apparence du nombre d\'avis et de la moyenne — PromptQuorum n\'a pas vérifié cette cause de manière indépendante, et une moyenne basée sur 6 avis est un échantillon très restreint qui peut fortement évoluer avec seulement un ou deux avis supplémentaires dans un sens ou dans l\'autre. Les lecteurs devraient consulter directement la [fiche App Store actuelle](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) pour la note et le nombre d\'avis à jour, car les deux auront probablement changé depuis l\'enregistrement de ce chiffre.',
          'PromptQuorum inclut ce chiffre spécifiquement parce que le développeur l\'a communiqué volontairement, et non parce qu\'il a été demandé — un faible nombre d\'avis avec une cause expliquée est un contexte plus utile pour les lecteurs que l\'absence totale d\'information sur les notes, à condition qu\'il soit clairement présenté comme attribué au développeur plutôt que confirmé indépendamment.',
        ],
      },
      tradeOffs: {
        id: 'tradeoffs',
        ...
        rows: [
          {
            ...
            'Limite / réserve': 'La saisie vocale nécessite le déblocage Pro à 4,99 $ ; elle n\'est pas incluse gratuitement.',
          },
          {
            'Avantage': 'Déblocage Pro unique à 4,99 $',
        ...
        rows: [
          {
            ...
            'Prix': 'Gratuit ; déblocage Pro unique à 4,99 $ pour la saisie vocale',
      ...
      whoShouldUse: {
        ...
        items: [
          ...
          '**Les lecteurs qui veulent spécifiquement éviter un abonnement.** Le palier gratuit comme le déblocage Pro à 4,99 $ sont des paiements uniques — aucun coût récurrent dans un cas comme dans l\'autre.',
          '**Les utilisateurs qui veulent une saisie vocale sur l\'appareil sans abonnement.** Le déblocage Pro est un achat unique de 4,99 $ pour une dictée basée sur Whisper que le développeur déclare s\'exécuter localement.',
      ...
      whoShouldNotUse: {
        ...
        items: [
          ...
          '**Les lecteurs qui ont besoin d\'un historique établi avant de faire confiance à une application.** Pocket AI est plus récente sur l\'App Store que des alternatives comme Private LLM et, selon la propre divulgation du développeur, n\'avait que 6 avis avant le 8 septembre 2026 — les lecteurs qui accordent beaucoup de poids au volume d\'avis devraient en tenir compte.',
        ...
        faqs: [
          {
            ...
            a: 'Oui. L\'application est gratuite à télécharger et à utiliser pour le chat textuel avec n\'importe quel modèle de son catalogue. Un achat intégré unique de 4,99 $ débloque la saisie vocale Whisper sur l\'appareil ; aucun des deux paliers ne comporte d\'abonnement.',
          },
          {
          ...
          {
            q: 'Que comprend le déblocage Pro à 4,99 $ ?',
            a: 'Le déblocage Pro unique à 4,99 $ ajoute la saisie vocale Whisper sur l\'appareil pour la dictée. Il n\'ajoute pas de modèles ou de fonctionnalités supplémentaires au-delà de la saisie vocale, selon la propre description du développeur. Il n\'y a pas d\'abonnement.',
          },
          {
            q: 'Pourquoi Pocket AI a-t-elle une note App Store aussi basse ?',
            a: 'Au moment du premier contact du développeur avec PromptQuorum, avant le 8 septembre 2026, la fiche App Store affichait une note de 2,8 sur 6 avis. Le développeur attribue cela au fait que l\'application n\'invitait pas les utilisateurs à laisser un avis avant qu\'une invite intégrée ne soit déployée le 4 septembre 2026. C\'est l\'explication propre du développeur, non vérifiée indépendamment par PromptQuorum, et la note actuelle peut avoir changé — consultez directement la [fiche App Store](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) pour le chiffre à jour.',
          },
          {
      ...
      verdict: {
        ...
        title: 'Verdict',
        content:
          'Pocket AI - No Internet tient une promesse simple : une application iPhone gratuite qui exécute des modèles à poids ouverts entièrement sur l\'appareil via llama.cpp, sans compte ni aller-retour serveur, avec un déblocage vocal réellement facultatif à 4,99 $ (paiement unique) plutôt qu\'un abonnement. Le catalogue de 28 modèles couvrant les familles Qwen, Gemma, Llama, Mistral, Phi et DeepSeek est un point de départ raisonnable pour une application gratuite, et le raisonnement propre du développeur pour le minimum iPhone 12 — l\'inférence limitée par la mémoire — est une explication plausible et précise plutôt qu\'une exigence matérielle vague. Ce qui rend cet avis prudent plutôt qu\'enthousiaste, c\'est la jeunesse de l\'application : elle est à code source fermé, de sorte que les affirmations sur la confidentialité et le hors-ligne reposent sur la parole du développeur plutôt que sur un examen de code indépendant, et la propre divulgation du développeur d\'une note de 2,8 sur seulement 6 avis (avant le 8 septembre 2026) reflète un historique encore en construction, aussi bien expliqué que soit ce faible nombre d\'avis. Les lecteurs qui veulent une application de chat gratuite, sans abonnement, avec une histoire de confidentialité sur l\'appareil plausible devraient l\'essayer ; les lecteurs qui accordent plus de poids à un historique App Store établi et à un code source auditable de manière indépendante devraient plutôt commencer par [PocketPal AI](/fr/power-local-llm/pocketpal-ai-review) ou prévoir un budget pour [Private LLM](/fr/power-local-llm/private-llm-review).',
      },
      sources: {
        id: 'sources',
        title: 'Sources',
        items: [
          '[Pocket AI - No Internet sur l\'App Store](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) — prix, compatibilité des plateformes et note.',
          '[Site officiel de Pocket AI](https://mypocketai.app/) — présentation du produit.',
          'Correspondance directe avec Faisal, développeur chez Dataweal, transmise à PromptQuorum avant le 8 septembre 2026 — description de l\'application, nombre de modèles au catalogue, modèle de tarification, raisonnement sur la configuration matérielle requise, statut open source et contexte de la note App Store.',
        ],
      },
      relatedReading: {
        id: 'related-reading',
  ...
  ja: {
    ...
    intro:
      '[Pocket AI - No Internet](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702)は、開発者Faisal氏が[Dataweal](https://mypocketai.app/)で開発した無料のiPhoneアプリで、オープンウェイトの言語モデルを[llama.cpp](https://github.com/ggml-org/llama.cpp)とMetalアクセラレーションによって端末上で直接実行します。アカウントは不要で、サーバーとの通信も発生せず、モデルをダウンロードすればアプリは機内モードでも動作します。アプリ自体は無料で、1回限り4.99ドルのPro解除でオンデバイスのWhisper音声入力が追加されますが、サブスクリプションはありません。本レビューでは、アプリの機能、料金、必要なハードウェア、そして開発者が自ら開示した現在のApp Storeの評価とその理由について扱います。',
    metaDescription:
      'Pocket AI - No Internetレビュー2026:llama.cpp経由でオフラインのオンデバイスAIチャットを実現する無料iPhoneアプリ。Whisper音声入力のための4.99ドルの1回限りPro解除、サブスクリプションなし。料金、要件、開発者が開示した評価の背景。',
    twitterDescription:
    ...
    current_hardware_mentioned: ['iPhone 12'],
    leadAnswerBlock:
      '**Pocket AI - No Internetは、[llama.cpp](https://github.com/ggml-org/llama.cpp)とMetalアクセラレーションによってオープンウェイトのAIモデルを完全に端末上で実行する無料のiPhoneアプリです——アカウント不要、サーバー不要で、モデルをダウンロードすれば完全オフラインで利用できます。** [Dataweal](https://mypocketai.app/)の開発者Faisal氏によって作られ、開発者によればカタログはQwen、Gemma、Llama、Mistral、Phi、DeepSeekの各ファミリーにまたがる28モデルに及びます。テキストチャットは無料で利用でき、1回限り4.99ドルのPro解除でオンデバイスのWhisper音声入力が追加されます(サブスクリプションなし)。推論がメモリに制約されるため、開発者はiPhone 12以降が必要としています。比較のためには[Private LLMレビュー](/ja/power-local-llm/private-llm-review)や[PocketPal AIレビュー](/ja/power-local-llm/pocketpal-ai-review)もご覧ください。',
    quickAnswerTop: {
      en: {
        question: 'Is Pocket AI - No Internet worth downloading?',
        answer:
      ...
      ja: {
        ...
        answer:
          'はい、llama.cpp経由でオープンウェイトのAIモデルを完全にオフラインで実行できる、無料でアカウント不要のiPhoneアプリを求めていて、iPhone 12以降を所有している場合はおすすめです。4.99ドルのPro解除は任意で、オンデバイスのWhisper音声入力を追加するだけです——テキストチャットは無料です。より大規模で成熟したモデルライブラリやAndroid/デスクトップ対応が必要な場合は見送り、まずPrivate LLMやPocketPal AIと比較してください。',
        bullets: [
          '無料でダウンロードでき、テキストチャットに利用可能。アカウントやサインインは不要。',
          '1回限り4.99ドルのPro解除でオンデバイスのWhisper音声入力が追加される——サブスクリプションなし。',
          ...
          'PromptQuorumへの開発者からの最初の連絡時点(2026年9月8日より前)で、App Storeの評価は6件のレビューで2.8だった——開発者は、2026年9月4日にアプリ内評価プロンプトが導入されるまでアプリがユーザーに評価を促していなかったことがレビュー数の少なさの原因だとしている。これは開発者自身の説明であり、独立して検証されたものではない。',
        ],
        updatedDate: '2026-09-11',
      },
    },
    toc: [
      ...
      { label: 'App Storeの評価:開発者が開示した内容', anchor: 'app-store-rating' },
    ...
    sections: {
      tldr: {
        ...
        snippetBlocks: [
          {
            type: 'one-sentence',
            text: 'Pocket AI - No Internetは、Datawealの開発者Faisal氏による無料のiPhoneアプリで、オープンウェイトAIモデル(Qwen、Gemma、Llama、Mistral、Phi、DeepSeek)をllama.cppとMetalアクセラレーションによって完全に端末上で実行し、オンデバイスWhisper音声入力のための1回限り4.99ドルのPro解除があり、サブスクリプションはない。',
          },
          {
            type: 'plain-terms',
        ...
        items: [
          '料金:[ダウンロード](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702)は無料でテキストチャットに利用可能。1回限り4.99ドルのPro解除でオンデバイスWhisper音声入力が追加(サブスクリプションなし)。',
          ...
          'App Store評価:PromptQuorumへの開発者からの最初のメール時点(2026年9月8日より前)で6件のレビューによる2.8の評価。開発者は、2026年9月4日にアプリ内評価プロンプトが導入されるまで評価を促していなかったことがこの少なさの原因だとしている——これは開発者自身の説明であり独立検証はされておらず、現在の評価はこの数値と異なる可能性がある。',
        ],
      },
      whatIsPocketAI: {
        id: 'what-is-pocket-ai',
      ...
      howToGetStarted: {
        ...
        numberedItems: [
          ...
          },
          {
            ...
            whyItMatters: '1回限り4.99ドルのアプリ内購入で、口述筆記のためのオンデバイスWhisper音声入力が解除されます(サブスクリプションなし)。これは任意であり、テキストチャットはこれがなくても機能します。',
          },
        ],
      ...
      fromTheMaker: {
        ...
        title: '開発者から',
        content: [
          'PromptQuorumは、DatawealでPocket AIを開発しているFaisal氏に、アプリの内容と価格・プラットフォーム選択の背景にある考え方を説明するよう依頼しました。以下は、読みやすさのために軽く整形した開発者自身の言葉として提示するものであり、PromptQuorumによる独立した編集上の評価ではありません:',
        ],
        blockquote:
        ...
        blockquoteSource: 'Faisal氏、開発者、Dataweal',
      },
      appStoreRating: {
        id: 'app-store-rating',
        title: 'App Storeの評価:開発者が開示した内容',
        content: [
          '**PromptQuorumへの開発者からの最初の連絡時点、2026年9月8日より前において、Pocket AIのApp Store掲載情報は6件のレビューで2.8の評価を示していました。** 開発者はこの数値を、求められることなく自ら直接開示し、その理由も説明しました。2026年9月4日にアプリ内評価プロンプトが導入されるまで、アプリはユーザーに評価を求めたことがなかったというものです。',
          'これは、レビュー数と平均値がなぜそのように見えるかについての開発者自身の説明であり、PromptQuorumはその原因を独立して検証していません。また、6件のレビューに基づく評価平均は非常に小さなサンプルであり、あと1件か2件のレビューが加わるだけでも大きく変動し得ます。読者は、この数値が記録されて以降、両方とも変化している可能性が高いため、最新の評価と件数について直接[現在のApp Store掲載情報](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702)を確認すべきです。',
          'PromptQuorumがこの数値を掲載しているのは、それが求められたからではなく開発者が自ら進んで提供したためです——理由が説明された低いレビュー件数は、それが独立して確認されたものではなく開発者に帰属するものであると明確に示されている限り、評価情報が全くないよりも読者にとって有用な文脈となります。',
        ],
      },
      tradeOffs: {
        id: 'tradeoffs',
        ...
        rows: [
          {
            ...
            '制約・注意点': '音声入力には4.99ドルのPro解除が必要で、無料では含まれない。',
          },
          {
            'メリット': '1回限り4.99ドルのPro解除',
        ...
        rows: [
          {
            ...
            'プラットフォーム': 'iPhoneのみ',
            '価格': '無料。音声入力のための4.99ドルの1回限りPro解除',
      ...
      whoShouldUse: {
        ...
        items: [
          ...
          '**特にサブスクリプションを避けたい読者。** 無料ティアも4.99ドルのPro解除も1回限りで、どちらの場合も継続的な請求はありません。',
          '**サブスクリプションなしでオンデバイス音声入力を望むユーザー。** Pro解除は、開発者によればローカルで動作するWhisperベースの口述筆記のための4.99ドルの1回限り購入です。',
      ...
      whoShouldNotUse: {
        ...
        items: [
          ...
          '**アプリを信頼する前に確立された実績を必要とする読者。** Pocket AIはPrivate LLMのような代替アプリよりApp Storeでの歴史が浅く、開発者自身の開示によれば2026年9月8日より前の時点でレビューはわずか6件でした——レビュー件数を重視する読者はこれを考慮すべきです。',
        ...
        faqs: [
          {
            ...
            a: 'はい。アプリはダウンロードもテキストチャットの利用も無料で、カタログ内の任意のモデルを使えます。1回限り4.99ドルのアプリ内購入でオンデバイスWhisper音声入力が解除されますが、どちらのティアにもサブスクリプションはありません。',
          },
          {
          ...
          {
            q: '4.99ドルのPro解除には何が含まれますか?',
            a: '1回限り4.99ドルのPro解除は、口述筆記のためのオンデバイスWhisper音声入力を追加します。開発者自身の説明によれば、音声入力以外の追加モデルや機能は追加されません。サブスクリプションはありません。',
          },
          {
            q: 'なぜPocket AIのApp Store評価はこれほど低いのですか?',
            a: 'PromptQuorumへの開発者からの最初の連絡時点、2026年9月8日より前において、App Storeの掲載情報は6件のレビューで2.8の評価を示していました。開発者は、2026年9月4日にアプリ内評価プロンプトが導入されるまでアプリがユーザーに評価を求めなかったことが原因だとしています。これは開発者自身の説明であり、PromptQuorumによって独立して検証されたものではなく、現在の評価は変化している可能性があります——最新の数値については[App Storeの掲載情報](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702)を直接確認してください。',
          },
          {
          ...
          },
          {
            ...
            a: 'Pocket AIは無料で、任意の1回限り4.99ドルの音声解除があり、クローズドソースで、App Storeでの歴史が浅いです。[Private LLM](/ja/power-local-llm/private-llm-review)は発売当初から4.99ドルの1回限り購入で、より大規模で確立された140以上のモデルライブラリを持ちます。[PocketPal AI](/ja/power-local-llm/pocketpal-ai-review)は無料でオープンソースですが、GGUFモデルファイルを手動で入手する必要があります。コスト、モデルライブラリの規模と成熟度、あるいはソースコードの透明性のどれを最も重視するかで選んでください。',
          },
        ],
      },
      verdict: {
        ...
        title: '総評',
        content:
          'Pocket AI - No Internetは、シンプルな約束を果たしています。アカウントもサーバー通信もなく、llama.cpp経由でオープンウェイトモデルを完全に端末上で実行する無料のiPhoneアプリであり、音声入力のためのサブスクリプションではなく、真に任意の1回限り4.99ドルの解除を提供します。Qwen、Gemma、Llama、Mistral、Phi、DeepSeekの各ファミリーにまたがる28モデルのカタログは、無料アプリとしては妥当な出発点であり、iPhone 12という最低要件についての開発者自身の理由づけ——メモリに制約される推論——は、曖昧なハードウェア要件ではなく、もっともらしく具体的な説明です。本レビューが熱狂的ではなく慎重な姿勢を保っているのは、アプリがまだ新しいためです。クローズドソースであるため、プライバシーやオフラインに関する主張は独立したコードレビューではなく開発者の言葉に依拠しており、開発者自身が開示したわずか6件のレビューによる2.8の評価(2026年9月8日より前)は、その低いレビュー件数がどれだけうまく説明されていても、まだ構築途上の実績を反映しています。オンデバイスのもっともらしいプライバシーの仕組みを備えた、無料でサブスクリプションのないチャットアプリを求める読者は試してみるべきです。確立されたApp Store実績と独立して監査可能なソースコードをより重視する読者は、代わりに[PocketPal AI](/ja/power-local-llm/pocketpal-ai-review)から始めるか、[Private LLM](/ja/power-local-llm/private-llm-review)の予算を検討すべきです。',
      },
      sources: {
        id: 'sources',
        title: '出典',
        items: [
          '[App StoreのPocket AI - No Internet](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) — 価格、プラットフォームの互換性、評価。',
          '[Pocket AI公式サイト](https://mypocketai.app/) — 製品概要。',
          '2026年9月8日より前にPromptQuorumへ提供された、Datawealの開発者Faisal氏との直接のやり取り — アプリの説明、モデルカタログの数、価格モデル、ハードウェア要件の理由、オープンソース状況、App Store評価の背景。',
        ],
      },
      relatedReading: {
        id: 'related-reading',
  ...
  zh: {
    ...
    intro:
      '[Pocket AI - No Internet](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702)是由开发者Faisal在[Dataweal](https://mypocketai.app/)开发的一款免费iPhone应用,通过[llama.cpp](https://github.com/ggml-org/llama.cpp)配合Metal加速,直接在设备上运行开放权重语言模型。无需账户,也不与服务器通信,一旦模型下载完成,应用即可在飞行模式下使用。应用本身免费;一次性支付4.99美元解锁Pro版即可获得设备端Whisper语音输入,没有订阅费。本评测将介绍该应用的功能、价格、硬件要求,以及——因为开发者主动、直接披露了这一点——目前App Store评分的情况及其原因。',
    metaDescription:
      'Pocket AI - No Internet评测2026:通过llama.cpp实现离线设备端AI聊天的免费iPhone应用,一次性4.99美元解锁Pro版获得Whisper语音输入,无订阅。价格、要求以及开发者披露的评分背景。',
    twitterDescription:
    ...
    quickAnswerTop: {
      ...
      zh: {
        ...
        answer:
          '如果您想要一款免费、无需账户、通过llama.cpp完全离线运行开放权重AI模型的iPhone应用,并且拥有iPhone 12或更新机型,那么值得下载。4.99美元的Pro解锁是可选的,仅增加设备端Whisper语音输入——文本聊天是免费的。如果您需要更大、更成熟的精选模型库,或需要Android/桌面端支持,则可以跳过;建议先与Private LLM或PocketPal AI进行比较。',
        bullets: [
          '免费下载并用于文本聊天;无需账户或登录。',
          '一次性4.99美元的Pro解锁可增加设备端Whisper语音听写功能——无订阅。',
          '通过llama.cpp配合Metal加速运行开放权重模型:Qwen、Gemma、Llama、Mistral、Phi和DeepSeek系列,据开发者介绍模型库共有28个模型。',
          '模型下载完成后可完全离线运行(飞行模式)。',
          '需要iPhone 12或更新机型;据开发者介绍,该应用为闭源。',
          '截至开发者首次联系PromptQuorum时(2026年9月8日之前),App Store评分为2.8分,共6条评分——开发者将评分数量少归因于该应用在2026年9月4日推出应用内评分提示之前从未提示用户评分。这是开发者自己的解释,未经独立核实。',
        ],
        updatedDate: '2026-09-11',
      },
    },
    toc: [
      ...
      { label: 'App Store评分:开发者披露的信息', anchor: 'app-store-rating' },
    ...
    sections: {
      tldr: {
        ...
        snippetBlocks: [
          {
            type: 'one-sentence',
            text: 'Pocket AI - No Internet是Dataweal开发者Faisal打造的一款免费iPhone应用,通过llama.cpp配合Metal加速,完全在设备端运行开放权重AI模型(Qwen、Gemma、Llama、Mistral、Phi、DeepSeek),提供一次性4.99美元的Pro解锁以获得设备端Whisper语音输入,无订阅。',
          },
          {
            type: 'plain-terms',
        ...
        items: [
          '价格:免费[下载](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702)并用于文本聊天;一次性4.99美元的Pro解锁可增加设备端Whisper语音输入,无订阅。',
          ...
          'App Store评分:截至开发者首次发邮件给PromptQuorum时(2026年9月8日之前),评分为2.8分,共6条评分;开发者将评分数量少归因于该应用在2026年9月4日推出应用内评分提示之前从未提示评分——这是开发者自己的解释,未经独立核实,当前评分可能与此数字不同。',
        ],
      },
      whatIsPocketAI: {
        id: 'what-is-pocket-ai',
      ...
      howToGetStarted: {
        ...
        numberedItems: [
          ...
          },
          {
            ...
            whyItMatters: '一次性4.99美元的应用内购买可解锁设备端Whisper语音输入用于听写,无订阅。这是可选的——文本聊天无需此功能即可使用。',
          },
        ],
      ...
      fromTheMaker: {
        ...
        title: '来自开发者',
        content: [
          'PromptQuorum请Dataweal公司负责开发Pocket AI的开发者Faisal描述这款应用,以及其定价和平台选择背后的考量。以下内容以开发者本人的话呈现,为便于阅读略作排版,并非PromptQuorum独立的编辑评估:',
        ],
        blockquote:
        ...
        blockquoteSource: 'Faisal,开发者,Dataweal',
      },
      appStoreRating: {
        id: 'app-store-rating',
        title: 'App Store评分:开发者披露的信息',
        content: [
          '**截至开发者首次联系PromptQuorum时,即2026年9月8日之前,Pocket AI的App Store页面显示评分为2.8分,共6条评分。** 开发者主动、直接披露了这一数字,并附带了解释:该应用在2026年9月4日推出应用内评分提示之前,从未提示用户留下评分。',
          '这是开发者对评分数量和平均分为何呈现如此结果的自我解释——PromptQuorum并未独立核实其原因,而基于6条评分得出的平均分是一个非常小的样本,只需再增加一两条评分就可能大幅波动。读者应直接查看[当前的App Store页面](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702),获取最新的评分和数量,因为自记录该数字以来两者很可能都已发生变化。',
          'PromptQuorum之所以纳入这一数字,是因为开发者是主动提供的,而非应要求提供——一个有明确解释原因的低评分数量,对读者而言比完全没有评分信息更有用,前提是明确标注为开发者所述,而非独立确认的事实。',
        ],
      },
      tradeOffs: {
        id: 'tradeoffs',
        ...
        rows: [
          {
            ...
            '局限/注意事项': '语音输入需要4.99美元的Pro解锁;并非免费包含。',
          },
          {
            '优点': '一次性4.99美元的Pro解锁',
      ...
      whoShouldUse: {
        ...
        items: [
          ...
          '**特别想避免订阅的读者。** 无论是免费等级还是4.99美元的Pro解锁,都是一次性的——两种情况下都没有经常性费用。',
          '**希望在不订阅的情况下使用设备端语音输入的用户。** Pro解锁是一次性支付4.99美元,用于据开发者介绍在本地运行的基于Whisper的听写功能。',
      ...
      whoShouldNotUse: {
        ...
        items: [
          ...
          '**在信任一款应用之前需要看到成熟记录的读者。** Pocket AI在App Store上的历史比Private LLM等替代方案更短,据开发者自己披露,截至2026年9月8日之前仅有6条评分——非常看重评论数量的读者应考虑这一点。',
        ...
        faqs: [
          {
            ...
            a: '是的。该应用可免费下载,并用于与模型库中任意模型进行文本聊天。一次性4.99美元的应用内购买可解锁设备端Whisper语音输入;两个等级都没有订阅费。',
          },
          {
          ...
          {
            q: '4.99美元的Pro解锁包含什么?',
            a: '一次性4.99美元的Pro解锁可增加用于听写的设备端Whisper语音输入功能。据开发者自述,除语音输入外,它不会增加额外的模型或功能。没有订阅费。',
          },
          {
            q: '为什么Pocket AI的App Store评分这么低?',
            a: '截至开发者首次联系PromptQuorum时,即2026年9月8日之前,App Store页面显示评分为2.8分,共6条评分。开发者将此归因于该应用在2026年9月4日推出应用内评分提示之前从未提示用户留下评分。这是开发者自己的解释,未经PromptQuorum独立核实,当前评分可能已发生变化——请直接查看[App Store页面](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702)获取最新数字。',
          },
          {
          ...
          },
          {
            ...
            a: 'Pocket AI免费,可选一次性4.99美元的语音解锁,闭源,且在App Store上的历史较短。[Private LLM](/zh/power-local-llm/private-llm-review)从发布之初就是一次性支付4.99美元,拥有更大、更成熟的140多个模型库。[PocketPal AI](/zh/power-local-llm/pocketpal-ai-review)免费且开源,但需要手动获取GGUF模型文件。请根据您最看重成本、模型库规模与成熟度,还是源代码透明度来做出选择。',
          },
        ],
      },
      verdict: {
        ...
        title: '总结',
        content:
          'Pocket AI - No Internet兑现了一个简单的承诺:一款免费iPhone应用,通过llama.cpp完全在设备端运行开放权重模型,无需账户,不与服务器通信,并提供真正可选的一次性4.99美元语音输入解锁,而非订阅。涵盖Qwen、Gemma、Llama、Mistral、Phi和DeepSeek系列的28个模型库,对于一款免费应用来说是一个合理的起点,而开发者对iPhone 12最低要求的自述理由——推理受内存限制——是一个可信且具体的解释,而非模糊的硬件要求。使本评测保持谨慎而非热情的原因,在于该应用尚属年轻:它是闭源的,因此隐私和离线方面的说法依赖于开发者的一面之词,而非独立代码审查;开发者自己披露的、仅基于6条评分得出的2.8分评分(2026年9月8日之前),无论对评分数量少的解释多么充分,都反映出这是一份仍在建立中的记录。希望获得免费、无订阅、具有合理设备端隐私保障的聊天应用的读者可以一试;更看重成熟的App Store记录和可独立审计源代码的读者,则应改为从[PocketPal AI](/zh/power-local-llm/pocketpal-ai-review)入手,或为[Private LLM](/zh/power-local-llm/private-llm-review)预留预算。',
      },
      sources: {
        id: 'sources',
        title: '来源',
        items: [
          '[App Store上的Pocket AI - No Internet](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) — 价格、平台兼容性和评分。',
          '[Pocket AI官方网站](https://mypocketai.app/) — 产品概述。',
          '2026年9月8日之前提供给PromptQuorum的、与Dataweal开发者Faisal的直接通信 — 应用描述、模型库数量、定价模式、硬件要求的理由、开源状态以及App Store评分背景。',
        ],
      },
      relatedReading: {
        id: 'related-reading',
  ...
  es: {
    ...
    intro:
      '[Pocket AI - No Internet](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702), creada por el desarrollador Faisal en [Dataweal](https://mypocketai.app/), es una aplicación gratuita para iPhone que ejecuta modelos de lenguaje de pesos abiertos directamente en el dispositivo mediante [llama.cpp](https://github.com/ggml-org/llama.cpp) con aceleración Metal. No requiere cuenta ni comunicación con un servidor, y una vez descargado un modelo, la app funciona en modo avión. La app en sí es gratuita; un desbloqueo Pro de pago único de 4,99 $ añade entrada de voz Whisper en el dispositivo, sin suscripción. Esta reseña cubre qué hace la app, cuánto cuesta, qué hardware necesita y —porque el desarrollador lo reveló de forma directa y sin que se le pidiera— cómo es actualmente su calificación en la App Store y por qué.',
    metaDescription:
      'Reseña de Pocket AI - No Internet 2026: app gratuita para iPhone de chat de IA offline en el dispositivo vía llama.cpp, desbloqueo Pro de pago único de 4,99 $ para entrada de voz Whisper, sin suscripción. Precios, requisitos y contexto de calificación revelado por el desarrollador.',
    twitterDescription:
    ...
    quickAnswerTop: {
      ...
      es: {
        ...
        answer:
          'Sí, si busca una app gratuita para iPhone sin cuenta que ejecute modelos de IA de pesos abiertos completamente offline mediante llama.cpp, y posee un iPhone 12 o más reciente. El desbloqueo Pro de 4,99 $ es opcional y solo añade entrada de voz Whisper en el dispositivo — el chat de texto es gratuito. Sáltesela si necesita una biblioteca de modelos más grande y madura, o compatibilidad con Android/escritorio; compárela primero con Private LLM o PocketPal AI.',
        bullets: [
          'Gratis para descargar y usar en chat de texto; no requiere cuenta ni inicio de sesión.',
          'El desbloqueo Pro de pago único de 4,99 $ añade dictado por voz Whisper en el dispositivo — sin suscripción.',
          ...
          'Según el primer contacto del desarrollador con PromptQuorum (antes del 8 de septiembre de 2026), la ficha de la App Store mostraba una calificación de 2,8 con 6 reseñas — el desarrollador atribuye este bajo número de reseñas a que la app no pedía valoraciones hasta que se implementó un aviso interno el 4 de septiembre de 2026. Esta es la propia explicación del desarrollador, no verificada de forma independiente.',
        ],
        updatedDate: '2026-09-11',
      },
    },
    toc: [
      ...
      { label: 'Calificación en la App Store: lo que reveló el desarrollador', anchor: 'app-store-rating' },
    ...
    sections: {
      tldr: {
        ...
        snippetBlocks: [
          {
            type: 'one-sentence',
            text: 'Pocket AI - No Internet es una app gratuita para iPhone del desarrollador Faisal en Dataweal que ejecuta modelos de IA de pesos abiertos (Qwen, Gemma, Llama, Mistral, Phi, DeepSeek) completamente en el dispositivo mediante llama.cpp con aceleración Metal, con un desbloqueo Pro de pago único de 4,99 $ para entrada de voz Whisper en el dispositivo y sin suscripción.',
          },
          {
            type: 'plain-terms',
        ...
        items: [
          'Precio: gratis para [descargar](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) y usar en chat de texto; un desbloqueo Pro de pago único de 4,99 $ añade entrada de voz Whisper en el dispositivo, sin suscripción.',
          ...
          'Calificación en la App Store: 2,8 con 6 reseñas según el primer correo del desarrollador a PromptQuorum (antes del 8 de septiembre de 2026); el desarrollador atribuye este número bajo a que la app no pedía valoraciones hasta que se implementó un aviso interno el 4 de septiembre de 2026 — esta es la propia explicación del desarrollador, no verificada de forma independiente, y la calificación actual puede diferir de esta cifra.',
        ],
      },
      whatIsPocketAI: {
        id: 'what-is-pocket-ai',
      ...
      howToGetStarted: {
        ...
        numberedItems: [
          ...
          },
          {
            ...
            whyItMatters: 'Una compra dentro de la app de pago único de 4,99 $ desbloquea la entrada de voz Whisper en el dispositivo para dictado, sin suscripción. Esto es opcional — el chat de texto funciona sin ello.',
          },
        ],
      ...
      fromTheMaker: {
        ...
        blockquoteSource: 'Faisal, desarrollador, Dataweal',
      },
      appStoreRating: {
        id: 'app-store-rating',
        title: 'Calificación en la App Store: lo que reveló el desarrollador',
        content: [
          '**Según el primer contacto del desarrollador con PromptQuorum, antes del 8 de septiembre de 2026, la ficha de la App Store de Pocket AI mostraba una calificación de 2,8 con 6 reseñas.** El desarrollador reveló esta cifra de forma directa y sin que se le pidiera, junto con una explicación: la app nunca había pedido a los usuarios que dejaran una valoración hasta que se implementó un aviso interno de valoración el 4 de septiembre de 2026.',
          'Esta es la propia explicación del desarrollador sobre por qué el número de reseñas y el promedio se ven así — PromptQuorum no ha verificado la causa de forma independiente, y un promedio de calificación basado en 6 reseñas es una muestra muy pequeña que puede moverse bruscamente con solo una o dos reseñas adicionales en cualquier dirección. Los lectores deberían consultar directamente la [ficha actual de la App Store](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) para conocer la calificación y el número actualizados, ya que es probable que ambos hayan cambiado desde que se registró esta cifra.',
          'PromptQuorum incluye esta cifra específicamente porque el desarrollador la ofreció voluntariamente, no porque se le haya solicitado — un número bajo de reseñas con una causa explicada es un contexto más útil para los lectores que no tener ninguna información de calificación, siempre que quede claramente etiquetado como atribuido al desarrollador y no como confirmado de forma independiente.',
        ],
      },
      tradeOffs: {
        id: 'tradeoffs',
        ...
        rows: [
          {
            ...
            'Limitación / advertencia': 'La entrada de voz requiere el desbloqueo Pro de 4,99 $; no está incluida gratis.',
          },
          {
            'Ventaja': 'Desbloqueo Pro de pago único de 4,99 $',
        ...
        rows: [
          {
            ...
            'Precio': 'Gratis; desbloqueo Pro único de 4,99 $ para entrada de voz',
      ...
      whoShouldUse: {
        ...
        items: [
          ...
          '**Lectores que específicamente quieren evitar una suscripción.** Tanto el nivel gratuito como el desbloqueo Pro de 4,99 $ son de pago único — no hay cargo recurrente en ningún caso.',
          '**Usuarios que quieren entrada de voz en el dispositivo sin suscripción.** El desbloqueo Pro es una compra única de 4,99 $ para dictado basado en Whisper que, según el desarrollador, se ejecuta localmente.',
      ...
      whoShouldNotUse: {
        ...
        items: [
          ...
          '**Lectores que necesitan un historial establecido antes de confiar en una app.** Pocket AI es más nueva en la App Store que alternativas como Private LLM y, según la propia revelación del desarrollador, tenía solo 6 reseñas antes del 8 de septiembre de 2026 — los lectores que valoran mucho el volumen de reseñas deberían tenerlo en cuenta.',
        ...
        faqs: [
          {
            ...
            a: 'Sí. La app es gratuita para descargar y usar en chat de texto con cualquier modelo de su catálogo. Una compra dentro de la app de pago único de 4,99 $ desbloquea la entrada de voz Whisper en el dispositivo; ninguno de los dos niveles tiene suscripción.',
          },
          {
          ...
          {
            q: '¿Qué incluye el desbloqueo Pro de 4,99 $?',
            a: 'El desbloqueo Pro de pago único de 4,99 $ añade entrada de voz Whisper en el dispositivo para dictado. Según la propia descripción del desarrollador, no añade modelos ni funciones adicionales más allá de la entrada de voz. No hay suscripción.',
          },
          {
            q: '¿Por qué tiene Pocket AI una calificación tan baja en la App Store?',
            a: 'Según el primer contacto del desarrollador con PromptQuorum, antes del 8 de septiembre de 2026, la ficha de la App Store mostraba una calificación de 2,8 con 6 reseñas. El desarrollador lo atribuye a que la app no pedía a los usuarios que dejaran una valoración hasta que se implementó un aviso interno el 4 de septiembre de 2026. Esta es la propia explicación del desarrollador, no verificada de forma independiente por PromptQuorum, y la calificación actual puede haber cambiado — consulte directamente la [ficha de la App Store](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) para conocer la cifra actualizada.',
          },
          {
      ...
      verdict: {
        ...
        title: 'Veredicto',
        content:
          'Pocket AI - No Internet cumple una promesa sencilla: una app gratuita para iPhone que ejecuta modelos de pesos abiertos completamente en el dispositivo mediante llama.cpp, sin cuenta ni comunicación con servidor, con un desbloqueo de voz genuinamente opcional de pago único de 4,99 $ en lugar de una suscripción. El catálogo de 28 modelos de las familias Qwen, Gemma, Llama, Mistral, Phi y DeepSeek es un punto de partida razonable para una app gratuita, y el propio razonamiento del desarrollador para el mínimo de iPhone 12 —inferencia limitada por la memoria— es una explicación plausible y específica en lugar de un requisito de hardware vago. Lo que hace que esta reseña sea cautelosa en lugar de entusiasta es la juventud de la app: es de código cerrado, por lo que las afirmaciones sobre privacidad y funcionamiento offline se basan en la palabra del desarrollador en lugar de una revisión de código independiente, y la propia revelación del desarrollador de una calificación de 2,8 con solo 6 reseñas (antes del 8 de septiembre de 2026) refleja un historial que todavía se está construyendo, por bien explicado que esté ese bajo número de reseñas. Los lectores que quieran una app de chat gratuita, sin suscripción, con una historia de privacidad en el dispositivo plausible deberían probarla; los lectores que valoren más un historial establecido en la App Store y un código fuente auditable de forma independiente deberían empezar en su lugar con [PocketPal AI](/es/power-local-llm/pocketpal-ai-review) o presupuestar [Private LLM](/es/power-local-llm/private-llm-review).',
      },
      sources: {
        id: 'sources',
        title: 'Fuentes',
        items: [
          '[Pocket AI - No Internet en la App Store](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) — precio, compatibilidad de plataforma y calificación.',
          '[Sitio oficial de Pocket AI](https://mypocketai.app/) — resumen del producto.',
          'Correspondencia directa con Faisal, desarrollador en Dataweal, facilitada a PromptQuorum antes del 8 de septiembre de 2026 — descripción de la app, número de modelos en el catálogo, modelo de precios, razonamiento del requisito de hardware, estado de código abierto y contexto de la calificación en la App Store.',
        ],
      },
      relatedReading: {
        id: 'related-reading',
  ...
  pt: {
    ...
    intro:
      'O [Pocket AI - No Internet](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702), criado pelo desenvolvedor Faisal na [Dataweal](https://mypocketai.app/), é um aplicativo gratuito para iPhone que executa modelos de linguagem de pesos abertos diretamente no dispositivo usando [llama.cpp](https://github.com/ggml-org/llama.cpp) com aceleração Metal. Não há conta nem comunicação com servidor, e depois que um modelo é baixado, o aplicativo funciona no modo avião. O aplicativo em si é gratuito; um desbloqueio Pro de pagamento único de US$ 4,99 adiciona entrada de voz Whisper no dispositivo, sem assinatura. Esta análise aborda o que o aplicativo faz, quanto custa, qual hardware exige e — porque o desenvolvedor divulgou isso diretamente, sem ser solicitado — como está atualmente a avaliação na App Store e por quê.',
    metaDescription:
      'Análise do Pocket AI - No Internet 2026: aplicativo gratuito para iPhone de chat de IA offline no dispositivo via llama.cpp, desbloqueio Pro de pagamento único de US$ 4,99 para entrada de voz Whisper, sem assinatura. Preços, requisitos e o contexto da avaliação divulgado pelo desenvolvedor.',
    twitterDescription:
    ...
    quickAnswerTop: {
      ...
      pt: {
        ...
        answer:
          'Sim, se você quer um aplicativo gratuito para iPhone, sem conta, que execute modelos de IA de pesos abertos totalmente offline via llama.cpp, e possui um iPhone 12 ou mais recente. O desbloqueio Pro de US$ 4,99 é opcional e adiciona apenas entrada de voz Whisper no dispositivo — o chat de texto é gratuito. Não vale a pena se você precisa de uma biblioteca de modelos maior e mais madura, ou suporte para Android/desktop; compare primeiro com o Private LLM ou o PocketPal AI.',
        bullets: [
          'Gratuito para baixar e usar em chat de texto; não exige conta ou login.',
          'O desbloqueio Pro de pagamento único de US$ 4,99 adiciona ditado por voz Whisper no dispositivo — sem assinatura.',
          ...
          'Segundo o primeiro contato do desenvolvedor com a PromptQuorum (antes de 8 de setembro de 2026), a ficha na App Store mostrava uma avaliação de 2,8 com 6 avaliações — o desenvolvedor atribui esse baixo número ao fato de o aplicativo nunca ter solicitado avaliações até que um aviso interno fosse lançado em 4 de setembro de 2026. Esta é a explicação do próprio desenvolvedor, não verificada de forma independente.',
        ],
        updatedDate: '2026-09-11',
      },
    },
    toc: [
      ...
      { label: 'Avaliação na App Store: o que o desenvolvedor revelou', anchor: 'app-store-rating' },
    ...
    sections: {
      tldr: {
        ...
        items: [
          'Preço: gratuito para [baixar](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) e usar em chat de texto; um desbloqueio Pro de pagamento único de US$ 4,99 adiciona entrada de voz Whisper no dispositivo, sem assinatura.',
          ...
          'Avaliação na App Store: 2,8 com 6 avaliações no momento do primeiro e-mail do desenvolvedor à PromptQuorum (antes de 8 de setembro de 2026); o desenvolvedor atribui esse número baixo ao fato de o aplicativo não solicitar avaliações até que um aviso interno fosse lançado em 4 de setembro de 2026 — esta é a explicação do próprio desenvolvedor, não verificada de forma independente, e a avaliação atual pode ser diferente desse número.',
        ],
      },
      whatIsPocketAI: {
        id: 'what-is-pocket-ai',
      ...
      howToGetStarted: {
        ...
        numberedItems: [
          ...
          },
          {
            ...
            whyItMatters: 'Uma compra dentro do aplicativo de pagamento único de US$ 4,99 desbloqueia a entrada de voz Whisper no dispositivo para ditado, sem assinatura. Isso é opcional — o chat de texto funciona sem isso.',
          },
        ],
      ...
      fromTheMaker: {
        ...
        content: [
          'A PromptQuorum pediu a Faisal, o desenvolvedor do Pocket AI na Dataweal, que descrevesse o aplicativo e o raciocínio por trás de suas decisões de preço e plataforma. O que se segue é apresentado como as próprias palavras do desenvolvedor, levemente formatadas para facilitar a leitura, e não como uma avaliação editorial independente da PromptQuorum:',
        ],
        blockquote:
        ...
        blockquoteSource: 'Faisal, desenvolvedor, Dataweal',
      },
      appStoreRating: {
        id: 'app-store-rating',
        title: 'Avaliação na App Store: o que o desenvolvedor revelou',
        content: [
          '**No momento do primeiro contato do desenvolvedor com a PromptQuorum, antes de 8 de setembro de 2026, a ficha do Pocket AI na App Store mostrava uma avaliação de 2,8 com 6 avaliações.** O desenvolvedor divulgou esse número diretamente, sem ser solicitado, junto com uma explicação: o aplicativo nunca havia solicitado que os usuários deixassem uma avaliação até que um aviso interno de avaliação fosse lançado em 4 de setembro de 2026.',
          'Essa é a própria explicação do desenvolvedor sobre o motivo pelo qual o número de avaliações e a média parecem assim — a PromptQuorum não verificou a causa de forma independente, e uma média de avaliação baseada em 6 avaliações é uma amostra muito pequena que pode mudar drasticamente com apenas uma ou duas avaliações adicionais em qualquer direção. Os leitores devem consultar diretamente a [ficha atual da App Store](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) para obter a avaliação e o número atualizados, já que ambos provavelmente mudaram desde que esse número foi registrado.',
          'A PromptQuorum inclui esse número especificamente porque o desenvolvedor o ofereceu voluntariamente, e não porque foi solicitado — um número baixo de avaliações com uma causa explicada é um contexto mais útil para os leitores do que nenhuma informação de avaliação, desde que claramente identificado como atribuído ao desenvolvedor, e não confirmado de forma independente.',
        ],
      },
      tradeOffs: {
        id: 'tradeoffs',
        ...
        rows: [
          {
            ...
            'Limitação / ressalva': 'A entrada de voz exige o desbloqueio Pro de US$ 4,99; não é incluída gratuitamente.',
          },
          {
            'Benefício': 'Desbloqueio Pro de pagamento único de US$ 4,99',
        ...
        rows: [
          {
            ...
            'Preço': 'Gratuito; desbloqueio Pro único de US$ 4,99 para entrada de voz',
      ...
      whoShouldUse: {
        ...
        items: [
          ...
          '**Leitores que especificamente querem evitar uma assinatura.** Tanto o nível gratuito quanto o desbloqueio Pro de US$ 4,99 são pagamentos únicos — sem cobrança recorrente em nenhum dos casos.',
          '**Usuários que querem entrada de voz no dispositivo sem assinatura.** O desbloqueio Pro é uma compra única de US$ 4,99 para ditado baseado em Whisper que, segundo o desenvolvedor, é executado localmente.',
      ...
      whoShouldNotUse: {
        ...
        items: [
          ...
          '**Leitores que precisam de um histórico estabelecido antes de confiar em um aplicativo.** O Pocket AI é mais novo na App Store do que alternativas como o Private LLM e, segundo a própria divulgação do desenvolvedor, tinha apenas 6 avaliações antes de 8 de setembro de 2026 — leitores que valorizam muito o volume de avaliações devem levar isso em conta.',
        ...
        faqs: [
          {
            ...
            a: 'Sim. O aplicativo é gratuito para baixar e usar em chat de texto com qualquer modelo do seu catálogo. Uma compra dentro do aplicativo de pagamento único de US$ 4,99 desbloqueia a entrada de voz Whisper no dispositivo; nenhum dos dois níveis tem assinatura.',
          },
          {
          ...
          {
            q: 'O que está incluído no desbloqueio Pro de US$ 4,99?',
            a: 'O desbloqueio Pro de pagamento único de US$ 4,99 adiciona entrada de voz Whisper no dispositivo para ditado. Segundo a própria descrição do desenvolvedor, ele não adiciona modelos ou recursos adicionais além da entrada de voz. Não há assinatura.',
          },
          {
            q: 'Por que o Pocket AI tem uma avaliação tão baixa na App Store?',
            a: 'No momento do primeiro contato do desenvolvedor com a PromptQuorum, antes de 8 de setembro de 2026, a ficha da App Store mostrava uma avaliação de 2,8 com 6 avaliações. O desenvolvedor atribui isso ao fato de o aplicativo não solicitar que os usuários deixassem uma avaliação até que um aviso interno de avaliação fosse lançado em 4 de setembro de 2026. Esta é a própria explicação do desenvolvedor, não verificada de forma independente pela PromptQuorum, e a avaliação atual pode ter mudado — consulte diretamente a [ficha da App Store](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) para o número atualizado.',
          },
          {
      ...
      verdict: {
        ...
        title: 'Veredito',
        content:
          'O Pocket AI - No Internet cumpre uma promessa simples: um aplicativo gratuito para iPhone que executa modelos de pesos abertos totalmente no dispositivo via llama.cpp, sem conta e sem comunicação com servidor, com um desbloqueio de voz genuinamente opcional de pagamento único de US$ 4,99 em vez de uma assinatura. O catálogo de 28 modelos das famílias Qwen, Gemma, Llama, Mistral, Phi e DeepSeek é um ponto de partida razoável para um aplicativo gratuito, e o próprio raciocínio do desenvolvedor para o mínimo de iPhone 12 — inferência limitada pela memória — é uma explicação plausível e específica, em vez de uma exigência vaga de hardware. O que mantém esta análise cautelosa em vez de entusiasmada é a pouca idade do aplicativo: ele é de código fechado, então as alegações sobre privacidade e funcionamento offline se baseiam na palavra do desenvolvedor, e não em uma revisão de código independente, e a própria divulgação do desenvolvedor de uma avaliação de 2,8 com apenas 6 avaliações (antes de 8 de setembro de 2026) reflete um histórico que ainda está sendo construído, por mais bem explicado que seja esse baixo número de avaliações. Leitores que querem um aplicativo de chat gratuito, sem assinatura, com uma história de privacidade plausível no dispositivo deveriam experimentá-lo; leitores que valorizam mais um histórico estabelecido na App Store e um código-fonte auditável de forma independente deveriam começar com o [PocketPal AI](/pt/power-local-llm/pocketpal-ai-review) ou reservar orçamento para o [Private LLM](/pt/power-local-llm/private-llm-review).',
      },
      sources: {
        id: 'sources',
        title: 'Fontes',
        items: [
          '[Pocket AI - No Internet na App Store](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) — preço, compatibilidade de plataforma e avaliação.',
          '[Site oficial do Pocket AI](https://mypocketai.app/) — visão geral do produto.',
          'Correspondência direta com Faisal, desenvolvedor na Dataweal, fornecida à PromptQuorum antes de 8 de setembro de 2026 — descrição do aplicativo, número de modelos no catálogo, modelo de preços, raciocínio sobre o requisito de hardware, status de código aberto e contexto da avaliação na App Store.',
        ],
      },
      relatedReading: {
        id: 'related-reading',
  ...
  ar: {
    ...
    intro:
      '[Pocket AI - No Internet](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702)، من تطوير فيصل في شركة [Dataweal](https://mypocketai.app/)، هو تطبيق مجاني لآيفون يشغّل نماذج لغوية مفتوحة الأوزان مباشرة على الجهاز باستخدام [llama.cpp](https://github.com/ggml-org/llama.cpp) مع تسريع Metal. لا يوجد حساب ولا اتصال بخادم، وبمجرد تنزيل النموذج، يعمل التطبيق في وضع الطيران. التطبيق نفسه مجاني؛ ويضيف فتح نسخة Pro بدفعة واحدة قدرها 4.99 دولار إدخال صوت Whisper على الجهاز، دون أي اشتراك. تتناول هذه المراجعة ما يفعله التطبيق، وتكلفته، والأجهزة المطلوبة، و— لأن المطوّر كشف عن ذلك مباشرةً ودون طلب — وضع تقييمه الحالي على متجر التطبيقات وسبب ذلك.',
    metaDescription:
      'مراجعة Pocket AI - No Internet 2026: تطبيق آيفون مجاني للدردشة بالذكاء الاصطناعي دون إنترنت على الجهاز عبر llama.cpp، فتح Pro بدفعة واحدة قدرها 4.99 دولار لإدخال الصوت عبر Whisper، دون اشتراك. الأسعار والمتطلبات وسياق التقييم الذي كشف عنه المطوّر.',
    twitterDescription:
    ...
    current_hardware_mentioned: ['iPhone 12'],
    leadAnswerBlock:
      '**Pocket AI - No Internet تطبيق مجاني لآيفون يشغّل نماذج ذكاء اصطناعي مفتوحة الأوزان بالكامل على الجهاز باستخدام [llama.cpp](https://github.com/ggml-org/llama.cpp) مع تسريع Metal — دون حساب، ودون خادم، ويمكن استخدامه بالكامل دون إنترنت بمجرد تنزيل نموذج.** من تطوير فيصل في [Dataweal](https://mypocketai.app/)، وتضم مكتبته وفق المطوّر 28 نموذجًا من عائلات Qwen وGemma وLlama وMistral وPhi وDeepSeek. التطبيق مجاني للدردشة النصية؛ ويضيف فتح Pro بدفعة واحدة (4.99 دولار) إدخال صوت Whisper على الجهاز، دون اشتراك. يتطلب آيفون 12 أو أحدث، وهو ما يعزوه المطوّر إلى كون الاستدلال محدودًا بحجم الذاكرة. للمقارنة، راجع أيضًا [مراجعة Private LLM](/ar/power-local-llm/private-llm-review) أو [مراجعة PocketPal AI](/ar/power-local-llm/pocketpal-ai-review).',
    quickAnswerTop: {
      en: {
        question: 'Is Pocket AI - No Internet worth downloading?',
        answer:
      ...
      ar: {
        ...
        answer:
          'نعم، إذا كنت تريد تطبيق آيفون مجانيًا ودون حساب يشغّل نماذج ذكاء اصطناعي مفتوحة الأوزان دون إنترنت بالكامل عبر llama.cpp، وتمتلك آيفون 12 أو أحدث. فتح Pro بقيمة 4.99 دولار اختياري ويضيف فقط إدخال صوت Whisper على الجهاز — الدردشة النصية مجانية. تجاوزه إذا كنت بحاجة إلى مكتبة نماذج أكبر وأكثر نضجًا أو دعم أندرويد/سطح المكتب؛ قارنه أولًا بـ Private LLM أو PocketPal AI.',
        bullets: [
          'مجاني للتنزيل والاستخدام في الدردشة النصية؛ لا يتطلب حسابًا أو تسجيل دخول.',
          'فتح Pro بدفعة واحدة قدرها 4.99 دولار يضيف إملاء صوتي عبر Whisper على الجهاز — دون اشتراك.',
          ...
          'وفق أول تواصل من المطوّر مع PromptQuorum (قبل 8 سبتمبر 2026)، أظهرت صفحة متجر التطبيقات تقييمًا بلغ 2.8 من 6 تقييمات — ويعزو المطوّر انخفاض عدد التقييمات إلى أن التطبيق لم يكن يطلب من المستخدمين التقييم حتى إطلاق تنبيه تقييم داخل التطبيق في 4 سبتمبر 2026. هذا تفسير المطوّر نفسه، ولم يُتحقق منه بشكل مستقل.',
        ],
        updatedDate: '2026-09-11',
      },
    },
    toc: [
      ...
      { label: 'تقييم متجر التطبيقات: ما كشفه المطوّر', anchor: 'app-store-rating' },
    ...
    sections: {
      tldr: {
        ...
        snippetBlocks: [
          {
            type: 'one-sentence',
            text: 'Pocket AI - No Internet تطبيق آيفون مجاني من المطوّر فيصل في Dataweal يشغّل نماذج ذكاء اصطناعي مفتوحة الأوزان (Qwen وGemma وLlama وMistral وPhi وDeepSeek) بالكامل على الجهاز عبر llama.cpp مع تسريع Metal، مع فتح Pro بدفعة واحدة قدرها 4.99 دولار لإدخال صوت Whisper على الجهاز ودون اشتراك.',
          },
          {
            type: 'plain-terms',
        ...
        items: [
          'السعر: مجاني [للتنزيل](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) والاستخدام في الدردشة النصية؛ فتح Pro بدفعة واحدة قدرها 4.99 دولار يضيف إدخال صوت Whisper على الجهاز، دون اشتراك.',
          ...
          'تقييم متجر التطبيقات: 2.8 من 6 تقييمات وقت أول بريد إلكتروني من المطوّر إلى PromptQuorum (قبل 8 سبتمبر 2026)؛ ويعزو المطوّر هذا العدد المنخفض إلى أن التطبيق لم يكن يطلب التقييمات حتى إطلاق تنبيه تقييم داخل التطبيق في 4 سبتمبر 2026 — هذا تفسير المطوّر نفسه، ولم يُتحقق منه بشكل مستقل، وقد يختلف التقييم الحالي عن هذا الرقم.',
        ],
      },
      whatIsPocketAI: {
        id: 'what-is-pocket-ai',
      ...
      howToGetStarted: {
        ...
        numberedItems: [
          ...
          },
          {
            ...
            whyItMatters: 'يفتح شراء لمرة واحدة داخل التطبيق بقيمة 4.99 دولار إدخال صوت Whisper على الجهاز للإملاء، دون اشتراك. هذا اختياري — تعمل الدردشة النصية دونه.',
          },
        ],
      ...
      fromTheMaker: {
        ...
        content: [
          'طلبت PromptQuorum من فيصل، مطوّر Pocket AI في Dataweal، أن يصف التطبيق والمنطق وراء قراراته المتعلقة بالتسعير والمنصة. ما يلي مقدَّم بصفته كلمات المطوّر نفسه، منسّقة قليلًا لتسهيل القراءة، وليس بوصفه تقييمًا تحريريًا مستقلًا من PromptQuorum:',
        ],
        blockquote:
        ...
        blockquoteSource: 'فيصل، مطوّر، Dataweal',
      },
      appStoreRating: {
        id: 'app-store-rating',
        title: 'تقييم متجر التطبيقات: ما كشفه المطوّر',
        content: [
          '**وقت أول تواصل من المطوّر مع PromptQuorum، قبل 8 سبتمبر 2026، أظهرت صفحة Pocket AI على متجر التطبيقات تقييمًا بلغ 2.8 من 6 تقييمات.** كشف المطوّر عن هذا الرقم مباشرةً ودون طلب، مع تفسير له: لم يكن التطبيق يطلب من المستخدمين أبدًا ترك تقييم حتى تم إطلاق تنبيه تقييم داخل التطبيق في 4 سبتمبر 2026.',
          'هذا هو تفسير المطوّر نفسه لسبب ظهور عدد التقييمات والمتوسط على هذا النحو — ولم تتحقق PromptQuorum من السبب بشكل مستقل، ومتوسط تقييم قائم على 6 تقييمات هو عينة صغيرة جدًا يمكن أن تتحرك بشكل حاد مع مجرد تقييم أو اثنين إضافيين في أي اتجاه. يجب على القراء مراجعة [صفحة متجر التطبيقات الحالية](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) مباشرة للحصول على التقييم والعدد المحدَّثين، لأن من المرجح أن يكون كلاهما قد تغيّر منذ تسجيل هذا الرقم.',
          'تُدرج PromptQuorum هذا الرقم تحديدًا لأن المطوّر قدّمه طوعًا وليس لأنه طُلب منه — فعدد تقييمات منخفض مع سبب موضّح يمثل سياقًا أكثر فائدة للقراء من عدم وجود أي معلومات عن التقييم على الإطلاق، شريطة أن يكون موسومًا بوضوح على أنه منسوب إلى المطوّر وليس مؤكَّدًا بشكل مستقل.',
        ],
      },
      tradeOffs: {
        id: 'tradeoffs',
        ...
        rows: [
          {
            ...
            'القيد / الملاحظة': 'يتطلب الإدخال الصوتي فتح Pro بقيمة 4.99 دولار؛ وهو غير مُضمَّن مجانًا.',
          },
          {
            'الميزة': 'فتح Pro بدفعة واحدة قدرها 4.99 دولار',
        ...
        rows: [
          {
            ...
            'المنصات': 'آيفون فقط',
            'السعر': 'مجاني؛ فتح Pro لمرة واحدة بقيمة 4.99 دولار للإدخال الصوتي',
      ...
      whoShouldUse: {
        ...
        items: [
          ...
          '**القراء الذين يريدون تحديدًا تجنب الاشتراك.** كل من المستوى المجاني وفتح Pro بقيمة 4.99 دولار هما دفعة واحدة — لا رسوم متكررة في أي من الحالتين.',
          '**المستخدمون الذين يريدون إدخالًا صوتيًا على الجهاز دون اشتراك.** فتح Pro هو شراء لمرة واحدة بقيمة 4.99 دولار لإملاء معتمد على Whisper يذكر المطوّر أنه يعمل محليًا.',
      ...
      whoShouldNotUse: {
        ...
        items: [
          ...
          '**القراء الذين يحتاجون إلى سجل راسخ قبل الثقة بتطبيق ما.** Pocket AI أحدث على متجر التطبيقات من بدائل مثل Private LLM، ووفق إفصاح المطوّر نفسه، كان لديه 6 تقييمات فقط قبل 8 سبتمبر 2026 — يجب على القراء الذين يعطون وزنًا كبيرًا لحجم المراجعات أخذ ذلك في الاعتبار.',
        ...
        faqs: [
          {
            ...
            a: 'نعم. التطبيق مجاني للتنزيل والاستخدام في الدردشة النصية مع أي نموذج من كتالوجه. يفتح شراء لمرة واحدة داخل التطبيق بقيمة 4.99 دولار إدخال صوت Whisper على الجهاز؛ ولا يوجد اشتراك لأي من المستويين.',
          },
          {
          ...
          {
            q: 'ما الذي يتضمنه فتح Pro بقيمة 4.99 دولار؟',
            a: 'يضيف فتح Pro بدفعة واحدة قدرها 4.99 دولار إدخال صوت Whisper على الجهاز للإملاء. وفق وصف المطوّر نفسه، لا يضيف نماذج أو ميزات إضافية غير الإدخال الصوتي. لا يوجد اشتراك.',
          },
          {
            q: 'لماذا يحمل Pocket AI تقييمًا منخفضًا على متجر التطبيقات؟',
            a: 'وقت أول تواصل من المطوّر مع PromptQuorum، قبل 8 سبتمبر 2026، أظهرت صفحة متجر التطبيقات تقييمًا بلغ 2.8 من 6 تقييمات. يعزو المطوّر ذلك إلى أن التطبيق لم يكن يطلب من المستخدمين ترك تقييم حتى إطلاق تنبيه تقييم داخل التطبيق في 4 سبتمبر 2026. هذا تفسير المطوّر نفسه، ولم تتحقق منه PromptQuorum بشكل مستقل، وقد يكون التقييم الحالي قد تغيّر — راجع [صفحة متجر التطبيقات](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) مباشرة للحصول على الرقم المحدَّث.',
          },
          {
          ...
          },
          {
            ...
            a: 'Pocket AI مجاني مع فتح صوتي اختياري بدفعة واحدة قدرها 4.99 دولار، ومغلق المصدر، وأحدث على متجر التطبيقات. [Private LLM](/ar/power-local-llm/private-llm-review) هو شراء لمرة واحدة بقيمة 4.99 دولار منذ الإطلاق مع مكتبة أكبر وأكثر رسوخًا تضم أكثر من 140 نموذجًا. [PocketPal AI](/ar/power-local-llm/pocketpal-ai-review) مجاني ومفتوح المصدر لكنه يتطلب الحصول يدويًا على ملفات نماذج GGUF. اختر بناءً على ما تقدّره أكثر: التكلفة، أو حجم ونضج مكتبة النماذج، أو شفافية شيفرة المصدر.',
          },
        ],
      },
      verdict: {
        ...
        title: 'الحكم النهائي',
        content:
          'يفي Pocket AI - No Internet بوعد بسيط: تطبيق آيفون مجاني يشغّل نماذج مفتوحة الأوزان بالكامل على الجهاز عبر llama.cpp، دون حساب ودون اتصال بخادم، مع فتح صوتي اختياري حقًا بدفعة واحدة قدرها 4.99 دولار بدلًا من اشتراك. يمثل كتالوج الـ28 نموذجًا عبر عائلات Qwen وGemma وLlama وMistral وPhi وDeepSeek نقطة انطلاق معقولة لتطبيق مجاني، ويُعد تبرير المطوّر نفسه للحد الأدنى المتمثل في آيفون 12 — الاستدلال المحدود بالذاكرة — تفسيرًا معقولًا ومحددًا بدلًا من متطلب أجهزة غامض. ما يجعل هذه المراجعة حذرة بدلًا من متحمسة هو حداثة عمر التطبيق: فهو مغلق المصدر، لذا تستند ادعاءات الخصوصية والعمل دون إنترنت إلى كلمة المطوّر بدلًا من مراجعة مستقلة للشيفرة، وإفصاح المطوّر نفسه عن تقييم 2.8 من 6 تقييمات فقط (قبل 8 سبتمبر 2026) يعكس سجلًا لا يزال قيد البناء، مهما كان تفسير انخفاض عدد التقييمات جيدًا. القراء الذين يريدون تطبيق دردشة مجانيًا دون اشتراك مع قصة خصوصية معقولة على الجهاز ينبغي عليهم تجربته؛ أما القراء الذين يعطون وزنًا أكبر لسجل راسخ على متجر التطبيقات وشيفرة مصدر قابلة للتدقيق المستقل فينبغي عليهم البدء بدلًا من ذلك بـ[PocketPal AI](/ar/power-local-llm/pocketpal-ai-review) أو تخصيص ميزانية لـ[Private LLM](/ar/power-local-llm/private-llm-review).',
      },
      sources: {
        id: 'sources',
        title: 'المصادر',
        items: [
          '[Pocket AI - No Internet على متجر التطبيقات](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) — السعر وتوافق المنصات والتقييم.',
          '[الموقع الرسمي لـ Pocket AI](https://mypocketai.app/) — نظرة عامة على المنتج.',
          'مراسلات مباشرة مع فيصل، المطوّر في Dataweal، قُدِّمت إلى PromptQuorum قبل 8 سبتمبر 2026 — وصف التطبيق، وعدد النماذج في الكتالوج، ونموذج التسعير، وتبرير متطلبات الأجهزة، وحالة المصدر المفتوح، وسياق تقييم متجر التطبيقات.',
        ],
      },
      relatedReading: {
        id: 'related-reading',
  ...
  ko: {
    ...
    intro:
      '[Pocket AI - No Internet](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702)은 [Dataweal](https://mypocketai.app/)의 개발자 Faisal이 만든 무료 아이폰 앱으로, [llama.cpp](https://github.com/ggml-org/llama.cpp)와 Metal 가속을 이용해 오픈 웨이트 언어 모델을 기기에서 직접 실행합니다. 계정이 필요 없고 서버와의 통신도 없으며, 모델을 다운로드하면 비행기 모드에서도 앱이 작동합니다. 앱 자체는 무료이며, 4.99달러의 일회성 Pro 잠금 해제로 온디바이스 Whisper 음성 입력이 추가되고 구독은 없습니다. 이 리뷰에서는 앱이 무엇을 하는지, 비용은 얼마인지, 어떤 하드웨어가 필요한지, 그리고 — 개발자가 요청받지 않고 직접 공개했기 때문에 — 현재 앱스토어 평점이 어떤 상태이고 그 이유는 무엇인지 다룹니다.',
    metaDescription:
      'Pocket AI - No Internet 리뷰 2026: llama.cpp를 통해 오프라인 온디바이스 AI 채팅을 제공하는 무료 아이폰 앱. Whisper 음성 입력을 위한 4.99달러 일회성 Pro 잠금 해제, 구독 없음. 가격, 요구 사항, 개발자가 공개한 평점 배경.',
    twitterDescription:
    ...
    quickAnswerTop: {
      ...
      ko: {
        ...
        answer:
          '네, llama.cpp를 통해 오픈 웨이트 AI 모델을 완전히 오프라인으로 실행하는 무료, 계정 불필요 아이폰 앱을 원하고 아이폰 12 이상을 보유하고 있다면 그렇습니다. 4.99달러 Pro 잠금 해제는 선택 사항이며 온디바이스 Whisper 음성 입력만 추가합니다 — 텍스트 채팅은 무료입니다. 더 크고 성숙한 큐레이션 모델 라이브러리나 안드로이드/데스크톱 지원이 필요하다면 건너뛰고, 먼저 Private LLM이나 PocketPal AI와 비교해 보세요.',
        bullets: [
          '무료로 다운로드하여 텍스트 채팅에 사용 가능; 계정이나 로그인이 필요 없음.',
          '4.99달러 일회성 Pro 잠금 해제로 온디바이스 Whisper 음성 받아쓰기 추가 — 구독 없음.',
          ...
          'PromptQuorum에 대한 개발자의 첫 연락 시점(2026년 9월 8일 이전) 기준, 앱스토어 목록에는 6개 평점으로 2.8점이 표시되어 있었습니다 — 개발자는 2026년 9월 4일에 앱 내 평점 요청 안내가 도입되기 전까지 앱이 사용자에게 평점을 요청하지 않았던 것을 낮은 평점 수의 원인으로 지목합니다. 이는 개발자 자신의 설명이며 독립적으로 검증되지 않았습니다.',
        ],
        updatedDate: '2026-09-11',
      },
    },
    toc: [
      ...
      { label: '앱스토어 평점: 개발자가 공개한 내용', anchor: 'app-store-rating' },
    ...
    sections: {
      tldr: {
        ...
        snippetBlocks: [
          {
            type: 'one-sentence',
            text: 'Pocket AI - No Internet는 Dataweal의 개발자 Faisal이 만든 무료 아이폰 앱으로, llama.cpp와 Metal 가속을 통해 오픈 웨이트 AI 모델(Qwen, Gemma, Llama, Mistral, Phi, DeepSeek)을 완전히 기기에서 실행하며, 온디바이스 Whisper 음성 입력을 위한 4.99달러 일회성 Pro 잠금 해제가 있고 구독은 없습니다.',
          },
          {
            type: 'plain-terms',
        ...
        items: [
          '가격: 무료로 [다운로드](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702)하여 텍스트 채팅에 사용 가능; 4.99달러 일회성 Pro 잠금 해제로 온디바이스 Whisper 음성 입력 추가, 구독 없음.',
          ...
          '앱스토어 평점: PromptQuorum에 보낸 개발자의 첫 이메일 시점(2026년 9월 8일 이전) 기준 6개 평점으로 2.8점; 개발자는 2026년 9월 4일 앱 내 평점 요청 안내가 도입되기 전까지 평점을 요청하지 않았던 것을 이 낮은 수치의 원인으로 지목 — 이는 개발자 자신의 설명이며 독립적으로 검증되지 않았고, 현재 평점은 이 수치와 다를 수 있습니다.',
        ],
      },
      whatIsPocketAI: {
        id: 'what-is-pocket-ai',
      ...
      howToGetStarted: {
        ...
        numberedItems: [
          ...
          },
          {
            ...
            whyItMatters: '4.99달러의 일회성 앱 내 구매로 받아쓰기용 온디바이스 Whisper 음성 입력이 잠금 해제되며, 구독은 없습니다. 이는 선택 사항이며 — 텍스트 채팅은 이것 없이도 작동합니다.',
          },
        ],
      ...
      fromTheMaker: {
        ...
        title: '개발자의 말',
        content: [
          'PromptQuorum은 Dataweal에서 Pocket AI를 개발한 Faisal에게 앱과 가격 및 플랫폼 선택 뒤에 있는 사고방식을 설명해 달라고 요청했습니다. 다음 내용은 가독성을 위해 약간 정리되었을 뿐, 개발자 본인의 말 그대로 제시되는 것이며 PromptQuorum의 독립적인 편집상의 평가가 아닙니다:',
        ],
        blockquote:
        ...
        blockquoteSource: 'Faisal, 개발자, Dataweal',
      },
      appStoreRating: {
        id: 'app-store-rating',
        title: '앱스토어 평점: 개발자가 공개한 내용',
        content: [
          '**PromptQuorum에 대한 개발자의 첫 연락 시점, 즉 2026년 9월 8일 이전 기준으로 Pocket AI의 앱스토어 목록에는 6개 평점으로 2.8점이 표시되어 있었습니다.** 개발자는 요청받지 않고 이 수치를 직접 공개했으며, 그에 대한 설명도 함께 제공했습니다. 이 앱은 2026년 9월 4일에 앱 내 평점 요청 안내가 도입되기 전까지 사용자에게 평점을 남겨 달라고 요청한 적이 없었다는 것입니다.',
          '이는 평점 수와 평균이 왜 그렇게 보이는지에 대한 개발자 자신의 설명이며 — PromptQuorum은 그 원인을 독립적으로 검증하지 않았고, 6개 평점을 기반으로 한 평균 평점은 단 한두 개의 추가 평점만으로도 어느 방향으로든 크게 흔들릴 수 있는 매우 작은 표본입니다. 독자는 이 수치가 기록된 이후 두 가지 모두 변경되었을 가능성이 크므로, 최신 평점과 개수를 확인하려면 [현재 앱스토어 목록](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702)을 직접 확인해야 합니다.',
          'PromptQuorum이 이 수치를 포함하는 것은 특별히 개발자가 요청받지 않고 자발적으로 제공했기 때문이지 요청되었기 때문이 아닙니다 — 이유가 설명된 낮은 평점 수는 그것이 독립적으로 확인된 것이 아니라 개발자에게 귀속된다는 점이 명확히 표시되어 있는 한, 평점 정보가 전혀 없는 것보다 독자에게 더 유용한 맥락입니다.',
        ],
      },
      tradeOffs: {
        id: 'tradeoffs',
        ...
        rows: [
          {
            ...
            '한계 / 유의 사항': '음성 입력에는 4.99달러의 Pro 잠금 해제가 필요하며, 무료로 포함되지 않습니다.',
          },
          {
            '이점': '4.99달러 일회성 Pro 잠금 해제',
        ...
        rows: [
          {
            ...
            '플랫폼': '아이폰 전용',
            '가격': '무료; 음성 입력을 위한 4.99달러 일회성 Pro 잠금 해제',
      ...
      whoShouldUse: {
        ...
        items: [
          ...
          '**구독을 특별히 피하고 싶은 독자.** 무료 등급과 4.99달러 Pro 잠금 해제 모두 일회성이며, 어느 쪽도 반복 청구가 없습니다.',
          '**구독 없이 온디바이스 음성 입력을 원하는 사용자.** Pro 잠금 해제는 개발자에 따르면 로컬에서 실행되는 Whisper 기반 받아쓰기를 위한 4.99달러 일회성 구매입니다.',
      ...
      whoShouldNotUse: {
        ...
        items: [
          ...
          '**앱을 신뢰하기 전에 확립된 실적을 필요로 하는 독자.** Pocket AI는 Private LLM 같은 대안보다 앱스토어에서의 역사가 짧으며, 개발자 자신의 공개에 따르면 2026년 9월 8일 이전 기준으로 평점이 단 6개뿐이었습니다 — 리뷰 수를 중요하게 여기는 독자는 이를 고려해야 합니다.',
        ...
        faqs: [
          {
            ...
            a: '네. 이 앱은 카탈로그의 어떤 모델과도 텍스트 채팅을 하는 데 무료로 다운로드하고 사용할 수 있습니다. 4.99달러의 일회성 앱 내 구매로 온디바이스 Whisper 음성 입력이 잠금 해제되며, 두 등급 모두 구독이 없습니다.',
          },
          {
          ...
          {
            q: '4.99달러 Pro 잠금 해제에는 무엇이 포함되나요?',
            a: '4.99달러 일회성 Pro 잠금 해제는 받아쓰기를 위한 온디바이스 Whisper 음성 입력을 추가합니다. 개발자 자신의 설명에 따르면 음성 입력 외에 추가 모델이나 기능은 추가하지 않습니다. 구독은 없습니다.',
          },
          {
            q: 'Pocket AI는 왜 앱스토어 평점이 이렇게 낮은가요?',
            a: 'PromptQuorum에 대한 개발자의 첫 연락 시점, 즉 2026년 9월 8일 이전 기준으로 앱스토어 목록에는 6개 평점으로 2.8점이 표시되어 있었습니다. 개발자는 2026년 9월 4일에 앱 내 평점 요청 안내가 도입되기 전까지 앱이 사용자에게 평점을 요청하지 않았던 것을 그 원인으로 지목합니다. 이는 개발자 자신의 설명이며 PromptQuorum이 독립적으로 검증하지 않았고, 현재 평점은 변경되었을 수 있습니다 — 최신 수치는 [앱스토어 목록](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702)을 직접 확인하세요.',
          },
          {
      ...
      verdict: {
        ...
        title: '총평',
        content:
          'Pocket AI - No Internet는 간단한 약속을 지킵니다. 계정도 서버 통신도 없이 llama.cpp를 통해 오픈 웨이트 모델을 완전히 기기에서 실행하는 무료 아이폰 앱이며, 구독 대신 진정으로 선택적인 4.99달러 일회성 음성 잠금 해제를 제공합니다. Qwen, Gemma, Llama, Mistral, Phi, DeepSeek 계열을 아우르는 28개 모델 카탈로그는 무료 앱치고는 합리적인 출발점이며, 아이폰 12 최소 사양에 대한 개발자 자신의 이유 — 메모리에 제약을 받는 추론 — 는 모호한 하드웨어 요구 사항이 아니라 그럴듯하고 구체적인 설명입니다. 이 리뷰가 열광적이기보다 신중한 태도를 유지하는 이유는 앱이 아직 새롭기 때문입니다. 앱이 비공개 소스이기 때문에 개인정보 보호 및 오프라인 작동 주장은 독립적인 코드 검토가 아니라 개발자의 말에 근거하며, 개발자 자신이 공개한 단 6개 평점으로 이루어진 2.8점(2026년 9월 8일 이전)은 그 낮은 평점 수가 아무리 잘 설명되었더라도 여전히 구축 중인 실적을 반영합니다. 무료이고 구독이 없으며 그럴듯한 온디바이스 개인정보 보호 방식을 갖춘 채팅 앱을 원하는 독자는 시도해 볼 만합니다. 확립된 앱스토어 실적과 독립적으로 감사 가능한 소스 코드를 더 중요하게 여기는 독자는 대신 [PocketPal AI](/ko/power-local-llm/pocketpal-ai-review)로 시작하거나 [Private LLM](/ko/power-local-llm/private-llm-review)을 위한 예산을 책정해야 합니다.',
      },
      sources: {
        id: 'sources',
        title: '출처',
        items: [
          '[앱스토어의 Pocket AI - No Internet](https://apps.apple.com/us/app/pocket-ai-no-internet/id6759989702) — 가격, 플랫폼 호환성, 평점.',
          '[Pocket AI 공식 사이트](https://mypocketai.app/) — 제품 개요.',
          '2026년 9월 8일 이전에 PromptQuorum에 제공된 Dataweal의 개발자 Faisal과의 직접 서신 — 앱 설명, 모델 카탈로그 수, 가격 모델, 하드웨어 요구 사항에 대한 이유, 오픈소스 여부, 앱스토어 평점 배경.',
        ],
      },
      relatedReading: {
        id: 'related-reading',
