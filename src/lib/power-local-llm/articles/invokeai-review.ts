  ...
  de: {
    ...
    sections: {
      ...
      vsAlternatives: {
        ...
        note: 'Dies ist eine Positionierungs-Zusammenfassung basierend auf den eigenen Dokumentationen und Lizenzdateien der jeweiligen Projekte, Stand 06.09.2026, kein praktischer Benchmark. Siehe [Lokale KI-Bildgenerierung vs. Cloud](/de/power-local-llm/local-ai-image-generation-vs-cloud) für den Vergleich dieser lokalen Optionen mit Cloud-Tools wie Midjourney und Adobe Firefly, und [Stable Diffusion Test](/de/power-local-llm/stable-diffusion-review) für die zugrunde liegende Modellfamilie, die all diese Oberflächen ausführen.',
      },
      whoShouldUse: {
        id: 'who-should-use',
  ...
  es: {
    ...
    sections: {
      ...
      whatIsInvokeAI: {
        ...
        content: [
          'InvokeAI es una aplicación de interfaz que carga los pesos de los modelos Stable Diffusion y FLUX y ejecuta la inferencia en tu propio hardware. Al igual que la WebUI de AUTOMATIC1111, ComfyUI y Fooocus, no incluye los pesos del modelo en sí — estos provienen de Stability AI, Black Forest Labs y otros editores de modelos, y se descargan por separado (consulta el [Análisis de Stable Diffusion](/es/power-local-llm/stable-diffusion-review) para conocer la familia de modelos subyacente y sus propios términos de licencia).',
  ...
  fr: {
    ...
    sections: {
      ...
      vsAlternatives: {
        ...
        note: 'Ceci est un résumé de positionnement basé sur la documentation et les fichiers de licence propres à chaque projet au 06/09/2026, pas un benchmark pratique. Voir [Génération locale d\'images IA vs. cloud](/fr/power-local-llm/local-ai-image-generation-vs-cloud) pour la comparaison de ces options locales avec des outils cloud comme Midjourney et Adobe Firefly, et [Test de Stable Diffusion](/fr/power-local-llm/stable-diffusion-review) pour la famille de modèles sous-jacente que ces interfaces exécutent toutes.',
      },
      whoShouldUse: {
        id: 'who-should-use',
  ...
  ja: {
    ...
    sections: {
      ...
      whatIsInvokeAI: {
        ...
        content: [
          'InvokeAIはStable DiffusionとFLUXのモデルの重みを読み込み、自分のハードウェア上で推論を実行するフロントエンドアプリケーションです。AUTOMATIC1111のStable Diffusion WebUI、ComfyUI、Fooocusと同様に、モデルの重み自体は含まれません。これらはStability AI、Black Forest Labs、その他のモデル発行元から提供され、別途ダウンロードする必要があります(基盤となるモデルファミリーとそのライセンス条件については[Stable Diffusionレビュー](/ja/power-local-llm/stable-diffusion-review)を参照)。',
      ...
      vsAlternatives: {
        ...
        note: 'これは2026年9月6日時点の各プロジェクト自身のドキュメントとライセンスファイルに基づく位置づけの要約であり、実機ベンチマークではありません。これらのローカルの選択肢がMidjourneyやAdobe Fireflyなどのクラウドツールとどう比較されるかについては[ローカルAI画像生成 vs. クラウド](/ja/power-local-llm/local-ai-image-generation-vs-cloud)を、これらすべてのUIが実行する基盤モデルファミリーについては[Stable Diffusionレビュー](/ja/power-local-llm/stable-diffusion-review)を参照してください。',
      },
      whoShouldUse: {
        id: 'who-should-use',
  ...
  pt: {
    ...
    sections: {
      ...
      whatIsInvokeAI: {
        ...
        content: [
          'O InvokeAI é uma aplicação de interface que carrega os pesos dos modelos Stable Diffusion e FLUX e executa a inferência no seu próprio hardware. Assim como a WebUI do AUTOMATIC1111, o ComfyUI e o Fooocus, ele não inclui os pesos do modelo em si — eles vêm da Stability AI, da Black Forest Labs e de outros editores de modelos, e são baixados separadamente (veja a [Análise do Stable Diffusion](/pt/power-local-llm/stable-diffusion-review) para o contexto sobre a família de modelos subjacente e seus próprios termos de licença).',
      ...
      vsAlternatives: {
        ...
        note: 'Este é um resumo de posicionamento baseado na documentação e nos arquivos de licença de cada projeto em 06/09/2026, não um benchmark prático. Veja [Geração local de imagens por IA vs. nuvem](/pt/power-local-llm/local-ai-image-generation-vs-cloud) para a comparação dessas opções locais com ferramentas em nuvem como Midjourney e Adobe Firefly, e a [Análise do Stable Diffusion](/pt/power-local-llm/stable-diffusion-review) para a família de modelos subjacente que todas essas interfaces executam.',
      },
      whoShouldUse: {
        id: 'who-should-use',
  ...
  ar: {
    ...
    sections: {
      ...
      whatIsInvokeAI: {
        ...
        content: [
          'InvokeAI هو تطبيق واجهة أمامية يحمّل أوزان نماذج Stable Diffusion وFLUX وينفذ الاستدلال على أجهزتك الخاصة. مثل واجهة AUTOMATIC1111 وComfyUI وFooocus، لا يتضمن أوزان النموذج نفسها — فهي تأتي من Stability AI وBlack Forest Labs وناشري نماذج آخرين، ويتم تنزيلها بشكل منفصل (راجع [مراجعة Stable Diffusion](/ar/power-local-llm/stable-diffusion-review) للاطلاع على خلفية عائلة النماذج الأساسية وشروط ترخيصها الخاصة).',
      ...
      vsAlternatives: {
        ...
        note: 'هذا ملخص لتحديد المواقع استناداً إلى وثائق كل مشروع وملفات الترخيص الخاصة به حتى تاريخ 06-09-2026، وليس اختباراً عملياً. راجع [توليد الصور بالذكاء الاصطناعي محلياً مقابل السحابة](/ar/power-local-llm/local-ai-image-generation-vs-cloud) لمعرفة كيف تقارَن هذه الخيارات المحلية بأدوات سحابية مثل Midjourney وAdobe Firefly، و[مراجعة Stable Diffusion](/ar/power-local-llm/stable-diffusion-review) لعائلة النماذج الأساسية التي تشغّلها كل هذه الواجهات.',
      },
      whoShouldUse: {
        id: 'who-should-use',
  ...
  zh: {
    ...
    sections: {
      ...
      whatIsInvokeAI: {
        ...
        content: [
          'InvokeAI是一个前端应用程序,加载Stable Diffusion和FLUX模型权重并在你自己的硬件上执行推理。与AUTOMATIC1111的Stable Diffusion WebUI、ComfyUI和Fooocus一样,它本身不包含模型权重——这些权重来自Stability AI、Black Forest Labs和其他模型发布方,需要单独下载(有关底层模型系列及其自身许可条款的背景信息,请参见[Stable Diffusion评测](/zh/power-local-llm/stable-diffusion-review))。',
      ...
      vsAlternatives: {
        ...
        note: '这是截至2026年9月6日基于各项目自身文档和许可证文件得出的定位总结,而非实测基准测试。有关这些本地方案与Midjourney、Adobe Firefly等云端工具的比较,请参见[本地AI图像生成与云端对比](/zh/power-local-llm/local-ai-image-generation-vs-cloud);有关这些界面背后共同运行的模型系列,请参见[Stable Diffusion评测](/zh/power-local-llm/stable-diffusion-review)。',
      },
      whoShouldUse: {
        id: 'who-should-use',
        title: '谁适合使用InvokeAI',
  ...
  ko: {
    ...
    sections: {
      ...
      vsAlternatives: {
        ...
        note: '이는 2026년 9월 6일 기준으로 각 프로젝트 자체의 문서와 라이선스 파일을 근거로 한 포지셔닝 요약이며, 실제 벤치마크가 아닙니다. 이러한 로컬 옵션이 Midjourney, Adobe Firefly 같은 클라우드 도구와 어떻게 비교되는지는 [로컬 AI 이미지 생성 대 클라우드](/ko/power-local-llm/local-ai-image-generation-vs-cloud)를, 이 모든 UI가 실행하는 기본 모델 계열에 대해서는 [Stable Diffusion 리뷰](/ko/power-local-llm/stable-diffusion-review)를 참고하세요.',
      },
      whoShouldUse: {
        id: 'who-should-use',
