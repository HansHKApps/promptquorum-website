...
export const article: Partial<Record<Language, LLMArticle>> = {
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          '**Tool-calling reliability** depends on the model, not the harness. Qwen3-Coder 30B and DeepSeek-V3 are the most reliable tool-callers in May 2026; smaller models (7B class) regularly emit malformed tool calls regardless of which harness wraps them.',
      ...
      quickFacts: {
        ...
        items: [
          ...
          '**Tool-call reliability is a model property, not a harness property.** Qwen3-Coder 30B and DeepSeek-V3 are reliable in all three; 7B-class models fail in all three.',
      ...
      continueDevDeep: {
        ...
        items: [
          ...
          '**Local model setup:** native Ollama support is one config block. llama.cpp, LM Studio, and any OpenAI-compatible endpoint also work. Separate models for autocomplete (small, fast — Qwen3-Coder 1.5B class) and chat (larger — Qwen3-Coder 30B, DeepSeek-V3) is the standard pattern.',
      ...
      clineDeep: {
        ...
        items: [
          ...
          '**Local model setup:** any OpenAI-compatible endpoint. Ollama is one click. For llama.cpp or vLLM, point Cline at the OpenAI-compatible server URL. The agent loop is token-heavy — reserve a model with at least a 32K context window (Qwen3-Coder 30B at 32K, DeepSeek-V3 at 128K).',
      ...
      editModel: {
        ...
        imageCaption: 'Edit model reliability by harness: on 7B models, Continue.dev is most forgiving, Aider rejects mismatched blocks, Cline stalls on malformed XML; on 30B+ coding models (Qwen3-Coder, DeepSeek-V3), all three are reliable.',
        items: [
          ...
          '**Reliability ranking on a 30B+ tool-calling model (Qwen3-Coder 30B, DeepSeek-V3):** all three converge — the model has the capacity to emit precise diff blocks and well-formed tool calls consistently.',
      ...
      toolCalls: {
        ...
        items: [
          '**Models that tool-call reliably for these harnesses (May 2026):** Qwen3-Coder 30B (best-in-class on Cline\'s XML format), DeepSeek-V3 (large context, strong on multi-step plans), Llama 3.3 70B (slow but reliable), Codestral 22B (fast, smaller plan horizon).',
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[Bodega One Code Review (2026)](/power-local-llm/bodega-one-code-review) — a standalone BYOLLM IDE that bundles editor, chat, and agent instead of extending VS Code like the tools above.',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          '**Tool-Call-Zuverlässigkeit** hängt vom Modell ab, nicht vom Harness. Qwen3-Coder 30B und DeepSeek-V3 sind die zuverlässigsten Tool-Caller im Mai 2026; kleinere Modelle (7B-Klasse) geben regelmäßig malformed Tool-Calls aus, unabhängig davon, welcher Harness sie umhüllt.',
      ...
      quickFacts: {
        ...
        items: [
          ...
          '**Tool-Call-Zuverlässigkeit ist eine Modell-Eigenschaft, keine Harness-Eigenschaft.** Qwen3-Coder 30B und DeepSeek-V3 sind zuverlässig in allen drei; 7B-Klasse-Modelle fehlschlagen in allen drei.',
      ...
      editModel: {
        ...
        imageCaption: 'Bearbeitungsmodell-Zuverlässigkeit: Bei 7B-Modellen ist Continue.dev am tolerantesten, Aider lehnt falsche Blöcke ab, Cline stoppt bei fehlerhaftem XML; bei 30B+ Coding-Modellen (Qwen3-Coder, DeepSeek-V3) sind alle drei zuverlässig.',
        items: [
          ...
          '**Zuverlässigkeits-Ranking bei einem 30B+ Tool-Calling-Modell (Qwen3-Coder 30B, DeepSeek-V3):** alle drei konvergieren — das Modell hat die Kapazität, präzise Diff-Blöcke und wohlgeformte Tool-Calls konsistent auszugeben.',
          'Für strukturierte Prompting-Techniken, die Code-Generierung unabhängig davon verbessern, welcher Harness sie umhüllt, sehen Sie [Schreiben Sie besseren Code mit KI](/de/prompt-engineering/write-better-code-with-ai).',
        ],
      },
      undo: {
        id: 'undo',
      ...
      context: {
        ...
        items: [
          ...
          'Für eine tiefere Erklärung von Kontext-Fenstern und warum Modelle mittleren Kontext Informationen verlieren, sehen Sie [Kontext-Fenster erklärt: Warum KI vergisst](/de/prompt-engineering/context-windows-explained-why-ai-forgets).',
        ],
      },
      toolCalls: {
        id: 'tool-calls',
        ...
        items: [
          '**Modelle, die diese Harnesses zuverlässig Tool-Call (Mai 2026):** Qwen3-Coder 30B (Best-in-Class auf Clines XML-Format), DeepSeek-V3 (großer Kontext, stark bei Multi-Step-Plänen), Llama 3.3 70B (langsam aber zuverlässig), Codestral 22B (schnell, kleinerer Plan-Horizont).',
      ...
      relatedReading: {
        ...
        items: [
          '[GitHub Copilot durch einen lokalen LLM ersetzen](/de/power-local-llm/replace-github-copilot-with-local-llm) — breiterer Ersatz-Kontext, einschließlich Kosten und Privacy-Mathematik.',
          '[Best Local Coding Models in 2026](/de/power-local-llm/best-local-coding-models-2026) — die Modell-Seite der Gleichung: Qwen3-Coder vs DeepSeek vs Codestral auf realen Benchmarks.',
          '[Local AI Agents With MCP in 2026](/de/power-local-llm/local-ai-agents-with-mcp-2026) — für Nutzer, die erweiterte Tool-Calling jenseits der obigen Harnesses möchten.',
          '[Best Local Models for Tool Calling in 2026](/de/power-local-llm/best-local-models-tool-calling-2026) — angrenzende technische Tiefe auf Tool-Call-Zuverlässigkeit.',
          '[llama.cpp vs Ollama vs vLLM](/de/local-llms/llamacpp-vs-ollama-vs-vllm) — die Inference-Engine-Schicht unter allen drei Harnesses.',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          "**La fiabilité des appels d'outils** dépend du modèle, pas de l'outil. Qwen3-Coder 30B et DeepSeek-V3 sont les appelants d'outils les plus fiables en mai 2026 ; les modèles plus petits (classe 7B) émettent régulièrement des appels d'outils malformés, indépendamment de l'outil qui les enveloppe.",
      ...
      quickFacts: {
        ...
        items: [
          ...
          "**La fiabilité des appels d'outils est une propriété du modèle, pas de l'outil.** Qwen3-Coder 30B et DeepSeek-V3 sont fiables dans les trois ; les modèles de classe 7B échouent dans les trois.",
      ...
      continueDevDeep: {
        ...
        items: [
          ...
          "**Configuration du modèle local :** le support natif d'Ollama est un bloc de configuration. llama.cpp, LM Studio et tout endpoint compatible OpenAI fonctionnent également. Le motif standard consiste à exécuter des modèles distincts pour l'autocomplétion (petit, rapide — classe Qwen3-Coder 1.5B) et le chat (plus grand — Qwen3-Coder 30B, DeepSeek-V3).",
      ...
      clineDeep: {
        ...
        items: [
          ...
          "**Configuration du modèle local :** tout endpoint compatible OpenAI. Ollama est un clic. Pour llama.cpp ou vLLM, pointez Cline vers l'URL du serveur compatible OpenAI. La boucle d'agent consomme beaucoup de jetons — réservez un modèle avec au moins une fenêtre de contexte 32K (Qwen3-Coder 30B à 32K, DeepSeek-V3 à 128K).",
      ...
      editModel: {
        ...
        imageCaption: "Fiabilité du modèle d'édition : sur les modèles 7B, Continue.dev est le plus tolérant, Aider rejette les blocs incorrects, Cline bloque sur XML malformé ; sur les modèles 30B+ (Qwen3-Coder, DeepSeek-V3), les trois sont fiables.",
        items: [
          ...
          "**Classement de fiabilité sur un modèle d'appels d'outils 30B+ (Qwen3-Coder 30B, DeepSeek-V3) :** les trois convergent — le modèle a la capacité d'émettre des blocs de diff précis et des appels d'outils bien formés de manière cohérente.",
          "Pour les techniques d'invite structurée qui améliorent la génération de code indépendamment de l'outil qui les enveloppe, voir [Écrire du meilleur code avec l'IA](/fr/prompt-engineering/write-better-code-with-ai).",
        ],
      },
      undo: {
        id: 'undo',
      ...
      context: {
        ...
        items: [
          ...
          "Pour une explication plus approfondie des fenêtres de contexte et pourquoi les modèles oublient les informations au milieu du contexte, voir [Fenêtres de contexte expliquées : Pourquoi l'IA oublie](/fr/prompt-engineering/context-windows-explained-why-ai-forgets).",
        ],
      },
      toolCalls: {
        id: 'tool-calls',
        ...
        items: [
          "**Modèles qui font des appels d'outils fiables pour ces outils (mai 2026) :** Qwen3-Coder 30B (best-in-class sur le format XML de Cline), DeepSeek-V3 (grand contexte, fort sur les plans multi-étapes), Llama 3.3 70B (lent mais fiable), Codestral 22B (rapide, horizon de plan plus petit).",
      ...
      relatedReading: {
        ...
        items: [
          '[Remplacer GitHub Copilot avec un LLM local](/fr/power-local-llm/replace-github-copilot-with-local-llm) — contexte de remplacement plus large, y compris le calcul des coûts et de la confidentialité.',
          '[Meilleurs modèles de codage locaux 2026](/fr/power-local-llm/best-local-coding-models-2026) — le côté modèle de l\'équation : Qwen3-Coder vs DeepSeek vs Codestral sur les benchmarks réels.',
          '[Agents IA locaux avec MCP en 2026](/fr/power-local-llm/local-ai-agents-with-mcp-2026) — pour les utilisateurs qui veulent des appels d\'outils étendus au-delà des outils ci-dessus.',
          '[Meilleurs modèles locaux pour les appels d\'outils 2026](/fr/power-local-llm/best-local-models-tool-calling-2026) — profondeur technique adjacente sur la fiabilité des appels d\'outils.',
          '[llama.cpp vs Ollama vs vLLM](/fr/local-llms/llamacpp-vs-ollama-vs-vllm) — la couche du moteur d\'inférence sous les trois outils.',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          '**ツール呼び出し信頼性**はモデルに依存し、ツールには依存しません。Qwen3-Coder 30B と DeepSeek-V3 は 2026 年 5 月の最も信頼できるツールコーラーです。より小さいモデル（7B クラス）は定期的に不正なツール呼び出しを出力し、どのツールでもラップされます。',
      ...
      quickFacts: {
        ...
        items: [
          ...
          '**ツール呼び出し信頼性はモデル特性であり、ツール特性ではありません。** Qwen3-Coder 30B と DeepSeek-V3 は 3 つすべてで信頼できます。7B クラスモデルは 3 つすべてで失敗します。',
      ...
      continueDevDeep: {
        ...
        items: [
          ...
          '**ローカルモデル設定：** ネイティブ Ollama サポートは 1 つの設定ブロックです。llama.cpp、LM Studio、OpenAI 互換エンドポイントも動作します。標準的なパターンは、オートコンプリート用の小さいモデル（小さい、高速—Qwen3-Coder 1.5B クラス）とチャット用のより大きいモデル（Qwen3-Coder 30B、DeepSeek-V3）を実行することです。',
      ...
      clineDeep: {
        ...
        items: [
          ...
          '**ローカルモデル設定：** OpenAI 互換エンドポイント。Ollama はワンクリック。llama.cpp または vLLM の場合、Cline を OpenAI 互換サーバー URL にポイントします。エージェントループはトークン集約的です。最低でも 32K コンテキストウィンドウを備えたモデルを予約します（Qwen3-Coder 30B at 32K、DeepSeek-V3 at 128K）。',
      ...
      editModel: {
        ...
        imageCaption: '編集モデルの信頼性：7Bモデルでは、Continue.devが最も寛容で、Aiderは不一致ブロックを拒否、ClineはXML不正でループ停止。30B+コーディングモデル（Qwen3-Coder、DeepSeek-V3）では3つすべて信頼性あり。',
        items: [
          ...
          '**30B+ ツール呼び出しモデル（Qwen3-Coder 30B、DeepSeek-V3）でのランキング：** 3 つすべてが収束します。モデルは一貫して正確な差分ブロックと形式が正しいツール呼び出しを出力する能力を備えています。',
          'ツールが包むもの に関係なくコード生成を改善する構造化プロンプト技術については、[AI でより良いコードを書く](/ja/prompt-engineering/write-better-code-with-ai) を参照してください。',
        ],
      },
      undo: {
        id: 'undo',
      ...
      context: {
        ...
        items: [
          ...
          'コンテキストウィンドウの詳細な説明と、モデルが中文脈で情報を失う理由については、[コンテキストウィンドウ説明：AI が忘れる理由](/ja/prompt-engineering/context-windows-explained-why-ai-forgets) を参照してください。',
        ],
      },
      toolCalls: {
        id: 'tool-calls',
        ...
        items: [
          '**これらのツール（2026 年 5 月）に対して信頼できるツール呼び出しをするモデル：** Qwen3-Coder 30B（Cline の XML 形式に対してクラス最高）、DeepSeek-V3（大きなコンテキスト、マルチステップのプランで強い）、Llama 3.3 70B（遅いが信頼できる）、Codestral 22B（高速で、より小さい計画展望）。',
      ...
      relatedReading: {
        ...
        title: '関連資料',
        items: [
          '[GitHub Copilot をローカル LLM に置き換える](/ja/power-local-llm/replace-github-copilot-with-local-llm) — より広い交換コンテキスト、コストとプライバシー数学を含む。',
          '[2026 年ベストローカルコーディングモデル](/ja/power-local-llm/best-local-coding-models-2026) — 方程式のモデル側：実際のベンチマーク上の Qwen3-Coder vs DeepSeek vs Codestral。',
          '[2026 年 MCP を備えたローカル AI エージェント](/ja/power-local-llm/local-ai-agents-with-mcp-2026) — 上記のツールを超えた拡張ツール呼び出しを希望するユーザー向け。',
          '[2026 年ツール呼び出しのベストローカルモデル](/ja/power-local-llm/best-local-models-tool-calling-2026) — ツール呼び出し信頼性に関する隣接する技術的深さ。',
          '[llama.cpp vs Ollama vs vLLM](/ja/local-llms/llamacpp-vs-ollama-vs-vllm) — 3 つのツール下の推論エンジン層。',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          '**工具调用可靠性**取决于模型，而不是工具。Qwen3-Coder 30B 和 DeepSeek-V3 是 2026 年 5 月最可靠的工具调用者；较小的模型（7B 类）无论哪个工具包装它们都会定期发出格式错误的工具调用。',
      ...
      quickFacts: {
        ...
        items: [
          ...
          '**工具调用可靠性是一个模型属性，不是工具属性。** Qwen3-Coder 30B 和 DeepSeek-V3 在三个中都可靠；7B 类模型在三个中都失败。',
      ...
      continueDevDeep: {
        ...
        items: [
          ...
          '**本地模型设置：** 原生 Ollama 支持是一个配置块。llama.cpp、LM Studio 和任何 OpenAI 兼容端点也可以工作。标准模式是运行用于自动完成的小型模型（小、快 — Qwen3-Coder 1.5B 类）和用于聊天的较大型模型（Qwen3-Coder 30B、DeepSeek-V3）。',
      ...
      clineDeep: {
        ...
        items: [
          ...
          '**本地模型设置：** 任何 OpenAI 兼容端点。Ollama 是一键。对于 llama.cpp 或 vLLM，将 Cline 指向 OpenAI 兼容服务器 URL。代理循环是令牌密集的 — 预留至少 32K 上下文窗口的模型（Qwen3-Coder 30B at 32K、DeepSeek-V3 at 128K）。',
      ...
      editModel: {
        ...
        imageCaption: '编辑模式可靠性：7B模型上，Continue.dev最宽容，Aider拒绝不匹配块，Cline因XML格式错误停滞；30B+编程模型（Qwen3-Coder、DeepSeek-V3）上，三者均可靠。',
        items: [
          ...
          '**30B+ 工具调用模型（Qwen3-Coder 30B、DeepSeek-V3）上的排名：** 三个都收敛 — 模型具有一致地输出精确差异块和格式正确工具调用的能力。',
          '对于改进代码生成的结构化提示技术（无论哪个工具包装它们），请参见 [用 AI 写更好的代码](/zh/prompt-engineering/write-better-code-with-ai)。',
        ],
      },
      undo: {
        id: 'undo',
      ...
      context: {
        ...
        items: [
          ...
          '对于上下文窗口的深入解释和为什么模型在中文脉络中丢失信息，请参见 [上下文窗口解释：为什么 AI 忘记](/zh/prompt-engineering/context-windows-explained-why-ai-forgets)。',
        ],
      },
      toolCalls: {
        id: 'tool-calls',
        ...
        items: [
          '**对这些工具可靠地进行工具调用的模型（2026 年 5 月）：** Qwen3-Coder 30B（Cline 的 XML 格式最好的），DeepSeek-V3（大上下文，多步计划强），Llama 3.3 70B（慢但可靠），Codestral 22B（快，较小的计划视野）。',
      ...
      relatedReading: {
        ...
        title: '相关阅读',
        items: [
          '[使用本地 LLM 替换 GitHub Copilot](/zh/power-local-llm/replace-github-copilot-with-local-llm) — 更广泛的替换背景，包括成本和隐私数学。',
          '[2026 年最佳本地编码模型](/zh/power-local-llm/best-local-coding-models-2026) — 方程式的模型端：实际基准上的 Qwen3-Coder vs DeepSeek vs Codestral。',
          '[2026 年 MCP 的本地 AI 代理](/zh/power-local-llm/local-ai-agents-with-mcp-2026) — 对于想要上述工具之外的扩展工具调用的用户。',
          '[2026 年工具调用的最佳本地模型](/zh/power-local-llm/best-local-models-tool-calling-2026) — 工具调用可靠性的相邻技术深度。',
          '[llama.cpp vs Ollama vs vLLM](/zh/local-llms/llamacpp-vs-ollama-vs-vllm) — 三个工具下的推论引擎层。',
  ...
  es: {
    ...
    audience:
      'Desarrolladores que ejecutan un LLM de código local (Qwen3-Coder, DeepSeek Coder, Codestral, Llama 3.3) y necesitan elegir una herramienta que encaje con su editor y flujo de trabajo de proyecto.',
    readTime: '14 min de lectura',
    educationalLevel: 'Advanced',
    primaryTerm: 'agente de código local',
    targetKeywords: [
      'continue.dev vs cline',
      'cline vs aider',
      'mejor agente de código local 2026',
      'herramienta de código llm local',
      'aider llm local',
      'continue.dev llm local',
    ],
    leadAnswerBlock:
      '**Continue.dev, Cline y Aider resuelven problemas distintos con el mismo modelo local. Continue.dev está construido en torno al autocompletado y un panel de chat integrado en el IDE — ideal para flujos de trabajo de edición mientras escribes en VS Code o JetBrains. Cline es un agente autónomo de VS Code que planifica, edita y ejecuta comandos en múltiples archivos con puertas de aprobación explícitas — ideal para refactorizaciones de múltiples archivos y búsquedas de errores exploratorias. Aider es un CLI orientado al terminal y nativo de Git que convierte cada edición en un commit con diffs de búsqueda y reemplazo quirúrgicos — ideal para trabajo a escala de repositorio y revisores que quieren cada cambio reflejado en `git log`. Elige según la primitiva de flujo de trabajo (autocompletado vs bucle de agente vs diff de Git), no según cuál herramienta suena más potente.**',
    quickAnswerTop: {
      es: {
        ...
        bullets: [
          ...
          'Aider — CLI de terminal con commits nativos de Git y diffs de búsqueda y reemplazo. Ideal para trabajo a escala de repositorio revisable y con soporte para deshacer.',
    ...
    toc: [
      ...
      { label: 'Continue.dev: Análisis en profundidad', anchor: '#continue-dev' },
      { label: 'Cline: Análisis en profundidad', anchor: '#cline' },
      { label: 'Aider: Análisis en profundidad', anchor: '#aider' },
      { label: 'Modelo de edición: cómo escribe código cada herramienta', anchor: '#edit-model' },
      { label: 'Deshacer y revertir', anchor: '#undo' },
      { label: 'Estrategia de ventana de contexto', anchor: '#context' },
      { label: 'Fiabilidad de llamadas a herramientas', anchor: '#tool-calls' },
      { label: 'Integración con IDE', anchor: '#ide-integration' },
      { label: 'Adecuación al flujo de trabajo por caso de uso', anchor: '#workflow-fit' },
    ...
    gammaEmbedUrl: '/presentations/continue-dev-vs-cline-vs-aider-local-static.html',
    gammaDescription: 'La presentación a continuación cubre: cómo difieren Continue.dev, Cline y Aider como herramientas de código local (autocompletado vs agente autónomo vs diffs nativos de git), una tabla comparativa de funciones, fiabilidad del modelo de edición por herramienta y tamaño de modelo, estrategia de ventana de contexto (32K vs 128K), integración con IDE (VS Code, JetBrains, Vim/SSH) y 6 errores comunes al elegir una herramienta. Descarga el PDF como tarjeta de referencia para agentes de código local.',
    sections: {
      tldr: {
        id: 'key-takeaways',
        isTldr: true,
        items: [
          ...
          '**La fiabilidad de las llamadas a herramientas** depende del modelo, no de la herramienta. Qwen3-Coder 30B y DeepSeek-V3 son los llamadores de herramientas más fiables en mayo de 2026; los modelos más pequeños (clase 7B) emiten regularmente llamadas a herramientas mal formadas independientemente de cuál herramienta los envuelva.',
          '**La estrategia de ventana de contexto** difiere notablemente: Continue.dev se apoya en el índice de base de código integrado y el contexto anclado con `@`, Cline transmite el contenido completo de los archivos a la conversación (alto uso de tokens), Aider usa un mapa de repositorio tree-sitter más `/add` y `/drop` explícitos para mantener pequeño el conjunto de trabajo.',
          'Para un desarrollador individual, la **adecuación al flujo de trabajo** domina los resultados más que cualquier benchmark. La herramienta que realmente mantienes abierta es la que deberías elegir.',
        ],
      },
      quickFacts: {
        id: 'quick-facts',
        ...
        items: [
          ...
          '**La fiabilidad de las llamadas a herramientas es una propiedad del modelo, no de la herramienta.** Qwen3-Coder 30B y DeepSeek-V3 son fiables en las tres; los modelos de clase 7B fallan en las tres.',
          '**Presupuesto de contexto:** Continue.dev y Aider se mantienen cómodos con un modelo de contexto de 32K; Cline necesita **128K** para tareas de múltiples archivos no triviales.',
          '**Elige según el editor:** JetBrains → Continue.dev o Aider. Vim/Neovim o SSH → Aider. VS Code con días de mucho autocompletado → Continue.dev. VS Code con refactorizaciones de múltiples archivos → Cline.',
        ],
      },
      comparisonTable: {
        id: 'comparison-table',
        title: 'Cómo se comparan Continue.dev, Cline y Aider en 2026',
        content:
          'Las tres herramientas son open-source, gratuitas y funcionan con cualquier endpoint de LLM local. Las diferencias estructurales a continuación determinan cuál encaja con tu flujo de trabajo.',
        image: '/images/coding-agent-comparison-es.svg',
        imageCaption: 'Continue.dev vs Cline vs Aider: Continue.dev para autocompletado en VS Code/JetBrains (contexto 32K), Cline para agentes autónomos de múltiples archivos en VS Code (128K recomendado), Aider para commits nativos de git en terminal (contexto 32K).',
        snippetBlocks: [
          {
            type: 'one-sentence',
            text: 'Continue.dev es autocompletado estilo Copilot, Cline es un agente autónomo que edita entre archivos, Aider es una herramienta de terminal que hace commit de cada edición a Git — elige según el flujo de trabajo, no según el benchmark.',
          },
          {
            type: 'plain-terms',
        ...
        rows: [
          {
            ...
            'Continue.dev': 'Extensión de VS Code y JetBrains',
            'Cline': 'Extensión de VS Code',
            'Aider': 'CLI de terminal (cualquier editor al lado)',
          },
          {
            'Característica': 'Primitiva de flujo de trabajo',
            'Continue.dev': 'Autocompletado + chat integrado en IDE',
            'Cline': 'Bucle de agente autónomo con puertas de aprobación',
            'Aider': 'Ediciones de diff nativo de Git, cada cambio un commit',
          },
          {
            'Característica': 'Modelo de edición',
            'Continue.dev': 'FIM en línea + aplicar al archivo desde el chat',
            'Cline': 'Llamadas a herramientas (read_file, write_to_file, replace_in_file)',
            'Aider': 'Bloques de diff de búsqueda y reemplazo; modo "entero" para archivos pequeños',
          },
          {
            'Característica': 'Ediciones de múltiples archivos',
            'Continue.dev': 'Manual: el chat sugiere, tú aplicas por archivo',
            'Cline': 'Nativo: el agente edita entre archivos en una sola tarea',
            'Aider': 'Nativo: nombra archivos en el prompt, las ediciones aterrizan en un commit',
          },
          {
            'Característica': 'Deshacer / Revertir',
            'Continue.dev': 'Pila de deshacer de VS Code en las ediciones aplicadas',
            'Cline': 'Revertir por edición en la interfaz del agente; Git para reversión completa',
            'Aider': '`/undo` revierte el último commit; historial completo de git debajo',
          },
          {
            'Característica': 'Protocolo de llamada a herramientas',
            'Continue.dev': 'Herramientas de OpenAI (function-calling) en modo Agente',
            'Cline': 'Bloques de herramientas XML personalizados con esquema estricto',
            'Aider': 'Sin llamadas a herramientas; indica al modelo que emita bloques de diff',
          },
          {
            'Característica': 'Contexto de base de código',
            'Continue.dev': 'Índice local (TF-IDF + embeddings) + proveedores `@`',
            'Cline': 'Transmite el contenido completo de archivos a la conversación',
            'Aider': 'Mapa de repositorio Tree-sitter + `/add` y `/drop` manual',
          },
          {
            'Característica': 'Aprobación / Seguridad',
            'Continue.dev': 'Vista previa de diff antes de cada aplicación',
            'Cline': 'Aprobación por paso; listas de permisos de auto-aprobación configurables',
            'Aider': 'Vista previa de diff en terminal; el paso de commit es automático',
          },
          {
            'Característica': 'Mejor para',
            'Continue.dev': 'Editar mientras escribes, chat integrado en IDE, días de mucho autocompletado',
            'Cline': 'Refactorizaciones de múltiples archivos, depuración exploratoria, tareas de repositorio',
            'Aider': 'Cambios revisables a escala de repositorio, flujos de trabajo orientados al terminal',
          },
        ],
      },
      whichOne: {
        ...
        content:
          '**La herramienta correcta la decide tu editor, tu tolerancia a las ediciones autónomas y cómo revisas los cambios.** Usa este atajo.',
        image: '/images/coding-agent-which-one-es.svg',
        imageCaption: 'Qué herramienta de código local elegir: Continue.dev para autocompletado y chat en IDE (VS Code + JetBrains), Cline para bucles de agente de múltiples archivos y ejecución de comandos, Aider para flujos de trabajo nativos de git en terminal y entornos SSH.',
        columns: ['Tu situación', 'Elige'],
        rows: [
          { 'Tu situación': 'Vivo en VS Code o un IDE de JetBrains y quiero autocompletado estilo Copilot', 'Elige': 'Continue.dev' },
          { 'Tu situación': 'Quiero un panel de chat lateral que acceda a mi base de código sin subir archivos manualmente', 'Elige': 'Continue.dev' },
          { 'Tu situación': 'Quiero un agente que planifique una refactorización en múltiples archivos y pregunte antes de cada paso', 'Elige': 'Cline' },
          { 'Tu situación': 'Quiero que el modelo pueda ejecutar comandos y leer la salida del terminal como parte de su bucle', 'Elige': 'Cline' },
          { 'Tu situación': 'Reviso los cambios con `git diff` y quiero que cada edición se haga commit automáticamente', 'Elige': 'Aider' },
          { 'Tu situación': 'Trabajo por SSH o en un entorno orientado al terminal', 'Elige': 'Aider' },
          { 'Tu situación': 'Necesito los tres flujos de trabajo ocasionalmente — elige el más sencillo primero', 'Elige': 'Empieza con Continue.dev; agrega Aider cuando necesites diffs nativos de Git' },
        ],
      },
      continueDevDeep: {
        id: 'continue-dev',
        title: 'Continue.dev: La elección orientada al autocompletado',
        content:
          '**Continue.dev es el análogo open-source más cercano a GitHub Copilot para modelos locales.** Su superficie principal es el editor: completado con Tab mientras escribes más un panel de chat lateral que conoce tu base de código.',
        items: [
          ...
          '**Configuración de modelo local:** el soporte nativo de Ollama es un bloque de configuración. llama.cpp, LM Studio y cualquier endpoint compatible con OpenAI también funcionan. El patrón estándar es ejecutar modelos separados para autocompletado (pequeño, rápido — clase Qwen3-Coder 1.5B) y chat (más grande — Qwen3-Coder 30B, DeepSeek-V3).',
      ...
      clineDeep: {
        id: 'cline',
        title: 'Cline: La elección de agente autónomo',
        content:
          '**Cline trata el código como una tarea de agente, no como una serie de ediciones.** Le das un objetivo; planifica, lee archivos, los edita, ejecuta comandos y te pide que apruebes cada paso.',
        items: [
          '**Ruta de instalación:** VS Code Marketplace (sin versión para JetBrains). La configuración se hace por GUI dentro del panel de la extensión — proveedor, modelo, configuración de auto-aprobación, instrucciones personalizadas.',
          '**Primitiva de flujo de trabajo:** bucle de agente autónomo con modos Plan y Actuar explícitos. El modo Plan razona sobre la tarea sin escribir archivos; el modo Actuar ejecuta el plan con llamadas a herramientas. Aparece una puerta de aprobación entre pasos a menos que coloques herramientas en una lista de permisos de auto-aprobación.',
          '**Configuración de modelo local:** cualquier endpoint compatible con OpenAI. Ollama es un clic. Para llama.cpp o vLLM, apunta Cline a la URL del servidor compatible con OpenAI. El bucle de agente consume muchos tokens — reserva un modelo con al menos una ventana de contexto de 32K (Qwen3-Coder 30B en 32K, DeepSeek-V3 en 128K).',
          '**Superficie de herramientas:** `read_file`, `write_to_file`, `replace_in_file`, `execute_command`, `list_files`, `search_files`, `browser_action`, `ask_followup_question`, `attempt_completion`. El conjunto es pequeño y bien documentado; los esquemas de herramientas usan un formato XML personalizado que el modelo debe emitir correctamente.',
          '**Modelo de edición:** para archivos de pocas centenas de líneas, Cline escribe el archivo completo. Para archivos más grandes, emite operaciones `replace_in_file` con bloques de búsqueda y reemplazo explícitos. Los diffs se muestran en el panel antes de cada escritura.',
          '**Donde destaca:** refactorizaciones de múltiples archivos, tareas exploratorias de "¿por qué falla este test?", flujos de trabajo de "añadir una función que toca tres capas". El más potente de los tres en tareas con más de cinco ediciones de archivos.',
          '**Donde se queda corto:** el consumo de tokens es alto — el contenido completo de los archivos se transmite a la conversación en cada lectura. Las sesiones largas agotan los modelos de contexto 32K; se recomiendan modelos de contexto 128K para tareas no triviales. La fiabilidad de las llamadas a herramientas se degrada drásticamente en modelos más pequeños (clase 7B) — emiten XML mal formado y el bucle se detiene.',
        ],
        callouts: [
          {
            type: 'note',
            text: 'La lista de permisos de auto-aprobación de Cline es la diferencia entre un agente útil y uno tedioso. Permite `read_file`, `list_files` y `search_files` por defecto; mantén `write_to_file`, `replace_in_file` y `execute_command` con puerta hasta que confíes en el modelo con esta base de código. Ajusta por proyecto.',
          },
        ],
      },
      aiderDeep: {
        id: 'aider',
        title: 'Aider: La elección nativa de Git',
        content:
          '**Aider es la única de las tres que trata tu repositorio como la fuente de verdad, no la conversación.** Cada edición aterriza como un commit de Git. Deshacer es `git reset`. El historial del chat es desechable; el log de commits es el artefacto.',
        items: [
          '**Ruta de instalación:** `pip install aider-chat` o `pipx install aider-chat`. Invocación CLI: `aider file1.py file2.py` (los archivos quedan en el ámbito de la conversación). Configuración vía `.aider.conf.yml` y sobreescrituras por bandera.',
          '**Primitiva de flujo de trabajo:** edición conversacional orientada al terminal. Escribe un prompt; Aider propone un diff; si lo aceptas, el cambio se hace commit. `/add` y `/drop` gestionan los archivos en ámbito. `/undo` revierte el último commit. `/diff` muestra el cambio más reciente contra HEAD.',
          '**Configuración de modelo local:** cualquier modelo accesible a través de LiteLLM — Ollama, servidores compatibles con OpenAI (llama.cpp, vLLM, LM Studio) o los proveedores de modelos directamente. `--model ollama_chat/qwen3-coder:30b` es la invocación local típica.',
          '**Contexto de base de código:** mapa de repositorio tree-sitter. Aider analiza tu repositorio en busca de definiciones (funciones, clases, métodos), las resume por proximidad a los archivos en ámbito, e incluye el resumen en el prompt del sistema. El mapa cabe aproximadamente en 1K–4K tokens para repositorios pequeños, más para grandes — `--map-tokens` limita el presupuesto.',
          '**Modelo de edición:** dos estrategias. El modo "diff" emite bloques de búsqueda y reemplazo que el modelo debe producir textualmente — quirúrgico, bajo en tokens, pero falla si el modelo parafrasea el bloque de búsqueda. El modo "entero" reescribe archivos completos — robusto en archivos pequeños, costoso en grandes. Aider selecciona automáticamente los valores predeterminados por modelo que coinciden con sus fortalezas conocidas.',
          '**Donde destaca:** cambios a escala de repositorio de los que quieres cada paso en el log de commits; programación en pareja con revisión de código incorporada (cada commit es una unidad de revisión discreta); flujos de trabajo SSH y devcontainer donde las extensiones de IDE no son una opción.',
          '**Donde se queda corto:** sin autocompletado. Sin vista previa en IDE por defecto (funciona bien en un panel de terminal de VS Code junto a tu editor, pero no está integrado). El formato de edición de diff es sensible a la calidad de salida del modelo — un modelo que parafrasea el bloque de búsqueda en vez de reproducirlo fallará las ediciones y verás intentos de reintento en el terminal.',
        ],
        callouts: [
          {
            type: 'tip',
            text: 'Establece `auto-commits: true` (el valor predeterminado) y usa `git rebase -i` para aplastar los commits de Aider al final de una función. Los commits detallados durante el trabajo son valiosos para `/undo`; el commit final aplastado es lo que revisa tu equipo. Aider documenta exactamente este patrón.',
          },
        ],
      },
      editModel: {
        id: 'edit-model',
        title: 'Modelo de edición: cómo escribe código cada herramienta',
        content:
          '**El modelo de edición es el mayor determinante individual de cómo se siente cada herramienta.** Mismo modelo, misma tarea — tres artefactos muy distintos.',
        image: '/images/coding-agent-edit-model-es.svg',
        imageCaption: 'Fiabilidad del modelo de edición por herramienta: en modelos 7B, Continue.dev es el más tolerante, Aider rechaza bloques que no coinciden, Cline se detiene con XML mal formado; en modelos 30B+ de código (Qwen3-Coder, DeepSeek-V3), los tres son fiables.',
        items: [
          '**Continue.dev** — para autocompletado: los tokens FIM alrededor del cursor producen completados en línea. Para ediciones guiadas por chat: el modelo devuelve un bloque de código delimitado; "Aplicar" hace un diff contra el archivo abierto y lo presenta para aceptar/rechazar. Para ediciones en línea (Cmd/Ctrl+I sobre una selección): el modelo reescribe la selección en su lugar y el diff se muestra allí mismo.',
          '**Cline** — cada edición es una llamada a herramienta. `write_to_file` para archivos nuevos o reescrituras completas, `replace_in_file` para cambios quirúrgicos con bloques explícitos `<<<<<<< SEARCH` / `>>>>>>> REPLACE`. Las llamadas a herramientas se validan contra un esquema; las mal formadas generan un "error de herramienta" que el agente ve y reintenta. La conversación son llamadas a herramientas estructuradas y resultados de herramientas, no prosa libre.',
          '**Aider** — bloques de diff de búsqueda y reemplazo renderizados en prosa simple, analizados en el CLI. El modelo emite algo parecido a un bloque delimitado de diff unificado; Aider lo aplica y hace commit. Si el bloque SEARCH no coincide exactamente con el archivo, la edición se rechaza y Aider pide al modelo que reintente.',
          '**Ranking de fiabilidad en un modelo de clase 7B:** Continue.dev > Aider > Cline. El "Aplicar" de Continue.dev es tolerante — las pequeñas paráfrasis las reconcilia el algoritmo de diff. Aider rechaza de plano en bloques de búsqueda que no coinciden, pero el fallo es ruidoso y recuperable. El formato de herramienta XML de Cline es el más frágil en modelos pequeños — las llamadas a herramientas mal formadas detienen el bucle.',
          '**Ranking de fiabilidad en un modelo de llamadas a herramientas 30B+ (Qwen3-Coder 30B, DeepSeek-V3):** los tres convergen — el modelo tiene la capacidad de emitir bloques de diff precisos y llamadas a herramientas bien formadas de manera consistente.',
          'Para técnicas de prompting estructurado que mejoran la generación de código independientemente de qué herramienta los envuelva, consulta [Escribe mejor código con IA](/es/prompt-engineering/write-better-code-with-ai).',
        ],
      },
      undo: {
        id: 'undo',
        title: 'Deshacer, revertir y la pregunta "¿Qué acaba de hacer el agente?"',
        content:
          '**Deshacer es la función sin glamour que solo notas cuando la necesitas.** Las tres herramientas son recuperables; la diferencia está en cuánta fricción hay entre "esto fue un error" y "el error ha desaparecido".',
        items: [
          '**Continue.dev** — las ediciones aplicadas se unen a la pila de deshacer de VS Code. Cmd/Ctrl+Z revierte la aplicación más reciente por archivo. Para revertir múltiples archivos, tu flujo de trabajo de Git existente hace el trabajo pesado. No hay un comando a nivel de herramienta de "deshacer toda esta conversación".',
          '**Cline** — la interfaz del agente muestra cada paso como una tarjeta; revertir por edición es un clic. Las tarjetas persisten durante la duración de la tarea. Tras finalizar la tarea, la reversión es vía Git — Cline no hace commit automáticamente, por lo que puede que tengas un montón de cambios sin commit tras una sesión larga.',
          '**Aider** — `/undo` revierte el commit más reciente; repite para retroceder más. Como cada edición es un commit separado, la granularidad coincide paso a paso con la conversación. `git reflog` recupera todo lo que `/undo` no pueda.',
          '**La implicación práctica:** si te inclinas a dejar un agente funcionando 20 minutos sin supervisión, Aider te da el artefacto más revisable después (un log de commits limpio) y Cline te da la pista de auditoría en progreso más legible (tarjetas por paso, pero sin commits). Continue.dev asume que te quedaste en el teclado durante todo el proceso.',
        ],
        callouts: [
          {
            type: 'tip',
            text: 'Para sesiones de Cline, ejecuta `git stash` antes de comenzar una tarea no trivial. Si el agente se descarrila, `git stash drop` limpia más rápido que revertir por archivo. Para Aider, el log de commits existente es la red de seguridad — no hagas nada extra.',
          },
        ],
      },
      context: {
        id: 'context',
        title: 'Estrategia de ventana de contexto: a dónde van los tokens',
        content:
          '**Las tres herramientas gastan tokens de manera diferente.** Esta es la decisión de presupuesto más importante cuando tienes un modelo con una ventana de contexto más pequeña (32K o menos) y un repositorio no trivial.',
        image: '/images/coding-agent-context-strategy-es.svg',
        imageCaption: 'Estrategia de ventana de contexto: Continue.dev usa recuperación de índice bajo demanda (mínimo 32K), Cline transmite archivos completos (128K recomendado para tareas de múltiples archivos), Aider usa mapa de repositorio tree-sitter más /add explícito (mínimo 32K).',
        items: [
          '**Continue.dev** — índice TF-IDF más embeddings sobre el proyecto. La recuperación es bajo demanda: solo los fragmentos relevantes para el mensaje de chat actual aterrizan en el prompt. `@codebase` desencadena una búsqueda semántica; `@file path/to/x.ts` inyecta el archivo literalmente. El uso de tokens escala con lo que anclas, no con el tamaño del repositorio.',
          '**Cline** — el contenido completo de los archivos se transmite a la conversación en cada lectura. Un archivo de 500 líneas consume ~3K–5K tokens; una tarea de agente que toca 8 archivos consume 25K–40K tokens antes de que ocurra cualquier razonamiento. Mínimo recomendado: ventana de contexto de 32K; 128K es cómodo.',
          '**Aider** — mapa de repositorio tree-sitter (por defecto ~2K tokens, configurable vía `--map-tokens`) más el contenido de los archivos explícitamente añadidos con `/add`. El mapa es un resumen solo de definiciones, no código completo, por lo que escala mucho mejor con el tamaño del repositorio que transmitir archivos. `/drop` los archivos cuando ya no estén en ámbito.',
          '**Implicación práctica para un modelo de contexto de 32K:** Continue.dev y Aider se mantienen cómodos en la mayoría de los repositorios; Cline empieza a sufrir presión de contexto en tareas que tocan más de 5–6 archivos a menos que cambies a un modelo de contexto de 128K.',
          '**Implicación práctica para un modelo de contexto de 128K:** los tres son cómodos. El mayor consumo de Cline deja de importar; el factor decisivo vuelve a la primitiva de flujo de trabajo.',
          'Para una explicación más profunda de las ventanas de contexto y por qué los modelos pierden información a mitad del contexto, consulta [Ventanas de contexto explicadas: por qué la IA olvida](/es/prompt-engineering/context-windows-explained-why-ai-forgets).',
        ],
      },
      toolCalls: {
        id: 'tool-calls',
        title: 'La fiabilidad de las llamadas a herramientas es una propiedad del modelo',
        content:
          '**Los fallos de llamadas a herramientas suelen atribuirse incorrectamente a la herramienta.** En la práctica el modelo emite una llamada mal formada, la herramienta valida estrictamente y el bucle se detiene. El mismo modelo en una herramienta diferente produce el mismo modo de fallo.',
        items: [
          '**Modelos que realizan llamadas a herramientas fiables para estas herramientas (mayo de 2026):** Qwen3-Coder 30B (el mejor en el formato XML de Cline), DeepSeek-V3 (contexto amplio, fuerte en planes de múltiples pasos), Llama 3.3 70B (lento pero fiable), Codestral 22B (rápido, menor horizonte de planificación).',
        ...
        callouts: [
          {
            type: 'note',
            text: 'Al evaluar un nuevo modelo local, pruébalo en la herramienta que realmente usas, no en un benchmark genérico de llamadas a herramientas. La señal relevante es si tu flujo de trabajo diario se detiene. Un modelo que obtiene 85% en un benchmark público de llamadas a herramientas puede fallar la mitad de tus tareas de Cline si no emite de manera fiable el XML específico de Cline.',
          },
        ],
      },
      ideIntegration: {
        id: 'ide-integration',
        title: 'Integración con IDE y alcance del editor',
        content:
          '**El alcance del editor es donde las tres herramientas divergen más visiblemente.** Esta es la dimensión que más importa a la mayoría de los lectores; también es la más fácil de probar en 10 minutos.',
        items: [
          '**Continue.dev** — VS Code Marketplace y JetBrains Marketplace. La build de JetBrains es real (IntelliJ, PyCharm, GoLand, WebStorm, etc.) y tiene paridad de funciones con la build de VS Code para autocompletado y chat. Los usuarios de Cursor pueden instalar la extensión de VS Code; el comportamiento coincide con VS Code.',
          '**Cline** — solo extensión de VS Code. Funciona en Cursor (que es un fork de VS Code) pero no está probado oficialmente; la superficie de la extensión es idéntica y la mayoría de los usuarios no reportan problemas. Los usuarios de JetBrains por ahora están sin opción.',
          '**Aider** — orientado al terminal; se integra con cualquier editor situándose junto a él. VS Code: abre el terminal integrado, ejecuta Aider, edita en el panel del editor mientras Aider hace commit. JetBrains: el mismo patrón en el terminal de JetBrains. Vim/Neovim: combinado con `:e!` para recargar tras los commits de Aider, esto es un flujo productivo. Cursor: idéntico a VS Code.',
          '**Implicación práctica para usuarios de JetBrains:** Continue.dev o Aider son tus únicas opciones.',
          '**Implicación práctica para usuarios de Vim/Neovim:** Aider es la opción natural. Continue.dev tiene integraciones de Neovim de la comunidad con madurez variable; la superficie oficial es VS Code y JetBrains.',
        ],
      },
      workflowFit: {
        id: 'workflow-fit',
        title: 'Adecuación al flujo de trabajo por caso de uso',
        content:
          '**El marco más útil es "qué tipo de día estás teniendo".** Adapta la herramienta al día, no al revés.',
        image: '/images/coding-agent-workflow-fit-es.svg',
        imageCaption: 'Adecuación al flujo de trabajo: Continue.dev para autocompletado y preguntas en el IDE; Cline para refactorizaciones de múltiples archivos y depuración exploratoria con acceso a comandos; Aider para cambios nativos de git, SSH y flujos de trabajo Vim/Neovim.',
        columns: ['Caso de uso', 'Mejor opción', 'Por qué'],
        rows: [
          {
            'Caso de uso': 'Añadir una función a un archivo existente con autocompletado',
            'Mejor opción': 'Continue.dev',
            'Por qué': 'El autocompletado FIM es la primitiva natural; no necesitas chat',
          },
          {
            'Caso de uso': 'Preguntar "¿qué hace esta función?" sin salir del editor',
            'Mejor opción': 'Continue.dev',
            'Por qué': 'El chat en IDE con `@file` es el camino con menos fricción',
          },
          {
            'Caso de uso': 'Refactorizar una función en 5+ archivos',
            'Mejor opción': 'Cline',
            'Por qué': 'Ediciones nativas de múltiples archivos con aprobación por paso',
          },
          {
            'Caso de uso': '"¿Por qué falla este test?" — búsqueda exploratoria de errores',
            'Mejor opción': 'Cline',
            'Por qué': 'El bucle de agente ejecuta comandos, lee logs e itera',
          },
          {
            'Caso de uso': 'Renombrado a escala de repositorio o migración de API con commits revisables',
            'Mejor opción': 'Aider',
            'Por qué': 'Cada edición se convierte en un commit; trivial de revisar y deshacer',
          },
          {
            'Caso de uso': 'SSH a un servidor, trabajar sin IDE',
            'Mejor opción': 'Aider',
            'Por qué': 'Solo terminal por diseño',
          },
          {
            'Caso de uso': 'Usuario de Vim o Neovim como herramienta principal',
            'Mejor opción': 'Aider',
            'Por qué': 'Se combina de forma natural con la edición orientada al terminal',
          },
          {
            'Caso de uso': 'Usuario de IDE JetBrains (IntelliJ, GoLand, PyCharm)',
            'Mejor opción': 'Continue.dev o Aider',
            'Por qué': 'Cline es solo para VS Code',
          },
        ],
      },
      decisionTree: {
        ...
        content:
          '**Cinco preguntas, en orden, llevan a la mayoría de los lectores a la elección correcta.**',
        items: [
          '**1. ¿Estás en JetBrains, Vim o trabajando por SSH?** → Si sí, estás entre Continue.dev (solo JetBrains) y Aider (Vim/SSH). Salta a la Q4. Si no, continúa.',
          '**2. ¿Quieres un agente autónomo que edite múltiples archivos en una sola tarea?** → Sí: Cline. No: continúa.',
          '**3. ¿Quieres autocompletado más un panel de chat integrado en el IDE?** → Sí: Continue.dev. No: continúa.',
          '**4. ¿Revisas los cambios con `git diff` y quieres que cada edición se haga commit automáticamente?** → Sí: Aider. No: Continue.dev (la opción predeterminada).',
          '**5. ¿Todavía no lo tienes claro?** → Empieza con Continue.dev. Es la instalación con menos fricción, funciona en el editor que probablemente ya usas, y agregar Aider más tarde para el trabajo orientado al terminal es sencillo. Cline es el más disruptivo de añadir a un flujo de trabajo; adóptalo solo cuando sepas que quieres el bucle de agente.',
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
        title: 'Errores comunes al elegir una herramienta de código local',
        items: [
          '**Error 1: elegir la herramienta antes que el modelo.** Un modelo ajustado para código de 30B en cualquiera de las tres herramientas supera a un modelo de propósito general de 7B en la herramienta "mejor". Elige el modelo primero; la herramienta es la segunda decisión.',
          '**Error 2: usar un único modelo para autocompletado y chat en Continue.dev.** El autocompletado necesita latencia inferior a 200 ms; el chat tolera segundos. Ejecuta un modelo de autocompletado pequeño y un modelo de chat más grande en la misma configuración.',
          '**Error 3: dejar la auto-aprobación de Cline completamente abierta.** Un bucle de modelo con `execute_command` auto-aprobado puede causar daño real. Mantén las herramientas destructivas con puerta hasta que confíes en el modelo con esta base de código.',
          '**Error 4: usar Aider con un modelo que parafrasea el código.** Si el modelo no puede reproducir los bloques SEARCH literalmente, cada edición falla. Usa un modelo ajustado para código de 22B o más (Qwen3-Coder, DeepSeek Coder, Codestral) para ediciones de diff fiables.',
          '**Error 5: cambiar de herramienta para solucionar un problema del modelo.** Si las llamadas a herramientas fallan en Cline, probablemente también fallarán en el modo Agente de Continue.dev por la misma razón. Diagnostica el modelo primero.',
          '**Error 6: ejecutar Cline contra un modelo de contexto de 32K en tareas de múltiples archivos.** El consumo de tokens agota el presupuesto a mitad de la tarea. Usa un modelo de contexto de 128K para el trabajo no trivial con Cline.',
        ],
      },
      sources: {
        id: 'sources',
        title: 'Fuentes',
        items: [
          '[Documentación de Continue.dev](https://docs.continue.dev/) — guía de configuración oficial, configuración de modelos, proveedores de contexto `@` y referencia del modo Agente.',
          '[Repositorio de GitHub de Cline](https://github.com/cline/cline) — código fuente, esquemas de herramientas, comportamiento del modo Plan/Actuar y arquitectura de la extensión.',
          '[Documentación de Aider](https://aider.chat/) — referencia oficial de CLI, documentación del formato de edición, mecánicas del mapa de repositorio y valores predeterminados por modelo.',
          '[Biblioteca de modelos de Ollama](https://ollama.com/library) — modelos locales disponibles y niveles de cuantización referenciados para cada herramienta.',
          '[Tarjeta del modelo Qwen3-Coder](https://huggingface.co/Qwen/Qwen3-Coder-30B) — arquitectura, longitudes de contexto compatibles y datos de entrenamiento de llamadas a herramientas para el modelo de código recomendado.',
        ],
      },
      faq: {
        id: 'faq',
        ...
        faqs: [
          {
            q: '¿Cuál es la mejor opción para usuarios de VS Code?',
            a: 'Las tres funcionan en VS Code, pero Continue.dev y Cline son las opciones naturales. Continue.dev para autocompletado primero y chat integrado en IDE; Cline para tareas de agente autónomo. Aider funciona perfectamente desde el terminal integrado de VS Code pero no se integra con la superficie del editor en sí — su interfaz es el terminal.',
          },
          {
            q: '¿Cline es compatible con el IDE Cursor?',
            a: 'Cursor es un fork de VS Code y acepta extensiones de VS Code. Cline se instala y ejecuta en Cursor; la superficie de la extensión es idéntica. El equipo de Cline no apunta oficialmente a Cursor, por lo que los casos límite (API específicas de Cursor, cambios futuros de Cursor) no están garantizados, pero en la práctica los usuarios no reportan problemas.',
          },
          {
            q: '¿Puede Aider funcionar sin Git?',
            a: 'Sí, pero perderías la primitiva de flujo de trabajo que lo define. Aider funciona en directorios sin Git con `--no-git`; las ediciones se aplican pero no hay commits, por lo que `/undo` queda deshabilitado y la reversión recae en el historial de diffs del modelo dentro de la sesión. Para trabajo no trivial, inicializa un repositorio Git — incluso uno ignorado — antes de comenzar.',
          },
          {
            q: '¿Cuál tiene la mejor refactorización de múltiples archivos?',
            a: 'Cline para trabajo autónomo de múltiples archivos — planifica entre archivos, los lee según se necesita y edita con aprobación por paso. Aider para trabajo revisable de múltiples archivos donde nombras los archivos explícitamente y quieres que cada edición quede como commit. Continue.dev requiere conducción manual por archivo; es el más débil de los tres en refactorizaciones de múltiples archivos.',
          },
          {
            q: '¿Son compatibles con Ollama local de forma inmediata?',
            a: 'Sí. Continue.dev tiene un proveedor nativo de Ollama con un bloque de configuración. Cline lista Ollama en el menú desplegable de proveedores. Aider soporta Ollama a través de LiteLLM con `--model ollama_chat/<model-name>`. Ollama es el backend local con menos fricción para las tres.',
          },
          {
            q: '¿Cuál funciona mejor con Qwen3-Coder?',
            a: 'Qwen3-Coder 30B es fiable en las tres herramientas en mayo de 2026 — maneja el formato de herramienta XML de Cline, los bloques de búsqueda y reemplazo de Aider, y los flujos de trabajo de autocompletado + chat de Continue.dev sin degradación apreciable. Las variantes más pequeñas de Qwen3-Coder (clase 7B) realizan llamadas a herramientas con menos fiabilidad y funcionan mejor en el modo solo autocompletado de Continue.dev que en el bucle de agente de Cline.',
          },
          {
            q: '¿Puedo usar estas herramientas con el modo local de Cursor?',
            a: 'Las funciones integradas de Cursor (autocompletado con Tab, Cmd+K) usan el enrutamiento de modelos propio de Cursor, no estas herramientas. Para usar un LLM local en Cursor, instala Continue.dev o Cline como extensiones de VS Code dentro de Cursor — traen su propia configuración de proveedor independiente de la de Cursor. Aider funciona en el terminal integrado de Cursor igual que en el de VS Code.',
          },
          {
            q: '¿Cuál es la más estable para sesiones largas?',
            a: 'Aider, al no tener estado en proceso que pueda corromperse — cada edición es un commit, y un fallo de sesión solo pierde la conversación, no el trabajo. Continue.dev es estable a lo largo de sesiones largas en el IDE. Las sesiones largas de Cline pueden sufrir presión en la ventana de contexto con modelos más pequeños; reiniciar tareas entre refactorizaciones grandes evita el problema.',
          },
          {
            q: '¿Son compatibles con prompts de sistema personalizados?',
            a: 'Las tres. Continue.dev: `systemMessage` por modelo en `config.json`. Cline: campo "Instrucciones personalizadas" en la configuración de la extensión. Aider: bandera `--system-prompt` o valores predeterminados por modelo en `.aider.conf.yml`. Los prompts personalizados son especialmente útiles para Cline (orientar el comportamiento del modo Plan) y Aider (aplicar un estilo de código consistente entre commits).',
          },
          {
            q: '¿Qué deben tener en cuenta los equipos DevOps o de plataforma sobre la privacidad al usar agentes de código local?',
            a: 'Si el código o los logs que procesan los LLM locales pueden contener datos de usuarios de la UE o América Latina, se aplican normativas de privacidad regionales. Los LLM locales en sí (Ollama, llama.cpp) no almacenan datos personales, pero los directorios de código y logs que indexan pueden contener información sensible. Asegúrate de que el modelo se despliegue en infraestructura de la empresa sin acceso a API externas. En España y Latinoamérica, consulta con tu equipo de cumplimiento antes de desplegar en producción en sectores regulados.',
          },
          {
            q: '¿Cuál es la mejor opción para pequeñas y medianas empresas hispanohablantes?',
            a: 'Aider ofrece la mejor relación coste-beneficio para pymes: sin dependencia de licencias de IDE, completamente basado en terminal (encaja con entornos de desarrollo existentes), requisitos mínimos de hardware (funciona con un modelo de contexto de 32K), y la pista de auditoría completa está en el log de Git (simplifica el cumplimiento y la revisión de código). Continue.dev es la segunda opción para equipos fuertemente invertidos en VS Code/JetBrains. Cline se vuelve costoso en consumo de tokens y requiere mayor hardware, pero encaja en flujos de trabajo ágiles de refactorización intensiva. Las pymes ahorran costes con despliegue en local con modelos locales — la elección de herramienta debe seguir la infraestructura existente y los flujos de trabajo de los desarrolladores, no el hype.',
          },
        ],
      },
      relatedReading: {
        ...
        items: [
          '[Reemplazar GitHub Copilot con un LLM local](/es/power-local-llm/replace-github-copilot-with-local-llm) — contexto más amplio de sustitución, incluidos los cálculos de coste y privacidad.',
          '[Mejores modelos de código local en 2026](/es/power-local-llm/best-local-coding-models-2026) — el lado del modelo en la ecuación: Qwen3-Coder vs DeepSeek vs Codestral en benchmarks reales.',
          '[Agentes de IA local con MCP en 2026](/es/power-local-llm/local-ai-agents-with-mcp-2026) — para usuarios que quieren llamadas a herramientas extendidas más allá de las herramientas anteriores.',
          '[Mejores modelos locales para llamadas a herramientas en 2026](/es/power-local-llm/best-local-models-tool-calling-2026) — profundidad técnica adyacente sobre fiabilidad de llamadas a herramientas.',
          '[llama.cpp vs Ollama vs vLLM](/es/local-llms/llamacpp-vs-ollama-vs-vllm) — la capa del motor de inferencia bajo las tres herramientas.',
          '[Hub Power Local LLM](/power-local-llm) — biblioteca completa de guías.',
        ],
      },
    },
      schema: {
  ...
  pt: {
    ...
    seoTitle: 'Continue.dev vs Cline vs Aider: Agente de código local 2026',
    intro:
      'Continue.dev, Cline e Aider são as três ferramentas de código open-source sérias que se integram corretamente com um LLM local em 2026. Elas não são intercambiáveis: cada uma é construída em torno de uma primitiva de fluxo de trabalho diferente — autocompletar, loop de agente autônomo ou diff nativo do Git. Escolha primeiro pelo fluxo de trabalho e depois pelo modelo, e você evita 90% das dores de integração.',
    metaDescription:
      'Continue.dev, Cline e Aider comparados como agentes de código local em 2026: modelo de edição, integração com IDE, desfazer e estratégia de contexto.',
    twitterDescription:
    ...
    audience:
      'Desenvolvedores que executam um LLM de código local (Qwen3-Coder, DeepSeek Coder, Codestral, Llama 3.3) e precisam escolher uma ferramenta que combine com seu editor e fluxo de trabalho de projeto.',
    ...
    targetKeywords: [
      'continue.dev vs cline',
      'cline vs aider',
      'melhor agente de código local 2026',
      'ferramenta de código llm local',
      'aider llm local',
      'continue.dev llm local',
    ],
    leadAnswerBlock:
      '**Continue.dev, Cline e Aider resolvem problemas distintos com o mesmo modelo local. Continue.dev é construído em torno do autocompletar e de um painel de chat integrado ao IDE — ideal para fluxos de trabalho de edição enquanto você digita no VS Code ou JetBrains. Cline é um agente autônomo do VS Code que planeja, edita e executa comandos em vários arquivos com portões de aprovação explícitos — ideal para refatorações de vários arquivos e caças a bugs exploratórias. Aider é uma CLI orientada ao terminal e nativa do Git que transforma cada edição em um commit com diffs cirúrgicos de busca e substituição — ideal para trabalho em escala de repositório e revisores que querem cada mudança refletida no `git log`. Escolha pela primitiva de fluxo de trabalho (autocompletar vs loop de agente vs diff do Git), não por qual ferramenta parece mais potente.**',
    quickAnswerTop: {
      pt: {
        ...
        bullets: [
          ...
          'Aider — CLI de terminal com commits nativos do Git e diffs de busca e substituição. Ideal para trabalho em escala de repositório revisável e com suporte a desfazer.',
    ...
    toc: [
      ...
      { label: 'Continue.dev: Análise aprofundada', anchor: '#continue-dev' },
      { label: 'Cline: Análise aprofundada', anchor: '#cline' },
      { label: 'Aider: Análise aprofundada', anchor: '#aider' },
      { label: 'Modelo de edição: como cada ferramenta escreve código', anchor: '#edit-model' },
      { label: 'Desfazer e reverter', anchor: '#undo' },
      { label: 'Estratégia de janela de contexto', anchor: '#context' },
      { label: 'Confiabilidade das chamadas de ferramentas', anchor: '#tool-calls' },
      { label: 'Integração com IDE', anchor: '#ide-integration' },
      { label: 'Adequação ao fluxo de trabalho por caso de uso', anchor: '#workflow-fit' },
      { label: 'Árvore de decisão', anchor: '#decision-tree' },
    ...
    gammaEmbedUrl: '/presentations/continue-dev-vs-cline-vs-aider-local-static.html',
    gammaDescription: 'A apresentação a seguir cobre: como Continue.dev, Cline e Aider diferem como ferramentas de código local (autocompletar vs agente autônomo vs diffs nativos do git), uma tabela comparativa de recursos, confiabilidade do modelo de edição por ferramenta e tamanho de modelo, estratégia de janela de contexto (32K vs 128K), integração com IDE (VS Code, JetBrains, Vim/SSH) e 6 erros comuns ao escolher uma ferramenta. Baixe o PDF como cartão de referência para agentes de código local.',
    sections: {
      tldr: {
        id: 'key-takeaways',
        isTldr: true,
        items: [
          ...
          'A **confiabilidade das chamadas de ferramentas** depende do modelo, não da ferramenta. Qwen3-Coder 30B e DeepSeek-V3 são os chamadores de ferramentas mais confiáveis em maio de 2026; modelos menores (classe 7B) emitem regularmente chamadas de ferramentas malformadas independentemente de qual ferramenta os envolva.',
          'A **estratégia de janela de contexto** difere bastante: o Continue.dev se apoia no índice de base de código integrado e no contexto fixado com `@`, o Cline transmite o conteúdo completo dos arquivos para a conversa (alto uso de tokens), o Aider usa um mapa de repositório tree-sitter mais `/add` e `/drop` explícitos para manter pequeno o conjunto de trabalho.',
          'Para um desenvolvedor individual, a **adequação ao fluxo de trabalho** domina os resultados mais do que qualquer benchmark. A ferramenta que você realmente mantém aberta é a que você deve escolher.',
        ],
      },
      quickFacts: {
        id: 'quick-facts',
        ...
        items: [
          ...
          '**A confiabilidade das chamadas de ferramentas é uma propriedade do modelo, não da ferramenta.** Qwen3-Coder 30B e DeepSeek-V3 são confiáveis nos três; modelos de classe 7B falham nos três.',
          '**Orçamento de contexto:** Continue.dev e Aider ficam confortáveis com um modelo de contexto de 32K; o Cline precisa de **128K** para tarefas de vários arquivos não triviais.',
          '**Escolha pelo editor:** JetBrains → Continue.dev ou Aider. Vim/Neovim ou SSH → Aider. VS Code com dias de muito autocompletar → Continue.dev. VS Code com refatorações de vários arquivos → Cline.',
        ],
      },
      comparisonTable: {
        id: 'comparison-table',
        title: 'Como Continue.dev, Cline e Aider se comparam em 2026',
        content:
          'As três ferramentas são open-source, gratuitas e funcionam com qualquer endpoint de LLM local. As diferenças estruturais abaixo determinam qual combina com o seu fluxo de trabalho.',
        image: '/images/coding-agent-comparison-en.svg',
        imageCaption: 'Continue.dev vs Cline vs Aider: Continue.dev para autocompletar no VS Code/JetBrains (contexto 32K), Cline para agentes autônomos de vários arquivos no VS Code (128K recomendado), Aider para commits nativos do git no terminal (contexto 32K).',
        snippetBlocks: [
          {
            type: 'one-sentence',
            text: 'Continue.dev é autocompletar estilo Copilot, Cline é um agente autônomo que edita entre arquivos, Aider é uma ferramenta de terminal que faz commit de cada edição no Git — escolha pelo fluxo de trabalho, não pelo benchmark.',
          },
          {
            type: 'plain-terms',
        ...
        rows: [
          {
            ...
            'Continue.dev': 'Extensão de VS Code e JetBrains',
            'Cline': 'Extensão do VS Code',
            'Aider': 'CLI de terminal (qualquer editor ao lado)',
          },
          {
            'Característica': 'Primitiva de fluxo de trabalho',
            'Continue.dev': 'Autocompletar + chat integrado ao IDE',
            'Cline': 'Loop de agente autônomo com portões de aprovação',
            'Aider': 'Edições de diff nativo do Git, cada mudança um commit',
          },
          {
            'Característica': 'Modelo de edição',
            'Continue.dev': 'FIM inline + aplicar ao arquivo a partir do chat',
            'Cline': 'Chamadas de ferramentas (read_file, write_to_file, replace_in_file)',
            'Aider': 'Blocos de diff de busca e substituição; modo "whole" para arquivos pequenos',
          },
          {
            'Característica': 'Edições de vários arquivos',
            'Continue.dev': 'Manual: o chat sugere, você aplica por arquivo',
            'Cline': 'Nativo: o agente edita entre arquivos em uma única tarefa',
            'Aider': 'Nativo: nomeie os arquivos no prompt, as edições caem em um commit',
          },
          {
            'Característica': 'Desfazer / Reverter',
            'Continue.dev': 'Pilha de desfazer do VS Code nas edições aplicadas',
            'Cline': 'Reverter por edição na interface do agente; Git para reversão completa',
            'Aider': '`/undo` reverte o último commit; histórico completo do git abaixo',
          },
          {
            'Característica': 'Protocolo de chamada de ferramentas',
            'Continue.dev': 'Ferramentas da OpenAI (function-calling) no modo Agent',
            'Cline': 'Blocos de ferramentas XML personalizados com esquema rígido',
            'Aider': 'Sem chamadas de ferramentas; instrui o modelo a emitir blocos de diff',
          },
          {
            'Característica': 'Contexto da base de código',
            'Continue.dev': 'Índice local (TF-IDF + embeddings) + provedores `@`',
            'Cline': 'Transmite o conteúdo completo dos arquivos para a conversa',
            'Aider': 'Mapa de repositório Tree-sitter + `/add` e `/drop` manual',
          },
          {
            'Característica': 'Aprovação / Segurança',
            'Continue.dev': 'Pré-visualização de diff antes de cada aplicação',
            'Cline': 'Aprovação por etapa; listas de permissões de autoaprovação configuráveis',
            'Aider': 'Pré-visualização de diff no terminal; a etapa de commit é automática',
          },
          {
            'Característica': 'Melhor para',
            'Continue.dev': 'Editar enquanto digita, chat integrado ao IDE, dias de muito autocompletar',
            'Cline': 'Refatorações de vários arquivos, depuração exploratória, tarefas de repositório',
            'Aider': 'Mudanças revisáveis em escala de repositório, fluxos de trabalho orientados ao terminal',
          },
        ],
      },
      whichOne: {
        ...
        content:
          '**A ferramenta certa é decidida pelo seu editor, pela sua tolerância a edições autônomas e por como você revisa as mudanças.** Use este atalho.',
        image: '/images/coding-agent-which-one-en.svg',
        imageCaption: 'Qual ferramenta de código local escolher: Continue.dev para autocompletar e chat no IDE (VS Code + JetBrains), Cline para loops de agente de vários arquivos e execução de comandos, Aider para fluxos de trabalho nativos do git no terminal e ambientes SSH.',
        columns: ['Sua situação', 'Escolha'],
        rows: [
          { 'Sua situação': 'Vivo no VS Code ou em um IDE JetBrains e quero autocompletar estilo Copilot', 'Escolha': 'Continue.dev' },
          { 'Sua situação': 'Quero um painel de chat lateral que acesse minha base de código sem enviar arquivos manualmente', 'Escolha': 'Continue.dev' },
          { 'Sua situação': 'Quero um agente que planeje uma refatoração em vários arquivos e pergunte antes de cada etapa', 'Escolha': 'Cline' },
          { 'Sua situação': 'Quero que o modelo possa executar comandos e ler a saída do terminal como parte do seu loop', 'Escolha': 'Cline' },
          { 'Sua situação': 'Reviso as mudanças com `git diff` e quero que cada edição seja commitada automaticamente', 'Escolha': 'Aider' },
          { 'Sua situação': 'Trabalho por SSH ou em um ambiente orientado ao terminal', 'Escolha': 'Aider' },
          { 'Sua situação': 'Preciso dos três fluxos de trabalho ocasionalmente — escolha o mais simples primeiro', 'Escolha': 'Comece com o Continue.dev; adicione o Aider quando precisar de diffs nativos do Git' },
        ],
      },
      continueDevDeep: {
        id: 'continue-dev',
        title: 'Continue.dev: A escolha orientada ao autocompletar',
        content:
          '**O Continue.dev é o análogo open-source mais próximo do GitHub Copilot para modelos locais.** Sua superfície principal é o editor: completar com Tab enquanto você digita mais um painel de chat lateral que conhece sua base de código.',
        items: [
          '**Caminho de instalação:** VS Code Marketplace e JetBrains Marketplace. A configuração fica em `~/.continue/config.json` (ou `config.yaml`); as definições de modelos, a configuração de autocompletar e os provedores de contexto `@` estão todos lá.',
          '**Primitiva de fluxo de trabalho:** autocompletar via FIM (fill-in-the-middle) mais um painel de chat lateral integrado ao IDE. O painel de chat pode aplicar edições diretamente nos arquivos abertos; um modo Agent adiciona loops de chamadas de ferramentas para tarefas de várias etapas.',
          '**Configuração de modelo local:** o suporte nativo ao Ollama é um bloco de configuração. llama.cpp, LM Studio e qualquer endpoint compatível com OpenAI também funcionam. O padrão usual é executar modelos separados para autocompletar (pequeno, rápido — classe Qwen3-Coder 1.5B) e chat (maior — Qwen3-Coder 30B, DeepSeek-V3).',
          '**Contexto da base de código:** índice local (TF-IDF mais embeddings) na sua pasta de projeto. `@codebase`, `@file`, `@folder`, `@docs`, `@terminal` e provedores personalizados acessam o índice a partir do painel de chat.',
          '**Modelo de edição:** no chat, o modelo propõe um bloco de código; clicar em "Apply" faz um diff contra o arquivo aberto e você aceita ou rejeita a mudança. As edições inline via Cmd/Ctrl+I reescrevem uma seleção no lugar. Ambos os fluxos mostram uma pré-visualização de diff antes de qualquer gravação.',
          '**Onde se destaca:** edição pelo teclado dentro de um IDE que você já usa, baixa sobrecarga cognitiva por edição, ciclo de feedback rápido. A ferramenta que você esquece que instalou.',
      ...
      clineDeep: {
        id: 'cline',
        title: 'Cline: A escolha de agente autônomo',
        content:
          '**O Cline trata o código como uma tarefa de agente, não como uma série de edições.** Você lhe dá um objetivo; ele planeja, lê arquivos, edita-os, executa comandos e pede que você aprove cada etapa.',
        items: [
          '**Caminho de instalação:** VS Code Marketplace (sem versão para JetBrains). A configuração é feita por GUI dentro do painel da extensão — provedor, modelo, configuração de autoaprovação, instruções personalizadas.',
          '**Primitiva de fluxo de trabalho:** loop de agente autônomo com modos Plan e Act explícitos. O modo Plan raciocina sobre a tarefa sem gravar arquivos; o modo Act executa o plano com chamadas de ferramentas. Um portão de aprovação aparece entre as etapas, a menos que você coloque ferramentas em uma lista de permissões de autoaprovação.',
          '**Configuração de modelo local:** qualquer endpoint compatível com OpenAI. O Ollama é um clique. Para llama.cpp ou vLLM, aponte o Cline para a URL do servidor compatível com OpenAI. O loop de agente consome muitos tokens — reserve um modelo com pelo menos uma janela de contexto de 32K (Qwen3-Coder 30B em 32K, DeepSeek-V3 em 128K).',
          '**Superfície de ferramentas:** `read_file`, `write_to_file`, `replace_in_file`, `execute_command`, `list_files`, `search_files`, `browser_action`, `ask_followup_question`, `attempt_completion`. O conjunto é pequeno e bem documentado; os esquemas de ferramentas usam um formato XML personalizado que o modelo deve emitir corretamente.',
          '**Modelo de edição:** para arquivos de algumas centenas de linhas, o Cline grava o arquivo inteiro. Para arquivos maiores, ele emite operações `replace_in_file` com blocos de busca e substituição explícitos. Os diffs são mostrados no painel antes de cada gravação.',
          '**Onde se destaca:** refatorações de vários arquivos, tarefas exploratórias de "por que este teste está falhando?", fluxos de trabalho de "adicionar um recurso que toca três camadas". O mais potente dos três em tarefas com mais de cinco edições de arquivos.',
          '**Onde fica aquém:** o consumo de tokens é alto — o conteúdo completo dos arquivos é transmitido para a conversa a cada leitura. Sessões longas esgotam os modelos de contexto 32K; modelos de contexto 128K são recomendados para tarefas não triviais. A confiabilidade das chamadas de ferramentas degrada drasticamente em modelos menores (classe 7B) — eles emitem XML malformado e o loop para.',
        ],
        callouts: [
          {
            type: 'note',
            text: 'A lista de permissões de autoaprovação do Cline é a diferença entre um agente útil e um tedioso. Permita `read_file`, `list_files` e `search_files` por padrão; mantenha `write_to_file`, `replace_in_file` e `execute_command` com portão até confiar no modelo nesta base de código. Ajuste por projeto.',
          },
        ],
      },
      aiderDeep: {
        id: 'aider',
        title: 'Aider: A escolha nativa do Git',
        content:
          '**O Aider é o único dos três que trata seu repositório como a fonte da verdade, não a conversa.** Cada edição cai como um commit do Git. Desfazer é `git reset`. O histórico do chat é descartável; o log de commits é o artefato.',
        items: [
          '**Caminho de instalação:** `pip install aider-chat` ou `pipx install aider-chat`. Invocação CLI: `aider file1.py file2.py` (os arquivos entram no escopo da conversa). Configuração via `.aider.conf.yml` e substituições por flag.',
          '**Primitiva de fluxo de trabalho:** edição conversacional orientada ao terminal. Digite um prompt; o Aider propõe um diff; se você aceitar, a mudança é commitada. `/add` e `/drop` gerenciam os arquivos no escopo. `/undo` reverte o último commit. `/diff` mostra a mudança mais recente contra HEAD.',
          '**Configuração de modelo local:** qualquer modelo acessível através do LiteLLM — Ollama, servidores compatíveis com OpenAI (llama.cpp, vLLM, LM Studio) ou os provedores de modelos diretamente. `--model ollama_chat/qwen3-coder:30b` é a invocação local típica.',
          '**Contexto da base de código:** mapa de repositório tree-sitter. O Aider analisa seu repositório em busca de definições (funções, classes, métodos), as resume por proximidade aos arquivos no escopo e inclui o resumo no prompt do sistema. O mapa cabe em aproximadamente 1K–4K tokens para repositórios pequenos, mais para grandes — `--map-tokens` limita o orçamento.',
          '**Modelo de edição:** duas estratégias. O modo "diff" emite blocos de busca e substituição que o modelo deve produzir textualmente — cirúrgico, baixo em tokens, mas falha se o modelo parafraseia o bloco de busca. O modo "whole" reescreve arquivos inteiros — robusto em arquivos pequenos, caro em grandes. O Aider seleciona automaticamente os padrões por modelo que combinam com seus pontos fortes conhecidos.',
          '**Onde se destaca:** mudanças em escala de repositório das quais você quer cada etapa no log de commits; programação em par com revisão de código embutida (cada commit é uma unidade de revisão discreta); fluxos de trabalho SSH e devcontainer onde as extensões de IDE não são uma opção.',
          '**Onde fica aquém:** sem autocompletar. Sem pré-visualização no IDE por padrão (funciona bem em um painel de terminal do VS Code ao lado do seu editor, mas não é integrado). O formato de edição de diff é sensível à qualidade da saída do modelo — um modelo que parafraseia o bloco de busca em vez de reproduzi-lo vai falhar as edições e você verá tentativas de repetição no terminal.',
        ],
        callouts: [
          {
            type: 'tip',
            text: 'Defina `auto-commits: true` (o valor padrão) e use `git rebase -i` para esmagar os commits do Aider no final de um recurso. Os commits detalhados durante o trabalho são valiosos para `/undo`; o commit final esmagado é o que seu time revisa. O Aider documenta exatamente esse padrão.',
          },
        ],
      },
      editModel: {
        id: 'edit-model',
        title: 'Modelo de edição: como cada ferramenta escreve código',
        content:
          '**O modelo de edição é o maior determinante individual de como cada ferramenta funciona.** Mesmo modelo, mesma tarefa — três artefatos muito distintos.',
        image: '/images/coding-agent-edit-model-en.svg',
        imageCaption: 'Confiabilidade do modelo de edição por ferramenta: em modelos 7B, o Continue.dev é o mais tolerante, o Aider rejeita blocos que não coincidem, o Cline para com XML malformado; em modelos 30B+ de código (Qwen3-Coder, DeepSeek-V3), os três são confiáveis.',
        items: [
          '**Continue.dev** — para autocompletar: os tokens FIM ao redor do cursor produzem completações inline. Para edições guiadas pelo chat: o modelo retorna um bloco de código delimitado; "Apply" faz um diff contra o arquivo aberto e o apresenta para aceitar/rejeitar. Para edições inline (Cmd/Ctrl+I sobre uma seleção): o modelo reescreve a seleção no lugar e o diff é mostrado ali mesmo.',
          '**Cline** — cada edição é uma chamada de ferramenta. `write_to_file` para arquivos novos ou reescritas completas, `replace_in_file` para mudanças cirúrgicas com blocos explícitos `<<<<<<< SEARCH` / `>>>>>>> REPLACE`. As chamadas de ferramentas são validadas contra um esquema; as malformadas geram um "tool error" que o agente vê e repete. A conversa são chamadas de ferramentas estruturadas e resultados de ferramentas, não prosa livre.',
          '**Aider** — blocos de diff de busca e substituição renderizados em prosa simples, analisados na CLI. O modelo emite algo parecido com um bloco delimitado de diff unificado; o Aider o aplica e faz commit. Se o bloco SEARCH não coincidir exatamente com o arquivo, a edição é rejeitada e o Aider pede ao modelo que repita.',
          '**Ranking de confiabilidade em um modelo de classe 7B:** Continue.dev > Aider > Cline. O "Apply" do Continue.dev é tolerante — as pequenas paráfrases são reconciliadas pelo algoritmo de diff. O Aider rejeita de imediato em blocos de busca que não coincidem, mas a falha é barulhenta e recuperável. O formato de ferramenta XML do Cline é o mais frágil em modelos pequenos — as chamadas de ferramentas malformadas param o loop.',
          '**Ranking de confiabilidade em um modelo de chamadas de ferramentas 30B+ (Qwen3-Coder 30B, DeepSeek-V3):** os três convergem — o modelo tem a capacidade de emitir blocos de diff precisos e chamadas de ferramentas bem formadas de maneira consistente.',
          'Para técnicas de prompting estruturado que melhoram a geração de código independentemente de qual ferramenta os envolva, consulte [Escreva código melhor com IA](/pt/prompt-engineering/write-better-code-with-ai).',
        ],
      },
      undo: {
        id: 'undo',
        title: 'Desfazer, reverter e a pergunta "O que o agente acabou de fazer?"',
        content:
          '**Desfazer é o recurso sem glamour que você só percebe quando precisa dele.** As três ferramentas são recuperáveis; a diferença está em quanto atrito há entre "isso foi um erro" e "o erro sumiu".',
        items: [
          '**Continue.dev** — as edições aplicadas se juntam à pilha de desfazer do VS Code. Cmd/Ctrl+Z reverte a aplicação mais recente por arquivo. Para reverter vários arquivos, seu fluxo de trabalho de Git existente faz o trabalho pesado. Não há um comando em nível de ferramenta de "desfazer toda esta conversa".',
          '**Cline** — a interface do agente mostra cada etapa como um cartão; reverter por edição é um clique. Os cartões persistem durante a duração da tarefa. Após finalizar a tarefa, a reversão é via Git — o Cline não faz commit automaticamente, então você pode ter um monte de mudanças sem commit após uma sessão longa.',
          '**Aider** — `/undo` reverte o commit mais recente; repita para voltar mais. Como cada edição é um commit separado, a granularidade coincide passo a passo com a conversa. `git reflog` recupera tudo o que `/undo` não puder.',
          '**A implicação prática:** se você tende a deixar um agente funcionando 20 minutos sem supervisão, o Aider lhe dá o artefato mais revisável depois (um log de commits limpo) e o Cline lhe dá a trilha de auditoria em andamento mais legível (cartões por etapa, mas sem commits). O Continue.dev pressupõe que você ficou no teclado durante todo o processo.',
        ],
        callouts: [
          {
            type: 'tip',
            text: 'Para sessões do Cline, execute `git stash` antes de começar uma tarefa não trivial. Se o agente sair do trilho, `git stash drop` limpa mais rápido do que reverter por arquivo. Para o Aider, o log de commits existente é a rede de segurança — não faça nada a mais.',
          },
        ],
      },
      context: {
        id: 'context',
        title: 'Estratégia de janela de contexto: para onde vão os tokens',
        content:
          '**As três ferramentas gastam tokens de maneira diferente.** Esta é a decisão de orçamento mais importante quando você tem um modelo com uma janela de contexto menor (32K ou menos) e um repositório não trivial.',
        image: '/images/coding-agent-context-strategy-en.svg',
        imageCaption: 'Estratégia de janela de contexto: Continue.dev usa recuperação de índice sob demanda (mínimo 32K), Cline transmite arquivos completos (128K recomendado para tarefas de vários arquivos), Aider usa mapa de repositório tree-sitter mais /add explícito (mínimo 32K).',
        items: [
          '**Continue.dev** — índice TF-IDF mais embeddings sobre o projeto. A recuperação é sob demanda: apenas os trechos relevantes para a mensagem de chat atual caem no prompt. `@codebase` dispara uma busca semântica; `@file path/to/x.ts` injeta o arquivo literalmente. O uso de tokens escala com o que você fixa, não com o tamanho do repositório.',
          '**Cline** — o conteúdo completo dos arquivos é transmitido para a conversa a cada leitura. Um arquivo de 500 linhas consome ~3K–5K tokens; uma tarefa de agente que toca 8 arquivos consome 25K–40K tokens antes de qualquer raciocínio acontecer. Mínimo recomendado: janela de contexto de 32K; 128K é confortável.',
          '**Aider** — mapa de repositório tree-sitter (por padrão ~2K tokens, configurável via `--map-tokens`) mais o conteúdo dos arquivos explicitamente adicionados com `/add`. O mapa é um resumo apenas de definições, não código completo, então escala muito melhor com o tamanho do repositório do que transmitir arquivos. `/drop` os arquivos quando não estiverem mais no escopo.',
          '**Implicação prática para um modelo de contexto de 32K:** Continue.dev e Aider ficam confortáveis na maioria dos repositórios; o Cline começa a sofrer pressão de contexto em tarefas que tocam mais de 5–6 arquivos, a menos que você troque para um modelo de contexto de 128K.',
          '**Implicação prática para um modelo de contexto de 128K:** os três são confortáveis. O maior consumo do Cline deixa de importar; o fator decisivo volta a ser a primitiva de fluxo de trabalho.',
          'Para uma explicação mais aprofundada sobre janelas de contexto e por que os modelos perdem informação no meio do contexto, consulte [Janelas de contexto explicadas: por que a IA esquece](/pt/prompt-engineering/context-windows-explained-why-ai-forgets).',
        ],
      },
      toolCalls: {
        id: 'tool-calls',
        title: 'A confiabilidade das chamadas de ferramentas é uma propriedade do modelo',
        content:
          '**As falhas de chamadas de ferramentas costumam ser atribuídas incorretamente à ferramenta.** Na prática, o modelo emite uma chamada malformada, a ferramenta valida estritamente e o loop para. O mesmo modelo em uma ferramenta diferente produz o mesmo modo de falha.',
        items: [
          '**Modelos que realizam chamadas de ferramentas confiáveis para estas ferramentas (maio de 2026):** Qwen3-Coder 30B (o melhor no formato XML do Cline), DeepSeek-V3 (contexto amplo, forte em planos de várias etapas), Llama 3.3 70B (lento, mas confiável), Codestral 22B (rápido, menor horizonte de planejamento).',
        ...
        callouts: [
          {
            type: 'note',
            text: 'Ao avaliar um novo modelo local, teste-o na ferramenta que você realmente usa, não em um benchmark genérico de chamadas de ferramentas. O sinal relevante é se o seu fluxo de trabalho diário para. Um modelo que obtém 85% em um benchmark público de chamadas de ferramentas pode falhar metade das suas tarefas do Cline se não emitir de maneira confiável o XML específico do Cline.',
          },
        ],
      },
      ideIntegration: {
        id: 'ide-integration',
        title: 'Integração com IDE e alcance do editor',
        content:
          '**O alcance do editor é onde as três ferramentas divergem de forma mais visível.** Esta é a dimensão que mais importa para a maioria dos leitores; também é a mais fácil de testar em 10 minutos.',
        items: [
          '**Continue.dev** — VS Code Marketplace e JetBrains Marketplace. A build do JetBrains é real (IntelliJ, PyCharm, GoLand, WebStorm, etc.) e tem paridade de recursos com a build do VS Code para autocompletar e chat. Os usuários do Cursor podem instalar a extensão do VS Code; o comportamento coincide com o VS Code.',
          '**Cline** — somente extensão do VS Code. Funciona no Cursor (que é um fork do VS Code) mas não é testado oficialmente; a superfície da extensão é idêntica e a maioria dos usuários não relata problemas. Os usuários do JetBrains por enquanto estão sem opção.',
          '**Aider** — orientado ao terminal; integra-se com qualquer editor ficando ao lado dele. VS Code: abra o terminal integrado, execute o Aider, edite no painel do editor enquanto o Aider faz commit. JetBrains: o mesmo padrão no terminal do JetBrains. Vim/Neovim: combinado com `:e!` para recarregar após os commits do Aider, este é um fluxo produtivo. Cursor: idêntico ao VS Code.',
          '**Implicação prática para usuários do JetBrains:** Continue.dev ou Aider são suas únicas opções.',
          '**Implicação prática para usuários do Vim/Neovim:** o Aider é a opção natural. O Continue.dev tem integrações de Neovim da comunidade com maturidade variável; a superfície oficial é VS Code e JetBrains.',
        ],
      },
      workflowFit: {
        id: 'workflow-fit',
        title: 'Adequação ao fluxo de trabalho por caso de uso',
        content:
          '**O modelo mais útil é "que tipo de dia você está tendo".** Adapte a ferramenta ao dia, não o contrário.',
        image: '/images/coding-agent-workflow-fit-en.svg',
        imageCaption: 'Adequação ao fluxo de trabalho: Continue.dev para autocompletar e perguntas no IDE; Cline para refatorações de vários arquivos e depuração exploratória com acesso a comandos; Aider para mudanças nativas do git, SSH e fluxos de trabalho Vim/Neovim.',
        columns: ['Caso de uso', 'Melhor opção', 'Por quê'],
        rows: [
          {
            'Caso de uso': 'Adicionar uma função a um arquivo existente com autocompletar',
            'Melhor opção': 'Continue.dev',
            'Por quê': 'O autocompletar FIM é a primitiva natural; você não precisa de chat',
          },
          {
            'Caso de uso': 'Perguntar "o que esta função faz?" sem sair do editor',
            'Melhor opção': 'Continue.dev',
            'Por quê': 'O chat no IDE com `@file` é o caminho com menos atrito',
          },
          {
            'Caso de uso': 'Refatorar uma função em mais de 5 arquivos',
            'Melhor opção': 'Cline',
            'Por quê': 'Edições nativas de vários arquivos com aprovação por etapa',
          },
          {
            'Caso de uso': '"Por que este teste está falhando?" — caça a bugs exploratória',
            'Melhor opção': 'Cline',
            'Por quê': 'O loop de agente executa comandos, lê logs e itera',
          },
          {
            'Caso de uso': 'Renomeação em escala de repositório ou migração de API com commits revisáveis',
            'Melhor opção': 'Aider',
            'Por quê': 'Cada edição se torna um commit; trivial de revisar e desfazer',
          },
          {
            'Caso de uso': 'SSH em um servidor, trabalhar sem IDE',
            'Melhor opção': 'Aider',
            'Por quê': 'Somente terminal por design',
          },
          {
            'Caso de uso': 'Usuário de Vim ou Neovim como ferramenta principal',
            'Melhor opção': 'Aider',
            'Por quê': 'Combina de forma natural com a edição orientada ao terminal',
          },
          {
            'Caso de uso': 'Usuário de IDE JetBrains (IntelliJ, GoLand, PyCharm)',
            'Melhor opção': 'Continue.dev ou Aider',
            'Por quê': 'O Cline é somente para VS Code',
          },
        ],
      },
      decisionTree: {
        ...
        content:
          '**Cinco perguntas, em ordem, levam a maioria dos leitores à escolha certa.**',
        items: [
          '**1. Você está no JetBrains, Vim ou trabalhando por SSH?** → Se sim, você está entre Continue.dev (somente JetBrains) e Aider (Vim/SSH). Pule para a Q4. Se não, continue.',
          '**2. Você quer um agente autônomo que edite vários arquivos em uma única tarefa?** → Sim: Cline. Não: continue.',
          '**3. Você quer autocompletar mais um painel de chat integrado ao IDE?** → Sim: Continue.dev. Não: continue.',
          '**4. Você revisa as mudanças com `git diff` e quer que cada edição seja commitada automaticamente?** → Sim: Aider. Não: Continue.dev (a opção padrão).',
          '**5. Ainda na dúvida?** → Comece com o Continue.dev. É a instalação com menos atrito, funciona no editor que você provavelmente já usa, e adicionar o Aider mais tarde para o trabalho orientado ao terminal é simples. O Cline é o mais disruptivo de adicionar a um fluxo de trabalho; adote-o somente quando souber que quer o loop de agente.',
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
        title: 'Erros comuns ao escolher uma ferramenta de código local',
        items: [
          '**Erro 1: escolher a ferramenta antes do modelo.** Um modelo ajustado para código de 30B em qualquer uma das três ferramentas supera um modelo de propósito geral de 7B na "melhor" ferramenta. Escolha o modelo primeiro; a ferramenta é a segunda decisão.',
          '**Erro 2: usar um único modelo para autocompletar e chat no Continue.dev.** O autocompletar precisa de latência abaixo de 200 ms; o chat tolera segundos. Execute um modelo de autocompletar pequeno e um modelo de chat maior na mesma configuração.',
          '**Erro 3: deixar a autoaprovação do Cline totalmente aberta.** Um loop de modelo com `execute_command` autoaprovado pode causar dano real. Mantenha as ferramentas destrutivas com portão até confiar no modelo nesta base de código.',
          '**Erro 4: usar o Aider com um modelo que parafraseia o código.** Se o modelo não consegue reproduzir os blocos SEARCH literalmente, cada edição falha. Use um modelo ajustado para código de 22B ou mais (Qwen3-Coder, DeepSeek Coder, Codestral) para edições de diff confiáveis.',
          '**Erro 5: trocar de ferramenta para resolver um problema do modelo.** Se as chamadas de ferramentas falham no Cline, provavelmente também falharão no modo Agent do Continue.dev pela mesma razão. Diagnostique o modelo primeiro.',
          '**Erro 6: executar o Cline contra um modelo de contexto de 32K em tarefas de vários arquivos.** O consumo de tokens estoura o orçamento no meio da tarefa. Use um modelo de contexto de 128K para o trabalho não trivial com o Cline.',
        ],
      },
      sources: {
        id: 'sources',
        title: 'Fontes',
        items: [
          '[Documentação do Continue.dev](https://docs.continue.dev/) — guia de configuração oficial, configuração de modelos, provedores de contexto `@` e referência do modo Agent.',
          '[Repositório do Cline no GitHub](https://github.com/cline/cline) — código-fonte, esquemas de ferramentas, comportamento do modo Plan/Act e arquitetura da extensão.',
          '[Documentação do Aider](https://aider.chat/) — referência oficial de CLI, documentação do formato de edição, mecânicas do mapa de repositório e padrões por modelo.',
          '[Biblioteca de modelos do Ollama](https://ollama.com/library) — modelos locais disponíveis e níveis de quantização referenciados para cada ferramenta.',
          '[Cartão do modelo Qwen3-Coder](https://huggingface.co/Qwen/Qwen3-Coder-30B) — arquitetura, comprimentos de contexto compatíveis e dados de treinamento de chamadas de ferramentas para o modelo de código recomendado.',
        ],
      },
      faq: {
        id: 'faq',
        ...
        faqs: [
          {
            q: 'Qual é a melhor opção para usuários do VS Code?',
            a: 'Os três funcionam no VS Code, mas Continue.dev e Cline são as opções naturais. Continue.dev para autocompletar em primeiro lugar e chat integrado ao IDE; Cline para tarefas de agente autônomo. O Aider funciona perfeitamente a partir do terminal integrado do VS Code, mas não se integra à superfície do editor em si — sua interface é o terminal.',
          },
          {
            q: 'O Cline é compatível com o IDE Cursor?',
            a: 'O Cursor é um fork do VS Code e aceita extensões do VS Code. O Cline se instala e executa no Cursor; a superfície da extensão é idêntica. O time do Cline não foca oficialmente no Cursor, então os casos limite (APIs específicas do Cursor, mudanças futuras do Cursor) não são garantidos, mas na prática os usuários não relatam problemas.',
          },
          {
            q: 'O Aider pode funcionar sem Git?',
            a: 'Sim, mas você perderia a primitiva de fluxo de trabalho que o define. O Aider funciona em diretórios sem Git com `--no-git`; as edições são aplicadas mas não há commits, então `/undo` fica desabilitado e a reversão recai no histórico de diffs do modelo dentro da sessão. Para trabalho não trivial, inicialize um repositório Git — mesmo um ignorado — antes de começar.',
          },
          {
            q: 'Qual tem a melhor refatoração de vários arquivos?',
            a: 'Cline para trabalho autônomo de vários arquivos — ele planeja entre arquivos, os lê conforme necessário e edita com aprovação por etapa. Aider para trabalho revisável de vários arquivos onde você nomeia os arquivos explicitamente e quer que cada edição seja commitada. O Continue.dev exige condução manual por arquivo; é o mais fraco dos três em refatorações de vários arquivos.',
          },
          {
            q: 'Eles são compatíveis com o Ollama local de imediato?',
            a: 'Sim. O Continue.dev tem um provedor nativo de Ollama com um bloco de configuração. O Cline lista o Ollama no menu suspenso de provedores. O Aider suporta o Ollama através do LiteLLM com `--model ollama_chat/<model-name>`. O Ollama é o backend local com menos atrito para os três.',
          },
          {
            q: 'Qual funciona melhor com o Qwen3-Coder?',
            a: 'O Qwen3-Coder 30B é confiável nas três ferramentas em maio de 2026 — lida com o formato de ferramenta XML do Cline, os blocos de busca e substituição do Aider e os fluxos de trabalho de autocompletar + chat do Continue.dev sem degradação perceptível. As variantes menores do Qwen3-Coder (classe 7B) realizam chamadas de ferramentas com menos confiabilidade e funcionam melhor no modo somente autocompletar do Continue.dev do que no loop de agente do Cline.',
          },
          {
            q: 'Posso usar essas ferramentas com o modo local do Cursor?',
            a: 'Os recursos integrados do Cursor (autocompletar com Tab, Cmd+K) usam o roteamento de modelos próprio do Cursor, não essas ferramentas. Para usar um LLM local no Cursor, instale o Continue.dev ou o Cline como extensões do VS Code dentro do Cursor — eles trazem sua própria configuração de provedor independente da do Cursor. O Aider funciona no terminal integrado do Cursor do mesmo jeito que no do VS Code.',
          },
          {
            q: 'Qual é a mais estável para sessões longas?',
            a: 'Aider, por não ter estado em processo que possa corromper — cada edição é um commit, e uma falha de sessão perde apenas a conversa, não o trabalho. O Continue.dev é estável ao longo de sessões longas no IDE. As sessões longas do Cline podem sofrer pressão na janela de contexto com modelos menores; reiniciar tarefas entre refatorações grandes evita o problema.',
          },
          {
            q: 'Eles são compatíveis com prompts de sistema personalizados?',
            a: 'Os três. Continue.dev: `systemMessage` por modelo em `config.json`. Cline: campo "Custom Instructions" na configuração da extensão. Aider: flag `--system-prompt` ou padrões por modelo em `.aider.conf.yml`. Os prompts personalizados são especialmente úteis para o Cline (orientar o comportamento do modo Plan) e o Aider (aplicar um estilo de código consistente entre commits).',
          },
          {
            q: 'O que os times de DevOps ou de plataforma devem considerar sobre privacidade ao usar agentes de código local?',
            a: 'Se o código ou os logs que os LLMs locais processam puderem conter dados de usuários brasileiros ou da América Latina, aplicam-se normas regionais de privacidade. Os LLMs locais em si (Ollama, llama.cpp) não armazenam dados pessoais, mas os diretórios de código e logs que indexam podem conter informações sensíveis. Garanta que o modelo seja implantado em infraestrutura da empresa sem acesso a APIs externas. No Brasil, a LGPD (Lei Geral de Proteção de Dados) e a ANPD (Autoridade Nacional de Proteção de Dados) regulam o tratamento de dados pessoais — consulte seu time de conformidade antes de implantar em produção em setores regulados.',
          },
          {
            q: 'Qual é a melhor opção para pequenas e médias empresas brasileiras?',
            a: 'O Aider oferece a melhor relação custo-benefício para PMEs: sem dependência de licenças de IDE, totalmente baseado em terminal (combina com ambientes de desenvolvimento existentes), requisitos mínimos de hardware (funciona com um modelo de contexto de 32K) e a trilha de auditoria completa fica no log do Git (simplifica a conformidade e a revisão de código). O Continue.dev é a segunda opção para times fortemente investidos em VS Code/JetBrains. O Cline fica caro em consumo de tokens e exige mais hardware, mas combina com fluxos de trabalho ágeis de refatoração intensiva. As PMEs economizam custos com implantação local com modelos locais — a escolha da ferramenta deve seguir a infraestrutura existente e os fluxos de trabalho dos desenvolvedores, não o hype.',
          },
        ],
      },
      relatedReading: {
        ...
        items: [
          '[Substituir o GitHub Copilot por um LLM local](/pt/power-local-llm/replace-github-copilot-with-local-llm) — contexto mais amplo de substituição, incluindo os cálculos de custo e privacidade.',
          '[Melhores modelos de código local em 2026](/pt/power-local-llm/best-local-coding-models-2026) — o lado do modelo na equação: Qwen3-Coder vs DeepSeek vs Codestral em benchmarks reais.',
          '[Agentes de IA local com MCP em 2026](/pt/power-local-llm/local-ai-agents-with-mcp-2026) — para usuários que querem chamadas de ferramentas estendidas além das ferramentas acima.',
          '[Melhores modelos locais para chamadas de ferramentas em 2026](/pt/power-local-llm/best-local-models-tool-calling-2026) — profundidade técnica adjacente sobre confiabilidade de chamadas de ferramentas.',
          '[llama.cpp vs Ollama vs vLLM](/pt/local-llms/llamacpp-vs-ollama-vs-vllm) — a camada do motor de inferência sob as três ferramentas.',
          '[Hub Power Local LLM](/power-local-llm) — biblioteca completa de guias.',
        ],
      },
    },
      schema: {
      '@context': 'https://schema.org',
      '@type': 'TechArticle',
      headline: 'Continue.dev vs Cline vs Aider: Melhor agente de código local em 2026',
      description: 'Continue.dev, Cline e Aider comparados como agentes de código local em 2026. Modelo de edição, integração com IDE, desfazer e estratégia de contexto — escolhidos por fluxo de trabalho, não por hype.',
      url: 'https://www.promptquorum.com/pt/power-local-llm/continue-dev-vs-cline-vs-aider-local',
      inLanguage: 'pt-BR',
      datePublished: '2026-05-24',
      dateModified: '2026-08-27',
      author: { '@type': 'Person', name: 'Hans Kuepper', sameAs: 'https://www.linkedin.com/in/hanskuepper/' },
  ...
  ar: {
    ...
    audience:
      'المطورون الذين يشغّلون نموذجاً لغوياً محلياً للبرمجة (Qwen3-Coder أو DeepSeek Coder أو Codestral أو Llama 3.3) ويحتاجون إلى اختيار أداة تتناسب مع محرر النصوص وسير عمل مشروعهم.',
    readTime: '14 دقائق قراءة',
    educationalLevel: 'Advanced',
    primaryTerm: 'وكيل البرمجة المحلي',
    targetKeywords: [
      'continue.dev مقابل cline',
      'cline مقابل aider',
      'أفضل وكيل برمجة محلي 2026',
      'أداة برمجة llm محلية',
      'aider نموذج لغوي محلي',
      'continue.dev نموذج لغوي محلي',
    ],
    leadAnswerBlock:
      '**تحلّ Continue.dev وCline وAider مشكلات مختلفة باستخدام النموذج المحلي ذاته. تُبنى Continue.dev حول الإكمال التلقائي ولوحة دردشة مدمجة في بيئة التطوير — الأمثل لسير عمل التحرير أثناء الكتابة في VS Code أو JetBrains. أما Cline فوكيل مستقل في VS Code يُخطط ويحرر وينفذ أوامر عبر ملفات متعددة مع بوابات موافقة صريحة — الأمثل لإعادة الهيكلة متعددة الملفات والبحث الاستكشافي عن الأخطاء. وأداة Aider هي واجهة سطر أوامر أصيلة في Git تحوّل كل تحرير إلى commit بفوارق جراحية من بحث واستبدال — الأمثل للعمل على نطاق المستودع والمراجعين الذين يريدون كل تغيير في `git log`. اختر حسب بدائية سير العمل (إكمال تلقائي مقابل حلقة وكيل مقابل فوارق Git)، لا حسب أيّ الأدوات يبدو أقوى.**',
    quickAnswerTop: {
      ar: {
        ...
        bullets: [
          ...
          'أداة Aider — واجهة سطر أوامر بـcommits أصيلة في Git وفوارق بحث واستبدال. الأمثل للعمل القابل للمراجعة على نطاق المستودع مع دعم التراجع.',
    ...
    toc: [
      ...
      { label: 'Continue.dev: دراسة معمّقة', anchor: '#continue-dev' },
      { label: 'Cline: دراسة معمّقة', anchor: '#cline' },
      { label: 'Aider: دراسة معمّقة', anchor: '#aider' },
      { label: 'نموذج التحرير: كيف تكتب كل أداة الكود', anchor: '#edit-model' },
      { label: 'التراجع والرجوع', anchor: '#undo' },
      { label: 'استراتيجية نافذة السياق', anchor: '#context' },
      { label: 'موثوقية استدعاءات الأدوات', anchor: '#tool-calls' },
      { label: 'التكامل مع بيئة التطوير', anchor: '#ide-integration' },
      { label: 'ملاءمة سير العمل حسب حالة الاستخدام', anchor: '#workflow-fit' },
      { label: 'شجرة القرار', anchor: '#decision-tree' },
    ...
    gammaEmbedUrl: '/presentations/continue-dev-vs-cline-vs-aider-local-static.html',
    gammaDescription: 'يتناول عرض الشرائح أدناه: كيف تختلف Continue.dev وCline وAider كأدوات برمجة محلية (إكمال تلقائي مقابل وكيل مستقل مقابل فوارق أصيلة في git)، وجدول مقارنة الميزات جنباً إلى جنب، وموثوقية نموذج التحرير حسب الأداة وحجم النموذج، واستراتيجية نافذة السياق (32K مقابل 128K)، والتكامل مع بيئة التطوير (VS Code وJetBrains وVim/SSH)، و6 أخطاء شائعة عند اختيار أداة. نزّل ملف PDF كبطاقة مرجعية لوكلاء البرمجة المحلية.',
    sections: {
      tldr: {
        id: 'key-takeaways',
        isTldr: true,
        items: [
          ...
          '**موثوقية استدعاءات الأدوات** تعتمد على النموذج لا على الأداة. Qwen3-Coder 30B وDeepSeek-V3 هما الأكثر موثوقية في استدعاءات الأدوات في مايو 2026؛ أما النماذج الأصغر (فئة 7B) فتُصدر باستمرار استدعاءات أدوات مشوّهة بغض النظر عن الأداة المستخدمة.',
          '**تختلف استراتيجية نافذة السياق** اختلافاً واضحاً: تعتمد Continue.dev على فهرس قاعدة الكود المضمّن والسياق المثبّت بـ`@`، بينما يبث Cline محتوى الملفات كاملاً إلى المحادثة (استهلاك توكن مرتفع)، ويستخدم Aider خريطة مستودع tree-sitter مع `/add` و`/drop` الصريحين للإبقاء على مجموعة العمل صغيرة.',
          'بالنسبة للمطور المنفرد، تُهيمن **ملاءمة سير العمل** على النتائج أكثر من أي معيار قياسي. الأداة التي تبقيها فعلاً مفتوحة هي التي ينبغي أن تختارها.',
        ],
      },
      quickFacts: {
        id: 'quick-facts',
        ...
        items: [
          ...
          '**موثوقية استدعاءات الأدوات خاصية النموذج لا الأداة.** Qwen3-Coder 30B وDeepSeek-V3 موثوقان في الأدوات الثلاث؛ ونماذج فئة 7B تفشل في الثلاثة.',
          '**ميزانية السياق:** Continue.dev وAider مرتاحتان مع نموذج سياق 32K؛ يحتاج Cline إلى **128K** للمهام غير البديهية متعددة الملفات.',
          '**اختر حسب المحرر:** JetBrains → Continue.dev أو Aider. Vim/Neovim أو SSH → Aider. VS Code مع أيام إكمال تلقائي مكثف → Continue.dev. VS Code مع إعادة هيكلة متعددة الملفات → Cline.',
        ],
      },
      comparisonTable: {
        id: 'comparison-table',
        title: 'كيف تتقابل Continue.dev وCline وAider في 2026',
        content:
          'الأدوات الثلاث مفتوحة المصدر ومجانية وتعمل مع أي نقطة نهاية لنموذج لغوي محلي. الفروق الهيكلية أدناه تحدد أيّها يناسب سير عملك.',
        image: '/images/coding-agent-comparison-es.svg',
        imageCaption: 'Continue.dev مقابل Cline مقابل Aider: Continue.dev للإكمال التلقائي في VS Code/JetBrains (سياق 32K)، Cline للوكلاء المستقلين متعددي الملفات في VS Code (128K موصى به)، Aider لـcommits git الأصيلة في الطرفية (سياق 32K).',
        snippetBlocks: [
          {
            type: 'one-sentence',
            text: 'تطبيق Continue.dev إكمال تلقائي بأسلوب Copilot، وأداة Cline وكيل مستقل يحرر بين الملفات، وأداة Aider أداة طرفية تودع كل تحرير في Git — اختر حسب سير العمل لا المعيار القياسي.',
          },
          {
            type: 'plain-terms',
        ...
        rows: [
          {
            ...
            'Continue.dev': 'إضافة لـVS Code وJetBrains',
            'Cline': 'إضافة لـVS Code',
            'Aider': 'واجهة سطر أوامر (أي محرر بجانبها)',
          },
          {
            'الميزة': 'بدائية سير العمل',
            'Continue.dev': 'إكمال تلقائي + دردشة مدمجة في بيئة التطوير',
            'Cline': 'حلقة وكيل مستقل مع بوابات موافقة',
            'Aider': 'تعديلات فارق أصيل في Git، كل تغيير commit',
          },
          {
            'الميزة': 'نموذج التحرير',
            'Continue.dev': 'FIM مضمّن + تطبيق على الملف من الدردشة',
            'Cline': 'استدعاءات أدوات (read_file وwrite_to_file وreplace_in_file)',
            'Aider': 'كتل فارق بحث واستبدال؛ وضع "كامل" للملفات الصغيرة',
          },
          {
            'الميزة': 'تعديلات متعددة الملفات',
            'Continue.dev': 'يدوي: الدردشة تقترح، وأنت تطبق ملفاً بملف',
            'Cline': 'أصيل: الوكيل يحرر بين الملفات في مهمة واحدة',
            'Aider': 'أصيل: سمّ الملفات في الطلب وتستقر التعديلات في commit',
          },
          {
            'الميزة': 'التراجع / الرجوع',
            'Continue.dev': 'مكدس تراجع VS Code على التعديلات المطبّقة',
            'Cline': 'رجوع لكل تعديل في واجهة الوكيل؛ Git للرجوع الكامل',
            'Aider': '`/undo` يرجع الـcommit الأخير؛ تاريخ git الكامل في الأسفل',
          },
          {
            'الميزة': 'بروتوكول استدعاء الأدوات',
            'Continue.dev': 'أدوات OpenAI (function-calling) في وضع الوكيل',
            'Cline': 'كتل أدوات XML مخصصة بمخطط صارم',
            'Aider': 'لا استدعاءات أدوات؛ يوجّه النموذج لإصدار كتل الفارق',
          },
          {
            'الميزة': 'سياق قاعدة الكود',
            'Continue.dev': 'فهرس محلي (TF-IDF + تضمينات) + موفرو `@`',
            'Cline': 'يبث محتوى الملفات كاملاً إلى المحادثة',
            'Aider': 'خريطة مستودع Tree-sitter + `/add` و`/drop` يدوياً',
          },
          {
            'الميزة': 'الموافقة / الأمان',
            'Continue.dev': 'معاينة الفارق قبل كل تطبيق',
            'Cline': 'موافقة خطوة بخطوة؛ قوائم أذونات الموافقة التلقائية قابلة للإعداد',
            'Aider': 'معاينة الفارق في الطرفية؛ خطوة الـcommit تلقائية',
          },
          {
            'الميزة': 'الأفضل لـ',
            'Continue.dev': 'التحرير أثناء الكتابة، دردشة مدمجة في بيئة التطوير، أيام الإكمال التلقائي المكثف',
            'Cline': 'إعادة الهيكلة متعددة الملفات، تصحيح الأخطاء الاستكشافي، مهام المستودع',
            'Aider': 'تغييرات قابلة للمراجعة على نطاق المستودع، سير عمل موجهة للطرفية',
          },
        ],
      },
      whichOne: {
        id: 'which-one',
        title: 'أيّها ينبغي أن تختار',
        content:
          '**الأداة الصحيحة يحددها محررك، وتحملك للتعديلات المستقلة، وطريقة مراجعة التغييرات.** استخدم هذا الاختصار.',
        image: '/images/coding-agent-which-one-es.svg',
        imageCaption: 'أي أداة برمجة محلية تختار: Continue.dev للإكمال التلقائي والدردشة في بيئة التطوير (VS Code + JetBrains)، Cline لحلقات الوكيل متعددة الملفات وتنفيذ الأوامر، Aider لسير العمل الأصيلة في git في الطرفية وبيئات SSH.',
        columns: ['وضعك', 'اختر'],
        rows: [
          { 'وضعك': 'أعيش في VS Code أو بيئة تطوير JetBrains وأريد إكمالاً تلقائياً بأسلوب Copilot', 'اختر': 'Continue.dev' },
          { 'وضعك': 'أريد لوحة دردشة جانبية تصل إلى قاعدة الكود دون رفع ملفات يدوياً', 'اختر': 'Continue.dev' },
          { 'وضعك': 'أريد وكيلاً يخطط لإعادة هيكلة عبر ملفات متعددة ويسأل قبل كل خطوة', 'اختر': 'Cline' },
          { 'وضعك': 'أريد أن يتمكن النموذج من تنفيذ الأوامر وقراءة مخرجات الطرفية ضمن حلقته', 'اختر': 'Cline' },
          { 'وضعك': 'أراجع التغييرات بـ`git diff` وأريد أن يُودَع كل تحرير تلقائياً', 'اختر': 'Aider' },
          { 'وضعك': 'أعمل عبر SSH أو في بيئة موجهة للطرفية', 'اختر': 'Aider' },
          { 'وضعك': 'أحتاج أحياناً إلى سير العمل الثلاثة — اختر الأبسط أولاً', 'اختر': 'ابدأ بـContinue.dev؛ أضف Aider حين تحتاج إلى فوارق أصيلة في Git' },
        ],
      },
      continueDevDeep: {
        id: 'continue-dev',
        title: 'Continue.dev: الخيار الموجه للإكمال التلقائي',
        content:
          '**تطبيق Continue.dev هو الأقرب مفتوح المصدر إلى GitHub Copilot للنماذج المحلية.** سطحه الرئيسي هو المحرر: الإكمال بـTab أثناء الكتابة مع لوحة دردشة جانبية تعرف قاعدة الكود لديك.',
        items: [
          '**مسار التثبيت:** VS Code Marketplace وJetBrains Marketplace. يقع الإعداد في `~/.continue/config.json` (أو `config.yaml`)؛ تعريفات النماذج وإعداد الإكمال التلقائي وموفرو السياق `@` كلها هناك.',
          '**بدائية سير العمل:** إكمال تلقائي عبر FIM (fill-in-the-middle) مع لوحة دردشة جانبية مدمجة في بيئة التطوير. يمكن للوحة الدردشة تطبيق التعديلات مباشرةً على الملفات المفتوحة؛ يضيف وضع الوكيل حلقات استدعاءات أدوات للمهام متعددة الخطوات.',
          '**إعداد النموذج المحلي:** دعم Ollama الأصيل عبر كتلة إعداد. وتعمل llama.cpp وLM Studio وأي نقطة نهاية متوافقة مع OpenAI أيضاً. النمط المعتاد هو تشغيل نماذج منفصلة للإكمال التلقائي (صغير وسريع — فئة Qwen3-Coder 1.5B) والدردشة (أكبر — Qwen3-Coder 30B وDeepSeek-V3).',
          '**سياق قاعدة الكود:** فهرس محلي (TF-IDF مع تضمينات) في مجلد مشروعك. يصل `@codebase` و`@file` و`@folder` و`@docs` و`@terminal` والموفرون المخصصون إلى الفهرس من لوحة الدردشة.',
          '**نموذج التحرير:** في الدردشة، يقترح النموذج كتلة كود؛ النقر على "Apply" يصنع فارقاً مع الملف المفتوح ويمكنك قبول التغيير أو رفضه. التعديلات المضمّنة عبر Cmd/Ctrl+I تعيد كتابة تحديد في مكانه. كلا التدفقين يُظهران معاينة الفارق قبل أي كتابة.',
          '**أين يتميز:** التحرير بلوحة المفاتيح داخل بيئة تطوير تستخدمها مسبقاً، وانخفاض العبء المعرفي لكل تعديل، وحلقة تغذية راجعة سريعة. الأداة التي تنسى أنك ثبّتتها.',
      ...
      clineDeep: {
        id: 'cline',
        title: 'Cline: خيار الوكيل المستقل',
        content:
          '**تتعامل أداة Cline مع الكود باعتباره مهمة وكيل لا سلسلة تعديلات.** تعطيها هدفاً؛ فتخطط وتقرأ الملفات وتحررها وتنفذ الأوامر وتطلب منك الموافقة في كل خطوة.',
        items: [
          '**مسار التثبيت:** VS Code Marketplace (لا إصدار لـJetBrains). يتم الإعداد عبر واجهة المستخدم داخل لوحة الإضافة — الموفر والنموذج وإعداد الموافقة التلقائية والتعليمات المخصصة.',
          '**بدائية سير العمل:** حلقة وكيل مستقل بأوضاع تخطيط وتنفيذ صريحة. يُفكّر وضع التخطيط في المهمة دون كتابة ملفات؛ يُنفّذ وضع التنفيذ الخطة عبر استدعاءات أدوات. تظهر بوابة موافقة بين الخطوات ما لم تضع الأدوات في قائمة أذونات الموافقة التلقائية.',
          '**إعداد النموذج المحلي:** أي نقطة نهاية متوافقة مع OpenAI. Ollama بنقرة واحدة. لـllama.cpp أو vLLM، وجّه Cline إلى URL الخادم المتوافق مع OpenAI. تستهلك حلقة الوكيل توكن كثيرة — احتفظ بنموذج بنافذة سياق لا تقل عن 32K (Qwen3-Coder 30B بـ32K، وDeepSeek-V3 بـ128K).',
          '**سطح الأدوات:** `read_file` و`write_to_file` و`replace_in_file` و`execute_command` و`list_files` و`search_files` و`browser_action` و`ask_followup_question` و`attempt_completion`. المجموعة صغيرة وموثقة جيداً؛ تستخدم مخططات الأدوات صيغة XML مخصصة يجب أن يصدرها النموذج بشكل صحيح.',
          '**نموذج التحرير:** للملفات ذات بضع مئات من الأسطر، يكتب Cline الملف بأكمله. للملفات الأكبر، يُصدر عمليات `replace_in_file` مع كتل بحث واستبدال صريحة. تُعرض الفوارق في اللوحة قبل كل كتابة.',
          '**أين يتميز:** إعادة الهيكلة متعددة الملفات، ومهام "لماذا يفشل هذا الاختبار؟" الاستكشافية، وسير عمل "إضافة ميزة تمس ثلاث طبقات". الأقوى بين الثلاثة في المهام التي تتجاوز خمسة تعديلات ملفات.',
          '**أين يقصر:** استهلاك التوكن مرتفع — محتوى الملفات الكامل يُبث إلى المحادثة عند كل قراءة. الجلسات الطويلة تستنزف نماذج السياق 32K؛ يُوصى بنماذج سياق 128K للمهام غير البديهية. تتراجع موثوقية استدعاءات الأدوات بشكل حاد في النماذج الأصغر (فئة 7B) — تُصدر XML مشوّهاً وتتوقف الحلقة.',
        ],
        callouts: [
          {
            type: 'note',
            text: 'قائمة أذونات الموافقة التلقائية في Cline هي الفرق بين وكيل مفيد ووكيل مُمل. اسمح بـ`read_file` و`list_files` و`search_files` افتراضياً؛ وابقِ `write_to_file` و`replace_in_file` و`execute_command` محمية بسلوك البوابة حتى تثق في النموذج مع قاعدة الكود هذه. اضبط حسب المشروع.',
          },
        ],
      },
      aiderDeep: {
        id: 'aider',
        title: 'Aider: الخيار الأصيل في Git',
        content:
          '**أداة Aider هي الوحيدة من بين الثلاثة التي تتعامل مع مستودعك باعتباره مصدر الحقيقة لا المحادثة.** كل تعديل يستقر كـcommit في Git. التراجع هو `git reset`. تاريخ الدردشة قابل للتجاهل؛ سجل الـcommits هو الأثر.',
        items: [
          '**مسار التثبيت:** `pip install aider-chat` أو `pipx install aider-chat`. استدعاء واجهة سطر الأوامر: `aider file1.py file2.py` (تدخل الملفات في نطاق المحادثة). الإعداد عبر `.aider.conf.yml` وإلغاءات العلم.',
          '**بدائية سير العمل:** تحرير تحادثي موجه للطرفية. اكتب طلباً؛ يقترح Aider فارقاً؛ وإذا قبلت، يُودَع التغيير. `/add` و`/drop` يديران الملفات في النطاق. `/undo` يرجع الـcommit الأخير. `/diff` يُظهر أحدث تغيير مقارنةً بـHEAD.',
          '**إعداد النموذج المحلي:** أي نموذج يمكن الوصول إليه عبر LiteLLM — Ollama أو الخوادم المتوافقة مع OpenAI (llama.cpp وvLLM وLM Studio) أو موفري النماذج مباشرةً. `--model ollama_chat/qwen3-coder:30b` هو الاستدعاء المحلي النموذجي.',
          '**سياق قاعدة الكود:** خريطة مستودع tree-sitter. يحلل Aider مستودعك بحثاً عن التعريفات (الدوال والأصناف والطرق)، يلخصها بالقرب من الملفات في النطاق، ويضمّن الملخص في طلب النظام. تتسع الخريطة في حدود 1K–4K توكن للمستودعات الصغيرة وأكثر للكبيرة — `--map-tokens` يحدد الميزانية.',
          '**نموذج التحرير:** استراتيجيتان. يُصدر وضع "diff" كتل بحث واستبدال يجب أن ينتجها النموذج حرفياً — جراحي ومنخفض التوكن، لكنه يفشل إن أعاد النموذج صياغة كتلة البحث. وضع "كامل" يعيد كتابة الملفات بأكملها — قوي للملفات الصغيرة ومكلف للكبيرة. يختار Aider تلقائياً القيم الافتراضية الأنسب لكل نموذج.',
          '**أين يتميز:** التغييرات على نطاق المستودع التي تريد كل خطوة في سجل الـcommits؛ البرمجة الثنائية مع مراجعة الكود المدمجة (كل commit وحدة مراجعة مستقلة)؛ سير عمل SSH وdevcontainer حيث إضافات بيئة التطوير ليست خياراً.',
          '**أين يقصر:** لا إكمال تلقائي. لا معاينة في بيئة التطوير افتراضياً (يعمل بشكل جيد في لوحة طرفية بـVS Code بجانب محررك، لكنه غير مدمج). صيغة تعديل الفارق حساسة لجودة مخرجات النموذج — نموذج يعيد صياغة كتلة البحث بدلاً من إعادة إنتاجها سيفشل التعديلات وستشاهد محاولات إعادة في الطرفية.',
        ],
        callouts: [
          {
            type: 'tip',
            text: 'اضبط `auto-commits: true` (القيمة الافتراضية) واستخدم `git rebase -i` لدمج commits Aider في نهاية ميزة. الـcommits التفصيلية أثناء العمل ثمينة لـ`/undo`؛ أما الـcommit النهائي المدمج فهو ما يراجعه فريقك. يوثّق Aider هذا النمط تحديداً.',
          },
        ],
      },
      editModel: {
        id: 'edit-model',
        title: 'نموذج التحرير: كيف تكتب كل أداة الكود',
        content:
          '**نموذج التحرير هو أكبر محدد منفرد لكيفية شعورك بكل أداة.** نفس النموذج، نفس المهمة — ثلاثة مخرجات مختلفة تماماً.',
        image: '/images/coding-agent-edit-model-es.svg',
        imageCaption: 'موثوقية نموذج التحرير حسب الأداة: في نماذج 7B، Continue.dev الأكثر تسامحاً، Aider يرفض الكتل غير المتطابقة، Cline يتوقف عند XML مشوّه؛ في نماذج البرمجة 30B+ (Qwen3-Coder وDeepSeek-V3)، الأدوات الثلاث موثوقة.',
        items: [
          '**تطبيق Continue.dev** — للإكمال التلقائي: توكنات FIM حول المؤشر تنتج إكمالات مضمّنة. للتعديلات الموجهة بالدردشة: يُعيد النموذج كتلة كود محددة؛ "Apply" يصنع فارقاً مع الملف المفتوح ويُعرضه للقبول/الرفض. للتعديلات المضمّنة (Cmd/Ctrl+I على تحديد): يعيد النموذج كتابة التحديد في مكانه ويُظهر الفارق هناك.',
          '**أداة Cline** — كل تعديل استدعاء أداة. `write_to_file` للملفات الجديدة أو إعادة الكتابة الكاملة، `replace_in_file` للتغييرات الجراحية مع كتل `<<<<<<< SEARCH` / `>>>>>>> REPLACE` الصريحة. تُتحقق من استدعاءات الأدوات مقابل مخطط؛ المشوّهة منها تولّد "خطأ أداة" يراه الوكيل ويُعيد المحاولة. المحادثة هي استدعاءات أدوات منظمة ونتائج أدوات، لا نثر حر.',
          '**أداة Aider** — كتل فارق بحث واستبدال مُقدَّمة بنثر بسيط، تُحلَّل في واجهة سطر الأوامر. يُصدر النموذج ما يشبه كتلة فارق موحّد محددة؛ يطبقها Aider ويودّعها. إن لم تتطابق كتلة SEARCH تماماً مع الملف، يُرفض التعديل ويطلب Aider من النموذج إعادة المحاولة.',
          '**ترتيب الموثوقية في نموذج فئة 7B:** Continue.dev > Aider > Cline. "Apply" في Continue.dev متسامح — الخوارزمية تتصالح مع الصياغات الطفيفة المختلفة. Aider يرفض عند عدم تطابق كتل البحث لكن الفشل صاخب وقابل للتعافي. صيغة أداة XML لـCline هي الأكثر هشاشة في النماذج الصغيرة — استدعاءات الأدوات المشوّهة توقف الحلقة.',
          '**ترتيب الموثوقية في نموذج استدعاء الأدوات 30B+ (Qwen3-Coder 30B وDeepSeek-V3):** تتقارب الأدوات الثلاث — النموذج قادر على إصدار كتل فوارق دقيقة واستدعاءات أدوات مُشكَّلة جيداً باستمرار.',
          'لتقنيات الـprompting المنظم التي تحسّن توليد الكود بغض النظر عن الأداة، راجع [اكتب كوداً أفضل بالذكاء الاصطناعي](/ar/prompt-engineering/write-better-code-with-ai).',
        ],
      },
      undo: {
        id: 'undo',
        title: 'التراجع والرجوع وسؤال "ماذا فعل الوكيل للتو؟"',
        content:
          '**التراجع هو الميزة التي لا تلاحظها إلا حين تحتاجها.** الأدوات الثلاث قابلة للتعافي؛ والفرق في مقدار الاحتكاك بين "هذا خطأ" و"اختفى الخطأ".',
        items: [
          '**تطبيق Continue.dev** — تنضم التعديلات المطبّقة إلى مكدس تراجع VS Code. Cmd/Ctrl+Z يرجع آخر تطبيق لكل ملف. لرجوع ملفات متعددة، سير عمل Git الموجود لديك يؤدي العمل الثقيل. لا يوجد أمر "تراجع عن هذه المحادثة كلها" على مستوى الأداة.',
          '**أداة Cline** — تُظهر واجهة الوكيل كل خطوة كبطاقة؛ الرجوع لكل تعديل نقرة واحدة. تبقى البطاقات طوال مدة المهمة. بعد إنهاء المهمة، الرجوع يكون عبر Git — لا يودّع Cline تلقائياً، لذا قد تجد كومة من التغييرات غير المودَعة بعد جلسة طويلة.',
          '**أداة Aider** — `/undo` يرجع الـcommit الأخير؛ كرر للرجوع أكثر. بما أن كل تعديل commit منفصل، التفصيل يتطابق خطوةً بخطوة مع المحادثة. `git reflog` يسترد كل ما لا يستطيع `/undo` استرداده.',
          '**الانعكاس العملي:** إن كنت تميل إلى ترك الوكيل يعمل 20 دقيقة دون إشراف، يمنحك Aider المخرج الأكثر قابلية للمراجعة بعدها (سجل commits نظيف) ويمنحك Cline أوضح مسار تدقيق جارٍ (بطاقات لكل خطوة لكن دون commits). يفترض Continue.dev أنك بقيت أمام لوحة المفاتيح طوال العملية.',
        ],
        callouts: [
          {
            type: 'tip',
            text: 'لجلسات Cline، نفّذ `git stash` قبل بدء مهمة غير بديهية. إن خرج الوكيل عن المسار، `git stash drop` ينظف أسرع من الرجوع ملفاً بملف. لـAider، سجل الـcommits الموجود هو شبكة الأمان — لا تفعل شيئاً إضافياً.',
          },
        ],
      },
      context: {
        id: 'context',
        title: 'استراتيجية نافذة السياق: أين تذهب التوكنات',
        content:
          '**تنفق الأدوات الثلاث التوكنات بطرق مختلفة.** هذا هو أهم قرار للميزانية حين يكون لديك نموذج بنافذة سياق أصغر (32K أو أقل) ومستودع غير بديهي.',
        image: '/images/coding-agent-context-strategy-es.svg',
        imageCaption: 'استراتيجية نافذة السياق: Continue.dev تستخدم استرداد الفهرس عند الطلب (32K كحد أدنى)، Cline يبث الملفات كاملة (128K موصى به للمهام متعددة الملفات)، Aider تستخدم خريطة مستودع tree-sitter مع /add صريح (32K كحد أدنى).',
        items: [
          '**تطبيق Continue.dev** — فهرس TF-IDF مع تضمينات على المشروع. الاسترداد عند الطلب: فقط المقاطع ذات الصلة بالرسالة الحالية في الدردشة تصل إلى الطلب. `@codebase` يُطلق بحثاً دلالياً؛ `@file path/to/x.ts` يضم الملف حرفياً. استهلاك التوكن يتناسب مع ما تثبّته لا حجم المستودع.',
          '**أداة Cline** — محتوى الملفات الكامل يُبث إلى المحادثة عند كل قراءة. ملف من 500 سطر يستهلك ~3K–5K توكن؛ مهمة وكيل تمس 8 ملفات تستهلك 25K–40K توكن قبل أي استنتاج. الحد الأدنى الموصى به: نافذة سياق 32K؛ 128K مريح.',
          '**أداة Aider** — خريطة مستودع tree-sitter (افتراضياً ~2K توكن، قابلة للإعداد عبر `--map-tokens`) مع محتوى الملفات المضافة صراحةً بـ`/add`. الخريطة ملخص تعريفات فقط لا كود كامل، لذا تتناسب أفضل بكثير مع حجم المستودع من بث الملفات. `/drop` الملفات حين لم تعد في النطاق.',
          '**الانعكاس العملي لنموذج سياق 32K:** تعمل Continue.dev وAider بشكل مريح في معظم المستودعات؛ يبدأ Cline في الشعور بضغط السياق في المهام التي تمس أكثر من 5–6 ملفات ما لم تتحول إلى نموذج سياق 128K.',
          '**الانعكاس العملي لنموذج سياق 128K:** الأدوات الثلاث مريحة. الاستهلاك الأعلى لـCline يتوقف عن أهميته؛ يعود العامل الحاسم إلى بدائية سير العمل.',
          'لشرح أعمق لنوافذ السياق ولماذا تفقد النماذج المعلومات في منتصف السياق، راجع [نوافذ السياق موضحة: لماذا تنسى الذكاء الاصطناعي](/ar/prompt-engineering/context-windows-explained-why-ai-forgets).',
        ],
      },
      toolCalls: {
        id: 'tool-calls',
        title: 'موثوقية استدعاءات الأدوات خاصية النموذج',
        content:
          '**كثيراً ما تُنسب إخفاقات استدعاءات الأدوات خطأً إلى الأداة.** في الواقع يُصدر النموذج استدعاءً مشوّهاً، تتحقق منه الأداة بصرامة وتتوقف الحلقة. نفس النموذج في أداة مختلفة ينتج نفس نمط الفشل.',
        items: [
          '**النماذج التي تُجري استدعاءات أدوات موثوقة لهذه الأدوات (مايو 2026):** Qwen3-Coder 30B (الأفضل في صيغة XML لـCline)، DeepSeek-V3 (سياق واسع، قوي في الخطط متعددة الخطوات)، Llama 3.3 70B (بطيء لكن موثوق)، Codestral 22B (سريع، أفق تخطيط أصغر).',
        ...
        callouts: [
          {
            type: 'note',
            text: 'عند تقييم نموذج محلي جديد، اختبره في الأداة التي تستخدمها فعلاً، لا في معيار قياسي عام لاستدعاءات الأدوات. الإشارة ذات الصلة هي هل يتوقف سير عملك اليومي. نموذج يحصل على 85% في معيار قياسي عام لاستدعاءات الأدوات قد يفشل في نصف مهام Cline إن لم يُصدر XML Cline المحدد بشكل موثوق.',
          },
        ],
      },
      ideIntegration: {
        id: 'ide-integration',
        title: 'التكامل مع بيئة التطوير ونطاق المحرر',
        content:
          '**نطاق المحرر هو المكان الذي تتباين فيه الأدوات الثلاث بشكل أوضح.** هذا هو البُعد الأهم لمعظم القراء؛ وهو أيضاً الأسهل في الاختبار خلال 10 دقائق.',
        items: [
          '**تطبيق Continue.dev** — VS Code Marketplace وJetBrains Marketplace. إصدار JetBrains حقيقي (IntelliJ وPyCharm وGoLand وWebStorm وغيرها) ومتكافئ بالميزات مع إصدار VS Code للإكمال التلقائي والدردشة. يمكن لمستخدمي Cursor تثبيت إضافة VS Code؛ السلوك مماثل لـVS Code.',
          '**أداة Cline** — إضافة VS Code فقط. تعمل في Cursor (وهو fork من VS Code) لكنها غير مختبرة رسمياً؛ سطح الإضافة متطابق ومعظم المستخدمين لا يُبلغون عن مشكلات. مستخدمو JetBrains ليس لديهم خيار حالياً.',
          '**أداة Aider** — موجهة للطرفية؛ تتكامل مع أي محرر بوجودها بجانبه. VS Code: افتح الطرفية المدمجة، شغّل Aider، حرر في لوحة المحرر بينما يودّع Aider. JetBrains: نفس النمط في طرفية JetBrains. Vim/Neovim: مقترن مع `:e!` لإعادة التحميل بعد commits Aider، هذا تدفق منتج. Cursor: مماثل لـVS Code.',
          '**الانعكاس العملي لمستخدمي JetBrains:** Continue.dev أو Aider هما خياراك الوحيدان.',
          '**الانعكاس العملي لمستخدمي Vim/Neovim:** Aider هو الخيار الطبيعي. لـContinue.dev تكاملات Neovim من المجتمع بنضج متفاوت؛ السطح الرسمي هو VS Code وJetBrains.',
        ],
      },
      workflowFit: {
        id: 'workflow-fit',
        title: 'ملاءمة سير العمل حسب حالة الاستخدام',
        content:
          '**الإطار الأكثر فائدة هو "أي نوع من الأيام تمر به".** كيّف الأداة مع اليوم لا العكس.',
        image: '/images/coding-agent-workflow-fit-es.svg',
        imageCaption: 'ملاءمة سير العمل: Continue.dev للإكمال التلقائي والأسئلة في بيئة التطوير؛ Cline لإعادة الهيكلة متعددة الملفات والتصحيح الاستكشافي مع وصول للأوامر؛ Aider للتغييرات الأصيلة في git وSSH وسير عمل Vim/Neovim.',
        columns: ['حالة الاستخدام', 'الخيار الأفضل', 'السبب'],
        rows: [
          {
            'حالة الاستخدام': 'إضافة دالة إلى ملف موجود بالإكمال التلقائي',
            'الخيار الأفضل': 'Continue.dev',
            'السبب': 'الإكمال التلقائي FIM هو البدائية الطبيعية؛ لا تحتاج إلى دردشة',
          },
          {
            'حالة الاستخدام': 'السؤال "ماذا تفعل هذه الدالة؟" دون مغادرة المحرر',
            'الخيار الأفضل': 'Continue.dev',
            'السبب': 'الدردشة في بيئة التطوير مع `@file` هو المسار الأقل احتكاكاً',
          },
          {
            'حالة الاستخدام': 'إعادة هيكلة دالة عبر 5 ملفات أو أكثر',
            'الخيار الأفضل': 'Cline',
            'السبب': 'تعديلات أصيلة متعددة الملفات مع موافقة خطوة بخطوة',
          },
          {
            'حالة الاستخدام': '"لماذا يفشل هذا الاختبار؟" — بحث استكشافي عن الأخطاء',
            'الخيار الأفضل': 'Cline',
            'السبب': 'حلقة الوكيل تنفذ الأوامر وتقرأ السجلات وتتكرر',
          },
          {
            'حالة الاستخدام': 'إعادة تسمية على نطاق المستودع أو ترحيل واجهة برمجية مع commits قابلة للمراجعة',
            'الخيار الأفضل': 'Aider',
            'السبب': 'كل تعديل يصبح commit؛ بديهي للمراجعة والتراجع',
          },
          {
            'حالة الاستخدام': 'SSH إلى خادم، العمل بدون بيئة تطوير',
            'الخيار الأفضل': 'Aider',
            'السبب': 'طرفية فقط بتصميم',
          },
          {
            'حالة الاستخدام': 'مستخدم Vim أو Neovim كأداة رئيسية',
            'الخيار الأفضل': 'Aider',
            'السبب': 'يتكامل بشكل طبيعي مع التحرير الموجه للطرفية',
          },
          {
            'حالة الاستخدام': 'مستخدم بيئة تطوير JetBrains (IntelliJ وGoLand وPyCharm)',
            'الخيار الأفضل': 'Continue.dev أو Aider',
            'السبب': 'Cline لـVS Code فقط',
          },
        ],
      },
      decisionTree: {
        id: 'decision-tree',
        title: 'شجرة القرار: أيّها ينبغي أن تختار',
        content:
          '**خمسة أسئلة بالترتيب تقود معظم القراء إلى الاختيار الصحيح.**',
        items: [
          '**1. هل أنت في JetBrains أو Vim أو تعمل عبر SSH؟** → إن نعم، أنت بين Continue.dev (JetBrains فقط) وAider (Vim/SSH). انتقل إلى السؤال 4. إن لا، تابع.',
          '**2. هل تريد وكيلاً مستقلاً يحرر ملفات متعددة في مهمة واحدة؟** → نعم: Cline. لا: تابع.',
          '**3. هل تريد إكمالاً تلقائياً مع لوحة دردشة مدمجة في بيئة التطوير؟** → نعم: Continue.dev. لا: تابع.',
          '**4. هل تراجع التغييرات بـ`git diff` وتريد أن يُودَع كل تحرير تلقائياً؟** → نعم: Aider. لا: Continue.dev (الخيار الافتراضي).',
          '**5. لا تزال غير متأكد؟** → ابدأ بـContinue.dev. هو الأقل احتكاكاً في التثبيت، ويعمل في المحرر الذي تستخدمه على الأرجح، وإضافة Aider لاحقاً للعمل الموجه للطرفية أمر بسيط. Cline هو الأكثر تعطيلاً لإضافته إلى سير عمل؛ تبنّه فقط حين تعرف أنك تريد حلقة الوكيل.',
        ],
      },
      commonMistakes: {
        id: 'common-mistakes',
        title: 'الأخطاء الشائعة عند اختيار أداة برمجة محلية',
        items: [
          '**الخطأ 1: اختيار الأداة قبل النموذج.** نموذج مضبوط دقيقاً للكود من 30B في أي من الأدوات الثلاث يتفوق على نموذج أغراض عامة من 7B في الأداة "الأفضل". اختر النموذج أولاً؛ الأداة القرار الثاني.',
          '**الخطأ 2: استخدام نموذج واحد للإكمال التلقائي والدردشة في Continue.dev.** الإكمال التلقائي يحتاج زمن استجابة أقل من 200 مللي ثانية؛ الدردشة تتحمل الثواني. شغّل نموذج إكمال تلقائي صغيراً ونموذج دردشة أكبر في نفس الإعداد.',
          '**الخطأ 3: ترك الموافقة التلقائية لـCline مفتوحة تماماً.** حلقة نموذج مع `execute_command` بموافقة تلقائية قد تُسبب ضرراً حقيقياً. ابقِ الأدوات التدميرية محمية بالبوابة حتى تثق في النموذج مع قاعدة الكود هذه.',
          '**الخطأ 4: استخدام Aider مع نموذج يعيد صياغة الكود.** إن عجز النموذج عن إعادة إنتاج كتل SEARCH حرفياً، يفشل كل تعديل. استخدم نموذجاً مضبوطاً دقيقاً للكود من 22B فما فوق (Qwen3-Coder وDeepSeek Coder وCodestral) لتعديلات فارق موثوقة.',
          '**الخطأ 5: تغيير الأداة لحل مشكلة نموذج.** إن فشلت استدعاءات الأدوات في Cline، على الأرجح ستفشل في وضع الوكيل في Continue.dev للسبب ذاته. شخّص النموذج أولاً.',
          '**الخطأ 6: تشغيل Cline مع نموذج سياق 32K في مهام متعددة الملفات.** استهلاك التوكن يستنزف الميزانية في منتصف المهمة. استخدم نموذج سياق 128K للعمل غير البديهي مع Cline.',
        ],
      },
      sources: {
        id: 'sources',
        title: 'المصادر',
        items: [
          '[توثيق Continue.dev](https://docs.continue.dev/) — دليل الإعداد الرسمي، وإعداد النماذج، وموفرو السياق `@` ومرجع وضع الوكيل.',
          '[مستودع Cline على GitHub](https://github.com/cline/cline) — الكود المصدري ومخططات الأدوات وسلوك وضع التخطيط/التنفيذ وبنية الإضافة.',
          '[توثيق Aider](https://aider.chat/) — مرجع واجهة سطر الأوامر الرسمي، وتوثيق صيغة التحرير، وآليات خريطة المستودع، والقيم الافتراضية حسب النموذج.',
          '[مكتبة نماذج Ollama](https://ollama.com/library) — النماذج المحلية المتاحة ومستويات الكمية المرجعية لكل أداة.',
          '[بطاقة نموذج Qwen3-Coder](https://huggingface.co/Qwen/Qwen3-Coder-30B) — البنية وأطوال السياق المدعومة وبيانات تدريب استدعاءات الأدوات للنموذج البرمجي الموصى به.',
        ],
      },
      faq: {
        id: 'faq',
        ...
        faqs: [
          {
            q: 'ما أفضل خيار لمستخدمي VS Code؟',
            a: 'تعمل الأدوات الثلاث في VS Code، لكن Continue.dev وCline هما الخياران الطبيعيان. Continue.dev للإكمال التلقائي أولاً والدردشة المدمجة في بيئة التطوير؛ Cline لمهام الوكيل المستقل. تعمل أداة Aider بشكل مثالي من الطرفية المدمجة في VS Code لكنها لا تتكامل مع سطح المحرر ذاته — واجهتها هي الطرفية.',
          },
          {
            q: 'هل أداة Cline متوافقة مع بيئة تطوير Cursor؟',
            a: 'Cursor هو fork من VS Code ويقبل إضافات VS Code. تُثبَّت Cline وتعمل في Cursor؛ سطح الإضافة متطابق. فريق Cline لا يستهدف Cursor رسمياً، لذا الحالات الحدية (واجهات برمجية خاصة بـCursor، وتغييرات Cursor المستقبلية) غير مضمونة، لكن عملياً المستخدمون لا يُبلغون عن مشكلات.',
          },
          {
            q: 'هل يمكن لأداة Aider العمل بدون Git؟',
            a: 'نعم، لكنك ستفقد بدائية سير العمل التي تميزها. تعمل Aider في مجلدات بدون Git مع `--no-git`؛ تُطبَّق التعديلات لكن لا commits، لذا يُعطَّل `/undo` ويقع التراجع على تاريخ الفوارق للنموذج داخل الجلسة. للعمل غير البديهي، ابدأ مستودع Git — حتى مستودعاً مُتجاهَلاً — قبل البدء.',
          },
          {
            q: 'أيّها يمتلك أفضل إعادة هيكلة متعددة الملفات؟',
            a: 'Cline للعمل المستقل متعدد الملفات — يخطط بين الملفات، ويقرأها عند الحاجة، ويحررها بموافقة خطوة بخطوة. Aider للعمل القابل للمراجعة متعدد الملفات حيث تسمي الملفات صراحةً وتريد أن يُودَع كل تعديل. تتطلب Continue.dev قيادة يدوية ملفاً بملف؛ وهي الأضعف بين الثلاثة في إعادة الهيكلة متعددة الملفات.',
          },
          {
            q: 'هل هي متوافقة مع Ollama المحلي مباشرةً؟',
            a: 'نعم. لدى Continue.dev موفر Ollama أصيل مع كتلة إعداد. تُدرج Cline Ollama في قائمة الموفرين المنسدلة. تدعم Aider Ollama عبر LiteLLM مع `--model ollama_chat/<model-name>`. Ollama هو الخلفية المحلية الأقل احتكاكاً للأدوات الثلاث.',
          },
          {
            q: 'أيّها يعمل أفضل مع Qwen3-Coder؟',
            a: 'Qwen3-Coder 30B موثوق في الأدوات الثلاث في مايو 2026 — يتعامل مع صيغة أداة XML لـCline وكتل البحث والاستبدال لـAider وسير عمل الإكمال التلقائي والدردشة في Continue.dev دون تدهور ملحوظ. تُجري المتغيرات الأصغر من Qwen3-Coder (فئة 7B) استدعاءات أدوات بموثوقية أقل وتعمل أفضل في وضع الإكمال التلقائي فقط في Continue.dev مقارنةً بحلقة الوكيل في Cline.',
          },
          {
            q: 'هل يمكنني استخدام هذه الأدوات مع الوضع المحلي لـCursor؟',
            a: 'الميزات المدمجة في Cursor (الإكمال التلقائي بـTab وCmd+K) تستخدم توجيه نماذج Cursor الخاص، لا هذه الأدوات. لاستخدام نموذج لغوي محلي في Cursor، ثبّت Continue.dev أو Cline كإضافات VS Code داخل Cursor — تجلبان إعداد موفر خاص بهما مستقلاً عن إعداد Cursor. تعمل Aider في الطرفية المدمجة في Cursor كما في VS Code.',
          },
          {
            q: 'أيّها الأكثر استقراراً للجلسات الطويلة؟',
            a: 'Aider، لعدم وجود حالة في العملية قابلة للتلف — كل تعديل commit، وفشل الجلسة يُضيع فقط المحادثة لا العمل. Continue.dev مستقرة عبر الجلسات الطويلة في بيئة التطوير. قد تعاني جلسات Cline الطويلة من ضغط نافذة السياق مع النماذج الأصغر؛ إعادة تشغيل المهام بين عمليات إعادة الهيكلة الكبيرة يتجنب المشكلة.',
          },
          {
            q: 'هل هي متوافقة مع طلبات النظام المخصصة؟',
            a: 'الأدوات الثلاث. Continue.dev: `systemMessage` لكل نموذج في `config.json`. Cline: حقل "Custom Instructions" في إعداد الإضافة. Aider: علم `--system-prompt` أو القيم الافتراضية حسب النموذج في `.aider.conf.yml`. طلبات النظام المخصصة مفيدة بشكل خاص لـCline (توجيه سلوك وضع التخطيط) ولـAider (تطبيق أسلوب كود متسق بين الـcommits).',
          },
          {
            q: 'ما الذي يجب أن تأخذه فرق DevOps أو المنصة بعين الاعتبار بشأن الخصوصية عند استخدام وكلاء البرمجة المحلية؟',
            a: 'إن كان الكود أو السجلات التي تعالجها النماذج اللغوية المحلية قد تحتوي على بيانات مستخدمين من المنطقة العربية أو دول الخليج، تسري اللوائح الإقليمية للخصوصية. النماذج اللغوية المحلية ذاتها (Ollama وllama.cpp) لا تُخزّن بيانات شخصية، لكن مجلدات الكود والسجلات التي تفهرسها قد تحتوي على معلومات حساسة. تأكد من نشر النموذج في بنية تحتية للشركة بدون وصول لواجهات برمجية خارجية. في دول الخليج والمنطقة العربية، استشر فريق الامتثال لديك قبل النشر في الإنتاج في القطاعات المنظمة.',
          },
          {
            q: 'ما أفضل خيار للشركات الصغيرة والمتوسطة الناطقة بالعربية؟',
            a: 'تُقدم Aider أفضل قيمة مقابل التكلفة للشركات الصغيرة والمتوسطة: لا اعتماد على تراخيص بيئة تطوير، وقائمة تماماً على الطرفية (تتوافق مع بيئات التطوير الموجودة)، وحد أدنى من متطلبات الأجهزة (تعمل مع نموذج سياق 32K)، ومسار التدقيق الكامل في سجل Git (يبسّط الامتثال ومراجعة الكود). Continue.dev الخيار الثاني للفرق المستثمرة بقوة في VS Code/JetBrains. Cline مكلف في استهلاك التوكن ويتطلب أجهزة أعلى، لكنه يناسب سير عمل Agile لإعادة الهيكلة المكثفة. توفّر الشركات الصغيرة والمتوسطة التكاليف بالنشر المحلي مع نماذج محلية — يجب أن يتبع اختيار الأداة البنية التحتية الموجودة وسير عمل المطورين، لا الضجيج الإعلامي.',
          },
        ],
      },
      relatedReading: {
        ...
        title: 'قراءة ذات صلة',
        items: [
          '[استبدال GitHub Copilot بنموذج لغوي محلي](/ar/power-local-llm/replace-github-copilot-with-local-llm) — سياق أوسع للاستبدال، بما في ذلك حسابات التكلفة والخصوصية.',
          '[أفضل نماذج البرمجة المحلية في 2026](/ar/power-local-llm/best-local-coding-models-2026) — الجانب النموذجي في المعادلة: Qwen3-Coder مقابل DeepSeek مقابل Codestral في المعايير القياسية الحقيقية.',
          '[وكلاء الذكاء الاصطناعي المحلي مع MCP في 2026](/ar/power-local-llm/local-ai-agents-with-mcp-2026) — للمستخدمين الذين يريدون استدعاءات أدوات موسّعة تتجاوز الأدوات السابقة.',
          '[أفضل النماذج المحلية لاستدعاءات الأدوات في 2026](/ar/power-local-llm/best-local-models-tool-calling-2026) — عمق تقني مجاور حول موثوقية استدعاءات الأدوات.',
          '[llama.cpp مقابل Ollama مقابل vLLM](/ar/local-llms/llamacpp-vs-ollama-vs-vllm) — طبقة محرك الاستدلال تحت الأدوات الثلاثة.',
          '[مركز Power Local LLM](/power-local-llm) — المكتبة الكاملة من الأدلة.',
        ],
      },
    },
      schema: {
      '@context': 'https://schema.org',
      '@type': 'TechArticle',
      headline: 'Continue.dev مقابل Cline مقابل Aider: أفضل وكيل برمجة محلي في 2026',
      description: 'مقارنة بين Continue.dev وCline وAider كوكلاء برمجة محلية في 2026. نموذج التحرير، والتكامل مع بيئة التطوير، والتراجع، واستراتيجية السياق — اختيار حسب سير العمل لا الضجيج الإعلامي.',
      url: 'https://www.promptquorum.com/ar/power-local-llm/continue-dev-vs-cline-vs-aider-local',
      inLanguage: 'ar',
      datePublished: '2026-05-24',
      dateModified: '2026-08-27',
      author: { '@type': 'Person', name: 'Hans Kuepper', sameAs: 'https://www.linkedin.com/in/hanskuepper/' },
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          '**도구 호출 신뢰성**은 도구가 아닌 모델에 의존합니다. Qwen3-Coder 30B와 DeepSeek-V3는 2026년 5월 기준 가장 신뢰할 수 있는 도구 호출자입니다. 소형 모델(7B 클래스)은 어느 도구로 감싸도 정기적으로 잘못된 형식의 도구 호출을 발생시킵니다.',
      ...
      quickFacts: {
        ...
        items: [
          ...
          '**도구 호출 신뢰성은 도구의 속성이 아닌 모델의 속성입니다.** Qwen3-Coder 30B와 DeepSeek-V3는 세 도구 모두에서 신뢰할 수 있으며, 7B 클래스 모델은 세 도구 모두에서 실패합니다.',
      ...
      continueDevDeep: {
        ...
        items: [
          ...
          '**로컬 모델 설정:** 네이티브 Ollama 지원은 하나의 설정 블록입니다. llama.cpp, LM Studio, 모든 OpenAI 호환 엔드포인트도 작동합니다. 표준 패턴은 자동완성(소형, 빠름 — Qwen3-Coder 1.5B 클래스)과 채팅(대형 — Qwen3-Coder 30B, DeepSeek-V3)에 별도 모델을 실행하는 것입니다.',
      ...
      clineDeep: {
        ...
        items: [
          ...
          '**로컬 모델 설정:** 모든 OpenAI 호환 엔드포인트. Ollama는 원클릭입니다. llama.cpp 또는 vLLM의 경우, Cline을 OpenAI 호환 서버 URL로 지정합니다. 에이전트 루프는 토큰을 많이 사용합니다 — 최소 32K 컨텍스트 윈도우의 모델을 사용하십시오(32K에서 Qwen3-Coder 30B, 128K에서 DeepSeek-V3).',
      ...
      editModel: {
        ...
        imageCaption: '도구별 편집 모델 신뢰성: 7B 모델에서 Continue.dev가 가장 관대하고, Aider는 일치하지 않는 블록을 거부하며, Cline은 잘못된 형식의 XML로 중단됩니다. 30B+ 코드 모델(Qwen3-Coder, DeepSeek-V3)에서는 세 가지 모두 신뢰할 수 있습니다.',
        items: [
          ...
          '**30B+ 도구 호출 모델(Qwen3-Coder 30B, DeepSeek-V3)의 신뢰성 순위:** 세 가지가 수렴합니다 — 모델에 정밀한 diff 블록과 올바른 형식의 도구 호출을 일관되게 발생시킬 능력이 있습니다.',
      ...
      toolCalls: {
        ...
        items: [
          '**이러한 도구들에 대해 신뢰할 수 있는 도구 호출을 수행하는 모델(2026년 5월):** Qwen3-Coder 30B(Cline의 XML 형식에서 최고), DeepSeek-V3(넓은 컨텍스트, 다단계 계획에서 강력), Llama 3.3 70B(느리지만 신뢰할 수 있음), Codestral 22B(빠르지만 계획 지평이 짧음).',
