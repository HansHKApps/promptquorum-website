...
export const article: Partial<Record<Language, LLMArticle>> = {
    ...
    quickAnswerTop: {
      en: {
        ...
        bullets: [
          ...
          'Tool-call reliability is a model property: Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B, and Llama 3.3 70B handle MCP cleanly. Models under 7B regularly emit malformed tool calls.',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          '**Tool-call reliability scales with model size and training.** Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B, and Llama 3.3 70B handle MCP cleanly at Q4_K_M. Models under 7B regularly emit malformed tool calls and stall the loop.',
      ...
      quickFacts: {
        ...
        items: [
          ...
          '**Compatible Ollama models:** any model with native tool-call training. In August 2026: Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B, Llama 3.3 70B.',
          ...
          '**Configuration lives in one file:** `~/.config/goose/config.yaml` (Goose), the MCP block of `~/.continue/config.json` ([Continue.dev](https://Continue.dev)), or `mcpServers` in Cline\'s settings UI. Same shape across all of them: server name, command, args, env vars.',
      ...
      setup: {
        ...
        items: [
          ...
          '**Step 2 — pull a tool-calling model.** Pick from Gemma 4 31B (`gemma4:31b`), GLM-4.7 Flash (`glm-4.7-flash`), Qwen3 32B (`qwen3:32b`), or Llama 3.3 70B (`llama3.3:70b`). 32 GB unified memory or 24 GB VRAM handles 26B–32B at Q4_K_M comfortably.',
        ...
        imageCaption:
          'Six-step Ollama and Goose setup flow: install Ollama, pull a tool-calling model like gemma4:31b, install Goose via pipx, configure the provider, add the filesystem MCP server, and verify with a real task in about 15 minutes.',
      },
      filesystemServer: {
      ...
      pickingModel: {
        ...
        items: [
          '**Gemma 4 31B (`gemma4:31b`)** — Google\'s tool-call training is best-in-class for the size. Fits in 16 GB unified memory or 24 GB VRAM at Q4_K_M. Good general reasoning; somewhat conservative on chained tool calls.',
          '**GLM-4.7 Flash (`glm-4.7-flash`)** — Zhipu\'s model has very strong tool-call reliability and a 128K context window out of the box. Slightly heavier than Gemma 4; fits comfortably on a 24 GB GPU.',
    ...
    quickAnswerTop: {
      de: {
        ...
        bullets: [
          ...
          'Tool-Call-Zuverlässigkeit ist eine Eigenschaft des Modells: Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B und Llama 3.3 70B verarbeiten MCP sauber. Modelle unter 7B geben regelmäßig fehlerhafte Tool-Calls aus.',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          '**Tool-Call-Zuverlässigkeit skaliert mit Modellgröße und Training.** Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B und Llama 3.3 70B verarbeiten MCP sauber bei Q4_K_M. Modelle unter 7B geben regelmäßig fehlerhafte Tool-Calls aus und blockieren den Loop.',
      ...
      quickFacts: {
        ...
        items: [
          ...
          '**Kompatible Ollama-Modelle:** jedes Modell mit nativem Tool-Call-Training. Im August 2026: Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B, Llama 3.3 70B.',
          ...
          '**Konfiguration in einer Datei:** `~/.config/goose/config.yaml` (Goose), der MCP-Block von `~/.continue/config.json` ([Continue.dev](https://Continue.dev)) oder `mcpServers` in der Settings-UI von Cline. Gleiche Form überall: Servername, Command, Args, Env-Vars.',
      ...
      setup: {
        ...
        items: [
          ...
          '**Schritt 2 — ein Tool-Calling-Modell ziehen.** Wählen Sie aus Gemma 4 31B (`gemma4:31b`), GLM-4.7 Flash (`glm-4.7-flash`), Qwen3 32B (`qwen3:32b`) oder Llama 3.3 70B (`llama3.3:70b`). 16 GB Unified Memory oder 12 GB VRAM bewältigen 27B–32B bei Q4_K_M komfortabel.',
        ...
        imageCaption:
          'Sechsstufiger Einrichtungsablauf für Ollama und Goose: Ollama installieren, ein toolfähiges Modell wie gemma4:31b laden, Goose per pipx installieren, den Provider konfigurieren, den Dateisystem-MCP-Server hinzufügen und mit einer echten Aufgabe in rund 15 Minuten prüfen.',
      },
      filesystemServer: {
        id: 'filesystem-server',
      ...
      pickingModel: {
        ...
        items: [
          '**Gemma 4 31B (`gemma4:31b`)** — Googles Tool-Call-Training ist Best-in-Class für die Größe. Passt in 16 GB Unified Memory oder 24 GB VRAM bei Q4_K_M. Gutes allgemeines Reasoning; etwas konservativ bei verketteten Tool-Calls.',
          '**GLM-4.7 Flash (`glm-4.7-flash`)** — Zhipus Modell hat sehr starke Tool-Call-Zuverlässigkeit und ein 128K-Context-Fenster ab Werk. Etwas schwerer als Gemma 4; passt komfortabel auf eine 24-GB-GPU.',
    ...
    quickAnswerTop: {
      fr: {
        ...
        bullets: [
          ...
          "La fiabilité des appels d'outils dépend du modèle : Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B et Llama 3.3 70B traitent MCP proprement. Les modèles de moins de 7B émettent régulièrement des appels malformés.",
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          "**La fiabilité des appels d'outils croît avec la taille et l'entraînement.** Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B et Llama 3.3 70B traitent MCP proprement en Q4_K_M. Les modèles de moins de 7B émettent régulièrement des appels malformés et bloquent la boucle.",
      ...
      quickFacts: {
        ...
        items: [
          ...
          "**Modèles Ollama compatibles :** tout modèle entraîné nativement aux appels d'outils. En août 2026 : Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B, Llama 3.3 70B.",
          ...
          "**Configuration dans un seul fichier :** `~/.config/goose/config.yaml` (Goose), le bloc MCP de `~/.continue/config.json` ([Continue.dev](https://Continue.dev)) ou `mcpServers` dans l'UI de Cline. Même forme partout : nom du serveur, command, args, variables d'environnement.",
      ...
      setup: {
        ...
        items: [
          ...
          "**Étape 2 — récupérer un modèle compatible tool calling.** Choisissez parmi Gemma 4 31B (`gemma4:31b`), GLM-4.7 Flash (`glm-4.7-flash`), Qwen3 32B (`qwen3:32b`) ou Llama 3.3 70B (`llama3.3:70b`). 16 Go de mémoire unifiée ou 12 Go de VRAM gèrent confortablement 27B–32B en Q4_K_M.",
        ...
        imageCaption:
          'Flux d\'installation en six étapes pour Ollama et Goose : installer Ollama, télécharger un modèle compatible outils comme gemma4:31b, installer Goose via pipx, configurer le fournisseur, ajouter le serveur MCP fichiers et vérifier avec une tâche réelle en environ 15 minutes.',
      },
      filesystemServer: {
        id: 'filesystem-server',
      ...
      pickingModel: {
        ...
        items: [
          "**Gemma 4 31B (`gemma4:31b`)** — l'entraînement tool calling de Google est best-in-class pour la taille. Tient en 16 Go de mémoire unifiée ou 24 Go de VRAM en Q4_K_M. Bon raisonnement général ; un peu conservateur sur les chaînes d'appels d'outils.",
          "**GLM-4.7 Flash (`glm-4.7-flash`)** — le modèle de Zhipu a une fiabilité tool calling très forte et une fenêtre de contexte 128K d'origine. Légèrement plus lourd que Gemma 4 ; tient confortablement sur un GPU 24 Go.",
    ...
    quickAnswerTop: {
      ja: {
        ...
        bullets: [
          ...
          'tool call の信頼性はモデル特性：Gemma 4 31B、GLM-4.7 Flash、Qwen3 32B、Qwen3-Coder 30B、Llama 3.3 70B はクリーンに動作します。7B 未満のモデルは tool call の形式不正を頻発します。',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          '**tool call の信頼性はモデルサイズと学習に比例します。** Gemma 4 31B、GLM-4.7 Flash、Qwen3 32B、Qwen3-Coder 30B、Llama 3.3 70B は Q4_K_M で MCP をクリーンに処理します。7B 未満のモデルは形式不正な tool call を頻繁に出してループを止めます。',
      ...
      quickFacts: {
        ...
        items: [
          ...
          '**互換 Ollama モデル：** ネイティブ tool call 学習を持つすべてのモデル。2026 年 5 月時点：Gemma 4 31B、GLM-4.7 Flash、Qwen3 32B、Qwen3-Coder 30B、Llama 3.3 70B。',
      ...
      setup: {
        ...
        items: [
          ...
          '**Step 2 — tool calling モデルを pull。** Gemma 4 31B（`gemma4:31b`）、GLM-4.7 Flash（`glm-4.7-flash`）、Qwen3 32B（`qwen3:32b`）または Llama 3.3 70B（`llama3.3:70b`）から選択。16 GB unified memory または 12 GB VRAM で 27B–32B を Q4_K_M で快適に動かせます。',
        ...
        imageCaption:
          'OllamaとGooseの6ステップセットアップフロー：Ollamaをインストールし、gemma4:31bなどツール呼び出し対応モデルを取得し、pipxでGooseをインストールし、プロバイダーを設定し、ファイルシステムMCPサーバーを追加し、約15分で実タスクにより確認する。',
      },
      filesystemServer: {
        id: 'filesystem-server',
      ...
      pickingModel: {
        ...
        items: [
          '**Gemma 4 31B（`gemma4:31b`）** — Google の tool call 学習はサイズに対して best-in-class。16 GB unified memory または 24 GB VRAM で Q4_K_M に収まります。一般推論は良好；連鎖した tool call にはやや保守的です。',
          '**GLM-4.7 Flash（`glm-4.7-flash`）** — Zhipu のモデルは tool call 信頼性が非常に高く、out of the box で 128K コンテキストウィンドウ。Gemma 4 よりやや重め；24 GB GPU に余裕で収まります。',
      ...
      faq: {
        ...
        faqs: [
          ...
          },
          {
            ...
            a: '2026 年 5 月時点で最も信頼できる選択肢は、Gemma 4 31B、GLM-4.7 Flash、Qwen3 32B（コード形態の作業なら Qwen3-Coder 30B）、Llama 3.3 70B です。4 つすべてが明示的な tool call 学習を持ち、MCP クライアントがルーティング可能なクリーンな function calling JSON を出力します。7B 未満（および tool call ファインチューニングのない汎用モデルの大半）は形式不正な tool call を頻繁に出します。',
          },
          {
    ...
    quickAnswerTop: {
      zh: {
        ...
        bullets: [
          ...
          'tool call 可靠性是模型属性：Gemma 4 31B、GLM-4.7 Flash、Qwen3 32B、Qwen3-Coder 30B 与 Llama 3.3 70B 均可干净处理 MCP；7B 以下模型经常输出格式错误的 tool call。',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          '**tool call 可靠性随模型规模与训练扩展。** Gemma 4 31B、GLM-4.7 Flash、Qwen3 32B、Qwen3-Coder 30B、Llama 3.3 70B 在 Q4_K_M 下干净处理 MCP；7B 以下模型经常输出格式错误的 tool call 并卡住循环。',
      ...
      quickFacts: {
        ...
        items: [
          ...
          '**兼容的 Ollama 模型：** 任何具备原生 tool call 训练的模型。2026 年 5 月：Gemma 4 31B、GLM-4.7 Flash、Qwen3 32B、Qwen3-Coder 30B、Llama 3.3 70B。',
      ...
      setup: {
        ...
        items: [
          ...
          '**Step 2 —— 拉取支持 tool calling 的模型。** 在 Gemma 4 31B (`gemma4:31b`)、GLM-4.7 Flash (`glm-4.7-flash`)、Qwen3 32B (`qwen3:32b`) 或 Llama 3.3 70B (`llama3.3:70b`) 中选一个。16 GB 统一内存或 12 GB VRAM 在 Q4_K_M 下从容运行 27B–32B。',
        ...
        imageCaption:
          'Ollama 与 Goose 的六步配置流程：安装 Ollama，拉取如 gemma4:31b 的支持工具调用的模型，通过 pipx 安装 Goose，配置提供方，添加文件系统 MCP 服务器，并在约 15 分钟内用真实任务验证。',
      },
      filesystemServer: {
        id: 'filesystem-server',
      ...
      pickingModel: {
        ...
        items: [
          '**Gemma 4 31B (`gemma4:31b`)** —— Google 的 tool call 训练在同尺寸中处于业内最佳。Q4_K_M 下可装入 16 GB 统一内存或 24 GB VRAM。通用推理良好；在链式 tool call 上略偏保守。',
          '**GLM-4.7 Flash (`glm-4.7-flash`)** —— 智谱的模型 tool call 可靠性极强，开箱 128K 上下文窗口。比 Gemma 4 略重；24 GB GPU 上从容运行。',
      ...
      faq: {
        ...
        faqs: [
          ...
          {
            q: '哪些本地模型对 MCP 支持最好？',
            a: '2026 年 5 月最可靠的选择是 Gemma 4 31B、GLM-4.7 Flash、Qwen3 32B（代码形态工作选 Qwen3-Coder 30B）和 Llama 3.3 70B。这四个都具备显式的 tool call 训练，输出干净的 function calling JSON 供 MCP 客户端路由。7B 以下模型（以及多数没有 tool call 微调的通用模型）经常产生格式错误的 tool call。',
          },
          {
  ...
  es: {
    ...
    next_refresh_due: '2027-02-27',
    theme: 'Local AI Agents & Tool Use',
    heroImage: '/images/local-ai-agents-with-mcp-2026-overview-hero-es.webp',
    ...
    quickAnswerTop: {
      es: {
        ...
        bullets: [
          ...
          'La fiabilidad del tool calling es una propiedad del modelo: Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B y Llama 3.3 70B manejan MCP sin problemas. Los modelos de menos de 7B suelen emitir tool calls malformados.',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          '**La fiabilidad del tool calling escala con el tamaño y el entrenamiento del modelo.** Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B y Llama 3.3 70B manejan MCP sin problemas en Q4_K_M. Los modelos de menos de 7B suelen emitir tool calls malformados y bloquear el bucle.',
      ...
      quickFacts: {
        ...
        items: [
          ...
          '**Modelos Ollama compatibles:** cualquier modelo con entrenamiento nativo de tool calling. En agosto de 2026: Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B, Llama 3.3 70B.',
          ...
          '**La configuración vive en un único archivo:** `~/.config/goose/config.yaml` (Goose), el bloque MCP de `~/.continue/config.json` ([Continue.dev](https://Continue.dev)) o `mcpServers` en la interfaz de configuración de Cline. La misma estructura en todos: nombre del servidor, comando, args, variables de entorno.',
      ...
      setup: {
        ...
        items: [
          ...
          '**Paso 2 — descarga un modelo con tool calling.** Elige entre Gemma 4 31B (`gemma4:31b`), GLM-4.7 Flash (`glm-4.7-flash`), Qwen3 32B (`qwen3:32b`) o Llama 3.3 70B (`llama3.3:70b`). 16 GB de memoria unificada o 12 GB de VRAM manejan cómodamente 27B–32B en Q4_K_M.',
        ...
        imageCaption:
          'Flujo de configuración en seis pasos para Ollama y Goose: instalar Ollama, descargar un modelo con llamadas a herramientas como gemma4:31b, instalar Goose con pipx, configurar el proveedor, añadir el servidor MCP de archivos y verificar con una tarea real en unos 15 minutos.',
      },
      filesystemServer: {
        id: 'filesystem-server',
      ...
      pickingModel: {
        ...
        items: [
          '**Gemma 4 31B (`gemma4:31b`)** — el entrenamiento de tool calling de Google es el mejor en su clase para este tamaño. Cabe en 16 GB de memoria unificada o 24 GB de VRAM en Q4_K_M. Buen razonamiento general; algo conservador en tool calls encadenados.',
          '**GLM-4.7 Flash (`glm-4.7-flash`)** — el modelo de Zhipu tiene una fiabilidad de tool calling muy alta y una ventana de contexto de 128K de serie. Ligeramente más pesado que Gemma 4; cabe cómodamente en una GPU de 24 GB.',
      ...
      url: 'https://www.promptquorum.com/es/power-local-llm/local-ai-agents-with-mcp-2026',
      inLanguage: 'es',
  ...
  pt: {
    ...
    next_refresh_due: '2027-02-27',
    theme: 'Local AI Agents & Tool Use',
    heroImage: '/images/local-ai-agents-with-mcp-2026-overview-hero-pt.webp',
    ...
    metaDescription:
      'Configure agentes de IA locais em 2026 com Ollama e MCP: filesystem, SQLite/Postgres, navegador, servidor GitHub, JSON funcional e um modelo de sandboxing.',
    twitterDescription:
    ...
    audience:
      'Desenvolvedores e usuários com conhecimentos técnicos que executam LLMs locais via Ollama e querem um modelo que possa fazer coisas reais — consultar bancos de dados, editar arquivos, automatizar navegadores — sem enviar nada a um provedor na nuvem.',
    readTime: '15 min de leitura',
    educationalLevel: 'Advanced',
    primaryTerm: 'agente MCP local',
    targetKeywords: [
      'mcp ollama português',
      'configurar servidor mcp local',
      'model context protocol local',
      'ollama tool calling 2026',
      'agente ia local mcp',
    ...
    quickAnswerTop: {
      pt: {
        ...
        bullets: [
          ...
          'A confiabilidade do tool calling é uma propriedade do modelo: Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B e Llama 3.3 70B lidam com MCP sem problemas. Modelos abaixo de 7B geralmente emitem tool calls malformados.',
    ...
    toc: [
      ...
      { label: 'O que o MCP desbloqueia', anchor: '#what-mcp-unlocks' },
      { label: 'Comparativo de servidores MCP', anchor: '#server-comparison' },
      { label: 'Arquitetura: como as peças se encaixam', anchor: '#architecture' },
      { label: 'Configuração: Ollama + Goose em 15 minutos', anchor: '#setup' },
      { label: 'Servidor Filesystem', anchor: '#filesystem-server' },
      { label: 'Servidores SQLite e Postgres', anchor: '#database-server' },
      { label: 'Servidor de navegador (Puppeteer / Playwright)', anchor: '#browser-server' },
      { label: 'Servidor GitHub', anchor: '#github-server' },
      { label: 'Modelo de segurança', anchor: '#security-model' },
      { label: 'MCP local vs Claude Desktop', anchor: '#vs-claude-desktop' },
      { label: 'Escolher um modelo com tool calling', anchor: '#picking-model' },
      { label: 'MCP vs Function Calling simples', anchor: '#mcp-vs-function-calling' },
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          '**A confiabilidade do tool calling escala com o tamanho e o treinamento do modelo.** Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B e Llama 3.3 70B lidam com MCP sem problemas em Q4_K_M. Modelos abaixo de 7B geralmente emitem tool calls malformados e bloqueiam o loop.',
      ...
      quickFacts: {
        ...
        items: [
          ...
          '**Modelos Ollama compatíveis:** qualquer modelo com treinamento nativo de tool calling. Em agosto de 2026: Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B, Llama 3.3 70B.',
          ...
          '**A configuração vive em um único arquivo:** `~/.config/goose/config.yaml` (Goose), o bloco MCP de `~/.continue/config.json` ([Continue.dev](https://Continue.dev)) ou `mcpServers` na interface de configuração do Cline. A mesma estrutura em todos: nome do servidor, comando, args, variáveis de ambiente.',
      ...
      whatMcpUnlocks: {
        id: 'what-mcp-unlocks',
        title: 'O que o MCP realmente desbloqueia para um modelo local',
        content:
      ...
      serverComparison: {
        id: 'server-comparison',
        title: 'Comparativo dos quatro servidores MCP mais usados',
        content:
        ...
        snippetBlocks: [
          {
            type: 'one-sentence',
            text: 'Comece com o servidor filesystem (5 minutos, baixo risco), adicione um servidor SQLite para trabalho com dados, adicione um servidor de navegador apenas quando precisar e traga o GitHub uma vez que você confie no modelo na sua máquina.',
          },
          {
            type: 'plain-terms',
            text: 'Quatro servidores cobrem 90% do que você vai querer que um agente local faça. O servidor filesystem lê e escreve arquivos em uma pasta que você escolhe. O servidor SQLite ou Postgres executa consultas contra um banco de dados. O servidor de navegador controla uma janela real do Chromium para que o modelo possa ler páginas que requerem JavaScript. O servidor GitHub abre issues e PRs nos seus repositórios.',
          },
        ],
        columns: ['Servidor MCP', 'O que habilita', 'Dificuldade de configuração', 'Nível de risco', 'Ideal para'],
        rows: [
          { 'Servidor MCP': 'Filesystem', 'O que habilita': 'Ler e escrever arquivos dentro de um diretório isolado', 'Dificuldade de configuração': 'Fácil (um caminho na allowlist)', 'Nível de risco': 'Médio — limite o escopo com cuidado', 'Ideal para': 'Automação pessoal, anotações, resumo de repositórios' },
          { 'Servidor MCP': 'SQLite', 'O que habilita': 'Consultar um arquivo de banco de dados SQLite local', 'Dificuldade de configuração': 'Fácil (caminho para o arquivo .db)', 'Nível de risco': 'Baixo em somente leitura; médio com escritas', 'Ideal para': 'Exploração de dados, análise de logs, prototipagem' },
          { 'Servidor MCP': 'Postgres', 'O que habilita': 'Consultar um banco de dados Postgres via string de conexão', 'Dificuldade de configuração': 'Médio (role + URL)', 'Nível de risco': 'Médio — use um role de somente leitura', 'Ideal para': 'Exploração de dados de produção, relatórios, protótipos BI' },
          { 'Servidor MCP': 'Puppeteer / Playwright', 'O que habilita': 'Controlar um Chromium sem cabeça ou visível para navegação, scraping, preenchimento de formulários', 'Dificuldade de configuração': 'Difícil (binários do navegador, seletores, latência)', 'Nível de risco': 'Alto — pode enviar formulários, clicar em qualquer coisa', 'Ideal para': 'Pesquisa, scraping, testes de regressão' },
          { 'Servidor MCP': 'GitHub', 'O que habilita': 'Listar repositórios, ler arquivos, abrir issues e PRs', 'Dificuldade de configuração': 'Fácil (PAT em variável de ambiente)', 'Nível de risco': 'Médio — limite o token a repositórios específicos', 'Ideal para': 'Fluxos de trabalho de desenvolvimento, triagem, rascunhos de PR' },
          { 'Servidor MCP': 'Custom', 'O que habilita': 'Qualquer coisa que você possa expressar como ferramentas JSON-RPC', 'Dificuldade de configuração': 'Difícil (escreva seu próprio servidor)', 'Nível de risco': 'Variável', 'Ideal para': 'APIs internas, sistemas de nicho, código de integração' },
        ],
      },
      architecture: {
        id: 'architecture',
      ...
      setup: {
        id: 'setup',
        title: 'Configuração: Ollama + Goose em 15 minutos',
        content:
        ...
        items: [
          ...
          '**Passo 2 — baixe um modelo com tool calling.** Escolha entre Gemma 4 31B (`gemma4:31b`), GLM-4.7 Flash (`glm-4.7-flash`), Qwen3 32B (`qwen3:32b`) ou Llama 3.3 70B (`llama3.3:70b`). 16 GB de memória unificada ou 12 GB de VRAM lidam confortavelmente com 27B–32B em Q4_K_M.',
        ...
        imageCaption:
          'Fluxo de configuração em seis passos para Ollama e Goose: instalar o Ollama, baixar um modelo com suporte a ferramentas como o gemma4:31b, instalar o Goose via pipx, configurar o provedor, adicionar o servidor MCP de arquivos e verificar com uma tarefa real em cerca de 15 minutos.',
      },
      filesystemServer: {
        id: 'filesystem-server',
        title: 'Servidor Filesystem: ler e escrever em um diretório isolado',
        content:
        ...
        callouts: [
          {
            type: 'warning',
            text: 'Nunca coloque `/` nem seu diretório home na allowlist. Crie uma pasta `agent-workspace` dedicada, coloque cópias dos arquivos que quer que o agente toque e deixe-o operar apenas dentro dessa pasta. Se o agente falhar, o raio de explosão para em um diretório.',
          },
        ],
      },
      databaseServer: {
        id: 'database-server',
        title: 'Servidores SQLite e Postgres: consultar dados reais',
        content:
      ...
      securityModel: {
        id: 'security-model',
        title: 'Um modelo de segurança que não confia no modelo',
        content:
        ...
        callouts: [
          {
            type: 'note',
            text: 'Um hábito útil de recuperação pós-falha: antes de uma tarefa de agente não trivial, faça `git stash` (ou `git checkout -b agent/<task>`). Após a tarefa, revise o diff, mantenha as partes que quer e descarte o resto.',
          },
        ],
      },
      vsClaudeDesktop: {
        id: 'vs-claude-desktop',
        title: 'MCP local vs Claude Desktop: o que muda, o que permanece',
        content:
        ...
        rows: [
          ...
          { 'Camada': 'Custo por solicitação', 'Claude Desktop': 'Gasto de API por token', 'Ollama local + Goose': '$0 — inferência local' },
          { 'Camada': 'Privacidade', 'Claude Desktop': 'A conversa vai para a Anthropic', 'Ollama local + Goose': 'Fica na máquina' },
          { 'Camada': 'Limites de velocidade', 'Claude Desktop': 'Aplicam-se os limites da API', 'Ollama local + Goose': 'Limitado apenas pelo desempenho do hardware' },
          { 'Camada': 'Qualidade do tool calling', 'Claude Desktop': 'A melhor da categoria', 'Ollama local + Goose': 'Boa com modelos de 27B+; degrada rapidamente abaixo de 7B' },
          { 'Camada': 'Requer internet', 'Claude Desktop': 'Sim', 'Ollama local + Goose': 'Apenas se um servidor fizer requisições (ex.: o navegador)' },
          { 'Camada': 'Tempo de configuração', 'Claude Desktop': '5 minutos', 'Ollama local + Goose': '15 minutos (uma única vez)' },
        ],
      },
      pickingModel: {
        id: 'picking-model',
        title: 'Escolher um modelo com tool calling para MCP local',
        content:
        ...
        items: [
          '**Gemma 4 31B (`gemma4:31b`)** — o treinamento de tool calling do Google é o melhor da categoria para esse tamanho. Cabe em 16 GB de memória unificada ou 24 GB de VRAM em Q4_K_M.',
          '**GLM-4.7 Flash (`glm-4.7-flash`)** — o modelo da Zhipu tem confiabilidade de tool calling muito alta e uma janela de contexto de 128K nativa.',
      ...
      mcpVsFunctionCalling: {
        id: 'mcp-vs-function-calling',
        title: 'MCP vs Function Calling simples: qual é a diferença',
        content:
      ...
      commonMistakes: {
        id: 'common-mistakes',
        title: 'Erros comuns ao configurar MCP local',
        items: [
      ...
      sources: {
        ...
        title: 'Fontes',
        items: [
          '[Especificação do Model Context Protocol](https://modelcontextprotocol.io/) — Especificação oficial, esquema JSON-RPC, definições de transporte e ciclo de vida.',
          '[Repositório GitHub modelcontextprotocol/servers](https://github.com/modelcontextprotocol/servers) — Servidores de referência (filesystem, sqlite, postgres, github, puppeteer, etc.) e sua documentação de configuração.',
          '[Documentação do projeto Goose](https://block.github.io/goose/) — Instalação do CLI, configuração do provedor Ollama, sintaxe de configuração de servidores MCP.',
          '[Biblioteca de modelos Ollama](https://ollama.com/library) — Modelos locais disponíveis, indicadores de suporte de tool calling e níveis de quantização referenciados neste guia.',
          '[Repositório GitHub do Cline](https://github.com/cline/cline) — Implementação do cliente MCP para VS Code, painel de servidores MCP.',
          '[Documentação do Continue.dev](https://docs.continue.dev/) — Referência do bloco de configuração `mcpServers` para o cliente Continue.dev.',
        ],
      },
      faq: {
        id: 'faq',
        ...
        faqs: [
          {
            q: 'O que é MCP e por que importa para a IA local?',
            a: 'O Model Context Protocol (MCP) é um protocolo aberto JSON-RPC 2.0 que permite que um cliente (Goose, Cline, Continue.dev, LM Studio, Claude Desktop) conecte um modelo de linguagem a servidores de ferramentas de forma uniforme. Importa para a IA local porque padroniza a camada que converte um modelo de chat em um agente — escreva um servidor de ferramentas uma vez e use-o sob qualquer cliente e qualquer modelo, incluindo um modelo Ollama local.',
          },
          {
            q: 'O MCP funciona sem Claude Desktop?',
            a: 'Sim. O protocolo é aberto e completamente independente do Claude Desktop. Em 2026, Goose, Cline, Continue.dev e LM Studio oferecem implementações de cliente MCP que funcionam com modelos Ollama locais. Os servidores de referência (filesystem, sqlite, postgres, puppeteer, github) funcionam sem mudanças sob qualquer cliente compatível.',
          },
          {
            q: 'Quais modelos locais suportam MCP melhor?',
            a: 'Em agosto de 2026, as opções mais confiáveis são Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B (ou Qwen3-Coder 30B para trabalho orientado a código) e Llama 3.3 70B. Os quatro têm treinamento explícito de tool calling e emitem JSON de function calling limpo que os clientes MCP podem rotear. Modelos abaixo de 7B (e a maioria dos modelos gerais sem fine-tuning de tool calling) produzem regularmente tool calls malformados.',
          },
          {
            q: 'O MCP é seguro? O agente pode excluir meus arquivos?',
            a: 'Pode se você permitir. A segurança vem de como você configura os servidores, não do protocolo. O servidor filesystem só opera dentro dos caminhos que você coloca na allowlist — limite-o a um diretório `agent-workspace` dedicado. O servidor de banco de dados funciona em somente leitura quando você usa um role apenas de SELECT. Sempre exija aprovação explícita para as ferramentas de escrita, shell e PR; aprove automaticamente apenas as operações de leitura.',
          },
          {
            q: 'Posso escrever meu próprio servidor MCP?',
            a: 'Sim — e os SDKs tornam isso simples. Os SDKs oficiais de TypeScript e Python (`@modelcontextprotocol/sdk` e `mcp`) lidam com o encanamento JSON-RPC. Você define ferramentas com seus JSON Schemas e uma função handler, e o SDK as expõe via stdio. Um servidor de propósito único (uma ou duas ferramentas que envolvem uma API interna) é um arquivo de 50–100 linhas.',
          },
          {
            q: 'O MCP funciona no Windows?',
            a: 'Sim. Ollama, Goose, Cline, Continue.dev e LM Studio todos funcionam no Windows. Os servidores MCP rodam como subprocessos de Node.js ou Python; ambos os ambientes de execução são totalmente compatíveis com Windows. O único detalhe específico da plataforma é o tratamento de caminhos — use barras para frente na configuração ou escape corretamente as barras invertidas.',
          },
          {
            q: 'Como faço sandbox das tool calls do MCP?',
            a: 'Três camadas cobrem a maior parte do risco. Primeiro, limite cada servidor estreitamente no nível de configuração: filesystem a um diretório, banco de dados a um role de somente leitura, GitHub a um PAT de granularidade fina para repositórios de teste. Segundo, use as regras de aprovação por ferramenta do cliente: aprovação automática para leituras, aprovação exigida para escritas. Terceiro, mantenha o agente dentro de um workspace compatível com `git stash` para que qualquer ação destrutiva seja desfazível via git.',
          },
          {
            q: 'Os agentes MCP podem fazer requisições HTTP?',
            a: 'Sim, através de servidores específicos. O servidor de navegador (puppeteer ou playwright) controla um Chromium real que faz as requisições que o modelo navega. Vários servidores de terceiros expõem ferramentas `http_get`/`http_post` de forma mais direta. Os servidores de filesystem e banco de dados não fazem requisições de rede; operam apenas em recursos locais.',
          },
          {
            q: 'O MCP funciona com o Ollama nativamente ou precisa de um wrapper?',
            a: 'O Ollama em si não fala MCP — serve uma API de chat compatível com OpenAI. Você precisa de um cliente (Goose, Cline, Continue.dev, LM Studio) para fazer a ponte entre a API de chat do Ollama e os servidores MCP. O cliente roteia os tool calls do modelo para o servidor MCP correto e retorna os resultados à conversa.',
          },
          {
            q: 'Qual é a diferença entre MCP e function calling?',
            a: 'O function calling é o LLM emitindo JSON estruturado com o nome de uma ferramenta e seus argumentos — é uma capacidade do modelo. O MCP é o protocolo que permite que os servidores de ferramentas e os clientes descrevam, descubram, invoquem e retornem essas ferramentas entre processos — é uma camada de interoperabilidade. Cooperam: o cliente converte as definições de ferramentas do MCP para o formato de function calling do modelo, o modelo emite um function call, o cliente mapeia a chamada de volta para um servidor MCP e o servidor a executa.',
          },
        ],
      },
      relatedReading: {
        ...
        items: [
          ...
          '[Substitua o Zapier com agentes de IA locais](/pt/power-local-llm/replace-zapier-with-local-ai-agents) — framework de automação de fluxos de trabalho para o mesmo stack MCP.',
  ...
  ar: {
    ...
    next_refresh_due: '2027-02-27',
    theme: 'Local AI Agents & Tool Use',
    heroImage: '/images/local-ai-agents-with-mcp-2026-overview-hero-ar.webp',
    ...
    metaDescription:
      'MCP يُتيح لـOllama استعلام Postgres وقراءة الملفات والتحكم في المتصفح محلياً. إعداد filesystem وSQLite وGitHub مع sandboxing للأمان 2026.',
    twitterDescription:
    ...
    quickAnswerTop: {
      ar: {
        ...
        bullets: [
          ...
          'موثوقية استدعاء الأدوات خاصية تعتمد على النموذج: Gemma 4 31B وGLM-4.7 Flash وQwen3 32B وQwen3-Coder 30B وLlama 3.3 70B يتعاملون مع MCP بشكل نظيف. النماذج التي تقل عن 7B تُصدر tool calls مشوّهة بانتظام.',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          '**موثوقية استدعاء الأدوات تعتمد على النموذج.** Gemma 4 31B وGLM-4.7 Flash وQwen3 32B وLlama 3.3 70B يعملون بشكل موثوق في 2026. النماذج الأقل من 7B تُصدر JSON مشوّهاً في استدعاءات الأدوات بانتظام بصرف النظر عن العميل.',
      ...
      quickFacts: {
        ...
        title: 'حقائق سريعة',
        items: [
          '**ما تحتاجه:** Ollama (مثبَّت) + عميل MCP (Goose أو Cline أو Continue.dev أو LM Studio) + ملف `mcp.json` + نموذج استدعاء أدوات (Gemma 4 31B أو أعلى مُوصى به).',
      ...
      setup: {
        ...
        numberedItems: [
          'ثبّت Ollama: `curl -fsSL https://ollama.ai/install.sh | sh` ثم `ollama pull gemma4:31b` (أو `qwen3:32b` للبدء بنموذج أصغر).',
        ...
        imageCaption:
          'تدفق إعداد من ست خطوات لـ Ollama وGoose: تثبيت Ollama، تنزيل نموذج يدعم استدعاء الأدوات مثل gemma4:31b، تثبيت Goose عبر pipx، تهيئة المزوّد، إضافة خادم الملفات MCP، والتحقق بمهمة حقيقية خلال نحو 15 دقيقة.',
      },
      filesystemServer: {
        id: 'filesystem-server',
      ...
      pickingModel: {
        ...
        items: [
          '**Gemma 4 31B (`gemma4:31b`):** توصيتي الافتراضية في 2026. موثوق في استدعاء الأدوات المتسلسلة. 16 جيجابايت ذاكرة موحدة أو 24 جيجابايت VRAM بـQ4_K_M.',
          '**GLM-4.7 Flash (`glm-4.7-flash`):** سياق 128K أصلي. أفضل لأعمال الوثائق الطويلة والاستعلامات الكبيرة.',
  ...
  ko: {
    ...
    next_refresh_due: '2027-02-27',
    theme: 'Local AI Agents & Tool Use',
    heroImage: '/images/local-ai-agents-with-mcp-2026-overview-hero-ko.webp',
    title: 'Ollama를 데이터베이스 및 API에 MCP로 연결하기: 로컬 AI 에이전트 설정 2026',
    seoTitle: 'Ollama + MCP 로컬 2026: AI를 데이터베이스 및 API에 연결하기',
    intro:
      'Model Context Protocol(MCP)은 로컬 Ollama 모델과 나머지 시스템 사이에 필요했던 계층입니다. 단 하나의 설정 파일과 tool calling을 지원하는 모델만 있으면, 동일한 에이전트가 Postgres 데이터베이스를 쿼리하고, 격리된 디렉터리에서 파일을 읽고 쓰고, headless 브라우저를 제어하고, GitHub에서 pull request를 열 수 있습니다 — 모두 노트북에서 실행되고, 모두 오프라인으로 작동합니다. 이 가이드는 모델을 신뢰하지 않는다고 가정하는 보안 모델과 함께 전체 과정을 안내합니다.',
    metaDescription:
      '2026년 Ollama와 MCP로 로컬 AI 에이전트 설정하기: filesystem, SQLite/Postgres, 브라우저, GitHub 서버, 실제 작동하는 JSON 설정, 샌드박싱 모델.',
    twitterDescription:
    ...
    audience:
      'Ollama를 통해 로컬 LLM을 실행하며 클라우드 제공업체에 아무것도 전송하지 않고 데이터베이스 쿼리, 파일 편집, 브라우저 자동화 등 실질적인 작업을 수행할 수 있는 모델을 원하는 기술적 지식을 가진 개발자 및 사용자.',
    readTime: '15분 분량',
    educationalLevel: 'Advanced',
    primaryTerm: '로컬 MCP 에이전트',
    targetKeywords: [
      '로컬 llm 에이전트 설정',
      'mcp ollama 한국어',
      'model context protocol 로컬',
      'ollama tool calling 2026',
      '로컬 ai 에이전트 mcp',
    ...
    quickAnswerTop: {
      ko: {
        ...
        bullets: [
          ...
          'tool calling 신뢰성은 모델의 속성입니다: Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B, Llama 3.3 70B는 MCP를 문제없이 처리합니다. 7B 미만 모델은 잘못된 형식의 tool call을 자주 생성합니다.',
    ...
    toc: [
      ...
      { label: 'MCP가 가능하게 하는 것', anchor: '#what-mcp-unlocks' },
      { label: 'MCP 서버 비교', anchor: '#server-comparison' },
      { label: '아키텍처: 구성 요소가 맞춰지는 방식', anchor: '#architecture' },
      { label: '설정: 15분 안에 Ollama + Goose', anchor: '#setup' },
      { label: 'Filesystem 서버', anchor: '#filesystem-server' },
      { label: 'SQLite 및 Postgres 서버', anchor: '#database-server' },
      { label: '브라우저 서버 (Puppeteer / Playwright)', anchor: '#browser-server' },
      { label: 'GitHub 서버', anchor: '#github-server' },
      { label: '보안 모델', anchor: '#security-model' },
      { label: '로컬 MCP vs Claude Desktop', anchor: '#vs-claude-desktop' },
      { label: 'tool calling 모델 선택', anchor: '#picking-model' },
      { label: 'MCP vs 일반 Function Calling', anchor: '#mcp-vs-function-calling' },
    ...
    gammaEmbedUrl: '/presentations/local-ai-agents-with-mcp-2026-static.html',
    gammaDescription:
      '이 프레젠테이션은 다음을 다룹니다: MCP가 로컬 Ollama 모델을 데이터베이스를 쿼리하고, 파일을 읽고, 브라우저를 제어하고, pull request를 여는 에이전트로 변환하는 방법 — 모두 오프라인으로; 4개의 참조 서버(filesystem, SQLite/Postgres, 브라우저, GitHub)의 설정 난이도 및 위험 수준; Goose 설정 6단계 가이드; LLM을 신뢰하지 않는 보안 모델; Claude Desktop과의 직접 비교. 로컬 MCP 에이전트 참조 카드로 PDF를 다운로드하십시오.',
    sections: {
      tldr: {
        id: 'key-takeaways',
        isTldr: true,
        items: [
          ...
          '**tool calling 신뢰성은 모델 크기 및 훈련과 함께 확장됩니다.** Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B, Llama 3.3 70B는 Q4_K_M에서 MCP를 문제없이 처리합니다. 7B 미만 모델은 잘못된 형식의 tool call을 자주 생성하고 루프를 차단합니다.',
      ...
      quickFacts: {
        ...
        items: [
          ...
          '**지원되는 Ollama 모델:** 네이티브 tool calling 훈련을 받은 모든 모델. 2026년 8월 기준: Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B, Qwen3-Coder 30B, Llama 3.3 70B.',
          '**기본 전송:** 로컬 프로세스를 위한 stdio; 서버를 기기 또는 에이전트 간에 공유해야 할 때만 HTTP/SSE.',
          '**설정은 단일 파일에 있습니다:** `~/.config/goose/config.yaml`(Goose), `~/.continue/config.json`의 MCP 블록([Continue.dev](https://Continue.dev)) 또는 Cline 설정 인터페이스의 `mcpServers`. 모두 동일한 구조: 서버 이름, 명령, args, 환경 변수.',
          '**Claude Desktop이 필요하지 않습니다.** 프로토콜은 Claude Desktop 독점 사용 이전부터 존재했습니다; 모든 참조 서버는 MIT/Apache 라이선스이며 모든 호환 클라이언트에서 작동합니다.',
        ],
      },
      whatMcpUnlocks: {
        id: 'what-mcp-unlocks',
        title: 'MCP가 로컬 모델에 실제로 가능하게 하는 것',
        content:
          '**도구 없는 로컬 LLM은 텍스트로만 응답할 수 있습니다. MCP를 사용하면 동일한 모델이 기기에서 행동할 수 있습니다.** 이 변화는 챗봇과 에이전트의 차이입니다.',
        items: [
          '**"이 저장소에서 모든 TODO를 찾아 파일별로 그룹화하고 `notes/todos.md`에 Markdown 요약을 작성하십시오."** — `filesystem` 서버가 읽고, 모델이 그룹화하고, 동일한 서버가 씁니다. 처음부터 끝까지 단 한 번의 왕복.',
          '**"이번 분기 수익 기준 상위 10개 고객을 보여주고 차트를 만드십시오."** — `postgres` 서버가 SQL을 실행하고(읽기 전용 역할), 모델이 요약하고 차트 도구를 위해 `filesystem`을 통해 CSV를 씁니다.',
          '**"Hacker News 첫 페이지를 열어 상위 3개 AI 기사를 찾아 요약하고 내 읽기 목록에 추가하십시오."** — `puppeteer` 서버가 headless 브라우저를 제어하고, 모델이 추출 및 요약하고, `filesystem`이 추가합니다.',
          '**"내 포크에 대해 `chore: bump deps` 제목으로 draft PR을 열고 실패한 CI 실행을 링크하십시오."** — `github` 서버가 PR을 생성하고, 실행을 가져오고, 설명에 링크를 씁니다.',
          '**"`events.db`의 최근 100개 행을 보고 새 오류 급증에 책임이 있는 사용자 ID를 알려주십시오."** — `sqlite` 서버가 쿼리하고; 모델이 추론하고; 채팅 패널에서 답을 읽습니다.',
          '이 각각은 이전에 호스팅된 도구가 있는 클라우드 모델 또는 손으로 작성한 스크립트가 필요했던 문장에서 행동으로의 워크플로입니다. MCP는 클라이언트 간에 동일한 서버를 재사용하고 서버 간에 동일한 모델을 재사용할 수 있게 하는 계층입니다.',
        ],
      },
      serverComparison: {
        id: 'server-comparison',
        title: '가장 많이 사용되는 4개의 MCP 서버 비교',
        content:
          '아래의 참조 서버들은 "내 로컬 모델이 실제 작업을 수행하길 원한다"는 긴 여정을 커버합니다. 모두 오픈 소스이며 MCP 클라이언트가 시작하는 로컬 하위 프로세스로 실행됩니다.',
        snippetBlocks: [
          {
            type: 'one-sentence',
            text: 'filesystem 서버(5분, 낮은 위험)로 시작하고, 데이터 작업을 위해 SQLite 서버를 추가하고, 필요할 때만 브라우저 서버를 추가하고, 기기에서 모델을 신뢰할 수 있을 때 GitHub을 도입하십시오.',
          },
          {
            type: 'plain-terms',
            text: '4개의 서버가 로컬 에이전트에게 원하는 작업의 90%를 커버합니다. filesystem 서버는 당신이 선택한 폴더에서 파일을 읽고 씁니다. SQLite 또는 Postgres 서버는 데이터베이스에 대한 쿼리를 실행합니다. 브라우저 서버는 모델이 JavaScript가 필요한 페이지를 읽을 수 있도록 실제 Chromium 창을 제어합니다. GitHub 서버는 저장소에서 이슈와 PR을 엽니다. 모두 하나의 명령으로 설치되고, 모두 자체 기기에서 하위 프로세스로 실행되며, 명시적으로 필요하지 않은 한 인터넷을 호출하지 않습니다(브라우저는 호출하지만 나머지는 하지 않습니다).',
          },
        ],
        columns: ['MCP 서버', '활성화하는 기능', '설정 난이도', '위험 수준', '이상적인 용도'],
        rows: [
          { 'MCP 서버': 'Filesystem', '활성화하는 기능': '격리된 디렉터리 내에서 파일 읽기 및 쓰기', '설정 난이도': '쉬움(허용 목록에 경로 하나)', '위험 수준': '중간 — 범위를 신중하게 제한하십시오', '이상적인 용도': '개인 자동화, 노트 작성, 저장소 요약' },
          { 'MCP 서버': 'SQLite', '활성화하는 기능': '로컬 SQLite 데이터베이스 파일 쿼리', '설정 난이도': '쉬움(.db 파일 경로)', '위험 수준': '읽기 전용에서 낮음; 쓰기 시 중간', '이상적인 용도': '데이터 탐색, 로그 분석, 프로토타이핑' },
          { 'MCP 서버': 'Postgres', '활성화하는 기능': '연결 문자열을 통해 Postgres 데이터베이스 쿼리', '설정 난이도': '중간(역할 + URL)', '위험 수준': '중간 — 읽기 전용 역할을 사용하십시오', '이상적인 용도': '프로덕션 데이터 탐색, 보고, BI 프로토타입' },
          { 'MCP 서버': 'Puppeteer / Playwright', '활성화하는 기능': '탐색, 스크래핑, 양식 작성을 위한 headless 또는 보이는 Chromium 제어', '설정 난이도': '어려움(브라우저 바이너리, 선택기, 지연)', '위험 수준': '높음 — 양식을 제출하고 무엇이든 클릭할 수 있습니다', '이상적인 용도': '연구, 스크래핑, 회귀 테스트' },
          { 'MCP 서버': 'GitHub', '활성화하는 기능': '저장소 나열, 파일 읽기, 이슈 및 PR 열기', '설정 난이도': '쉬움(환경 변수의 PAT)', '위험 수준': '중간 — 토큰을 특정 저장소로 제한하십시오', '이상적인 용도': '개발 워크플로, 트리아지, PR 초안' },
          { 'MCP 서버': 'Custom', '활성화하는 기능': 'JSON-RPC 도구로 표현할 수 있는 모든 것', '설정 난이도': '어려움(자체 서버 작성)', '위험 수준': '가변적', '이상적인 용도': '내부 API, 틈새 시스템, 통합 코드' },
        ],
      },
      architecture: {
        id: 'architecture',
        title: '구성 요소가 맞춰지는 방식',
        content:
        ...
        items: [
          '**Ollama**는 `127.0.0.1:11434`에서 백그라운드 서비스로 실행되며 OpenAI 호환 API를 통해 모델을 제공합니다. MCP가 무엇인지 알지 못합니다 — 단순히 채팅 완성에 응답하고 모델이 요청할 때 tool call을 생성합니다.',
          '**MCP 클라이언트**(Goose, Cline, Continue.dev, LM Studio)가 다리 역할을 합니다. 모델을 위해 Ollama와 통신하고 도구를 위해 MCP 서버와 통신합니다. 모델이 tool call을 생성하면 클라이언트가 올바른 서버로 라우팅하고, 결과를 받아 대화에 반환합니다.',
          '**MCP 서버**는 독립적인 하위 프로세스로, 각 기능마다 하나씩 있습니다. stdio를 통해 JSON-RPC 2.0을 말합니다. 각 서버는 Tools, Resources, Prompts 목록을 공개하며, 클라이언트는 이들을 모델에 제시되는 도구 표면으로 결합합니다.',
          ...
          '**모델은 평평한 도구 목록을 봅니다.** 모델의 관점에서 서버가 없습니다 — `filesystem.read_file`, `sqlite.query`, `puppeteer.navigate`와 같은 도구 이름의 목록만 있습니다. 클라이언트가 라우팅을 관리합니다.',
        ],
        callouts: [
          {
            type: 'note',
      ...
      setup: {
        ...
        items: [
          ...
          '**2단계 — tool calling 모델 다운로드.** Gemma 4 31B(`gemma4:31b`), GLM-4.7 Flash(`glm-4.7-flash`), Qwen3 32B(`qwen3:32b`) 또는 Llama 3.3 70B(`llama3.3:70b`) 중에서 선택하십시오. 16GB 통합 메모리 또는 12GB VRAM으로 Q4_K_M에서 27B~32B를 편안하게 처리할 수 있습니다.',
        ...
        imageCaption:
          'Ollama와 Goose의 6단계 설정 흐름: Ollama 설치, gemma4:31b 같은 도구 호출 모델 다운로드, pipx로 Goose 설치, 제공자 구성, 파일 시스템 MCP 서버 추가, 약 15분 내 실제 작업으로 검증.',
      },
      filesystemServer: {
        id: 'filesystem-server',
        title: 'Filesystem 서버: 격리된 디렉터리에서 읽기 및 쓰기',
        content:
          '**filesystem 서버는 처음 설치하고 안전하게 제한하기 가장 쉽습니다.** `read_file`, `write_file`, `list_directory`, `move_file`, `search_files`, `create_directory`를 노출합니다 — 모두 하나 이상의 허용 목록 경로로 제한됩니다.',
        items: [
          '**설치:** 참조 서버는 `@modelcontextprotocol/server-filesystem`으로, `npx -y`를 통해 실행됩니다(전역 설치 불필요). Goose, Cline, Continue.dev가 설정 블록에서 자동으로 시작합니다.',
          '**허용 목록 경로:** 서버는 하나 이상의 디렉터리 인수를 받아들이고 그 밖의 작업을 거부합니다. 항상 명시적이고 좁은 경로를 전달하십시오 — `~`나 `/`는 절대 사용하지 마십시오.',
          '**노출된 도구:** `read_file`, `read_multiple_files`, `write_file`, `edit_file`(행 대체), `list_directory`, `search_files`, `move_file`, `create_directory`, `directory_tree`. 모델은 이것들을 `filesystem.read_file` 등으로 봅니다.',
          '**사용 편의성:** `directory_tree`는 JSON 트리를 반환합니다; 특정 파일을 읽기 전에 모델이 방향을 잡을 때 이상적입니다. `search_files`는 grep과 유사한 재귀 검색을 수행합니다.',
        ...
        callouts: [
          {
            type: 'warning',
            text: '허용 목록에 `/`나 홈 디렉터리를 절대 넣지 마십시오. 전용 `agent-workspace` 폴더를 만들고, 에이전트가 작업할 파일의 복사본을 그 안에 넣고, 에이전트가 그 폴더 내에서만 작동하도록 하십시오. 에이전트가 실패하더라도 피해 범위는 하나의 디렉터리에 그칩니다.',
          },
        ],
      },
      databaseServer: {
        id: 'database-server',
        title: 'SQLite 및 Postgres 서버: 실제 데이터 쿼리',
        content:
          '**데이터베이스 서버는 모델을 읽기 전용으로 유지하는 한 실제 데이터 기반 질문에 답할 수 있는 주니어 분석가로 만들어줍니다.** 두 참조 서버 모두 `query` 도구와 선택적인 `write_query` 도구를 포함합니다.',
        items: [
          '**SQLite 서버(`@modelcontextprotocol/server-sqlite`)**는 `.db` 파일 경로를 받습니다. 데이터베이스를 실행할 필요 없이 로그 분석, 스키마 프로토타이핑, 내보내기 탐색에 유용합니다.',
          '**Postgres 서버(`@modelcontextprotocol/server-postgres`)**는 연결 문자열을 받습니다. 권장 패턴은 에이전트를 위한 전용 읽기 전용 역할을 만들고 해당 역할의 연결 문자열을 사용하는 것입니다.',
          '**노출된 도구:** `query`(읽기 전용으로 구성되면 SELECT만), `list_tables`, `describe_table`. Postgres 서버는 `list_schemas`를 추가합니다. 일부 포크는 `write_query`를 추가합니다 — 이 데이터베이스에서 모델을 신뢰하지 않는 한 비활성화된 상태로 유지하십시오.',
          '**스키마 인식:** 분석적 질문을 하기 전에 에이전트에게 "테이블을 나열하고 가장 많이 사용되는 5개를 설명하십시오"를 요청하십시오 — 모델이 열 이름을 추측하는 것보다 `describe_table`을 호출한 후에 훨씬 더 정확합니다.',
      ...
      browserServer: {
        id: 'browser-server',
        title: '브라우저 서버: Puppeteer 또는 Playwright로 Chromium 제어',
        content:
          '**브라우저 서버는 4개 중 가장 강력하고 가장 위험합니다.** 실제 Chromium을 실행하고 탐색, 클릭, 양식 작성, 스크린샷을 노출합니다 — 즉, 브라우저에서 할 수 있는 모든 것을 할 수 있으며 양식 제출도 포함됩니다.',
        items: [
          '**참조 서버:** `@modelcontextprotocol/server-puppeteer`(더 가볍고 기본적으로 headless)와 `@modelcontextprotocol/server-playwright`(더 무겁고 여러 브라우저 지원). 로컬 에이전트의 경우 Puppeteer로 충분합니다.',
          '**노출된 도구:** `navigate`, `screenshot`, `click`, `fill`, `select`, `evaluate`(JavaScript 실행), `get_page_content`. 모델은 `get_page_content`를 구조화된 텍스트를 읽기 위해, `screenshot`을 시각적으로 확인하기 위해 사용합니다.',
          '**지연:** 실제 브라우저 세션은 작업당 1~5초가 걸립니다. 다단계 탐색은 페이지 내용이 크기 때문에 쉽게 30~60초와 수만 토큰을 소비할 수 있습니다. 32K+ 컨텍스트 창을 사용하십시오.',
        ...
        callouts: [
          {
            type: 'warning',
            text: '브라우저 서버에 자격 증명을 절대 제공하지 마십시오. 인증된 세션이 필요하다면 에이전트에게 사전 인증된 브라우저 프로파일(`userDataDir`을 통해)을 전달하고 고영향 사이트(뱅킹, 이메일, 클라우드 콘솔, 결제 양식)를 탐색하지 못하게 하십시오. 모델은 버튼이 무엇을 하는지 판단하지 않습니다 — 텍스트를 보고 클릭합니다. 맥락도 없고 선택권도 없는 인턴처럼 취급하십시오.',
          },
        ],
      },
      githubServer: {
        id: 'github-server',
        title: 'GitHub 서버: 로컬 모델에서 저장소, 이슈, PR',
        content:
          '**GitHub 서버는 저장소에 대한 자연어 작업을 API 호출로 변환합니다.** 4개 중 설정이 가장 간단하며 personal access token(PAT) 권한을 통해 제한하기 가장 쉽습니다.',
        items: [
          '**설치:** `@modelcontextprotocol/server-github`, 환경 변수 `GITHUB_PERSONAL_ACCESS_TOKEN`의 PAT와 함께 실행. 토큰이 유일한 인증입니다 — 서버 자체에는 별도 설정이 없습니다.',
          '**노출된 도구:** `search_repositories`, `get_file_contents`, `create_or_update_file`, `create_pull_request`, `list_issues`, `create_issue`, `add_issue_comment`, `merge_pull_request`, 그 외 수십 가지 추가 도구. 전체 표면이 크지만 대부분의 작업은 5~10개의 도구를 사용합니다.',
          '**PAT 범위 제한.** 최소 필요 권한(탐색을 위한 Read, PR/이슈 생성을 위한 Write)으로 특정 저장소에 범위가 지정된 세분화된 PAT를 사용하십시오. 실험적 에이전트에 `repo`가 있는 클래식 PAT를 사용하지 마십시오.',
          '**실제 워크플로:** 트리아지("최근 20개의 열린 이슈를 읽어 그룹화하고 레이블을 제안하십시오"), 초안("README를 읽고 오타를 수정하는 PR을 여십시오"), 보고서("이번 주 비활성 PR이 무엇입니까").',
      ...
      securityModel: {
        id: 'security-model',
        title: '모델을 신뢰하지 않는 보안 모델',
        content:
          '**올바른 멘탈 모델은 "LLM은 당신이 주는 열쇠를 가진 신뢰하지 않는 인턴"입니다.** 기능은 허용 목록에 포함하는 서버와 표면에서 옵니다 — 모델의 판단에서 오는 것이 아닙니다.',
        items: [
          '**filesystem 서버를 하나의 디렉터리로 제한하십시오.** `~`나 `/`는 절대 안 됩니다. `agent-workspace/` 폴더를 선택하고 에이전트가 작업할 파일의 복사본을 그 안에 넣으십시오. 에이전트가 실패하더라도 최악의 경우는 하나의 폴더입니다.',
          '**데이터베이스 서버를 기본적으로 읽기 전용으로 실행하십시오.** `SELECT` 권한만 있고 30초 statement timeout이 있는 전용 `agent_ro` 역할이 전체 사고 클래스를 제거합니다.',
          '**모든 쓰기 또는 셸 도구를 명시적 승인 뒤에 두십시오.** Goose, Cline, Continue.dev는 도구별 승인 규칙을 지원합니다. 기본적으로 읽기 도구를 허용하고; `write_file`, `edit_file`, `execute_command`, `create_pull_request`, 양식을 제출하는 모든 브라우저 작업에 승인을 요구하십시오.',
          '**감사 로그를 사용하십시오.** 모든 MCP 클라이언트는 tool call과 결과를 기록합니다. 긴 세션 후에 로그를 검토하십시오: 모델이 예상치 못한 것을 시도하는 것을 발견할 것입니다(때로는 무해하지만, 때로는 권한 조정을 정당화합니다).',
        ...
        callouts: [
          {
            type: 'note',
            text: '유용한 장애 복구 습관: 중요하지 않은 에이전트 작업 전에 `git stash`(또는 `git checkout -b agent/<task>`)를 수행하십시오. 작업 후 diff를 검토하고, 원하는 부분을 유지하고, 나머지는 버리십시오. 이것은 긴 Cline 또는 Aider 세션을 안전하게 만드는 것과 동일한 관행입니다 — 더 넓은 패턴은 [Continue.dev vs Cline vs Aider 비교](/ko/power-local-llm/continue-dev-vs-cline-vs-aider-local)를 참조하십시오.',
          },
        ],
      },
      vsClaudeDesktop: {
        id: 'vs-claude-desktop',
        title: '로컬 MCP vs Claude Desktop: 무엇이 바뀌고 무엇이 남는가',
        content:
        ...
        rows: [
          ...
          { '레이어': '요청당 비용', 'Claude Desktop': '토큰당 API 지출', 'Ollama 로컬 + Goose': '$0 — 로컬 추론' },
          { '레이어': '프라이버시', 'Claude Desktop': '대화가 Anthropic으로 전송됨', 'Ollama 로컬 + Goose': '기기에서만 유지됨' },
          { '레이어': '속도 제한', 'Claude Desktop': 'API 제한 적용', 'Ollama 로컬 + Goose': '하드웨어 성능에 의해서만 제한됨' },
          { '레이어': 'tool calling 품질', 'Claude Desktop': '최고 수준', 'Ollama 로컬 + Goose': '27B+ 모델에서 좋음; 7B 미만에서 급격히 저하됨' },
          { '레이어': '인터넷 필요', 'Claude Desktop': '예', 'Ollama 로컬 + Goose': '서버가 요청을 할 경우에만(예: 브라우저)' },
          { '레이어': '설정 시간', 'Claude Desktop': '5분', 'Ollama 로컬 + Goose': '15분(일회성)' },
        ],
      },
      pickingModel: {
        id: 'picking-model',
        title: '로컬 MCP를 위한 tool calling 모델 선택',
        content:
          '**tool calling 신뢰성은 harness가 아닌 모델 크기와 훈련과 함께 확장됩니다.** Cline에서 잘못된 형식의 tool call을 생성하는 모델은 같은 이유로 Goose에서도 생성합니다.',
        items: [
          '**Gemma 4 31B(`gemma4:31b`)** — Google의 tool calling 훈련은 이 크기에서 최고 수준입니다. Q4_K_M에서 16GB 통합 메모리 또는 24GB VRAM에 맞습니다. 훌륭한 일반 추론; 연쇄 tool call에서 약간 보수적.',
          '**GLM-4.7 Flash(`glm-4.7-flash`)** — Zhipu의 모델은 매우 높은 tool calling 신뢰성과 표준 128K 컨텍스트 창을 가집니다. Gemma 4보다 약간 무겁지만 24GB GPU에 편안하게 맞습니다.',
          ...
          '모든 모델에서 tool calling 품질을 향상시키는 구조적 프롬프팅 기법은 [Chain-of-thought 프롬프팅](/ko/prompt-engineering/chain-of-thought-prompting)을 참조하십시오.',
          '비교 데이터는 [2026년 tool calling을 위한 최고의 로컬 모델](/ko/power-local-llm/best-local-models-tool-calling-2026)을 참조하십시오.',
        ],
      },
      mcpVsFunctionCalling: {
        id: 'mcp-vs-function-calling',
        title: 'MCP vs 일반 Function Calling: 차이점은 무엇인가',
        content:
          '**Function calling은 모델이 생성하는 것입니다. MCP는 클라이언트와 도구가 만날 수 있게 하는 프로토콜입니다.** 두 가지는 다른 레이어에 있으며 협력합니다; 하나가 다른 것을 대체하지 않습니다.',
        items: [
          '**Function calling**은 LLM 측의 기능입니다: 모델이 도구 이름과 인수를 설명하는 구조화된 JSON 객체를 생성합니다. OpenAI의 도구, Anthropic의 도구, Ollama의 tool call API는 약간 다른 wire 형식으로 동일한 아이디어를 사용합니다.',
          '**MCP**는 그 위에 위치합니다: 도구가 프로세스 간에 설명, 발견, 호출, 반환되는 방식을 표준화합니다. function calling만 있는 모델은 filesystem에 대해 아무것도 모릅니다; MCP 서버는 filesystem 작업을 사용 가능하게 만들고, 클라이언트는 이를 모델의 function calling API에 매핑하고, 모델이 호출할 수 있습니다.',
      ...
      commonMistakes: {
        ...
        items: [
          ...
          '**실수 2: 허용 목록에 홈 디렉터리 넣기.** "테스트만을 위한" `~` 허용 목록은 일상적인 사용까지 살아남습니다. 처음부터 전용 `agent-workspace`를 만드십시오.',
          '**실수 3: 데이터베이스 서버를 읽기/쓰기 모드로 두기.** 실제 테이블에 대한 자신감 있는 에이전트의 `DELETE` 쿼리가 정확히 이것이 방지하는 사고입니다. `agent_ro`를 기본 역할로 만드십시오; 그것을 명시적으로 필요로 하는 작업에만, 그리고 그 작업 동안에만 별도의 쓰기 가능한 역할을 설정하십시오.',
          '**실수 4: 모든 도구 자동 승인.** "모두 승인" 버튼은 편리하고 위험합니다. 읽기 도구(`read_file`, `list_directory`, `query`)를 자동 승인하십시오; 쓰기/셸/PR 도구는 항상 승인이 필요합니다.',
          '**실수 5: 다단계 브라우저 작업에 32K 컨텍스트 모델 사용.** 페이지 내용이 크기 때문에 세 페이지를 탐색하는 에이전트가 추론 전에 32K 토큰을 소진할 수 있습니다. 브라우저 집약적인 작업에는 128K 컨텍스트 모델을 사용하십시오.',
          '**실수 6: 에이전트가 판단력이 있다고 가정하기.** 그렇지 않습니다. 모델은 "이것이 프로덕션 데이터베이스이다" 또는 "이 PR이 배포될 것이다"라는 개념이 없습니다. 권한이 유일한 장벽입니다.',
          '**실수 7: 처음부터 모든 참조 서버 설치.** 더 많은 도구 = 더 큰 시스템 프롬프트 = 더 느리고 덜 신뢰할 수 있는 도구 선택. `filesystem`으로 시작하십시오. 그것을 필요로 하는 워크플로가 있을 때만 다른 것을 추가하십시오.',
        ],
      },
      sources: {
        id: 'sources',
        title: '출처',
        items: [
          '[Model Context Protocol 사양](https://modelcontextprotocol.io/) — 공식 사양, JSON-RPC 스키마, 전송 및 수명 주기 정의.',
          '[GitHub modelcontextprotocol/servers 저장소](https://github.com/modelcontextprotocol/servers) — 참조 서버(filesystem, sqlite, postgres, github, puppeteer 등) 및 설정 문서.',
          '[Goose 프로젝트 문서](https://block.github.io/goose/) — CLI 설치, Ollama 제공자 설정, MCP 서버 설정 구문.',
          '[Ollama 모델 라이브러리](https://ollama.com/library) — 이 가이드에서 참조하는 사용 가능한 로컬 모델, tool calling 지원 지표 및 양자화 수준.',
          '[Cline GitHub 저장소](https://github.com/cline/cline) — VS Code용 MCP 클라이언트 구현, MCP 서버 패널.',
          '[Continue.dev 문서](https://docs.continue.dev/) — Continue.dev 클라이언트를 위한 `mcpServers` 설정 블록 참조.',
        ],
      },
      faq: {
        id: 'faq',
        ...
        faqs: [
          {
            q: 'MCP란 무엇이며 로컬 AI에 왜 중요합니까?',
            a: 'Model Context Protocol(MCP)은 클라이언트(Goose, Cline, Continue.dev, LM Studio, Claude Desktop)가 언어 모델을 도구 서버에 일관된 방식으로 연결할 수 있게 하는 오픈 JSON-RPC 2.0 프로토콜입니다. 로컬 AI에 중요한 이유는 채팅 모델을 에이전트로 변환하는 계층을 표준화하기 때문입니다 — 도구 서버를 한 번 작성하면 로컬 Ollama 모델을 포함한 모든 클라이언트와 모든 모델에서 사용하십시오. MCP 없이는 각 프로젝트가 자체 클라이언트에 대해 파일/데이터베이스/브라우저 도구를 재발명합니다.',
          },
          {
            q: 'MCP는 Claude Desktop 없이 작동합니까?',
            a: '네. 프로토콜은 오픈이며 Claude Desktop과 완전히 독립적입니다. 2026년에 Goose, Cline, Continue.dev, LM Studio는 로컬 Ollama 모델과 작동하는 MCP 클라이언트 구현을 제공합니다. 참조 서버(filesystem, sqlite, postgres, puppeteer, github)는 호환 클라이언트에서 변경 없이 작동합니다.',
          },
          {
            q: '어떤 로컬 모델이 MCP를 가장 잘 지원합니까?',
            a: '2026년 8월 기준으로 가장 신뢰할 수 있는 옵션은 Gemma 4 31B, GLM-4.7 Flash, Qwen3 32B(코드 지향 작업의 경우 Qwen3-Coder 30B), Llama 3.3 70B입니다. 4개 모두 명시적인 tool calling 훈련을 받았으며 MCP 클라이언트가 라우팅할 수 있는 깔끔한 function calling JSON을 생성합니다. 7B 미만 모델(그리고 tool calling 파인튜닝 없는 대부분의 범용 모델)은 정기적으로 잘못된 형식의 tool call을 생성합니다.',
          },
          {
            q: 'MCP는 안전합니까? 에이전트가 내 파일을 삭제할 수 있습니까?',
            a: '허용하면 가능합니다. 보안은 프로토콜이 아닌 서버를 구성하는 방식에서 옵니다. filesystem 서버는 허용 목록에 넣은 경로 내에서만 작동합니다 — 전용 `agent-workspace` 디렉터리로 제한하십시오. 데이터베이스 서버는 SELECT 전용 역할을 사용할 때 읽기 전용으로 작동합니다. 쓰기, 셸, PR 도구에 대해 항상 명시적 승인을 요구하십시오; 읽기 작업만 자동 승인하십시오. 감사 로그는 에이전트가 한 일을 사후에 정확히 보여줍니다.',
          },
          {
            q: '나만의 MCP 서버를 작성할 수 있습니까?',
            a: '네 — 그리고 SDK가 간단하게 만들어줍니다. 공식 TypeScript 및 Python SDK(`@modelcontextprotocol/sdk` 및 `mcp`)가 JSON-RPC 배관을 처리합니다. JSON 스키마와 핸들러 함수로 도구를 정의하면 SDK가 stdio를 통해 노출합니다. 단일 목적 서버(내부 API를 래핑하는 하나 또는 두 개의 도구)는 50~100줄 파일입니다.',
          },
          {
            q: 'MCP는 Windows에서 작동합니까?',
            a: '네. Ollama, Goose, Cline, Continue.dev, LM Studio 모두 Windows에서 작동합니다. MCP 서버는 Node.js 또는 Python 하위 프로세스로 실행됩니다; 두 런타임 모두 Windows와 완전히 호환됩니다. 유일한 플랫폼 특정 세부 사항은 경로 처리입니다 — 설정에서 슬래시를 사용하거나 백슬래시를 올바르게 이스케이프하십시오. 그 외에는 경험이 macOS 및 Linux와 동일합니다.',
          },
          {
            q: 'MCP tool call을 어떻게 샌드박싱합니까?',
            a: '세 가지 레이어가 대부분의 위험을 커버합니다. 첫째, 설정 수준에서 각 서버를 좁게 제한하십시오: filesystem은 하나의 디렉터리로, 데이터베이스는 읽기 전용 역할로, GitHub는 테스트 저장소를 위한 세분화된 PAT로. 둘째, 클라이언트의 도구별 승인 규칙을 사용하십시오: 읽기에 자동 승인, 쓰기에 필수 승인. 셋째, 에이전트를 `git stash` 호환 workspace 내에서 유지하여 모든 파괴적인 작업이 git을 통해 취소 가능하도록 하십시오. 민감한 작업의 경우 서버가 명시적으로 필요로 하는 엔드포인트를 제외하고 네트워크 접근 없이 실행하십시오.',
          },
          {
            q: 'MCP 에이전트가 HTTP 요청을 할 수 있습니까?',
            a: '네, 특정 서버를 통해. 브라우저 서버(puppeteer 또는 playwright)는 모델이 탐색하는 요청을 하는 실제 Chromium을 제어합니다. 여러 타사 서버는 더 직접적으로 `http_get`/`http_post` 도구를 노출합니다. filesystem 및 데이터베이스 서버는 네트워크 요청을 하지 않습니다; 로컬 리소스에서만 작동합니다.',
          },
          {
            q: 'MCP가 Ollama와 기본적으로 작동합니까, 아니면 래퍼가 필요합니까?',
            a: 'Ollama 자체는 MCP를 말하지 않습니다 — OpenAI 호환 채팅 API를 제공합니다. Ollama의 채팅 API와 MCP 서버 사이를 연결하기 위해 클라이언트(Goose, Cline, Continue.dev, LM Studio)가 필요합니다. 클라이언트가 모델의 tool call을 올바른 MCP 서버로 라우팅하고 결과를 대화에 반환합니다. 사용자 관점에서 클라이언트를 설치하고 Ollama를 가리키는 것 외에 추가 설정이 없습니다.',
          },
          {
            q: 'MCP와 function calling의 차이점은 무엇입니까?',
            a: 'Function calling은 LLM이 도구 이름과 인수를 포함한 구조화된 JSON을 생성하는 것입니다 — 이것은 모델 기능입니다. MCP는 도구 서버와 클라이언트가 프로세스 간에 도구를 설명, 발견, 호출, 반환하는 방식을 표준화하는 프로토콜입니다 — 이것은 상호 운용성 계층입니다. 협력합니다: 클라이언트가 MCP 도구 정의를 모델의 function calling 형식으로 변환하고, 모델이 function call을 생성하고, 클라이언트가 호출을 MCP 서버에 다시 매핑하고, 서버가 실행합니다. MCP 없이는 function calling을 할 수 있지만 프로젝트별로 filesystem/데이터베이스/브라우저 핸들러를 재구현하게 됩니다. MCP를 사용하면 동일한 서버가 모든 클라이언트에서 작동합니다.',
          },
        ],
      },
      relatedReading: {
        ...
        items: [
          ...
          '[로컬 AI 에이전트로 Zapier 대체하기](/ko/power-local-llm/replace-zapier-with-local-ai-agents) — 동일한 MCP 스택을 위한 워크플로 자동화 프레임워크.',
      ...
      '@context': 'https://schema.org',
      '@type': 'TechArticle',
      headline: 'Ollama를 데이터베이스 및 API에 MCP로 연결하기: 로컬 AI 에이전트 설정 2026',
      description: '2026년 Ollama와 Model Context Protocol(MCP)을 사용한 로컬 AI 에이전트 설정 가이드. Filesystem, SQLite/Postgres, 브라우저, GitHub, 실제 작동하는 JSON 설정, LLM을 신뢰하지 않는 샌드박싱 모델.',
      url: 'https://www.promptquorum.com/ko/power-local-llm/local-ai-agents-with-mcp-2026',
      inLanguage: 'ko',
      datePublished: '2026-05-24',
      dateModified: '2026-08-27',
      author: { '@type': 'Person', name: 'Hans Kuepper', sameAs: 'https://www.linkedin.com/in/hanskuepper/' },
