...
print("\\n".join(f"[{i+1}s] {d}" for i, d in enumerate(descriptions)))`,
      ...
      relatedReading: {
        ...
        items: [
          '[VLMs in Cameras and on Drones 2026](/edge-ai/vlm-video-analytics-drones-edge) -- Desktop VLM basics are covered here; for embedded camera and drone deployment, see this edge-focused guide.',
...
print("\\n".join(f"[{i+1}s] {d}" for i, d in enumerate(descriptions)))`,
      ...
      relatedReading: {
        ...
        items: [
          '[VLMs in Kameras und auf Drohnen 2026](/de/edge-ai/vlm-video-analytics-drones-edge) -- Desktop-VLM-Grundlagen stehen hier; fuer eingebettete Kameras und Drohnen siehe diesen Edge-fokussierten Guide.',
...
print("\\n".join(f"[{i+1}s] {d}" for i, d in enumerate(descriptions)))`,
      ...
      relatedReading: {
        ...
        items: [
          '[VLM dans les cameras et sur les drones 2026](/fr/edge-ai/vlm-video-analytics-drones-edge) -- Les bases du VLM de bureau sont ici ; pour un deploiement embarque camera/drone, voir ce guide dedie au edge computing.',
...
print("\\n".join(f"[{i+1}s] {d}" for i, d in enumerate(descriptions)))`,
      ...
      relatedReading: {
        ...
        title: '関連記事',
        items: [
          '[カメラとドローンにおけるVLM 2026](/ja/edge-ai/vlm-video-analytics-drones-edge) -- デスクトップVLMの基礎はこちら。組み込みカメラ・ドローン展開についてはこのエッジ向けガイドを参照。',
...
print("\\n".join(f"[{i+1}s] {d}" for i, d in enumerate(descriptions)))`,
      ...
      relatedReading: {
        ...
        title: '延伸阅读',
        items: [
          '[摄像头与无人机中的 VLM 2026](/zh/edge-ai/vlm-video-analytics-drones-edge) -- 桌面端 VLM 基础见本文；嵌入式摄像头与无人机部署请参阅这份边缘专属指南。',
...
print("\\n".join(f"[{i+1}s] {d}" for i, d in enumerate(descriptions)))`,
      ...
      relatedReading: {
        ...
        items: [
          '[VLM en Camaras y Drones 2026](/es/edge-ai/vlm-video-analytics-drones-edge) -- Los fundamentos de VLM de escritorio estan aqui; para despliegue embebido en camaras y drones, consulta esta guia centrada en el borde.',
...
print("\\n".join(f"[{i+1}s] {d}" for i, d in enumerate(descriptions)))`,
      ...
      relatedReading: {
        ...
        items: [
          '[VLMs em Cameras e Drones 2026](/pt/edge-ai/vlm-video-analytics-drones-edge) -- Os fundamentos de VLM de desktop estao aqui; para implantacao embarcada em cameras e drones, veja este guia focado em edge.',
...
print("\\n".join(f"[{i+1}s] {d}" for i, d in enumerate(descriptions)))`,
      ...
      relatedReading: {
        ...
        items: [
          '[نماذج الرؤية اللغوية في الكاميرات والطائرات المسيرة 2026](/ar/edge-ai/vlm-video-analytics-drones-edge) -- أساسيات VLM لسطح المكتب موجودة هنا؛ للنشر المدمج في الكاميرات والطائرات المسيرة، راجع هذا الدليل المخصص للحوسبة الطرفية.',
...
print("\n".join(f"[{i+1}초] {d}" for i, d in enumerate(descriptions)))`,
      ...
      relatedReading: {
        ...
        title: '관련 자료',
        items: [
          '[카메라와 드론의 VLM 2026](/ko/edge-ai/vlm-video-analytics-drones-edge) -- 데스크톱 VLM 기초는 여기에서 다룹니다. 임베디드 카메라·드론 배포는 이 엣지 전용 가이드를 참고하세요.',
