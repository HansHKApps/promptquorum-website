...
export const article: Partial<Record<Language, LLMArticle>> = {
    ...
    sections: {
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[Répertoire logiciel LLM local](/fr/directory) — le catalogue complet d\'outils d\'IA locale auquel appartient cet article.',
        ],
      },
    },
  },
  ...
  ko: {
    theme: 'Overview & Reference',
    heroImage: '/images/tensorrt-llm-explained-hero-ko.webp',
    freshness_tier: 'semi_annual',
    next_refresh_due: '2027-03-06',
    publishDate: '2026-09-06',
    dateModified: '2026-09-06',
    educationalLevel: 'Intermediate',
    audience: 'NVIDIA GPU 하드웨어에서 최대 처리량을 목표로 LLM을 배포하는 플랫폼 엔지니어와 ML 인프라 팀 대상 — 원클릭 데스크톱 앱을 비교하는 개인 사용자 대상이 아님',
    primaryTerm: 'TensorRT-LLM',
    ...
    quickAnswerTop: {
      ko: {
        ...
        answer: 'TensorRT-LLM은 NVIDIA GPU에서 LLM 추론을 최적화하기 위한 NVIDIA의 무료 오픈소스(Apache 2.0) 라이브러리입니다. NVIDIA의 TensorRT 추론 SDK 위에 구축되어 있으며, 인플라이트 배칭, 페이지 기반 KV 캐시, 커스텀 어텐션 커널 같은 기술을 사용해 모델을 GPU 전용 최적화 엔진으로 사전에 컴파일합니다. FP8과 INT4까지의 양자화를 지원하며, 최신 포맷은 현행 세대(Hopper/Ada/Blackwell) NVIDIA GPU가 필요합니다. 일반적으로 NVIDIA Triton Inference Server를 통해 배포되거나 NVIDIA NIM 마이크로서비스 내부에 배포됩니다.',
        bullets: [
          'Apache 2.0 라이선스, [NVIDIA가 GitHub에 공개](https://github.com/NVIDIA/TensorRT-LLM)',
          '모델을 GPU 전용 최적화 엔진으로 사전 컴파일 — vLLM이나 llama.cpp의 직접 로드 방식과는 다름',
          '높은 처리량을 위한 인플라이트(연속) 배칭과 페이지 기반 KV 캐시',
          'FP8, INT4/INT8을 포함한 양자화 지원, AWQ 및 GPTQ 포맷 포함',
          'NVIDIA GPU에서만 작동; FP8, FP4 같은 최신 포맷은 현행 세대 Hopper/Ada/Blackwell 하드웨어가 필요',
        ],
        updatedDate: '2026-09',
      },
    },
    toc: [
      { label: 'TL;DR', anchor: 'tldr' },
      { label: 'TensorRT-LLM이란?', anchor: 'what-is-tensorrt-llm' },
      { label: 'TensorRT-LLM이 빠른 이유', anchor: 'how-tensorrt-llm-works' },
      { label: '하드웨어 요구 사항', anchor: 'hardware-requirements' },
      { label: '양자화 지원', anchor: 'quantization-support' },
      { label: 'TensorRT-LLM 배포 방식', anchor: 'deployment-triton-nim' },
      { label: 'TensorRT-LLM 엔진 구축 및 실행 방법', anchor: 'how-to-build-run-engine' },
      { label: 'TensorRT-LLM vs. vLLM vs. llama.cpp', anchor: 'tensorrt-llm-vs-vllm-llama-cpp' },
      { label: 'TensorRT-LLM vs. NVIDIA NIM 및 Triton', anchor: 'tensorrt-llm-vs-nim-triton' },
      { label: 'TensorRT-LLM에 적합한 사용자', anchor: 'who-should-use-tensorrt-llm' },
    ...
    sections: {
      tldr: {
        id: 'tldr',
        title: 'TL;DR — 2026년의 TensorRT-LLM',
        isTldr: true,
        snippetBlocks: [
          { type: 'one-sentence', text: 'TensorRT-LLM은 인플라이트 배칭, 페이지 기반 KV 캐시, 양자화를 활용해 LLM을 GPU 전용 최적화 추론 엔진으로 컴파일하는 NVIDIA의 무료 Apache 2.0 라이선스 라이브러리로, NVIDIA GPU에서의 최대 처리량을 목표로 한다.' },
          { type: 'plain-terms', text: '모델을 단순히 로드해서 실행하는 대신, TensorRT-LLM에는 추가적인 "빌드" 단계가 있다. 자신의 특정 NVIDIA GPU에 맞춰 모델을 한 번 컴파일하면, 결과로 나온 엔진이 범용 로더보다 해당 GPU에서 더 빠르게 실행된다 — 다만 NVIDIA 하드웨어에서만 작동하며 GPU 세대나 모델을 바꾸면 재빌드가 필요하다.' },
        ],
        items: [
          '무료이자 Apache 2.0 라이선스 오픈소스, [NVIDIA가 GitHub에 공개](https://github.com/NVIDIA/TensorRT-LLM)',
        ...
        callouts: [
          { type: 'note', text: '이 글은 NVIDIA의 공식 [TensorRT-LLM GitHub 저장소](https://github.com/NVIDIA/TensorRT-LLM)와 공개 문서를 기반으로 작성되었으며, 독립적인 벤치마크가 아닙니다. 구체적인 처리량이나 지연 시간 수치는 이 글을 위해 독립적으로 측정되지 않았고 GPU 세대, 모델, 배치 구성, TensorRT-LLM 버전에 따라 크게 달라지기 때문에 포함하지 않았습니다. 이 글은 제3자로서 TensorRT-LLM을 사실에 기반해 설명하는 것이며, NVIDIA와 제휴하거나 NVIDIA의 승인을 받은 것이 아닙니다.' },
        ],
      },
      overview: {
        id: 'what-is-tensorrt-llm',
        title: 'TensorRT-LLM이란?',
        content: 'TensorRT-LLM은 NVIDIA GPU에서 대규모 언어 모델 추론을 최적화하고 실행하기 위해 NVIDIA가 공개한 오픈소스 라이브러리입니다. NVIDIA의 범용 딥러닝 추론 SDK인 [TensorRT](https://developer.nvidia.com/tensorrt) 위에 구축되었으며, LLM 전용 런타임 기능 계층과 모델을 정의하고 최적화된 엔진으로 빌드하기 위한 Python API를 추가로 제공합니다.',
        items: [
          'NVIDIA가 공개하고 유지 관리하며, [GitHub](https://github.com/NVIDIA/TensorRT-LLM)에서 Apache 2.0 라이선스로 오픈소스 배포',
          'NVIDIA의 범용 TensorRT 추론 SDK를 트랜스포머 및 LLM 전용 최적화로 확장',
          '모델을 컴파일하기 위한 Python API(`tensorrt_llm.LLM`)와 `trtllm-build` 명령줄 워크플로 제공',
      ...
      howItWorks: {
        id: 'how-tensorrt-llm-works',
        title: 'TensorRT-LLM이 빠른 이유는?',
        content: 'TensorRT-LLM의 성능 접근 방식은 단일 기술에 의존하는 대신 사전 컴파일 단계와 여러 LLM 전용 런타임 최적화를 결합합니다.',
        items: [
        ...
        note: '이 글에는 위 기술 중 어느 하나에 귀속되는, 독립적으로 측정된 처리량 향상 수치가 포함되어 있지 않습니다 — NVIDIA 자체 문서와 저장소는 메커니즘과 그 근거를 설명하지만, 정확한 속도 향상 폭은 GPU 세대, 모델 아키텍처, 배치 구성, TensorRT-LLM 버전에 크게 좌우되므로 다른 곳에서 본 헤드라인성 배수는 자신의 워크로드에서 검증해야 할 주장으로 취급해야 합니다.',
      },
      hardware: {
        id: 'hardware-requirements',
        title: 'TensorRT-LLM에 필요한 하드웨어는?',
        content: 'TensorRT-LLM은 NVIDIA GPU에서만 작동합니다 — CPU 전용, AMD, Intel, Apple Silicon용 백엔드는 존재하지 않습니다. NVIDIA 자체 GPU 라인업 내에서도 사용 가능한 최적화는 GPU의 아키텍처 세대에 따라 달라집니다.',
        itemHeadings: true,
        columns: ['GPU 세대', '세부 내용'],
        rows: [
          { 'GPU 세대': 'Blackwell(예: B200)', '세부 내용': '이 글 작성 시점 기준 가장 최신으로 지원되는 아키텍처. NVIDIA 문서에 따르면 FP8과 더불어 하드웨어 가속 FP4(NVFP4) 지원이 추가됨.' },
          { 'GPU 세대': 'Hopper(예: H100, H200)', '세부 내용': '하드웨어 FP8 지원; NVIDIA가 TensorRT-LLM의 최신 양자화 및 어텐션 최적화에서 가장 성숙한 경로 중 하나로 문서화함.' },
      ...
      quantization: {
        id: 'quantization-support',
        title: 'TensorRT-LLM이 지원하는 양자화 포맷은?',
        content: 'TensorRT-LLM은 메모리 사용량을 줄이고 처리량을 높이기 위해 낮은 수치 정밀도로 모델을 실행하는 것을 지원하며, 사용 가능한 구체적인 포맷은 대상 GPU 세대에 따라 달라집니다.',
        itemHeadings: true,
        columns: ['포맷', '세부 내용'],
        rows: [
          { '포맷': 'FP8', '세부 내용': 'Hopper 및 Blackwell GPU에서 하드웨어 가속되는 8비트 부동소수점; NVIDIA는 이를 해당 세대에서 일반적으로 정밀도/처리량 균형이 가장 좋은 방식으로 문서화함.' },
          { '포맷': 'FP4(NVFP4)', '세부 내용': 'Blackwell 전용 4비트 부동소수점 포맷으로, NVIDIA가 Blackwell 세대 GPU 및 현행 TensorRT/CUDA 툴체인 버전에 대해 문서화함.' },
          { '포맷': 'INT8 / INT4', '세부 내용': 'SmoothQuant를 포함한 정수 양자화 경로로, 완전한 FP8 툴링 지원 범위가 좁은 GPU 세대(예: Ada)에서 대체 수단으로 문서화됨.' },
          { '포맷': 'AWQ / GPTQ', '세부 내용': '커뮤니티에서 확립된 4비트 가중치 양자화 방식으로, TensorRT-LLM은 자체 정밀도 옵션과 함께 이 포맷들을 지원함.' },
        ],
        note: '이 글에는 GPU 세대별 각 포맷의 품질 저하에 대해 독립적으로 측정한 수치가 포함되어 있지 않습니다 — 이는 모델 아키텍처와 작업에 따라 달라지므로, 자신의 프롬프트와 하드웨어에서 몇 가지 포맷의 출력을 비교해 보는 것이 트레이드오프를 판단하는 가장 신뢰할 수 있는 방법입니다.',
      },
      deployment: {
        id: 'deployment-triton-nim',
        title: 'TensorRT-LLM은 어떻게 배포되나요?',
        content: 'TensorRT-LLM은 자체 Python/C++ 런타임을 통해 직접 실행할 수 있지만, 프로덕션 환경에서는 그 주변에 구축된 두 가지 NVIDIA 배포 계층 중 하나를 통해 사용되는 경우가 가장 흔합니다.',
        items: [
      ...
      setupGuide: {
        id: 'how-to-build-run-engine',
        title: 'TensorRT-LLM 엔진은 어떻게 구축하고 실행하나요?',
        content: 'TensorRT-LLM은 특정 CUDA 및 TensorRT 툴체인 버전에 밀접하게 의존하기 때문에, NVIDIA GPU와 이에 맞는 CUDA 드라이버, 그리고 일반적으로 종속성 버전 불일치를 피하기 위한 NVIDIA 자체 컨테이너 이미지가 필요합니다.',
        numberedItems: [
          '지원되는 NVIDIA GPU(최신 최적화를 위해서는 Hopper, Ada, Blackwell 세대)와 최신 CUDA 드라이버가 설치되어 있는지 확인합니다.',
          'NVIDIA의 공식 TensorRT-LLM 컨테이너 이미지를 가져오거나, 맞는 CUDA 환경에 `tensorrt_llm` Python 패키지를 설치합니다 — 컨테이너화 경로가 대부분의 종속성 버전 불일치를 피할 수 있습니다.',
          'TensorRT-LLM Python API나 해당 모델 계열용 예제 변환 스크립트를 사용해 소스 모델 체크포인트(예: [Hugging Face](https://huggingface.co/models)에서)를 변환하거나 로드합니다.',
          '`trtllm-build` 명령으로 특정 GPU에 맞춘 최적화 엔진을 구축하며, 빌드 시점에 정밀도(FP16, FP8, INT4/INT8, 또는 Blackwell의 FP4)와 배칭 구성을 선택합니다.',
          '빌드된 엔진을 실행합니다. OpenAI 호환 엔드포인트를 위해 직접 `trtllm-serve`로 실행하거나, NVIDIA Triton Inference Server의 TensorRT-LLM 백엔드를 엔진 디렉터리로 지정합니다.',
          '프로덕션 트래픽을 라우팅하기 전에, 배포된 엔드포인트로 테스트 요청(`curl` 또는 임의의 OpenAI API 호환 클라이언트)을 보내 엔진이 올바르게 로드되고 생성되는지 확인합니다.',
          'GPU 세대나 모델을 변경하거나 새로운 TensorRT-LLM 릴리스를 반영하고 싶을 때마다 빌드 단계를 다시 실행합니다 — 한 GPU 세대용으로 빌드된 엔진이 다른 세대에서 최적으로, 혹은 아예 작동한다는 보장은 없습니다.',
        ],
        faqs: [
          { q: 'GPU마다 엔진을 다시 빌드해야 하나요?', a: '최적의 결과를 위해서는 일반적으로 그렇습니다 — 엔진은 특정 GPU 아키텍처 세대에 맞춘 커널 선택과 최적화로 컴파일되므로, 다른 GPU 세대로 옮기려면 보통 다시 빌드해야 합니다.' },
          { q: 'TensorRT-LLM에서 사전 양자화된 모델을 사용할 수 있나요?', a: '네 — TensorRT-LLM은 빌드 단계에서 적용되는 자체 FP8/INT8/INT4/FP4 양자화 외에도, AWQ나 GPTQ로 양자화된 모델로부터 엔진을 구축하는 것을 지원합니다.' },
        ],
      },
      vsVllmLlamaCpp: {
        id: 'tensorrt-llm-vs-vllm-llama-cpp',
      ...
      vsNimTriton: {
        id: 'tensorrt-llm-vs-nim-triton',
        title: 'TensorRT-LLM은 NVIDIA NIM, Triton과 어떤 관계인가요?',
        content: 'TensorRT-LLM, NVIDIA NIM, NVIDIA Triton Inference Server는 서로 경쟁 관계가 아닙니다 — 이들은 동일한 NVIDIA 추론 스택의 서로 다른 계층이며, 배포를 계획할 때 이 차이를 이해하는 것이 중요합니다.',
        itemHeadings: true,
        columns: ['계층', '세부 내용'],
        rows: [
          { '계층': 'TensorRT-LLM', '세부 내용': '엔진이자 컴파일러: 모델을 최적화된 GPU 전용 추론 엔진으로 변환함. 무료이자 오픈소스(Apache 2.0)이며, 직접 운영함.' },
      ...
      whoShouldUse: {
        id: 'who-should-use-tensorrt-llm',
        title: 'TensorRT-LLM은 누구에게 적합한가요?',
        content: 'TensorRT-LLM은 이미 NVIDIA GPU 하드웨어에 투자했고 그로부터 달성 가능한 최고의 추론 처리량이 필요한 팀에 적합하며, 모델을 실행하는 가장 간단한 방법을 찾는 사람에게는 적합하지 않습니다.',
        subsections: [
          {
            title: 'TensorRT-LLM을 사용해야 하는 경우',
            list: [
              '배포가 전적으로 NVIDIA GPU에서 이루어지며 GPU당 처리량과 비용 효율을 극대화하고 싶은 경우',
              '현행 세대 하드웨어(Hopper, Ada, Blackwell)를 대상으로 하며 FP8이나 FP4 같은 최신 양자화 포맷을 사용하고 싶은 경우',
              '모델, GPU 세대, TensorRT-LLM 버전이 바뀔 때마다 다시 실행해야 하는 빌드/컴파일 파이프라인을 유지 관리하는 데 부담이 없는 경우',
              'NVIDIA Triton Inference Server를 통한 배포를 계획하고 있거나, NVIDIA NIM을 평가 중이며 그 이면의 엔진을 이해하고 싶은 경우',
              '팀이 직접 로드형 엔진보다 더 복잡한 배포 파이프라인을 관리할 CUDA 및 MLOps 전문성을 갖춘 경우',
            ],
          },
          {
      ...
      comparisonTable: {
        id: 'tensorrt-llm-alternatives-summary',
        title: 'TensorRT-LLM과 대안 한눈에 비교',
        content: '이 도구들은 설정 복잡성과 최고 성능 사이의 스펙트럼에서 서로 다른 위치에 있습니다.',
        itemHeadings: true,
      ...
      limitations: {
        id: 'limitations',
        title: '이 글이 다루지 않는 내용은?',
        content: '이 글은 NVIDIA의 공개 문서와 저장소를 바탕으로 작성된 설명 글이며, 실측 벤치마크 보고서가 아닙니다.',
        items: [
        ...
        note: '자신의 하드웨어와 모델에서의 정확한 처리량이 의사 결정에 결정적이라면, 가장 신뢰할 수 있는 방법은 이 글을 포함한 어떤 단일 글의 수치에 의존하지 않고 직접 엔진을 구축하여 자신의 GPU 세대, 모델, 요청 패턴으로 부하 테스트를 하는 것입니다. 하드웨어, 드라이버 버전, TensorRT-LLM 릴리스가 모두 결과를 바꿀 수 있기 때문입니다.',
      },
      commonMistakes: {
        id: 'common-mistakes',
        title: 'TensorRT-LLM을 시도할 때 흔한 실수',
        content: 'TensorRT-LLM에서 겪는 마찰은 대부분 빌드/컴파일 단계를 과소평가하거나 직접 로드형 엔진처럼 작동하기를 기대하는 데서 비롯됩니다.',
        subsections: [
          {
            title: '실수 1: llama.cpp나 vLLM처럼 모델을 로드할 수 있을 것이라고 기대함',
            content: 'TensorRT-LLM은 트래픽을 처리하기 전에 모델을 GPU 전용 엔진으로 컴파일해야 합니다. "체크포인트를 지정하기만 하면 바로 실행"되는 경로는 없습니다 — 빌드 단계를 건너뛸 수는 없으며, 빌드 자체도 모델 크기에 따라 상당한 시간이 걸릴 수 있습니다.',
          },
          {
            title: '실수 2: 한 GPU용으로 빌드한 엔진이 다른 GPU에서도 작동할 것이라고 가정함',
            content: '특정 GPU 아키텍처 세대용으로 컴파일된 엔진은 해당 하드웨어의 커널과 정밀도 지원에 맞춰 튜닝되어 있습니다. 재빌드 없이 다른 GPU 세대로 옮기면 성능이 나빠지거나 아예 작동하지 않을 수 있습니다 — 엔진은 대상 GPU 세대마다 다시 빌드해야 합니다.',
          },
          {
      ...
      faq: {
        ...
        title: '자주 묻는 질문',
        faqs: [
          { q: 'TensorRT-LLM이란 무엇인가요?', a: 'TensorRT-LLM은 NVIDIA가 공개한 무료 오픈소스(Apache 2.0) 라이브러리로, NVIDIA의 TensorRT 딥러닝 추론 SDK 위에 구축되어 대규모 언어 모델을 NVIDIA GPU 전용으로 구축된 최적화 추론 엔진으로 컴파일합니다.' },
          { q: 'TensorRT-LLM은 무료인가요?', a: '네. TensorRT-LLM 자체는 Apache 2.0 라이선스로 배포되는 무료 오픈소스 소프트웨어입니다. TensorRT-LLM 백엔드를 패키징한 별도의 유료 마이크로서비스 계층인 NVIDIA NIM은 NVIDIA AI Enterprise 구독이 필요합니다.' },
          { q: 'TensorRT-LLM은 AMD나 애플 GPU에서 작동하나요?', a: '아니요. TensorRT-LLM은 NVIDIA GPU에서만 작동합니다 — 더 폭넓은 하드웨어를 지원하는 vLLM이나 llama.cpp와 달리 CPU 전용, AMD, Intel, Apple Silicon용 백엔드는 존재하지 않습니다.' },
          { q: '왜 TensorRT-LLM은 먼저 모델을 컴파일해야 하나요?', a: 'TensorRT-LLM은 특정 GPU 아키텍처 세대에 맞춰 커널 선택과 최적화를 고정한 엔진으로 모델을 사전에 빌드하며, 이것이 성능 목표를 달성하는 방식입니다. 이는 로드 시점에 모든 것을 동적으로 결정하는 엔진에 비해, 빌드 단계와 낮은 하드웨어 간 유연성을 대가로 해당 특정 NVIDIA GPU에서 더 높은 최고 처리량을 얻는 방식입니다.' },
          { q: 'TensorRT-LLM은 어떤 양자화 포맷을 지원하나요?', a: 'TensorRT-LLM은 FP8과 FP4(FP4는 Blackwell 세대 GPU 전용), SmoothQuant를 포함한 INT8과 INT4, 그리고 AWQ와 GPTQ 같은 커뮤니티 포맷을 지원하며, 정확한 지원 여부는 대상 GPU 세대에 따라 다릅니다.' },
          { q: 'TensorRT-LLM이 vLLM보다 나은가요?', a: '"더 낫다"는 것은 용도에 따라 다릅니다: TensorRT-LLM은 사전 컴파일 단계와 NVIDIA 전용 지원이라는 비용을 감수하며 특히 NVIDIA 하드웨어에서 달성 가능한 최고의 GPU당 처리량을 목표로 합니다. vLLM은 컴파일 단계 없이 모델을 직접 로드하며 NVIDIA GPU를 넘어서는 백엔드 지원을 문서화하고 있습니다. 어느 쪽도 모든 경우에 보편적으로 더 빠르지는 않습니다 — 위 비교표를 참고하세요.' },
          { q: 'TensorRT-LLM과 NVIDIA NIM의 차이는 무엇인가요?', a: 'TensorRT-LLM은 직접 운영하는 무료 오픈소스 엔진이자 컴파일러입니다. NVIDIA NIM은 표준화된 API 뒤에 TensorRT-LLM 최적화 백엔드를 패키징하고 제조사 지원을 제공하는 별도의 유료 마이크로서비스 계층으로, NVIDIA AI Enterprise 구독의 일부로 판매됩니다.' },
          { q: 'TensorRT-LLM은 여러 GPU에 걸쳐 모델을 서빙할 수 있나요?', a: '네. NVIDIA는 단일 GPU에 들어가지 않는 모델을 위한 TensorRT-LLM의 멀티 GPU 및 멀티 노드 서빙 지원을 문서화하고 있으며, 일반적으로 프로덕션 오케스트레이션을 위해 NVIDIA Triton Inference Server를 통해 배포됩니다.' },
        ],
      },
      sources: {
        id: 'sources',
        title: '출처',
        links: [
          { url: 'https://github.com/NVIDIA/TensorRT-LLM', title: 'TensorRT-LLM — 공식 GitHub 저장소', description: 'NVIDIA가 공개한 TensorRT-LLM의 소스 코드, README, 문서 링크, 릴리스 노트.' },
          { url: 'https://nvidia.github.io/TensorRT-LLM/', title: 'TensorRT-LLM 문서', description: '설치, 빌드 워크플로, 양자화, 배포를 다루는 공식 문서.' },
          { url: 'https://developer.nvidia.com/triton-inference-server', title: 'NVIDIA Triton Inference Server', description: 'TensorRT-LLM 백엔드를 갖춘 모델 서빙 플랫폼의 공식 제품 페이지.' },
        ],
      },
      relatedReading: {
        id: 'related-reading',
  ...
  ar: {
    theme: 'Overview & Reference',
    heroImage: '/images/tensorrt-llm-explained-hero-ar.webp',
    freshness_tier: 'semi_annual',
    next_refresh_due: '2027-03-06',
    publishDate: '2026-09-06',
    dateModified: '2026-09-06',
    educationalLevel: 'Intermediate',
    audience: 'مهندسو المنصات وفرق البنية التحتية لتعلم الآلة الذين ينشرون نماذج اللغة الكبيرة لتحقيق أقصى إنتاجية على عتاد GPU من NVIDIA — وليس المستخدمين الأفراد الذين يقارنون تطبيقات سطح المكتب سهلة التثبيت',
    primaryTerm: 'TensorRT-LLM',
    ...
    targetKeywords: [
      'شرح TensorRT-LLM',
      'ما هو TensorRT-LLM',
      'TensorRT-LLM مقابل vLLM',
      'TensorRT-LLM مقابل llama.cpp',
    ...
    quickAnswerTop: {
      ar: {
        ...
        answer: 'TensorRT-LLM هي مكتبة NVIDIA المجانية ومفتوحة المصدر (Apache 2.0) لتحسين استدلال نماذج اللغة الكبيرة على وحدات GPU من NVIDIA. وهي مبنية على حزمة أدوات الاستدلال TensorRT من NVIDIA، وتقوم بتجميع النموذج مسبقًا في محرك مُحسَّن مخصص لوحدة GPU معينة، باستخدام تقنيات مثل التجميع المستمر أثناء التشغيل، وذاكرة تخزين مؤقت KV مُقسَّمة إلى صفحات، ونوى انتباه مخصصة. وتدعم التكميم حتى FP8 وINT4، مع اشتراط أن تتطلب الصيغ الأحدث عتاد NVIDIA من الجيل الحالي (Hopper/Ada/Blackwell). وعادة ما تُنشر عبر NVIDIA Triton Inference Server أو داخل خدمات NIM المصغّرة من NVIDIA.',
        bullets: [
          'مرخّصة بموجب Apache 2.0، ونشرتها [NVIDIA على GitHub](https://github.com/NVIDIA/TensorRT-LLM)',
          'تجمّع النماذج مسبقًا في محرك مُحسَّن مخصص لوحدة GPU — على عكس أسلوب التحميل المباشر في vLLM أو llama.cpp',
          'تجميع مستمر أثناء التشغيل (in-flight) وذاكرة تخزين مؤقت KV مُقسَّمة إلى صفحات لخدمة عالية الإنتاجية',
          'دعم التكميم يشمل FP8 وINT4/INT8، بما في ذلك صيغتي AWQ وGPTQ',
          'تعمل فقط على وحدات GPU من NVIDIA؛ وتحتاج الصيغ الأحدث مثل FP8 وFP4 إلى عتاد من جيل Hopper/Ada/Blackwell الحالي',
        ],
        updatedDate: '2026-09',
      },
    },
    toc: [
      ...
      { label: 'ما هو TensorRT-LLM؟', anchor: 'what-is-tensorrt-llm' },
      { label: 'ما الذي يجعل TensorRT-LLM سريعًا؟', anchor: 'how-tensorrt-llm-works' },
      { label: 'متطلبات العتاد', anchor: 'hardware-requirements' },
      { label: 'دعم التكميم', anchor: 'quantization-support' },
      { label: 'كيفية نشر TensorRT-LLM', anchor: 'deployment-triton-nim' },
      { label: 'كيفية بناء وتشغيل محرك TensorRT-LLM', anchor: 'how-to-build-run-engine' },
      { label: 'TensorRT-LLM مقابل vLLM مقابل llama.cpp', anchor: 'tensorrt-llm-vs-vllm-llama-cpp' },
      { label: 'TensorRT-LLM مقابل NVIDIA NIM وTriton', anchor: 'tensorrt-llm-vs-nim-triton' },
      { label: 'لمن يناسب TensorRT-LLM', anchor: 'who-should-use-tensorrt-llm' },
    ...
    sections: {
      tldr: {
        id: 'tldr',
        title: 'الملخص السريع — TensorRT-LLM في 2026',
        isTldr: true,
        snippetBlocks: [
          { type: 'one-sentence', text: 'TensorRT-LLM هي مكتبة NVIDIA المجانية بترخيص Apache 2.0 التي تجمّع نماذج اللغة الكبيرة في محركات استدلال مُحسَّنة مخصصة لوحدة GPU معينة باستخدام التجميع المستمر أثناء التشغيل وذاكرة تخزين مؤقت KV المُقسَّمة إلى صفحات والتكميم، بهدف تحقيق أقصى إنتاجية على وحدات GPU من NVIDIA.' },
          { type: 'plain-terms', text: 'بدلًا من مجرد تحميل نموذج وتشغيله، تضيف TensorRT-LLM خطوة "بناء" إضافية: تقوم بتجميع النموذج مرة واحدة خصيصًا لوحدة GPU الخاصة بك من NVIDIA، فيعمل المحرك الناتج بسرعة أكبر على تلك الوحدة مقارنة بمُحمِّل عام الغرض — لكنه يعمل فقط على عتاد NVIDIA ويحتاج إلى إعادة بناء إذا غيّرت جيل وحدة GPU أو النموذج.' },
        ],
        items: [
          'مجانية ومفتوحة المصدر بترخيص Apache 2.0، نشرتها [NVIDIA على GitHub](https://github.com/NVIDIA/TensorRT-LLM)',
        ...
        callouts: [
          { type: 'note', text: 'يستند هذا المقال إلى [مستودع GitHub الرسمي لـ TensorRT-LLM](https://github.com/NVIDIA/TensorRT-LLM) من NVIDIA وإلى وثائقها العامة، وليس إلى اختبارات أداء مستقلة. ولم تُدرَج أرقام محددة للإنتاجية أو زمن الاستجابة لأنها لم تُقَس بشكل مستقل لهذا المقال، وتتفاوت بشدة حسب جيل وحدة GPU والنموذج وتركيبة الدُفعات وإصدار TensorRT-LLM. يصف هذا المقال TensorRT-LLM بشكل واقعي كطرف ثالث؛ وهو غير تابع لـ NVIDIA ولا معتمَد منها.' },
        ],
      },
      overview: {
        id: 'what-is-tensorrt-llm',
        title: 'ما هو TensorRT-LLM؟',
        content: 'TensorRT-LLM هي مكتبة مفتوحة المصدر نشرتها NVIDIA لتحسين وتشغيل استدلال نماذج اللغة الكبيرة على وحدات GPU من NVIDIA. وهي مبنية فوق [TensorRT](https://developer.nvidia.com/tensorrt)، حزمة أدوات الاستدلال العامة للتعلم العميق من NVIDIA، وتضيف طبقة من ميزات وقت التشغيل الخاصة بنماذج اللغة الكبيرة وواجهة برمجة تطبيقات بلغة Python لتعريف النماذج وتجميعها في محركات مُحسَّنة.',
        items: [
          'نشرتها وتصونها NVIDIA، وتُوزَّع كمصدر مفتوح بترخيص Apache 2.0 على [GitHub](https://github.com/NVIDIA/TensorRT-LLM)',
          'توسِّع حزمة أدوات الاستدلال العامة TensorRT من NVIDIA بتحسينات خاصة بمحولات Transformer ونماذج اللغة الكبيرة',
          'توفر واجهة برمجة تطبيقات بلغة Python (`tensorrt_llm.LLM`) بالإضافة إلى سير عمل عبر سطر الأوامر `trtllm-build` لتجميع النماذج',
      ...
      howItWorks: {
        id: 'how-tensorrt-llm-works',
        title: 'ما الذي يجعل TensorRT-LLM سريعًا؟',
        content: 'يجمع نهج الأداء في TensorRT-LLM بين خطوة تجميع مسبقة وعدة تحسينات وقت تشغيل خاصة بنماذج اللغة الكبيرة، بدلًا من الاعتماد على تقنية واحدة فقط.',
        items: [
        ...
        note: 'لا يتضمن هذا المقال أي مكاسب إنتاجية مقيسة بشكل مستقل تُعزى إلى أي من التقنيات المذكورة أعلاه بمفردها — تصف وثائق NVIDIA ومستودعها الآليات ومنطقها، لكن مقدار التسريع الدقيق يعتمد بشدة على جيل وحدة GPU وبنية النموذج وتركيبة الدُفعات وإصدار TensorRT-LLM، لذا ينبغي التعامل مع أي معامل ضرب بارز تراه في مكان آخر كادّعاء يجب التحقق منه على حِملك الخاص.',
      },
      hardware: {
        id: 'hardware-requirements',
        title: 'ما العتاد الذي يحتاجه TensorRT-LLM؟',
        content: 'يعمل TensorRT-LLM حصريًا على وحدات GPU من NVIDIA — ولا توجد خلفية مخصصة لوحدة المعالجة المركزية وحدها أو AMD أو Intel أو Apple Silicon. وضمن تشكيلة وحدات GPU الخاصة بـ NVIDIA نفسها، تعتمد التحسينات المتاحة على جيل بنية وحدة GPU.',
        itemHeadings: true,
        columns: ['جيل وحدة GPU', 'التفاصيل'],
        rows: [
          { 'جيل وحدة GPU': 'Blackwell (مثل B200)', 'التفاصيل': 'أحدث بنية مدعومة حتى وقت كتابة هذا المقال. تضيف دعمًا لصيغة FP4 (NVFP4) بتسريع عتادي إلى جانب FP8، وفقًا لوثائق NVIDIA.' },
          { 'جيل وحدة GPU': 'Hopper (مثل H100 وH200)', 'التفاصيل': 'دعم عتادي لصيغة FP8؛ وتوثِّقه NVIDIA كأحد أنضج المسارات لتحسينات التكميم والانتباه الأحدث في TensorRT-LLM.' },
      ...
      quantization: {
        id: 'quantization-support',
        title: 'ما صيغ التكميم التي تدعمها TensorRT-LLM؟',
        content: 'تدعم TensorRT-LLM تشغيل النماذج بدقة رقمية مخفَّضة لتقليل استهلاك الذاكرة وزيادة الإنتاجية، مع اعتماد الصيغ المتاحة تحديدًا على جيل وحدة GPU المستهدفة.',
        itemHeadings: true,
        columns: ['الصيغة', 'التفاصيل'],
        rows: [
          { 'الصيغة': 'FP8', 'التفاصيل': 'فاصلة عائمة بـ8 بتات بتسريع عتادي على وحدات GPU من Hopper وBlackwell؛ وتوثِّق NVIDIA هذا عمومًا كأفضل موازنة بين الدقة والإنتاجية على هذين الجيلين.' },
          { 'الصيغة': 'FP4 (NVFP4)', 'التفاصيل': 'صيغة فاصلة عائمة بـ4 بتات حصرية لـBlackwell، توثِّقها NVIDIA لوحدات GPU من جيل Blackwell وإصدارات حديثة من سلسلة أدوات TensorRT/CUDA.' },
          { 'الصيغة': 'INT8 / INT4', 'التفاصيل': 'مسارات تكميم بأعداد صحيحة تشمل SmoothQuant، موثَّقة كبديل على أجيال وحدات GPU (مثل Ada) حيث تكون تغطية أدوات FP8 الكاملة أضيق.' },
          { 'الصيغة': 'AWQ / GPTQ', 'التفاصيل': 'أساليب راسخة من المجتمع لتكميم الأوزان بـ4 بتات، وتدعمها TensorRT-LLM إلى جانب خيارات الدقة الخاصة بها.' },
        ],
        note: 'لا يتضمن هذا المقال أرقامًا مقيسة بشكل مستقل لفقدان الجودة لكل صيغة على كل جيل من وحدات GPU — فهذه الأرقام تختلف حسب بنية النموذج والمهمة، لذا فإن مقارنة مخرجات بضع صيغ باستخدام موجّهاتك (prompts) وعتادك الخاص هي الطريقة الأكثر موثوقية للحكم على المقايضة.',
      },
      deployment: {
        id: 'deployment-triton-nim',
        title: 'كيف يُنشر TensorRT-LLM؟',
        content: 'يمكن تشغيل TensorRT-LLM مباشرة عبر بيئة تشغيله الخاصة بلغتي Python/C++، لكن في بيئات الإنتاج غالبًا ما يُستخدم عبر إحدى طبقتي نشر من NVIDIA مبنيتين حوله.',
        items: [
      ...
      setupGuide: {
        id: 'how-to-build-run-engine',
        title: 'كيف تبني وتشغّل محرك TensorRT-LLM؟',
        content: 'تحتاج TensorRT-LLM إلى وحدة GPU من NVIDIA، وتعريف CUDA مطابق، وعادةً إلى صورة الحاوية الرسمية من NVIDIA لتجنب تعارضات إصدارات الاعتماديات، نظرًا لاعتمادها الوثيق على إصدارات محددة من سلسلة أدوات CUDA وTensorRT.',
        numberedItems: [
          'تأكد من امتلاكك وحدة GPU مدعومة من NVIDIA (جيل Hopper أو Ada أو Blackwell للحصول على أحدث التحسينات) مع تعريف CUDA حديث مثبَّت.',
          'اسحب صورة الحاوية الرسمية لـ TensorRT-LLM من NVIDIA، أو ثبّت حزمة Python باسم `tensorrt_llm` في بيئة CUDA مطابقة — يتجنب المسار المُحوسَب (containerized) معظم تعارضات الإصدارات.',
          'حوِّل أو حمِّل نقطة تفتيش النموذج المصدر (على سبيل المثال من [Hugging Face](https://huggingface.co/models)) باستخدام واجهة برمجة تطبيقات TensorRT-LLM بلغة Python أو نصوص التحويل النموذجية الخاصة بتلك العائلة من النماذج.',
          'ابنِ المحرك المُحسَّن لوحدة GPU المحددة الخاصة بك باستخدام أمر `trtllm-build`، واختر دقة (FP16 أو FP8 أو INT4/INT8 أو FP4 على Blackwell) وتهيئة تجميع دفعات وقت البناء.',
          'شغّل المحرك المبني، إما مباشرة باستخدام `trtllm-serve` للحصول على نقطة نهاية متوافقة مع OpenAI، أو بتوجيه خلفية TensorRT-LLM في NVIDIA Triton Inference Server إلى دليل المحرك.',
          'أرسل طلبًا اختباريًا إلى نقطة النهاية المنشورة (`curl` أو أي عميل متوافق مع واجهة برمجة تطبيقات OpenAI) للتأكد من أن المحرك يُحمَّل ويولّد بشكل صحيح قبل توجيه حركة إنتاج فعلية إليه.',
          'أعِد تشغيل خطوة البناء كلما غيّرت جيل وحدة GPU أو النموذج، أو أردت اعتماد إصدار جديد من TensorRT-LLM — فالمحرك المبني لجيل معين من وحدات GPU ليس مضمونًا أن يعمل بشكل مثالي، أو حتى أن يعمل إطلاقًا، على جيل مختلف.',
        ],
        faqs: [
          { q: 'هل عليّ إعادة بناء المحرك لكل وحدة GPU؟', a: 'عمومًا نعم للحصول على أفضل النتائج — يُجمَّع المحرك مع اختيارات نوى وتحسينات مخصصة لجيل بنية معين من وحدات GPU، لذا فإن الانتقال إلى جيل مختلف يتطلب عادة إعادة بنائه.' },
          { q: 'هل يمكنني استخدام نموذج مُكمَّم مسبقًا مع TensorRT-LLM؟', a: 'نعم — تدعم TensorRT-LLM بناء المحركات من نماذج مُكمَّمة بواسطة AWQ أو GPTQ، إضافة إلى تكميمها الخاص بصيغ FP8/INT8/INT4/FP4 المطبَّق أثناء خطوة البناء.' },
        ],
      },
      vsVllmLlamaCpp: {
        id: 'tensorrt-llm-vs-vllm-llama-cpp',
      ...
      vsNimTriton: {
        id: 'tensorrt-llm-vs-nim-triton',
        title: 'كيف ترتبط TensorRT-LLM بـNVIDIA NIM وTriton؟',
        content: 'ليست TensorRT-LLM وNVIDIA NIM وNVIDIA Triton Inference Server متنافسات — بل هي طبقات مختلفة من نفس مكدس الاستدلال الخاص بـNVIDIA، وفهم الفرق مهم عند التخطيط لعملية نشر.',
        itemHeadings: true,
        columns: ['الطبقة', 'التفاصيل'],
        rows: [
          { 'الطبقة': 'TensorRT-LLM', 'التفاصيل': 'المحرك والمُجمِّع: يحوّل النموذج إلى محرك استدلال مُحسَّن مخصص لوحدة GPU. مجاني ومفتوح المصدر (Apache 2.0)؛ تشغّله بنفسك.' },
      ...
      whoShouldUse: {
        id: 'who-should-use-tensorrt-llm',
        title: 'لمن يناسب TensorRT-LLM؟',
        content: 'تناسب TensorRT-LLM الفرق التي التزمت بعتاد GPU من NVIDIA وتحتاج إلى أعلى إنتاجية استدلال ممكنة منه، وليس الأشخاص الباحثين عن أبسط طريقة لتشغيل نموذج.',
        subsections: [
          {
            title: 'استخدم TensorRT-LLM إذا',
            list: [
              'كان نشرك يعمل حصريًا على وحدات GPU من NVIDIA وتريد تعظيم الإنتاجية وكفاءة التكلفة لكل وحدة GPU',
              'كنت تستهدف عتادًا من الجيل الحالي (Hopper أو Ada أو Blackwell) وتريد استخدام أحدث صيغ التكميم فيه، مثل FP8 أو FP4',
              'كنت مرتاحًا للحفاظ على خط أنابيب بناء/تجميع يُعاد تشغيله عند تغيّر النموذج أو جيل وحدة GPU أو إصدار TensorRT-LLM',
              'كنت تخطط للنشر عبر NVIDIA Triton Inference Server، أو تقيّم NVIDIA NIM وتريد فهم المحرك الكامن وراءها',
              'كان فريقك يمتلك خبرة CUDA وMLOps اللازمة لإدارة خط أنابيب نشر أكثر تعقيدًا مما يتطلبه محرك التحميل المباشر',
            ],
          },
          {
      ...
      comparisonTable: {
        id: 'tensorrt-llm-alternatives-summary',
        title: 'TensorRT-LLM مقابل البدائل بلمحة سريعة',
        content: 'تقع هذه الأدوات في نقاط مختلفة على طيف تعقيد الإعداد مقابل أقصى أداء.',
        itemHeadings: true,
      ...
      limitations: {
        ...
        content: 'هذا مقال تفسيري مبني على الوثائق العامة ومستودع NVIDIA، وليس تقرير اختبار أداء عملي.',
        items: [
        ...
        note: 'إذا كانت الإنتاجية الدقيقة على عتادك ونموذجك حاسمة لاتخاذ قرار، فإن الخطوة الأكثر موثوقية هي بناء المحرك بنفسك وإجراء اختبار حِمل باستخدام جيل وحدة GPU ونموذجك ونمط الطلبات الفعلي الخاص بك، بدلًا من الاعتماد على أرقام أي مقال واحد — بما في ذلك هذا المقال، إذ يغيّر العتاد وإصدارات التعريفات وإصدارات TensorRT-LLM جميعها النتائج.',
      },
      commonMistakes: {
        id: 'common-mistakes',
        title: 'أخطاء شائعة عند تجربة TensorRT-LLM',
        content: 'معظم الاحتكاك مع TensorRT-LLM ينشأ من التقليل من شأن خطوة البناء/التجميع أو توقّع أن تتصرف مثل محرك تحميل مباشر.',
        subsections: [
          {
            title: 'الخطأ الأول: توقّع تحميل نموذج كما في llama.cpp أو vLLM',
            content: 'تتطلب TensorRT-LLM تجميع النموذج في محرك مخصص لوحدة GPU قبل أن يتمكن من خدمة أي طلبات. لا يوجد مسار "أشِر فقط إلى نقطة تفتيش وابدأ" — تخطي خطوة البناء ليس خيارًا متاحًا، وقد يستغرق البناء نفسه وقتًا معتبرًا حسب حجم النموذج.',
          },
          {
            title: 'الخطأ الثاني: افتراض أن محركًا مبنيًا لوحدة GPU يعمل على وحدة أخرى',
            content: 'يُضبط المحرك المُجمَّع لجيل بنية معين من وحدات GPU بما يتوافق مع نوى وقدرات الدقة الخاصة بذلك العتاد. ونقله إلى جيل مختلف من وحدات GPU دون إعادة بناء قد يؤدي إلى أداء ضعيف أو إلى فشل تام في التشغيل — إذ يجب إعادة بناء المحرك لكل جيل مستهدف من وحدات GPU.',
          },
          {
      ...
      faq: {
        ...
        faqs: [
          { q: 'ما هو TensorRT-LLM؟', a: 'TensorRT-LLM هي مكتبة مجانية ومفتوحة المصدر (Apache 2.0) نشرتها NVIDIA، تقوم بتجميع نماذج اللغة الكبيرة في محركات استدلال مُحسَّنة مبنية خصيصًا لوحدات GPU من NVIDIA، وهي مبنية فوق حزمة أدوات الاستدلال للتعلم العميق TensorRT من NVIDIA.' },
          { q: 'هل TensorRT-LLM مجانية؟', a: 'نعم. TensorRT-LLM نفسها برمجية مجانية ومفتوحة المصدر مُصدَرة بموجب ترخيص Apache 2.0. أما NVIDIA NIM، وهي طبقة خدمات مصغّرة مدفوعة ومنفصلة تُغلِّف خلفية TensorRT-LLM، فتتطلب اشتراكًا في NVIDIA AI Enterprise.' },
          { q: 'هل تعمل TensorRT-LLM على وحدات GPU من AMD أو Apple؟', a: 'لا. تعمل TensorRT-LLM حصريًا على وحدات GPU من NVIDIA — ولا توجد خلفية مخصصة لوحدة المعالجة المركزية وحدها أو AMD أو Intel أو Apple Silicon، على عكس vLLM أو llama.cpp اللذين يدعمان عتادًا أوسع.' },
          { q: 'لماذا تتطلب TensorRT-LLM تجميع النموذج أولًا؟', a: 'تبني TensorRT-LLM النموذج مسبقًا في محرك تُثبَّت فيه اختيارات النوى والتحسينات لجيل بنية معين من وحدات GPU، وهذه هي الطريقة التي تحقق بها هدف الأداء. وهذا يقايض خطوة بناء ومرونة أقل بين أنواع العتاد مقابل إنتاجية قصوى أعلى على تلك الوحدة المحددة من NVIDIA، مقارنة بالمحركات التي تحدد كل شيء ديناميكيًا عند التحميل.' },
          { q: 'ما صيغ التكميم التي تدعمها TensorRT-LLM؟', a: 'تدعم TensorRT-LLM صيغتي FP8 وFP4 (FP4 حصرية لوحدات GPU من جيل Blackwell)، وINT8 وINT4 بما في ذلك SmoothQuant، وصيغًا من المجتمع مثل AWQ وGPTQ، مع اعتماد التوافر الدقيق على جيل وحدة GPU المستهدفة.' },
          { q: 'هل TensorRT-LLM أفضل من vLLM؟', a: '"الأفضل" يعتمد على الاستخدام: تستهدف TensorRT-LLM أعلى إنتاجية ممكنة لكل وحدة GPU تحديدًا على عتاد NVIDIA، مقابل تكلفة خطوة تجميع مسبقة ودعم حصري لـNVIDIA. أما vLLM فتحمّل النماذج مباشرة دون خطوة تجميع وتوثِّق دعمًا لخلفيات تتجاوز وحدات GPU من NVIDIA وحدها. ولا تُعد أيٌّ منهما أسرع بشكل شامل — راجع جدول المقارنة أعلاه.' },
          { q: 'ما الفرق بين TensorRT-LLM وNVIDIA NIM؟', a: 'TensorRT-LLM هي المحرك والمُجمِّع المجاني ومفتوح المصدر الذي تشغّله بنفسك. أما NVIDIA NIM فهي طبقة خدمات مصغّرة مدفوعة ومنفصلة تُغلِّف خلفية مُحسَّنة بواسطة TensorRT-LLM خلف واجهة برمجة تطبيقات موحّدة مع دعم من المورّد، وتُباع كجزء من اشتراك NVIDIA AI Enterprise.' },
          { q: 'هل يمكن لـTensorRT-LLM خدمة نماذج عبر وحدات GPU متعددة؟', a: 'نعم. توثِّق NVIDIA دعم الخدمة عبر وحدات GPU متعددة وعبر عُقد متعددة في TensorRT-LLM للنماذج الكبيرة جدًا بحيث لا تتسع في وحدة GPU واحدة، ويُنشر ذلك عادة عبر NVIDIA Triton Inference Server لتنسيق الإنتاج.' },
        ],
      },
      sources: {
        id: 'sources',
        title: 'المصادر',
        links: [
          { url: 'https://github.com/NVIDIA/TensorRT-LLM', title: 'TensorRT-LLM — مستودع GitHub الرسمي', description: 'الشيفرة المصدرية وملف README وروابط الوثائق وملاحظات الإصدار لـTensorRT-LLM، التي نشرتها NVIDIA.' },
          { url: 'https://nvidia.github.io/TensorRT-LLM/', title: 'وثائق TensorRT-LLM', description: 'الوثائق الرسمية التي تغطي التثبيت وسير عمل البناء والتكميم والنشر.' },
          { url: 'https://developer.nvidia.com/triton-inference-server', title: 'NVIDIA Triton Inference Server', description: 'صفحة المنتج الرسمية لمنصة خدمة النماذج ذات خلفية TensorRT-LLM.' },
        ],
      },
      relatedReading: {
        id: 'related-reading',
        ...
        items: [
          ...
          '[دليل برمجيات نماذج اللغة الكبيرة المحلية](/ar/directory) — الكتالوج الكامل لأدوات الذكاء الاصطناعي المحلية الذي ينتمي إليه هذا المقال.',
        ],
      },
    },
  },
