  ...
  pt: {
    ...
    sections: {
      ...
      tradeSecretRisk: {
        ...
        content: [
          '**A ponte mais forte de todo este conjunto de conteúdo é o risco de segredo comercial: um engenheiro cola código-fonte proprietário no prompt de uma ferramenta de IA pública, e a informação confidencial deixou a organização no momento em que o prompt foi enviado — independentemente do que a IA devolva como resultado.** Este é o mesmo incidente subjacente coberto em [Shadow AI: quais controles realmente se ajustam ao tamanho da sua empresa](/pt/power-local-llm/shadow-ai-controls-by-company-size), visto pelo lado do risco de PI em vez do lado dos controles de segurança: shadow AI é o comportamento; o vazamento de segredo comercial por meio de um prompt de IA é o dano específico que esse comportamento causa.',
  ...
  zh: {
    ...
    sections: {
      ...
      tradeSecretRisk: {
        ...
        content: [
          '**在整个内容体系中，最强有力的联系点就是商业秘密风险：一名工程师将专有源代码粘贴进某个公开 AI 工具的提示词中，机密信息在提示词发送的那一刻就已离开企业——无论 AI 之后输出什么结果。** 这与[Shadow AI：哪些控制措施真正适合您公司的规模](/zh/power-local-llm/shadow-ai-controls-by-company-size)一文中讨论的是同一起底层事件，只是从知识产权风险的角度而非安全控制的角度来看：Shadow AI 是行为本身；通过 AI 提示词泄露商业秘密，则是该行为造成的具体危害。',
