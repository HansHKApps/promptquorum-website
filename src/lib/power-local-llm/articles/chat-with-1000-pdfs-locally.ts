...
// Complete localization: EN, DE, FR, JA, ZH, ES, KO, PT, AR
...
export const article: Partial<Record<Language, LLMArticle>> = {
  ...
  zh: {
    ...
    toc: [
      ...
      { label: '为什么默认RAG在超过1000个文档后会失效', anchor: '#why-defaults-break' },
      { label: '架构决策树', anchor: '#decision-tree' },
      { label: '架构对比表', anchor: '#architecture-comparison' },
      { label: '方案一：AnythingLLM 调优版（100-1k 文档）', anchor: '#option-anythingllm' },
      { label: '方案二：LlamaIndex 本地部署（1k-5k 文档）', anchor: '#option-llamaindex' },
      { label: '方案三：Ollama + ChromaDB 自定义方案（5k-10k 文档）', anchor: '#option-chromadb' },
      { label: '方案四：Ollama + Qdrant 生产级方案（10k+ 文档）', anchor: '#option-qdrant' },
      { label: '混合搜索：BM25 + 向量', anchor: '#hybrid-search' },
      { label: '重排序：Top-N 精炼阶段', anchor: '#reranking' },
      { label: '元数据过滤：应对大规模文档集', anchor: '#metadata-filtering' },
      { label: '分层检索模式', anchor: '#hierarchical-retrieval' },
      { label: '100、1000、10000 文档的实测基准', anchor: '#benchmarks' },
      { label: '存储规划与硬件要求', anchor: '#storage-hardware' },
      { label: '增量索引与去重', anchor: '#incremental-indexing' },
      { label: '大规模RAG质量监控', anchor: '#monitoring' },
    ...
    sections: {
      ...
      whyDefaultsBreak: {
        ...
        callouts: [
          {
            type: 'note',
            text: '"扩展悬崖"不是一个固定的数字。它是语料库、硬件和检索设置相互作用到足以让答案明显变差的那个点。在16 GB笔记本上，这个悬崖大约在5000个chunks处。在配备NVMe的32 GB工作站上，它会推迟到15000-20000个chunks。本文中的修复方案——混合搜索、reranking、元数据过滤——能把这个悬崖彻底抹平。',
          },
        ],
      },
      decisionTree: {
        id: 'decision-tree',
        title: '架构决策树：先按语料库大小选择',
        image: '/images/chat-with-1000-pdfs-locally-architecture-decision-en.svg',
        ...
        snippetBlocks: [
          {
            type: 'one-sentence',
            text: '与最多1000个PDF聊天的最快本地RAG方案是AnythingLLM Desktop，chunk大小设为1000、重叠200，embedding模型用nomic-embed-text-v1.5——不需要写代码，且完全运行在你自己的机器上。',
          },
          {
            type: 'plain-terms',
            text: '按文档数量选择架构：AnythingLLM适合1000个PDF以下（无需代码，拖放式）；LlamaIndex本地部署适合1000-5000个（150行Python）；自定义Ollama + ChromaDB适合5000-10000个（300-400行，加入混合搜索和reranking）；Ollama + Qdrant适合10000个以上（Docker、元数据过滤、生产级）。正确的选择是能处理你语料库规模的最简单方案——为小规模文档集过度设计架构，只会增加维护成本，不会提升答案质量。',
          },
        ],
        items: [
          '**1000个文档以下（约5000个chunks以下）：** AnythingLLM Desktop，chunk大小1000、重叠200，embedding模型用nomic-embed-text-v1.5。不需要自定义代码。搭建步骤见[30分钟入门指南](/zh/power-local-llm/local-rag-on-your-pdfs-step-by-step)。',
        ...
        callouts: [
          {
            type: 'tip',
            text: '拿不准时，从比当前语料库规模高一档的方案开始。如果你现在有800个PDF，预计每月新增200个，直接从LlamaIndex这一档起步——日后从AnythingLLM重新架构，比现在多做一档"过度设计"痛苦得多。',
          },
        ],
      },
      architectureComparison: {
        ...
        rows: [
          {
            '架构': 'AnythingLLM（默认设置）',
            '搭建复杂度': '拖放式，无需代码',
            '实测最大文档数': '~2,000个文档后检索质量下降',
            '查询p50 @ 1k文档': '~450 ms',
            '查询p50 @ 10k文档': '不可用（召回率跌破50%）',
            '最适合': '演示和小型语料库；不要用于超过500个PDF的场景',
          },
          {
            '架构': 'AnythingLLM（调优版）',
            '搭建复杂度': '无需代码；仅调整设置（1000/200 + nomic-embed-text）',
            '实测最大文档数': '~3,000个文档可稳定运行',
            '查询p50 @ 1k文档': '~310 ms',
            '查询p50 @ 10k文档': '~1.4 s，召回率~70%',
            '最适合': '100-1,000个文档，没有自定义代码的预算',
          },
          {
            '架构': 'LlamaIndex本地',
            '搭建复杂度': '~150行Python，长驻进程',
            '实测最大文档数': '~8,000个文档',
            '查询p50 @ 1k文档': '~280 ms',
            '查询p50 @ 10k文档': '~700 ms（配合分层索引）',
            '最适合': '1,000-5,000个文档，结构化检索管道',
          },
          {
            '架构': '自定义Ollama + ChromaDB',
            '搭建复杂度': '~300-400行Python，集成BM25 + reranker',
            '实测最大文档数': '~12,000个文档',
            '查询p50 @ 1k文档': '~340 ms',
            '查询p50 @ 10k文档': '~520 ms（配合混合搜索+rerank）',
            '最适合': '5,000-10,000个文档，需要混合搜索',
          },
          {
            '架构': 'Ollama + Qdrant',
            '搭建复杂度': '~500行Python，Docker，payload schema',
            '实测最大文档数': '50,000+个文档',
            '查询p50 @ 1k文档': '~310 ms',
            '查询p50 @ 10k文档': '~410 ms（原生混合搜索+过滤）',
            '最适合': '10,000+个文档，重度依赖元数据过滤',
          },
        ],
      },
      optionAnythingLLM: {
        id: 'option-anythingllm',
        title: '方案一：AnythingLLM 调优版（100-1,000 文档）',
        content:
        ...
        callouts: [
          {
            type: 'warning',
            text: 'AnythingLLM没有原生混合搜索，也没有原生reranker。超过约2000个文档后，你会看到"文档对了，chunk错了"的失败：模型引用了正确的论文，却引用了错误的段落。这个症状就是该升级到LlamaIndex这一档的信号。',
          },
        ],
      },
      optionLlamaIndex: {
        id: 'option-llamaindex',
        title: '方案二：LlamaIndex 本地部署（1,000-5,000 文档）',
        content:
      ...
      optionChromaDB: {
        id: 'option-chromadb',
        title: '方案三：Ollama + ChromaDB 自定义方案（5,000-10,000 文档）',
        content:
        ...
        items: [
          ...
          '**ChromaDB元数据字段：** 索引时为每个chunk填充`source_filename`、`page_number`、`document_type`、`author`、`year`。查询时按payload过滤（`where={"document_type": "contract"}`）能把检索空间缩小5-10倍，且不损失质量。',
        ...
        callouts: [
          {
            type: 'tip',
            text: 'BGE-reranker-v2-m3是这个规模下单项影响最大的改进。没有它，大约15-25%的情况下你会得到正确的文档但错误的chunk。加上它，这个比例降到5%以下，LLM也有了干净的依据可用。它会给查询延迟增加200-500ms，但这个代价完全值得。',
          },
        ],
      },
      optionQdrant: {
        id: 'option-qdrant',
        title: '方案四：Ollama + Qdrant 生产级方案（10,000+ 文档）',
        content:
        ...
        items: [
          ...
          '**HNSW调优：** 在50000+个向量规模下，把索引构建的`ef_construct`提高到200、`m`提高到32，查询时使用`ef=128`。默认值可用，但会用约10%的召回率换取更快的构建速度。',
          '**用于过滤的payload schema：** Qdrant把payload当作一等公民。把`author`、`document_type`、`year`、`tags`索引为keyword payload，实现亚毫秒级的预过滤。',
          '**分层检索：** 维护两个collection——`summaries`（每个文档一个向量）和`chunks`（常规chunk）。先在summary collection中路由查询，再在命中的文档ID范围内做chunk搜索。',
      ...
      hybridSearch: {
        ...
        content:
          '**纯余弦检索会漏掉依赖罕见专有名词、法条编号或特定标识符的查询。纯BM25会漏掉与原文措辞不同的查询。两者结合的效果优于单独使用其中之一，尤其是超过1000个文档之后。** 实现成本：一次额外的检索调用加一个融合步骤。',
        items: [
          '**为什么纯密集检索会失效：** embedding会低估罕见token。像"RFC 9110 section 7.4"或"MNDA-2024-0143"这样的查询，会被embedding到通用的IETF/合同类chunk附近。BM25能捕捉到精确标识符；密集检索会漏掉。',
          '**为什么纯BM25会失效：** 词汇匹配会漏掉改写后的说法。查询"我们怎么取消？"对上标题为"终止程序"的chunk，在密集空间中能匹配，但在BM25里得分为0。',
          '**倒数排名融合（RRF）是标准的合并方法：** 对出现在任一结果列表中的每个chunk，按`1/(60+rank_dense) + 1/(60+rank_bm25)`计算分数，降序排列。60是一个平滑常数；实践中30-100之间都可用。',
          '**实用配方：** 从每种方法各取top-25，通过RRF合并，取top-50送入reranker，最后取top 6-8传给LLM。这是超过1000个文档规模后的标准生产管道。',
          '**存储成本：** BM25索引很小（每10000个文档约50-150 MB），相比之下同等规模的密集索引要500 MB-2 GB。在已有的密集索引上加装BM25成本很低。',
        ],
        callouts: [
          {
            type: 'note',
      ...
      hierarchicalRetrieval: {
        ...
        items: [
          ...
          '**LlamaIndex内置了这个模式：** `DocumentSummaryIndex`加`RouterQueryEngine`就是一个30行的实现。用ChromaDB或Qdrant的自定义Python实现约80-120行。',
        ],
      },
      benchmarks: {
        id: 'benchmarks',
        title: '100、1000、10000 文档的实测基准',
        image: '/images/chat-with-1000-pdfs-locally-storage-hardware-en.svg',
        ...
        columns: ['技术栈', '指标', '@ 100 文档', '@ 1,000 文档', '@ 10,000 文档'],
        rows: [
          {
            '技术栈': 'AnythingLLM 调优版',
            '指标': '索引时间',
          ...
          },
          {
            ...
            '@ 100 文档': '~210 / 480 ms',
            '@ 1,000 文档': '~280 / 720 ms',
            '@ 10,000 文档': '~700 / 1,400 ms',
          },
          {
            '技术栈': '自定义 Ollama+ChromaDB',
            '指标': '索引时间',
            '@ 100 文档': '~2 分钟',
            '@ 1,000 文档': '~18 分钟',
            '@ 10,000 文档': '~2.8 小时',
          ...
          },
          {
            '技术栈': '自定义 Ollama+ChromaDB',
            '指标': '查询 p50 / p95',
            '@ 100 文档': '~240 / 540 ms（含rerank）',
            '@ 1,000 文档': '~340 / 760 ms',
            '@ 10,000 文档': '~520 / 1,100 ms',
          },
          {
            '技术栈': 'Ollama + Qdrant',
            '指标': '索引时间',
      ...
      incrementalIndexing: {
        ...
        callouts: [
          {
            type: 'warning',
            text: '长期运行的个人RAG系统中最常见的隐性故障是重复导入：同一篇论文以两种不同格式被加入两次，或同一个wiki页面被导出了两次。症状包括"模型反复引用同一个chunk三次"和"综合类查询的回答莫名重复"。在语料库超过1000个文档之前就加上内容哈希去重。',
          },
        ],
      },
      monitoring: {
        ...
        items: [
          ...
          '**Trulens或RAGAS**可用于自动化评估框架，两者都能本地运行并与LlamaIndex集成。人工评估30-50条查询同样可行，往往还更准确。',
      ...
      faq: {
        ...
        faqs: [
          {
            q: '默认RAG设置在多少文档规模时会失效？',
            a: '在16 GB笔记本上使用默认设置（512-token chunk、无重叠、默认embedder、top-K为4），检索质量大约从1000-2000个文档开始明显变差，超过5000个文档后就不可用了。两种失效模式是"文档对了、chunk错了"（规模扩大后top-K太窄）和索引超出RAM导致的隐性召回率下降。调优后的AnythingLLM设置（1000/200分块 + nomic-embed-text-v1.5）能把这个悬崖推到约3000个文档。超过这个规模，就需要混合搜索和reranker了。',
          },
          {
            q: '我应该使用混合搜索（BM25 + 向量）吗？',
            a: '超过1000个文档后，应该使用。纯密集检索会漏掉带有罕见专有名词、法条编号或特定标识符的查询（比如"Section 230(c)(1)"或合同的MSA编号）。纯BM25会漏掉改写后的查询。对两个top-25列表做倒数排名融合（RRF）是标准的合并方法。Qdrant和Weaviate原生支持混合搜索；ChromaDB需要外挂Whoosh或Tantivy。额外的检索成本约50-100ms；质量提升很明显。',
          },
          {
            q: '1000个PDF在embedding后需要多少存储空间？',
            a: '使用nomic-embed-text-v1.5（768维），在1000-token分块、200-token重叠的情况下，密集向量索引在磁盘上大约需要250-400 MB。如果使用混合搜索，BM25索引再加约50-150 MB；如果使用分层检索，每文档摘要再加约50-100 MB。大多数向量数据库并不存储原始PDF本身——只存提取的文本和embedding。一个10000份PDF的语料库，向量部分需要约3-5 GB，另外还要加上原始PDF本身占用的空间。',
          },
          {
            q: 'Reranking在规模扩大后有帮助吗？',
            a: '有——在5000到50000个文档之间，reranking是单项影响最大的改进。没有reranker时，"文档对了、chunk错了"的故障在这个规模下发生率约15-25%。用BGE-reranker-v2-m3对混合搜索得到的top-50候选重排后，这个比例降到5%以下。Reranker会在CPU上增加约200-500ms、在GPU上增加约80-150ms的延迟。1000个文档以下，质量提升不足以抵消延迟成本；超过5000个文档后跳过它，就是在白白丢掉可观的召回率。',
          },
          {
            q: '我该如何处理重复或近似重复的文档？',
            a: '三层去重：对原始文件字节做SHA-256（捕捉完全相同的文件）、对空白归一化后提取的纯文本做SHA-256（捕捉同一内容的不同文件格式）、用Jaccard阈值约0.85的MinHash签名（捕捉多份草稿或OCR变体这类近似重复）。在embedding之前，这三层都要在导入阶段跑一遍。跳过去重最常见的症状是"综合类查询的回答莫名重复"——同一个chunk以三个不同ID存了三份，LLM在上下文里看到了三次。',
          },
          {
            q: '我可以增量添加文档而不用全部重新索引吗？',
            a: '可以，本文涉及的每种架构都支持增量新增。ChromaDB和Qdrant通过简单的insert调用接受新chunk；LanceDB追加写入其只追加文件；LlamaIndex对这些做了封装。唯一的例外是更换embedding模型——那会强制完全重新索引，因为在同一个索引里混用两个模型版本的embedding会悄悄降低检索质量。在语料库超过5000个文档之前就选好embedder，并至少坚持使用一年。',
          },
          {
            q: '大规模文档集应该使用元数据过滤吗？',
            a: '应该——元数据过滤是规模扩大后成本最低的5-10倍加速手段。索引时为每个chunk填充`source_filename`、`page_number`、`document_type`、`author`、`year`以及任何领域特定标签。查询时，在向量搜索运行之前先按payload预过滤。在一个10000文档的语料库上，典型的过滤器能把搜索空间缩小到几百个chunk，且不损失质量。Qdrant和Weaviate对payload有一等公民级别的支持；ChromaDB和LanceDB也支持，但在超过10万个chunk时过滤执行会稍慢一些。',
          },
          {
            q: '我该如何监控大规模RAG的质量？',
            a: '建立一个小型黄金集合——30-50条人工标注的查询/答案对，涵盖事实检索、综合类、跨文档、边界情况和已知漏检查询——并在每次有意义的改动后（新的导入、更换embedder、调整chunk大小、修改提示词）重新运行。追踪检索召回率（正确的chunk是否出现在top-K中？）、生成忠实度（答案是否与chunk内容相符？）和拒答率（该说"语料库中没有"的时候系统是否这样说？）。Trulens和RAGAS可以自动化这个过程；人工评估30条查询同样可行，往往还更准确。',
          },
          {
            q: '处理10000个文档需要什么硬件？',
            a: '底线配置：32 GB系统内存，50 GB+可用NVMe SSD空间，以及一块8 GB+ VRAM的独立GPU或32 GB+统一内存的Apple Silicon。GPU/Apple Silicon主要用于加快一次性索引速度（在10000文档的索引过程中能节省1-3小时）；索引建好后，查询时的推理在CPU上也能流畅运行。SATA SSD可以接受，但会给冷查询延迟增加30-100%；机械硬盘超过约2000个文档就不可用了。RAM是最先触及的瓶颈——配合HNSW索引，一个5 GB的索引在16 GB内存下查询体验很流畅。',
          },
          {
            q: '我可以在本地部署多用户RAG吗？',
            a: '可以——在本文任意一种架构前放一个Open WebUI，或者把你的自定义Python技术栈包装成一个小型FastAPI服务。多用户改变的是运维方式（认证、按用户的文档隔离、限流，以及可选的按用户工作区），而不是检索架构本身。Open WebUI内置了认证、OAuth和基于角色的文档访问控制。对于一个10000文档语料库上的5个以上并发用户，建议索引阶段用GPU跑embedder，查询阶段的embedding则根据QPS选CPU或GPU——单个CPU embedder能稳定支撑约3-5 QPS。',
          },
        ],
      },
      relatedReading: {
        ...
        items: [
          ...
          '[30分钟在你的PDF上搭建本地RAG（Ollama + AnythingLLM）](/zh/power-local-llm/local-rag-on-your-pdfs-step-by-step) — 触及扩展悬崖之前的入门级搭建方案。',
          '[RAG详解：如何让AI答案扎根于真实数据（2026）](/zh/prompt-engineering/rag-explained) — 关于RAG本质及各个检索组件为何重要的概念性权威解读。',
          '[2026本地LLM硬件指南](/zh/local-llms/local-llm-hardware-guide-2026) — 面向10000+文档语料库的硬件选型参考。',
          '[本地强力LLM中心](/zh/power-local-llm) — 该系列的完整指南库。',
        ],
      },
    },
    schema: {
      ...
      'proficiencyLevel': 'Advanced',
      'about': [
        { '@type': 'Thing', 'name': '检索增强生成（RAG）' },
        { '@type': 'Thing', 'name': '本地RAG' },
        { '@type': 'Thing', 'name': '混合搜索' },
        { '@type': 'Thing', 'name': 'BM25' },
        { '@type': 'Thing', 'name': 'Reranking' },
        { '@type': 'Thing', 'name': '分层检索' },
        { '@type': 'Thing', 'name': 'Ollama' },
        { '@type': 'Thing', 'name': 'AnythingLLM' },
        { '@type': 'Thing', 'name': 'LlamaIndex' },
        { '@type': 'Thing', 'name': 'ChromaDB' },
