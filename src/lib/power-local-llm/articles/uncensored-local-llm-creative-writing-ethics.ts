    ...
    sections: {
      ...
      whatUncensoredMeans: {
        ...
        callouts: [
          {
            type: 'tip',
            text: '無検閲は制御不能を意味しません。無検閲モデルはプロンプト構造に従い、語数上限を守り、キャラクターの声を維持し、修正指示に応答します。違いは、未要求のコンテンツ警告を追加せず、道徳的にダークな方向を拒否せず、場面が害を描写していることを指摘するためにキャラクターを崩さないことです。フィクション執筆テンプレートの構造化プロンプトを、他のモデルと同様に使用してください――システムプロンプトとユーザープロンプトの区別は、モデルの無検閲レベルよりも重要です。その理由は[システムプロンプトとユーザープロンプトの違い](/prompt-engineering/system-prompt-vs-user-prompt-whats-the-difference?lang=ja)を参照してください。',
          },
        ],
        items: [
      ...
      workflow: {
        ...
        items: [
          ...
          '**同じプロンプトテンプレートを使用する。** [フィクション作家向けローカルLLMプロンプト](/power-local-llm/local-llm-prompts-for-fiction-writers?lang=ja)の5パート場面テンプレート、サブテキスト対話構造、キャラクター矛盾プロンプトは無検閲モデルで同一に機能します。異なるプロンプト構造は必要ありません。',
      ...
      relatedReading: {
        ...
        title: '関連読み物',
        items: [
          '[2026年創作向けベストローカルLLM](/power-local-llm/best-local-llm-creative-writing-2026?lang=ja) ― モデル比較。ナラティブ生成のモデル選択、サンプリング設定、パフォーマンスを含む。',
          '[フィクション作家向けローカルLLMプロンプト：テンプレートとテクニック](/power-local-llm/local-llm-prompts-for-fiction-writers?lang=ja) ― 場面執筆、対話、世界構築、スタイル転換のプロンプトテンプレート。標準モデルと無検閲モデルの両方で機能する。',
          '[SillyTavern vs Agnai vs RisuAI：ベストローカルロールプレイフロントエンド](/power-local-llm/sillytavern-vs-agnai-vs-risuai-roleplay?lang=ja) ― キャラクターカードロールプレイのフロントエンド比較。Ollamaバックエンドのセットアップ手順を含む。',
          '[小説執筆と脚本向けローカルLLMの使い方](/power-local-llm/local-llm-screenwriting-and-novel-drafting?lang=ja) ― 長編フィクションプロジェクトのための長編草稿ワークフローとチャプター管理。',
          '[プロンプトインジェクションとLLMセキュリティ](/prompt-engineering/prompt-injection-and-security?lang=ja) ― 敵対的プロンプトがセキュリティ層とどう相互作用するか。無検閲化が何を除去するかを理解するための有用な背景。',
          '[システムプロンプトとユーザープロンプトの違い](/prompt-engineering/system-prompt-vs-user-prompt-whats-the-difference?lang=ja) ― モデル動作を管理する権限構造。一貫した無検閲出力にはモデル選択よりシステムプロンプトの区別の方が重要。',
          '[プライベートビジネスデータのためのローカルRAG](/power-local-llm/local-rag-for-private-business-data?lang=ja) ― プライバシーに敏感なユースケースのためのローカル専用AIセットアップ。ローカルフィクション生成と同じプライバシー原則。',
        ],
      },
    },
  },
    ...
    sections: {
      ...
      whatUncensoredMeans: {
        ...
        callouts: [
          {
            type: 'tip',
            text: '无审查并不意味着无法管控。无审查模型仍然遵循提示词结构、尊重字数上限、维持角色声音，并响应修改指令。区别在于它们不会主动添加内容警告、拒绝道德上黑暗的方向，也不会为了指出某个场景描绘了伤害而打破角色。像使用其他任何模型一样使用创作模板中的结构化提示词——系统提示词与用户提示词的区分比模型的无审查程度更重要。了解原因，请参阅[系统提示词与用户提示词的区别](/prompt-engineering/system-prompt-vs-user-prompt-whats-the-difference?lang=zh)。',
          },
        ],
        items: [
      ...
      workflow: {
        ...
        items: [
          ...
          '**使用相同的提示词模板。** 来自[小说作家本地LLM提示词指南](/power-local-llm/local-llm-prompts-for-fiction-writers?lang=zh)的5部分场景模板、潜台词对话结构和角色矛盾提示词在无审查模型上运作完全相同。不需要不同的提示词结构。',
      ...
      relatedReading: {
        ...
        title: '延伸阅读',
        items: [
          '[2026年创意写作最佳本地LLM](/power-local-llm/best-local-llm-creative-writing-2026?lang=zh) — 模型对比，包括模型选择、采样设置和叙事生成性能。',
          '[小说作家本地LLM提示词指南：模板与技巧](/power-local-llm/local-llm-prompts-for-fiction-writers?lang=zh) — 场景写作、对话、世界构建和风格迁移的提示词模板，在标准和无审查模型上均有效。',
          '[SillyTavern vs Agnai vs RisuAI：最佳本地角色扮演前端](/power-local-llm/sillytavern-vs-agnai-vs-risuai-roleplay?lang=zh) — 角色卡角色扮演前端对比，包括Ollama后端配置说明。',
          '[如何用本地LLM进行剧本创作和小说起草](/power-local-llm/local-llm-screenwriting-and-novel-drafting?lang=zh) — 长篇小说项目的长篇草稿工作流和章节管理。',
          '[提示词注入与LLM安全](/prompt-engineering/prompt-injection-and-security?lang=zh) — 对抗性提示词如何与安全层交互；理解无审查移除了什么的有用背景。',
          '[系统提示词与用户提示词：有什么区别](/prompt-engineering/system-prompt-vs-user-prompt-whats-the-difference?lang=zh) — 管理模型行为的权限结构；对于一致的无审查输出，系统提示词的区分比模型选择更重要。',
          '[私有业务数据的本地RAG](/power-local-llm/local-rag-for-private-business-data?lang=zh) — 隐私敏感使用场景的纯本地AI配置；与本地创作生成相同的隐私原则。',
        ],
      },
    },
  },
