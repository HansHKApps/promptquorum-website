    ...
    quickAnswerTop: {
      en: {
        ...
        bullets: [
          ...
          'AutoGPT-local was the weakest performer in our May 2026 evaluation (Python 3.11, Ollama 0.3.x, 5 task runs) — dependency conflicts against that combo and a planning loop that drifted into circular calls in every run. This is about classic AutoGPT-local\'s unscoped-loop architecture, not project abandonment — the AutoGPT GitHub repo itself remains active; see our full [AutoGPT review](/power-local-llm/autogpt-local-review) for the current classic-vs-Platform breakdown.',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          '**One stack is unusable testing:** AutoGPT-local — dependencies did not resolve against Python 3.11/Ollama 0.3.x, the planning loop drifted into circular tool calls in all 5 evaluation runs, and no maintainer responded to open issues at time of testing. Check the GitHub repository for current activity before drawing permanent conclusions.',
      ...
      multiAgentOrchestration: {
        ...
        items: [
          ...
          '**CrewAI reliability:** we did not run a large-enough sample to quantify this ourselves; anecdotal reports from CrewAI users online cluster around a meaningful reliability gap for unattended runs — treat any specific percentage, including figures you see cited elsewhere, as directional rather than a measured benchmark. The most common silent failure is a model-name mismatch between the CrewAI config and what Ollama actually has pulled; when the Ollama server is not running, the failure surfaces on the first tool call with no earlier warning.',
      ...
      autogpt: {
        ...
        items: [
          ...
          '**For the current picture:** the project as a whole is very active (hundreds of open issues, commits landing the same week we last checked), but that activity is concentrated in the paid, hosted AutoGPT Platform, not the classic CLI agent evaluated here. See our full [AutoGPT Review 2026](/power-local-llm/autogpt-local-review) for the classic-vs-Platform split, the MIT/Polyform license difference, and how to point classic AutoGPT at Ollama if you want to experiment with it anyway.',
        ],
      },
      demosVsReality: {
        id: 'demos-vs-reality',
      ...
      relatedReading: {
        ...
        items: [
          '[Hermes Agent Review 2026](/power-local-llm/hermes-agent-review) — Nous Research\'s MIT-licensed autonomous agent with persistent memory, a self-improving skill loop, and messaging-platform access (Telegram, Discord, Slack, WhatsApp, Signal, email).',
          ...
          '[BabyAGI in 2026: What the First AI Agent Became](/power-local-llm/babyagi-review-autonomous-agent) — the history of one of the earliest viral autonomous-agent projects, and why it has no local-model support.',
    ...
    sections: {
      ...
      multiAgentOrchestration: {
        ...
        items: [
          ...
          '**Fiabilidad de CrewAI:** no realizamos una muestra lo bastante grande como para cuantificar esto nosotros mismos; los informes anecdóticos de usuarios de CrewAI en línea apuntan a una brecha de fiabilidad significativa para ejecuciones sin supervisión — trata cualquier porcentaje concreto, incluidas cifras que veas citadas en otros sitios, como orientativo y no como una medición. El fallo silencioso más común es un desajuste de nombre de modelo entre la configuración de CrewAI y lo que Ollama realmente tiene descargado; cuando el servidor de Ollama no está en marcha, el fallo aparece en la primera llamada a herramienta sin ningún aviso previo.',
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[BabyAGI en 2026: del script viral al framework de funciones](/es/power-local-llm/babyagi-review-autonomous-agent) — la historia de uno de los primeros proyectos virales de agentes autónomos, y por qué no soporta modelos locales.',
    ...
    quickAnswerTop: {
      fr: {
        ...
        bullets: [
          ...
          'AutoGPT-local a été le moins performant de notre évaluation de mai 2026 (Python 3.11, Ollama 0.3.x, 5 essais de tâches) — conflits de dépendances avec cette combinaison et une boucle de planification qui dérivait vers des appels circulaires à chaque essai. Cela concerne l\'architecture de boucle non limitée d\'AutoGPT-local classique, pas l\'abandon du projet — le dépôt GitHub d\'AutoGPT reste actif ; voir notre [revue complète d\'AutoGPT](/fr/power-local-llm/autogpt-local-review) pour le point actuel sur classic vs Platform.',
    ...
    sections: {
      ...
      multiAgentOrchestration: {
        ...
        items: [
          ...
          '**Fiabilité de CrewAI :** nous n\'avons pas mené un échantillon assez large pour quantifier cela nous-mêmes ; les retours d\'expérience d\'utilisateurs de CrewAI en ligne convergent vers un écart de fiabilité significatif pour les exécutions non surveillées — considérez tout pourcentage précis, y compris ceux cités ailleurs, comme indicatif et non comme une mesure. L\'échec silencieux le plus courant est une divergence de nom de modèle entre la config CrewAI et ce qu\'Ollama a réellement téléchargé ; quand le serveur Ollama n\'est pas lancé, l\'échec apparaît au premier appel d\'outil sans avertissement préalable.',
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[BabyAGI en 2026 : du script viral au framework de fonctions](/fr/power-local-llm/babyagi-review-autonomous-agent) — l\'histoire de l\'un des tout premiers projets viraux d\'agent autonome, et pourquoi il ne prend pas en charge les modèles locaux.',
    ...
    sections: {
      ...
      quickFacts: {
        ...
        content: [
          ...
          '**AutoGPT**: クラシック版CLIエージェントはデモでは素晴らしく見えますが、複数エージェント間の調整で失敗します。マルチタスク環境では監視コストが高くなります。これはクラシック版のアーキテクチャ上の弱点であり、プロジェクト自体が放棄されているわけではありません — GitHubリポジトリは現在も活発に更新されています。',
      ...
      autogpt: {
        ...
        content: [
          ...
          '**これはアーキテクチャの評価であり、プロジェクト放棄の話ではありません**: 上記はクラシックなAutoGPT CLIエージェントのアンスコープド・ループ・アーキテクチャに対する評価です。AutoGPTのGitHubリポジトリ自体は現在も活発に更新が続いています。運営組織はその後、有償のホスティング製品「AutoGPT Platform」への機能開発に軸足を移しており、元のCLIエージェントは `classic/` フォルダに残ってメンテナンスとセキュリティ関連のコミットは受けていますが、新機能は追加されていません。',
          '**実務推奨**: クラシック版AutoGPTは単純な探索タスク用の実験的プロトタイプに留め、実務的なワークフロー自動化には向きません。クラシック版とPlatform版の違い、ライセンスの違い、Ollamaでクラシック版AutoGPTを動かす方法については、[AutoGPTレビュー](/ja/power-local-llm/autogpt-local-review) で詳しく解説しています。',
        ],
      },
      demosVsReality: {
        title: 'なぜデモは成功するのか',
      ...
      supervisionCost: {
        ...
        content: [
          ...
          '**AutoGPT（クラシック版）**: タスク当たり平均 60 分。複数ステップの失敗では、どのステップで問題が発生したかを特定することが難しい。',
      ...
      outlook: {
        ...
        content: [
          ...
          '**予測**: 2026 年第 4 四半期までに、Cline と LangGraph が本番環境向けの「ほぼ安全」なスタックになる可能性があります。OpenInterpreter と MetaGPT は、設計上の課題（サンドボックス化なしのコード実行、過度なマルチエージェント調整オーバーヘッド）から、主流採用にはまだ時間がかかると見られます。AutoGPT については、この評価はクラシックCLIエージェントのみを指します。プロジェクト全体としては、有償のホスティング製品「AutoGPT Platform」への投資が続いており活発です。',
        ],
      },
      commonMistakes: {
        title: 'よくある間違い',
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[BabyAGI 2026：バイラルスクリプトから関数フレームワークへ](/ja/power-local-llm/babyagi-review-autonomous-agent) — 最も早期にバイラルとなった自律エージェントプロジェクトの一つの歴史と、ローカルモデルに対応していない理由。',
    ...
    quickAnswerTop: {
      de: {
        ...
        bullets: [
          ...
          'AutoGPT-local war der schwächste Performer in unserer Mai-2026-Evaluation (Python 3.11, Ollama 0.3.x, 5 Task-Durchläufe) — Abhängigkeitskonflikte mit dieser Kombination und eine Planungsschleife, die bei jedem Durchlauf in zirkuläre Calls driftete. Das betrifft die Unscoped-Loop-Architektur des klassischen AutoGPT-local, nicht die Aufgabe des Projekts — das AutoGPT-GitHub-Repo bleibt aktiv; siehe unsere vollständige [AutoGPT-Review](/de/power-local-llm/autogpt-local-review) für den aktuellen Classic-vs-Platform-Überblick.',
    ...
    sections: {
      ...
      multiAgentOrchestration: {
        ...
        items: [
          ...
          '**CrewAI-Zuverlaessigkeit:** Wir haben keine ausreichend grosse Stichprobe gefahren, um das selbst zu beziffern; anekdotische Berichte von CrewAI-Nutzern im Netz deuten auf eine deutliche Zuverlaessigkeitsluecke bei unbeaufsichtigten Durchlaeufen hin — behandeln Sie jede konkrete Prozentzahl, auch anderswo zitierte, als Richtwert, nicht als Messung. Der haeufigste stille Fehler ist eine Modellnamen-Diskrepanz zwischen der CrewAI-Konfiguration und dem, was Ollama tatsaechlich heruntergeladen hat; wenn der Ollama-Server nicht laeuft, zeigt sich der Fehler beim ersten Tool Call ohne vorherige Warnung.',
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[BabyAGI 2026: Vom viralen Skript zum Function-Framework](/de/power-local-llm/babyagi-review-autonomous-agent) — die Geschichte eines der frühesten viralen Projekte für autonome Agenten und warum es keine lokalen Modelle unterstützt.',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          '**AutoGPT-local是我们2026年5月评估中表现最差的方案（Python 3.11、Ollama 0.3.x，5次任务运行）——与该组合存在依赖冲突，规划循环在每次运行中都漂移成循环调用。这反映的是经典版AutoGPT-local无边界循环架构的问题，而非项目被放弃——AutoGPT的GitHub仓库本身仍然活跃；参见我们完整的[AutoGPT评测](/zh/power-local-llm/autogpt-local-review)，了解经典版与Platform版的最新对比。**',
      ...
      autogpt: {
        ...
        content: [
          ...
          '**这是架构层面的评价，而非项目被放弃**：以上结论针对的是经典版AutoGPT CLI代理的无边界循环架构，不代表项目本身停滞。AutoGPT的GitHub仓库目前仍在活跃更新。运营团队后续将主要功能开发精力转向了另一款付费托管产品"AutoGPT Platform"；原始CLI代理如今保留在`classic/`目录中，仍会收到维护和安全相关的提交，但不再获得新功能。',
          '**生产推荐：** 经典版AutoGPT仅适合简单探索任务的实验原型，不适合实务工作流自动化。关于经典版与Platform版的区别、许可证差异，以及如何在Ollama上运行经典版AutoGPT，请参见我们的[AutoGPT评测](/zh/power-local-llm/autogpt-local-review)。',
        ],
      },
      demosVsReality: {
        title: '为什么演示看起来更好',
      ...
      supervisionCost: {
        ...
        content: [
          ...
          '**AutoGPT（经典版）：** 每任务平均60分钟。多步失败中难以识别问题发生在哪一步。',
      ...
      outlook: {
        ...
        content: [
          ...
          '**预测：** 到2026年第4季度，Cline和LangGraph可能成为生产的"接近安全"堆栈。OpenInterpreter和MetaGPT受限于自身的设计问题（无沙箱代码执行、多代理协调开销过大），距离主流采用可能还需要更长时间。至于AutoGPT，这一评估仅针对经典版CLI代理——项目整体仍很活跃，投入持续集中在付费托管产品"AutoGPT Platform"上。',
        ],
      },
      commonMistakes: {
        title: '常见错误',
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[BabyAGI 2026：从爆红脚本到函数框架](/zh/power-local-llm/babyagi-review-autonomous-agent) — 最早爆红的自主智能体项目之一的历史，以及它为何不支持本地模型。',
    ...
    sections: {
      ...
      howWeTested: {
        ...
        items: [
          ...
          'A confiabilidade das chamadas de ferramentas é a camada subjacente a tudo isso. Para a comparação do lado do modelo, veja [Melhores modelos locais para tool calling em 2026](/pt/power-local-llm/best-local-models-tool-calling-2026); a camada de protocolo é coberta em [Conecte o Ollama a bancos de dados e APIs com MCP](/pt/power-local-llm/local-ai-agents-with-mcp-2026).',
        ],
      },
      realityTable: {
        id: 'reality-table',
      ...
      multiAgentOrchestration: {
        ...
        items: [
          ...
          '**Confiabilidade do CrewAI:** não rodamos uma amostra grande o suficiente para quantificar isso nós mesmos; relatos anedóticos de usuários do CrewAI online apontam para uma lacuna de confiabilidade significativa em execuções sem supervisão — trate qualquer percentual específico, incluindo números citados em outros lugares, como indicativo, não como medição. A falha silenciosa mais comum é uma incompatibilidade de nome de modelo entre a configuração do CrewAI e o que o Ollama realmente baixou; quando o servidor Ollama não está rodando, a falha aparece na primeira chamada de ferramenta sem aviso prévio.',
      ...
      autogpt: {
        ...
        items: [
          ...
          '**Sobre o panorama atual:** o projeto como um todo está muito ativo (centenas de issues abertas, commits chegando na mesma semana da nossa última checagem), mas essa atividade está concentrada no AutoGPT Platform pago e hospedado, não no agente CLI clássico avaliado aqui. Veja nossa [análise completa do AutoGPT 2026](/pt/power-local-llm/autogpt-local-review) para a divisão entre clássico e Platform, a diferença de licença MIT/Polyform e como apontar o AutoGPT clássico para o Ollama caso você queira experimentar mesmo assim.',
        ],
      },
      demosVsReality: {
        id: 'demos-vs-reality',
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[BabyAGI em 2026: do script viral ao framework de funções](/pt/power-local-llm/babyagi-review-autonomous-agent) — a história de um dos primeiros projetos virais de agente autônomo, e por que ele não suporta modelos locais.',
    ...
    sections: {
      ...
      multiAgentOrchestration: {
        ...
        items: [
          ...
          '**موثوقية CrewAI:** لم نُجرِ عينة كبيرة بما يكفي لقياس ذلك بأنفسنا؛ تشير تقارير غير رسمية من مستخدمي CrewAI على الإنترنت إلى فجوة موثوقية ملموسة في عمليات التشغيل دون إشراف — تعامل مع أي نسبة محددة، بما في ذلك الأرقام المذكورة في مصادر أخرى، على أنها إرشادية لا قياسية. أكثر الإخفاقات الصامتة شيوعاً هو عدم تطابق اسم النموذج بين إعدادات CrewAI وما حمّله Ollama فعلياً؛ وعندما لا يعمل خادم Ollama، يظهر الخلل عند أول استدعاء أداة دون أي تحذير سابق.',
      ...
      autogpt: {
        ...
        items: [
          ...
          '**عن الصورة الراهنة:** المشروع ككل نشط جداً (مئات المشكلات المفتوحة، والتزامات تصل في نفس الأسبوع الذي راجعناه فيه آخر مرة)، لكن هذا النشاط يتركّز في AutoGPT Platform المدفوع والمستضاف، لا في وكيل سطر الأوامر الكلاسيكي المُقيَّم هنا. راجع [مراجعتنا الكاملة لـ AutoGPT 2026](/ar/power-local-llm/autogpt-local-review) للاطلاع على الانقسام بين الكلاسيكي وPlatform، واختلاف ترخيص MIT/Polyform، وكيفية توجيه AutoGPT الكلاسيكي نحو Ollama إن أردت تجربته رغم ذلك.',
        ],
      },
      demosVsReality: {
        id: 'demos-vs-reality',
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[BabyAGI في 2026: من نص برمجي منتشر إلى إطار وظائف](/ar/power-local-llm/babyagi-review-autonomous-agent) — تاريخ أحد أوائل مشاريع العملاء المستقلين المنتشرة بسرعة، ولماذا لا يدعم النماذج المحلية.',
    ...
    sections: {
      ...
      multiAgentOrchestration: {
        ...
        items: [
          ...
          '**CrewAI 신뢰성:** 저희는 이를 직접 수치화할 만큼 충분히 큰 표본으로 테스트하지 않았습니다. CrewAI 사용자들의 온라인상 일화적 보고는 감독 없는 실행에서 상당한 신뢰성 격차가 있다는 데 몰려 있습니다 — 다른 곳에서 인용된 수치를 포함해 구체적인 퍼센트는 측정값이 아니라 참고용 방향성으로 받아들이십시오. 가장 흔한 무음 실패는 CrewAI 설정과 실제로 Ollama가 다운로드한 모델 이름 사이의 불일치입니다. Ollama 서버가 실행되지 않을 때 실패는 사전 경고 없이 첫 번째 tool 호출에서 드러납니다.',
      ...
      autogpt: {
        ...
        items: [
          ...
          '**현재 상황:** 프로젝트 전체는 매우 활발합니다(수백 개의 열린 이슈, 저희가 마지막으로 확인한 것과 같은 주에 들어오는 커밋들). 하지만 그 활동은 유료 호스팅 제품인 AutoGPT Platform에 집중되어 있으며, 여기서 평가한 클래식 CLI 에이전트에는 집중되어 있지 않습니다. 클래식과 Platform의 구분, MIT/Polyform 라이선스 차이, 그리고 그래도 실험해보고 싶다면 클래식 AutoGPT를 Ollama로 연결하는 방법은 저희의 전체 [AutoGPT 리뷰 2026](/ko/power-local-llm/autogpt-local-review)을 참고하십시오.',
        ],
      },
      demosVsReality: {
        id: 'demos-vs-reality',
      ...
      relatedReading: {
        ...
        items: [
          ...
          '[BabyAGI 2026: 바이럴 스크립트에서 함수 프레임워크로](/ko/power-local-llm/babyagi-review-autonomous-agent) — 가장 초기에 바이럴이 된 자율 에이전트 프로젝트 중 하나의 역사와, 로컬 모델을 지원하지 않는 이유.',
