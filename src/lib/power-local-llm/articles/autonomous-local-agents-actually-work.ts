  ...
  en: {
    ...
    sections: {
      ...
      relatedReading: {
        ...
        items: [
          '[Hermes Agent Review 2026](/power-local-llm/hermes-agent-review-2026) — Nous Research\'s MIT-licensed autonomous agent with persistent memory, a self-improving skill loop, and messaging-platform access (Telegram, Discord, Slack, WhatsApp, Signal, email).',
    ...
    sections: {
      ...
      autogpt: {
        ...
        content: [
          ...
          '**実務推奨**: クラシック版AutoGPTは単純な探索タスク用の実験的プロトタイプに留め、実務的なワークフロー自動化には向きません。クラシック版とPlatform版の違い、ライセンスの違い、Ollamaでクラシック版AutoGPTを動かす方法については、[AutoGPTレビュー](/power-local-llm/autogpt-local-review-2026) で詳しく解説しています。',
        ],
      },
      demosVsReality: {
        title: 'なぜデモは成功するのか',
    ...
    sections: {
      ...
      autogpt: {
        ...
        content: [
          ...
          '**生产推荐：** 经典版AutoGPT仅适合简单探索任务的实验原型，不适合实务工作流自动化。关于经典版与Platform版的区别、许可证差异，以及如何在Ollama上运行经典版AutoGPT，请参见我们的[AutoGPT评测](/power-local-llm/autogpt-local-review-2026)。',
        ],
      },
      demosVsReality: {
        title: '为什么演示看起来更好',
