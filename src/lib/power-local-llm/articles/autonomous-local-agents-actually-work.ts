    ...
    quickAnswerTop: {
      fr: {
        ...
        bullets: [
          ...
          'AutoGPT-local a été le moins performant de notre évaluation de mai 2026 (Python 3.11, Ollama 0.3.x, 5 essais de tâches) — conflits de dépendances avec cette combinaison et une boucle de planification qui dérivait vers des appels circulaires à chaque essai. Cela concerne l\'architecture de boucle non limitée d\'AutoGPT-local classique, pas l\'abandon du projet — le dépôt GitHub d\'AutoGPT reste actif ; voir notre [revue complète d\'AutoGPT](/power-local-llm/autogpt-local-review) pour le point actuel sur classic vs Platform.',
    ...
    sections: {
      ...
      autogpt: {
        ...
        content: [
          ...
          '**実務推奨**: クラシック版AutoGPTは単純な探索タスク用の実験的プロトタイプに留め、実務的なワークフロー自動化には向きません。クラシック版とPlatform版の違い、ライセンスの違い、Ollamaでクラシック版AutoGPTを動かす方法については、[AutoGPTレビュー](/power-local-llm/autogpt-local-review) で詳しく解説しています。',
        ],
      },
      demosVsReality: {
        title: 'なぜデモは成功するのか',
    ...
    quickAnswerTop: {
      de: {
        ...
        bullets: [
          ...
          'AutoGPT-local war der schwächste Performer in unserer Mai-2026-Evaluation (Python 3.11, Ollama 0.3.x, 5 Task-Durchläufe) — Abhängigkeitskonflikte mit dieser Kombination und eine Planungsschleife, die bei jedem Durchlauf in zirkuläre Calls driftete. Das betrifft die Unscoped-Loop-Architektur des klassischen AutoGPT-local, nicht die Aufgabe des Projekts — das AutoGPT-GitHub-Repo bleibt aktiv; siehe unsere vollständige [AutoGPT-Review](/power-local-llm/autogpt-local-review) für den aktuellen Classic-vs-Platform-Überblick.',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          '**AutoGPT-local是我们2026年5月评估中表现最差的方案（Python 3.11、Ollama 0.3.x，5次任务运行）——与该组合存在依赖冲突，规划循环在每次运行中都漂移成循环调用。这反映的是经典版AutoGPT-local无边界循环架构的问题，而非项目被放弃——AutoGPT的GitHub仓库本身仍然活跃；参见我们完整的[AutoGPT评测](/power-local-llm/autogpt-local-review)，了解经典版与Platform版的最新对比。**',
      ...
      autogpt: {
        ...
        content: [
          ...
          '**生产推荐：** 经典版AutoGPT仅适合简单探索任务的实验原型，不适合实务工作流自动化。关于经典版与Platform版的区别、许可证差异，以及如何在Ollama上运行经典版AutoGPT，请参见我们的[AutoGPT评测](/power-local-llm/autogpt-local-review)。',
        ],
      },
      demosVsReality: {
        title: '为什么演示看起来更好',
    ...
    sections: {
      ...
      autogpt: {
        ...
        items: [
          ...
          '**Sobre o panorama atual:** o projeto como um todo está muito ativo (centenas de issues abertas, commits chegando na mesma semana da nossa última checagem), mas essa atividade está concentrada no AutoGPT Platform pago e hospedado, não no agente CLI clássico avaliado aqui. Veja nossa [análise completa do AutoGPT 2026](/power-local-llm/autogpt-local-review) para a divisão entre clássico e Platform, a diferença de licença MIT/Polyform e como apontar o AutoGPT clássico para o Ollama caso você queira experimentar mesmo assim.',
        ],
      },
      demosVsReality: {
        id: 'demos-vs-reality',
    ...
    sections: {
      ...
      autogpt: {
        ...
        items: [
          ...
          '**عن الصورة الراهنة:** المشروع ككل نشط جداً (مئات المشكلات المفتوحة، والتزامات تصل في نفس الأسبوع الذي راجعناه فيه آخر مرة)، لكن هذا النشاط يتركّز في AutoGPT Platform المدفوع والمستضاف، لا في وكيل سطر الأوامر الكلاسيكي المُقيَّم هنا. راجع [مراجعتنا الكاملة لـ AutoGPT 2026](/power-local-llm/autogpt-local-review) للاطلاع على الانقسام بين الكلاسيكي وPlatform، واختلاف ترخيص MIT/Polyform، وكيفية توجيه AutoGPT الكلاسيكي نحو Ollama إن أردت تجربته رغم ذلك.',
        ],
      },
      demosVsReality: {
        id: 'demos-vs-reality',
    ...
    sections: {
      ...
      autogpt: {
        ...
        items: [
          ...
          '**현재 상황:** 프로젝트 전체는 매우 활발합니다(수백 개의 열린 이슈, 저희가 마지막으로 확인한 것과 같은 주에 들어오는 커밋들). 하지만 그 활동은 유료 호스팅 제품인 AutoGPT Platform에 집중되어 있으며, 여기서 평가한 클래식 CLI 에이전트에는 집중되어 있지 않습니다. 클래식과 Platform의 구분, MIT/Polyform 라이선스 차이, 그리고 그래도 실험해보고 싶다면 클래식 AutoGPT를 Ollama로 연결하는 방법은 저희의 전체 [AutoGPT 리뷰 2026](/power-local-llm/autogpt-local-review)을 참고하십시오.',
        ],
      },
      demosVsReality: {
        id: 'demos-vs-reality',
