...
export const article: Partial<Record<Language, LLMArticle>> = {
  en: {
    ...
    heroImage: '/images/local-coding-llm-without-internet-overview-hero-en.webp',
    ...
    gammaDescription: 'The slide deck below covers: a 5-component offline coding stack (local LLM, editor harness, package cache, docs mirror, local search), the 14-hour flight test results, a 6-step pre-flight checklist, model comparison (Qwen3-Coder 30B vs 7B vs DeepSeek-V3), dependency caching per language (npm/pip/cargo/go), and 6 common mistakes to avoid. Download the PDF as a fully offline coding setup reference card.',
    sections: {
      tldr: {
        id: 'key-takeaways',
        isTldr: true,
      ...
      modelChoice: {
        ...
        content:
          '**The model is the biggest disk and memory line item; pick once, pick correctly.** Three reasonable choices in August 2026, ranked by how well they handle offline coding work specifically.',
        items: [
          ...
          '**DeepSeek-V3 — choose this if you need very long contexts.** DeepSeek\'s V3 supports 128K tokens; useful when you are debugging across many files in one prompt. Larger on disk (~25 GB at Q4_K_M); roughly equivalent to Qwen3-Coder 30B on raw quality.',
          ...
          '**Skip:** general-purpose models under 13B without a coding fine-tune (Llama 3.2 3B, Qwen3 8B), and any quantization harsher than Q4_K_M. Both fail in obvious ways on real coding work.',
        ...
        imageCaption: 'Local coding models for offline work: Qwen3-Coder 30B Q4_K_M (~18 GB, 24 GB RAM) is the recommended default; Qwen2.5-Coder 7B (~5 GB, 8 GB RAM) is the lightweight fallback at 80–85% quality; DeepSeek-V3 (~25 GB) for 128K-context workflows; Codestral 22B for fastest autocomplete.',
      },
      dependencyCache: {
        id: 'dependency-cache',
      ...
      whatBreaks: {
        ...
        items: [
          ...
          '**The model\'s knowledge of post-cutoff APIs.** Qwen3-Coder\'s training cutoff was October 2025 (August 2026 release); APIs released after that are at best guessed. The fix: pre-clone the source and `rg` for the real signature when in doubt. Never trust the model for libraries newer than its training data.',
      ...
      commonMistakes: {
        ...
        items: [
          ...
          '**Mistake 4: using a model under 7B for serious coding work.** Sub-7B coding models miss enough that you spend more time fixing their output than writing code. Drop to Qwen2.5-Coder 7B at the smallest; if your hardware cannot handle that, the offline coding setup is not viable on this laptop.',
  ...
  es: {
    ...
    next_refresh_due: '2027-02-27',
    theme: 'Coding Assistants',
    heroImage: '/images/local-coding-llm-without-internet-overview-hero-es.webp',
    ...
    sections: {
      ...
      modelChoice: {
        ...
        content:
          '**El modelo es la partida más grande de disco y memoria; elígelo bien desde el principio.** Tres opciones razonables en agosto de 2026, ordenadas por qué tan bien gestionan el trabajo de programación offline específicamente.',
        items: [
          ...
          '**DeepSeek-V3 — elige este si necesitas contextos muy largos.** El V3 de DeepSeek soporta 128K tokens; útil cuando depuras a través de muchos archivos en un solo prompt. Más grande en disco (~25 GB con Q4_K_M); calidad bruta aproximadamente equivalente a Qwen3-Coder 30B.',
          ...
          '**Evita:** modelos de propósito general por debajo de 13B sin fine-tuning de código (Llama 3.2 3B, Qwen3 8B) y cualquier cuantización más agresiva que Q4_K_M. Ambos fallan de forma obvia en trabajo de programación real.',
        ...
        imageCaption: 'Modelos locales de código para trabajo offline: Qwen3-Coder 30B Q4_K_M (~18 GB, 24 GB RAM) es el predeterminado recomendado; Qwen2.5-Coder 7B (~5 GB, 8 GB RAM) es el respaldo ligero con 80–85% de calidad; DeepSeek-V3 (~25 GB) para flujos de trabajo con contextos de 128K; Codestral 22B para el autocompletado más rápido.',
      },
      dependencyCache: {
        id: 'dependency-cache',
  ...
  pt: {
    ...
    next_refresh_due: '2027-02-27',
    theme: 'Coding Assistants',
    heroImage: '/images/local-coding-llm-without-internet-overview-hero-pt.webp',
    ...
    gammaEmbedUrl: '/presentations/local-coding-llm-without-internet-static.html',
    gammaDescription: 'A apresentação de slides abaixo cobre: uma stack de programação offline de 5 componentes (LLM local, integração de editor, cache de pacotes, espelho de documentação, busca local), os resultados do teste do voo de 14 horas, um checklist pré-voo de 6 passos, comparação de modelos (Qwen3-Coder 30B vs 7B vs DeepSeek-V3), cache de dependências por linguagem e 6 erros comuns. Baixe o PDF como cartão de referência para a configuração offline.',
    sections: {
      tldr: {
        id: 'key-takeaways',
        isTldr: true,
      ...
      modelChoice: {
        ...
        content:
          '**O modelo é a maior parcela de disco e memória; escolha bem desde o início.** Três opções razoáveis em agosto de 2026, ordenadas por quão bem gerenciam o trabalho de programação offline especificamente.',
        items: [
          ...
          '**DeepSeek-V3 — escolha este se você precisa de contextos muito longos.** O V3 da DeepSeek suporta 128K tokens; útil quando você depura através de muitos arquivos em um único prompt. Maior em disco (~25 GB com Q4_K_M); qualidade bruta aproximadamente equivalente ao Qwen3-Coder 30B.',
          ...
          '**Evite:** modelos de propósito geral abaixo de 13B sem fine-tuning de código (Llama 3.2 3B, Qwen3 8B) e qualquer quantização mais agressiva que Q4_K_M. Ambos falham de forma óbvia em trabalho de programação real.',
        ...
        image: '/images/local-coding-llm-model-choice-en.svg',
        imageCaption: 'Modelos locais de código para trabalho offline: Qwen3-Coder 30B Q4_K_M (~18 GB, 24 GB RAM) é o padrão recomendado; Qwen2.5-Coder 7B (~5 GB, 8 GB RAM) é a reserva leve com 80–85% de qualidade; DeepSeek-V3 (~25 GB) para fluxos de trabalho com contextos de 128K; Codestral 22B para o autocompletar mais rápido.',
      },
      dependencyCache: {
        id: 'dependency-cache',
      ...
      whatBreaks: {
        ...
        items: [
          ...
          '**O conhecimento do modelo sobre APIs lançadas após a sua data de corte.** A data de corte de treinamento do Qwen3-Coder foi outubro de 2025 (lançamento em agosto de 2026); as APIs lançadas depois são, no melhor caso, estimativas. A solução: pré-clone o código-fonte e use `rg` para encontrar a assinatura real quando tiver dúvidas. Nunca confie no modelo para bibliotecas mais novas que os seus dados de treinamento.',
  ...
  de: {
    ...
    next_refresh_due: '2027-02-27',
    theme: 'Coding Assistants',
    heroImage: '/images/local-coding-llm-without-internet-overview-hero-de.webp',
    ...
    current_models_mentioned: ['Qwen3-Coder 30B', 'Qwen2.5-Coder 7B', 'DeepSeek-V3', 'Codestral 22B', 'nomic-embed-text v1.5'],
    ...
    sections: {
      ...
      modelChoice: { id: 'model-choice', title: 'Wahl des richtigen lokalen Modells für Offline-Arbeit', content: '**Das Modell ist der größte Festplatte und Speicher-Zeilenelement; wählen Sie einmal, wählen Sie richtig.** Drei vernünftige Optionen im August 2026, geordnet nach ihrer Handhabung von Offline-Codierungsarbeit.', items: ['**Qwen3-Coder 30B Q4_K_M (~18 GB) – der empfohlene Standard.** Best-in-class bei TypeScript, Python, Rust und Go Autocomplete; zuverlässige Tool-Aufrufe; handhabt 32K-Token-Kontexte. Benötigt 24 GB verfügbaren Speicher (System-RAM auf Apple Silicon, VRAM auf diskreten GPUs).', '**Qwen2.5-Coder 7B Q4_K_M (~5 GB) – der leichte Fallback.** Läuft auf 8 GB Unified RAM oder 8 GB VRAM. Etwa 80–85 % der 30B-Qualität auf alltäglicher Arbeit; die Lücke zeigt sich bei Multi-Step-Umgestaltungen und langkontextigen Überlegungen. Die richtige Wahl, wenn Ihr Laptop weniger als 24 GB Speicher hat oder wenn Sie möchten, dass das Modell mit schweren Docker-Workloads koexistiert.', '**DeepSeek-V3 – wählen Sie dies, wenn Sie sehr lange Kontexte benötigen.** DeepSeeks V3 unterstützt 128K Token; nützlich, wenn Sie über viele Dateien in einem Prompt debuggen. Größer auf der Festplatte (~25 GB in Q4_K_M); ungefähr gleichwertig mit Qwen3-Coder 30B in Raw-Qualität.', '**Codestral 22B – die Geschwindigkeit. Schnelleres Autocomplete als Qwen3-Coder 30B; schwächer bei Tool-Aufrufen und Multi-Step-Plänen. Gut, wenn Ihr Offline-Workflow Autocomplete-dominiert ist und Sie keine Agent-Systeme verwenden.', '**Überspringen:** Allzweck-Modelle unter 13B ohne Coding-Fine-Tune (Llama 3.2 3B, Qwen3 8B) und jede Quantisierung härter als Q4_K_M. Beide schlagen bei echter Codierungsarbeit offensichtlich fehl.', '**Für den vollständigen Coding-Modell-Vergleich** einschließlich HumanEval+ Scores pro Sprache, siehe [Best Local Coding Models in 2026: Qwen3-Coder vs DeepSeek vs Codestral](/de/power-local-llm/best-local-coding-models-2026).'] },
  ...
  fr: {
    ...
    next_refresh_due: '2027-02-27',
    theme: 'Coding Assistants',
    heroImage: '/images/local-coding-llm-without-internet-overview-hero-fr.webp',
    ...
    gammaEmbedUrl: '/presentations/local-coding-llm-without-internet-static.html',
    gammaDescription: 'Le diaporama ci-dessous couvre : une pile de codage hors ligne à 5 composants (LLM local, intégration éditeur, cache de paquets, miroir de docs, recherche locale), les résultats du test de vol de 14 heures, une liste de contrôle pré-vol en 6 étapes, la comparaison des modèles (Qwen3-Coder 30B vs 7B vs DeepSeek-V3), la mise en cache des dépendances par langage et 6 erreurs courantes. Télécharger le PDF comme carte de référence pour la configuration hors ligne.',
    sections: {
  ...
  ja: {
    ...
    next_refresh_due: '2027-02-27',
    theme: 'Coding Assistants',
    heroImage: '/images/local-coding-llm-without-internet-overview-hero-ja.webp',
    ...
    gammaEmbedUrl: '/presentations/local-coding-llm-without-internet-static.html',
    gammaDescription: '下のスライドデッキは以下をカバーします：5コンポーネントのオフラインコーディングスタック（ローカルLLM、エディタ統合、パッケージキャッシュ、ドキュメントミラー、ローカル検索）、14時間フライトテストの結果、6ステップのフライト前チェックリスト、モデル比較（Qwen3-Coder 30B vs 7B vs DeepSeek-V3）、言語ごとの依存関係キャッシング、6つのよくある間違い。PDFをオフラインコーディングセットアップリファレンスカードとしてダウンロード。',
    sections: {
      ...
      modelChoice: { id: 'model-choice', title: 'オフライン作業の正しい地元のモデルを選ぶ', content: '**モデルは最も大きいディスクとメモリライン項目；1回選ぶ、正しく選ぶ。** 2026年8月の3つの合理的な選択肢、オフラインコーディング作業を処理する方法によってランク付け。', items: ['**Qwen3-Coder 30B Q4_K_M（約18GB）– 推奨デフォルト。** TypeScript、Python、Rust、Go オートコンプリートのベストインクラス；信頼できるツール呼び出し；32K トークンコンテキストを処理。利用可能メモリの 24GB が必要（Apple Silicon システムRAM、ディスクレートGPU VRAM）。', '**Qwen2.5-Coder 7B Q4_K_M（約5GB）– 軽量フォールバック。** 8GB 統一 RAM または 8GB VRAM で実行。日常の仕事で 30B 品質の約80～85％；ギャップはマルチステップリファクタリングおよび長文脈推論に表示されます。ラップトップが 24GB 未満メモリを持つ、または重いdockerワークロードと共存する場合の正しい選択。', '**DeepSeek-V3 – 非常に長いコンテキストが必要な場合これを選択。** DeepSeek-V3 は 128K トークン サポート；多くのファイル間でデバッグが有用。ディスク上より大きい（Q4_K_M での約25GB）；生品質で Qwen3-Coder 30B と大体同等。', '**Codestral 22B – 速度ピック。** Qwen3-Coder 30B より高速オートコンプリート；ツール呼び出しおよびマルチステッププランで弱い。オフラインワークフロー Autocompletion が支配的で、エージェントハーネスを使用しない場合良い。', '**スキップ：** コーディング ファイン チューンなしで 13B 未満の汎用モデル（Llama 3.2 3B、Qwen3 8B）および Q4_K_M より厳しい任意の量子化。両方は実際のコーディング作業で明白に失敗。', '**言語毎 HumanEval+ スコアを含む完全なコーディングモデル比較のため、** [Best Local Coding Models in 2026: Qwen3-Coder vs DeepSeek vs Codestral](/ja/power-local-llm/best-local-coding-models-2026) を参照してください。'] },
  ...
  zh: {
    ...
    next_refresh_due: '2027-02-27',
    theme: 'Coding Assistants',
    heroImage: '/images/local-coding-llm-without-internet-overview-hero-zh.webp',
    ...
    gammaEmbedUrl: '/presentations/local-coding-llm-without-internet-static.html',
    gammaDescription: '下方幻灯片涵盖：5个组件的离线编码堆栈（本地LLM、编辑器集成、包缓存、文档镜像、本地搜索）、14小时飞行测试结果、6步离飞前检查清单、模型比较（Qwen3-Coder 30B vs 7B vs DeepSeek-V3）、各语言依赖缓存和6个常见错误。下载PDF作为完全离线编码设置参考卡。',
    sections: {
      ...
      modelChoice: { id: 'model-choice', title: '为离线工作选择正确的本地模型', content: '**模型是最大的磁盘和内存线项；选择一次、正确选择。** 2026年8月的三个合理选择，按他们处理离线编码工作的良好程度排名。', items: ['**Qwen3-Coder 30B Q4_K_M（约18GB）– 推荐默认。** TypeScript、Python、Rust、Go自动完成的同类最佳；可靠的工具调用；处理32K令牌上下文。需要24GB可用内存（Apple Silicon系统RAM、离散GPU VRAM）。', '**Qwen2.5-Coder 7B Q4_K_M（约5GB）– 轻量级后备。** 在8GB统一RAM或8GB VRAM上运行。日常工作的30B质量约80～85%；差距显示在多步重构和长上下文推理。如果你的笔记本具有少于24GB内存或者你想让模型与重型Docker工作负载共存的正确选择。', '**DeepSeek-V3 – 如果你需要非常长的上下文选择这个。** DeepSeek的V3支持128K令牌；在一个提示中跨许多文件调试时有用。在磁盘上更大（Q4_K_M时约25GB）；原始质量大致等于Qwen3-Coder 30B。', '**Codestral 22B – 速度选择。** 比Qwen3-Coder 30B更快的自动完成；在工具调用和多步计划上较弱。如果你的离线工作流是自动完成主导的且你不使用代理工具的好处。', '**跳过：** 无编码微调的13B以下通用模型（Llama 3.2 3B、Qwen3 8B）和任何比Q4_K_M更严格的量化。两个在实际编码工作上明显失败。', '**对于包括每种语言HumanEval+分数的完整编码模型比较，** 参阅[Best Local Coding Models in 2026: Qwen3-Coder vs DeepSeek vs Codestral](/zh/power-local-llm/best-local-coding-models-2026)。'] },
      ...
      whatBreaks: { id: 'what-breaks', title: '什么真的在离线时破裂（诚实列表）', content: '**设置是genuinely稳健，但五件东西仍然失败。** 提前知道故障模式让你周围工作。', items: ['**安装全新第三方包。** 无缓存命中、无后备除手动源供应。修复是预防性的 – 预缓存任何你可能想要、包括拉伸目标。', '**模型知识关于后期截止API。** Qwen3-Coder训练截止是2025年10月（2026年8月发布）；在那之后发布的API最多猜测。修复：克隆源、疑问时`rg`真实签名。从不信任模型关于比其训练数据更新的库。', '**任何需要OAuth或API认证往返的事。** 登入云提供商、交换OAuth令牌、点击你的团队SSO门户 – 这些都离线不工作。修复：离起前做所有认证且依赖缓存令牌（通常12～24小时后过期）。', '**遥远服务的浏览器基测试。** 如果你的测试点击真实API或staging环境，它们离线失败。修复：使用本地模拟（msw、nock、vcr）且预记录fixtures。', '**调用外部服务的图像和资产生成。** 云基图像生成器、字体服务、CDN取回资产都失败。修复：将固定资产烤进repo或使用完全本地图像模型（那是独立堆栈）。', '**"那个库叫什么"问题的修复是模型本身。** 当你无法Google搜索时，问模型"X功能的包名是什么" – 对于它训练数据中的东西它80～90%正确回答。安装前针对包缓存检查。'] },
  ...
  ar: {
    ...
    next_refresh_due: '2027-02-27',
    theme: 'Coding Assistants',
    heroImage: '/images/local-coding-llm-without-internet-overview-hero-ar.webp',
    ...
    gammaEmbedUrl: '/presentations/local-coding-llm-without-internet-static.html',
    gammaDescription: 'يغطي عرض الشرائح أدناه: إعداد برمجة دون اتصال مكوّن من 5 عناصر (نموذج LLM محلي، وتكامل مع المحرر، وذاكرة تخزين مؤقت للحزم، ومرآة توثيق، وبحث محلي)، ونتائج اختبار رحلة 14 ساعة، وقائمة مراجعة مكوّنة من 6 خطوات ما قبل الرحلة، ومقارنة النماذج (Qwen3-Coder 30B مقابل 7B مقابل DeepSeek-V3)، وتخزين التبعيات مؤقتاً حسب اللغة، و6 أخطاء شائعة يجب تجنبها. نزّل ملف PDF كبطاقة مرجعية لإعداد البرمجة دون اتصال.',
    sections: {
      tldr: {
        id: 'key-takeaways',
        isTldr: true,
      ...
      modelChoice: {
        ...
        content:
          '**النموذج هو أكبر بند في القرص والذاكرة؛ اختر مرة واحدة واختر بصورة صحيحة.** ثلاثة خيارات معقولة في أغسطس 2026، مرتبة حسب مدى تعاملها مع أعمال البرمجة دون اتصال تحديداً.',
        items: [
          ...
          '**DeepSeek-V3 — اخترهُ إن احتجت سياقات طويلة جداً.** يدعم V3 من DeepSeek 128K رمز؛ مفيد عند تصحيح الأخطاء عبر ملفات كثيرة في أمر واحد. أكبر حجماً على القرص (نحو 25 غيغابايت عند Q4_K_M)؛ جودة خام مماثلة تقريباً لـ Qwen3-Coder 30B.',
          ...
          '**تجنّب:** النماذج العامة دون الضبط الدقيق للبرمجة وأقل من 13B (Llama 3.2 3B، وQwen3 8B)، وأي ضغط أشد من Q4_K_M. كلاهما يفشل بشكل واضح في أعمال البرمجة الحقيقية.',
        ...
        image: '/images/local-coding-llm-model-choice-en.svg',
        imageCaption: 'نماذج البرمجة المحلية للعمل دون اتصال: Qwen3-Coder 30B Q4_K_M (نحو 18 غيغابايت، 24 غيغابايت ذاكرة) هو الافتراضي الموصى به؛ Qwen2.5-Coder 7B (نحو 5 غيغابايت، 8 غيغابايت ذاكرة) هو البديل الخفيف بنسبة 80–85٪ من الجودة؛ DeepSeek-V3 (نحو 25 غيغابايت) لسير العمل بسياقات 128K؛ Codestral 22B لأسرع إكمال تلقائي.',
      },
      dependencyCache: {
        id: 'dependency-cache',
      ...
      whatBreaks: {
        ...
        items: [
          ...
          '**معرفة النموذج بواجهات API صدرت بعد تاريخ الانقطاع.** تاريخ انقطاع تدريب Qwen3-Coder كان أكتوبر 2025 (الإصدار أغسطس 2026)؛ الـ API الصادرة بعده في أحسن الأحوال تخمين. الحل: استنسخ المصدر واستخدم `rg` للتوقيع الفعلي عند الشك. لا تثق بالنموذج أبداً للمكتبات الأحدث من بيانات تدريبه.',
      ...
      commonMistakes: {
        ...
        items: [
          ...
          '**الخطأ 4: استخدام نموذج أقل من 7B لأعمال البرمجة الجادة.** نماذج البرمجة دون 7B تخطئ بما يكفي لأن تقضي وقتاً أطول في تصحيح مخرجاتها من كتابة الكود. انزل إلى Qwen2.5-Coder 7B على أقل تقدير؛ إن لم يستطع جهازك ذلك، فإعداد البرمجة دون اتصال غير مجدٍ على هذا الكمبيوتر المحمول.',
  ...
  ko: {
    ...
    next_refresh_due: '2027-02-27',
    theme: 'Coding Assistants',
    heroImage: '/images/local-coding-llm-without-internet-overview-hero-ko.webp',
    ...
    gammaEmbedUrl: '/presentations/local-coding-llm-without-internet-static.html',
    gammaDescription: '슬라이드 프레젠테이션에서 다루는 내용: 5개 구성 요소의 오프라인 코딩 스택(로컬 LLM, 에디터 통합, 패키지 캐시, 문서 미러, 로컬 검색), 14시간 비행 테스트 결과, 6단계 비행 전 체크리스트, 모델 비교(Qwen3-Coder 30B vs 7B vs DeepSeek-V3), 언어별 의존성 캐싱, 6가지 흔한 실수. PDF를 오프라인 설정 참고 카드로 다운로드하십시오.',
    sections: {
      tldr: {
        id: 'key-takeaways',
        isTldr: true,
      ...
      modelChoice: {
        ...
        content:
          '**모델은 디스크와 메모리에서 가장 큰 항목입니다; 처음부터 올바르게 선택하십시오.** 2026년 8월 기준으로 특히 오프라인 코딩 작업을 얼마나 잘 처리하는지 순서대로 정리한 세 가지 합리적인 옵션입니다.',
        items: [
          ...
          '**Qwen2.5-Coder 7B Q4_K_M (~5 GB) — 경량 백업.** 통합 메모리 또는 VRAM 8 GB로 실행됩니다. 일상적인 작업에서 30B 품질의 약 80–85%; 다단계 리팩토링과 긴 컨텍스트 추론에서 차이가 납니다. 노트북 메모리가 24 GB 미만이거나 무거운 Docker 워크로드와 함께 모델을 사용하려는 경우의 올바른 선택.',
          '**DeepSeek-V3 — 매우 긴 컨텍스트가 필요한 경우 선택.** DeepSeek V3는 128K 토큰을 지원합니다; 단일 프롬프트에서 많은 파일에 걸쳐 디버깅할 때 유용합니다. 디스크에서 더 큽니다(Q4_K_M으로 ~25 GB); 원시 품질은 Qwen3-Coder 30B와 대략 동등합니다.',
          '**Codestral 22B — 속도 옵션.** Qwen3-Coder 30B보다 빠른 자동 완성; 도구 호출과 다단계 계획에서 약함. 오프라인 워크플로우가 자동 완성 위주이고 에이전트를 사용하지 않는다면 좋은 선택.',
          '**피해야 할 것:** 코드 파인튜닝 없이 13B 미만의 범용 모델(Llama 3.2 3B, Qwen3 8B)과 Q4_K_M보다 더 공격적인 양자화. 둘 다 실제 코딩 작업에서 명백하게 실패합니다.',
        ...
        image: '/images/local-coding-llm-model-choice-es.svg',
        imageCaption: '오프라인 작업을 위한 로컬 코딩 모델: Qwen3-Coder 30B Q4_K_M (~18 GB, RAM 24 GB)이 기본 권장; Qwen2.5-Coder 7B (~5 GB, RAM 8 GB)는 80–85% 품질의 경량 백업; DeepSeek-V3 (~25 GB)는 128K 컨텍스트 워크플로우용; Codestral 22B는 가장 빠른 자동 완성용.',
      },
      dependencyCache: {
        id: 'dependency-cache',
      ...
      whatBreaks: {
        ...
        items: [
          ...
          '**학습 데이터 마감 이후 출시된 API에 대한 모델 지식.** Qwen3-Coder의 학습 데이터 마감일은 2025년 10월입니다(2026년 8월 출시); 그 이후에 출시된 API는 기껏해야 추측입니다. 해결책: 소스 코드를 미리 클론하고 의심스러울 때 `rg`를 사용해 실제 시그니처를 찾으십시오. 학습 데이터보다 최신의 라이브러리에 대해 모델을 신뢰하지 마십시오.',
      ...
      commonMistakes: {
        ...
        items: [
          ...
          '**실수 4: 진지한 코딩 작업에 7B 미만 모델 사용.** 7B 미만 코딩 모델은 출력을 수정하는 데 코드를 직접 작성하는 것보다 더 많은 시간을 소비할 만큼 충분히 실패합니다. 최소한 Qwen2.5-Coder 7B로 내려가십시오; 하드웨어가 그것도 처리할 수 없다면 그 노트북에서는 오프라인 코딩 설정이 실용적이지 않습니다.',
