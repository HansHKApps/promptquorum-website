...
export const article: Partial<Record<Language, PromptBiteArticle>> = {
  en: {
    ...
    current_models_mentioned: ['Llama 3.2 1B', 'Llama 3.2 3B', 'Qwen2.5 1.5B'],
    ...
    leadAnswerBlock: '<strong>Yes — a Raspberry Pi 5 runs a local LLM today, and Ollama is the easiest way to do it: the 8GB board handles 1B-3B models like Llama 3.2 1B, Llama 3.2 3B, or Qwen2.5 1.5B on its CPU alone.</strong> For real GenAI acceleration and larger models, add the official Raspberry Pi AI HAT+ 2 — its Hailo-10H NPU is purpose-built for on-device LLM inference and ships with its own Hailo Ollama server.',
    toc: [
  ...
  ar: {
    ...
    sections: {
      ...
      bestPick: {
        ...
        content: [
          '<strong>يشغّل جهاز Raspberry Pi 5 العاري (8GB) نماذج بمعاملات 1B-3B بتكميم Q4 عبر Ollama أو llama.cpp — نماذج مثل Llama 3.2 1B وLlama 3.2 3B وQwen2.5 1.5B.</strong> كل شيء يعمل على المعالج رباعي النوى: وحدة الرسومات VideoCore المدمجة في Pi 5 ليست مسرّعًا عمليًا لـ llama.cpp، لذا لا توجد زيادة سرعة تُذكر من GPU على اللوح العاري.',
  ...
  es: {
    ...
    siblingBites: ['best-local-llm-6gb-vram', 'how-much-ram-for-7b-model'],
    is_living_page: false,
    leadAnswerBlock: '<strong>Sí — una Raspberry Pi 5 ejecuta un LLM local hoy mismo, y Ollama es la forma más fácil de hacerlo: la placa de 8GB maneja modelos de 1B-3B como Llama 3.2 1B, Llama 3.2 3B o Qwen2.5 1.5B usando solo la CPU.</strong> Para aceleración GenAI real y modelos más grandes, añade la Raspberry Pi AI HAT+ 2 oficial — su NPU Hailo-10H está diseñada específicamente para inferencia de LLM en el dispositivo e incluye su propio servidor Hailo Ollama.',
    toc: [
  ...
  ja: {
    ...
    siblingBites: ['best-local-llm-6gb-vram', 'how-much-ram-for-7b-model'],
    is_living_page: false,
    leadAnswerBlock: '<strong>はい — Raspberry Pi 5は今日ローカルLLMを動かせます。最も簡単な方法はOllamaです。8GBボードはLlama 3.2 1B、Llama 3.2 3B、Qwen2.5 1.5BのようなモデルをCPUだけで扱えます。</strong>本格的なGenAIアクセラレーションとより大きなモデルには、公式のRaspberry Pi AI HAT+ 2を追加してください — そのHailo-10H NPUはオンデバイスLLM推論向けに作られており、専用のHailo Ollamaサーバーを搭載しています。',
    toc: [
    ...
    sections: {
      ...
      bestPick: {
        ...
        content: [
          '<strong>素のRaspberry Pi 5(8GB)は、Ollamaやllama.cpp経由でQ4量子化の1B-3Bパラメータモデル — Llama 3.2 1B、Llama 3.2 3B、Qwen2.5 1.5Bなど — を動かせます。</strong>すべてクアッドコアCPUで実行されます。Pi 5の統合VideoCore GPUは実用的なllama.cppアクセラレータではないため、素のボードでは意味のあるGPU高速化はありません。',
  ...
  ko: {
    ...
    siblingBites: ['best-local-llm-6gb-vram', 'how-much-ram-for-7b-model'],
    is_living_page: false,
    leadAnswerBlock: '<strong>예 — Raspberry Pi 5는 오늘날 로컬 LLM을 구동할 수 있으며, Ollama가 가장 쉬운 방법입니다: 8GB 보드는 Llama 3.2 1B, Llama 3.2 3B, Qwen2.5 1.5B 같은 모델을 CPU만으로 처리합니다.</strong> 진짜 GenAI 가속과 더 큰 모델을 위해서는 공식 Raspberry Pi AI HAT+ 2를 추가하십시오 — 이 제품의 Hailo-10H NPU는 온디바이스 LLM 추론을 위해 특별히 설계되었으며 자체 Hailo Ollama 서버를 탑재하고 있습니다.',
    toc: [
    ...
    sections: {
      ...
      bestPick: {
        ...
        content: [
          '<strong>순정 Raspberry Pi 5(8GB)는 Ollama나 llama.cpp를 통해 Q4 양자화된 1B-3B 파라미터 모델 — Llama 3.2 1B, Llama 3.2 3B, Qwen2.5 1.5B 같은 모델 — 을 구동합니다.</strong> 모든 것이 쿼드코어 CPU에서 실행됩니다. Pi 5의 통합 VideoCore GPU는 실용적인 llama.cpp 가속기가 아니므로, 순정 보드에서는 의미 있는 GPU 속도 향상이 없습니다.',
  ...
  pt: {
    ...
    sections: {
      ...
      bestPick: {
        ...
        content: [
          '<strong>Um Raspberry Pi 5 puro (8GB) roda modelos de 1B-3B parâmetros com quantização Q4 através do Ollama ou llama.cpp — modelos como Llama 3.2 1B, Llama 3.2 3B ou Qwen2.5 1.5B.</strong> Tudo roda na CPU quad-core: a GPU VideoCore integrada do Pi 5 não é um acelerador prático para o llama.cpp, então não há ganho relevante de GPU na placa pura.',
  ...
  zh: {
    ...
    siblingBites: ['best-local-llm-6gb-vram', 'how-much-ram-for-7b-model'],
    is_living_page: false,
    leadAnswerBlock: '<strong>可以——Raspberry Pi 5 今天就能运行本地 LLM，Ollama 是最简单的方式：8GB 主板仅靠 CPU 就能运行 Llama 3.2 1B、Llama 3.2 3B 或 Qwen2.5 1.5B 等模型。</strong>若需要真正的 GenAI 加速和更大的模型，可加装官方 Raspberry Pi AI HAT+ 2——其 Hailo-10H NPU 专为设备端 LLM 推理打造，并自带 Hailo Ollama 服务器。',
    toc: [
    ...
    sections: {
      ...
      bestPick: {
        ...
        content: [
          '<strong>裸机 Raspberry Pi 5（8GB）可通过 Ollama 或 llama.cpp 运行 Q4 量化的 1B-3B 参数模型——如 Llama 3.2 1B、Llama 3.2 3B 或 Qwen2.5 1.5B。</strong>一切都在四核 CPU 上运行：Pi 5 集成的 VideoCore GPU 并非实用的 llama.cpp 加速器，因此裸机主板上没有明显的 GPU 加速效果。',
