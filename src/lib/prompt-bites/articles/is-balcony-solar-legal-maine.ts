  ...
  fr: {
    ...
    sections: {
      ...
      body1: {
        ...
        content: [
          '**La loi LD 1730 du Maine est la seule des 8 lois sur le solaire de balcon adoptées à prévoir un plafond de puissance à deux niveaux selon qui installe le système.** Un résident peut autoinstaller un appareil jusqu\'à 420 W sans électricien agréé. Pour atteindre le plafond standard de 1 200 W appliqué dans la plupart des autres États, un électricien agréé doit réaliser le raccordement. Cette répartition bricolage/professionnel n\'existe dans aucune des 7 autres lois d\'État de cette série — c\'est la particularité du Maine.',
  ...
  ja: {
    ...
    sections: {
      ...
      body1: {
        ...
        content: [
          '**メイン州のLD 1730は、法制化された8つのバルコニーソーラー法の中で唯一、誰がシステムを設置するかによってワット数の上限が2段階に分かれています。** 住民は有資格電気技師なしで420Wまでのデバイスをセルフインストールできます。他の多くの州で採用されている標準の1,200Wの上限まで引き上げるには、有資格電気技師が接続を行う必要があります。このDIY/プロ施工の区分は、このシリーズの他の7州の法律には見られない、メイン州独自の特徴です。',
      ...
      body2: {
        ...
        content: [
          '**メイン州を含む、法制化されたすべての州の法律が参照する安全認証規格UL 3700は、本記事執筆時点で、完全なプラグイン型ソーラーシステムでの完全取得はまだ確認されていません。** この規格は2025年12月に公開され、認証テストは2026年1月に開始されました。テストには数か月かかるため、「設置は合法」と「認証済み製品が入手可能」の間のギャップは、メイン州固有の問題ではなく、8州すべてに共通するパターンです。EcoFlowのSTREAM Ultraが市場に最も近い製品ですが、現在は全国展開ではなく、ユタ州限定で販売されています。',
  ...
  zh: {
    ...
    sections: {
      ...
      body1: {
        ...
        content: [
          '**缅因州LD 1730是已立法的8项阳台太阳能法案中,唯一根据安装者身份设置两级功率上限的法案。**居民可在无需持证电工的情况下自行安装最高420W的设备。若要达到大多数其他州采用的标准1200W上限,必须由持证电工完成接线。这种DIY/专业安装的区分在本系列其他7个州的法律中均未出现——是缅因州的独特之处。',
      ...
      body2: {
        title: '认证套件目前是否真的能买到?',
        content: [
          '**UL 3700是包括缅因州在内所有已立法州法律都参照的安全认证标准,截至撰稿时,尚未有完整的插接式太阳能系统完全获得该认证。**该标准于2025年12月发布,认证测试于2026年1月启动——测试需要数月时间,因此"合法安装"与"认证产品上市"之间存在差距,这是所有8个州的共同现象,并非缅因州特有的问题。EcoFlow STREAM Ultra是最接近上市的产品,但目前仅在犹他州有售,尚未面向全国销售。',
  ...
  ko: {
    ...
    sections: {
      ...
      body1: {
        ...
        content: [
          '**메인주 LD 1730은 법제화된 8개 발코니 태양광 법 가운데 유일하게 시스템 설치 주체에 따라 와트 상한을 2단계로 구분합니다.** 주민은 면허 전기기사 없이 최대 420W까지 기기를 자가 설치할 수 있습니다. 대부분의 다른 주에서 사용하는 표준 상한인 1,200W까지 올리려면 면허 전기기사가 연결 작업을 수행해야 합니다. 이러한 DIY/전문 시공 구분은 이 시리즈의 다른 7개 주 법률 어디에도 존재하지 않으며, 메인주만의 특징입니다.',
