...
export const article: Partial<Record<Language, PromptBiteArticle>> = {
  en: {
    ...
    current_models_mentioned: ['Llama 3.1 8B', 'Qwen3 14B', 'Qwen3 32B', 'Qwen3-30B-A3B', 'gpt-oss-120b'],
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          'Speed on Ryzen AI Max 395: Llama 3.1 8B ~22 tok/s, Qwen3 14B ~13 tok/s, Qwen3 32B ~7 tok/s via Vulkan',
      ...
      body1: {
        ...
        rows: [
          ...
          { 'Model': 'Qwen3 14B', 'VRAM at Q4_K_M': '9.3 GB', 'Speed (MAX 395 Vulkan)': '~13 tok/s', 'Fits 96 GB?': '✓' },
          { 'Model': 'Qwen3 32B', 'VRAM at Q4_K_M': '19.4 GB', 'Speed (MAX 395 Vulkan)': '~7 tok/s', 'Fits 96 GB?': '✓' },
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          'Geschwindigkeit auf Ryzen AI Max 395: Llama 3.1 8B ~22 tok/s, Qwen3 14B ~13 tok/s, Qwen3 32B ~7 tok/s via Vulkan',
      ...
      body1: {
        ...
        rows: [
          ...
          { 'Modell': 'Qwen3 14B', 'VRAM bei Q4_K_M': '9,3 GB', 'Geschwindigkeit (MAX 395 Vulkan)': '~13 tok/s', 'Passt in 96 GB?': '✓' },
          { 'Modell': 'Qwen3 32B', 'VRAM bei Q4_K_M': '19,4 GB', 'Geschwindigkeit (MAX 395 Vulkan)': '~7 tok/s', 'Passt in 96 GB?': '✓' },
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          'Vitesse sur Ryzen AI Max 395 : Llama 3.1 8B ~22 tok/s, Qwen3 14B ~13 tok/s, Qwen3 32B ~7 tok/s via Vulkan',
      ...
      body1: {
        ...
        rows: [
          ...
          { 'Modèle': 'Qwen3 14B', 'VRAM à Q4_K_M': '9,3 Go', 'Vitesse (MAX 395 Vulkan)': '~13 tok/s', 'Entre dans 96 Go ?': '✓' },
          { 'Modèle': 'Qwen3 32B', 'VRAM à Q4_K_M': '19,4 Go', 'Vitesse (MAX 395 Vulkan)': '~7 tok/s', 'Entre dans 96 Go ?': '✓' },
      ...
      body2: {
        ...
        content: [
          ...
          'L\'architecture du modèle compte autant que sa taille pour la vitesse réelle. Les modèles à mélange d\'experts (MoE) comme Qwen 3 30B-A3B tournent à environ 86 tok/s sur Strix Halo Vulkan — nettement plus vite qu\'un modèle dense de taille similaire comme Qwen3 32B (~7 tok/s), car seule une fraction des paramètres s\'active par token. GPT-OSS 120B, également MoE, tourne à environ 55 tok/s malgré une taille totale bien plus grande. Si la vitesse interactive compte plus que faire tenir le plus grand modèle dense possible, un modèle MoE est généralement le meilleur choix pour ce matériel.',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          'Ryzen AI Max 395での速度：Llama 3.1 8B ~22 tok/s、Qwen3 14B ~13 tok/s、Qwen3 32B ~7 tok/s（Vulkan経由）',
      ...
      body1: {
        ...
        rows: [
          ...
          { 'モデル': 'Qwen3 14B', 'Q4_K_M時のVRAM': '9.3 GB', '速度（MAX 395 Vulkan）': '~13 tok/s', '96 GBに収まるか？': '✓' },
          { 'モデル': 'Qwen3 32B', 'Q4_K_M時のVRAM': '19.4 GB', '速度（MAX 395 Vulkan）': '~7 tok/s', '96 GBに収まるか？': '✓' },
      ...
      body2: {
        ...
        content: [
          ...
          'Strix Haloの実用的なユースケースは、クラウドAPIなしで32B〜70Bモデルをローカルで実行することです。Qwen3 32B（Q4_K_M、~19 GB）は~7 tok/sで動作します — インタラクティブなチャットには遅いですが、バッチ要約、文書処理、または一晩のファインチューニング作業には十分です。Llama 3.3 70B（Q4_K_M、~41 GB）は~3 tok/sで実行可能で、高品質な単発クエリに適しています。',
          ...
          'モデルアーキテクチャは、サイズと同じくらい実際の速度に影響します。Qwen 3 30B-A3BのようなMixture-of-Experts（MoE）モデルは、Strix Halo Vulkan上で約86 tok/sで動作します——同程度のサイズの密なモデルであるQwen3 32B（約7 tok/s）よりはるかに高速です。これはトークンごとに一部のパラメータしか活性化しないためです。同じくMoEのGPT-OSS 120Bも、総サイズがはるかに大きいにもかかわらず約55 tok/sで動作します。可能な限り大きな密なモデルを詰め込むことよりインタラクティブな速度が重要なら、このハードウェアではMoEモデルの方が通常より適しています。',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          'Ryzen AI Max 395 的速度：Llama 3.1 8B ~22 tok/s，Qwen3 14B ~13 tok/s，Qwen3 32B ~7 tok/s（通过 Vulkan）',
      ...
      body1: {
        ...
        rows: [
          ...
          { '模型': 'Qwen3 14B', 'Q4_K_M 时 VRAM': '9.3 GB', '速度（MAX 395 Vulkan）': '~13 tok/s', '能否放入 96 GB？': '✓' },
          { '模型': 'Qwen3 32B', 'Q4_K_M 时 VRAM': '19.4 GB', '速度（MAX 395 Vulkan）': '~7 tok/s', '能否放入 96 GB？': '✓' },
      ...
      body2: {
        ...
        content: [
          ...
          'Strix Halo 的实际使用场景是在不依赖云 API 的情况下在本地运行 32B–70B 模型。Qwen3 32B（Q4_K_M，~19 GB）以 ~7 tok/s 运行——对于交互式聊天较慢，但适用于批量摘要、文档处理或整夜微调任务。Llama 3.3 70B（Q4_K_M，~41 GB）可以 ~3 tok/s 运行，适合高质量单次查询。',
          ...
          '模型架构对实际速度的影响和模型大小一样重要。像 Qwen 3 30B-A3B 这样的混合专家（MoE）模型在 Strix Halo Vulkan 上运行速度约为 86 tok/s——比同等规模的密集模型（如 Qwen3 32B，约 7 tok/s）快得多，因为每个 token 只激活一小部分参数。同样是 MoE 架构的 GPT-OSS 120B，尽管总体规模大得多，运行速度也约为 55 tok/s。如果交互速度比塞入尽可能大的密集模型更重要，MoE 模型通常更适合这类硬件。',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          'Velocidade no Ryzen AI Max 395: Llama 3.1 8B ~22 tok/s, Qwen3 14B ~13 tok/s, Qwen3 32B ~7 tok/s via Vulkan',
      ...
      body1: {
        ...
        rows: [
          ...
          { 'Modelo': 'Qwen3 14B', 'VRAM em Q4_K_M': '9,3 GB', 'Velocidade (MAX 395 Vulkan)': '~13 tok/s', 'Cabe em 96 GB?': '✓' },
          { 'Modelo': 'Qwen3 32B', 'VRAM em Q4_K_M': '19,4 GB', 'Velocidade (MAX 395 Vulkan)': '~7 tok/s', 'Cabe em 96 GB?': '✓' },
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          'Velocidad en Ryzen AI Max 395: Llama 3.1 8B ~22 tok/s, Qwen3 14B ~13 tok/s, Qwen3 32B ~7 tok/s vía Vulkan',
      ...
      body1: {
        ...
        rows: [
          ...
          { 'Modelo': 'Qwen3 14B', 'VRAM en Q4_K_M': '9.3 GB', 'Velocidad (MAX 395 Vulkan)': '~13 tok/s', '¿Cabe en 96 GB?': '✓' },
          { 'Modelo': 'Qwen3 32B', 'VRAM en Q4_K_M': '19.4 GB', 'Velocidad (MAX 395 Vulkan)': '~7 tok/s', '¿Cabe en 96 GB?': '✓' },
      ...
      body2: {
        ...
        content: [
          ...
          'La arquitectura del modelo importa tanto como el tamaño para la velocidad real. Los modelos de mezcla de expertos (MoE) como Qwen 3 30B-A3B funcionan a unos 86 tok/s en Strix Halo Vulkan — mucho más rápido que un modelo denso de tamaño similar como Qwen3 32B (~7 tok/s), porque solo se activa una fracción de los parámetros por token. GPT-OSS 120B, también MoE, funciona a unos 55 tok/s pese a su tamaño total mucho mayor. Si la velocidad interactiva importa más que encajar el modelo denso más grande posible, un modelo MoE suele ser la mejor opción para este hardware.',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          'السرعة على Ryzen AI Max 395: Llama 3.1 8B ~22 tok/s، Qwen3 14B ~13 tok/s، Qwen3 32B ~7 tok/s عبر Vulkan',
      ...
      body1: {
        ...
        rows: [
          ...
          { 'الطراز': 'Qwen3 14B', 'VRAM بتحديد Q4_K_M': '9.3 GB', 'السرعة (MAX 395 Vulkan)': '~13 tok/s', 'هل يتناسب مع 96 GB؟': '✓' },
          { 'الطراز': 'Qwen3 32B', 'VRAM بتحديد Q4_K_M': '19.4 GB', 'السرعة (MAX 395 Vulkan)': '~7 tok/s', 'هل يتناسب مع 96 GB؟': '✓' },
      ...
      body2: {
        ...
        content: [
          ...
          'الحالة الاستخدامية العملية لـ Strix Halo هي تشغيل نماذج 32B–70B محليًا بدون واجهات API سحابية. يعمل Qwen3 32B بـ Q4_K_M (~19 GB) بـ ~7 tok/s — بطيء للدردشة التفاعلية، لكن كافٍ للتلخيص الدفعي ومعالجة المستندات أو مهام الضبط الدقيق الليلية. يمكن الوصول إلى Llama 3.3 70B بـ Q4_K_M (~41 GB) بـ ~3 tok/s، مناسب للاستعلامات الفردية عالية الجودة.',
          ...
          'تؤثر بنية النموذج على السرعة الفعلية بقدر تأثير الحجم. تعمل نماذج خليط الخبراء (MoE) مثل Qwen 3 30B-A3B بسرعة تبلغ نحو 86 رمزًا/ثانية على Strix Halo عبر Vulkan — أسرع بكثير من نموذج كثيف بحجم مماثل مثل Qwen3 32B (~7 رموز/ثانية)، لأن جزءًا فقط من المعاملات يُفعَّل لكل رمز. يعمل GPT-OSS 120B، وهو أيضًا من نوع MoE، بسرعة نحو 55 رمزًا/ثانية رغم حجمه الإجمالي الأكبر بكثير. إذا كانت السرعة التفاعلية أهم من استيعاب أكبر نموذج كثيف ممكن، فإن نموذج MoE عادةً ما يكون الخيار الأفضل لهذا الجهاز.',
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          'Ryzen AI Max 395 속도: Llama 3.1 8B ~22 tok/s, Qwen3 14B ~13 tok/s, Qwen3 32B ~7 tok/s (Vulkan 기준)',
      ...
      body1: {
        ...
        rows: [
          ...
          { '모델': 'Qwen3 14B', 'Q4_K_M VRAM': '9.3 GB', '속도 (MAX 395 Vulkan)': '~13 tok/s', '96 GB 적합 여부': '✓' },
          { '모델': 'Qwen3 32B', 'Q4_K_M VRAM': '19.4 GB', '속도 (MAX 395 Vulkan)': '~7 tok/s', '96 GB 적합 여부': '✓' },
      ...
      body2: {
        ...
        content: [
          ...
          'Strix Halo의 실용적인 사용 사례는 클라우드 API 없이 로컬에서 32B–70B 모델을 실행하는 것입니다. Q4_K_M의 Qwen3 32B (~19 GB)는 ~7 tok/s로 실행됩니다. 대화형 채팅에는 느리지만 일괄 요약, 문서 처리 또는 야간 파인튜닝 작업에는 유효합니다. Q4_K_M의 Llama 3.3 70B (~41 GB)는 ~3 tok/s로 도달 가능하며 고품질 단일 쿼리에 적합합니다.',
          ...
          '모델 아키텍처는 크기만큼이나 실제 속도에 중요합니다. Qwen 3 30B-A3B와 같은 전문가 혼합(MoE) 모델은 Strix Halo Vulkan에서 약 86 tok/s로 실행됩니다 — 토큰당 파라미터의 일부만 활성화되기 때문에, 비슷한 크기의 밀집 모델인 Qwen3 32B(약 7 tok/s)보다 훨씬 빠릅니다. 역시 MoE인 GPT-OSS 120B는 전체 크기가 훨씬 큼에도 약 55 tok/s로 실행됩니다. 가능한 가장 큰 밀집 모델을 넣는 것보다 대화형 속도가 더 중요하다면, 이 하드웨어에는 대개 MoE 모델이 더 적합합니다.',
