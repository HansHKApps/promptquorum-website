...
export const article: Partial<Record<Language, PromptBiteArticle>> = {
  en: {
    ...
    metaDescription: 'Llama 3.1 8B and Qwen 3 14B support 128K+ context on Ollama. Qwen 3 reaches 1M tokens. Full context increases VRAM significantly. Updated 2026.',
    publishDate: '2026-05-18',
    freshness_tier: 'semi_annual',
    next_refresh_due: '2026-11-18',
    current_models_mentioned: ['Llama 3.1 8B', 'Qwen 3 14B'],
    ...
    quickAnswerTop: {
      en: {
        ...
        answer: 'Llama 3.1 8B supports 128K context on Ollama. Qwen 3 14B reaches 1M tokens. Note: running full context dramatically increases VRAM — a 128K window needs 3–4× more VRAM than the default 4K window.',
        bullets: [
      ...
      de: { question: 'Welche Ollama-Modelle unterstützen 128K-Kontext?', answer: 'Llama 3.1 8B unterstützt 128K-Kontext in Ollama. Qwen 3 14B erreicht 1M Token. Hinweis: Das Ausführen mit vollem Kontext erhöht den VRAM drastisch — ein 128K-Fenster benötigt 3–4× mehr VRAM als das Standard-4K-Fenster.', bullets: ['Llama 3.1 8B: 128K-Kontext, ~16 GB VRAM bei vollem Kontext', 'Qwen 3 14B: bis zu 1M Token, 24+ GB VRAM bei vollem Kontext', 'Für normalen Einsatz --num-ctx 4096 setzen, um VRAM zu sparen'], updatedDate: '2026-05' },
      fr: { question: 'Quels modèles Ollama supportent le contexte 128K ?', answer: 'Llama 3.1 8B prend en charge un contexte de 128K sur Ollama. Qwen 3 14B atteint 1M de tokens. Attention : exécuter le contexte complet augmente fortement la VRAM — une fenêtre de 128K nécessite 3 à 4× plus de VRAM que la fenêtre par défaut de 4K.', bullets: ['Llama 3.1 8B : contexte 128K, ~16 Go de VRAM au contexte complet', 'Qwen 3 14B : jusqu\'à 1M de tokens, 24+ Go de VRAM au contexte complet', 'Utiliser --num-ctx 4096 pour un usage normal et économiser la VRAM'], updatedDate: '2026-05' },
      ja: { question: 'Ollamaで128Kコンテキスト対応モデルは？', answer: 'Llama 3.1 8BはOllamaで128Kコンテキストに対応しています。Qwen 3 14Bは1Mトークンに達します。注意：フルコンテキストで実行するとVRAMが大幅に増加します — 128Kウィンドウはデフォルト4Kの3〜4倍のVRAMが必要です。', bullets: ['Llama 3.1 8B：128Kコンテキスト、フルコンテキストで~16 GB VRAM', 'Qwen 3 14B：最大1Mトークン、フルコンテキストで24+ GB VRAM', '通常使用は--num-ctx 4096でVRAMを節約'], updatedDate: '2026-05' },
      zh: { question: 'Ollama上哪些模型支持128K上下文？', answer: 'Llama 3.1 8B在Ollama上支持128K上下文。Qwen 3 14B可达1M token。注意：运行完整上下文会大幅增加显存占用——128K窗口比默认4K窗口需要多3–4倍显存。', bullets: ['Llama 3.1 8B：128K上下文，完整上下文需~16 GB显存', 'Qwen 3 14B：最多1M token，完整上下文需24+ GB显存', '日常使用设置--num-ctx 4096以节省显存'], updatedDate: '2026-05' },
      es: { question: '¿Qué modelos de Ollama admiten contexto de 128K?', answer: 'Llama 3.1 8B admite contexto de 128K en Ollama. Qwen 3 14B alcanza 1M de tokens. Nota: ejecutar el contexto completo aumenta drásticamente la VRAM — una ventana de 128K necesita entre 3 y 4 veces más VRAM que la ventana predeterminada de 4K.', bullets: ['Llama 3.1 8B: contexto de 128K, ~16 GB de VRAM con contexto completo', 'Qwen 3 14B: hasta 1M de tokens, 24+ GB de VRAM con contexto completo', 'Establece --num-ctx 4096 para uso normal y ahorra VRAM'], updatedDate: '2026-05' },
    },
    sections: {
      tldr: {
        id: 'key-takeaways',
        ...
        items: [
          ...
          'Llama 3.1 8B and Qwen 3 14B are the two models that deliver reliable quality at full 128K',
  ...
  de: {
    ...
    metaDescription: 'Llama 3.1 8B und Qwen 3 14B unterstützen 128K+ Kontext auf Ollama. Qwen 3 erreicht 1M Tokens. Voller Kontext erhöht den VRAM stark. Stand Mai 2026.',
    publishDate: '2026-05-18',
    ...
    quickAnswerTop: {
      de: {
        ...
        answer: 'Llama 3.1 8B unterstützt 128K Kontext auf Ollama. Qwen 3 14B erreicht 1M Tokens. Hinweis: Voller Kontext erhöht den VRAM-Bedarf erheblich — ein 128K-Fenster benötigt 3–4× mehr VRAM als das Standard-4K-Fenster.',
        bullets: [
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          'Llama 3.1 8B und Qwen 3 14B sind die zwei Modelle, die bei vollem 128K zuverlässige Qualität bieten',
      ...
      body1: {
        ...
        content: [
          ...
          'Zwei Modelle halten bei vollem 128K Kontext auf Ollama zuverlässig die Qualität: <strong>Llama 3.1 8B</strong> (nativ auf 128K trainiert) und <strong>Qwen 3 14B</strong> (bis zu 1M Tokens, wobei VRAM-Beschränkungen 128K zur praktischen Consumer-Grenze machen). Bei den meisten anderen 7B-Modellen nimmt die Ausgabequalität oberhalb von 32K Tokens merklich ab.',
          'Wenn Ihre Aufgabe Dokumente mit mehr als 20.000 Wörtern umfasst, beginnen Sie mit Llama 3.1 8B. Wenn Sie die stärkste Long-Context-Qualität benötigen und 12+ GB VRAM haben, ist Qwen 3 14B die bessere Wahl.',
        ],
      },
      body2: {
  ...
  fr: {
    ...
    metaDescription: 'Llama 3.1 8B et Qwen 3 14B supportent 128K+ contexte sur Ollama. Qwen 3 atteint 1M tokens. Le contexte plein augmente le VRAM. Vérifié mai 2026.',
    publishDate: '2026-05-18',
    dateModified: '2026-06-02',
    freshness_tier: 'semi_annual',
    next_refresh_due: '2026-11-18',
    quickAnswerTop: {
      fr: {
        ...
        answer: 'Llama 3.1 8B supporte le contexte 128K sur Ollama. Qwen 3 14B atteint 1M de tokens. Attention : le contexte complet augmente fortement la VRAM — une fenêtre 128K nécessite 3–4× plus de VRAM que la fenêtre par défaut de 4K.',
        bullets: [
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          'Llama 3.1 8B et Qwen 3 14B sont les deux modèles qui délivrent une qualité fiable à 128K complet',
      ...
      body1: {
        ...
        content: [
          ...
          'Si votre tâche implique des documents de plus de 20 000 mots, commencez par Llama 3.1 8B. Si vous avez besoin de la meilleure qualité de contexte long et disposez de 12+ GB de VRAM, Qwen 3 14B est le meilleur choix.',
        ],
      },
      body2: {
  ...
  ja: {
    ...
    quickAnswerTop: {
      ja: {
        ...
        answer: 'Llama 3.1 8BはOllamaで128Kコンテキストに対応。Qwen 3 14Bは1Mトークンに到達。注意：フルコンテキストの実行はVRAMを大幅に増加させます——128Kウィンドウはデフォルトの4Kウィンドウの3〜4倍のVRAMが必要です。',
        bullets: [
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          'Llama 3.1 8BとQwen 3 14Bは128Kフルコンテキストで安定した品質を提供する2つのモデル',
      ...
      body1: {
        ...
        content: [
          ...
          'Ollamaで128Kフルコンテキストの品質を安定して維持するモデルは2つ：<strong>Llama 3.1 8B</strong>（128Kでネイティブ訓練）と<strong>Qwen 3 14B</strong>（最大1Mトークン、ただしVRAM制約により128Kが実用的なコンシューマー上限）。他の7Bモデルの大部分は、32Kトークン以上では出力品質が著しく低下します。',
          '20,000語以上の文書を扱うタスクにはLlama 3.1 8Bから始めてください。最高の長文コンテキスト品質が必要で12+ GB VRAMがある場合は、Qwen 3 14Bが優れた選択肢です。',
        ],
      },
      body2: {
  ...
  zh: {
    ...
    metaDescription: 'Llama 3.1 8B和Qwen 3 14B在Ollama上支持128K以上上下文。Qwen 3可达1M个token。完整上下文大幅增加VRAM需求。2026年5月更新。',
    publishDate: '2026-05-18',
    dateModified: '2026-06-02',
    freshness_tier: 'semi_annual',
    next_refresh_due: '2026-11-18',
    quickAnswerTop: {
      zh: {
        ...
        answer: 'Llama 3.1 8B在Ollama上支持128K上下文。Qwen 3 14B可达1M个token。注意：运行完整上下文会大幅增加VRAM——128K窗口比默认4K窗口需要多3–4倍VRAM。',
        bullets: [
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          'Llama 3.1 8B和Qwen 3 14B是能在完整128K下稳定输出高质量结果的两个模型',
      ...
      body1: {
        ...
        content: [
          ...
          '在Ollama上能在128K完整上下文下稳定保持质量的模型有两个：<strong>Llama 3.1 8B</strong>（原生在128K下训练）和<strong>Qwen 3 14B</strong>（最多1M token，但VRAM限制使128K成为消费级实用上限）。对于大多数其他7B模型，输出质量在超过32K token后会明显下降。',
          '如果您的任务涉及超过20,000字的文档，从Llama 3.1 8B开始。如果需要最强的长上下文质量且有12+ GB VRAM，Qwen 3 14B是更好的选择。',
        ],
      },
      body2: {
        title: '长上下文的VRAM成本',
  ...
  pt: {
    ...
    metaDescription: 'Llama 3.1 8B e Qwen 3 14B suportam 128K+ de contexto no Ollama; Qwen 3 chega a 1M de tokens. O contexto completo aumenta muito a VRAM. 2026.',
    publishDate: '2026-05-18',
    dateModified: '2026-06-02',
    freshness_tier: 'semi_annual',
    next_refresh_due: '2026-11-18',
    quickAnswerTop: {
      pt: {
        ...
        answer: 'Llama 3.1 8B suporta contexto de 128K no Ollama. Qwen 3 14B chega a 1M de tokens. Nota: executar o contexto completo aumenta drasticamente a VRAM — uma janela de 128K precisa de 3 a 4 vezes mais VRAM que a janela padrão de 4K.',
        bullets: [
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          'Llama 3.1 8B e Qwen 3 14B são os dois modelos que oferecem qualidade confiável no máximo de 128K',
      ...
      body1: {
        ...
        content: [
          ...
          'Dois modelos mantêm de forma confiável a qualidade no contexto completo de 128K no Ollama: <strong>Llama 3.1 8B</strong> (nativamente treinado a 128K) e <strong>Qwen 3 14B</strong> (até 1M de tokens, embora as restrições de VRAM tornem 128K o limite prático para o consumidor). Para a maioria dos outros modelos 7B, a qualidade de saída se degrada notavelmente acima de 32K tokens.',
          'Se sua tarefa envolve documentos com mais de 20.000 palavras, comece com Llama 3.1 8B. Se precisar da melhor qualidade de contexto longo e tiver 12+ GB de VRAM, Qwen 3 14B é a melhor opção.',
        ],
      },
      body2: {
  ...
  es: {
    ...
    seoTitle: 'Ollama contexto 128K 2026 | Prompt Bites | PromptQuorum',
    metaDescription: 'Llama 3.1 8B y Qwen 3 14B admiten 128K de contexto o más en Ollama; Qwen 3 llega a 1M de tokens. El contexto completo aumenta mucho la VRAM. 2026.',
    publishDate: '2026-05-18',
    dateModified: '2026-06-02',
    freshness_tier: 'semi_annual',
    next_refresh_due: '2026-11-18',
    quickAnswerTop: {
      es: {
        ...
        answer: 'Llama 3.1 8B admite contexto de 128K en Ollama. Qwen 3 14B alcanza 1M de tokens. Nota: ejecutar el contexto completo aumenta drásticamente la VRAM — una ventana de 128K necesita entre 3 y 4 veces más VRAM que la ventana predeterminada de 4K.',
        bullets: [
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          'Llama 3.1 8B y Qwen 3 14B son los dos modelos que ofrecen calidad fiable al máximo de 128K',
      ...
      body1: {
        ...
        content: [
          ...
          'Si tu tarea implica documentos de más de 20.000 palabras, empieza con Llama 3.1 8B. Si necesitas la máxima calidad de contexto largo y tienes 12 GB o más de VRAM, Qwen 3 14B es la mejor opción.',
        ],
      },
      body2: {
  ...
  ar: {
    ...
    metaDescription: '⁨Llama 3.1 8B⁩ يدعم سياق ⁨128K⁩ على ⁨Ollama⁩؛ ⁨Qwen 3 14B⁩ يصل إلى مليون رمز. نافذة ⁨128K⁩ تستهلك ⁨3⁩–⁨4⁩ أضعاف ⁨VRAM⁩ مقارنةً بالإعداد الافتراضي ⁨4K⁩.',
    publishDate: '2026-05-18',
    dateModified: '2026-06-20',
    freshness_tier: 'semi_annual',
    next_refresh_due: '2026-11-18',
    quickAnswerTop: {
      ar: {
        ...
        answer: 'يدعم Llama 3.1 8B سياقاً بحجم 128K في Ollama. يصل Qwen 3 14B إلى مليون رمز. ملاحظة: تشغيل السياق الكامل يزيد VRAM بشكل كبير — نافذة بحجم 128K تحتاج إلى 3 إلى 4 أضعاف الـ VRAM مقارنةً بالنافذة الافتراضية البالغة 4K.',
        bullets: [
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          'Llama 3.1 8B وQwen 3 14B هما النموذجان اللذان يوفران جودة موثوقة عند الحد الأقصى 128K',
      ...
      body1: {
        ...
        content: [
          ...
          'نموذجان يحافظان بشكل موثوق على الجودة مع السياق الكامل 128K في Ollama: <strong>Llama 3.1 8B</strong> (مدرب بشكل أصلي على 128K) و<strong>Qwen 3 14B</strong> (حتى مليون رمز، وإن كانت قيود VRAM تجعل 128K هو الحد العملي للمستهلك). لمعظم النماذج الأخرى 7B، تتراجع جودة المخرجات بشكل ملحوظ فوق 32K رمز.',
          'إذا كانت مهمتك تتضمن مستندات تزيد عن 20,000 كلمة، ابدأ بـ Llama 3.1 8B. إذا كنت بحاجة إلى أعلى جودة للسياق الطويل ولديك 12 GB أو أكثر من VRAM، فـ Qwen 3 14B هو الخيار الأفضل.',
        ],
      },
      body2: {
  ...
  ko: {
    ...
    metaDescription: 'Llama 3.1 8B와 Qwen 3 14B는 Ollama에서 128K 이상의 컨텍스트를 지원합니다. Qwen 3는 1M 토큰에 도달합니다. 전체 컨텍스트 실행 시 VRAM이 크게 증가합니다. 2026년 업데이트.',
    publishDate: '2026-05-18',
    ...
    quickAnswerTop: {
      ko: {
        ...
        answer: 'Llama 3.1 8B는 Ollama에서 128K 컨텍스트를 지원합니다. Qwen 3 14B는 1M 토큰에 도달합니다. 주의: 전체 컨텍스트 실행 시 VRAM이 크게 증가합니다 — 128K 윈도우는 기본 4K 윈도우보다 3~4배 더 많은 VRAM이 필요합니다.',
        bullets: [
    ...
    sections: {
      tldr: {
        ...
        items: [
          ...
          'Llama 3.1 8B와 Qwen 3 14B는 전체 128K에서 안정적인 품질을 제공하는 두 가지 모델입니다',
      ...
      body1: {
        ...
        snippetBlocks: [
          { type: 'one-sentence', text: '2026년 5월 기준, Ollama에서 전체 128K 컨텍스트 품질을 안정적으로 유지하는 모델은 Llama 3.1 8B와 Qwen 3 14B 두 가지입니다.' },
          { type: 'plain-terms', text: 'Llama 3.1 8B는 128K 컨텍스트로 네이티브 학습된 모델이며, Qwen 3 14B는 최대 1M 토큰까지 지원하지만 소비자 환경에서는 VRAM 제약으로 인해 128K가 실질적인 한계입니다. 대부분의 다른 7B 모델은 32K 토큰 이상에서 출력 품질이 눈에 띄게 저하됩니다.' },
        ],
        content: [
          ...
          'Ollama에서 전체 128K 컨텍스트로 안정적으로 품질을 유지하는 모델은 두 가지입니다: <strong>Llama 3.1 8B</strong>(128K로 네이티브 학습)와 <strong>Qwen 3 14B</strong>(최대 1M 토큰이지만 VRAM 제약으로 소비자에게는 128K가 실질적 한계). 대부분의 다른 7B 모델의 경우, 32K 토큰 이상에서 출력 품질이 눈에 띄게 저하됩니다.',
          '20,000단어 이상의 문서를 처리하는 작업이라면 Llama 3.1 8B부터 시작하십시오. 최고의 긴 컨텍스트 품질이 필요하고 12 GB 이상의 VRAM을 보유하고 있다면, Qwen 3 14B가 더 나은 선택입니다.',
        ],
      },
      body2: {
