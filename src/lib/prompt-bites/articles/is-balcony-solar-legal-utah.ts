  ...
  ja: {
    ...
    sections: {
      ...
      body1: {
        ...
        content: [
          '**ユタ州のHB 340は「小型ポータブルソーラー発電装置」を、出力1,200ワット以下で、標準120V ACコンセント経由で接続し、全米的に認知された試験機関の認証を受けた機器と定義しています。** この定義を満たす機器は、電力会社との系統連系契約、承認要件、手数料のいずれからも免除されます — 電力会社への通知すら一切不要です。その代わり、この機器はネットメータリングの対象外となり、電力網に輸出された余剰電力への対価は発生しません。州最大の電力会社であるRocky Mountain Powerは、Raymond Ward州議会議員が安全基準の文言について直接協議した後、この法案に対して正式な立場を表明しませんでした。',
      ...
      body2: {
        ...
        content: [
          '**ユタ州の法律はUL 3700認証を要件としていますが、HB 340が可決された時点ではこの規格自体が存在していませんでした — UL Solutionsが規格を公表したのは法施行から8か月後の2025年12月で、認証試験の開始は2026年1月からでした。** 執筆時点で、プラグイン式ソーラーシステムでUL 3700の完全認証を取得したと確認されている製品はありません。EcoFlowのSTREAM Ultra（1.92kWh、1,200W、定価$1,279、LFPバッテリー10年保証）はユタ州向けに販売されていますが、完全に合法な利用は認証の完了に左右されます — 「UL 3700準拠」として販売されている製品でも認証プロセスが完了しているとは限らないため、具体的な表示内容を必ず確認してください。',
  ...
  zh: {
    ...
    sections: {
      ...
      body1: {
        ...
        content: [
          '**犹他州HB 340法案将"小型便携式太阳能发电设备"定义为输出功率不超过1,200瓦、通过标准120V交流插座连接、并经国家认可检测实验室认证的设备。** 符合该定义的设备可免除电力公司并网协议、审批要求和费用——完全无需向电力公司提交任何通知。代价是：该设备不能参与净计量电价，任何输出到电网的多余电力都不会获得补偿。该州最大的电力公司Rocky Mountain Power在州众议员Raymond Ward与其就安全条款直接沟通后，未对该法案表态。',
      ...
      body2: {
        title: '认证套件真的已经上市了吗？',
        content: [
          '**犹他州法律要求UL 3700认证，但HB 340通过时该标准尚不存在——UL Solutions直到2025年12月才发布该标准，比法律生效晚了八个月，认证测试则到2026年1月才启动。** 截至撰写本文时，尚无任何完整的插电式太阳能系统确认已完成完整的UL 3700认证。EcoFlow的STREAM Ultra（1.92千瓦时、1,200瓦、标价1,279美元、10年LFP电池质保）专门面向犹他州市场销售，但完全合法的使用仍取决于该认证的完成情况——不要假定标注"符合UL 3700"的产品已完成认证流程，请核实具体标注内容。',
