...
export async function buildArticleMetadata(slug: string, lang: Language): Promise<Metadata> {
  ...
  const ogImageUrl = post.heroImage || `https://www.promptquorum.com/api/og/blog/${slug}?lang=${lang}`
...
export async function buildHubMetadata(lang: Language): Promise<Metadata> {
...
export async function buildHubPageElement(lang: Language) {
  const jsonLd = {
    '@context': 'https://schema.org',
    '@graph': [
      {
  ...
  return (
    <>
      ...
      <BlogIndexClient initialLang={lang} />
    </>
  )
}
