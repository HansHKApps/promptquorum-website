    ...
    sections: {
      ...
      howPQImplements: {
        ...
        content: [
          ...
          'Wenn Sie einen RISEN-Prompt mit PromptQuorum ausführen, werden die Ergebnisse an mehreren KI-Modellen (GPT-5.6, Claude Opus 5, Gemini 3.1 Pro) parallel getestet. Sie sehen sofort, wie verschiedene Modelle auf dieselbe RISEN-Struktur reagieren. Dies ist unbezahlbar, wenn Sie einen Workflow auf Robustheit prüfen.',
    ...
    sections: {
      ...
      howPQImplements: {
        ...
        content: [
          ...
          'Lorsque vous exécutez un prompt RISEN avec PromptQuorum, les résultats sont testés en parallèle sur plusieurs modèles IA (GPT-5.6, Claude Opus 5, Gemini 3.1 Pro). Vous voyez immédiatement comment différents modèles répondent à la même structure RISEN. Ceci est inestimable lorsque vous testez la robustesse d\'un workflow.',
    ...
    sections: {
      ...
      howPQImplements: {
        ...
        content: [
          ...
          'PromptQuorumでRISENプロンプトを実行すると、複数のAIモデル（GPT-5.6、Claude Opus 5、Gemini 3.1 Pro）で結果が並列にテストされます。異なるモデルが同じRISEN構造にどのように応答するかをすぐに見ることができます。これはワークフローの堅牢性をテストするときに非常に重要です。',
    ...
    sections: {
      ...
      howPQImplements: {
        ...
        content: [
          ...
          '当您使用PromptQuorum运行RISEN提示时，结果会在多个AI模型（GPT-5.6、Claude Opus 5、Gemini 3.1 Pro）上并行测试。您可以立即看到不同的模型如何对相同的RISEN结构做出响应。当您测试工作流的鲁棒性时，这是无价的。',
