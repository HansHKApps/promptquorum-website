...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `https://www.promptquorum.com/ko/enterprise-ai/${slug}`
...
export default async function KoEnterpriseAiArticlePage({ params }: PageProps) {
  const { slug } = await params
  const key = ENTERPRISE_AI_SLUG_TO_KEY[slug]
  if (!key) notFound()
  ...
  if (!enterpriseAiContent[key]['ko'] && !enterpriseAiContent[key]['en']) notFound()

  return (
    <>
      <EnterpriseAiArticleJsonLd slug={slug} articleKey={key} lang="ko" />
      <EnterpriseAiPostClient
        slug={slug}
        initialLang="ko"
        {...narrowArticleData(enterpriseAiContent[key], "ko")}
      />
    </>
  )
}
