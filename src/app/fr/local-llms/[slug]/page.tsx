...
export default async function FrLocalLLMsArticlePage({ params }: PageProps) {
  const { slug } = await params
  const key = LLM_SLUG_TO_KEY[slug]
  ...
  if (!llmContent[key]['fr'] && !llmContent[key]['en']) notFound()

  return (
    <>
      <LocalLLMArticleJsonLd slug={slug} articleKey={key} lang="fr" />
      <LocalLLMsPostClient
        slug={slug}
        initialLang="fr"
        {...narrowArticleData(llmContent[key], "fr")}
      />
    </>
  )
}
