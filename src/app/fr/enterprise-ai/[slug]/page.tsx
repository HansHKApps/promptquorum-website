...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `https://www.promptquorum.com/fr/enterprise-ai/${slug}`
...
export default async function FrEnterpriseAiArticlePage({ params }: PageProps) {
  const { slug } = await params
  const key = ENTERPRISE_AI_SLUG_TO_KEY[slug]
  if (!key) notFound()
  ...
  if (!enterpriseAiContent[key]['fr'] && !enterpriseAiContent[key]['en']) notFound()

  return (
    <>
      <EnterpriseAiArticleJsonLd slug={slug} articleKey={key} lang="fr" />
      <EnterpriseAiPostClient
        slug={slug}
        initialLang="fr"
        {...narrowArticleData(enterpriseAiContent[key], "fr")}
      />
    </>
  )
}
