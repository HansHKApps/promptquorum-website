...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `https://www.promptquorum.com/fr/prompt-engineering/${slug}`
...
export default async function FrPromptEngineeringArticlePage({ params }: PageProps) {
  const { slug } = await params
  const key = PE_SLUG_TO_KEY[slug]

  if (!key) notFound()

  const article = peContent[key]['fr'] ?? peContent[key]['en']
  if (!article) notFound()

  return (
    <>
      <PEArticleJsonLd slug={slug} articleKey={key} lang="fr" />
      <PromptEngineeringPostClient
        slug={slug}
        initialLang="fr"
        {...narrowArticleData(peContent[key], "fr")}
      />
    </>
  )
}
