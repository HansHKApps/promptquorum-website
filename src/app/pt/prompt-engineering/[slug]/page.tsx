...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `https://www.promptquorum.com/pt/prompt-engineering/${slug}`
...
export default async function EsPromptEngineeringArticlePage({ params }: PageProps) {
  ...
  const article = peContent[key]['pt'] ?? peContent[key]['en']
  if (!article) notFound()

  return (
    <>
      <PEArticleJsonLd slug={slug} articleKey={key} lang="pt" />
      <PromptEngineeringPostClient
        slug={slug}
        initialLang="pt"
        {...narrowArticleData(peContent[key], "pt")}
      />
    </>
  )
}
