...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `https://www.promptquorum.com/pt/local-llms/${slug}`
...
export default async function EsLocalLLMsArticlePage({ params }: PageProps) {
  ...
  if (!llmContent[key]['pt'] && !llmContent[key]['en']) notFound()

  return (
    <>
      <LocalLLMArticleJsonLd slug={slug} articleKey={key} lang="pt" />
      <LocalLLMsPostClient
        slug={slug}
        initialLang="pt"
