...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `https://www.promptquorum.com/pt/enterprise-ai/${slug}`
...
export default async function PtEnterpriseAiArticlePage({ params }: PageProps) {
  const { slug } = await params
  const key = ENTERPRISE_AI_SLUG_TO_KEY[slug]
  if (!key) notFound()
  ...
  if (!enterpriseAiContent[key]['pt'] && !enterpriseAiContent[key]['en']) notFound()

  return (
    <>
      <EnterpriseAiArticleJsonLd slug={slug} articleKey={key} lang="pt" />
      <EnterpriseAiPostClient
        slug={slug}
        initialLang="pt"
        {...narrowArticleData(enterpriseAiContent[key], "pt")}
      />
    </>
  )
}
