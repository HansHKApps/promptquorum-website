...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `https://www.promptquorum.com/ja/local-llms/${slug}`
  ...
  return {
    ...
    alternates: generateAlternates(`/local-llms/${slug}`, lang, hasTranslation, availableLangsForMeta, ['ja']),
    openGraph: {
      title: pageTitle,
      description: metaDesc,
      url: canonicalUrl,
...
export default async function JaLocalLLMsArticlePage({ params }: PageProps) {
  const { slug } = await params
  const key = LLM_SLUG_TO_KEY[slug]
  ...
  // Schema for Japanese breadcrumbs
