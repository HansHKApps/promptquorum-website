...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `https://www.promptquorum.com/ja/enterprise-ai/${slug}`
...
export default async function JaEnterpriseAiArticlePage({ params }: PageProps) {
  const { slug } = await params
  const key = ENTERPRISE_AI_SLUG_TO_KEY[slug]
  if (!key) notFound()
  ...
  if (!enterpriseAiContent[key]['ja'] && !enterpriseAiContent[key]['en']) notFound()

  return (
    <>
      <EnterpriseAiArticleJsonLd slug={slug} articleKey={key} lang="ja" />
      <EnterpriseAiPostClient
        slug={slug}
        initialLang="ja"
        {...narrowArticleData(enterpriseAiContent[key], "ja")}
      />
    </>
  )
}
