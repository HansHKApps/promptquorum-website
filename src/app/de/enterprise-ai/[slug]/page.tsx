...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `https://www.promptquorum.com/de/enterprise-ai/${slug}`
...
export default async function DeEnterpriseAiArticlePage({ params }: PageProps) {
  const { slug } = await params
  const key = ENTERPRISE_AI_SLUG_TO_KEY[slug]
  if (!key) notFound()
  ...
  if (!enterpriseAiContent[key]['de'] && !enterpriseAiContent[key]['en']) notFound()

  return (
    <>
      <EnterpriseAiArticleJsonLd slug={slug} articleKey={key} lang="de" />
      <EnterpriseAiPostClient
        slug={slug}
        initialLang="de"
        {...narrowArticleData(enterpriseAiContent[key], "de")}
      />
    </>
  )
}
