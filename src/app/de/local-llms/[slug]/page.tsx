...
export default async function DeLocalLLMsArticlePage({ params }: PageProps) {
  const { slug } = await params
  const key = LLM_SLUG_TO_KEY[slug]
  ...
  if (!llmContent[key]['de'] && !llmContent[key]['en']) notFound()

  return (
    <>
      <LocalLLMArticleJsonLd slug={slug} articleKey={key} lang="de" />
      <LocalLLMsPostClient
        slug={slug}
        initialLang="de"
        {...narrowArticleData(llmContent[key], "de")}
      />
    </>
  )
}
