...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `https://www.promptquorum.com/de/prompt-engineering/${slug}`
  ...
  return {
    ...
    alternates: generateAlternates(`/prompt-engineering/${slug}`, lang, hasTranslation, availableLangsForMeta, ['ja', 'zh', 'de']),
    openGraph: {
      title: pageTitle,
      description: metaDesc,
      url: canonicalUrl,
...
export default async function DePromptEngineeringArticlePage({ params }: PageProps) {
  const { slug } = await params
  const key = PE_SLUG_TO_KEY[slug]

  if (!key) notFound()

  const article = peContent[key]['ja'] ?? peContent[key]['en']
  if (!article) notFound()
