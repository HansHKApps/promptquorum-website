...
export default async function ArFrameworkPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const fw = getFrameworkLocalized(slug, LANG)
  if (!fw) notFound()
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify([
            ...
            },
            {
              ...
              '@type': 'BreadcrumbList',
              'itemListElement': [
                { '@type': 'ListItem', 'position': 1, 'name': labels.home, 'item': 'https://www.promptquorum.com/ar' },
                { '@type': 'ListItem', 'position': 2, 'name': labels.frameworks, 'item': 'https://www.promptquorum.com/ar/frameworks' },
      ...
      <div className="min-h-screen bg-surface pt-24 pb-20" dir="rtl">
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          ...
          <nav className="flex items-center gap-2 text-xs text-text-muted mb-8">
            <Link href="/ar" className="hover:text-primary transition-colors">{labels.home}</Link>
            <span>/</span>
            <Link href="/ar/frameworks" className="hover:text-primary transition-colors">{labels.frameworks}</Link>
            <span>/</span>
          ...
          <div className="mb-12">
            ...
            <p className="text-lg text-primary font-mono mb-4" dir="ltr">{fw.expansion}</p>
            ...
            <div className="space-y-4">
              {fw.fields.map((field, i) => (
                <div key={field.name} className="flex gap-4 bg-card border border-primary/20 rounded-xl p-5">
                  ...
                  <div>
                    <h3 className="font-bold text-text-primary mb-1" dir="ltr">{field.name}</h3>
          ...
          <section className="mb-12">
            ...
            <div className="bg-card border border-primary/30 rounded-xl p-6">
              ...
              <p className="text-text-secondary leading-relaxed font-mono text-sm whitespace-pre-wrap" dir="ltr">{fw.example.prompt}</p>
            </div>
          </section>
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                {related.map(r => (
                  ...
                  >
                    ...
                    <div className="text-xs text-text-muted" dir="ltr">{r.expansion}</div>
                  </Link>
                ))}
              </div>
            </section>
          ...
          <div className="flex flex-wrap gap-4 pt-8 border-t border-primary/20">
            <Link href="/ar" className="text-primary hover:text-primary/80 font-medium text-sm">← {labels.home}</Link>
            <Link href="/ar/frameworks" className="text-primary hover:text-primary/80 font-medium text-sm">{labels.allFrameworks}</Link>
          </div>
