...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `https://www.promptquorum.com/ar/enterprise-ai/${slug}`
...
export default async function ArEnterpriseAiArticlePage({ params }: PageProps) {
  const { slug } = await params
  const key = ENTERPRISE_AI_SLUG_TO_KEY[slug]
  if (!key) notFound()
  ...
  if (!enterpriseAiContent[key]['ar'] && !enterpriseAiContent[key]['en']) notFound()

  return (
    <>
      <EnterpriseAiArticleJsonLd slug={slug} articleKey={key} lang="ar" />
      <EnterpriseAiPostClient
        slug={slug}
        initialLang="ar"
        {...narrowArticleData(enterpriseAiContent[key], "ar")}
      />
    </>
  )
}
