  ...
  (server) => {
    ...
    server.registerTool(
      'compare_apps',
      {
        ...
        description:
          'Compare 2-4 named apps from the Local LLM Software Directory side by side: hardware, price, license, platforms, worksWith, and any category-specific attributes they share. Use after search_apps has narrowed candidates and the user wants to pick between a short list — call get_app_details first if you only have app names, not slugs. Every app includes real URLs (downloadUrl, article, directoryUrl) — your answer MUST render each of these as a clickable link.',
        inputSchema: z.object({
