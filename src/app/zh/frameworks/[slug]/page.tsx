...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      type: 'article',
      url: `https://www.promptquorum.com/zh/frameworks/${fw.slug}`,
...
export default async function ZhFrameworkPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const fw = getFrameworkLocalized(slug, LANG)
  if (!fw) notFound()
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify([
            ...
            },
            {
              ...
              '@type': 'BreadcrumbList',
              'itemListElement': [
                { '@type': 'ListItem', 'position': 1, 'name': labels.home, 'item': 'https://www.promptquorum.com/zh' },
                { '@type': 'ListItem', 'position': 2, 'name': labels.frameworks, 'item': 'https://www.promptquorum.com/zh/frameworks' },
      ...
      <div className="min-h-screen bg-surface pt-24 pb-20">
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          ...
          <nav className="flex items-center gap-2 text-xs text-text-muted mb-8">
            <Link href="/zh" className="hover:text-primary transition-colors">{labels.home}</Link>
            <span>/</span>
            <Link href="/zh/frameworks" className="hover:text-primary transition-colors">{labels.frameworks}</Link>
            <span>/</span>
          ...
          <div className="flex flex-wrap gap-4 pt-8 border-t border-primary/20">
            <Link href="/zh" className="text-primary hover:text-primary/80 font-medium text-sm">← {labels.home}</Link>
            <Link href="/zh/frameworks" className="text-primary hover:text-primary/80 font-medium text-sm">{labels.allFrameworks}</Link>
          </div>
