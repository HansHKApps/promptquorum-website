...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `https://www.promptquorum.com/zh/enterprise-ai/${slug}`
...
export default async function ZhEnterpriseAiArticlePage({ params }: PageProps) {
  const { slug } = await params
  const key = ENTERPRISE_AI_SLUG_TO_KEY[slug]
  if (!key) notFound()
  ...
  if (!enterpriseAiContent[key]['zh'] && !enterpriseAiContent[key]['en']) notFound()

  return (
    <>
      <EnterpriseAiArticleJsonLd slug={slug} articleKey={key} lang="zh" />
      <EnterpriseAiPostClient
        slug={slug}
        initialLang="zh"
        {...narrowArticleData(enterpriseAiContent[key], "zh")}
      />
    </>
  )
}
