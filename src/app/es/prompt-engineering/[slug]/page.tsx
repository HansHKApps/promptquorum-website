...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `https://www.promptquorum.com/es/prompt-engineering/${slug}`
...
export default async function EsPromptEngineeringArticlePage({ params }: PageProps) {
  ...
  return (
    <>
      <PEArticleJsonLd slug={slug} articleKey={key} lang="es" />
      <PromptEngineeringPostClient
        slug={slug}
        initialLang="es"
        {...narrowArticleData(peContent[key], "es")}
      />
    </>
  )
}
