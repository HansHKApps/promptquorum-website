...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const availableLangsForMeta = VALID_LANGS_META.filter(l => {
    const c = enterpriseAiContent[key]?.[l]
    return Boolean(c) && Object.keys(c?.sections ?? {}).length > 0
  })

  const canonicalUrl = `https://www.promptquorum.com/es/enterprise-ai/${slug}`
  const ogImageUrl = `https://www.promptquorum.com/api/og/${slug}?lang=${lang}`
  const hasTranslation = Boolean(enterpriseAiContent[key][lang]) && Object.keys(enterpriseAiContent[key][lang]?.sections ?? {}).length > 0
  ...
  return {
    ...
    alternates: generateAlternates(`/enterprise-ai/${slug}`, lang, hasTranslation, availableLangsForMeta, [...PATH_PREFIX_LANGS]),
    openGraph: {
      title: pageTitle,
      description: metaDesc,
      url: canonicalUrl,
...
export default async function EsEnterpriseAiArticlePage({ params }: PageProps) {
  const { slug } = await params
  const key = ENTERPRISE_AI_SLUG_TO_KEY[slug]
  if (!key) notFound()

  if (!enterpriseAiContent[key]) {
    return (
      <div className="min-h-screen bg-white pt-32 pb-20">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          <h1 className="text-3xl font-bold mb-4">Coming Soon</h1>
          <p className="text-gray-600 mb-8">This article is coming soon. Explore our Enterprise AI hub in the meantime.</p>
        </div>
      </div>
    )
  }

  if (!enterpriseAiContent[key]['es'] && !enterpriseAiContent[key]['en']) notFound()

  return (
    <>
      <EnterpriseAiArticleJsonLd slug={slug} articleKey={key} lang="es" />
      <EnterpriseAiPostClient
        slug={slug}
        initialLang="es"
        {...narrowArticleData(enterpriseAiContent[key], "es")}
      />
    </>
  )
}
