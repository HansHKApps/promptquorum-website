#!/usr/bin/env node
// One-off generator for the microsoft-agent-framework-review article hero
// images (9 locales). Extracts each locale's real tldr bullets + title
// straight out of the article source (not re-authored copy) and posts them
// to /api/hero-image, saving the result to
// public/images/microsoft-agent-framework-review-hero-{lang}.png.
// Usage: node scripts/gen-microsoft-agent-framework-review-heroes.mjs
// (requires the dev server running on the port below).

import fs from 'node:fs/promises'

const BASE = 'http://localhost:3499'
const ARTICLE_PATH = 'src/lib/power-local-llm/articles/microsoft-agent-framework-review.ts'
...
async function generate(lang, spec) {
  ...
  const outPath = `public/images/microsoft-agent-framework-review-hero-${lang}.png`
