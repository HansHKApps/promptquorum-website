#!/usr/bin/env node
// One-off generator for the semantic-kernel-review article hero images (9 locales).
...
// saving the result to public/images/semantic-kernel-review-hero-{lang}.png.
// Usage: node scripts/gen-semantic-kernel-review-heroes.mjs (requires the dev
// server running on the port below).

import fs from 'node:fs/promises'

const BASE = 'http://localhost:3416'
const ARTICLE_PATH = 'src/lib/power-local-llm/articles/semantic-kernel-review.ts'
...
async function generate(lang, spec) {
  ...
  const outPath = `public/images/semantic-kernel-review-hero-${lang}.png`
