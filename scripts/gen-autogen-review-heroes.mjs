#!/usr/bin/env node
// One-off generator for the autogen-review article hero images (9 locales).
...
// saving the result to public/images/autogen-review-hero-{lang}.png.
// Usage: node scripts/gen-autogen-review-heroes.mjs (requires the dev server
// running on the port below).

import fs from 'node:fs/promises'

const BASE = 'http://localhost:3499'
const ARTICLE_PATH = 'src/lib/power-local-llm/articles/autogen-review.ts'
...
function extractBlock(src, lang) {
  ...
  // Find the matching closing "  },\n" for this top-level lang block by
  // brace counting from the opening "{".
...
async function generate(lang, spec) {
  ...
  const outPath = `public/images/autogen-review-hero-${lang}.png`
